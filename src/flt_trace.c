  ...
  do {                                                                         \
    if (!(conf->flags & TRACE_F_QUIET))                                        \
      fprintf(stderr, "%d.%06d [%-20s] " fmt "\n", (int)now.tv_sec,            \
              (int)now.tv_usec, (conf)->name, ##__VA_ARGS__);                  \
  } while (0)
  ...
  do {                                                                         \
    if (!(conf->flags & TRACE_F_QUIET))                                        \
      fprintf(stderr, "%d.%06d [%-20s] [strm %p(%x) 0x%08x 0x%08x] " fmt "\n", \
              (int)now.tv_sec, (int)now.tv_usec, (conf)->name, strm,           \
              (strm ? ((struct stream *)strm)->uniq_id : ~0U),                 \
              (strm ? strm->req.analysers : 0),                                \
              (strm ? strm->res.analysers : 0), ##__VA_ARGS__);                \
  } while (0)

static const char *channel_label(const struct channel *chn) {
  return (chn->flags & CF_ISRESP) ? "RESPONSE" : "REQUEST";
}

static const char *proxy_mode(const struct stream *s) {
  struct proxy *px = (s->flags & SF_BE_ASSIGNED ? s->be : strm_fe(s));

  return ((px->mode == PR_MODE_HTTP) ? "HTTP" : "TCP");
}

static const char *stream_pos(const struct stream *s) {
  return (s->flags & SF_BE_ASSIGNED) ? "backend" : "frontend";
}

static const char *filter_type(const struct filter *f) {
  return (f->flags & FLT_FL_IS_BACKEND_FILTER) ? "backend" : "frontend";
}

static void trace_hexdump(struct ist ist) {
  int i, j, padding;
  ...
  for (i = 0; i < ist.len + padding; i++) {
    ...
    if (i % 16 == 15) {
      ...
      for (j = i - 15; j <= i && j < ist.len; j++)
        fprintf(
            stderr, "%c",
            (isprint((unsigned char)*(ist.ptr + j)) ? *(ist.ptr + j) : '.'));
      fprintf(stderr, "|\n");
    }
  }
}

static void trace_raw_hexdump(struct buffer *buf, unsigned int offset,
                              unsigned int len) {
  unsigned char p[len];
  int block1, block2;
...
static void trace_htx_hexdump(struct htx *htx, unsigned int offset,
                              unsigned int len) {
  struct htx_blk *blk;

  for (blk = htx_get_first_blk(htx); blk && len;
...
static unsigned int trace_get_htx_datalen(struct htx *htx, unsigned int offset,
                                          unsigned int len) {
  struct htx_blk *blk;
...
static int trace_init(struct proxy *px, struct flt_conf *fconf) {
...
static void trace_deinit(struct proxy *px, struct flt_conf *fconf) {
  struct trace_config *conf = fconf->conf;
...
static int trace_check(struct proxy *px, struct flt_conf *fconf) { return 0; }

/* Initialize the filter for each thread. Return -1 on error, else 0. */
static int trace_init_per_thread(struct proxy *px, struct flt_conf *fconf) {
  struct trace_config *conf = fconf->conf;
...
static void trace_deinit_per_thread(struct proxy *px, struct flt_conf *fconf) {
  struct trace_config *conf = fconf->conf;
...
static int trace_attach(struct stream *s, struct filter *filter) {
  struct trace_config *conf = FLT_CONF(filter);

  FLT_STRM_TRACE(conf, s, "%-25s: filter-type=%s", __FUNCTION__,
                 filter_type(filter));
 ...
 * destruction */
static void trace_detach(struct stream *s, struct filter *filter) {
  struct trace_config *conf = FLT_CONF(filter);
...
static int trace_stream_start(struct stream *s, struct filter *filter) {
  struct trace_config *conf = FLT_CONF(filter);

  FLT_STRM_TRACE(conf, s, "%-25s", __FUNCTION__);
  return 0;
}

/* Called when a backend is set for a stream */
static int trace_stream_set_backend(struct stream *s, struct filter *filter,
                                    struct proxy *be) {
  struct trace_config *conf = FLT_CONF(filter);

  FLT_STRM_TRACE(conf, s, "%-25s: backend=%s", __FUNCTION__, be->id);
  return 0;
}

/* Called when a stream is destroyed */
static void trace_stream_stop(struct stream *s, struct filter *filter) {
  struct trace_config *conf = FLT_CONF(filter);
...
static void trace_check_timeouts(struct stream *s, struct filter *filter) {
  struct trace_config *conf = FLT_CONF(filter);
...
static int trace_chn_start_analyze(struct stream *s, struct filter *filter,
                                   struct channel *chn) {
  struct trace_config *conf = FLT_CONF(filter);

  FLT_STRM_TRACE(conf, s, "%-25s: channel=%-10s - mode=%-5s (%s)", __FUNCTION__,
...
static int trace_chn_analyze(struct stream *s, struct filter *filter,
  ...
  FLT_STRM_TRACE(conf, s,
                 "%-25s: channel=%-10s - mode=%-5s (%s) - "
                 "analyzer=%s - step=%s",
                 __FUNCTION__, channel_label(chn), proxy_mode(s), stream_pos(s),
                 ana, ((chn->analysers & an_bit) ? "PRE" : "POST"));
  return 1;
}

/* Called when analyze ends for a given channel */
static int trace_chn_end_analyze(struct stream *s, struct filter *filter,
                                 struct channel *chn) {
  struct trace_config *conf = FLT_CONF(filter);
 ...
 *************************************************************************/
static int trace_http_headers(struct stream *s, struct filter *filter,
                              struct http_msg *msg) {
  struct trace_config *conf = FLT_CONF(filter);
  ...
  FLT_STRM_TRACE(
      conf, s, "%-25s: channel=%-10s - mode=%-5s (%s)\t%.*s %.*s %.*s",
      __FUNCTION__, channel_label(msg->chn), proxy_mode(s), stream_pos(s),
      HTX_SL_P1_LEN(sl), HTX_SL_P1_PTR(sl), HTX_SL_P2_LEN(sl),
      HTX_SL_P2_PTR(sl), HTX_SL_P3_LEN(sl), HTX_SL_P3_PTR(sl));
  ...
  for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
    ...
    v = htx_get_blk_value(htx, blk);
    FLT_STRM_TRACE(conf, s, "\t%.*s: %.*s", (int)n.len, n.ptr, (int)v.len,
                   v.ptr);
  }
  return 1;
}

static int trace_http_payload(struct stream *s, struct filter *filter,
  ...
  if (ret && (conf->flags & TRACE_F_RAND_FWD)) {
    unsigned int data =
        trace_get_htx_datalen(htxbuf(&msg->chn->buf), offset, len);
  ...
  FLT_STRM_TRACE(conf, s,
                 ...
                 __FUNCTION__, channel_label(msg->chn), proxy_mode(s),
                 stream_pos(s), offset, len, ret);
...
static int trace_http_end(struct stream *s, struct filter *filter,
                          struct http_msg *msg) {
  struct trace_config *conf = FLT_CONF(filter);
...
static void trace_http_reset(struct stream *s, struct filter *filter,
                             struct http_msg *msg) {
  struct trace_config *conf = FLT_CONF(filter);
...
static void trace_http_reply(struct stream *s, struct filter *filter,
                             short status, const struct buffer *msg) {
  struct trace_config *conf = FLT_CONF(filter);

  FLT_STRM_TRACE(conf, s, "%-25s: channel=%-10s - mode=%-5s (%s)", __FUNCTION__,
                 "-", proxy_mode(s), stream_pos(s));
}

/**************************************************************************
 * Hooks to filter TCP data
 *************************************************************************/
static int trace_tcp_payload(struct stream *s, struct filter *filter,
  ...
  if (s->flags & SF_HTX) {
    ...
    FLT_STRM_TRACE(conf, s,
                   "%-25s: channel=%-10s - mode=%-5s (%s) - "
                   "offset=%u - len=%u - forward=%d",
                   __FUNCTION__, channel_label(chn), proxy_mode(s),
                   stream_pos(s), offset, len, ret);
...
/* Return -1 on error, else 0 */
static int parse_trace_flt(char **args, int *cur_arg, struct proxy *px,
