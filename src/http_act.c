 ...
 */
static int check_act_http_reply(struct act_rule *rule, struct proxy *px,
                                char **err) {
 ...
 */
static enum act_return http_action_set_req_line(struct act_rule *rule,
                                                struct proxy *px,
                                                struct session *sess,
  ...
  replace->data +=
      build_logline(s, replace->area + replace->data,
                    replace->size - replace->data, &rule->arg.http.fmt);

  if (http_req_replace_stline(rule->action, replace->area, replace->data, px,
                              s) == -1)
    goto fail_rewrite;
 ...
 *   set-uri
 *
 * All of them accept a single argument of type string representing a
 * log-format. The resulting rule makes use of <http.fmt> to store the
 * log-format list head, and <.action> to store the action type as an int
 * (0=method, 1=path, 2=query, 3=uri). It returns ACT_RET_PRS_OK on success,
 * ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_set_req_line(const char **args, int *orig_arg,
                                             struct proxy *px,
  ...
  if (!parse_logformat_string(
          args[cur_arg], px, &rule->arg.http.fmt, LOG_OPT_HTTP,
 ...
 * processing continue. Otherwsize ACT_RET_ERR is returned.
 */
static enum act_return http_action_replace_uri(struct act_rule *rule,
                                               struct proxy *px,
                                               struct session *sess,
                                               struct stream *s, int flags) {
  enum act_return ret = ACT_RET_CONT;
  ...
  if (!regex_exec_match2(rule->arg.http.re, uri.ptr, uri.len, MAX_MATCH, pmatch,
                         0))
    goto leave;

  replace->data =
      build_logline(s, replace->area, replace->size, &rule->arg.http.fmt);
 ...
 */
static enum act_parse_ret parse_replace_uri(const char **args, int *orig_arg,
                                            struct proxy *px,
  ...
  if (!*args[cur_arg] || !*args[cur_arg + 1] ||
    ...
    memprintf(err,
              "expects exactly 2 arguments <match-regex> and <replace-format>");
    return ACT_RET_PRS_ERR;
  }
  ...
  px->conf.args.ctx = ARGC_HRQ;
  if (!parse_logformat_string(
          args[cur_arg + 1], px, &rule->arg.http.fmt, LOG_OPT_HTTP,
          (px->cap & PR_CAP_FE) ? SMP_VAL_FE_HRQ_HDR : SMP_VAL_BE_HRQ_HDR,
          err)) {
    regex_free(rule->arg.http.re);
    return ACT_RET_PRS_ERR;
...
static enum act_return action_http_set_status(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
                                              struct stream *s, int flags) {
 ...
 * ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_set_status(const char **args,
  ...
  if (!*args[*orig_arg]) {
    memprintf(
        err,
        "expects 1 argument: <status>; or 3 arguments: <status> reason <fmt>");
    return ACT_RET_PRS_ERR;
  }
  ...
  rule->arg.http.str =
      ist(NULL); // If null, we use the default reason for the status code.
 ...
 */
static enum act_return http_action_reject(struct act_rule *rule,
                                          struct proxy *px,
                                          struct session *sess,
                                          struct stream *s, int flags) {
...
parse_http_action_reject(const char **args, int *orig_arg, struct proxy *px,
                         struct act_rule *rule, char **err) {
  rule->action = ACT_CUSTOM;
 ...
 * return ACT_RET_CONT;
 */
static enum act_return http_req_disable_l7_retry(struct act_rule *rule,
                                                 struct proxy *px,
                                                 struct session *sess,
                                                 struct stream *s, int flags) {
 ...
 */
static enum act_return http_action_req_capture(struct act_rule *rule,
                                               struct proxy *px,
                                               struct session *sess,
                                               struct stream *s, int flags) {
  struct sample *key;
  ...
  key = sample_fetch_as_type(s->be, sess, s, SMP_OPT_DIR_REQ | SMP_OPT_FINAL,
                             rule->arg.cap.expr, SMP_T_STR);
  if (!key)
    return ACT_RET_CONT;
 ...
 */
static enum act_return http_action_req_capture_by_id(struct act_rule *rule,
                                                     struct proxy *px,
                                                     struct session *sess,
                                                     struct stream *s,
                                                     int flags) {
  ...
  for (h = fe->req_cap, i = fe->nb_req_cap - 1;
       h != NULL && i != rule->arg.capid.idx; i--, h = h->next)
    ;
  if (!h)
    return ACT_RET_CONT;

  key = sample_fetch_as_type(s->be, sess, s, SMP_OPT_DIR_REQ | SMP_OPT_FINAL,
                             rule->arg.capid.expr, SMP_T_STR);
  if (!key)
    return ACT_RET_CONT;
 ...
 * filled.
 */
static int check_http_req_capture(struct act_rule *rule, struct proxy *px,
                                  char **err) {
  ...
  if (px->cap & PR_CAP_FE && rule->arg.capid.idx >= px->nb_req_cap) {
    memprintf(err,
              "unable to find capture id '%d' referenced by http-request "
              "capture rule",
              rule->arg.capid.idx);
    return 0;
  }
 ...
 */
static enum act_parse_ret
parse_http_req_capture(const char **args, int *orig_arg, struct proxy *px,
                       struct act_rule *rule, char **err) {
  ...
  for (cur_arg = *orig_arg; cur_arg < *orig_arg + 3 && *args[cur_arg];
       cur_arg++)
  ...
  if (!(expr->fetch->val & SMP_VAL_FE_HRQ_HDR)) {
    memprintf(err,
              "fetch method '%s' extracts information from '%s', none of which "
              "is available here",
 ...
 * error occurs the action is cancelled, but the rule processing continues.
 */
static enum act_return http_action_res_capture_by_id(struct act_rule *rule,
                                                     struct proxy *px,
                                                     struct session *sess,
                                                     struct stream *s,
                                                     int flags) {
  ...
  key = sample_fetch_as_type(s->be, sess, s, SMP_OPT_DIR_RES | SMP_OPT_FINAL,
                             rule->arg.capid.expr, SMP_T_STR);
  if (!key)
    return ACT_RET_CONT;
 ...
 * filled.
 */
static int check_http_res_capture(struct act_rule *rule, struct proxy *px,
                                  char **err) {
  ...
  if (px->cap & PR_CAP_FE && rule->arg.capid.idx >= px->nb_rsp_cap) {
    memprintf(err,
              "unable to find capture id '%d' referenced by http-response "
              "capture rule",
              rule->arg.capid.idx);
    return 0;
  }
 ...
 */
static enum act_parse_ret
parse_http_res_capture(const char **args, int *orig_arg, struct proxy *px,
                       struct act_rule *rule, char **err) {
  struct sample_expr *expr;
  int cur_arg;
  int id;
...
/* Parse a "allow" action for a request or a response rule. It takes no
 * argument. It returns ACT_RET_PRS_OK on success, ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_allow(const char **args, int *orig_arg,
                                           struct proxy *px,
                                           struct act_rule *rule, char **err) {
 ...
 * <.arg.http_reply>.
 */
static enum act_parse_ret parse_http_deny(const char **args, int *orig_arg,
                                          struct proxy *px,
                                          struct act_rule *rule, char **err) {
  int default_status;
  int cur_arg, arg = 0;
  ...
  if (!*(args[cur_arg]) || strcmp(args[cur_arg], "if") == 0 ||
      strcmp(args[cur_arg], "unless") == 0) {
    rule->arg.http_reply =
        http_parse_http_reply((const char *[]){"default-errorfiles", ""}, &arg,
                              px, default_status, err);
    goto end;
  }

  if (strcmp(args[cur_arg], "deny_status") == 0) {
    if (!*(args[cur_arg + 2]) || strcmp(args[cur_arg + 2], "if") == 0 ||
        strcmp(args[cur_arg + 2], "unless") == 0) {
      rule->arg.http_reply =
          http_parse_http_reply((const char *[]){"status", args[cur_arg + 1],
                                                 "default-errorfiles", ""},
  ...
  rule->arg.http_reply =
      http_parse_http_reply(args, orig_arg, px, default_status, err);
 ...
 */
static enum act_parse_ret parse_http_auth(const char **args, int *orig_arg,
                                          struct proxy *px,
                                          struct act_rule *rule, char **err) {
  int cur_arg;
 ...
 */
static enum act_parse_ret parse_http_set_nice(const char **args, int *orig_arg,
                                              struct proxy *px,
                                              struct act_rule *rule,
                                              char **err) {
  int cur_arg;
 ...
 * ACT_RET_PRS_OK on success, ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_set_tos(const char **args, int *orig_arg,
                                             struct proxy *px,
                                             struct act_rule *rule,
                                             char **err) {
#ifdef IP_TOS
  ...
  if (endp && *endp != '\0') {
    memprintf(err,
              "invalid character starting at '%s' (integer/hex value expected)",
              endp);
    return ACT_RET_PRS_ERR;
  }
 ...
 * ACT_RET_PRS_OK on success, ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_set_mark(const char **args, int *orig_arg,
                                              struct proxy *px,
                                              struct act_rule *rule,
                                              char **err) {
#ifdef SO_MARK
 ...
 */
static enum act_parse_ret
parse_http_set_log_level(const char **args, int *orig_arg, struct proxy *px,
                         struct act_rule *rule, char **err) {
  int cur_arg;
 ...
 * returned.
 */
static enum act_return http_action_early_hint(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
                                              struct stream *s, int flags) {
  ...
  /* if no previous rule or previous rule is not early-hint, start a new
   * response. Otherwise,
   * continue to add link to a previously started response */
  if (&prev_rule->list == s->current_rule_list ||
      prev_rule->action_ptr != http_action_early_hint) {
    struct htx_sl *sl;
    unsigned int flags = (HTX_SL_F_IS_RESP | HTX_SL_F_VER_11 |
                          HTX_SL_F_XFER_LEN | HTX_SL_F_BODYLESS);

    sl = htx_add_stline(htx, HTX_BLK_RES_SL, flags, ist("HTTP/1.1"), ist("103"),
                        ist("Early Hints"));
    if (!sl)
      goto error;
  ...
  value->data =
      build_logline(s, b_tail(value), b_room(value), &rule->arg.http.fmt);
  if (!htx_add_header(htx, rule->arg.http.str,
                      ist2(b_head(value), b_data(value))))
    goto error;

  /* if it is the last rule or the next one is not an early-hint, terminate the
   * current response. */
  if (&next_rule->list == s->current_rule_list ||
      next_rule->action_ptr != http_action_early_hint) {
 ...
 */
static enum act_return http_action_set_header(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
                                              struct stream *s, int flags) {
  struct http_msg *msg =
      ((rule->from == ACT_F_HTTP_REQ) ? &s->txn->req : &s->txn->rsp);
 ...
 */
static enum act_parse_ret parse_http_set_header(const char **args,
                                                int *orig_arg, struct proxy *px,
                                                struct act_rule *rule,
                                                char **err) {
  int cap, cur_arg;
  ...
  cur_arg++;
  if (!parse_logformat_string(args[cur_arg], px, &rule->arg.http.fmt,
                              LOG_OPT_HTTP, cap, err)) {
    istfree(&rule->arg.http.str);
    return ACT_RET_PRS_ERR;
  }
 ...
 * http_action_replace_header(). On success, it returns ACT_RET_CONT. If an
 * error occurs while soft rewrites are enabled, the action is canceled, but the
 * rule processing continue. Otherwsize ACT_RET_ERR is returned.
 */
static enum act_return http_action_replace_header(struct act_rule *rule,
                                                  struct proxy *px,
                                                  struct session *sess,
                                                  struct stream *s, int flags) {
  struct http_msg *msg =
  ...
  r = http_replace_hdrs(s, htx, rule->arg.http.str, replace->area,
                        rule->arg.http.re, (rule->action == 0));
  if (r == -1)
    goto fail_rewrite;
 ...
 */
static enum act_parse_ret
parse_http_replace_header(const char **args, int *orig_arg, struct proxy *px,
                          struct act_rule *rule, char **err) {
  int cap, cur_arg;
 ...
 * header name. It finds the matching method to be performed in <.action>,
 * previously filled by function parse_http_del_header(). On success, it returns
 * ACT_RET_CONT. Otherwise ACT_RET_ERR is returned.
 */
static enum act_return http_action_del_header(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
                                              struct stream *s, int flags) {
  struct http_hdr_ctx ctx;
 ...
 * optional flag (currently only -m) and optional matching method of input
 * string with header name to be deleted. Default matching method is exact match
 * (-m str). It returns ACT_RET_PRS_OK on success, ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_del_header(const char **args,
                                                int *orig_arg, struct proxy *px,
                                                struct act_rule *rule,
                                                char **err) {
  int cur_arg;
 ...
 * ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_redirect(const char **args, int *orig_arg,
                                              struct proxy *px,
                                              struct act_rule *rule,
                                              char **err) {
  struct redirect_rule *redir;
  ...
  if ((redir = http_parse_redirect_rule(px->conf.args.file, px->conf.args.line,
                                        px, &args[cur_arg], err, 1, dir)) ==
      NULL)
    return ACT_RET_PRS_ERR;
 ...
 */
static enum act_return http_action_set_map(struct act_rule *rule,
                                           struct proxy *px,
                                           struct session *sess,
                                           struct stream *s, int flags) {
  struct pat_ref *ref;
  ...
  case 1: // set-map
    ...
    /* collect value */
    value->data =
        build_logline(s, value->area, value->size, &rule->arg.map.value);
 ...
 */
static enum act_parse_ret parse_http_set_map(const char **args, int *orig_arg,
                                             struct proxy *px,
                                             struct act_rule *rule,
                                             char **err) {
  int cap, cur_arg;
   ...
   */
  rule->arg.map.ref =
      my_strndup(args[cur_arg - 1] + 8, strlen(args[cur_arg - 1]) - 9);
  ...
  if (rule->action == 1) {
    ...
    if (!parse_logformat_string(args[cur_arg], px, &rule->arg.map.value,
                                LOG_OPT_HTTP, cap, err)) {
      free(rule->arg.map.ref);
      return ACT_RET_PRS_ERR;
    }
 ...
 */
static enum act_return http_action_track_sc(struct act_rule *rule,
                                            struct proxy *px,
                                            struct session *sess,
                                            struct stream *s, int flags) {
  struct stktable *t;
  ...
  opt = ((rule->from == ACT_F_HTTP_REQ) ? SMP_OPT_DIR_REQ : SMP_OPT_DIR_RES) |
        SMP_OPT_FINAL;

  t = rule->arg.trk_ctr.table.t;
  key =
      stktable_fetch_key(t, s->be, sess, s, opt, rule->arg.trk_ctr.expr, NULL);
 ...
 * ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret parse_http_track_sc(const char **args, int *orig_arg,
                                              struct proxy *px,
                                              struct act_rule *rule,
                                              char **err) {
  struct sample_expr *expr;
  ...
  if (cfg_parse_track_sc_num(&tsc_num, tsc_num_str,
                             tsc_num_str + strlen(tsc_num_str), err) == -1)
    return ACT_RET_PRS_ERR;
  ...
  where = 0;
  if (px->cap & PR_CAP_FE)
    where |= (rule->from == ACT_F_HTTP_REQ ? SMP_VAL_FE_HRQ_HDR
                                           : SMP_VAL_FE_HRS_HDR);
  if (px->cap & PR_CAP_BE)
    where |= (rule->from == ACT_F_HTTP_REQ ? SMP_VAL_BE_HRQ_HDR
                                           : SMP_VAL_BE_HRS_HDR);
...
static enum act_return action_timeout_set_stream_timeout(struct act_rule *rule,
  ...
  if (rule->arg.timeout.expr) {
    key = sample_fetch_as_type(px, sess, s, SMP_OPT_FINAL,
                               rule->arg.timeout.expr, SMP_T_SINT);
    if (!key)
      return ACT_RET_CONT;

    stream_set_timeout(s, rule->arg.timeout.type,
                       MS_TO_TICKS(key->data.u.sint));
  } else {
    stream_set_timeout(s, rule->arg.timeout.type,
                       MS_TO_TICKS(rule->arg.timeout.value));
  }
 ...
 */
static enum act_parse_ret
parse_http_set_timeout(const char **args, int *orig_arg, struct proxy *px,
                       struct act_rule *rule, char **err) {
  int cur_arg;
  ...
  if (cfg_parse_rule_set_timeout(
          args, cur_arg, &rule->arg.timeout.value, &rule->arg.timeout.type,
          &rule->arg.timeout.expr, err, px->conf.args.file, px->conf.args.line,
          &px->conf.args) == -1) {
    return ACT_RET_PRS_ERR;
  }
 ...
 * ACT_RET_CONT
 */
static enum act_return http_action_strict_mode(struct act_rule *rule,
                                               struct proxy *px,
                                               struct session *sess,
                                               struct stream *s, int flags) {
  struct http_msg *msg =
 ...
 */
static enum act_parse_ret
parse_http_strict_mode(const char **args, int *orig_arg, struct proxy *px,
                       struct act_rule *rule, char **err) {
  int cur_arg;
  ...
  if (strcasecmp(args[cur_arg], "on") == 0)
    ...
    memprintf(err, "Unexpected value '%s'. Only 'on' and 'off' are supported",
              args[cur_arg]);
    return ACT_RET_PRS_ERR;
  }
 ...
 * returned.
 */
static enum act_return http_action_return(struct act_rule *rule,
                                          struct proxy *px,
                                          struct session *sess,
                                          struct stream *s, int flags) {
  struct channel *req = &s->req;
 ...
 * <.arg.http_reply>.
 */
static enum act_parse_ret parse_http_return(const char **args, int *orig_arg,
                                            struct proxy *px,
                                            struct act_rule *rule, char **err) {
  /* Prepare parsing of log-format strings */
  px->conf.args.ctx = ((rule->from == ACT_F_HTTP_REQ) ? ARGC_HRQ : ARGC_HRS);
...
static struct action_kw_list http_req_actions = {
    .kw = {{"add-acl", parse_http_set_map, 1},
...
static struct action_kw_list http_after_res_actions = {
    .kw = {{"add-header", parse_http_set_header, 0},
           {"allow", parse_http_allow, 0},
           {"del-header", parse_http_del_header, 0},
           {"replace-header", parse_http_replace_header, 0},
           {"replace-value", parse_http_replace_header, 0},
