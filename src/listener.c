...
static struct task *manage_global_listener_queue(struct task *t, void *context,
                                                 unsigned short state);

#if defined(USE_THREAD)

struct accept_queue_ring accept_queue_rings[MAX_THREADS]
 ...
 * producers.
 */
int accept_queue_push_mp(struct accept_queue_ring *ring,
...
static struct task *accept_queue_process(struct task *t, void *context,
                                         unsigned short state) {
...
void dequeue_all_listeners() {
  ...
  while ((listener = MT_LIST_POP(&global_listener_queue, struct listener *,
                                 wait_queue))) {
...
void dequeue_proxy_listeners(struct proxy *px) {
  ...
  while ((listener = MT_LIST_POP(&px->listener_queue, struct listener *,
                                 wait_queue))) {
    /* This cannot fail because the listeners are by definition in
     * the LI_LIMITED state.
     */
 ...
 * returns non-zero on success, zero on error with the error message set in
 * <err>.
 */
int create_listeners(struct bind_conf *bc, const struct sockaddr_storage *ss,
                     int portl, int porth, int fd, struct protocol *proto,
                     char **err) {
  struct listener *l;
  int port;
...
/* This function is called on a read event from a listening socket,
 * corresponding to an accept. It tries to accept as many connections as
 * possible, and for each calls the listener's accept handler (generally the
 * frontend's accept handler).
 */
void listener_accept(struct listener *l) {
  ...
  if (!(l->options & LI_O_UNLIMITED) && global.sps_lim) {
    ...
    if (unlikely(!max)) {
      ...
      expire = tick_add(
          now_ms, next_event_delay(&global.sess_per_sec, global.sps_lim, 0));
      goto limit_global;
    }
  ...
  if (!(l->options & LI_O_UNLIMITED) && global.cps_lim) {
    ...
    if (unlikely(!max)) {
      /* frontend accept rate limit was reached */
      expire = tick_add(
          now_ms, next_event_delay(&global.conn_per_sec, global.cps_lim, 0));
      goto limit_global;
    }
  ...
  if (!(l->options & LI_O_UNLIMITED) && global.ssl_lim && l->bind_conf &&
    ...
    if (unlikely(!max)) {
      /* frontend accept rate limit was reached */
      expire = tick_add(
          now_ms, next_event_delay(&global.ssl_per_sec, global.ssl_lim, 0));
      goto limit_global;
    }
  ...
  if (p && p->fe_sps_lim) {
    ...
    if (unlikely(!max)) {
      /* frontend accept rate limit was reached */
      expire = tick_add(
          now_ms, next_event_delay(&p->fe_sess_per_sec, p->fe_sps_lim, 0));
      goto limit_proxy;
    }
  ...
  for (; max_accept; next_conn = next_feconn = next_actconn = 0, max_accept--) {
    ...
    if (unlikely(cli_conn->handle.fd >= global.maxsock)) {
      send_log(p, LOG_EMERG,
               "Proxy %s reached the configured maximum connection limit. "
               "Please check the global 'maxconn' value.\n",
               p->id);
    ...
    if (atleast2(mask) && (global.tune.options & GTUNE_LISTENER_MQ) &&
      ...
      do {
        ...
        q1 = accept_queue_rings[t1].tail - accept_queue_rings[t1].head +
             ACCEPT_QUEUE_SIZE;
        if (q1 >= ACCEPT_QUEUE_SIZE)
          q1 -= ACCEPT_QUEUE_SIZE;

        q2 = accept_queue_rings[t2].tail - accept_queue_rings[t2].head +
             ACCEPT_QUEUE_SIZE;
  ...
  if ((l->state == LI_FULL && (!l->maxconn || l->nbconn < l->maxconn)) ||
    ...
    if (p && !MT_LIST_ISEMPTY(&p->listener_queue) &&
        (!p->fe_sps_lim ||
         freq_ctr_remain(&p->fe_sess_per_sec, p->fe_sps_lim, 0) > 0))
...
void listener_release(struct listener *l) {
  ...
  if (!MT_LIST_ISEMPTY(&fe->listener_queue) &&
      (!fe->fe_sps_lim ||
       freq_ctr_remain(&fe->fe_sess_per_sec, fe->fe_sps_lim, 0) > 0))
...
REGISTER_CONFIG_POSTPARSER("multi-threaded listener queue",
                           listener_queue_init);
REGISTER_POST_DEINIT(listener_queue_deinit);

/* This is the global management task for listeners. It enables listeners
 * waiting for global resources when there are enough free resource, or at least
 * once in a while. It is designed to be called as a task.
 */
static struct task *manage_global_listener_queue(struct task *t, void *context,
                                                 unsigned short state) {
...
static int smp_fetch_dconn(const struct arg *args, struct sample *smp,
...
static int smp_fetch_so_id(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = smp->sess->listener->luid;
  return 1;
}
static int smp_fetch_so_name(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
...
static int bind_parse_accept_proxy(char **args, int cur_arg, struct proxy *px,
                                   struct bind_conf *conf, char **err) {
  struct listener *l;

  list_for_each_entry(l, &conf->listeners, by_bind) l->options |=
      LI_O_ACC_PROXY;
...
static int bind_parse_accept_netscaler_cip(char **args, int cur_arg,
                                           struct proxy *px,
                                           struct bind_conf *conf, char **err) {
  struct listener *l;
  uint32_t val;
...
static int bind_parse_backlog(char **args, int cur_arg, struct proxy *px,
                              struct bind_conf *conf, char **err) {
  struct listener *l;
  int val;
  ...
  list_for_each_entry(l, &conf->listeners, by_bind) l->backlog = val;

  return 0;
}

/* parse the "id" bind keyword */
static int bind_parse_id(char **args, int cur_arg, struct proxy *px,
                         struct bind_conf *conf, char **err) {
  ...
  if (*error != '\0') {
    memprintf(err, "'%s' : expects an integer argument, found '%s'",
...
static int bind_parse_maxconn(char **args, int cur_arg, struct proxy *px,
                              struct bind_conf *conf, char **err) {
  struct listener *l;
  int val;
  ...
  list_for_each_entry(l, &conf->listeners, by_bind) l->maxconn = val;

  return 0;
}

/* parse the "name" bind keyword */
static int bind_parse_name(char **args, int cur_arg, struct proxy *px,
                           struct bind_conf *conf, char **err) {
  struct listener *l;
  ...
  list_for_each_entry(l, &conf->listeners, by_bind) l->name =
...
static int bind_parse_nice(char **args, int cur_arg, struct proxy *px,
                           struct bind_conf *conf, char **err) {
  struct listener *l;
  int val;
  ...
  if (val < -1024 || val > 1024) {
    memprintf(err, "'%s' : invalid value %d, allowed range is -1024..1024",
              args[cur_arg], val);
    return ERR_ALERT | ERR_FATAL;
  }

  list_for_each_entry(l, &conf->listeners, by_bind) l->nice = val;

  return 0;
}

/* parse the "process" bind keyword */
static int bind_parse_process(char **args, int cur_arg, struct proxy *px,
                              struct bind_conf *conf, char **err) {
  char *slash;
...
static int bind_parse_proto(char **args, int cur_arg, struct proxy *px,
                            struct bind_conf *conf, char **err) {
  struct ist proto;
 ...
 */
static int cfg_parse_tune_listener_mq(char **args, int section_type,
...
static struct bind_kw_list bind_kws = {
    ...
    {},
    {
        {"accept-netscaler-cip", bind_parse_accept_netscaler_cip,
         1}, /* enable NetScaler Client IP insertion protocol */
        {"accept-proxy", bind_parse_accept_proxy,
         0}, /* enable PROXY protocol */
        {"backlog", bind_parse_backlog,
         1},                      /* set backlog of listening socket */
        {"id", bind_parse_id, 1}, /* set id of listening socket */
        {"maxconn", bind_parse_maxconn,
         1},                          /* set maxconn of listening socket */
        {"name", bind_parse_name, 1}, /* set name of listening socket */
        {"nice", bind_parse_nice, 1}, /* set nice of listening socket */
        {"process", bind_parse_process,
         1}, /* set list of allowed process for this socket */
        {"proto", bind_parse_proto,
         1}, /* set the proto to use for all incoming connections */
        {/* END */},
    }};
