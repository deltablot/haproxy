...
static int da_log_level(char **args, int section_type, struct proxy *curpx,
  ...
  if (loglevel < 0 || loglevel > 3) {
    memprintf(err,
              "deviceatlas log level : expects a log level between 0 and 3, %s "
              "given.\n",
              args[1]);
  } else {
...
static int da_property_separator(char **args, int section_type,
                                 struct proxy *curpx, struct proxy *defpx,
                                 const char *file, int line, char **err) {
  if (*(args[1]) == 0) {
    memprintf(
        err,
        "deviceatlas property separator : expects a character argument.\n");
    return -1;
  }
...
static int da_properties_cookie(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  if (*(args[1]) == 0) {
...
static int init_deviceatlas(void) {
  ...
  if (global_deviceatlas.jsonpath != 0) {
    ...
    if (jsonp == 0) {
      ha_alert(
          "deviceatlas : '%s' json file has invalid path or is not readable.\n",
    ...
    global_deviceatlas.useragentid =
        da_atlas_header_evidence_id(&global_deviceatlas.atlas, "user-agent");
...
static int da_haproxy(const struct arg *args, struct sample *smp,
  ...
  for (; propname != 0; i++, propname = (const char *)args[i].data.str.area) {
    status = da_atlas_getpropid(&global_deviceatlas.atlas, propname, &prop);
    if (status != DA_OK) {
...
static int da_haproxy_conv(const struct arg *args, struct sample *smp,
                           void *private) {
  da_deviceinfo_t devinfo;
  da_status_t status;
  ...
  i = smp->data.u.str.data > sizeof(useragentbuf) ? sizeof(useragentbuf)
                                                  : smp->data.u.str.data;
...
static int da_haproxy_fetch(const struct arg *args, struct sample *smp,
  ...
  i = 0;
  for (blk = htx_get_first_blk(htx); nbh < DA_MAX_HEADERS && blk;
    ...
    if (strcasecmp(hbuf, "Accept-Language") == 0) {
      ...
      if (http_extract_cookie_value(pval, eval, global_deviceatlas.cookiename,
                                    global_deviceatlas.cookienamelen, 1, &p,
                                    &pl) == NULL) {
        continue;
      }
  ...
  status = da_searchv(&global_deviceatlas.atlas, &devinfo, ev, nbh);
...
#else
  memprintf(&ptr, "Built with DeviceAtlas support (library version %u.%u).",
            MOBI_DA_MAJOR, MOBI_DA_MINOR);
#endif
