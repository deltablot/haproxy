 ...
 * Copyright (C) 2020 HAProxy Technologies, William Lallemand
...
static int ssl_load_global_issuers_from_path(char **args, int section_type,
                                             struct proxy *curpx,
                                             struct proxy *defpx,
  ...
  if (*path == 0 || stat(path, &buf)) {
    memprintf(
        err,
        "%sglobal statement '%s' expects a directory path as an argument.\n",
        err && *err ? *err : "", args[0]);
    return -1;
  }
  ...
  for (end = path + strlen(path) - 1; end >= path && *end == '/'; end--)
    *end = 0;
  /* path already parsed? */
  if (global_ssl.issuers_chain_path &&
      strcmp(global_ssl.issuers_chain_path, path) == 0)
    return 0;
  ...
  if (n < 0) {
    memprintf(err,
              "%sglobal statement '%s': unable to scan directory '%s' : %s.\n",
 ...
 */
static int ssl_parse_global_ssl_async(char **args, int section_type,
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_ssl_engine(char **args, int section_type,
                                       struct proxy *curpx, struct proxy *defpx,
                                       const char *file, int line, char **err) {
  char *algo;
  int ret = -1;
  ...
  if (*(args[1]) == 0) {
    memprintf(
        err,
        "global statement '%s' expects a valid engine name as an argument.",
        args[0]);
    return ret;
  }
  ...
  /* otherwise the expected format is ssl-engine <engine_name> algo <list of
   * algo> */
  if (strcmp(args[2], "algo") != 0) {
    memprintf(err, "global statement '%s' expects to have algo keyword.",
              args[0]);
    return ret;
  }
  ...
  if (*(args[3]) == 0) {
    memprintf(err,
              "global statement '%s' expects algorithm names as an argument.",
              args[0]);
    return ret;
  }
  algo = strdup(args[3]);
 ...
 */
static int ssl_parse_global_ciphers(char **args, int section_type,
                                    struct proxy *curpx, struct proxy *defpx,
                                    const char *file, int line, char **err) {
  char **target;

  target = (args[0][12] == 'b') ? &global_ssl.listen_default_ciphers
  ...
  if (*(args[1]) == 0) {
    memprintf(err,
              "global statement '%s' expects a cipher suite as an argument.",
              args[0]);
    return -1;
  }
...
/* parse the "ssl-default-bind-ciphersuites" / "ssl-default-server-ciphersuites"
 * keywords in global section. Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_ciphersuites(char **args, int section_type,
                                         struct proxy *curpx,
                                         struct proxy *defpx, const char *file,
                                         int line, char **err) {
  char **target;

  target = (args[0][12] == 'b') ? &global_ssl.listen_default_ciphersuites
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_curves(char **args, int section_type,
                                   struct proxy *curpx, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  char **target;
  ...
  if (*(args[1]) == 0) {
    memprintf(err,
              "global statement '%s' expects a curves suite as an arguments.",
              args[0]);
    return -1;
  }
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_int(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  int *target;
  ...
  if (strcmp(args[0], "tune.ssl.cachesize") == 0)
    ...
    memprintf(err, "'%s' keyword not unhandled (please report this bug).",
              args[0]);
    return -1;
  }
...
static int ssl_parse_global_capture_cipherlist(char **args, int section_type,
                                               struct proxy *curpx,
                                               struct proxy *defpx,
                                               const char *file, int line,
                                               char **err) {
  ...
  pool_head_ssl_capture = create_pool(
      "ssl-capture", sizeof(struct ssl_capture) + global_ssl.capture_cipherlist,
      MEM_F_SHARED);
  if (!pool_head_ssl_capture) {
...
static int ssl_parse_global_keylog(char **args, int section_type,
                                   struct proxy *curpx, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  ...
  pool_head_ssl_keylog =
      create_pool("ssl-keylogfile", sizeof(struct ssl_keylog), MEM_F_SHARED);
  ...
  pool_head_ssl_keylog_str =
      create_pool("ssl-keylogfile-str",
                  sizeof(char) * SSL_KEYLOG_MAX_SECRET_SIZE, MEM_F_SHARED);
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_private_cache(char **args, int section_type,
                                          struct proxy *curpx,
                                          struct proxy *defpx, const char *file,
                                          int line, char **err) {
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_lifetime(char **args, int section_type,
                                     struct proxy *curpx, struct proxy *defpx,
                                     const char *file, int line, char **err) {
  const char *res;
  ...
  if (*(args[1]) == 0) {
    memprintf(err,
              "'%s' expects ssl sessions <lifetime> in seconds as argument.",
              args[0]);
    return -1;
  }
  ...
  if (res == PARSE_TIME_OVER) {
    ...
    memprintf(err, "unexpected character '%c' in argument to <%s>.", *res,
              args[0]);
    return -1;
  }
  return 0;
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_dh_param_file(char **args, int section_type,
                                          struct proxy *curpx,
                                          struct proxy *defpx, const char *file,
                                          int line, char **err) {
  if (too_many_args(1, args, err, NULL))
  ...
  if (ssl_sock_load_global_dh_param_from_file(args[1])) {
    memprintf(err, "'%s': unable to load DH parameters from file <%s>.",
              args[0], args[1]);
    return -1;
  }
  return 0;
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_default_dh(char **args, int section_type,
                                       struct proxy *curpx, struct proxy *defpx,
                                       const char *file, int line, char **err) {
  if (too_many_args(1, args, err, NULL))
    return -1;
 ...
 * multiple arguments are allowed: "bundle", "sctl", "ocsp", "issuer", "all",
 * "none"
 */
static int ssl_parse_global_extra_files(char **args, int section_type,
                                        struct proxy *curpx,
                                        struct proxy *defpx, const char *file,
                                        int line, char **err) {
  int i;
...
err_arg:
  memprintf(err,
            "'%s' expects one or multiple arguments (none, all, bundle, sctl, "
            "ocsp, issuer).",
            args[0]);
  return -1;
}
...
static int ssl_parse_global_extra_noext(char **args, int section_type,
                                        struct proxy *curpx,
                                        struct proxy *defpx, const char *file,
                                        int line, char **err) {
...
static int ssl_bind_parse_ca_file_common(char **args, int cur_arg,
                                         char **ca_file_p, char **err) {
  if (!*args[cur_arg + 1]) {
...
static int ssl_bind_parse_ca_verify_file(char **args, int cur_arg,
                                         struct proxy *px,
                                         struct ssl_bind_conf *conf,
                                         char **err) {
  return ssl_bind_parse_ca_file_common(args, cur_arg, &conf->ca_verify_file,
                                       err);
}
static int bind_parse_ca_verify_file(char **args, int cur_arg, struct proxy *px,
                                     struct bind_conf *conf, char **err) {
  return ssl_bind_parse_ca_verify_file(args, cur_arg, px, &conf->ssl_conf, err);
}

/* parse the "ca-sign-file" bind keyword */
static int bind_parse_ca_sign_file(char **args, int cur_arg, struct proxy *px,
                                   struct bind_conf *conf, char **err) {
  if (!*args[cur_arg + 1]) {
    memprintf(err, "'%s' : missing CAfile path", args[cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  ...
  if ((*args[cur_arg + 1] != '/') && global_ssl.ca_base)
    memprintf(&conf->ca_sign_file, "%s/%s", global_ssl.ca_base,
              args[cur_arg + 1]);
  else
...
static int ssl_bind_parse_ciphersuites(char **args, int cur_arg,
                                       struct proxy *px,
                                       struct ssl_bind_conf *conf, char **err) {
  if (!*args[cur_arg + 1]) {
    memprintf(err, "'%s' : missing cipher suite", args[cur_arg]);
...
/* parse the "crt" bind keyword. Returns a set of ERR_* flags possibly with an
 * error in <err>. */
static int bind_parse_crt(char **args, int cur_arg, struct proxy *px,
                          struct bind_conf *conf, char **err) {
  ...
  if ((*args[cur_arg + 1] != '/') && global_ssl.crt_base) {
    if ((strlen(global_ssl.crt_base) + 1 + strlen(args[cur_arg + 1]) + 1) >
        MAXPATHLEN) {
      memprintf(err, "'%s' : path too long", args[cur_arg]);
      return ERR_ALERT | ERR_FATAL;
    }
    snprintf(path, sizeof(path), "%s/%s", global_ssl.crt_base,
             args[cur_arg + 1]);
...
/* parse the "crt-list" bind keyword. Returns a set of ERR_* flags possibly with
 * an error in <err>. */
static int bind_parse_crt_list(char **args, int cur_arg, struct proxy *px,
                               struct bind_conf *conf, char **err) {
  int err_code;
...
static int ssl_bind_parse_ecdhe(char **args, int cur_arg, struct proxy *px,
  ...
  memprintf(
      err,
      "'%s' : library does not support elliptic curve Diffie-Hellman (too old)",
      args[cur_arg]);
  return ERR_ALERT | ERR_FATAL;
#elif defined(OPENSSL_NO_ECDH)
  memprintf(err,
            "'%s' : library does not support elliptic curve Diffie-Hellman "
            "(disabled via OPENSSL_NO_ECDH)",
            args[cur_arg]);
  return ERR_ALERT | ERR_FATAL;
#else
  if (!*args[cur_arg + 1]) {
...
static int bind_parse_ecdhe(char **args, int cur_arg, struct proxy *px,
                            struct bind_conf *conf, char **err) {
...
static int bind_parse_ignore_err(char **args, int cur_arg, struct proxy *px,
  ...
  while (p) {
    ...
    if ((code <= 0) || (code > 63)) {
      memprintf(err,
                "'%s' : ID '%d' out of range (1..63) in error IDs list '%s'",
...
static int parse_tls_method_options(char *arg,
                                    struct tls_version_filter *methods,
                                    char **err) {
  uint16_t v;
  char *p;
  p = strchr(arg, '-');
...
static int bind_parse_tls_method_options(char **args, int cur_arg,
                                         struct proxy *px,
                                         struct bind_conf *conf, char **err) {
  return parse_tls_method_options(args[cur_arg], &conf->ssl_conf.ssl_methods,
                                  err);
}

static int srv_parse_tls_method_options(char **args, int *cur_arg,
                                        struct proxy *px, struct server *newsrv,
                                        char **err) {
  return parse_tls_method_options(args[*cur_arg], &newsrv->ssl_ctx.methods,
                                  err);
}

/* parse tls_method min/max: "ssl-min-ver" and "ssl-max-ver" */
static int parse_tls_method_minmax(char **args, int cur_arg,
                                   struct tls_version_filter *methods,
                                   char **err) {
  uint16_t i, v = 0;
...
static int ssl_bind_parse_tls_method_minmax(char **args, int cur_arg,
                                            struct proxy *px,
                                            struct ssl_bind_conf *conf,
                                            char **err) {
  int ret;
  ...
  ha_warning("crt-list: ssl-min-ver and ssl-max-ver are not supported with "
...
static int bind_parse_tls_method_minmax(char **args, int cur_arg,
                                        struct proxy *px,
                                        struct bind_conf *conf, char **err) {
  return parse_tls_method_minmax(args, cur_arg, &conf->ssl_conf.ssl_methods,
                                 err);
}

static int srv_parse_tls_method_minmax(char **args, int *cur_arg,
                                       struct proxy *px, struct server *newsrv,
                                       char **err) {
...
static int bind_parse_no_tls_tickets(char **args, int cur_arg, struct proxy *px,
                                     struct bind_conf *conf, char **err) {
...
static int ssl_bind_parse_allow_0rtt(char **args, int cur_arg, struct proxy *px,
                                     struct ssl_bind_conf *conf, char **err) {
  conf->early_data = 1;
  return 0;
}
...
static int ssl_bind_parse_npn(char **args, int cur_arg, struct proxy *px,
  ...
  if (!*args[cur_arg + 1]) {
    memprintf(err, "'%s' : missing the comma-delimited NPN protocol suite",
              args[cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  }
  ...
  while (1) {
    ...
    if (p2 - (p1 + 1) > 255) {
      *p2 = '\0';
      memprintf(err, "'%s' : NPN protocol name too long : '%s'", args[cur_arg],
                p1 + 1);
      return ERR_ALERT | ERR_FATAL;
    }
  ...
  return 0;
#else
  memprintf(err, "'%s' : library does not support TLS NPN extension",
            args[cur_arg]);
  return ERR_ALERT | ERR_FATAL;
#endif
}

static int bind_parse_npn(char **args, int cur_arg, struct proxy *px,
                          struct bind_conf *conf, char **err) {
...
static int ssl_bind_parse_alpn(char **args, int cur_arg, struct proxy *px,
  ...
  ret = ssl_sock_parse_alpn(args[cur_arg + 1], &conf->alpn_str, &conf->alpn_len,
                            err);
  if (ret)
    memprintf(err, "'%s' : %s", args[cur_arg], *err);
  return ret;
#else
  memprintf(err, "'%s' : library does not support TLS ALPN extension",
            args[cur_arg]);
  return ERR_ALERT | ERR_FATAL;
#endif
}

static int bind_parse_alpn(char **args, int cur_arg, struct proxy *px,
                           struct bind_conf *conf, char **err) {
  return ssl_bind_parse_alpn(args, cur_arg, px, &conf->ssl_conf, err);
}

/* parse the "ssl" bind keyword */
static int bind_parse_ssl(char **args, int cur_arg, struct proxy *px,
                          struct bind_conf *conf, char **err) {
  conf->xprt = &ssl_sock;
  conf->is_ssl = 1;
...
static int bind_parse_pcc(char **args, int cur_arg, struct proxy *px,
                          struct bind_conf *conf, char **err) {
...
static int bind_parse_generate_certs(char **args, int cur_arg, struct proxy *px,
                                     struct bind_conf *conf, char **err) {
...
#else
  memprintf(err,
            "%sthis version of openssl cannot generate SSL certificates.\n",
            err && *err ? *err : "");
#endif
  return 0;
}
...
static int bind_parse_strict_sni(char **args, int cur_arg, struct proxy *px,
                                 struct bind_conf *conf, char **err) {
  conf->strict_sni = 1;
  return 0;
}
...
static int bind_parse_tls_ticket_keys(char **args, int cur_arg,
  ...
  if ((f = fopen(args[cur_arg + 1], "r")) == NULL) {
    memprintf(err, "'%s' : unable to load ssl tickets keys file",
              args[cur_arg + 1]);
    goto fail;
  }
  ...
  while (fgets(thisline, sizeof(thisline), f) != NULL) {
    ...
    dec_size = base64dec(thisline, len,
                         (char *)(keys_ref->tlskeys + i % TLS_TICKETS_NO),
                         sizeof(union tls_sess_key));
    if (dec_size < 0) {
      memprintf(err, "'%s' : unable to decode base64 key on line %d",
                args[cur_arg + 1], i + 1);
      goto fail;
    } else if (!keys_ref->key_size_bits &&
               (dec_size == sizeof(struct tls_sess_key_128))) {
      keys_ref->key_size_bits = 128;
    } else if (!keys_ref->key_size_bits &&
               (dec_size == sizeof(struct tls_sess_key_256))) {
      keys_ref->key_size_bits = 256;
    } else if (((dec_size != sizeof(struct tls_sess_key_128)) &&
                (dec_size != sizeof(struct tls_sess_key_256))) ||
               ((dec_size == sizeof(struct tls_sess_key_128) &&
                 (keys_ref->key_size_bits != 128))) ||
               ((dec_size == sizeof(struct tls_sess_key_256) &&
                 (keys_ref->key_size_bits != 256)))) {
      memprintf(err, "'%s' : wrong sized key on line %d", args[cur_arg + 1],
                i + 1);
      goto fail;
    }
    i++;
  ...
  if (i < TLS_TICKETS_NO) {
    memprintf(err,
              "'%s' : please supply at least %d keys in the tls-tickets-file",
              args[cur_arg + 1], TLS_TICKETS_NO);
    goto fail;
  }
...
#else
  memprintf(err, "'%s' : TLS ticket callback extension not supported",
            args[cur_arg]);
  return ERR_ALERT | ERR_FATAL;
...
static int ssl_bind_parse_verify(char **args, int cur_arg, struct proxy *px,
  ...
  if (strcmp(args[cur_arg + 1], "none") == 0)
    ...
    memprintf(err,
              "'%s' : unknown verify method '%s', only 'none', 'optional', and "
...
static int ssl_bind_parse_no_ca_names(char **args, int cur_arg,
                                      struct proxy *px,
                                      struct ssl_bind_conf *conf, char **err) {
  conf->no_ca_names = 1;
  return 0;
...
/* parse the "npn" bind keyword */
static int srv_parse_npn(char **args, int *cur_arg, struct proxy *px,
  ...
  while (1) {
    p2 = memchr(p1 + 1, ',',
                newsrv->ssl_ctx.npn_str + newsrv->ssl_ctx.npn_len - (p1 + 1));
    if (!p2)
      p2 = p1 + 1 + strlen(p1 + 1);
...
static int srv_parse_alpn(char **args, int *cur_arg, struct proxy *px,
                          struct server *newsrv, char **err) {
#ifdef TLSEXT_TYPE_application_layer_protocol_negotiation
  char **alpn_str;
  int *alpn_len;
...
static int srv_parse_ca_file(char **args, int *cur_arg, struct proxy *px,
                             struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
    memprintf(err, "'%s' : missing CAfile path", args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  ...
  if ((*args[*cur_arg + 1] != '/') && global_ssl.ca_base)
    memprintf(&newsrv->ssl_ctx.ca_file, "%s/%s", global_ssl.ca_base,
              args[*cur_arg + 1]);
  else
...
static int srv_parse_ciphers(char **args, int *cur_arg, struct proxy *px,
                             struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
    memprintf(err, "'%s' : missing cipher suite", args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
...
static int srv_parse_ciphersuites(char **args, int *cur_arg, struct proxy *px,
                                  struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
    memprintf(err, "'%s' : missing cipher suite", args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
...
static int srv_parse_crl_file(char **args, int *cur_arg, struct proxy *px,
                              struct server *newsrv, char **err) {
#ifndef X509_V_FLAG_CRL_CHECK
  memprintf(err, "'%s' : library does not support CRL verify", args[*cur_arg]);
  return ERR_ALERT | ERR_FATAL;
  ...
  if ((*args[*cur_arg + 1] != '/') && global_ssl.ca_base)
    memprintf(&newsrv->ssl_ctx.crl_file, "%s/%s", global_ssl.ca_base,
              args[*cur_arg + 1]);
  else
...
static int srv_parse_crt(char **args, int *cur_arg, struct proxy *px,
                         struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
...
static int srv_parse_no_check_ssl(char **args, int *cur_arg, struct proxy *px,
                                  struct server *newsrv, char **err) {
  newsrv->check.use_ssl = -1;
  free(newsrv->ssl_ctx.ciphers);
...
static int srv_parse_no_send_proxy_ssl(char **args, int *cur_arg,
                                       struct proxy *px, struct server *newsrv,
                                       char **err) {
...
static int srv_parse_no_send_proxy_cn(char **args, int *cur_arg,
                                      struct proxy *px, struct server *newsrv,
                                      char **err) {
  newsrv->pp_opts &= ~SRV_PP_V2;
  newsrv->pp_opts &= ~SRV_PP_V2_SSL;
...
static int srv_parse_no_ssl(char **args, int *cur_arg, struct proxy *px,
                            struct server *newsrv, char **err) {
...
static int srv_parse_no_tls_tickets(char **args, int *cur_arg, struct proxy *px,
                                    struct server *newsrv, char **err) {
  newsrv->ssl_ctx.options |= SRV_SSL_O_NO_TLS_TICKETS;
  return 0;
}
/* parse the "send-proxy-v2-ssl" server keyword */
static int srv_parse_send_proxy_ssl(char **args, int *cur_arg, struct proxy *px,
                                    struct server *newsrv, char **err) {
  newsrv->pp_opts |= SRV_PP_V2;
  newsrv->pp_opts |= SRV_PP_V2_SSL;
  return 0;
...
static int srv_parse_send_proxy_cn(char **args, int *cur_arg, struct proxy *px,
                                   struct server *newsrv, char **err) {
  newsrv->pp_opts |= SRV_PP_V2;
  newsrv->pp_opts |= SRV_PP_V2_SSL;
  newsrv->pp_opts |= SRV_PP_V2_SSL_CN;
...
static int srv_parse_sni(char **args, int *cur_arg, struct proxy *px,
                         struct server *newsrv, char **err) {
#ifndef SSL_CTRL_SET_TLSEXT_HOSTNAME
  memprintf(
      err,
      "'%s' : the current SSL library doesn't support the SNI TLS extension",
      args[*cur_arg]);
  return ERR_ALERT | ERR_FATAL;
#else
  char *arg;
...
static int srv_parse_ssl(char **args, int *cur_arg, struct proxy *px,
                         struct server *newsrv, char **err) {
  newsrv->use_ssl = 1;
  ssl_sock_init_srv(newsrv);
  return 0;
...
static int srv_parse_ssl_reuse(char **args, int *cur_arg, struct proxy *px,
                               struct server *newsrv, char **err) {
  newsrv->ssl_ctx.options &= ~SRV_SSL_O_NO_REUSE;
  return 0;
}
...
static int srv_parse_tls_tickets(char **args, int *cur_arg, struct proxy *px,
                                 struct server *newsrv, char **err) {
  newsrv->ssl_ctx.options &= ~SRV_SSL_O_NO_TLS_TICKETS;
  return 0;
}
...
static int srv_parse_verify(char **args, int *cur_arg, struct proxy *px,
                            struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
    memprintf(err, "'%s' : missing verify method", args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  ...
  if (strcmp(args[*cur_arg + 1], "none") == 0)
    ...
    memprintf(err,
              "'%s' : unknown verify method '%s', only 'none' and 'required' "
              "are supported\n",
              args[*cur_arg], args[*cur_arg + 1]);
    return ERR_ALERT | ERR_FATAL;
  }
...
static int srv_parse_verifyhost(char **args, int *cur_arg, struct proxy *px,
                                struct server *newsrv, char **err) {
  if (!*args[*cur_arg + 1]) {
...
static int ssl_parse_default_bind_options(char **args, int section_type,
                                          struct proxy *curpx,
                                          struct proxy *defpx, const char *file,
                                          int line, char **err) {
  int i = 1;
  ...
  if (*(args[i]) == 0) {
    memprintf(err, "global statement '%s' expects an option as an argument.",
              args[0]);
    return -1;
  }
  while (*(args[i])) {
    ...
    else if (!strcmp(args[i], "ssl-min-ver") ||
             !strcmp(args[i], "ssl-max-ver")) {
      if (!parse_tls_method_minmax(args, i,
                                   &global_ssl.listen_default_sslmethods, err))
        i++;
      else {
        memprintf(err, "%s on global statement '%s'.", *err, args[0]);
        return -1;
      }
    } else if (parse_tls_method_options(
                   args[i], &global_ssl.listen_default_sslmethods, err)) {
      memprintf(err, "unknown option '%s' on global statement '%s'.", args[i],
                args[0]);
      return -1;
    }
    i++;
...
static int ssl_parse_default_server_options(char **args, int section_type,
                                            struct proxy *curpx,
                                            struct proxy *defpx,
                                            const char *file, int line,
                                            char **err) {
  ...
  while (*(args[i])) {
    if (!strcmp(args[i], "no-tls-tickets"))
      global_ssl.connect_default_ssloptions |= SRV_SSL_O_NO_TLS_TICKETS;
             ...
             !strcmp(args[i], "ssl-max-ver")) {
      if (!parse_tls_method_minmax(args, i,
                                   &global_ssl.connect_default_sslmethods, err))
        i++;
      else {
        memprintf(err, "%s on global statement '%s'.", *err, args[0]);
        return -1;
                   ...
                   args[i], &global_ssl.connect_default_sslmethods, err)) {
      memprintf(err, "unknown option '%s' on global statement '%s'.", args[i],
                args[0]);
      return -1;
    }
 ...
 * Returns <0 on alert, >0 on warning, 0 on success.
 */
static int ssl_parse_global_ca_crt_base(char **args, int section_type,
                                        struct proxy *curpx,
                                        struct proxy *defpx, const char *file,
                                        int line, char **err) {
  char **target;
  ...
  if (*(args[1]) == 0) {
    memprintf(err,
              "global statement '%s' expects a directory path as an argument.",
              args[0]);
    return -1;
  }
  *target = strdup(args[1]);
...
static int ssl_parse_skip_self_issued_ca(char **args, int section_type,
                                         struct proxy *curpx,
                                         struct proxy *defpx, const char *file,
                                         int line, char **err) {
  ...
  return 0;
#else
  memprintf(err, "global statement '%s' requires at least OpenSSL 1.0.2.",
            args[0]);
  return -1;
#endif
}
...
/* the <ssl_bind_kws> keywords are used for crt-list parsing, they *MUST* be
 * safe with their proxy argument NULL and must only fill the ssl_bind_conf */
struct ssl_bind_kw ssl_bind_kws[] = {
    {"allow-0rtt", ssl_bind_parse_allow_0rtt, 0}, /* allow 0-RTT */
    {"alpn", ssl_bind_parse_alpn, 1}, /* set ALPN supported protocols */
    {"ca-file", ssl_bind_parse_ca_file,
     1}, /* set CAfile to process ca-names and verify on client cert */
    {"ca-verify-file", ssl_bind_parse_ca_verify_file,
     1}, /* set CAverify file to process verify on client cert */
    {"ciphers", ssl_bind_parse_ciphers, 1}, /* set SSL cipher suite */
#ifdef HAVE_SSL_CTX_SET_CIPHERSUITES
    {"ciphersuites", ssl_bind_parse_ciphersuites,
     1}, /* set TLS 1.3 cipher suite */
#endif
    {"crl-file", ssl_bind_parse_crl_file,
     1}, /* set certificate revocation list file use on client cert verify */
    {"curves", ssl_bind_parse_curves, 1}, /* set SSL curve suite */
    {"ecdhe", ssl_bind_parse_ecdhe,
     1}, /* defines named curve for elliptic curve Diffie-Hellman */
    {"no-ca-names", ssl_bind_parse_no_ca_names,
     0}, /* do not send ca names to clients (ca_file related) */
...
static struct bind_kw_list bind_kws = {
    ...
    {},
    {
        ...
        {"ca-file", bind_parse_ca_file,
         1}, /* set CAfile to process ca-names and verify on client cert */
        {"ca-verify-file", bind_parse_ca_verify_file,
         1}, /* set CAverify file to process verify on client cert */
        {"ca-ignore-err", bind_parse_ignore_err,
         1}, /* set error IDs to ignore on verify depth > 0 */
        {"ca-sign-file", bind_parse_ca_sign_file,
         1}, /* set CAFile used to generate and sign server certs */
        {"ca-sign-pass", bind_parse_ca_sign_pass, 1}, /* set CAKey passphrase */
        {"ciphers", bind_parse_ciphers, 1},           /* set SSL cipher suite */
#ifdef HAVE_SSL_CTX_SET_CIPHERSUITES
        {"ciphersuites", bind_parse_ciphersuites,
         1}, /* set TLS 1.3 cipher suite */
#endif
        {"crl-file", bind_parse_crl_file,
         1}, /* set certificate revocation list file use on client cert verify
              */
        {"crt", bind_parse_crt,
         1}, /* load SSL certificates from this location */
        {"crt-ignore-err", bind_parse_ignore_err,
         1}, /* set error IDs to ignore on verify depth == 0 */
        {"crt-list", bind_parse_crt_list,
         1}, /* load a list of crt from this location */
        {"curves", bind_parse_curves, 1}, /* set SSL curve suite */
        {"ecdhe", bind_parse_ecdhe,
         1}, /* defines named curve for elliptic curve Diffie-Hellman */
        ...
        {"generate-certificates", bind_parse_generate_certs,
         0}, /* enable the server certificates generation */
        {"no-ca-names", bind_parse_no_ca_names,
         0}, /* do not send ca names to clients (ca_file related) */
        ...
        {"no-tls-tickets", bind_parse_no_tls_tickets,
         0},                        /* disable session resumption tickets */
        ...
        {"strict-sni", bind_parse_strict_sni,
         0}, /* refuse negotiation if sni doesn't match a certificate */
        {"tls-ticket-keys", bind_parse_tls_ticket_keys,
         1}, /* set file to load TLS ticket keys from */
        {"verify", bind_parse_verify, 1}, /* set SSL verify method */
        {"npn", bind_parse_npn, 1},       /* set NPN supported protocols */
        {"prefer-client-ciphers", bind_parse_pcc,
         0}, /* prefer client ciphers */
        {NULL, NULL, 0},
    }};
...
static struct srv_kw_list srv_kws = {
    ...
    {},
    {
        {"allow-0rtt", srv_parse_allow_0rtt, 0,
         1}, /* Allow using early data on this server */
        {"alpn", srv_parse_alpn, 1, 1}, /* Set ALPN supported protocols */
        {"ca-file", srv_parse_ca_file, 1,
         1}, /* set CAfile to process verify server cert */
        {"check-alpn", srv_parse_alpn, 1, 1},     /* Set ALPN used for checks */
        {"check-sni", srv_parse_check_sni, 1, 1}, /* set SNI */
        {"check-ssl", srv_parse_check_ssl, 0,
         1},                                  /* enable SSL for health checks */
        {"ciphers", srv_parse_ciphers, 1, 1}, /* select the cipher suite */
#ifdef HAVE_SSL_CTX_SET_CIPHERSUITES
        {"ciphersuites", srv_parse_ciphersuites, 1,
         1}, /* select the cipher suite */
#endif
        {"crl-file", srv_parse_crl_file, 1,
         1}, /* set certificate revocation list file use on server cert verify
              */
        ...
        {"no-check-ssl", srv_parse_no_check_ssl, 0,
         1}, /* disable SSL for health checks */
        {"no-send-proxy-v2-ssl", srv_parse_no_send_proxy_ssl, 0,
         1}, /* do not send PROXY protocol header v2 with SSL info */
        {"no-send-proxy-v2-ssl-cn", srv_parse_no_send_proxy_cn, 0,
         1}, /* do not send PROXY protocol header v2 with CN */
        {"no-ssl", srv_parse_no_ssl, 0, 1}, /* disable SSL processing */
        {"no-ssl-reuse", srv_parse_no_ssl_reuse, 0,
         1}, /* disable session reuse */
        ...
        {"no-tls-tickets", srv_parse_no_tls_tickets, 0,
         1},                          /* disable session resumption tickets */
        {"npn", srv_parse_npn, 1, 1}, /* Set NPN supported protocols */
        {"send-proxy-v2-ssl", srv_parse_send_proxy_ssl, 0,
         1}, /* send PROXY protocol header v2 with SSL info */
        {"send-proxy-v2-ssl-cn", srv_parse_send_proxy_cn, 0,
         1}, /* send PROXY protocol header v2 with CN */
        {"sni", srv_parse_sni, 1, 1}, /* send SNI extension */
        {"ssl", srv_parse_ssl, 0, 1}, /* enable SSL processing */
        {"ssl-min-ver", srv_parse_tls_method_minmax, 1,
         1}, /* minimum version */
        {"ssl-max-ver", srv_parse_tls_method_minmax, 1,
         1},                                      /* maximum version */
        {"ssl-reuse", srv_parse_ssl_reuse, 0, 1}, /* enable session reuse */
        {"tls-tickets", srv_parse_tls_tickets, 0,
         1}, /* enable session resumption tickets */
        {"verify", srv_parse_verify, 1, 1}, /* set SSL verify method */
        {"verifyhost", srv_parse_verifyhost, 1,
         1}, /* require that SSL cert verifies for hostname */
        {NULL, NULL, 0, 0},
    }};
...
#endif
        {CFG_GLOBAL, "tune.ssl.force-private-cache",
