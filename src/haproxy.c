 ...
 *   - solaris only : sometimes, an HTTP proxy with only a dispatch address
 * causes the proxy to terminate (no core) if the client breaks the connection
 * during the response. Seen on 1.1.8pre4, but never reproduced. May not be
 * related to the snprintf() bug since requests were simple (GET / HTTP/1.0),
 * but may be related to missing setsid() (fixed in 1.1.15)
...
volatile unsigned long sleeping_thread_mask =
    0; /* Threads that are about to sleep in poll() */
volatile unsigned long stopping_thread_mask =
    0; /* Threads acknowledged stopping */
    ...
    .tune =
        {
            ...
            .bufsize =
                (BUFSIZE + 2 * sizeof(void *) - 1) & -(2 * sizeof(void *)),
...
int unstoppable_jobs =
    0; /* number of active jobs that can't be stopped during a soft stop */
int active_peers =
    0; /* number of active peers (connection attempts and connected) */
...
void mworker_reload() {
  ...
  if (getenv("HAPROXY_MWORKER_WAIT_ONLY") == NULL) {
    ...
    list_for_each_entry(ptdf, &per_thread_deinit_list, list) ptdf->fct();
    if (fdtab)
      deinit_pollers();
  }
  ...
  if (setrlimit(RLIMIT_NOFILE, &limit) == -1) {
    ...
    ha_warning("Failed to restore initial FD limits (cur=%u max=%u), using "
               "cur=%u max=%u\n",
...
alloc_error:
  ...
  next_argv = NULL;
  ha_warning(
      "Failed to reexecute the master process [%d]: Cannot allocate memory\n",
      pid);
  return;
}
...
static void sig_listen(struct sig_handler *sh) {
  if (protocol_resume_all() & ERR_FATAL) {
    const char *msg = "Some proxies refused to resume, probably due to a "
                      "conflict on a listening port. You may want to try again "
                      "after the conflicting application is stopped, otherwise "
                      "a restart might be needed to resume safe operations.\n";
    ha_warning("%s", msg);
    send_log(NULL, LOG_WARNING, "%s", msg);
  }
}
...
static void sig_dump_state(struct sig_handler *sh) {
  ...
  while (p) {
    ...
    send_log(p, LOG_NOTICE,
             "SIGHUP received, dumping servers states for proxy %s.\n", p->id);
    while (s) {
      chunk_printf(
          &trash,
          "SIGHUP: Server %s/%s is %s. Conn: %d act, %d pend, %lld tot.", p->id,
          s->id, (s->cur_state != SRV_ST_STOPPED) ? "UP" : "DOWN", s->cur_sess,
          s->nbpend, s->counters.cum_sess);
      ha_warning("%s\n", trash.area);
      send_log(p, LOG_NOTICE, "%s\n", trash.area);
      s = s->next;
    }

    /* FIXME: those info are a bit outdated. We should be able to distinguish
     * between FE and BE. */
    if (!p->srv) {
      chunk_printf(&trash,
                   "SIGHUP: Proxy %s has no servers. Conn: act(FE+BE): %d+%d, "
                   "%d pend (%d unass), tot(FE+BE): %lld+%lld.",
                   p->id, p->feconn, p->beconn, p->totpend, p->nbpend,
                   p->fe_counters.cum_conn, p->be_counters.cum_conn);
    } else if (p->srv_act == 0) {
      chunk_printf(&trash,
                   "SIGHUP: Proxy %s %s ! Conn: act(FE+BE): %d+%d, %d pend (%d "
                   "unass), tot(FE+BE): %lld+%lld.",
                   p->id,
      ...
      chunk_printf(&trash,
                   ...
                   "servers available."
                   " Conn: act(FE+BE): %d+%d, %d pend (%d unass), tot(FE+BE): "
                   "%lld+%lld.",
                   p->id, p->srv_act, p->srv_bck, p->feconn, p->beconn,
                   p->totpend, p->nbpend, p->fe_counters.cum_conn,
      ...
      *newargv++ = *argv++;
      ...
      if (flag[0] == '-' && flag[1] == 0) {
        ...
        *newargv++ = *argv++;
        ...
        case 'p':
        case 'S':
          /* these options have only 1 parameter which must be copied and can
           * start with a '-' */
          *newargv++ = *argv++;
          argc--;
          if (argc == 0)
          ...
          *newargv++ = *argv++;
          ...
          break;
        default:
          /* for other options just copy them without parameters, this is also
           * done for options like "--foo", but this  will fail in the argument
           * parser.
           * */
          *newargv++ = *argv++;
          argc--;
 ...
 * srandom_r() only takes 32 bits and purposely provides a reproducible
 * sequence, which means that there will only be 4 billion possible random
 * sequences once srandom() is called, regardless of the internal state. Not
 * calling it is even worse as we'll always produce the same randoms sequences.
 * What we do here is to create an initial sequence from various entropy
 * sources, hash it using SHA1 and keep the resulting 160 bits available
 * globally.
 *
...
static void ha_random_boot(char *const *argv) {
  ...
  write_u32(m, tv.tv_sec);
  m += 4;
  write_u32(m, tv.tv_usec);
  m += 4;
...
static void init(int argc, char **argv) {
  ...
  chunk_initlen(&global.log_tag, strdup(progname), strlen(progname),
                strlen(progname));
  ...
  while (argc > 0) {
    ...
    if (**argv == '-') {
      ...
      if (*flag == 'v') {
...
#else
        ha_alert("master-worker mode with systemd support (-Ws) requested, but "
                 "not compiled. Use master-worker mode (-W) if you are not "
                 "using Type=notify in your unit file or recompile with "
                 "USE_SYSTEMD=1.\n\n");
        usage(progname);
#endif
      } else if (*flag == 'W')
        ...
        if (argc <= 1) {
          ha_alert("Socket and optional bind parameters expected with the -S "
                   "flag\n");
          usage(progname);
        }
        if ((c = malloc(sizeof(*c))) == NULL ||
            (c->s = strdup(argv[1])) == NULL) {
        ...
        case 'n':
          ...
          break;
        case 'm':
          global.rlimit_memmax_all = atol(*argv);
          break;
        case 'N':
  ...
  global.maxsock =
      10; /* reserve 10 fds ; will be incremented by socket eaters */
  ...
  if (global.nbproc > 1 && !global.nbthread) {
    ha_warning("nbproc is deprecated!\n"
               "  | For suffering many limitations, the 'nbproc' directive is "
               "now deprecated\n"
               "  | and scheduled for removal in 2.5. Just comment it out: "
               "haproxy will use\n"
               "  | threads and will run on all allocated processors. You may "
               "also switch to\n"
               "  | 'nbthread %d' to keep the same number of processors. If "
               "you absolutely\n"
               "  | want to run in multi-process mode, you can silence this "
               "warning by adding\n"
               "  | 'nbthread 1', but then please report your use case to "
               "developers.\n",
               global.nbproc);
  }
  ...
  for (px = proxies_list; px; px = px->next) {
    ...
    list_for_each_entry(ppcf, &post_proxy_check_list, list) err_code |=
        ppcf->fct(px);
  }
  if (err_code & (ERR_ABORT | ERR_FATAL)) {
    ha_alert("Fatal errors found in configuration.\n");
  ...
  if (global.mode & MODE_CHECK) {
    ...
    qfprintf(stdout, "Configuration file has no error but will not start (no "
                     "listener) => exit(2).\n");
    exit(2);
  }
  ...
  if (!global.rlimit_memmax) {
    ...
    mem -=
        global.tune.sslcachesize * 200; // about 200 bytes per SSL cache entry
    ...
    do {
      global.maxconn =
          mem /
          ((STREAM_MAX_COST +
            2 * global.tune.bufsize) + // stream + 2 buffers per stream
    ...
    if (global.mode & (MODE_VERBOSE | MODE_DEBUG))
      fprintf(
          stderr,
          "Note: setting global.maxconn to %d and global.maxsslconn to %d.\n",
    ...
    sslmem = mem - global.maxconn *
                       (int64_t)(STREAM_MAX_COST + 2 * global.tune.bufsize);
    global.maxsslconn =
        sslmem / (global.ssl_session_max_cost + global.ssl_handshake_max_cost);
    global.maxsslconn = round_2dig(global.maxsslconn);

    if (sslmem <= 0 || global.maxsslconn < sides) {
      ha_alert("Cannot compute the automatic maxsslconn because global.maxconn "
               "is already too "
               "high for the global.memmax value (%d MB). The absolute maximum "
               "possible value "
    ...
    if (sides)
      clearmem -=
          (global.ssl_session_max_cost + global.ssl_handshake_max_cost) *
          (int64_t)global.maxsslconn;
    ...
    do {
      ...
      if (clearmem <= 0 || !global.maxconn) {
        ha_alert("Cannot compute the automatic maxconn because "
    ...
    if (global.mode & (MODE_VERBOSE | MODE_DEBUG)) {
      if (sides && global.maxsslconn > sides * global.maxconn) {
        fprintf(
            stderr,
            "Note: global.maxsslconn is forced to %d which causes "
            "global.maxconn "
  ...
  global.tune.pool_low_count =
      ((long long)global.maxsock * global.tune.pool_low_ratio + 99) / 100;
  global.tune.pool_high_count =
      ((long long)global.maxsock * global.tune.pool_high_ratio + 99) / 100;
  ...
  if ((global.mode & MODE_DEBUG) &&
      (global.mode & (MODE_DAEMON | MODE_QUIET))) {
    ha_warning("<debug> mode incompatible with <quiet> and <daemon>. Keeping "
               "<debug> only.\n");
    global.mode &= ~(MODE_DAEMON | MODE_QUIET);
  }
  ...
  if ((global.nbproc > 1) && !(global.mode & (MODE_DAEMON | MODE_MWORKER))) {
    if (!(global.mode & (MODE_FOREGROUND | MODE_DEBUG)))
      ha_warning("<nbproc> is only meaningful in daemon mode or master-worker "
                 "mode. Setting limit to 1 process.\n");
    global.nbproc = 1;
  }
  ...
  if (!init_pollers()) {
    ha_alert("No polling mechanism available.\n"
             "  It is likely that haproxy was built with TARGET=generic and "
             "that FD_SETSIZE\n"
             "  is too low on this platform to support maxconn and the number "
             "of listeners\n"
             "  and servers. You should rebuild haproxy specifying your system "
             "using TARGET=\n"
             "  in order to support other polling systems (poll, epoll, "
             "kqueue) or reduce the\n"
             "  global maxconn setting to accommodate the system's limitation. "
             "For reference,\n"
             "  FD_SETSIZE=%d on this system, global.maxconn=%d resulting in a "
             "maximum of\n"
             "  %d file descriptors. You should thus reduce global.maxconn by "
             "%d. Also,\n"
...
void deinit(void) {
  ...
  while (p) {
    ...
    while (s) {
      ...
      list_for_each_entry(srvdf, &server_deinit_list, list) srvdf->fct(s);
    ...
    list_for_each_entry(pxdf, &proxy_deinit_list, list) pxdf->fct(p);
...
static void *run_thread_poll_loop(void *data) {
  ...
  if (!(global.tune.options & GTUNE_INSECURE_SETUID) && !master) {
    ...
    if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) == -1 &&
        !_HA_ATOMIC_XADD(&warn_fail, 1)) {
      ha_warning(
          "Failed to disable setuid, please report to developers with detailed "
          "information about your operating system. You can silence this "
          "warning "
  ...
  if (!(global.tune.options & GTUNE_INSECURE_FORK) && !master) {
    ...
    if (setrlimit(RLIMIT_NPROC, &limit) == -1 &&
        !_HA_ATOMIC_XADD(&warn_fail, 1)) {
      ha_warning(
          "Failed to disable forks, please report to developers with detailed "
          "information about your operating system. You can silence this "
          "warning "
  ...
  list_for_each_entry(ptff, &per_thread_free_list, list) ptff->fct();
...
static void set_identity(const char *program_name) {
  if (global.gid) {
    if (getgroups(0, NULL) > 0 && setgroups(0, NULL) == -1)
      ha_warning(
          "[%s.main()] Failed to drop supplementary groups. Using 'gid'/'group'"
...
int main(int argc, char **argv) {
  ...
  if (global.rlimit_memmax) {
    limit.rlim_cur = limit.rlim_max = global.rlimit_memmax * 1048576ULL;
#ifdef RLIMIT_AS
  ...
  if ((err & ~ERR_WARN) != ERR_NONE) {
    ha_alert("[%s.main()] Some protocols failed to start their listeners! "
             "Exiting.\n",
             argv[0]);
  ...
  if (!(global.mode & MODE_MWORKER_WAIT) && listeners == 0) {
    ha_alert("[%s.main()] No enabled listener found (check for 'bind' "
             "directives) ! Exiting.\n",
             argv[0]);
  ...
  if (getenv("HAPROXY_MWORKER_REEXEC") != NULL) {
    ...
    if ((global.mode & MODE_DAEMON)) {
      /* daemon mode re-executing, stdin/stdout/stderr are already closed so
       * keep quiet */
      global.mode &= ~MODE_VERBOSE;
      global.mode |=
          MODE_QUIET; /* ensure that we won't say anything from now */
    }
  } else {
  ...
  if ((global.last_checks & LSTCHK_NETADM) && global.uid) {
    ha_alert("[%s.main()] Some configuration options require full privileges, "
             "so global.uid cannot be changed.\n"
             "",
             argv[0]);
    protocol_unbind_all();
    exit(1);
  ...
  if (global.mode & (MODE_DAEMON | MODE_MWORKER | MODE_MWORKER_WAIT)) {
    ...
    if ((getenv("HAPROXY_MWORKER_REEXEC") == NULL) &&
        (global.mode & MODE_MWORKER) && (global.mode & MODE_DAEMON)) {
      ret = fork();
      if (ret < 0) {
    ...
    if (!(global.mode & MODE_MWORKER_WAIT)) {
      ...
      for (proc = 0; proc < global.nbproc; proc++) {
        ...
        if (global.mode & MODE_MWORKER) {
          ...
          list_for_each_entry(child, &proc_list, list) {
            if (child->relative_pid == relative_pid && child->reloads == 0 &&
    ...
    if (proc < global.nbproc && /* child */
        ...
        global.cpu_map
            .proc[proc]) /* only do this if the process has a CPU map */
#ifdef __FreeBSD__
    {
      cpuset_t cpuset;
      int i;
    ...
    if (global.stats_fe) {
      ...
      list_for_each_entry(bind_conf, &global.stats_fe->conf.bind, by_fe) {
        if (bind_conf->level & ACCESS_FD_LISTENERS) {
          if (!bind_conf->settings.bind_proc ||
    ...
    if (!(global.mode & MODE_MWORKER)) /* in mworker mode we don't want a new
                                          pgid for the children */
      setsid();
    fork_poller();
  }
...
#ifdef USE_THREAD
  {
    ...
    for (i = 1; i < global.nbthread; i++)
      pthread_create(&ha_thread_info[i].pthread, NULL, &run_thread_poll_loop,
                     (void *)(long)i);
    ...
    for (i = 0; i < global.nbthread; i++) {
      ...
      if (i < MAX_THREADS && /* only the first 32/64 threads may be pinned */
          global.cpu_map
              .thread[i]) { /* only do this if the thread has a THREAD map */
        ...
        while ((j = ffsl(cpu_map)) > 0) {
          ...
          thread_port_t mthread =
              pthread_mach_thread_np(ha_thread_info[i].pthread);
          thread_policy_set(mthread, THREAD_AFFINITY_POLICY,
                            (thread_policy_t)&cpu_set, 1);
          cpu_map &= ~(1UL << (j - 1));
        }
#else
        ...
        pthread_setaffinity_np(ha_thread_info[i].pthread, sizeof(cpuset),
                               &cpuset);
#endif
      }
    }
