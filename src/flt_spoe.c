...
DECLARE_STATIC_POOL(pool_head_spoe_ctx, "spoe_ctx",
                    sizeof(struct spoe_context));
DECLARE_STATIC_POOL(pool_head_spoe_appctx, "spoe_appctx",
                    sizeof(struct spoe_appctx));

struct flt_ops spoe_ops;

static int spoe_queue_context(struct spoe_context *ctx);
static int spoe_acquire_buffer(struct buffer *buf,
                               struct buffer_wait *buffer_wait);
static void spoe_release_buffer(struct buffer *buf,
                                struct buffer_wait *buffer_wait);
...
static inline void spoe_update_stat_time(struct timeval *tv, long *t) {
  if (*t == -1)
...
static int spoe_str_to_vsn(const char *str, size_t len) {
  const char *p, *end;
  int maj, min, vsn;
...
static int spoe_prepare_hahello_frame(struct appctx *appctx, char *frame,
                                      size_t size) {
  struct buffer *chk;
 ...
 * encoded bytes in the frame on success, 0 if an encoding error occurred and -1
 * if a fatal error occurred.  */
static int spoe_prepare_hadiscon_frame(struct appctx *appctx, char *frame,
                                       size_t size) {
  const char *reason;
  char *p, *end;
  unsigned int flags = SPOE_FRM_FL_FIN;
 ...
 * encoded bytes in the frame on success, 0 if an encoding error occurred and -1
 * if a fatal error occurred. */
static int spoe_prepare_hanotify_frame(struct appctx *appctx,
                                       struct spoe_context *ctx, char *frame,
                                       size_t size) {
  char *p, *end;
...
static int spoe_prepare_hafrag_frame(struct appctx *appctx,
                                     struct spoe_context *ctx, char *frame,
                                     size_t size) {
  char *p, *end;
  unsigned int stream_id, frame_id;
 ...
 * error occurred. */
static int spoe_handle_agenthello_frame(struct appctx *appctx, char *frame,
                                        size_t size) {
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
  char *p, *end;
  ...
  while (p < end) {
    ...
    if (sz >= strlen(VERSION_KEY) &&
        !memcmp(str, VERSION_KEY, strlen(VERSION_KEY))) {
      int i, type = *p++;
    ...
    else if (sz >= strlen(MAX_FRAME_SIZE_KEY) &&
             !memcmp(str, MAX_FRAME_SIZE_KEY, strlen(MAX_FRAME_SIZE_KEY))) {
      int type = *p++;
    ...
    else if (sz >= strlen(CAPABILITIES_KEY) &&
             !memcmp(str, CAPABILITIES_KEY, strlen(CAPABILITIES_KEY))) {
      int type = *p++;
  ...
  if (!agent)
    ...
    if ((flags & SPOE_APPCTX_FL_PIPELINING) &&
        !(agent->flags & SPOE_FL_PIPELINING))
      flags &= ~SPOE_APPCTX_FL_PIPELINING;
 ...
 * occurred. */
static int spoe_handle_agentdiscon_frame(struct appctx *appctx, char *frame,
                                         size_t size) {
  char *p, *end;
  unsigned int flags;
  ...
  while (p < end) {
    ...
    if (sz >= strlen(STATUS_CODE_KEY) &&
        !memcmp(str, STATUS_CODE_KEY, strlen(STATUS_CODE_KEY))) {
      int type = *p++;
...
static int spoe_handle_agentack_frame(struct appctx *appctx,
                                      struct spoe_context **ctx, char *frame,
                                      size_t size) {
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
  char *p, *end;
  ...
  *ctx = NULL;
  ...
  SPOE_PRINTF(stderr,
              "%d.%06d [SPOE/%-15s] %s: appctx=%p"
              " - Ignore ACK frame"
              " - stream-id=%u - frame-id=%u\n",
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__,
              appctx, (unsigned int)stream_id, (unsigned int)frame_id);
...
found:
  ...
  SPOE_PRINTF(stderr,
              ...
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__,
              appctx, *ctx, (*ctx)->stream_id, (*ctx)->frame_id, flags);
  return (p - frame);
}
...
static struct task *spoe_process_appctx(struct task *task, void *context,
...
static void spoe_release_appctx(struct appctx *appctx) {
  ...
  SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: appctx=%p\n", (int)now.tv_sec,
              (int)now.tv_usec, agent->id, __FUNCTION__, appctx);
  ...
  spoe_release_buffer(&spoe_appctx->buffer, &spoe_appctx->buffer_wait);
  ...
  list_for_each_entry(spoe_appctx, &agent->rt[tid].applets, list)
      HA_ATOMIC_UPDATE_MIN(&agent->rt[tid].frame_size,
                           spoe_appctx->max_frame_size);
}

static int spoe_handle_connect_appctx(struct appctx *appctx) {
  struct stream_interface *si = appctx->owner;
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
  char *frame, *buf;
  int ret;
...
static int spoe_handle_connecting_appctx(struct appctx *appctx) {
  struct stream_interface *si = appctx->owner;
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
  char *frame;
  int ret;
...
static int spoe_handle_sending_frame_appctx(struct appctx *appctx, int *skip) {
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
...
static int spoe_handle_processing_appctx(struct appctx *appctx) {
  ...
  SPOE_PRINTF(
      ...
      SPOE_APPCTX(appctx)->cur_fpa, agent->max_fpa,
      spoe_appctx_state_str[appctx->st0], SPOE_APPCTX(appctx)->node.key,
...
static int spoe_handle_disconnect_appctx(struct appctx *appctx) {
  struct stream_interface *si = appctx->owner;
  struct spoe_agent *agent = SPOE_APPCTX(appctx)->agent;
  char *frame, *buf;
  int ret;
  ...
  default:
    SPOE_PRINTF(stderr,
                ...
                (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__,
                appctx, SPOE_APPCTX(appctx)->status_code,
...
static int spoe_handle_disconnecting_appctx(struct appctx *appctx) {
  struct stream_interface *si = appctx->owner;
  char *frame;
  int ret;
...
switchstate:
  SPOE_PRINTF(stderr,
              ...
              " - appctx-state=%s\n",
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__,
              appctx, spoe_appctx_state_str[appctx->st0]);
  ...
  case SPOE_APPCTX_ST_IDLE:
    ...
    if (stopping && LIST_ISEMPTY(&agent->rt[tid].sending_queue) &&
...
static struct appctx *spoe_create_appctx(struct spoe_config *conf) {
  ...
  SPOE_APPCTX(appctx)->buffer_wait.wakeup_cb =
      (int (*)(void *))spoe_wakeup_appctx;
...
static int spoe_encode_message(struct stream *s, struct spoe_context *ctx,
                               struct spoe_message *msg, int dir, char **buf,
                               char *end) {
  struct sample *smp;
  struct spoe_arg *arg;
  int ret;
...
encode_msg_payload:
  ...
  encode_arg_value:
    ...
    smp =
        sample_process(s->be, s->sess, s, dir | SMP_OPT_FINAL, arg->expr, NULL);
    if (smp) {
...
static int spoe_encode_messages(struct stream *s, struct spoe_context *ctx,
  ...
  SPOE_PRINTF(stderr,
              ...
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
              ((ctx->flags & SPOE_CTX_FL_FRAGMENTED) ? "last fragment of"
                                                     : "unfragmented"),
...
too_big:
  ...
  SPOE_PRINTF(stderr,
              ...
              " - max_size=%u - encoded=%ld\n",
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
              ctx->spoe_appctx, ctx->frag_ctx.curmsg, ctx->frag_ctx.curarg,
              ctx->frag_ctx.curoff,
              (agent->rt[tid].frame_size - FRAME_HDR_SIZE),
              p - b_head(&ctx->buffer));
...
static void spoe_set_var(struct spoe_context *ctx, char *scope, char *name,
                         int len, struct sample *smp) {
  struct spoe_config *conf = FLT_CONF(ctx->filter);
  struct spoe_agent *agent = conf->agent;
  char varname[64];

  memset(varname, 0, sizeof(varname));
  len = snprintf(varname, sizeof(varname), "%s.%s.%.*s", scope, agent->var_pfx,
                 len, name);
...
static void spoe_unset_var(struct spoe_context *ctx, char *scope, char *name,
                           int len, struct sample *smp) {
  struct spoe_config *conf = FLT_CONF(ctx->filter);
  struct spoe_agent *agent = conf->agent;
  char varname[64];
...
static inline int spoe_decode_action_set_var(struct stream *s,
  ...
  switch (*p++) {
  case SPOE_SCOPE_PROC:
    scope = "proc";
    break;
  case SPOE_SCOPE_SESS:
    scope = "sess";
    break;
  case SPOE_SCOPE_TXN:
    scope = "txn";
...
static inline int spoe_decode_action_unset_var(struct stream *s,
                                               struct spoe_context *ctx,
                                               char **buf, char *end, int dir) {
  char *str, *scope, *p = *buf;
  struct sample smp;
...
static int spoe_process_actions(struct stream *s, struct spoe_context *ctx,
                                int dir) {
  char *p, *end;
  int ret;
...
static void spoe_update_stats(struct stream *s, struct spoe_agent *agent,
  ...
  if (agent->var_t_process) {
    ...
    spoe_set_var(ctx, "txn", agent->var_t_process, strlen(agent->var_t_process),
                 &smp);
  }
  ...
  if (agent->var_t_total) {
    ...
    spoe_set_var(ctx, "txn", agent->var_t_total, strlen(agent->var_t_total),
                 &smp);
  }
}

static void spoe_handle_processing_error(struct stream *s,
                                         struct spoe_agent *agent,
                                         struct spoe_context *ctx, int dir) {
  if (agent->eps_max > 0)
  ...
  if (agent->var_on_error) {
    ...
    spoe_set_var(ctx, "txn", agent->var_on_error, strlen(agent->var_on_error),
                 &smp);
  }

  ctx->state = ((agent->flags & SPOE_FL_CONT_ON_ERR) ? SPOE_CTX_ST_READY
                                                     : SPOE_CTX_ST_NONE);
}

static inline int spoe_start_processing(struct spoe_agent *agent,
                                        struct spoe_context *ctx, int dir) {
   ...
   * in same time. */
  ctx->flags |= ((dir == SMP_OPT_DIR_REQ) ? SPOE_CTX_FL_REQ_PROCESS
                                          : SPOE_CTX_FL_RSP_PROCESS);
  return 1;
}

static inline void spoe_stop_processing(struct spoe_agent *agent,
                                        struct spoe_context *ctx) {
...
static int spoe_process_messages(struct stream *s, struct spoe_context *ctx,
  ...
  if (ctx->state == SPOE_CTX_ST_READY) {
    ...
    if (!tick_isset(ctx->process_exp)) {
      ...
      s->task->expire = tick_first(
          (tick_is_expired(s->task->expire, now_ms) ? 0 : s->task->expire),
          ctx->process_exp);
    }
...
static int spoe_process_group(struct stream *s, struct spoe_context *ctx,
  ...
  SPOE_PRINTF(stderr,
              ...
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
              spoe_ctx_state_str[ctx->state], group->id);

  if (LIST_ISEMPTY(&group->messages))
    return 1;

  ret =
      spoe_process_messages(s, ctx, &group->messages, dir, SPOE_MSGS_BY_GROUP);
  if (ret && ctx->stats.t_process != -1) {
    SPOE_PRINTF(stderr,
                "%d.%06d [SPOE/%-15s] %s: stream=%p"
                " - <GROUP:%s> sid=%u st=%u %ld/%ld/%ld/%ld/%ld %u/%u %u/%u "
                "%llu/%llu %u/%u\n",
                (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
                group->id, s->uniq_id, ctx->status_code, ctx->stats.t_request,
                ctx->stats.t_queue, ctx->stats.t_waiting, ctx->stats.t_response,
                ctx->stats.t_process, agent->counters.idles,
                agent->counters.applets, agent->counters.nb_sending,
                agent->counters.nb_waiting, agent->counters.nb_errors,
                agent->counters.nb_processed, agent->rt[tid].processing,
                read_freq_ctr(&agent->rt[tid].processing_per_sec));
    if (ctx->status_code || !(conf->agent_fe.options2 & PR_O2_NOLOGNORM))
      send_log(&conf->agent_fe, (!ctx->status_code ? LOG_NOTICE : LOG_WARNING),
               "SPOE: [%s] <GROUP:%s> sid=%u st=%u %ld/%ld/%ld/%ld/%ld %u/%u "
               "%u/%u %llu/%llu\n",
...
static int spoe_process_event(struct stream *s, struct spoe_context *ctx,
  ...
  SPOE_PRINTF(stderr,
              ...
              (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
              spoe_ctx_state_str[ctx->state], spoe_event_str[ev]);
  ...
  ret = spoe_process_messages(s, ctx, &(ctx->events[ev]), dir,
                              SPOE_MSGS_BY_EVENT);
  if (ret && ctx->stats.t_process != -1) {
    SPOE_PRINTF(stderr,
                "%d.%06d [SPOE/%-15s] %s: stream=%p"
                " - <EVENT:%s> sid=%u st=%u %ld/%ld/%ld/%ld/%ld %u/%u %u/%u "
                "%llu/%llu %u/%u\n",
                (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s,
                spoe_event_str[ev], s->uniq_id, ctx->status_code,
                ctx->stats.t_request, ctx->stats.t_queue, ctx->stats.t_waiting,
                ctx->stats.t_response, ctx->stats.t_process,
                agent->counters.idles, agent->counters.applets,
                agent->counters.nb_sending, agent->counters.nb_waiting,
    ...
    if (ctx->status_code || !(conf->agent_fe.options2 & PR_O2_NOLOGNORM))
      send_log(&conf->agent_fe, (!ctx->status_code ? LOG_NOTICE : LOG_WARNING),
               "SPOE: [%s] <EVENT:%s> sid=%u st=%u %ld/%ld/%ld/%ld/%ld %u/%u "
               "%u/%u %llu/%llu\n",
...
static struct spoe_context *spoe_create_context(struct stream *s,
...
static void spoe_sig_stop(struct sig_handler *sh) {
  struct proxy *p;
...
static int spoe_check(struct proxy *px, struct flt_conf *fconf) {
  ...
  if (target->mode != PR_MODE_TCP) {
    ha_alert("Proxy %s : backend '%s' used by SPOE agent '%s' declared"
             ...
             px->id, target->id, conf->agent->id, conf->agent->conf.file,
  ...
  if ((conf->agent->rt = calloc(global.nbthread, sizeof(*conf->agent->rt))) ==
      NULL) {
    ha_alert("Proxy %s : out of memory initializing SPOE agent '%s' declared "
             "at %s:%d.\n",
...
static int spoe_init_per_thread(struct proxy *p, struct flt_conf *fconf) {
  struct spoe_config *conf = fconf->conf;
  struct spoe_agent *agent = conf->agent;
...
static int spoe_start(struct stream *s, struct filter *filter) {
  ...
  SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: stream=%p\n", (int)now.tv_sec,
              (int)now.tv_usec, agent->id, __FUNCTION__, s);
...
static void spoe_stop(struct stream *s, struct filter *filter) {
  SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: stream=%p\n", (int)now.tv_sec,
              (int)now.tv_usec,
              ((struct spoe_config *)FLT_CONF(filter))->agent->id, __FUNCTION__,
              s);
...
static int spoe_start_analyze(struct stream *s, struct filter *filter,
                              struct channel *chn) {
  struct spoe_context *ctx = filter->ctx;
  int ret = 1;

  SPOE_PRINTF(stderr,
              "%d.%06d [SPOE/%-15s] %s: stream=%p - ctx-state=%s"
              " - ctx-flags=0x%08x\n",
              (int)now.tv_sec, (int)now.tv_usec,
              ((struct spoe_config *)FLT_CONF(filter))->agent->id, __FUNCTION__,
              s, spoe_ctx_state_str[ctx->state], ctx->flags);
...
static int spoe_chn_pre_analyze(struct stream *s, struct filter *filter,
  ...
  SPOE_PRINTF(stderr,
              ...
              (int)now.tv_sec, (int)now.tv_usec,
              ((struct spoe_config *)FLT_CONF(filter))->agent->id, __FUNCTION__,
              s, spoe_ctx_state_str[ctx->state], ctx->flags, an_bit);
...
static int spoe_end_analyze(struct stream *s, struct filter *filter,
                            struct channel *chn) {
  struct spoe_context *ctx = filter->ctx;
...
static int cfg_parse_spoe_agent(const char *file, int linenum, char **args,
                                int kwm) {
  const char *err;
  int i, err_code = 0;
  ...
  if (!strcmp(args[0], "spoe-agent")) { /* new spoe-agent section */
    ...
    if (curagent != NULL) {
      ha_alert(
          "parsing [%s:%d] : another spoe-agent section previously defined.\n",
          file, linenum);
      err_code |= ERR_ALERT | ERR_ABORT;
      goto out;
    }
    ...
    curagent->flags =
        (SPOE_FL_ASYNC | SPOE_FL_PIPELINING | SPOE_FL_SND_FRAGMENTATION);
    curagent->cps_max = 0;
    curagent->eps_max = 0;
    ...
    if (!*args[1]) {
      ha_alert("parsing [%s:%d] : 'timeout' expects 'hello', 'idle' and "
               "'processing'.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!strcmp(args[1], "hello"))
      ...
      ha_alert("parsing [%s:%d] : 'timeout' supports 'hello', 'idle' or "
               "'processing' (got %s).\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!*args[2]) {
      ha_alert("parsing [%s:%d] : 'timeout %s' expects an integer value (in "
               "milliseconds).\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (res == PARSE_TIME_OVER) {
      ...
      ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to <%s %s>, "
    ...
    *tv = MS_TO_TICKS(timeout);
    ...
    if (!strcmp(args[1], "var-prefix")) {
      ...
      while (*tmp) {
        if (!isalnum((unsigned char)*tmp) && *tmp != '_' && *tmp != '.') {
          ha_alert(
              "parsing [%s:%d]: '%s %s' only supports [a-zA-Z0-9_.] chars.\n",
    ...
    if (curagent->max_frame_size < MIN_FRAME_SIZE ||
        curagent->max_frame_size > MAX_FRAME_SIZE) {
      ha_alert("parsing [%s:%d] : '%s' expects a positive integer argument in "
               "the range [%d, %d].\n",
...
static int cfg_parse_spoe_group(const char *file, int linenum, char **args,
                                int kwm) {
...
static int cfg_parse_spoe_message(const char *file, int linenum, char **args,
                                  int kwm) {
  struct spoe_message *msg;
  struct spoe_arg *arg;
  const char *err;
  ...
  if (!strcmp(args[0], "spoe-message")) { /* new spoe-message section */
    ...
    while (*args[cur_arg]) {
      ...
      arg->expr =
          sample_parse_expr((char *[]){delim, NULL}, &idx, file, linenum,
                            &errmsg, &curproxy->conf.args, NULL);
      if (arg->expr == NULL) {
        ha_alert("parsing [%s:%d] : '%s': %s.\n", file, linenum, args[0],
                 errmsg);
        err_code |= ERR_ALERT | ERR_FATAL;
    ...
    if (strcmp(args[2], "if") == 0 || strcmp(args[2], "unless") == 0) {
      ...
      cond = build_acl_cond(file, linenum, &curmsg->acls, curproxy,
                            (const char **)args + 2, &errmsg);
      if (cond == NULL) {
    ...
    ha_alert(
        "parsing [%s:%d] : unknown keyword '%s' in spoe-message section.\n",
        file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  }
...
static int parse_spoe_flt(char **args, int *cur_arg, struct proxy *px,
  ...
  while (*args[pos]) {
    if (!strcmp(args[pos], "config")) {
      if (!*args[pos + 1]) {
        memprintf(err, "'%s' : '%s' option without value", args[*cur_arg],
                  args[pos]);
        goto error;
      }
      file = args[pos + 1];
  ...
  if (ret == -1) {
    memprintf(err, "Could not open configuration file %s : %s", file,
              strerror(errno));
    goto error;
  }
  ...
  if (curagent->timeout.hello == TICK_ETERNITY ||
      ...
      curagent->timeout.processing == TICK_ETERNITY) {
    ha_warning(
        "Proxy '%s': missing timeouts for SPOE agent '%s' declare at %s:%d.\n"
        ...
        "following\n"
        "   | timeouts are set to a non-zero value: 'hello', 'idle', "
        "'processing'.\n",
  ...
  if (curagent->var_pfx == NULL) {
    ...
    while (*tmp) {
      if (!isalnum((unsigned char)*tmp) && *tmp != '_' && *tmp != '.') {
        memprintf(err,
                  "Invalid variable prefix '%s' for SPOE agent '%s' declared "
                  "at %s:%d. "
                  "Use 'option var-prefix' to set it. Only [a-zA-Z0-9_.] chars "
                  "are supported.\n",
                  curagent->id, curagent->id, curagent->conf.file,
                  curagent->conf.line);
        goto error;
      }
      tmp++;
  ...
  if (curagent->var_on_error) {
    ...
    arg.data.str.size =
        0; /* Set it to 0 to not release it in vars_check_args() */
  ...
  if (LIST_ISEMPTY(&curmphs) && LIST_ISEMPTY(&curgphs)) {
    ha_warning("Proxy '%s': No message/group used by SPOE agent '%s' declared "
               "at %s:%d.\n",
               px->id, curagent->id, curagent->conf.file, curagent->conf.line);
    goto finish;
  }
  ...
  list_for_each_entry(ph, &curmphs, list) {
    list_for_each_entry(msg, &curmsgs, list) {
      ...
      if (!strcmp(msg->id, ph->id)) {
        ...
        if (!(px->cap & PR_CAP_FE) && (msg->event == SPOE_EV_ON_CLIENT_SESS ||
                                       ...
                                       msg->event == SPOE_EV_ON_HTTP_REQ_FE)) {
          ha_warning(
              "Proxy '%s': frontend event used on a backend proxy at %s:%d.\n",
              px->id, msg->conf.file, msg->conf.line);
          goto next_mph;
        }
        if (msg->event == SPOE_EV_NONE) {
          ha_warning(
              "Proxy '%s': Ignore SPOE message '%s' without event at %s:%d.\n",
    ...
    memprintf(err,
              "SPOE agent '%s' try to use undefined SPOE message '%s' at %s:%d",
  ...
  list_for_each_entry(ph, &curgphs, list) {
    ...
    memprintf(err,
              "SPOE agent '%s' try to use undefined SPOE group '%s' at %s:%d",
              curagent->id, ph->id, curagent->conf.file, curagent->conf.line);
    goto error;
  next_aph:
    continue;
    ...
    list_for_each_entry_safe(ph, phback, &grp->phs, list) {
      list_for_each_entry(msg, &curmsgs, list) {
        if (!strcmp(msg->id, ph->id)) {
          if (msg->group != NULL) {
            memprintf(err,
                      "SPOE message '%s' already belongs to "
                      "the SPOE group '%s' declare at %s:%d",
                      msg->id, msg->group->id, msg->group->conf.file,
                      msg->group->conf.line);
            goto error;
          }
      ...
      memprintf(
          err,
          "SPOE group '%s' try to use undefined SPOE message '%s' at %s:%d",
 ...
 *
 * It returns ACT_RET_CONT if processing is finished (with error or not), it
 * returns ACT_RET_YIELD if the action is in progress. */
static enum act_return spoe_send_group(struct act_rule *rule, struct proxy *px,
  ...
  default:
    SPOE_PRINTF(stderr,
                ...
                (int)now.tv_sec, (int)now.tv_usec, agent->id, __FUNCTION__, s);
    send_log(px, LOG_ERR,
             "SPOE: [%s] internal error while execute spoe-send-group\n",
             agent->id);
    return ACT_RET_CONT;
  }
 ...
 * filled.
 */
static int check_send_spoe_group(struct act_rule *rule, struct proxy *px,
                                 char **err) {
  struct flt_conf *fconf;
  struct spoe_config *conf;
  struct spoe_agent *agent = NULL;
  ...
  if (agent == NULL) {
    memprintf(
        err, "unable to find SPOE engine '%s' used by the send-spoe-group '%s'",
        engine_id, group_id);
    goto error;
  }
  ...
  if (&group->list == &agent->groups) {
    memprintf(
        err,
        "unable to find SPOE group '%s' into SPOE engine '%s' configuration",
        group_id, engine_id);
    goto error;
  }
  ...
  list_for_each_entry(msg, &group->messages, by_grp) {
    ...
    list_for_each_entry(arg, &msg->args, list) {
      if (!(arg->expr->fetch->val & where)) {
        memprintf(err,
                  "Invalid SPOE message '%s' used by SPOE group '%s' at %s:%d: "
                  "some args extract information from '%s',"
                  "none of which is available here ('%s')",
 ...
 */
static enum act_parse_ret parse_send_spoe_group(const char **args,
                                                int *orig_arg, struct proxy *px,
                                                struct act_rule *rule,
                                                char **err) {
  if (!*args[*orig_arg] || !*args[*orig_arg + 1] ||
      (*args[*orig_arg + 2] && strcmp(args[*orig_arg + 2], "if") != 0 &&
       strcmp(args[*orig_arg + 2], "unless") != 0)) {
