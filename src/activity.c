...
static int cfg_parse_prof_tasks(char **args, int section_type,
  ...
  if (strcmp(args[1], "on") == 0)
    ...
    memprintf(err, "'%s' expects either 'on', 'auto', or 'off' but got '%s'.",
              args[0], args[1]);
    return -1;
  }
  return 0;
...
static int cli_parse_set_profiling(char **args, char *payload,
  ...
  if (strcmp(args[3], "on") == 0) {
    ...
    while (!_HA_ATOMIC_CAS(&profiling, &old,
                           (old & ~HA_PROF_TASKS_MASK) | HA_PROF_TASKS_ON))
      ;
  } else if (strcmp(args[3], "auto") == 0) {
    unsigned int old = profiling;
    while (!_HA_ATOMIC_CAS(&profiling, &old,
                           (old & ~HA_PROF_TASKS_MASK) | HA_PROF_TASKS_AUTO))
      ;
  } else if (strcmp(args[3], "off") == 0) {
    unsigned int old = profiling;
    while (!_HA_ATOMIC_CAS(&profiling, &old,
                           (old & ~HA_PROF_TASKS_MASK) | HA_PROF_TASKS_OFF))
      ;
  } else
...
static int cli_io_handler_show_profiling(struct appctx *appctx) {
  ...
  case HA_PROF_TASKS_AUTO:
    str = "auto";
    break;
  case HA_PROF_TASKS_ON:
    str = "on";
    break;
  default:
    str = "off";
  ...
  chunk_printf(&trash,
               "Per-task CPU profiling              : %s      # set profiling "
               "tasks {on|auto|off}\n",
               str);
...
static struct cfg_kw_list cfg_kws = {
    ILH,
    {{CFG_GLOBAL, "profiling.tasks", cfg_parse_prof_tasks}, {0, NULL, NULL}}};
