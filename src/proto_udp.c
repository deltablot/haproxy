 ...
 *
 * Copyright 2019 HAProxy Technologies, Frédéric Lécaille
...
static int udp_bind_listener(struct listener *listener, char *errmsg,
                             int errlen);
 ...
 * still be valid.
 */
static void udp_disable_listener(struct listener *l) { fd_stop_recv(l->rx.fd); }
