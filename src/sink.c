...
struct sink *sink_find(const char *name) {
  ...
  list_for_each_entry(sink, &sink_list,
                      sink_list) if (strcmp(sink->name, name) == 0) return sink;
  return NULL;
}
 ...
 */
struct sink *sink_new_fd(const char *name, const char *desc, enum log_fmt fmt,
                         int fd) {
  struct sink *sink;
 ...
 * largest one is kept.
 */
struct sink *sink_new_buf(const char *name, const char *desc, enum log_fmt fmt,
                          size_t size) {
  struct sink *sink;
...
send:
  if (sink->type == SINK_TYPE_FD) {
    return fd_write_frag_line(sink->ctx.fd, sink->maxlen, pfx, npfx, msg, nmsg,
                              1);
...
/* parse the "show events" command, returns 1 if a message is returned,
 * otherwise zero */
static int cli_parse_show_events(char **args, char *payload,
  ...
  if (!*args[1]) {
    ...
    chunk_printf(&trash, "Supported events sinks are listed below. Add "
                         "-w(wait), -n(new). Any key to stop\n");
...
sink_forward_session_create(struct sink *sink,
 ...
 */
static struct task *process_sink_forward(struct task *task, void *context,
...
int cfg_parse_ring(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "ring") == 0) { /* new peers section */
    ...
    if (inv) {
      ha_alert("parsing [%s:%d] : invalid ring name '%s' (character '%c' is "
               "not permitted).\n",
    ...
    if (!cfg_sink || cfg_sink->type != SINK_TYPE_BUFFER) {
      ha_alert("parsing [%s:%d] : unable to create a new sink buffer for ring "
               "'%s'.\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto err;
    ...
    err_code |=
        parse_server(file, linenum, args, cfg_sink->forward_px, NULL, 1, 0, 1);
    ...
    if (strcmp(args[1], "connect") == 0 || strcmp(args[1], "server") == 0) {
      const char *res;
      unsigned int tout;
...
int cfg_post_parse_ring() {
  ...
  if (cfg_sink && (cfg_sink->type == SINK_TYPE_BUFFER)) {
    if (cfg_sink->maxlen > b_size(&cfg_sink->ctx.ring->buf)) {
      ha_warning("ring '%s' event max length '%u' exceeds size, forced to size "
                 "'%lu'.\n",
                 cfg_sink->name, cfg_sink->maxlen,
                 (unsigned long)b_size(&cfg_sink->ctx.ring->buf));
    ...
    if (cfg_sink->forward_px) {
      ...
      while (srv) {
        struct sink_forward_target *sft;
        /* init ssl if needed */
        if (srv->use_ssl == 1 && xprt_get(XPRT_SSL) &&
        ...
        if (!sft) {
          ha_alert("memory allocation error initializing server '%s' in ring "
                   "'%s'.\n",
                   srv->id, cfg_sink->name);
          err_code |= ERR_ALERT | ERR_FATAL;
          break;
