 ...
 * Master Worker
 *
 * Copyright HAProxy Technologies 2019 - William Lallemand
...
static int max_reloads =
    -1; /* number max of reloads a worker can have until they are killed */
...
static void mworker_kill(int sig) {
  ...
  list_for_each_entry(child, &proc_list, list) {
    /* careful there, we must be sure that the pid > 0, we don't want to emit a
     * kill -1 */
...
int mworker_current_child(int pid) {
  ...
  list_for_each_entry(child, &proc_list, list) {
    if ((child->options & (PROC_O_TYPE_WORKER | PROC_O_TYPE_PROG)) &&
        (!(child->options & PROC_O_LEAVING)) && (child->pid == pid))
      return 1;
  }
  return 0;
}
...
void mworker_proc_list_to_env() {
  ...
  list_for_each_entry(child, &proc_list, list) {
    ...
    if (child->pid > -1)
      memprintf(&msg,
                "%s|type=%c;fd=%d;pid=%d;rpid=%d;reloads=%d;timestamp=%d;id=%s;"
                "version=%s",
                msg ? msg : "", type, child->ipc_fd[0], child->pid,
                child->relative_pid, child->reloads, child->timestamp,
                child->id ? child->id : "", child->version);
  }
  if (msg)
...
void mworker_unblock_signals() { haproxy_unblock_signals(); }

/* ----- mworker signal handlers ----- */

/* broadcast the configured signal to the workers */
void mworker_broadcast_signal(struct sig_handler *sh) { mworker_kill(sh->arg); }

/*
 * When called, this function reexec haproxy with -sf followed by current
 * children PIDs and possibly old children PIDs if they didn't leave yet.
 */
void mworker_catch_sighup(struct sig_handler *sh) { mworker_reload(); }
...
restart_wait:
  ...
  if (exitpid > 0) {
    ...
    if (!childfound) {
      /* We didn't find the PID in the list, that shouldn't happen but we can
       * emit a warning */
      ha_warning("Process %d exited with code %d (%s)\n", exitpid, status,
                 (status >= 128) ? strsignal(status - 128) : "Exit");
    } else {
      ...
      if (!(child->options & PROC_O_LEAVING)) {
        ...
        /* 0 & SIGTERM (143) are normal, but we should report SIGINT (130) and
         * other signals */
  ...
  /* Better rely on the system than on a list of process to check if it was the
     last one */
    ...
    atexit_flag = 0;
    if (exitcode > 0)
      exit(exitcode); /* parent must leave using the status code that provoked
                         the exit */
    exit(EXIT_SUCCESS);
  }
}
...
static int mworker_pipe_register_per_thread() {
   ...
   */
  fd_insert(proc_self->ipc_fd[1], fdtab[proc_self->ipc_fd[1]].owner,
            mworker_accept_wrapper, tid_bit);
...
/* ----- proxies ----- */
/*
 * Upon a reload, the master worker needs to close all listeners FDs but the
 * mworker_pipe fd, and the FD provided by fd@
 */
...
static int cli_io_handler_show_proc(struct appctx *appctx) {
  ...
  chunk_printf(&trash, "#%-14s %-15s %-15s %-15s %-15s %-15s\n", "<PID>",
               ...
               "<version>");
  memprintf(&uptime, "%dd%02dh%02dm%02ds", up / 86400, (up % 86400) / 3600,
            (up % 3600) / 60, (up % 60));
  chunk_appendf(&trash, "%-15u %-15s %-15u %-15d %-15s %-15s\n",
                (unsigned int)getpid(), "master", 0, proc_self->reloads, uptime,
                haproxy_version);
  free(uptime);
  uptime = NULL;
  ...
  list_for_each_entry(child, &proc_list, list) {
    ...
    memprintf(&uptime, "%dd%02dh%02dm%02ds", up / 86400, (up % 86400) / 3600,
              (up % 3600) / 60, (up % 60));
    chunk_appendf(&trash, "%-15u %-15s %-15u %-15d %-15s %-15s\n", child->pid,
                  "worker", child->relative_pid, child->reloads, uptime,
                  child->version);
    free(uptime);
    uptime = NULL;
  }
  ...
  if (old) {
    ...
    list_for_each_entry(child, &proc_list, list) {
      ...
      if (child->options & PROC_O_LEAVING) {
        ...
        memprintf(&uptime, "%dd%02dh%02dm%02ds", up / 86400,
...
static int cli_parse_reload(char **args, char *payload, struct appctx *appctx,
                            void *private) {
...
static int mworker_parse_global_max_reloads(char **args, int section_type,
                                            struct proxy *curpx,
                                            struct proxy *defpx,
  ...
  if (max_reloads < 0) {
    memprintf(err, "%sparsing [%s:%d] '%s' : invalid value %d, must be >= 0",
              *err, file, linenum, args[0], max_reloads);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  }
