...
void fwlc_init_server_tree(struct proxy *p) {
  ...
  for (srv = p->srv; srv; srv = srv->next) {
    if (!srv_currently_usable(srv))
      continue;
    srv->lb_tree =
        (srv->flags & SRV_F_BACKUP) ? &p->lbprm.fwlc.bck : &p->lbprm.fwlc.act;
    fwlc_queue_srv(srv, srv->next_eweight);
  }
}
 ...
 * The server's lock must be held. The lbprm's lock will be used.
 */
struct server *fwlc_get_next_server(struct proxy *p,
  ...
  while (node) {
    ...
    if (!s->maxconn ||
        s->served + s->nbpend < srv_dynamic_maxconn(s) + s->maxqueue) {
      if (s != srvtoavoid) {
        srv = s;
        break;
      }
