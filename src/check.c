...
struct analyze_status {
  ...
  unsigned char
      lr[HANA_OBS_SIZE]; /* result for l4/l7: 0 = ignore, 1 - error, 2 - OK */
};
...
static const struct check_status check_statuses[HCHK_STATUS_SIZE] = {
    ...
    [HCHK_STATUS_L4CON] = {CHK_RES_FAILED, "L4CON",
    ...
    [HCHK_STATUS_L7OKCD] = {CHK_RES_CONDPASS, "L7OKC",
    ...
    [HCHK_STATUS_PROCTOUT] = {CHK_RES_FAILED, "PROCTOUT",
...
static inline int unclean_errno(int err) {
  if (err == EAGAIN || err == EINPROGRESS || err == EISCONN || err == EALREADY)
    return 0;
  return err;
}
 ...
 */
void set_server_check_status(struct check *check, short status,
                             const char *desc) {
  ...
  if (s->proxy->options2 & PR_O2_LOGHCHKS &&
    ...
    chunk_printf(&trash, "%s check for %sserver %s/%s %s%s",
    ...
    chunk_appendf(&trash, ", status: %d/%d %s",
                  (check->health >= check->rise)
                      ? check->health - check->rise + 1
                      : check->health,
                  (check->health >= check->rise) ? check->fall : check->rise,
                  (check->health >= check->rise) ? (s->uweight ? "UP" : "DRAIN")
                                                 : "DOWN");
...
void check_notify_failure(struct check *check) {
  ...
  srv_set_stopped(s, NULL,
                  (!s->track && !(s->proxy->options2 & PR_O2_LOGHCHKS)) ? check
                                                                        : NULL);
}
...
void check_notify_success(struct check *check) {
  ...
  srv_set_running(s, NULL,
                  (!s->track && !(s->proxy->options2 & PR_O2_LOGHCHKS)) ? check
                                                                        : NULL);
}

/* Marks the check <check> as valid and tries to set its server into stopping
 * mode if it was running or starting, and provided it isn't in maintenance and
 * other checks comply. The conditions for the server to be marked in stopping
 * mode are the same as for it to be turned up. Also, only the health checks
 * support the nolb mode.
 */
...
void __health_adjust(struct server *s, short status) {
  ...
  case HANA_ONERR_FAILCHK:
    ...
    set_server_check_status(&s->check, HCHK_STATUS_HANA, trash.area);
...
void chk_report_conn_err(struct check *check, int errno_bck, int expired) {
  ...
  if (check->type == PR_O2_TCPCHK_CHK &&
    ...
    if (!step)
      ...
      if (check->current_step &&
          check->current_step->action == TCPCHK_ACT_CONNECT) {
        if (check->current_step->connect.port)
          chunk_appendf(chk, " (connect port %d)",
                        check->current_step->connect.port);
        else
        ...
        case TCPCHK_EXPECT_STRING:
          chunk_appendf(chk, " (expect string '%.*s')",
                        (unsigned int)istlen(expect->data),
                        istptr(expect->data));
          break;
        case TCPCHK_EXPECT_BINARY:
          chunk_appendf(chk, " (expect binary '%.*s')",
                        (unsigned int)istlen(expect->data),
                        istptr(expect->data));
          break;
        ...
        case TCPCHK_EXPECT_HTTP_BODY:
          chunk_appendf(chk, " (expect HTTP body content '%.*s')",
                        (unsigned int)istlen(expect->data),
                        istptr(expect->data));
          break;
  ...
  if (conn && conn->err_code) {
    if (unclean_errno(errno))
      chunk_printf(&trash, "%s (%s)%s", conn_err_code_str(conn),
                   strerror(errno), chk->area);
    else
      chunk_printf(&trash, "%s%s", conn_err_code_str(conn), chk->area);
    err_msg = trash.area;
  } else {
    if (unclean_errno(errno)) {
      chunk_printf(&trash, "%s%s", strerror(errno), chk->area);
      err_msg = trash.area;
    } else {
      err_msg = chk->area;
    }
  ...
  if (!conn || !conn->ctrl) {
    /* error before any connection attempt (connection allocation error or no
     * control layer) */
    set_server_check_status(check, HCHK_STATUS_SOCKERR, err_msg);
...
int httpchk_build_status_header(struct server *s, struct buffer *buf) {
  ...
  char addr[46];
  char port[6];
  const char *srv_hlt_st[7] = {"DOWN",       "DOWN %d/%d", "UP %d/%d", "UP",
                               "NOLB %d/%d", "NOLB",       "no check"};
  ...
  chunk_appendf(
      buf, srv_hlt_st[sv_state],
      (s->cur_state != SRV_ST_STOPPED) ? (s->check.health - s->check.rise + 1)
                                       : (s->check.health),
  ...
  chunk_appendf(
      buf,
      "; address=%s; port=%s; name=%s/%s; node=%s; weight=%d/%d; scur=%d/%d; "
      "qcur=%d",
      addr, port, s->proxy->id, s->id, global.node,
      (s->cur_eweight * s->proxy->lbprm.wmult + s->proxy->lbprm.wdiv - 1) /
          s->proxy->lbprm.wdiv,
      (s->proxy->lbprm.tot_weight * s->proxy->lbprm.wmult +
       s->proxy->lbprm.wdiv - 1) /
          s->proxy->lbprm.wdiv,
      s->cur_sess, s->proxy->beconn - s->proxy->nbpend, s->nbpend);
...
static struct task *event_srv_chk_io(struct task *t, void *ctx,
 ...
 */
static struct task *process_chk_conn(struct task *t, void *context,
                                     unsigned short state) {
...
static int start_check_task(struct check *check, int mininter, int nbcheck,
                            int srvpos) {
  struct task *t;
 ...
 */
static struct task *server_warmup(struct task *t, void *context,
                                  unsigned short state) {
  struct server *s = context;

  /* by default, plan on stopping the task */
  t->expire = TICK_ETERNITY;
  if ((s->next_admin & SRV_ADMF_MAINT) || (s->next_state != SRV_ST_STARTING))
    return t;
      ...
      if (s->slowstart) {
        ...
        if (s->next_state == SRV_ST_STARTING)
          task_schedule(
              ...
              tick_add(
                  now_ms,
                  MS_TO_TICKS(MAX(1000, (now.tv_sec - s->last_change)) / 20)));
      }
...
/* Initializes an health-check attached to the server <srv>. Non-zero is
...
static int init_srv_check(struct server *srv) {
  ...
  if (srv->mux_proto && !srv->check.mux_proto &&
      ((srv->mux_proto->mode == PROTO_MODE_HTTP &&
        check_type == TCPCHK_RULES_HTTP_CHK) ||
       (srv->mux_proto->mode == PROTO_MODE_TCP &&
        check_type != TCPCHK_RULES_HTTP_CHK))) {
    srv->check.mux_proto = srv->mux_proto;
  }
  /* test that check proto is valid if explicitly defined */
  else if (srv->check.mux_proto &&
           ((srv->check.mux_proto->mode == PROTO_MODE_HTTP &&
             check_type != TCPCHK_RULES_HTTP_CHK) ||
            (srv->check.mux_proto->mode == PROTO_MODE_TCP &&
             check_type == TCPCHK_RULES_HTTP_CHK))) {
    ha_alert("config: %s '%s': server '%s' uses an incompatible MUX protocol "
             "for the selected check type\n",
  ...
  if ((srv_check_healthcheck_port(&srv->check) != 0) ||
      (!is_inet_addr(&srv->check.addr) &&
       (is_addr(&srv->check.addr) || !is_inet_addr(&srv->addr))))
    goto init;

  if (!srv->proxy->tcpcheck_rules.list ||
      LIST_ISEMPTY(srv->proxy->tcpcheck_rules.list)) {
    ha_alert("config: %s '%s': server '%s' has neither service port nor check "
             "port.\n",
             proxy_type_str(srv->proxy), srv->proxy->id, srv->id);
  ...
  if (!r || (r->action != TCPCHK_ACT_CONNECT) ||
      (!r->connect.port && !get_host_port(&r->connect.addr))) {
    ha_alert(
        "config: %s '%s': server '%s' has neither service port nor check port "
  ...
  list_for_each_entry(r, srv->proxy->tcpcheck_rules.list, list) {
    if ((r->action == TCPCHK_ACT_CONNECT) &&
        (!r->connect.port || !get_host_port(&r->connect.addr))) {
      ha_alert("config: %s '%s': server '%s' has neither service port nor "
               "check port, "
...
static int init_srv_agent_check(struct server *srv) {
  ...
  if (err) {
    ha_alert(
        "config: %s '%s': unable to init agent-check for server '%s' (%s).\n",
        proxy_type_str(srv->proxy), srv->proxy->id, srv->id, err);
    ret |= ERR_ALERT | ERR_ABORT;
    goto out;
  }
...
static int srv_parse_addr(char **args, int *cur_arg, struct proxy *curpx,
                          struct server *srv, char **errmsg) {
...
static int srv_parse_agent_addr(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  int err_code = 0;
  ...
  if (str2ip(args[*cur_arg + 1], &srv->agent.addr) == NULL) {
    memprintf(errmsg,
              "parsing agent-addr failed. Check if '%s' is correct address.",
              args[*cur_arg + 1]);
    goto error;
  }
...
static int srv_parse_agent_check(char **args, int *cur_arg, struct proxy *curpx,
                                 struct server *srv, char **errmsg) {
  struct tcpcheck_ruleset *rs = NULL;
  ...
  chk = parse_tcpcheck_send(
      (char *[]){"tcp-check", "send-lf", "%[var(check.agent_string)]", ""}, 1,
      curpx, &rs->rules, srv->conf.file, srv->conf.line, errmsg);
  if (!chk) {
    memprintf(errmsg, "'%s': %s", args[*cur_arg], *errmsg);
    goto error;
  }
...
static int srv_parse_agent_inter(char **args, int *cur_arg, struct proxy *curpx,
                                 struct server *srv, char **errmsg) {
  const char *err = NULL;
  unsigned int delay;
  int err_code = 0;
  ...
  if (err == PARSE_TIME_OVER) {
    memprintf(errmsg,
              "timer overflow in argument <%s> to <%s> of server %s, maximum "
              "value is 2147483647 ms (~24.8 days).",
              args[*cur_arg + 1], args[*cur_arg], srv->id);
    goto error;
  } else if (err == PARSE_TIME_UNDER) {
    memprintf(errmsg,
              "timer underflow in argument <%s> to <%s> of server %s, minimum "
    ...
    memprintf(
        errmsg,
        "unexpected character '%c' in 'agent-inter' argument of server %s.",
        *err, srv->id);
    goto error;
  }
  if (delay <= 0) {
    memprintf(errmsg, "invalid value %d for argument '%s' of server %s.", delay,
              args[*cur_arg], srv->id);
    goto error;
  }
...
static int srv_parse_agent_port(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  int err_code = 0;

  if (!*(args[*cur_arg + 1])) {
    memprintf(errmsg, "'%s' expects a port number as argument.",
              args[*cur_arg]);
    goto error;
  }
...
static int srv_parse_agent_send(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  struct tcpcheck_rules *rules = srv->agent.tcpcheck_rules;
  int err_code = 0;
...
static int srv_parse_check(char **args, int *cur_arg, struct proxy *curpx,
                           struct server *srv, char **errmsg) {
  srv->do_check = 1;
  return 0;
}
...
static int srv_parse_no_check(char **args, int *cur_arg, struct proxy *curpx,
                              struct server *srv, char **errmsg) {
...
static int srv_parse_check_proto(char **args, int *cur_arg, struct proxy *px,
...
static int srv_parse_check_rise(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  int err_code = 0;
...
static int srv_parse_check_fall(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  int err_code = 0;
...
static int srv_parse_check_inter(char **args, int *cur_arg, struct proxy *curpx,
                                 struct server *srv, char **errmsg) {
  const char *err = NULL;
  unsigned int delay;
  int err_code = 0;
...
static int srv_parse_check_fastinter(char **args, int *cur_arg,
                                     struct proxy *curpx, struct server *srv,
                                     char **errmsg) {
  const char *err = NULL;
  unsigned int delay;
...
static int srv_parse_check_downinter(char **args, int *cur_arg,
                                     struct proxy *curpx, struct server *srv,
                                     char **errmsg) {
  const char *err = NULL;
  unsigned int delay;
...
static int srv_parse_check_port(char **args, int *cur_arg, struct proxy *curpx,
                                struct server *srv, char **errmsg) {
  int err_code = 0;
...
static struct srv_kw_list srv_kws = {
    ...
    {},
    {
        {"addr", srv_parse_addr, 1,
         1}, /* IP address to send health to or to probe from agent-check */
        {"agent-addr", srv_parse_agent_addr, 1,
         1}, /* Enable an auxiliary agent check */
        {"agent-check", srv_parse_agent_check, 0, 1}, /* Enable agent checks */
        {"agent-inter", srv_parse_agent_inter, 1,
         1}, /* Set the interval between two agent checks */
        {"agent-port", srv_parse_agent_port, 1,
         1}, /* Set the TCP port used for agent checks. */
        {"agent-send", srv_parse_agent_send, 1,
         1},                              /* Set string to send to agent. */
        {"check", srv_parse_check, 0, 1}, /* Enable health checks */
        {"check-proto", srv_parse_check_proto, 1,
         1}, /* Set the mux protocol for health checks  */
        {"check-send-proxy", srv_parse_check_send_proxy, 0,
         1}, /* Enable PROXY protocol for health checks */
        {"check-via-socks4", srv_parse_check_via_socks4, 0,
         1}, /* Enable socks4 proxy for health checks */
        {"no-agent-check", srv_parse_no_agent_check, 0,
         1}, /* Do not enable any auxiliary agent check */
        {"no-check", srv_parse_no_check, 0, 1}, /* Disable health checks */
        {"no-check-send-proxy", srv_parse_no_check_send_proxy, 0,
         1}, /* Disable PROXY protol for health checks */
        {"rise", srv_parse_check_rise, 1,
         1}, /* Set rise value for health checks */
        {"fall", srv_parse_check_fall, 1,
         1}, /* Set fall value for health checks */
        {"inter", srv_parse_check_inter, 1,
         1}, /* Set inter value for health checks */
        {"fastinter", srv_parse_check_fastinter, 1,
         1}, /* Set fastinter value for health checks */
        {"downinter", srv_parse_check_downinter, 1,
         1}, /* Set downinter value for health checks */
        {"port", srv_parse_check_port, 1,
         1}, /* Set the TCP port used for health checks. */
        {NULL, NULL, 0},
    }};
