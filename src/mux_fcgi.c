 ...
 *
 * Copyright (C) 2019 HAProxy Technologies, Christopher Faulet
 * <cfaulet@haproxy.com>
 *
...
#define FCGI_CF_MUX_MALLOC                                                     \
  0x00000001 /* mux is blocked on lack connection's mux buffer */
#define FCGI_CF_MUX_MFULL                                                      \
  0x00000002 /* mux is blocked on connection's mux buffer full */
#define FCGI_CF_MUX_BLOCK_ANY                                                  \
  0x00000003 /* mux is blocked on connection's mux buffer full */
 ...
 * received data.
 */
#define FCGI_CF_DEM_DALLOC                                                     \
  0x00000004 /* demux blocked on lack of connection's demux buffer */
#define FCGI_CF_DEM_DFULL                                                      \
  0x00000008 /* demux blocked on connection's demux buffer full */
#define FCGI_CF_DEM_MROOM                                                      \
  0x00000010 /* demux blocked on lack of room in mux buffer */
#define FCGI_CF_DEM_SALLOC                                                     \
  0x00000020 /* demux blocked on lack of stream's rx buffer */
#define FCGI_CF_DEM_SFULL                                                      \
  0x00000040 /* demux blocked on stream request buffer full */
#define FCGI_CF_DEM_TOOMANY                                                    \
  0x00000080 /* demux blocked waiting for some conn_streams to leave */
#define FCGI_CF_DEM_BLOCK_ANY                                                  \
  0x000000F0 /* aggregate of the demux flags above except DALLOC/DFULL */

/* Other flags */
#define FCGI_CF_MPXS_CONNS 0x00000100 /* connection multiplexing is supported  \
                                       */
#define FCGI_CF_ABRTS_SENT                                                     \
  0x00000200 /* a record ABORT was successfully sent to all active streams */
#define FCGI_CF_ABRTS_FAILED                                                   \
  0x00000400 /* failed to abort processing of all streams */
#define FCGI_CF_WAIT_FOR_HS                                                    \
  0x00000800 /* We did check that at least a stream was waiting for handshake  \
              */
#define FCGI_CF_KEEP_CONN                                                      \
  0x00001000 /* HAproxy is responsible to close the connection */
...
struct fcgi_conn {
  ...
  int16_t
      max_id; /* highest ID known on this connection, <0 before mgmt records */
  uint32_t streams_limit; /* maximum number of concurrent streams the peer
                             supports */
...
#define FCGI_SF_ES_RCVD                                                        \
  0x00000001 /* end-of-stream received (empty STDOUT or EDN_REQUEST record) */
...
#define FCGI_SF_BLK_MBUSY                                                      \
  0x00000010 /* blocked waiting for mux access (transient) */
...
#define FCGI_SF_BEGIN_SENT                                                     \
  0x00000100 /* a BEGIN_REQUEST record was sent for this stream */
#define FCGI_SF_OUTGOING_DATA                                                  \
  0x00000200 /* set whenever we've seen outgoing data */
#define FCGI_SF_NOTIFIED                                                       \
  0x00000400 /* a paused stream was notified to try to send again */

#define FCGI_SF_WANT_SHUTR                                                     \
  0x00001000 /* a stream couldn't shutr() (mux full/busy) */
#define FCGI_SF_WANT_SHUTW                                                     \
  0x00002000 /* a stream couldn't shutw() (mux full/busy) */
#define FCGI_SF_KILL_CONN                                                      \
  0x00004000 /* kill the whole connection with this stream */
...
struct fcgi_strm {
  ...
  struct buffer
      rxbuf; /* receive buffer, always valid (buf_empty or real buffer) */

  struct eb32_node by_id;  /* place in fcgi_conn's streams_by_id */
  struct wait_event *subs; /* Address of the wait_event the conn_stream
                              associated is waiting on */
  struct list send_list;   /* To be used when adding in fcgi_conn->send_list */
  struct tasklet *shut_tl; /* deferred shutdown tasklet, to retry to close after
                              we failed to by lack of space */
};
...
#define FCGI_SP_URI_MASK                                                       \
  (FCGI_SP_SCRIPT_NAME | FCGI_SP_PATH_INFO | FCGI_SP_REQ_QS)
...
static void fcgi_trace(enum trace_level level, uint64_t mask,
                       ...
                       const struct ist func, const void *a1, const void *a2,
...
#define FCGI_EV_FCONN_NEW (1ULL << 0)
    {.mask = FCGI_EV_FCONN_NEW,
     .name = "fconn_new",
     .desc = "new FCGI connection"},
#define FCGI_EV_FCONN_RECV (1ULL << 1)
    {.mask = FCGI_EV_FCONN_RECV,
     .name = "fconn_recv",
     .desc = "Rx on FCGI connection"},
#define FCGI_EV_FCONN_SEND (1ULL << 2)
    {.mask = FCGI_EV_FCONN_SEND,
     .name = "fconn_send",
     .desc = "Tx on FCGI connection"},
#define FCGI_EV_FCONN_BLK (1ULL << 3)
    {.mask = FCGI_EV_FCONN_BLK,
     .name = "fconn_blk",
     .desc = "FCGI connection blocked"},
#define FCGI_EV_FCONN_WAKE (1ULL << 4)
    {.mask = FCGI_EV_FCONN_WAKE,
     .name = "fconn_wake",
     .desc = "FCGI connection woken up"},
#define FCGI_EV_FCONN_END (1ULL << 5)
    {.mask = FCGI_EV_FCONN_END,
     .name = "fconn_end",
     .desc = "FCGI connection terminated"},
#define FCGI_EV_FCONN_ERR (1ULL << 6)
    {.mask = FCGI_EV_FCONN_ERR,
     .name = "fconn_err",
     .desc = "error on FCGI connection"},

#define FCGI_EV_RX_FHDR (1ULL << 7)
    {.mask = FCGI_EV_RX_FHDR,
     .name = "rx_fhdr",
     .desc = "FCGI record header received"},
#define FCGI_EV_RX_RECORD (1ULL << 8)
    {.mask = FCGI_EV_RX_RECORD,
     .name = "rx_record",
     .desc = "receipt of any FCGI record"},
#define FCGI_EV_RX_EOI (1ULL << 9)
    {.mask = FCGI_EV_RX_EOI,
     .name = "rx_eoi",
     .desc = "receipt of end of FCGI input"},
#define FCGI_EV_RX_GETVAL (1ULL << 10)
    {.mask = FCGI_EV_RX_GETVAL,
     .name = "rx_get_values",
     .desc = "receipt of FCGI GET_VALUES_RESULT record"},
#define FCGI_EV_RX_STDOUT (1ULL << 11)
    {.mask = FCGI_EV_RX_STDOUT,
     .name = "rx_stdout",
     .desc = "receipt of FCGI STDOUT record"},
#define FCGI_EV_RX_STDERR (1ULL << 12)
    {.mask = FCGI_EV_RX_STDERR,
     .name = "rx_stderr",
     .desc = "receipt of FCGI STDERR record"},
#define FCGI_EV_RX_ENDREQ (1ULL << 13)
    {.mask = FCGI_EV_RX_ENDREQ,
     .name = "rx_end_req",
     .desc = "receipt of FCGI END_REQUEST record"},

#define FCGI_EV_TX_RECORD (1ULL << 14)
    {.mask = FCGI_EV_TX_RECORD,
     .name = "tx_record",
     .desc = "transmission of any FCGI record"},
#define FCGI_EV_TX_EOI (1ULL << 15)
    {.mask = FCGI_EV_TX_EOI,
     .name = "tx_eoi",
     .desc = "transmission of FCGI end of input"},
#define FCGI_EV_TX_BEGREQ (1ULL << 16)
    {.mask = FCGI_EV_TX_BEGREQ,
     .name = "tx_begin_request",
     .desc = "transmission of FCGI BEGIN_REQUEST record"},
#define FCGI_EV_TX_GETVAL (1ULL << 17)
    {.mask = FCGI_EV_TX_GETVAL,
     .name = "tx_get_values",
     .desc = "transmission of FCGI GET_VALUES record"},
#define FCGI_EV_TX_PARAMS (1ULL << 18)
    {.mask = FCGI_EV_TX_PARAMS,
     .name = "tx_params",
     .desc = "transmission of FCGI PARAMS record"},
#define FCGI_EV_TX_STDIN (1ULL << 19)
    {.mask = FCGI_EV_TX_STDIN,
     .name = "tx_stding",
     .desc = "transmission of FCGI STDIN record"},
#define FCGI_EV_TX_ABORT (1ULL << 20)
    {.mask = FCGI_EV_TX_ABORT,
     .name = "tx_abort",
     .desc = "transmission of FCGI ABORT record"},

#define FCGI_EV_RSP_DATA (1ULL << 21)
    {.mask = FCGI_EV_RSP_DATA,
     .name = "rsp_data",
     .desc = "parse any data of H1 response"},
#define FCGI_EV_RSP_EOM (1ULL << 22)
    {.mask = FCGI_EV_RSP_EOM,
     .name = "rsp_eom",
     .desc = "reach the end of message of H1 response"},
#define FCGI_EV_RSP_HDRS (1ULL << 23)
    {.mask = FCGI_EV_RSP_HDRS,
     .name = "rsp_headers",
     .desc = "parse headers of H1 response"},
#define FCGI_EV_RSP_BODY (1ULL << 24)
    {.mask = FCGI_EV_RSP_BODY,
     .name = "rsp_body",
     .desc = "parse body part of H1 response"},
#define FCGI_EV_RSP_TLRS (1ULL << 25)
    {.mask = FCGI_EV_RSP_TLRS,
     .name = "rsp_trailerus",
     .desc = "parse trailers of H1 response"},
...
#define FCGI_EV_FSTRM_BLK (1ULL << 27)
    {.mask = FCGI_EV_FSTRM_BLK,
     .name = "fstrm_blk",
     .desc = "FCGI stream blocked"},
#define FCGI_EV_FSTRM_END (1ULL << 28)
    {.mask = FCGI_EV_FSTRM_END,
     .name = "fstrm_end",
     .desc = "FCGI stream terminated"},
#define FCGI_EV_FSTRM_ERR (1ULL << 29)
    {.mask = FCGI_EV_FSTRM_ERR,
     .name = "fstrm_err",
     .desc = "error on FCGI stream"},

#define FCGI_EV_STRM_NEW (1ULL << 30)
    {.mask = FCGI_EV_STRM_NEW,
     .name = "strm_new",
     .desc = "app-layer stream creation"},
#define FCGI_EV_STRM_RECV (1ULL << 31)
    {.mask = FCGI_EV_STRM_RECV,
     .name = "strm_recv",
     .desc = "receiving data for stream"},
#define FCGI_EV_STRM_SEND (1ULL << 32)
    {.mask = FCGI_EV_STRM_SEND,
     .name = "strm_send",
     .desc = "sending data for stream"},
#define FCGI_EV_STRM_FULL (1ULL << 33)
    {.mask = FCGI_EV_STRM_FULL,
     .name = "strm_full",
     .desc = "stream buffer full"},
...
#define FCGI_EV_STRM_END (1ULL << 36)
    {.mask = FCGI_EV_STRM_END,
     .name = "strm_end",
     .desc = "detaching app-layer stream"},
...
#define FCGI_VERB_CLEAN 1
    {.name = "clean",
     .desc = "only user-friendly stuff, generally suitable for level \"user\""},
#define FCGI_VERB_MINIMAL 2
    {.name = "minimal",
     .desc = "report only fconn/fstrm state and flags, no real decoding"},
#define FCGI_VERB_SIMPLE 3
    {.name = "simple",
     .desc = "add request/response status line or htx info when available"},
#define FCGI_VERB_ADVANCED 4
    {.name = "advanced",
     .desc = "add header fields or record decoding when available"},
...
static struct task *fcgi_timeout_task(struct task *t, void *context,
...
static inline struct fcgi_strm *fcgi_conn_st_by_id(struct fcgi_conn *fconn,
                                                   int id);
static struct task *fcgi_deferred_shut(struct task *t, void *ctx,
                                       unsigned short state);
static struct fcgi_strm *fcgi_conn_stream_new(struct fcgi_conn *fconn,
                                              struct conn_stream *cs,
                                              struct session *sess);
...
static int fcgi_strm_send_abort(struct fcgi_conn *fconn,
...
static inline const char *fconn_st_to_str(enum fcgi_conn_st st) {
  ...
  case FCGI_CS_INIT:
    return "INI";
  case FCGI_CS_SETTINGS:
    return "STG";
  case FCGI_CS_RECORD_H:
    return "RDH";
  case FCGI_CS_RECORD_D:
    return "RDD";
  case FCGI_CS_RECORD_P:
    return "RDP";
  case FCGI_CS_CLOSED:
    return "CLO";
  default:
...
static void fcgi_trace(enum trace_level level, uint64_t mask,
  ...
  if (src->verbosity > FCGI_VERB_MINIMAL && htx && htx_nbblks(htx)) {
    ...
    if (type == HTX_BLK_REQ_SL || type == HTX_BLK_RES_SL)
      chunk_appendf(&trace_buf, " - \"%.*s %.*s %.*s\"", HTX_SL_P1_LEN(sl),
                    HTX_SL_P1_PTR(sl), HTX_SL_P2_LEN(sl), HTX_SL_P2_PTR(sl),
                    HTX_SL_P3_LEN(sl), HTX_SL_P3_PTR(sl));
  }

  /* Display fconn info and, if defined, fstrm info */
  chunk_appendf(&trace_buf, " - fconn=%p(%s,0x%08x)", fconn,
                fconn_st_to_str(fconn->state), fconn->flags);
  if (fstrm)
    chunk_appendf(&trace_buf, " fstrm=%p(%d,%s,0x%08x)", fstrm, fstrm->id,
                  fstrm_st_to_str(fstrm->state), fstrm->flags);
  ...
  if (src->level > TRACE_LEVEL_USER) {
    if (src->verbosity == FCGI_VERB_COMPLETE ||
        (src->verbosity == FCGI_VERB_ADVANCED &&
         (mask & (FCGI_EV_FCONN_RECV | FCGI_EV_RX_RECORD))))
      chunk_appendf(&trace_buf, " dbuf=%u@%p+%u/%u",
                    (unsigned int)b_data(&fconn->dbuf), b_orig(&fconn->dbuf),
                    (unsigned int)b_head_ofs(&fconn->dbuf),
                    (unsigned int)b_size(&fconn->dbuf));
    if (src->verbosity == FCGI_VERB_COMPLETE ||
        (src->verbosity == FCGI_VERB_ADVANCED &&
         (mask & (FCGI_EV_FCONN_SEND | FCGI_EV_TX_RECORD)))) {
      struct buffer *hmbuf = br_head((struct buffer *)fconn->mbuf);
      struct buffer *tmbuf = br_tail((struct buffer *)fconn->mbuf);

      chunk_appendf(
          &trace_buf, " .mbuf=[%u..%u|%u],h=[%u@%p+%u/%u],t=[%u@%p+%u/%u]",
          br_head_idx(fconn->mbuf), br_tail_idx(fconn->mbuf),
          br_size(fconn->mbuf), (unsigned int)b_data(hmbuf), b_orig(hmbuf),
    ...
    if (fstrm && (src->verbosity == FCGI_VERB_COMPLETE ||
                  (src->verbosity == FCGI_VERB_ADVANCED &&
                   (mask & (FCGI_EV_STRM_RECV | FCGI_EV_RSP_DATA)))))
      chunk_appendf(&trace_buf, " rxbuf=%u@%p+%u/%u",
                    (unsigned int)b_data(&fstrm->rxbuf), b_orig(&fstrm->rxbuf),
                    (unsigned int)b_head_ofs(&fstrm->rxbuf),
                    (unsigned int)b_size(&fstrm->rxbuf));
  }
  ...
  if (src->level > TRACE_LEVEL_USER && htx) {
    ...
    if (src->level > TRACE_LEVEL_STATE) {
      if (src->verbosity == FCGI_VERB_COMPLETE)
        full = 1;
      else if (src->verbosity == FCGI_VERB_ADVANCED &&
               (mask & (FCGI_EV_RSP_HDRS | FCGI_EV_TX_PARAMS)))
        full = 1;
    }
...
static inline int fcgi_recv_allowed(const struct fcgi_conn *fconn) {
  if (b_data(&fconn->dbuf) == 0 &&
      (fconn->state == FCGI_CS_CLOSED || fconn->conn->flags & CO_FL_ERROR ||
...
static inline void fcgi_conn_restart_reading(const struct fcgi_conn *fconn,
                                             int consider_buffer) {
...
static int fcgi_buf_available(void *target) {
  ...
  if ((fconn->flags & FCGI_CF_DEM_DALLOC) && b_alloc_margin(&fconn->dbuf, 0)) {
    TRACE_STATE("unblocking fconn, dbuf allocated",
                FCGI_EV_FCONN_RECV | FCGI_EV_FCONN_BLK | FCGI_EV_FCONN_WAKE,
                fconn->conn);
  ...
  if ((fconn->flags & FCGI_CF_MUX_MALLOC) &&
      b_alloc_margin(br_tail(fconn->mbuf), 0)) {
    TRACE_STATE("unblocking fconn, mbuf allocated",
                FCGI_EV_FCONN_SEND | FCGI_EV_FCONN_BLK | FCGI_EV_FCONN_WAKE,
                fconn->conn);
  ...
  if ((fconn->flags & FCGI_CF_DEM_SALLOC) &&
    ...
    TRACE_STATE("unblocking fstrm, rxbuf allocated",
                FCGI_EV_STRM_RECV | FCGI_EV_FSTRM_BLK | FCGI_EV_STRM_WAKE,
                fconn->conn, fstrm);
...
static inline struct buffer *fcgi_get_buf(struct fcgi_conn *fconn,
                                          struct buffer *bptr) {
...
static inline void fcgi_release_buf(struct fcgi_conn *fconn,
                                    struct buffer *bptr) {
  if (bptr->size) {
    b_free(bptr);
...
static int fcgi_avail_streams(struct connection *conn) {
  ...
  if (ret1 > 0 && srv && srv->max_reuse >= 0) {
    ret2 = ((fconn->stream_cnt <= srv->max_reuse)
                ? srv->max_reuse - fconn->stream_cnt + 1
                : 0);
    ret1 = MIN(ret1, ret2);
  }
  return ret1;
 ...
 */
static int fcgi_init(struct connection *conn, struct proxy *px,
...
fail_conn:
  ...
  TRACE_DEVEL("leaving in error",
              FCGI_EV_FCONN_NEW | FCGI_EV_FCONN_END | FCGI_EV_FCONN_ERR);
  return -1;
}
...
static inline int fcgi_conn_is_dead(struct fcgi_conn *fconn) {
  if (eb_is_empty(&fconn->streams_by_id) && /* don't close if streams exist */
      (!(fconn->flags &
         FCGI_CF_KEEP_CONN) || /* don't keep the connection alive */
       (fconn->conn->flags & CO_FL_ERROR) || /* errors close immediately */
       (fconn->state == FCGI_CS_CLOSED &&
        !fconn->task) ||          /* a timeout stroke earlier */
       (!(fconn->conn->owner)) || /* Nobody's left to take care of the
                                     connection, drop it now */
       (!br_data(fconn->mbuf) &&  /* mux buffer empty, also process clean events
                                     below */
        conn_xprt_read0_pending(fconn->conn))))
    return 1;
  return 0;
...
static inline void fcgi_strm_error(struct fcgi_strm *fstrm) {
  if (fstrm->id && fstrm->state != FCGI_SS_ERROR) {
    ...
    if (fstrm->state < FCGI_SS_ERROR) {
      ...
      TRACE_STATE("switching to ERROR", FCGI_EV_FSTRM_ERR, fstrm->fconn->conn,
                  fstrm);
    }
    if (fstrm->cs)
...
static inline void fcgi_strm_close(struct fcgi_strm *fstrm) {
  if (fstrm->state != FCGI_SS_CLOSED) {
    ...
    TRACE_STATE("switching to CLOSED", FCGI_EV_FSTRM_END, fstrm->fconn->conn,
                fstrm);
...
out:
  TRACE_DEVEL("leaving in error",
              FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_ERR | FCGI_EV_FSTRM_END,
              fconn->conn);
  return NULL;
}
...
static struct fcgi_strm *fcgi_conn_stream_new(struct fcgi_conn *fconn,
  ...
  if (fconn->nb_streams >= fconn->streams_limit) {
    TRACE_DEVEL("leaving on streams_limit reached",
                FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_END | FCGI_EV_FSTRM_ERR,
                fconn->conn);
    goto out;
  }

  if (fcgi_streams_left(fconn) < 1) {
    TRACE_DEVEL("leaving on !streams_left",
                FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_END | FCGI_EV_FSTRM_ERR,
                fconn->conn);
    goto out;
  }
  ...
  if (!fstrm) {
    TRACE_DEVEL("leaving on fstrm creation failure",
                FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_END | FCGI_EV_FSTRM_ERR,
                fconn->conn);
    goto out;
  }
...
static void fcgi_strm_wake_one_stream(struct fcgi_strm *fstrm) {
  ...
  if (fcgi_conn_read0_pending(fconn)) {
    if (fstrm->state == FCGI_SS_OPEN) {
      ...
      TRACE_STATE("swtiching to HREM", FCGI_EV_STRM_WAKE | FCGI_EV_FSTRM_END,
                  fconn->conn, fstrm);
  ...
  if ((fconn->state == FCGI_CS_CLOSED || fconn->conn->flags & CO_FL_ERROR)) {
    ...
    if (fstrm->state < FCGI_SS_ERROR) {
      fstrm->state = FCGI_SS_ERROR;
      TRACE_STATE("switching to ERROR", FCGI_EV_STRM_WAKE | FCGI_EV_FSTRM_END,
                  fconn->conn, fstrm);
    }
  }
...
static int fcgi_set_default_param(struct fcgi_conn *fconn,
                                  struct fcgi_strm *fstrm, struct htx *htx,
                                  struct htx_sl *sl,
  ...
  if (!(params->mask & FCGI_SP_SRV_NAME)) {
    ...
    if (!istlen(params->srv_name)) {
      ...
      if (cli_conn && conn_get_dst(cli_conn))
        if (addr_to_str(cli_conn->dst, b_tail(params->p), b_room(params->p)) !=
            -1)
          ptr = b_tail(params->p);
      if (ptr) {
  ...
  if (!(params->mask & FCGI_SP_REM_ADDR)) {
    ...
    if (cli_conn && conn_get_src(cli_conn))
      if (addr_to_str(cli_conn->src, b_tail(params->p), b_room(params->p)) !=
          -1)
        ptr = b_tail(params->p);
    if (ptr) {
  ...
  if ((params->mask & FCGI_SP_URI_MASK) != FCGI_SP_URI_MASK) {
     ...
     */
    if (!regex_exec_match2(fconn->app->pathinfo_re, path.ptr, len, MAX_MATCH,
                           pmatch, 0))
      goto check_index;
     ...
     */
    params->scriptname =
        ist2(path.ptr + pmatch[1].rm_so, pmatch[1].rm_eo - pmatch[1].rm_so);
    if (!(params->mask & FCGI_SP_PATH_INFO) &&
        (pmatch[2].rm_so == -1 || pmatch[2].rm_eo == -1))
      params->pathinfo =
          ist2(path.ptr + pmatch[2].rm_so, pmatch[2].rm_eo - pmatch[2].rm_so);
...
static int fcgi_encode_default_param(struct fcgi_conn *fconn,
                                     struct fcgi_strm *fstrm,
                                     struct fcgi_strm_params *params,
                                     struct buffer *outbuf, int flag) {
  struct fcgi_param p;
 ...
 * and cannot be encoded in one time, an error is triggered and the connection
 * is closed. GET_VALUES record cannot be split.
 */
...
retry:
  if (!fcgi_get_buf(fconn, mbuf)) {
    fconn->flags |= FCGI_CF_MUX_MALLOC;
    fconn->flags |= FCGI_CF_DEM_MROOM;
    TRACE_STATE("waiting for fconn mbuf ring allocation",
                FCGI_EV_TX_RECORD | FCGI_EV_FCONN_BLK, fconn->conn);
    ret = 0;
    goto end;
  }
   ...
   *       handled by HAProxy.
   */
  if (!fcgi_encode_param(&outbuf, &max_reqs) ||
      !fcgi_encode_param(&outbuf, &mpxs_conns))
    goto full;

  /* update the record's size now */
  TRACE_PROTO("FCGI GET_VALUES record xferred",
              FCGI_EV_TX_RECORD | FCGI_EV_TX_GETVAL, fconn->conn, 0, 0,
              (size_t[]){outbuf.data - 8});
...
full:
  ...
  fconn->flags |= FCGI_CF_DEM_MROOM;
  TRACE_STATE("mbuf ring full", FCGI_EV_TX_RECORD | FCGI_EV_FCONN_BLK,
              fconn->conn);
  ret = 0;
  goto end;
fail:
  fconn->state = FCGI_CS_CLOSED;
  TRACE_STATE("switching to CLOSED",
              FCGI_EV_TX_RECORD | FCGI_EV_TX_GETVAL | FCGI_EV_FCONN_END,
              fconn->conn);
  TRACE_DEVEL("leaving on error",
              FCGI_EV_TX_RECORD | FCGI_EV_TX_GETVAL | FCGI_EV_FCONN_ERR,
              fconn->conn);
  return 0;
}
...
static int fcgi_conn_handle_values_result(struct fcgi_conn *fconn) {
  ...
  if (b_data(dbuf) < (fconn->drl + fconn->drp)) {
    TRACE_DEVEL("leaving on missing data",
                FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL, fconn->conn);
    return 0;
  }
  ...
  for (offset = 0; offset < b_data(&inbuf);) {
    ...
    if (isteqi(p.n, ist("FCGI_MPXS_CONNS"))) {
      if (isteq(p.v, ist("1"))) {
        TRACE_STATE("set mpxs param", FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL,
                    fconn->conn, 0, 0, (size_t[]){1});
        fconn->flags |= FCGI_CF_MPXS_CONNS;
      } else {
        TRACE_STATE("set mpxs param", FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL,
                    fconn->conn, 0, 0, (size_t[]){0});
        fconn->flags &= ~FCGI_CF_MPXS_CONNS;
      }
    } else if (isteqi(p.n, ist("FCGI_MAX_REQS"))) {
      fconn->streams_limit = strl2ui(p.v.ptr, p.v.len);
      TRACE_STATE("set streams_limit", FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL,
                  fconn->conn, 0, 0, (size_t[]){fconn->streams_limit});
    }
    /*
  ...
  if (!(fconn->flags & FCGI_CF_MPXS_CONNS)) {
    ...
    TRACE_STATE("no mpxs for streams_limit to 1",
                FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL, fconn->conn);
  }
  ...
  TRACE_PROTO("FCGI GET_VALUES_RESULT record rcvd",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL, fconn->conn, 0, 0,
              (size_t[]){fconn->drl});
  ...
  fcgi_wake_unassigned_streams(fconn);
  TRACE_STATE("switching to RECORD_H", FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR,
              fconn->conn);
  TRACE_LEAVE(FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL, fconn->conn);
  return 1;
fail:
  fconn->state = FCGI_CS_CLOSED;
  TRACE_STATE("switching to CLOSED", FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL,
              fconn->conn);
  TRACE_DEVEL("leaving on error",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL | FCGI_EV_FCONN_ERR,
              fconn->conn);
  return 0;
}
 ...
 */
static int fcgi_strm_send_begin_request(struct fcgi_conn *fconn,
                                        struct fcgi_strm *fstrm) {
  struct buffer outbuf;
  struct buffer *mbuf;
...
retry:
  if (!fcgi_get_buf(fconn, mbuf)) {
    ...
    TRACE_STATE("waiting for fconn mbuf ring allocation",
                FCGI_EV_TX_RECORD | FCGI_EV_FSTRM_BLK | FCGI_EV_FCONN_BLK,
                fconn->conn, fstrm);
    ret = 0;
    goto end;
  }
  ...
  if (fconn->flags & FCGI_CF_KEEP_CONN) {
    TRACE_STATE("keep connection opened", FCGI_EV_TX_RECORD | FCGI_EV_TX_BEGREQ,
                fconn->conn, fstrm);
  ...
  /* commit the record */
  TRACE_PROTO("FCGI BEGIN_REQUEST record xferred",
              FCGI_EV_TX_RECORD | FCGI_EV_TX_BEGREQ, fconn->conn, fstrm, 0,
              (size_t[]){0});
  b_add(mbuf, outbuf.data);
  fstrm->flags |= FCGI_SF_BEGIN_SENT;
  fstrm->state = FCGI_SS_OPEN;
  TRACE_STATE("switching to OPEN", FCGI_EV_TX_RECORD | FCGI_EV_TX_BEGREQ,
              fconn->conn, fstrm);
  ret = 1;
 ...
 * space to proceed. It is small enough to be encoded in an empty buffer.
 */
static int fcgi_strm_send_empty_record(struct fcgi_conn *fconn,
                                       struct fcgi_strm *fstrm,
 ...
 */
static int fcgi_strm_send_empty_params(struct fcgi_conn *fconn,
                                       struct fcgi_strm *fstrm) {
  int ret;
  ...
  if (ret)
    TRACE_PROTO("FCGI PARAMS record xferred",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm, 0,
                (size_t[]){0});
  return ret;
}
 ...
 */
static int fcgi_strm_send_empty_stdin(struct fcgi_conn *fconn,
                                      struct fcgi_strm *fstrm) {
  int ret;
  ...
  if (ret) {
    ...
    TRACE_PROTO("FCGI STDIN record xferred",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm, 0,
                (size_t[]){0});
    TRACE_USER("FCGI request fully xferred",
               FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN | FCGI_EV_TX_EOI,
               fconn->conn, fstrm);
    TRACE_STATE("stdin data fully sent",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN | FCGI_EV_TX_EOI,
                fconn->conn, fstrm);
  }
  return ret;
...
static int fcgi_strm_send_abort(struct fcgi_conn *fconn,
  ...
  if (ret) {
    ...
    TRACE_PROTO("FCGI ABORT record xferred",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_ABORT, fconn->conn, fstrm, 0,
                (size_t[]){0});
    TRACE_USER("FCGI request aborted", FCGI_EV_TX_RECORD | FCGI_EV_TX_ABORT,
               fconn->conn, fstrm);
    TRACE_STATE("abort sent", FCGI_EV_TX_RECORD | FCGI_EV_TX_ABORT, fconn->conn,
                fstrm);
  }
  return ret;
}
 ...
 * records.
 */
static size_t fcgi_strm_send_params(struct fcgi_conn *fconn,
                                    struct fcgi_strm *fstrm, struct htx *htx) {
  struct buffer outbuf;
  struct buffer *mbuf;
  struct htx_blk *blk;
...
retry:
  ...
  while (blk) {
    ...
    case HTX_BLK_HDR:
      ...
      if (istmatch(p.n, ist(":fcgi-"))) {
        ...
        if (fconn->proxy->server_id_hdr_name &&
            isteq(p.n, ist2(fconn->proxy->server_id_hdr_name,
                            fconn->proxy->server_id_hdr_len)))
          break;
    ...
    case HTX_BLK_EOH:
      if (fconn->proxy->server_id_hdr_name) {
        ...
        memcpy(trash.area, "http_", 5);
        memcpy(trash.area + 5, fconn->proxy->server_id_hdr_name,
               fconn->proxy->server_id_hdr_len);
        ...
        TRACE_STATE("add server name header",
                    FCGI_EV_TX_RECORD | FCGI_EV_TX_PARAMS, fconn->conn, fstrm);
      }
      goto done;
  ...
  if (!fcgi_encode_default_param(fconn, fstrm, &params, &outbuf,
                                 FCGI_SP_CGI_GATEWAY) ||
      !fcgi_encode_default_param(fconn, fstrm, &params, &outbuf,
                                 FCGI_SP_DOC_ROOT) ||
      !fcgi_encode_default_param(fconn, fstrm, &params, &outbuf,
                                 FCGI_SP_SCRIPT_NAME) ||
      !fcgi_encode_default_param(fconn, fstrm, &params, &outbuf,
  ...
  TRACE_PROTO("FCGI PARAMS record xferred",
              FCGI_EV_TX_RECORD | FCGI_EV_TX_PARAMS, fconn->conn, fstrm, 0,
              (size_t[]){outbuf.data - 8});
  fcgi_set_record_size(outbuf.area, outbuf.data - 8);
  b_add(mbuf, outbuf.data);

end:
  TRACE_LEAVE(FCGI_EV_TX_RECORD | FCGI_EV_TX_PARAMS, fconn->conn, fstrm, htx,
              (size_t[]){total});
  return total;
full:
  if ((mbuf = br_tail_add(fconn->mbuf)) != NULL)
...
error:
  ...
  TRACE_PROTO("processing error", FCGI_EV_TX_RECORD | FCGI_EV_STRM_ERR,
              fconn->conn, fstrm);
 ...
 */
static size_t fcgi_strm_send_stdin(struct fcgi_conn *fconn,
                                   struct fcgi_strm *fstrm, struct htx *htx,
                                   size_t count, struct buffer *buf) {
  struct buffer outbuf;
  struct buffer *mbuf;
  struct htx_blk *blk;
  enum htx_blk_type type;
  ...
  TRACE_ENTER(FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm, htx,
              (size_t[]){count});
  if (!count)
    goto end;
...
retry:
  ...
  if (unlikely(size == count && htx_nbblks(htx) == 1 && type == HTX_BLK_DATA)) {
    ...
    TRACE_PROTO("sending stding data (zero-copy)",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm, htx,
                (size_t[]){size});
    /* map a FCGI record to the HTX block so that we can put the
     * record header there.
     */
    *mbuf = b_make(buf->area, buf->size, sizeof(struct htx) + blk->addr - 8,
                   size + 8);
...
copy:
  ...
  while (blk && count) {
    ...
    switch (type) {
    case HTX_BLK_DATA:
      TRACE_PROTO("sending stding data", FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN,
                  fconn->conn, fstrm, htx, (size_t[]){size});
      ...
      if (v.len > b_room(&outbuf)) {
         ...
         * buffer now.
         */
        if (b_space_wraps(mbuf) && b_data(&outbuf) + v.len <= b_room(mbuf) &&
  ...
  /* update the record's size */
  TRACE_PROTO("FCGI STDIN record xferred", FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN,
              fconn->conn, fstrm, 0, (size_t[]){outbuf.data - 8});
  fcgi_set_record_size(outbuf.area, outbuf.data - 8);
  b_add(mbuf, outbuf.data);

end:
  TRACE_LEAVE(FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm, htx,
              (size_t[]){total});
  return total;
full:
  if ((mbuf = br_tail_add(fconn->mbuf)) != NULL)
 ...
 */
static int fcgi_strm_handle_stdout(struct fcgi_conn *fconn,
                                   struct fcgi_strm *fstrm) {
  struct buffer *dbuf;
  size_t ret;
  size_t max;
  ...
  if (b_data(dbuf) < (fconn->drl + fconn->drp) &&
      b_size(dbuf) > (fconn->drl + fconn->drp) && buf_room_for_htx_data(dbuf))
    goto fail; // incomplete record

  if (!fcgi_get_buf(fconn, &fstrm->rxbuf)) {
    fconn->flags |= FCGI_CF_DEM_SALLOC;
    TRACE_STATE("waiting for fstrm rxbuf allocation",
                FCGI_EV_RX_RECORD | FCGI_EV_FSTRM_BLK, fconn->conn, fstrm);
    goto fail;
  }
  ...
  if (!ret)
    goto fail;
  fconn->drl -= ret;
  TRACE_DATA("move some data to fstrm rxbuf",
             FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT, fconn->conn, fstrm, 0,
             (size_t[]){ret});
  TRACE_PROTO("FCGI STDOUT record rcvd", FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT,
              fconn->conn, fstrm, 0, (size_t[]){ret});

  if (!buf_room_for_htx_data(&fstrm->rxbuf)) {
    fconn->flags |= FCGI_CF_DEM_SFULL;
    TRACE_STATE("fstrm rxbuf full", FCGI_EV_RX_RECORD | FCGI_EV_FSTRM_BLK,
                fconn->conn, fstrm);
  }
...
end_transfer:
  ...
  return 1;
fail:
  TRACE_DEVEL("leaving on missing data or error",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT, fconn->conn, fstrm);
  return 0;
}
 ...
 */
static int fcgi_strm_handle_empty_stdout(struct fcgi_conn *fconn,
                                         struct fcgi_strm *fstrm) {
  int ret;
  ...
  fconn->state = FCGI_CS_RECORD_P;
  TRACE_STATE("switching to RECORD_P", FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT,
              fconn->conn, fstrm);
  fconn->drl += fconn->drp;
  fconn->drp = 0;
  ret = MIN(b_data(&fconn->dbuf), fconn->drl);
  ...
  TRACE_PROTO("FCGI STDOUT record rcvd", FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT,
              fconn->conn, fstrm, 0, (size_t[]){0});
  TRACE_STATE("stdout data fully send, switching to RECORD_H",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR | FCGI_EV_RX_EOI, fconn->conn,
              fstrm);
  TRACE_LEAVE(FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT, fconn->conn, fstrm);
  return 1;
}
 ...
 * anything.
 */
static int fcgi_strm_handle_stderr(struct fcgi_conn *fconn,
                                   struct fcgi_strm *fstrm) {
  struct buffer *dbuf;
  struct buffer tag;
  size_t ret;
  ...
  if (!ret)
    goto fail;
  fconn->drl -= ret;
  TRACE_PROTO("FCGI STDERR record rcvd", FCGI_EV_RX_RECORD | FCGI_EV_RX_STDERR,
              fconn->conn, fstrm, 0, (size_t[]){ret});

  trash.area[ret] = '\n';
  trash.area[ret + 1] = '\0';
  tag.area = fconn->app->name;
  tag.data = strlen(fconn->app->name);
...
fail:
  TRACE_DEVEL("leaving on missing data or error",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_STDERR, fconn->conn, fstrm);
  return 0;
}
 ...
 */
static int fcgi_strm_handle_end_request(struct fcgi_conn *fconn,
                                        struct fcgi_strm *fstrm) {
  struct buffer inbuf;
  struct buffer *dbuf;
  ...
  if (b_data(dbuf) < (fconn->drl + fconn->drp)) {
    TRACE_DEVEL("leaving on missing data",
                FCGI_EV_RX_RECORD | FCGI_EV_RX_ENDREQ, fconn->conn);
    return 0;
  }
  ...
  fstrm->flags |= FCGI_SF_ES_RCVD;
  TRACE_STATE("end of script reported",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_ENDREQ | FCGI_EV_RX_EOI,
              fconn->conn, fstrm);
  TRACE_PROTO("FCGI END_REQUEST record rcvd",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_ENDREQ, fconn->conn, fstrm, 0,
              (size_t[]){fconn->drl});
...
fail:
  fcgi_strm_error(fstrm);
  TRACE_DEVEL("leaving on error",
              FCGI_EV_RX_RECORD | FCGI_EV_RX_ENDREQ | FCGI_EV_FSTRM_ERR,
              fconn->conn, fstrm);
  return 0;
}
...
static void fcgi_process_demux(struct fcgi_conn *fconn) {
  ...
  if (unlikely(fconn->state < FCGI_CS_RECORD_H)) {
    if (fconn->state == FCGI_CS_INIT) {
      TRACE_STATE("waiting FCGI GET_VALUES to be sent",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR | FCGI_EV_RX_GETVAL,
                  fconn->conn);
      return;
    }
    if (fconn->state == FCGI_CS_SETTINGS) {
      /* ensure that what is pending is a valid GET_VALUES_RESULT record. */
      TRACE_STATE("receiving FCGI record header",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR, fconn->conn);
      ret = fcgi_decode_record_hdr(&fconn->dbuf, 0, &hdr);
      if (!ret)
        goto fail;
      b_del(&fconn->dbuf, ret);

      if (hdr.id || (hdr.type != FCGI_GET_VALUES_RESULT &&
                     hdr.type != FCGI_UNKNOWN_TYPE)) {
        fconn->state = FCGI_CS_CLOSED;
        TRACE_PROTO("unexpected record type or flags",
                    FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR | FCGI_EV_RX_GETVAL |
                        FCGI_EV_FCONN_ERR,
                    fconn->conn);
        TRACE_STATE("switching to CLOSED",
  ...
  while (1) {
    ...
    if (fconn->state == FCGI_CS_CLOSED) {
      TRACE_STATE("end of connection reported",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_EOI, fconn->conn);
      break;
    }

    if (fconn->state == FCGI_CS_RECORD_H) {
      TRACE_PROTO("receiving FCGI record header",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR, fconn->conn);
      ret = fcgi_decode_record_hdr(&fconn->dbuf, 0, &hdr);
      if (!ret)
        break;
    ...
    new_record:
      ...
      TRACE_STATE("FCGI record header rcvd, switching to RECORD_D",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR, fconn->conn);
    }
    ...
    if (tmp_fstrm != fstrm && fstrm && fstrm->cs &&
        (b_data(&fstrm->rxbuf) || fcgi_conn_read0_pending(fconn) ||
         fstrm->state == FCGI_SS_CLOSED || (fstrm->flags & FCGI_SF_ES_RCVD) ||
         (fstrm->cs->flags & (CS_FL_ERROR | CS_FL_ERR_PENDING | CS_FL_EOS)))) {
      /* we may have to signal the upper layers */
      TRACE_DEVEL("notifying stream before switching SID",
                  FCGI_EV_RX_RECORD | FCGI_EV_STRM_WAKE, fconn->conn, fstrm);
    ...
    case FCGI_GET_VALUES_RESULT:
      TRACE_PROTO("receiving FCGI GET_VALUES_RESULT record",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL, fconn->conn);
    ...
    case FCGI_STDOUT:
      ...
      TRACE_PROTO("receiving FCGI STDOUT record",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_STDOUT, fconn->conn, fstrm);
      if (fconn->drl)
    ...
    case FCGI_STDERR:
      TRACE_PROTO("receiving FCGI STDERR record",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_STDERR, fconn->conn, fstrm);
      ret = fcgi_strm_handle_stderr(fconn, fstrm);
      break;

    case FCGI_END_REQUEST:
      TRACE_PROTO("receiving FCGI END_REQUEST record",
                  FCGI_EV_RX_RECORD | FCGI_EV_RX_ENDREQ, fconn->conn, fstrm);
    ...
    ignore_record:
      ...
      fconn->drp = 0;
      ret = MIN(b_data(&fconn->dbuf), fconn->drl);
      TRACE_PROTO("receiving FCGI ignored record", FCGI_EV_RX_RECORD,
                  fconn->conn, fstrm, 0, (size_t[]){ret});
      TRACE_STATE("switching to RECORD_P", FCGI_EV_RX_RECORD, fconn->conn,
                  fstrm);
      b_del(&fconn->dbuf, ret);
      fconn->drl -= ret;
      ret = (fconn->drl == 0);
    ...
    if (ret <= 0) {
      TRACE_DEVEL("insufficient data to proceed", FCGI_EV_RX_RECORD,
                  fconn->conn, fstrm);
      break;
    }
  ...
  if (unlikely(fconn->state < FCGI_CS_RECORD_H)) {
    if (unlikely(fconn->state == FCGI_CS_INIT)) {
      if (!(fconn->flags & FCGI_CF_GET_VALUES)) {
        fconn->state = FCGI_CS_RECORD_H;
        TRACE_STATE("switching to RECORD_H",
                    FCGI_EV_TX_RECORD | FCGI_EV_RX_RECORD | FCGI_EV_RX_FHDR,
                    fconn->conn);
        fcgi_wake_unassigned_streams(fconn);
        goto mux;
      }
      TRACE_PROTO("sending FCGI GET_VALUES record",
                  FCGI_EV_TX_RECORD | FCGI_EV_TX_GETVAL, fconn->conn);
      if (unlikely(!fcgi_conn_send_get_values(fconn)))
        goto fail;
      fconn->state = FCGI_CS_SETTINGS;
      TRACE_STATE("switching to SETTINGS",
                  FCGI_EV_TX_RECORD | FCGI_EV_RX_RECORD | FCGI_EV_RX_GETVAL,
                  fconn->conn);
    }
  ...
  if (fconn->state == FCGI_CS_CLOSED) {
    if (fconn->stream_cnt - fconn->nb_reserved > 0) {
      ...
      if (fconn->flags & FCGI_CF_MUX_BLOCK_ANY) {
        TRACE_DEVEL("leaving in blocked situation",
                    FCGI_EV_FCONN_WAKE | FCGI_EV_FCONN_BLK, fconn->conn);
        return 0;
      }
    }
  }
...
static int fcgi_recv(struct fcgi_conn *fconn) {
  ...
  if (!buf) {
    TRACE_DEVEL("waiting for fconn dbuf allocation",
                FCGI_EV_FCONN_RECV | FCGI_EV_FCONN_BLK, conn);
    fconn->flags |= FCGI_CF_DEM_DALLOC;
    return 0;
  }
  ...
  if (!b_data(buf)) {
     ...
     */
    max =
        buf_room_for_htx_data(buf) + (fconn->state == FCGI_CS_RECORD_H ? 8 : 0);
  ...
  if (ret == max) {
    TRACE_DEVEL("fconn dbuf full", FCGI_EV_FCONN_RECV | FCGI_EV_FCONN_BLK,
                conn);
...
static int fcgi_send(struct fcgi_conn *fconn) {
  ...
  while (!done) {
    ...
    while (((fconn->flags & (FCGI_CF_MUX_MFULL | FCGI_CF_MUX_MALLOC)) == 0) &&
           !done)
    ...
    for (buf = br_head(fconn->mbuf); b_size(buf);
         buf = br_del_head(fconn->mbuf)) {
      if (b_data(buf)) {
        int ret;

        ret =
            conn->xprt->snd_buf(conn, conn->xprt_ctx, buf, b_data(buf), flags);
        if (!ret) {
          done = 1;
          break;
        }
        sent = 1;
        TRACE_DATA("send data", FCGI_EV_FCONN_SEND, conn, 0, 0,
                   (size_t[]){ret});
        b_del(buf, ret);
        if (b_data(buf)) {
          done = 1;
    ...
    if (fconn->flags & (FCGI_CF_MUX_MFULL | FCGI_CF_DEM_MROOM))
      TRACE_STATE("fconn mbuf ring not fill anymore",
                  FCGI_EV_FCONN_SEND | FCGI_EV_FCONN_BLK, conn);
  ...
  if (!(fconn->flags & (FCGI_CF_MUX_MFULL | FCGI_CF_DEM_MROOM)) &&
    ...
    list_for_each_entry(fstrm, &fconn->send_list, send_list) {
      if (fconn->state == FCGI_CS_CLOSED ||
          fconn->flags & FCGI_CF_MUX_BLOCK_ANY)
        break;
      ...
      if (fstrm->subs && fstrm->subs->events & SUB_RETRY_SEND) {
        TRACE_DEVEL("waking up pending stream",
                    FCGI_EV_FCONN_SEND | FCGI_EV_STRM_WAKE, conn, fstrm);
        fstrm->flags &= ~FCGI_SF_BLK_ANY;
        fstrm->flags |= FCGI_SF_NOTIFIED;
        tasklet_wakeup(fstrm->subs->tasklet);
        fstrm->subs->events &= ~SUB_RETRY_SEND;
...
schedule:
  if (!(conn->flags & CO_FL_ERROR) &&
      !(fconn->wait_event.events & SUB_RETRY_SEND)) {
    TRACE_STATE("more data to send, subscribing", FCGI_EV_FCONN_SEND, conn);
    conn->xprt->subscribe(conn, conn->xprt_ctx, SUB_RETRY_SEND,
                          &fconn->wait_event);
  }
...
static struct task *fcgi_io_cb(struct task *t, void *ctx,
...
static int fcgi_process(struct fcgi_conn *fconn) {
  ...
  if (unlikely(fconn->proxy->disabled)) {
     ...
     */
    TRACE_STATE("proxy stopped, sending ABORT to all streams",
                FCGI_EV_FCONN_WAKE | FCGI_EV_TX_RECORD, conn);
  ...
  if (!(fconn->flags & FCGI_CF_WAIT_FOR_HS) &&
      (conn->flags & (CO_FL_EARLY_SSL_HS | CO_FL_WAIT_XPRT |
  ...
  if ((conn->flags & CO_FL_SOCK_WR_SH) || fconn->state == FCGI_CS_CLOSED ||
      (fconn->flags & FCGI_CF_ABRTS_FAILED) ||
      (!br_data(fconn->mbuf) && ((fconn->flags & FCGI_CF_MUX_BLOCK_ANY) ||
                                 LIST_ISEMPTY(&fconn->send_list))))
    fcgi_release_mbuf(fconn);

  if (fconn->task) {
    fconn->task->expire =
        tick_add(now_ms, (fconn->state == FCGI_CS_CLOSED ? fconn->shut_timeout
                                                         : fconn->timeout));
...
static int fcgi_ctl(struct connection *conn, enum mux_ctl_type mux_ctl,
                    void *output) {
  int ret = 0;
  switch (mux_ctl) {
  case MUX_STATUS:
 ...
 */
static void fcgi_show_error_snapshot(struct buffer *out,
                                     const struct error_snapshot *es) {
  chunk_appendf(out,
 ...
 */
static void fcgi_strm_capture_bad_message(struct fcgi_conn *fconn,
                                          struct fcgi_strm *fstrm,
  ...
  proxy_capture_error(proxy, 1, other_end, fconn->conn->target, sess, buf, 0, 0,
                      (h1m->err_pos >= 0) ? h1m->err_pos : h1m->next, &ctx,
                      fcgi_show_error_snapshot);
}

static size_t fcgi_strm_parse_headers(struct fcgi_strm *fstrm, struct h1m *h1m,
                                      struct htx *htx, struct buffer *buf,
                                      size_t *ofs, size_t max) {
  int ret;

  TRACE_ENTER(FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){max});
  ret = h1_parse_msg_hdrs(h1m, NULL, htx, buf, *ofs, max);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS, fstrm->fconn->conn, fstrm);
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      TRACE_USER("rejected H1 response",
                 FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS | FCGI_EV_FSTRM_ERR,
                 fstrm->fconn->conn, fstrm);
      fcgi_strm_error(fstrm);
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){ret});
  return ret;
}

static size_t fcgi_strm_parse_data(struct fcgi_strm *fstrm, struct h1m *h1m,
                                   struct htx **htx, struct buffer *buf,
                                   size_t *ofs, size_t max,
                                   struct buffer *htxbuf) {
  int ret;

  TRACE_ENTER(FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){max});
  ret = h1_parse_msg_data(h1m, htx, buf, *ofs, max, htxbuf);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY, fstrm->fconn->conn, fstrm);
    if ((*htx)->flags & HTX_FL_PARSING_ERROR) {
      TRACE_USER("rejected H1 response",
                 FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY | FCGI_EV_FSTRM_ERR,
                 fstrm->fconn->conn, fstrm);
      fcgi_strm_error(fstrm);
      fcgi_strm_capture_bad_message(fstrm->fconn, fstrm, h1m, buf);
    }
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){ret});
  return ret;
}

static size_t fcgi_strm_parse_trailers(struct fcgi_strm *fstrm, struct h1m *h1m,
                                       struct htx *htx, struct buffer *buf,
                                       size_t *ofs, size_t max) {
  int ret;

  TRACE_ENTER(FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){max});
  ret = h1_parse_msg_tlrs(h1m, htx, buf, *ofs, max);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS, fstrm->fconn->conn, fstrm);
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      TRACE_USER("rejected H1 response",
                 FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS | FCGI_EV_FSTRM_ERR,
                 fstrm->fconn->conn, fstrm);
      fcgi_strm_error(fstrm);
      fcgi_strm_capture_bad_message(fstrm->fconn, fstrm, h1m, buf);
    }
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){ret});
  return ret;
}

static size_t fcgi_strm_add_eom(struct fcgi_strm *fstrm, struct h1m *h1m,
                                struct htx *htx, struct buffer *buf,
                                size_t *ofs, size_t max) {
  int ret;

  TRACE_ENTER(FCGI_EV_RSP_DATA | FCGI_EV_RSP_EOM, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){max});
  ret = h1_parse_msg_eom(h1m, htx, max);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                FCGI_EV_RSP_DATA | FCGI_EV_RSP_EOM, fstrm->fconn->conn, fstrm);
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      TRACE_USER("rejected H1 response",
                 FCGI_EV_RSP_DATA | FCGI_EV_RSP_EOM | FCGI_EV_FSTRM_ERR,
                 fstrm->fconn->conn, fstrm);
      fcgi_strm_error(fstrm);
      fcgi_strm_capture_bad_message(fstrm->fconn, fstrm, h1m, buf);
    }
  ...
  TRACE_LEAVE(FCGI_EV_RSP_DATA | FCGI_EV_RSP_EOM, fstrm->fconn->conn, fstrm, 0,
              (size_t[]){ret});
  return ret;
}

static size_t fcgi_strm_parse_response(struct fcgi_strm *fstrm,
  ...
  do {
    ...
    if (h1m->state <= H1_MSG_LAST_LF) {
      TRACE_PROTO("parsing response headers",
                  FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS, fconn->conn, fstrm);
      ret = fcgi_strm_parse_headers(fstrm, h1m, htx, &fstrm->rxbuf, &total,
                                    count);
      if (!ret)
        break;

      TRACE_USER("rcvd H1 response headers",
                 FCGI_EV_RSP_DATA | FCGI_EV_RSP_HDRS, fconn->conn, fstrm, htx);
      ...
      TRACE_PROTO("parsing response payload",
                  FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY, fconn->conn, fstrm);
      ret = fcgi_strm_parse_data(fstrm, h1m, &htx, &fstrm->rxbuf, &total, count,
                                 buf);
      if (!ret && h1m->state != H1_MSG_DONE)
        break;

      TRACE_PROTO("rcvd response payload data",
                  FCGI_EV_RSP_DATA | FCGI_EV_RSP_BODY, fconn->conn, fstrm, htx);
    } else if (h1m->state == H1_MSG_TRAILERS) {
      TRACE_PROTO("parsing response trailers",
                  FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS, fconn->conn, fstrm);
      ret = fcgi_strm_parse_trailers(fstrm, h1m, htx, &fstrm->rxbuf, &total,
                                     count);
      if (!ret && h1m->state != H1_MSG_DONE)
        break;

      TRACE_PROTO("rcvd H1 response trailers",
                  FCGI_EV_RSP_DATA | FCGI_EV_RSP_TLRS, fconn->conn, fstrm, htx);
    } else if (h1m->state == H1_MSG_DONE) {
      if (!(fstrm->flags & FCGI_SF_H1_PARSING_DONE)) {
        if (!fcgi_strm_add_eom(fstrm, h1m, htx, &fstrm->rxbuf, &total, count))
          break;

        TRACE_USER("H1 response fully rcvd", FCGI_EV_RSP_DATA | FCGI_EV_RSP_EOM,
                   fconn->conn, fstrm, htx);
      }

      if (b_data(&fstrm->rxbuf) > total) {
        htx->flags |= HTX_FL_PARSING_ERROR;
        TRACE_PROTO("too much data, parsing error", FCGI_EV_RSP_DATA,
                    fconn->conn, fstrm);
        fcgi_strm_error(fstrm);
      }
      break;
    } else if (h1m->state == H1_MSG_TUNNEL) {
      TRACE_PROTO("parsing response tunneled data", FCGI_EV_RSP_DATA,
                  fconn->conn, fstrm);
      ret = fcgi_strm_parse_data(fstrm, h1m, &htx, &fstrm->rxbuf, &total, count,
                                 buf);

      if (fstrm->state != FCGI_SS_ERROR && (fstrm->flags & FCGI_SF_ES_RCVD) &&
          b_data(&fstrm->rxbuf) == total) {
        TRACE_DEVEL("end of tunneled data", FCGI_EV_RSP_DATA, fconn->conn,
                    fstrm);
        if ((h1m->flags & (H1_MF_VER_11 | H1_MF_XFER_LEN)) != H1_MF_VER_11)
          fstrm->flags |= FCGI_SF_H1_PARSING_DONE;
      ...
      TRACE_PROTO("rcvd H1 response tunneled data", FCGI_EV_RSP_DATA,
                  fconn->conn, fstrm, htx);
    } else {
      htx->flags |= HTX_FL_PROCESSING_ERROR;
  ...
  if (fstrm->state == FCGI_SS_ERROR) {
    b_reset(&fstrm->rxbuf);
    htx_to_buf(htx, buf);
    TRACE_DEVEL("leaving on error", FCGI_EV_RSP_DATA | FCGI_EV_STRM_ERR,
                fconn->conn, fstrm);
    return 0;
  }
 ...
 */
static struct conn_stream *fcgi_attach(struct connection *conn,
                                       struct session *sess) {
  struct conn_stream *cs;
  struct fcgi_strm *fstrm;
  struct fcgi_conn *fconn = conn->ctx;
  ...
  if (!cs) {
    TRACE_DEVEL("leaving on CS allocation failure",
                FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_ERR, conn);
    return NULL;
  }
  fstrm = fcgi_conn_stream_new(fconn, cs, sess);
  if (!fstrm) {
    TRACE_DEVEL("leaving on stream creation failure",
                FCGI_EV_FSTRM_NEW | FCGI_EV_FSTRM_ERR, conn);
    cs_free(cs);
    return NULL;
  }
 ...
 * orphans.
 */
static const struct conn_stream *
fcgi_get_first_cs(const struct connection *conn) {
  struct fcgi_conn *fconn = conn->ctx;
  struct fcgi_strm *fstrm;
  struct eb32_node *node;
...
static void fcgi_destroy(void *ctx) {
  ...
  if (eb_is_empty(&fconn->streams_by_id) || !fconn->conn ||
      fconn->conn->ctx != fconn)
    fcgi_release(fconn);
}
...
static void fcgi_detach(struct conn_stream *cs) {
   ...
   * it in this case.
   */
  if (!(cs->conn->flags & CO_FL_ERROR) && (fconn->state != FCGI_CS_CLOSED) &&
      (fstrm->flags & (FCGI_SF_BLK_MBUSY | FCGI_SF_BLK_MROOM)) &&
      (fstrm->subs ||
       (fstrm->flags & (FCGI_SF_WANT_SHUTR | FCGI_SF_WANT_SHUTW)))) {
    TRACE_DEVEL("leaving on stream blocked",
                FCGI_EV_STRM_END | FCGI_EV_FSTRM_BLK, fconn->conn, fstrm);
    return;
  }
    ...
    if (fconn->conn->flags & CO_FL_PRIVATE) {
      ...
      if (!session_add_conn(sess, fconn->conn, fconn->conn->target)) {
        ...
        if (eb_is_empty(&fconn->streams_by_id)) {
          ...
          TRACE_DEVEL("outgoing connection killed",
                      FCGI_EV_STRM_END | FCGI_EV_FCONN_ERR);
          return;
        }
      }
      if (eb_is_empty(&fconn->streams_by_id)) {
      ...
      if (eb_is_empty(&fconn->streams_by_id)) {
        ...
        return;
                 ...
                 fcgi_avail_streams(fconn->conn) > 0 &&
                 objt_server(fconn->conn->target) &&
                 !LIST_ADDED(&fconn->conn->session_list)) {
        LIST_ADD(&__objt_server(fconn->conn->target)->available_conns[tid],
                 mt_list_to_list(&fconn->conn->list));
      }
    }
  }
  ...
  if (fcgi_conn_is_dead(fconn)) {
    ...
    TRACE_DEVEL("leaving, killing dead connection", FCGI_EV_STRM_END,
                fconn->conn);
    fcgi_release(fconn);
  } else if (fconn->task) {
    fconn->task->expire =
        tick_add(now_ms, (fconn->state == FCGI_CS_CLOSED ? fconn->shut_timeout
                                                         : fconn->timeout));
    task_queue(fconn->task);
    TRACE_DEVEL("leaving, refreshing connection's timeout", FCGI_EV_STRM_END,
                fconn->conn);
  } else
...
static void fcgi_do_shutr(struct fcgi_strm *fstrm) {
  ...
  if ((fstrm->flags & FCGI_SF_KILL_CONN) &&
    ...
    TRACE_STATE("stream wants to kill the connection", FCGI_EV_STRM_SHUT,
                fconn->conn, fstrm);
    fconn->state = FCGI_CS_CLOSED;
  } else if (fstrm->flags & FCGI_SF_BEGIN_SENT) {
    TRACE_STATE("no headers sent yet, trying a retryable abort",
                FCGI_EV_STRM_SHUT, fconn->conn, fstrm);
 ...
 * deferred shutdowns when the fcgi_detach() was done but the mux buffer was
 * full and prevented the last record from being emitted.
 */
static struct task *fcgi_deferred_shut(struct task *t, void *ctx,
                                       unsigned short state) {
 ...
 */
static int fcgi_subscribe(struct conn_stream *cs, int event_type,
 ...
 */
static int fcgi_unsubscribe(struct conn_stream *cs, int event_type,
                            struct wait_event *es) {
  struct fcgi_strm *fstrm = cs->ctx;
  struct fcgi_conn *fconn = fstrm->fconn;
...
static size_t fcgi_rcv_buf(struct conn_stream *cs, struct buffer *buf,
  ...
  if (!(fconn->flags & FCGI_CF_DEM_SALLOC))
    ...
    TRACE_STATE("fstrm rxbuf not allocated",
                FCGI_EV_STRM_RECV | FCGI_EV_FSTRM_BLK, fconn->conn, fstrm);

  if (b_data(&fstrm->rxbuf) || (fstrm->h1m.state == H1_MSG_DONE &&
                                !(fstrm->flags & FCGI_SF_H1_PARSING_DONE)))
 ...
 */
static size_t fcgi_snd_buf(struct conn_stream *cs, struct buffer *buf,
                           size_t count, int flags) {
  struct fcgi_strm *fstrm = cs->ctx;
  struct fcgi_conn *fconn = fstrm->fconn;
  size_t total = 0;
  ...
  if (!(fstrm->flags & FCGI_SF_NOTIFIED) && !LIST_ISEMPTY(&fconn->send_list)) {
    TRACE_STATE("other streams already waiting, going to the queue and leaving",
                FCGI_EV_STRM_SEND | FCGI_EV_FSTRM_BLK, fconn->conn, fstrm);
    return 0;
  }
  fstrm->flags &= ~FCGI_SF_NOTIFIED;

  if (fconn->state < FCGI_CS_RECORD_H) {
    TRACE_STATE("connection not ready, leaving",
                FCGI_EV_STRM_SEND | FCGI_EV_FSTRM_BLK, fconn->conn, fstrm);
    return 0;
  }
  ...
  if (fstrm->id == 0) {
    ...
    if (id < 0) {
      fcgi_strm_close(fstrm);
      cs->flags |= CS_FL_ERROR;
      TRACE_DEVEL("couldn't get a stream ID, leaving in error",
  ...
  if (!(fstrm->flags & FCGI_SF_BEGIN_SENT)) {
    TRACE_PROTO("sending FCGI BEGIN_REQUEST record",
                FCGI_EV_TX_RECORD | FCGI_EV_TX_BEGREQ, fconn->conn, fstrm);
  ...
  while (fstrm->state < FCGI_SS_HLOC && !(fstrm->flags & FCGI_SF_BLK_ANY) &&
    ...
    case HTX_BLK_REQ_SL:
    case HTX_BLK_HDR:
      TRACE_USER("sending FCGI PARAMS record",
                 FCGI_EV_TX_RECORD | FCGI_EV_TX_PARAMS, fconn->conn, fstrm,
                 htx);
    ...
    case HTX_BLK_EOH:
      TRACE_PROTO("sending FCGI PARAMS record",
                  FCGI_EV_TX_RECORD | FCGI_EV_TX_PARAMS, fconn->conn, fstrm,
                  htx);
    ...
    case HTX_BLK_DATA:
      TRACE_PROTO("sending FCGI STDIN record",
                  FCGI_EV_TX_RECORD | FCGI_EV_TX_STDIN, fconn->conn, fstrm,
                  htx);
  ...
  if (fstrm->state == FCGI_SS_ERROR) {
    TRACE_DEVEL("reporting error to the app-layer stream",
                FCGI_EV_STRM_SEND | FCGI_EV_FSTRM_ERR | FCGI_EV_STRM_ERR,
                fconn->conn, fstrm);
    cs_set_error(cs);
    if (!(fstrm->flags & FCGI_SF_BEGIN_SENT) ||
  ...
  if (total > 0) {
    if (!(fconn->wait_event.events & SUB_RETRY_SEND)) {
      TRACE_DEVEL("data queued, waking up fconn sender",
                  FCGI_EV_STRM_SEND | FCGI_EV_FCONN_SEND | FCGI_EV_FCONN_WAKE,
                  fconn->conn, fstrm);
      tasklet_wakeup(fconn->wait_event.tasklet);
    }
...
static void fcgi_show_fd(struct buffer *msg, struct connection *conn) {
  ...
  list_for_each_entry(fstrm, &fconn->send_list, send_list) send_cnt++;
  ...
  chunk_appendf(msg,
                " fconn.st0=%d .maxid=%d .flg=0x%04x .nbst=%u"
                " .nbcs=%u .send_cnt=%d .tree_cnt=%d .orph_cnt=%d .sub=%d "
                ".dsi=%d .dbuf=%u@%p+%u/%u "
                ".mbuf=[%u..%u|%u],h=[%u@%p+%u/%u],t=[%u@%p+%u/%u]",
                fconn->state, fconn->max_id, fconn->flags, fconn->nb_streams,
                fconn->nb_cs, send_cnt, tree_cnt, orph_cnt,
                fconn->wait_event.events, fconn->dsi,
                (unsigned int)b_data(&fconn->dbuf), b_orig(&fconn->dbuf),
                (unsigned int)b_head_ofs(&fconn->dbuf),
                (unsigned int)b_size(&fconn->dbuf), br_head_idx(fconn->mbuf),
                br_tail_idx(fconn->mbuf), br_size(fconn->mbuf),
  ...
  if (fstrm) {
    chunk_appendf(
        msg, " last_fstrm=%p .id=%d .flg=0x%04x .rxbuf=%u@%p+%u/%u .cs=%p",
        fstrm, fstrm->id, fstrm->flags, (unsigned int)b_data(&fstrm->rxbuf),
        b_orig(&fstrm->rxbuf), (unsigned int)b_head_ofs(&fstrm->rxbuf),
        (unsigned int)b_size(&fstrm->rxbuf), fstrm->cs);
    if (fstrm->cs)
      chunk_appendf(msg, " .cs.flg=0x%08x .cs.data=%p", fstrm->cs->flags,
                    fstrm->cs->data);
  }
}
...
static int fcgi_takeover(struct connection *conn, int orig_tid) {
  ...
  if (conn->xprt->takeover &&
      conn->xprt->takeover(conn, conn->xprt_ctx, orig_tid) != 0) {
  ...
  fcgi->conn->xprt->subscribe(fcgi->conn, fcgi->conn->xprt_ctx, SUB_RETRY_RECV,
                              &fcgi->wait_event);
...
static struct mux_proto_list mux_proto_fcgi = {.token = IST("fcgi"),
