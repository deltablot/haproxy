...
#define H1C_F_IN_SALLOC                                                        \
  0x00000040 /* mux is blocked on lack of stream's request buffer */
/* 0x00000080 unused */

/* Flags indicating the connection state */
#define H1C_F_ST_EMBRYONIC                                                     \
  0x00000100 /* Set when a H1 stream with no conn-stream is attached to the    \
                connection */
#define H1C_F_ST_ATTACHED                                                      \
  0x00000200 /* Set when a H1 stream with a conn-stream is attached to the     \
                connection */
#define H1C_F_ST_IDLE                                                          \
  0x00000400 /* connection is idle and may be reused                           \
              * (exclusive to all H1C_F_ST flags and never set when an h1s is  \
              * attached) */
#define H1C_F_ST_ERROR                                                         \
  0x00000800 /* connection must be closed ASAP because an error occurred       \
                (conn-stream may still be attached) */
#define H1C_F_ST_SHUTDOWN                                                      \
  0x00001000 /* connection must be shut down ASAP flushing output first        \
                (conn-stream may still be attached) */
#define H1C_F_ST_ALIVE (H1C_F_ST_IDLE | H1C_F_ST_EMBRYONIC | H1C_F_ST_ATTACHED)
/* 0x00002000 - 0x00008000 unused */

#define H1C_F_WAIT_OPPOSITE                                                    \
  0x00010000 /* Don't read more data for now, waiting sync with opposite side  \
              */
#define H1C_F_WANT_SPLICE                                                      \
  0x00020000 /* Don't read into a bufffer because we want to use or we are     \
                using splicing */
#define H1C_F_ERR_PENDING                                                      \
  0x00040000 /* Send an error and close the connection ASAP (implies           \
                H1C_F_ST_ERROR) */
#define H1C_F_WAIT_NEXT_REQ                                                    \
  0x00080000 /*  waiting for the next request to start, use keep-alive timeout \
              */
#define H1C_F_UPG_H2C 0x00100000 /* set if an upgrade to h2 should be done */
#define H1C_F_CO_MSG_MORE                                                      \
  0x00200000 /* set if CO_SFL_MSG_MORE must be set when calling                \
                xprt->snd_buf() */
#define H1C_F_CO_STREAMER                                                      \
  0x00400000 /* set if CO_SFL_STREAMER must be set when calling                \
                xprt->snd_buf() */
...
#define H1S_F_REOS                                                             \
  0x00000008 /* End of input stream seen even if not delivered yet */
...
#define H1S_F_PARSING_DONE                                                     \
  0x00000400 /* Set when incoming message parsing is finished (EOM added) */
#define H1S_F_PARSING_ERROR                                                    \
  0x00000800 /* Set when an error occurred during the message parsing */
#define H1S_F_PROCESSING_ERROR                                                 \
  0x00001000 /* Set when an error occurred during the message xfer */
#define H1S_F_ERROR 0x00001800 /* stream error mask */

#define H1S_F_HAVE_SRV_NAME                                                    \
  0x00002000 /* Set during output process if the server name header was added  \
                to the request */
#define H1S_F_HAVE_O_CONN                                                      \
  0x00004000 /* Set during output process to know connection mode was          \
                processed */
...
struct h1c {
  ...
  unsigned int errcode; /* Status code when an error occurred at the H1
                           connection level */
  ...
  int idle_exp;      /* idle expiration date (http-keep-alive or http-request
                        timeout) */
...
#define H1_VERB_MINIMAL 2
    {.name = "minimal",
     .desc = "report only h1c/h1s state and flags, no real decoding"},
...
static struct task *h1_timeout_task(struct task *t, void *context,
...
static void h1_trace(enum trace_level level, uint64_t mask,
  ...
  chunk_appendf(&trace_buf, " : [%c]",
                ((h1c->flags & H1C_F_IS_BACK) ? 'B' : 'F'));

  /* Display request and response states if h1s is defined */
  if (h1s)
    chunk_appendf(&trace_buf, " [%s, %s]", h1m_state_str(h1s->req.state),
                  h1m_state_str(h1s->res.state));
  ...
  if (src->level > TRACE_LEVEL_USER) {
    if (src->verbosity == H1_VERB_COMPLETE ||
        (src->verbosity == H1_VERB_ADVANCED &&
         (mask & (H1_EV_H1C_RECV | H1_EV_STRM_RECV))))
      chunk_appendf(&trace_buf, " ibuf=%u@%p+%u/%u",
                    (unsigned int)b_data(&h1c->ibuf), b_orig(&h1c->ibuf),
                    (unsigned int)b_head_ofs(&h1c->ibuf),
                    (unsigned int)b_size(&h1c->ibuf));
    if (src->verbosity == H1_VERB_COMPLETE ||
        (src->verbosity == H1_VERB_ADVANCED &&
         (mask & (H1_EV_H1C_SEND | H1_EV_STRM_SEND))))
      chunk_appendf(&trace_buf, " obuf=%u@%p+%u/%u",
                    (unsigned int)b_data(&h1c->obuf), b_orig(&h1c->obuf),
                    (unsigned int)b_head_ofs(&h1c->obuf),
                    (unsigned int)b_size(&h1c->obuf));
  }
  ...
  if (src->level > TRACE_LEVEL_USER && htx) {
    ...
    if (src->level > TRACE_LEVEL_STATE) {
      if (src->verbosity == H1_VERB_COMPLETE)
        full = 1;
      else if (src->verbosity == H1_VERB_ADVANCED &&
               (mask & (H1_EV_RX_HDRS | H1_EV_TX_HDRS)))
        full = 1;
    }
 ...
 *     to receive
 *   - if the mux is not blocked on an input condition, we may attempt to
 * receive
...
static inline int h1_recv_allowed(const struct h1c *h1c) {
  if (h1c->flags & H1C_F_ST_ERROR) {
    TRACE_DEVEL("recv not allowed because of error on h1c",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
    return 0;
  }

  if (h1c->conn->flags & (CO_FL_ERROR | CO_FL_SOCK_RD_SH | CO_FL_WAIT_L4_CONN |
                          CO_FL_WAIT_L6_CONN)) {
    TRACE_DEVEL(
        "recv not allowed because of (error|read0|waitl4|waitl6) on connection",
        H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
    return 0;
  }

  if (h1c->h1s && (h1c->h1s->flags & H1S_F_ERROR)) {
    TRACE_DEVEL("recv not allowed because of error on h1s",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
    return 0;
  }

  if (h1c->flags & H1C_F_WAIT_OPPOSITE) {
    TRACE_DEVEL("recv not allowed (wait_opposite)",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
    return 0;
  }
  ...
  TRACE_DEVEL("recv not allowed because input is blocked",
              H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
  return 0;
}
...
static int h1_buf_available(void *target) {
  ...
  if ((h1c->flags & H1C_F_IN_ALLOC) && b_alloc_margin(&h1c->ibuf, 0)) {
    TRACE_STATE("unblocking h1c, ibuf allocated",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1c->conn);
  ...
  if ((h1c->flags & H1C_F_OUT_ALLOC) && b_alloc_margin(&h1c->obuf, 0)) {
    TRACE_STATE("unblocking h1s, obuf allocated",
                H1_EV_TX_DATA | H1_EV_H1S_BLK | H1_EV_STRM_WAKE, h1c->conn,
                h1c->h1s);
  ...
  if ((h1c->flags & H1C_F_IN_SALLOC) && h1c->h1s &&
      b_alloc_margin(&h1c->h1s->rxbuf, 0)) {
    TRACE_STATE("unblocking h1c, stream rxbuf allocated",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1c->conn);
...
static void h1_refresh_timeout(struct h1c *h1c) {
  if (h1c->task) {
    if (!(h1c->flags & H1C_F_ST_ALIVE) || (h1c->flags & H1C_F_ST_SHUTDOWN)) {
      ...
      TRACE_DEVEL("refreshing connection's timeout (dead or half-closed)",
                  H1_EV_H1C_SEND | H1_EV_H1C_RECV, h1c->conn);
    } else if (b_data(&h1c->obuf)) {
      /* connection with pending outgoing data, need a timeout (server or
       * client). */
      h1c->task->expire = tick_add(now_ms, h1c->timeout);
      TRACE_DEVEL("refreshing connection's timeout (pending outgoing data)",
                  H1_EV_H1C_SEND | H1_EV_H1C_RECV, h1c->conn);
    } else if (!(h1c->flags & H1C_F_IS_BACK) &&
               (h1c->flags & (H1C_F_ST_IDLE | H1C_F_ST_EMBRYONIC))) {
      /* front connections waiting for a stream need a timeout. */
      h1c->task->expire = tick_add(now_ms, h1c->timeout);
      TRACE_DEVEL(
          "refreshing connection's timeout (alive front h1c without a CS)",
          H1_EV_H1C_SEND | H1_EV_H1C_RECV, h1c->conn);
    } else {
      ...
      TRACE_DEVEL(
          "no connection timeout (alive back h1c or front h1c with a CS)",
          H1_EV_H1C_SEND | H1_EV_H1C_RECV, h1c->conn);
    }

    /* Finally set the idle expiration date if shorter */
    h1c->task->expire = tick_first(h1c->task->expire, h1c->idle_exp);
    TRACE_DEVEL("new expiration date", H1_EV_H1C_SEND | H1_EV_H1C_RECV,
                h1c->conn, 0, 0, (size_t[]){h1c->task->expire});
    task_queue(h1c->task);
  }
}
...
static void h1_set_idle_expiration(struct h1c *h1c) {
  if (h1c->flags & H1C_F_IS_BACK || !h1c->task) {
    TRACE_DEVEL("no idle expiration (backend connection || no task)",
                H1_EV_H1C_RECV, h1c->conn);
  ...
  if (h1c->flags & H1C_F_ST_IDLE) {
    if (!tick_isset(h1c->idle_exp)) {
      if ((h1c->flags & H1C_F_WAIT_NEXT_REQ) &&  /* Not the first request */
        ...
        TRACE_DEVEL("set idle expiration (keep-alive timeout)", H1_EV_H1C_RECV,
                    h1c->conn);
      } else {
        h1c->idle_exp = tick_add_ifset(now_ms, h1c->px->timeout.httpreq);
        TRACE_DEVEL("set idle expiration (http-request timeout)",
                    H1_EV_H1C_RECV, h1c->conn);
      }
    }
    ...
    if (!tick_isset(h1c->idle_exp)) {
      h1c->idle_exp = tick_add_ifset(now_ms, h1c->px->timeout.httpreq);
      TRACE_DEVEL("set idle expiration (http-request timeout)", H1_EV_H1C_RECV,
                  h1c->conn);
    }
  } else { // CS_ATTACHED or SHUTDOWN
    h1c->idle_exp = TICK_ETERNITY;
    TRACE_DEVEL("unset idle expiration (attached || shutdown)", H1_EV_H1C_RECV,
                h1c->conn);
  }
}
/*****************************************************************/
...
static struct conn_stream *h1s_new_cs(struct h1s *h1s, struct buffer *input) {
  ...
  if (!cs) {
    TRACE_DEVEL("leaving on CS allocation failure",
                H1_EV_STRM_NEW | H1_EV_STRM_END | H1_EV_STRM_ERR,
                h1s->h1c->conn, h1s);
    goto err;
  }
  h1s->cs = cs;
  ...
  if (global.tune.options & GTUNE_USE_SPLICE) {
    TRACE_STATE("notify the mux can use splicing", H1_EV_STRM_NEW,
                h1s->h1c->conn, h1s);
...
static struct h1s *h1s_new(struct h1c *h1c) {
  ...
  h1c->flags = (h1c->flags & ~(H1C_F_ST_IDLE | H1C_F_WAIT_NEXT_REQ)) |
               H1C_F_ST_EMBRYONIC;
...
fail:
  TRACE_DEVEL("leaving in error", H1_EV_H1S_NEW | H1_EV_H1S_END | H1_EV_H1S_ERR,
              h1c->conn);
  return NULL;
}
...
static struct h1s *h1c_bck_stream_new(struct h1c *h1c, struct conn_stream *cs,
                                      struct session *sess) {
  struct h1s *h1s;
...
static void h1s_destroy(struct h1s *h1s) {
  if (h1s) {
    ...
    h1c->flags &=
        ~(H1C_F_WAIT_OPPOSITE | H1C_F_WANT_SPLICE | H1C_F_ST_EMBRYONIC |
          H1C_F_ST_ATTACHED | H1C_F_OUT_FULL | H1C_F_OUT_ALLOC |
          H1C_F_IN_SALLOC | H1C_F_CO_MSG_MORE | H1C_F_CO_STREAMER);
    if (h1s->flags & H1S_F_ERROR) {
      h1c->flags |= H1C_F_ST_ERROR;
      TRACE_STATE("h1s on error, set error on h1c", H1_EV_H1C_ERR, h1c->conn,
                  h1s);
    }

    if (!(h1c->flags & (H1C_F_ST_ERROR |
                        H1C_F_ST_SHUTDOWN)) && /* No error/shutdown on h1c */
        !(h1c->conn->flags &
          (CO_FL_ERROR | CO_FL_SOCK_RD_SH |
           CO_FL_SOCK_WR_SH)) && /* No error/shutdown on conn */
        (h1s->flags & (H1S_F_WANT_KAL | H1S_F_PARSING_DONE)) ==
            (H1S_F_WANT_KAL | H1S_F_PARSING_DONE) && /* K/A possible */
        h1s->req.state == H1_MSG_DONE &&
        h1s->res.state == H1_MSG_DONE) { /* req/res in DONE state */
      h1c->flags |= (H1C_F_ST_IDLE | H1C_F_WAIT_NEXT_REQ);
      TRACE_STATE("set idle mode on h1c, waiting for the next request",
                  H1_EV_H1C_ERR, h1c->conn, h1s);
    } else {
...
static int h1_init(struct connection *conn, struct proxy *proxy,
  ...
  if (h1_recv_allowed(h1c))
    h1c->conn->xprt->subscribe(h1c->conn, h1c->conn->xprt_ctx, SUB_RETRY_RECV,
                               &h1c->wait_event);
    ...
    if (conn && h1c->flags & H1C_F_UPG_H2C) {
      ...
      if (h1c->wait_event.events)
        conn->xprt->unsubscribe(conn, conn->xprt_ctx, h1c->wait_event.events,
                                &h1c->wait_event);
      if (conn_upgrade_mux_fe(conn, NULL, &h1c->ibuf, ist("h2"),
                              PROTO_MODE_HTTP) != -1) {
...
static void h1_set_cli_conn_mode(struct h1s *h1s, struct h1m *h1m) {
  ...
  if (h1m->flags & H1_MF_RESP) {
    ...
    if ((h1s->meth == HTTP_METH_CONNECT && h1s->status == 200) ||
      ...
      TRACE_STATE("set tunnel mode (resp)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                  h1s->h1c->conn, h1s);
      ...
      if (!(h1m->flags & H1_MF_XFER_LEN) || (h1m->flags & H1_MF_CONN_CLO)) {
        ...
        TRACE_STATE("detect close mode (resp)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                    h1s->h1c->conn, h1s);
      } else if (!(h1m->flags & H1_MF_CONN_KAL) &&
                 (fe->options & PR_O_HTTP_MODE) == PR_O_HTTP_CLO) {
        /* no explicit keep-alive and option httpclose => close */
        h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
        TRACE_STATE("force close mode (resp)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                    h1s->h1c->conn, h1s);
      }
    }
  } else {
    ...
    if (!(h1m->flags & (H1_MF_VER_11 | H1_MF_CONN_KAL)) ||
      ...
      h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
      TRACE_STATE("detect close mode (req)", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                  h1s->h1c->conn, h1s);
    }
  }
  ...
  if (h1s->flags & H1S_F_WANT_KAL && fe->disabled) {
    h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
    TRACE_STATE("stopping, set close mode",
                H1_EV_RX_DATA | H1_EV_RX_HDRS | H1_EV_TX_DATA | H1_EV_TX_HDRS,
                h1s->h1c->conn, h1s);
  }
}
...
static void h1_set_srv_conn_mode(struct h1s *h1s, struct h1m *h1m) {
  ...
  if (h1m->flags & H1_MF_RESP) {
    ...
    if ((h1s->meth == HTTP_METH_CONNECT && h1s->status == 200) ||
       ...
       */
      h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_TUN;
      TRACE_STATE("set tunnel mode (resp)", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                  h1s->h1c->conn, h1s);
    } else if (h1s->flags & H1S_F_WANT_KAL) {
      ...
      if (!(h1m->flags & H1_MF_XFER_LEN) || h1m->flags & H1_MF_CONN_CLO ||
        ...
        /* no length known or explicit close or no explicit keep-alive in
         * HTTP/1.0 => close */
        h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
        TRACE_STATE("detect close mode (resp)", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                    h1s->h1c->conn, h1s);
      }
    }
  } else {
    ...
    if (h1m->flags & H1_MF_CONN_CLO) {
      ...
      h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
      TRACE_STATE("detect close mode (req)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                  h1s->h1c->conn, h1s);
    } else if (!(h1m->flags & H1_MF_CONN_KAL) &&
      ...
      h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
      TRACE_STATE("force close mode (req)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                  h1s->h1c->conn, h1s);
    }
  }
...
static void h1_update_req_conn_value(struct h1s *h1s, struct h1m *h1m,
                                     struct ist *conn_val) {
  ...
  if (h1s->flags & H1S_F_WANT_KAL || px->options2 & PR_O2_FAKE_KA) {
    if (!(h1m->flags & H1_MF_VER_11)) {
      TRACE_STATE("add \"Connection: keep-alive\"",
                  H1_EV_TX_DATA | H1_EV_TX_HDRS, h1s->h1c->conn, h1s);
      *conn_val = ist("keep-alive");
    }
  } else { /* H1S_F_WANT_CLO && !PR_O2_FAKE_KA */
    if (h1m->flags & H1_MF_VER_11) {
      TRACE_STATE("add \"Connection: close\"", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                  h1s->h1c->conn, h1s);
      *conn_val = ist("close");
    }
  }
...
static void h1_update_res_conn_value(struct h1s *h1s, struct h1m *h1m,
                                     struct ist *conn_val) {
  /* Don't update "Connection:" header in TUNNEL mode or if "Upgrage"
   * token is found
   */
...
static void h1_process_input_conn_mode(struct h1s *h1s, struct h1m *h1m,
                                       struct htx *htx) {
...
static void h1_process_output_conn_mode(struct h1s *h1s, struct h1m *h1m,
                                        struct ist *conn_val) {
  if (!(h1s->h1c->flags & H1C_F_IS_BACK))
    h1_set_cli_conn_mode(h1s, h1m);
  else
 ...
 * <hdrs_map>.
 */
static void h1_adjust_case_outgoing_hdr(struct h1s *h1s, struct h1m *h1m,
                                        struct ist *name) {
  struct ebpt_node *node;
  ...
  if (!(h1m->flags & H1_MF_RESP) &&
      !(h1s->h1c->px->options2 & PR_O2_H1_ADJ_BUGSRV))
    return;

  /* No conversion fo the response headers */
  if ((h1m->flags & H1_MF_RESP) &&
      !(h1s->h1c->px->options2 & PR_O2_H1_ADJ_BUGCLI))
    return;
 ...
 */
static void h1_show_error_snapshot(struct buffer *out,
...
static void h1_set_req_tunnel_mode(struct h1s *h1s) {
  ...
  TRACE_STATE("switch H1 request in tunnel mode", H1_EV_TX_DATA | H1_EV_TX_HDRS,
              h1s->h1c->conn, h1s);
  ...
  if (!(h1s->h1c->flags & H1C_F_IS_BACK)) {
    ...
    if (h1s->res.state < H1_MSG_DONE) {
      ...
      TRACE_STATE("Disable read on h1c (wait_opposite)",
                  H1_EV_RX_DATA | H1_EV_H1C_BLK, h1s->h1c->conn, h1s);
    }
  } else if (h1s->h1c->flags & H1C_F_WAIT_OPPOSITE) {
    h1s->h1c->flags &= ~H1C_F_WAIT_OPPOSITE;
    tasklet_wakeup(h1s->h1c->wait_event.tasklet);
    TRACE_STATE("Re-enable read on h1c",
                H1_EV_RX_DATA | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1s->h1c->conn,
                h1s);
  }
}
...
static void h1_set_res_tunnel_mode(struct h1s *h1s) {
  ...
  TRACE_STATE("switch H1 response in tunnel mode",
              H1_EV_TX_DATA | H1_EV_TX_HDRS, h1s->h1c->conn, h1s);
  ...
  if (h1s->h1c->flags & H1C_F_IS_BACK) {
    ...
    if (h1s->req.state < H1_MSG_DONE) {
      ...
      h1s->req.flags &= ~(H1_MF_XFER_LEN | H1_MF_CLEN | H1_MF_CHNK);
      h1s->req.state = H1_MSG_TUNNEL;
      TRACE_STATE("switch H1 request in tunnel mode",
                  H1_EV_TX_DATA | H1_EV_TX_HDRS, h1s->h1c->conn, h1s);
    }
  } else if (h1s->h1c->flags & H1C_F_WAIT_OPPOSITE) {
    h1s->h1c->flags &= ~H1C_F_WAIT_OPPOSITE;
...
static size_t h1_process_headers(struct h1s *h1s, struct h1m *h1m,
  ...
  TRACE_ENTER(H1_EV_RX_DATA | H1_EV_RX_HDRS, h1s->h1c->conn, h1s, 0,
              (size_t[]){max});
  ...
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                H1_EV_RX_DATA | H1_EV_RX_HDRS, h1s->h1c->conn, h1s);
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      h1s->flags |= H1S_F_PARSING_ERROR;
      TRACE_USER("parsing error, reject H1 message",
                 H1_EV_RX_DATA | H1_EV_RX_HDRS | H1_EV_H1S_ERR, h1s->h1c->conn,
                 h1s);
  ...
  if (h1m->err_pos >= 0) {
     ...
     * now.
     */
    TRACE_STATE("Ignored parsing error", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                h1s->h1c->conn, h1s);
    h1_capture_bad_message(h1s->h1c, h1s, h1m, buf);
  }
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(H1_EV_RX_DATA | H1_EV_RX_HDRS, h1s->h1c->conn, h1s, 0,
              (size_t[]){ret});
  return ret;
}
...
static size_t h1_process_data(struct h1s *h1s, struct h1m *h1m,
                              struct htx **htx, struct buffer *buf, size_t *ofs,
                              size_t max, struct buffer *htxbuf) {
  int ret;

  TRACE_ENTER(H1_EV_RX_DATA | H1_EV_RX_BODY, h1s->h1c->conn, h1s, 0,
              (size_t[]){max});
  ret = h1_parse_msg_data(h1m, htx, buf, *ofs, max, htxbuf);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                H1_EV_RX_DATA | H1_EV_RX_BODY, h1s->h1c->conn, h1s);
    if ((*htx)->flags & HTX_FL_PARSING_ERROR) {
      h1s->flags |= H1S_F_PARSING_ERROR;
      TRACE_USER("parsing error, reject H1 message",
                 H1_EV_RX_DATA | H1_EV_RX_BODY | H1_EV_H1S_ERR, h1s->h1c->conn,
                 h1s);
      h1_capture_bad_message(h1s->h1c, h1s, h1m, buf);
    }
    goto end;
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(H1_EV_RX_DATA | H1_EV_RX_BODY, h1s->h1c->conn, h1s, 0,
              (size_t[]){ret});
  return ret;
}
 ...
 */
static size_t h1_process_trailers(struct h1s *h1s, struct h1m *h1m,
                                  struct htx *htx, struct buffer *buf,
                                  size_t *ofs, size_t max) {
  int ret;

  TRACE_ENTER(H1_EV_RX_DATA | H1_EV_RX_TLRS, h1s->h1c->conn, h1s, 0,
              (size_t[]){max});
  ...
  if (!ret) {
    ...
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      h1s->flags |= H1S_F_PARSING_ERROR;
      TRACE_USER("parsing error, reject H1 message",
                 H1_EV_RX_DATA | H1_EV_RX_TLRS | H1_EV_H1S_ERR, h1s->h1c->conn,
                 h1s);
      h1_capture_bad_message(h1s->h1c, h1s, h1m, buf);
    }
    goto end;
  ...
  *ofs += ret;
  ...
  TRACE_LEAVE(H1_EV_RX_DATA | H1_EV_RX_TLRS, h1s->h1c->conn, h1s, 0,
              (size_t[]){ret});
  return ret;
}
...
static size_t h1_process_eom(struct h1s *h1s, struct h1m *h1m, struct htx *htx,
  ...
  TRACE_ENTER(H1_EV_RX_DATA | H1_EV_RX_EOI, h1s->h1c->conn, h1s, 0,
              (size_t[]){max});
  ret = h1_parse_msg_eom(h1m, htx, max);
  if (!ret) {
    TRACE_DEVEL("leaving on missing data or error",
                H1_EV_RX_DATA | H1_EV_RX_EOI, h1s->h1c->conn, h1s);
    if (htx->flags & HTX_FL_PARSING_ERROR) {
      h1s->flags |= H1S_F_PARSING_ERROR;
      TRACE_USER("parsing error, reject H1 message",
                 H1_EV_RX_DATA | H1_EV_RX_EOI | H1_EV_H1S_ERR, h1s->h1c->conn,
                 h1s);
      h1_capture_bad_message(h1s->h1c, h1s, h1m, buf);
    }
    goto end;
  ...
  TRACE_LEAVE(H1_EV_RX_DATA | H1_EV_RX_EOI, h1s->h1c->conn, h1s, 0,
              (size_t[]){ret});
  return ret;
}
 ...
 * it couldn't proceed.
 */
static size_t h1_process_input(struct h1c *h1c, struct buffer *buf,
                               size_t count) {
  ...
  do {
    ...
    if (h1m->state <= H1_MSG_LAST_LF) {
      TRACE_PROTO("parsing message headers", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                  h1c->conn, h1s);
      ret = h1_process_headers(h1s, h1m, htx, &h1c->ibuf, &total, count);
      if (!ret)
        break;

      TRACE_USER((!(h1m->flags & H1_MF_RESP) ? "rcvd H1 request headers"
                                             : "rcvd H1 response headers"),
                 H1_EV_RX_DATA | H1_EV_RX_HDRS, h1c->conn, h1s, htx,
                 (size_t[]){ret});

      if ((h1m->flags & H1_MF_RESP) && h1s->status < 200 &&
          (h1s->status == 100 || h1s->status >= 102)) {
        h1m_init_res(&h1s->res);
        h1m->flags |= (H1_MF_NO_PHDR | H1_MF_CLEAN_CONN_HDR);
        TRACE_STATE("1xx response rcvd", H1_EV_RX_DATA | H1_EV_RX_HDRS,
                    h1c->conn, h1s);
      }
    } else if (h1m->state < H1_MSG_TRAILERS) {
      TRACE_PROTO("parsing message payload", H1_EV_RX_DATA | H1_EV_RX_BODY,
                  h1c->conn, h1s);
      ...
      TRACE_PROTO(
          (!(h1m->flags & H1_MF_RESP) ? "rcvd H1 request payload data"
                                      : "rcvd H1 response payload data"),
          H1_EV_RX_DATA | H1_EV_RX_BODY, h1c->conn, h1s, htx, (size_t[]){ret});
    } else if (h1m->state == H1_MSG_TRAILERS) {
      TRACE_PROTO("parsing message trailers", H1_EV_RX_DATA | H1_EV_RX_TLRS,
                  h1c->conn, h1s);
      ret = h1_process_trailers(h1s, h1m, htx, &h1c->ibuf, &total, count);
      if (!ret && h1m->state != H1_MSG_DONE)
        break;

      TRACE_PROTO((!(h1m->flags & H1_MF_RESP) ? "rcvd H1 request trailers"
                                              : "rcvd H1 response trailers"),
                  H1_EV_RX_DATA | H1_EV_RX_TLRS, h1c->conn, h1s, htx,
                  (size_t[]){ret});
      ...
      if (!(h1s->flags & H1S_F_PARSING_DONE)) {
        ...
        TRACE_USER((!(h1m->flags & H1_MF_RESP) ? "H1 request fully rcvd"
                                               : "H1 response fully rcvd"),
      ...
      if (!(h1m->flags & H1_MF_RESP) && h1s->status == 101)
        ...
        TRACE_STATE("Disable read on h1c (wait_opposite)",
                    H1_EV_RX_DATA | H1_EV_H1C_BLK, h1c->conn, h1s);
        break;
      } else
        break;
      ...
      TRACE_PROTO(
          (!(h1m->flags & H1_MF_RESP) ? "rcvd H1 request tunneled data"
                                      : "rcvd H1 response tunneled data"),
  ...
  htx_to_buf(htx, buf);
  TRACE_DEVEL("incoming data parsed", H1_EV_RX_DATA, h1c->conn, h1s, htx,
              (size_t[]){ret});

  ret = htx->data - data;
  if ((h1c->flags & H1C_F_IN_FULL) && buf_room_for_htx_data(&h1c->ibuf)) {
    h1c->flags &= ~H1C_F_IN_FULL;
    TRACE_STATE("h1c ibuf not full anymore",
                H1_EV_RX_DATA | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1c->conn, h1s);
    h1c->conn->xprt->subscribe(h1c->conn, h1c->conn->xprt_ctx, SUB_RETRY_RECV,
                               &h1c->wait_event);
  }
  ...
  if (!h1s->cs) {
    if (h1m->state <= H1_MSG_LAST_LF) {
      TRACE_STATE("Incomplete message, subscribing",
                  H1_EV_RX_DATA | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1c->conn,
                  h1s);
      h1c->conn->xprt->subscribe(h1c->conn, h1c->conn->xprt_ctx, SUB_RETRY_RECV,
                                 &h1c->wait_event);
      goto end;
  ...
  if (!(h1m->flags & H1_MF_CHNK) &&
    ...
    TRACE_STATE("notify the mux can use splicing",
                H1_EV_RX_DATA | H1_EV_RX_BODY, h1c->conn, h1s);
    h1s->cs->flags |= CS_FL_MAY_SPLICE;
  } else {
    TRACE_STATE("notify the mux can't use splicing anymore",
                H1_EV_RX_DATA | H1_EV_RX_BODY, h1c->conn, h1s);
    h1s->cs->flags &= ~CS_FL_MAY_SPLICE;
  }
...
err:
  ...
  if (h1s->cs)
    h1s->cs->flags |= CS_FL_EOI;
  TRACE_DEVEL("leaving on error", H1_EV_RX_DATA | H1_EV_STRM_ERR, h1c->conn,
              h1s);
  return 0;
}

/*
 * Process outgoing data. It parses data and transfer them from the channel
 * buffer into h1c->obuf. It returns the number of bytes parsed and transferred
 * if > 0, or 0 if it couldn't proceed.
 */
static size_t h1_process_output(struct h1c *h1c, struct buffer *buf,
                                size_t count) {
  struct h1s *h1s = h1c->h1s;
  struct h1m *h1m;
  ...
  if (!h1_get_buf(h1c, &h1c->obuf)) {
    ...
    TRACE_STATE("waiting for h1c obuf allocation",
                H1_EV_TX_DATA | H1_EV_H1S_BLK, h1c->conn, h1s);
    goto end;
  }
   ...
   */
  if (!b_data(&h1c->obuf)) {
    if (htx_nbblks(chn_htx) == 1 && htx_get_blk_type(blk) == HTX_BLK_DATA &&
        htx_get_blk_value(chn_htx, blk).len == count) {
      void *old_area = h1c->obuf.area;

      TRACE_PROTO("sending message data (zero-copy)",
                  H1_EV_TX_DATA | H1_EV_TX_BODY, h1c->conn, h1s, chn_htx,
                  (size_t[]){count});
      ...
      if (h1m->state == H1_MSG_DATA)
        TRACE_PROTO((!(h1m->flags & H1_MF_RESP)
                         ? "H1 request payload data xferred"
                         : "H1 response payload data xferred"),
                    H1_EV_TX_DATA | H1_EV_TX_BODY, h1c->conn, h1s, 0,
                    (size_t[]){count});
      else
        TRACE_PROTO((!(h1m->flags & H1_MF_RESP)
                         ? "H1 request tunneled data xferred"
                         : "H1 response tunneled data xferred"),
                    H1_EV_TX_DATA | H1_EV_TX_BODY, h1c->conn, h1s, 0,
                    (size_t[]){count});
      goto out;
    }
  ...
  while (count && !(h1s->flags & H1S_F_PROCESSING_ERROR) && blk) {
    ...
    case H1_MSG_RQBEFORE:
      if (type != HTX_BLK_REQ_SL)
        goto error;
      TRACE_USER("sending request headers", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                 h1c->conn, h1s, chn_htx);
      ...
      if (h1c->flags & H1C_F_WAIT_OPPOSITE) {
        ...
        TRACE_STATE("Re-enable read on h1c",
                    H1_EV_TX_DATA | H1_EV_H1C_BLK | H1_EV_H1C_WAKE, h1c->conn,
                    h1s);
      }
      break;
    ...
    case H1_MSG_RPBEFORE:
      if (type != HTX_BLK_RES_SL)
        goto error;
      TRACE_USER("sending response headers", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                 h1c->conn, h1s, chn_htx);
      sl = htx_get_blk_ptr(chn_htx, blk);
    ...
    last_lf:
      ...
      if (!(h1s->flags & H1S_F_HAVE_O_CONN)) {
        ...
        if ((chn_htx->flags & HTX_FL_PROXY_RESP) &&
            h1s->req.state != H1_MSG_DONE) {
          h1s->flags = (h1s->flags & ~H1S_F_WANT_MSK) | H1S_F_WANT_CLO;
          TRACE_STATE("force close mode (resp)", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                      h1s->h1c->conn, h1s);
      ...
      if ((h1s->meth != HTTP_METH_CONNECT &&
           (h1m->flags & (H1_MF_VER_11 | H1_MF_RESP | H1_MF_CLEN | H1_MF_CHNK |
                          H1_MF_XFER_LEN)) ==
               (H1_MF_VER_11 | H1_MF_XFER_LEN)) ||
          (h1s->status >= 200 && h1s->status != 204 && h1s->status != 304 &&
           h1s->meth != HTTP_METH_HEAD &&
        ...
        if (!h1_format_htx_hdr(n, v, &tmp))
          goto full;
        TRACE_STATE("add \"Transfer-Encoding: chunked\"",
                    H1_EV_TX_DATA | H1_EV_TX_HDRS, h1c->conn, h1s);
        h1m->flags |= H1_MF_CHNK;
      }

      /* Now add the server name to a header (if requested) */
      if (!(h1s->flags & H1S_F_HAVE_SRV_NAME) && !(h1m->flags & H1_MF_RESP) &&
        ...
        TRACE_STATE("add server name header", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                    h1c->conn, h1s);
      ...
      TRACE_PROTO((!(h1m->flags & H1_MF_RESP) ? "H1 request headers xferred"
                                              : "H1 response headers xferred"),
                  H1_EV_TX_DATA | H1_EV_TX_HDRS, h1c->conn, h1s);
      ...
      if (!(h1m->flags & H1_MF_RESP) && h1s->meth == HTTP_METH_CONNECT) {
        ...
        TRACE_STATE("switch H1 response in tunnel mode",
                    H1_EV_TX_DATA | H1_EV_TX_HDRS, h1c->conn, h1s);
      } else if ((h1m->flags & H1_MF_RESP) && h1s->status < 200 &&
                 (h1s->status == 100 || h1s->status >= 102)) {
        h1m_init_res(&h1s->res);
        h1m->flags |= (H1_MF_NO_PHDR | H1_MF_CLEAN_CONN_HDR);
        h1s->flags &= ~H1S_F_HAVE_O_CONN;
        ...
        TRACE_STATE("HEAD response processed", H1_EV_TX_DATA | H1_EV_TX_HDRS,
                    h1c->conn, h1s);
      } else
    ...
    case H1_MSG_TUNNEL:
      if (type == HTX_BLK_EOM) {
        ...
        if ((h1m->flags & H1_MF_CHNK) && !chunk_memcat(&tmp, "0\r\n", 3))
          goto full;
        TRACE_PROTO("sending message trailers", H1_EV_TX_DATA | H1_EV_TX_TLRS,
                    h1c->conn, h1s, chn_htx);
        goto trailers;
      } else if (type != HTX_BLK_DATA)
        goto error;

      TRACE_PROTO("sending message data", H1_EV_TX_DATA | H1_EV_TX_BODY,
                  h1c->conn, h1s, chn_htx, (size_t[]){sz});
    ...
    trailers:
      ...
      if (type == HTX_BLK_EOT) {
        if (!chunk_memcat(&tmp, "\r\n", 2))
          goto full;
        TRACE_PROTO((!(h1m->flags & H1_MF_RESP)
                         ? "H1 request trailers xferred"
                         : "H1 response trailers xferred"),
    ...
    case H1_MSG_DONE:
      ...
      TRACE_USER((!(h1m->flags & H1_MF_RESP) ? "H1 request fully xferred"
    ...
    error:
      ...
      TRACE_STATE("processing error, set error on h1c/h1s",
                  H1_EV_H1C_ERR | H1_EV_H1S_ERR, h1c->conn, h1s);
      TRACE_DEVEL("unexpected error", H1_EV_TX_DATA | H1_EV_STRM_ERR, h1c->conn,
                  h1s);
      break;
    }
...
/* Try to send an HTTP error with h1c->errcode status code. It returns 1 on
 * success and 0 on error. The flag H1C_F_ERR_PENDING is set on the H1
 * connection for retryable errors (allocation error or buffer full). On
 * success, the error is copied in the output buffer.
 */
static int h1_send_error(struct h1c *h1c) {
  ...
  /// XXX: do a function !
  if (h1c->px->replies[rc] && h1c->px->replies[rc]->type == HTTP_REPLY_ERRMSG &&
  ...
  ret =
      b_istput(&h1c->obuf, ist2(http_err_msgs[rc], strlen(http_err_msgs[rc])));
  if (unlikely(ret <= 0)) {
    if (!ret) {
...
static int h1_handle_bad_req(struct h1c *h1c) {
  ...
  if (!b_data(&h1c->ibuf) && ((h1c->flags & H1C_F_WAIT_NEXT_REQ) ||
                              (sess->fe->options & PR_O_IGNORE_PRB)))
    goto end;
...
static int h1_recv(struct h1c *h1c) {
  ...
  if ((h1c->flags & H1C_F_WANT_SPLICE) || !h1_recv_allowed(h1c)) {
    TRACE_DEVEL("leaving on (want_splice|!recv_allowed)", H1_EV_H1C_RECV,
                h1c->conn);
    return 1;
  }
  ...
  if (!h1_get_buf(h1c, &h1c->ibuf)) {
    h1c->flags |= H1C_F_IN_ALLOC;
    TRACE_STATE("waiting for h1c ibuf allocation",
                H1_EV_H1C_RECV | H1_EV_H1C_BLK, h1c->conn);
    return 0;
  }
  ...
  if (!h1c->h1s ||
      (!(h1c->flags & H1C_F_IS_BACK) &&
       h1c->h1s->req.state == H1_MSG_RQBEFORE) ||
  ...
  if (max && !ret && h1_recv_allowed(h1c)) {
    TRACE_STATE("failed to receive data, subscribing", H1_EV_H1C_RECV,
                h1c->conn);
    conn->xprt->subscribe(conn, conn->xprt_ctx, SUB_RETRY_RECV,
                          &h1c->wait_event);
  } else {
    h1_wake_stream_for_recv(h1c->h1s);
    TRACE_DATA("data received", H1_EV_H1C_RECV, h1c->conn, 0, 0,
               (size_t[]){ret});
  }
...
static int h1_send(struct h1c *h1c) {
  ...
  ret = conn->xprt->snd_buf(conn, conn->xprt_ctx, &h1c->obuf,
                            b_data(&h1c->obuf), flags);
  if (ret > 0) {
    TRACE_DATA("data sent", H1_EV_H1C_SEND, h1c->conn, 0, 0, (size_t[]){ret});
    if (h1c->flags & H1C_F_OUT_FULL) {
      h1c->flags &= ~H1C_F_OUT_FULL;
      TRACE_STATE("h1c obuf not full anymore", H1_EV_STRM_SEND | H1_EV_H1S_BLK,
                  h1c->conn);
    }
    b_del(&h1c->obuf, ret);
    sent = 1;
  ...
  if (!b_data(&h1c->obuf)) {
    ...
    if (h1c->flags & H1C_F_ST_SHUTDOWN) {
      TRACE_STATE("process pending shutdown for writes", H1_EV_H1C_SEND,
                  h1c->conn);
...
static int h1_process(struct h1c *h1c) {
  ...
  /* Try to parse now the first block of a request, creating the H1 stream if
   * necessary */
  if (b_data(&h1c->ibuf) && /* Input data to be processed */
      (h1c->flags &
       (H1C_F_ST_IDLE | H1C_F_ST_EMBRYONIC)) && /* IDLE h1 connection or no CS
                                                   attached to the h1 stream */
      !(h1c->flags &
        H1C_F_IN_SALLOC)) { /* No allocation failure on the stream rxbuf */
    struct buffer *buf;
    size_t count;

    /* When it happens for a backend connection, we may release it (it is
     * probably a 408) */
    if (h1c->flags & H1C_F_IS_BACK)
      goto release;

    /* First of all handle H1 to H2 upgrade (no need to create the H1 stream) */
    if (((h1c->flags & (H1C_F_ST_IDLE | H1C_F_WAIT_NEXT_REQ)) ==
         H1C_F_ST_IDLE) && /* First request with no h1s */
        !(h1c->px->options2 &
          PR_O2_NO_H2_UPGRADE)) { /* H2 upgrade supported by the proxy */
      /* Try to match H2 preface before parsing the request headers. */
      if (b_isteq(&h1c->ibuf, 0, b_data(&h1c->ibuf), ist(H2_CONN_PREFACE)) >
          0) {
        h1c->flags |= H1C_F_UPG_H2C;
        TRACE_STATE("release h1c to perform H2 upgrade ",
                    H1_EV_RX_DATA | H1_EV_H1C_WAKE);
        goto release;
      }
    }
    ...
    if (h1s->sess->t_idle == -1)
      h1s->sess->t_idle =
          tv_ms_elapsed(&h1s->sess->tv_accept, &now) - h1s->sess->t_handshake;

    /* Get the stream rxbuf */
    buf = h1_get_buf(h1c, &h1s->rxbuf);
    if (!buf) {
      h1c->flags |= H1C_F_IN_SALLOC;
      TRACE_STATE("waiting for stream rxbuf allocation",
                  H1_EV_H1C_WAKE | H1_EV_H1C_BLK, h1c->conn);
      return 0;
    }
  ...
  if ((h1c->flags & H1C_F_WANT_SPLICE) && !h1s_data_pending(h1s)) {
    TRACE_DEVEL("xprt rcv_buf blocked (want_splice), notify h1s for recv",
                H1_EV_H1C_RECV, h1c->conn);
...
static struct task *h1_timeout_task(struct task *t, void *context,
  ...
  if (h1c) {
    ...
    if (h1c->flags & H1C_F_ST_ATTACHED) {
      ...
      TRACE_DEVEL("leaving (CS still attached)", H1_EV_H1C_WAKE, h1c->conn,
                  h1c->h1s);
      return t;
    }

    /* Try to send an error to the client */
    if (!(h1c->flags & (H1C_F_IS_BACK | H1C_F_ST_ERROR | H1C_F_ERR_PENDING |
                        H1C_F_ST_SHUTDOWN))) {
...
static struct conn_stream *h1_attach(struct connection *conn,
  ...
  if (h1c->flags & H1C_F_ST_ERROR) {
    TRACE_DEVEL("leaving on h1c error",
                H1_EV_STRM_NEW | H1_EV_STRM_END | H1_EV_STRM_ERR, conn);
    goto end;
  }
  ...
  if (h1s == NULL) {
    TRACE_DEVEL("leaving on h1s creation failure",
                H1_EV_STRM_NEW | H1_EV_STRM_END | H1_EV_STRM_ERR, conn);
    goto end;
  }
...
h1_get_first_cs(const struct connection *conn) {
  struct h1c *h1c = conn->ctx;
  struct h1s *h1s = h1c->h1s;
...
static void h1_detach(struct conn_stream *cs) {
  ...
  if ((h1c->flags & (H1C_F_IS_BACK | H1C_F_ST_IDLE)) ==
      (H1C_F_IS_BACK | H1C_F_ST_IDLE)) {
    ...
    if (b_data(&h1c->ibuf)) {
      ...
      TRACE_DEVEL("remaining data on detach, kill connection",
...
release:
  /* We don't want to close right now unless the connection is in error or shut
   * down for writes */
  if ((h1c->flags & H1C_F_ST_ERROR) ||
...
static void h1_shutr(struct conn_stream *cs, enum cs_shr_mode mode) {
  ...
  if (cs->flags & CS_FL_KILL_CONN) {
    TRACE_STATE("stream wants to kill the connection", H1_EV_STRM_SHUT,
                h1c->conn, h1s);
    goto do_shutr;
  }
  if (h1c->conn->flags & (CO_FL_ERROR | CO_FL_SOCK_RD_SH | CO_FL_SOCK_WR_SH)) {
    TRACE_STATE("shutdown on connection (error|rd_sh|wr_sh)", H1_EV_STRM_SHUT,
                h1c->conn, h1s);
    goto do_shutr;
  }

  if (h1s->flags & H1S_F_WANT_KAL) {
    TRACE_STATE("keep connection alive (want_kal)", H1_EV_STRM_SHUT, h1c->conn,
                h1s);
    goto end;
  }
...
do_shutr:
  ...
  if (conn_xprt_ready(cs->conn) && cs->conn->xprt->shutr)
    cs->conn->xprt->shutr(cs->conn, cs->conn->xprt_ctx, (mode == CS_SHR_DRAIN));
end:
...
static void h1_shutw(struct conn_stream *cs, enum cs_shw_mode mode) {
  ...
  if (((h1s->flags & H1S_F_WANT_KAL) && h1s->req.state == H1_MSG_DONE &&
 ...
 */
static int h1_unsubscribe(struct conn_stream *cs, int event_type,
 ...
 */
static int h1_subscribe(struct conn_stream *cs, int event_type,
                        struct wait_event *es) {
  struct h1s *h1s = cs->ctx;
  struct h1c *h1c;
  ...
  if (event_type & SUB_RETRY_SEND) {
    ...
    h1c = h1s->h1c;
    if (!(h1c->wait_event.events & SUB_RETRY_SEND))
      h1c->conn->xprt->subscribe(h1c->conn, h1c->conn->xprt_ctx, SUB_RETRY_SEND,
                                 &h1c->wait_event);
  }
  return 0;
}
...
static size_t h1_rcv_buf(struct conn_stream *cs, struct buffer *buf,
  ...
  if (!(h1c->flags & H1C_F_IN_ALLOC))
    ...
    TRACE_DEVEL("h1c ibuf not allocated", H1_EV_H1C_RECV | H1_EV_H1C_BLK,
                h1c->conn);
  ...
  if (flags & CO_RFL_BUF_FLUSH) {
    if (h1m->state == H1_MSG_TUNNEL ||
      ...
      TRACE_STATE("Block xprt rcv_buf to flush stream's buffer (want_splice)",
                  H1_EV_STRM_RECV, h1c->conn, h1s);
    }
  } else {
...
static size_t h1_snd_buf(struct conn_stream *cs, struct buffer *buf,
                         size_t count, int flags) {
  struct h1s *h1s = cs->ctx;
  struct h1c *h1c;
  size_t total = 0;
  ...
  while (count) {
    ...
    if (!(h1c->flags & (H1C_F_OUT_FULL | H1C_F_OUT_ALLOC)))
      ...
      TRACE_DEVEL("h1c obuf not allocated", H1_EV_STRM_SEND | H1_EV_H1S_BLK,
                  h1c->conn, h1s);
...
static int h1_rcv_pipe(struct conn_stream *cs, struct pipe *pipe,
                       unsigned int count) {
  struct h1s *h1s = cs->ctx;
  struct h1c *h1c = h1s->h1c;
  struct h1m *h1m = (!(h1c->flags & H1C_F_IS_BACK) ? &h1s->req : &h1s->res);
  ...
  if ((h1m->flags & H1_MF_CHNK) ||
    ...
    h1c->flags &= ~H1C_F_WANT_SPLICE;
    TRACE_STATE("Allow xprt rcv_buf on !(msg_data|msg_tunnel)", H1_EV_STRM_RECV,
                cs->conn, h1s);
    if (!(h1c->wait_event.events & SUB_RETRY_RECV)) {
      TRACE_STATE("restart receiving data, subscribing", H1_EV_STRM_RECV,
                  cs->conn, h1s);
      cs->conn->xprt->subscribe(cs->conn, cs->conn->xprt_ctx, SUB_RETRY_RECV,
                                &h1c->wait_event);
    }
    goto end;
  }
  ...
  if (!(h1c->flags & H1C_F_WANT_SPLICE)) {
    h1c->flags |= H1C_F_WANT_SPLICE;
    TRACE_STATE("Block xprt rcv_buf to perform splicing", H1_EV_STRM_RECV,
                cs->conn, h1s);
  }
  if (h1s_data_pending(h1s)) {
    TRACE_STATE("flush input buffer before splicing", H1_EV_STRM_RECV, cs->conn,
                h1s);
    goto end;
  }
  ...
  if (h1m->state == H1_MSG_DATA && ret >= 0) {
    ...
    if (!h1m->curr_len) {
      h1c->flags &= ~H1C_F_WANT_SPLICE;
      TRACE_STATE("Allow xprt rcv_buf on !curr_len", H1_EV_STRM_RECV, cs->conn,
                  h1s);
    }
  }
  ...
  if ((h1s->flags & H1S_F_REOS) ||
      ...
      (h1m->state == H1_MSG_DATA && !h1m->curr_len)) {
    TRACE_STATE("notify the mux can't use splicing anymore", H1_EV_STRM_RECV,
                h1c->conn, h1s);
    cs->flags &= ~CS_FL_MAY_SPLICE;
  }
...
static int h1_snd_pipe(struct conn_stream *cs, struct pipe *pipe) {
  ...
  if (pipe->data) {
    if (!(h1s->h1c->wait_event.events & SUB_RETRY_SEND)) {
      TRACE_STATE("more data to send, subscribing", H1_EV_STRM_SEND, cs->conn,
                  h1s);
      cs->conn->xprt->subscribe(cs->conn, cs->conn->xprt_ctx, SUB_RETRY_SEND,
                                &h1s->h1c->wait_event);
    }
  }
...
static int h1_ctl(struct connection *conn, enum mux_ctl_type mux_ctl,
                  void *output) {
...
static void h1_show_fd(struct buffer *msg, struct connection *conn) {
  ...
  chunk_appendf(
      msg, " h1c.flg=0x%x .sub=%d .ibuf=%u@%p+%u/%u .obuf=%u@%p+%u/%u",
      h1c->flags, h1c->wait_event.events, (unsigned int)b_data(&h1c->ibuf),
      b_orig(&h1c->ibuf), (unsigned int)b_head_ofs(&h1c->ibuf),
      (unsigned int)b_size(&h1c->ibuf), (unsigned int)b_data(&h1c->obuf),
      b_orig(&h1c->obuf), (unsigned int)b_head_ofs(&h1c->obuf),
      (unsigned int)b_size(&h1c->obuf));
  ...
  if (h1s) {
    ...
    if (h1s->meth < HTTP_METH_OTHER)
      ...
      method = "UNKNOWN";
    chunk_appendf(msg,
                  " h1s=%p h1s.flg=0x%x .req.state=%s .res.state=%s"
                  " .meth=%s status=%d",
                  h1s, h1s->flags, h1m_state_str(h1s->req.state),
                  h1m_state_str(h1s->res.state), method, h1s->status);
    if (h1s->cs)
      chunk_appendf(msg, " .cs.flg=0x%08x .cs.data=%p", h1s->cs->flags,
                    h1s->cs->data);
  }
}
...
static int cfg_h1_headers_case_adjust_postparser() {
  ...
  if (!file) {
    ha_alert("config : h1-outgoing-headers-case-adjust-file '%s': failed to "
             "open file.\n",
             hdrs_map.name);
  ...
  while (fgets(trash.area, trash.size, file) != NULL) {
    ...
    *value_end = '\0';
    ...
    if (rc < 0) {
      ha_alert("config : h1-outgoing-headers-case-adjust-file '%s' : %s at "
               "line %d.\n",
    ...
    if (rc > 0) {
      ha_warning("config : h1-outgoing-headers-case-adjust-file '%s' : %s at "
                 "line %d.\n",
                 hdrs_map.name, err, line);
      err_code |= ERR_WARN;
      free(err);
...
static int cfg_parse_h1_header_case_adjust(char **args, int section_type,
                                           struct proxy *curpx,
                                           struct proxy *defpx,
...
static int cfg_parse_h1_headers_case_adjust_file(char **args, int section_type,
                                                 struct proxy *curpx,
                                                 struct proxy *defpx,
                                                 const char *file, int line,
                                                 char **err) {
...
static struct cfg_kw_list cfg_kws = {
    {},
    {
        ...
        {CFG_GLOBAL, "h1-case-adjust-file",
         cfg_parse_h1_headers_case_adjust_file},
        {0, NULL, NULL},
    }};

INITCALL1(STG_REGISTER, cfg_register_keywords, &cfg_kws);
REGISTER_CONFIG_POSTPARSER("h1-headers-map",
                           cfg_h1_headers_case_adjust_postparser);
...
static struct mux_proto_list mux_proto_htx = {.token = IST(""),
