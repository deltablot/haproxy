...
DECLARE_STATIC_POOL(pool_head_comp_state, "comp_state",
                    sizeof(struct comp_state));
...
static int htx_compression_buffer_add_data(struct comp_state *st,
                                           const char *data, size_t len,
                                           struct buffer *out);
static int htx_compression_buffer_end(struct comp_state *st, struct buffer *out,
                                      int end);

/***********************************************************************/
static int comp_flt_init(struct proxy *px, struct flt_conf *fconf) {
...
static void comp_flt_deinit_per_thread(struct proxy *px,
...
static int comp_strm_init(struct stream *s, struct filter *filter) {
...
static void comp_strm_deinit(struct stream *s, struct filter *filter) {
...
static int comp_http_headers(struct stream *s, struct filter *filter,
                             struct http_msg *msg) {
  struct comp_state *st = filter->ctx;
...
static int comp_http_post_analyze(struct stream *s, struct filter *filter,
...
static int comp_http_payload(struct stream *s, struct filter *filter,
  ...
  for (; blk && len; blk = htx_get_next_blk(htx, blk)) {
    ...
    case HTX_BLK_DATA:
      ...
      v.len = ret;
      blk = htx_replace_blk_value(htx, blk, v,
                                  ist2(b_head(&trash), b_data(&trash)));
      break;
...
static int comp_http_end(struct stream *s, struct filter *filter,
                         struct http_msg *msg) {
  struct comp_state *st = filter->ctx;
...
static int set_compression_response_header(struct comp_state *st,
   ...
   * header.
   */
  if (st->comp_algo->cfg_name_len != 8 ||
      memcmp(st->comp_algo->cfg_name, "identity", 8) != 0) {
...
static int select_compression_request_header(struct comp_state *st,
  ...
  /* Disable compression for older user agents announcing themselves as
   * "Mozilla/4" unless they are known good (MSIE 6 with XP SP2, or MSIE 7 and
   * later). See http://zoompf.com/2012/02/lose-the-wait-http-compression for
   * more details.
   */
  ctx.blk = NULL;
  if (http_find_header(htx, ist("User-Agent"), &ctx, 1) && ctx.value.len >= 9 &&
      memcmp(ctx.value.ptr, "Mozilla/4", 9) == 0 &&
      (ctx.value.len < 31 || memcmp(ctx.value.ptr + 25, "MSIE ", 5) != 0 ||
       *(ctx.value.ptr + 30) < '6' ||
       (*(ctx.value.ptr + 30) == '6' &&
  ...
  if ((s->be->comp && (comp_algo_back = s->be->comp->algos)) ||
    ...
    while (http_find_header(htx, ist("Accept-Encoding"), &ctx, 0)) {
      ...
      while (1) {
        ...
        if (strncmp(qval, "q=", MIN(ctx.value.ptr + ctx.value.len - qval, 2)) ==
            0)
          break;
      ...
      /* here we have qval pointing to the first "q=" attribute or NULL if not
       * found */
      ...
      for (comp_algo = comp_algo_back; comp_algo; comp_algo = comp_algo->next) {
        if (*(ctx.value.ptr) == '*' ||
            word_match(ctx.value.ptr, toklen, comp_algo->ua_name,
                       comp_algo->ua_name_len)) {
  ...
  if ((s->be->comp && (comp_algo_back = s->be->comp->algos)) ||
      (strm_fe(s)->comp && (comp_algo_back = strm_fe(s)->comp->algos))) {
    for (comp_algo = comp_algo_back; comp_algo; comp_algo = comp_algo->next) {
      if (comp_algo->cfg_name_len == 8 &&
          memcmp(comp_algo->cfg_name, "identity", 8) == 0) {
        st->comp_algo = comp_algo;
        return 1;
      }
    }
...
static int select_compression_response_header(struct comp_state *st,
  ...
  if ((txn->status != 200) && (txn->status != 201) && (txn->status != 202) &&
      (txn->status != 203))
    goto fail;
  ...
  /* we don't want to compress multipart content-types, nor content-types that
   * are not listed in the "compression type" directive if any. If no
   * content-type was found but configuration requires one, we don't compress
   * either. Backend has the priority.
   */
  ctx.blk = NULL;
    ...
    if ((s->be->comp && (comp_type = s->be->comp->types)) ||
      ...
      for (; comp_type; comp_type = comp_type->next) {
        if (ctx.value.len >= comp_type->name_len &&
            strncasecmp(ctx.value.ptr, comp_type->name, comp_type->name_len) ==
                0)
...
static int parse_compression_options(char **args, int section,
...
static int parse_http_comp_flt(char **args, int *cur_arg, struct proxy *px,
 ...
 * response.
 */
static int smp_fetch_res_comp(const struct arg *args, struct sample *smp,
 ...
 * string, returns algo
 */
static int smp_fetch_res_comp_algo(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  struct http_txn *txn = smp->strm ? smp->strm->txn : NULL;
...
static struct flt_kw_list filter_kws = {
    "COMP",
    {},
    {
