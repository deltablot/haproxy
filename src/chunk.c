 ...
 * returned so that it is always possible to iterate data transformations
 * without losing the data being transformed. The blocks are initialized to the
 * size of a standard buffer, so they should be enough for everything. For
 * convenience, a zero is always emitted at the beginning of the string so that
 * it may be used as an empty string as well.
 */
...
int init_trash_buffers(int first) {
  ...
  pool_head_trash = create_pool(
      "trash", sizeof(struct buffer) + global.tune.bufsize, MEM_F_EXACT);
...
struct buffer *alloc_trash_chunk(void) {
  ...
  if (chunk) {
    ...
    *buf = 0;
    chunk_init(chunk, buf, pool_head_trash->size - sizeof(struct buffer));
  }
  return chunk;
}
...
int chunk_appendf(struct buffer *chk, const char *fmt, ...) {
  ...
  va_start(argp, fmt);
  ret = vsnprintf(chk->area + chk->data, chk->size - chk->data, fmt, argp);
...
int chunk_htmlencode(struct buffer *dst, struct buffer *src) {
  ...
  for (i = 0; i < src->data; i++) {
    ...
    if (!isascii((unsigned char)c) || !isprint((unsigned char)c) || c == '&' ||
        c == '"' || c == '\'' || c == '<' || c == '>') {
      l = snprintf(dst->area + dst->data, free, "&#%u;", (unsigned char)c);
...
int chunk_asciiencode(struct buffer *dst, struct buffer *src, char qc) {
  ...
  for (i = 0; i < src->data; i++) {
    ...
    if (!isascii((unsigned char)c) || !isprint((unsigned char)c) || c == '<' ||
        c == '>' || c == qc) {
      l = snprintf(dst->area + dst->data, free, "<%02X>", (unsigned char)c);
