...
struct cache {
  ...
  unsigned int max_secondary_entries; /* maximum number of secondary entries
                                         with the same primary hash */
  uint8_t vary_processing_enabled; /* boolean : manage Vary header (disabled by
                                      default) */
...
typedef int (*http_header_normalizer)(struct ist value, char *buf,
                                      unsigned int *buf_len);

struct vary_hashing_information {
  struct ist hdr_name; /* Header name */
  enum vary_header_bit
      value;                /* Bit repesenting the header in a vary signature */
...
static int accept_encoding_normalizer(struct ist value, char *buf,
                                      unsigned int *buf_len);
static int default_normalizer(struct ist value, char *buf,
                              unsigned int *buf_len);
...
const struct vary_hashing_information vary_information[] = {
    {IST("accept-encoding"), VARY_ACCEPT_ENCODING, sizeof(int),
...
static int http_request_build_secondary_key(struct stream *s,
                                            int vary_signature);
static int
http_request_reduce_secondary_key(unsigned int vary_signature,
...
struct cache_entry {
  unsigned int
      complete; /* An entry won't be valid until complete is not null. */
  ...
  unsigned int secondary_key_signature; /* Bitfield of the HTTP headers that
                                         * should be used to build secondary
                                         * keys for this cache entry. */
  unsigned int secondary_entries_count; /* Should only be filled in the last
                                           entry of a list of dup entries */
  unsigned int last_clear_ts;           /* Timestamp of the last call to
                                           clear_expired_duplicates. */

  unsigned int etag_length; /* Length of the ETag value (if one was found in the
                               response). */
  unsigned int etag_offset; /* Offset of the ETag value in the data buffer. */

  time_t last_modified; /* Origin server "Last-Modified" header value converted
                         * in seconds since epoch. If no "Last-Modified" header
                         * is found, use "Date" header value, otherwise use
                         * reception time. This field will be used in case of an
                         * "If-Modified-Since"-based conditional request. */
...
static struct list caches_config =
    LIST_HEAD_INIT(caches_config); /* cache config to init */
static struct cache *tmp_cache_config = NULL;

DECLARE_STATIC_POOL(pool_head_cache_st, "cache_st", sizeof(struct cache_st));

static struct eb32_node *insert_entry(struct cache *cache,
 ...
 */
struct cache_entry *secondary_entry_exist(struct cache *cache,
                                          struct cache_entry *entry,
                                          char *secondary_key) {
  ...
  while (entry && memcmp(entry->secondary_key, secondary_key,
                         HTTP_CACHE_SEC_KEY_LEN) != 0) {
...
static inline struct shared_context *shctx_ptr(struct cache *cache) {
  return (struct shared_context *)((unsigned char *)cache -
                                   ((struct shared_context *)NULL)->data);
}

static inline struct shared_block *block_ptr(struct cache_entry *entry) {
  return (struct shared_block *)((unsigned char *)entry -
                                 ((struct shared_block *)NULL)->data);
}

static int cache_store_init(struct proxy *px, struct flt_conf *fconf) {
  fconf->flags |= FLT_CFG_FL_HTX;
  return 0;
}

static void cache_store_deinit(struct proxy *px, struct flt_conf *fconf) {
...
static int cache_store_check(struct proxy *px, struct flt_conf *fconf) {
  struct cache_flt_conf *cconf = fconf->conf;
  struct flt_conf *f;
  struct cache *cache;
  int comp = 0;
  ...
  ha_alert("config: %s '%s': unable to find the cache '%s' referenced by the "
           "filter 'cache'.\n",
...
found:
  ...
  list_for_each_entry(f, &px->filter_configs, list) {
    if (f == fconf) {
      ...
      if (comp) {
        ha_alert(
            "config: %s '%s': unable to enable the compression filter before "
            "the cache '%s'.\n",
...
static int cache_store_strm_init(struct stream *s, struct filter *filter) {
  struct cache_st *st;
...
static void cache_store_strm_deinit(struct stream *s, struct filter *filter) {
...
static int cache_store_post_analyze(struct stream *s, struct filter *filter,
...
static int cache_store_http_headers(struct stream *s, struct filter *filter,
                                    struct http_msg *msg) {
  struct cache_st *st = filter->ctx;
...
static int cache_store_http_payload(struct stream *s, struct filter *filter,
...
static int cache_store_http_end(struct stream *s, struct filter *filter,
                                struct http_msg *msg) {
  struct cache_st *st = filter->ctx;
  struct cache_flt_conf *cconf = FLT_CONF(filter);
  struct cache *cache = cconf->c.cache;
 ...
 * the word was not found or if there wasn't any value assigned ot it return
 * NULL
 */
char *directive_value(const char *sample, int slen, const char *word,
                      int wlen) {
  int st = 0;
  ...
  if (maxage == -1 && smaxage == -1) {
    ...
    if (http_find_header(htx, ist("expires"), &ctx, 1)) {
      if (parse_http_date(istptr(ctx.value), istlen(ctx.value), &tm)) {
         ...
         * stale. */
        expires = (expires_val >= now.tv_sec) ? (expires_val - now.tv_sec) : 0;
      } else {
...
static void cache_free_blocks(struct shared_block *first,
...
/* As per RFC 7234#4.3.2, in case of "If-Modified-Since" conditional request,
 * the date value should be compared to a date determined by in a previous
 * response (for the same entity). This date could either be the "Last-Modified"
 * value, or the "Date" value of the response's reception time (by decreasing
 * order of priority). */
 ...
 */
static int http_check_vary_header(struct htx *htx,
...
enum act_return http_action_store_cache(struct act_rule *rule, struct proxy *px,
  ...
  if (cache->vary_processing_enabled) {
    if (!http_check_vary_header(htx, &vary_signature))
      goto out;
    if (vary_signature)
      http_request_reduce_secondary_key(vary_signature,
  ...
  if (old) {
    if (vary_signature)
      old =
          secondary_entry_exist(cconf->c.cache, old, txn->cache_secondary_hash);
    if (old) {
      if (!old->complete) {
  ...
  if (vary_signature)
    memcpy(object->secondary_key, txn->cache_secondary_hash,
  ...
  for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
    ...
    if (type == HTX_BLK_HDR) {
      ...
      if (isteq(header_name, ist("etag"))) {
        ...
        object->etag_offset = sizeof(struct cache_entry) + b_data(&trash) - sz +
                              istlen(header_name);
      }
    }
    if (type == HTX_BLK_EOH)
   ...
   * copy the headers */
  if (shctx_row_data_append(shctx, first, NULL, (unsigned char *)trash.area,
                            trash.data) < 0)
    goto out;
...
#define HTX_CACHE_EOM                                                          \
  3 /* Cache entry completely forwarded. Finish the HTX message */
...
static unsigned int htx_cache_dump_blk(struct appctx *appctx, struct htx *htx,
                                       enum htx_blk_type type, uint32_t info,
...
static unsigned int htx_cache_dump_data_blk(struct appctx *appctx,
                                            struct htx *htx, uint32_t info,
                                            struct shared_block *shblk,
                                            unsigned int offset) {
...
static size_t htx_cache_dump_msg(struct appctx *appctx, struct htx *htx,
                                 unsigned int len, enum htx_blk_type mark) {
  struct cache_flt_conf *cconf = appctx->rule->arg.act.p[0];
  struct shared_context *shctx = shctx_ptr(cconf->c.cache);
  struct shared_block *shblk;
...
static int htx_cache_add_age_hdr(struct appctx *appctx, struct htx *htx) {
  ...
  chunk_reset(&trash);
  age =
      MAX(0, (int)(now.tv_sec - cache_ptr->latest_validation)) + cache_ptr->age;
...
static int parse_cache_rule(struct proxy *proxy, const char *name,
...
enum act_parse_ret parse_cache_store(const char **args, int *orig_arg,
                                     struct proxy *proxy, struct act_rule *rule,
                                     char **err) {
 ...
 */
static int should_send_notmodified_response(struct cache *cache,
                                            struct htx *htx,
                                            struct cache_entry *entry) {
  int retval = 0;
...
enum act_return http_action_req_cache_use(struct act_rule *rule,
                                          struct proxy *px,
                                          struct session *sess,
  ...
  if (res && res->complete) {
    ...
    if (res->secondary_key_signature) {
      if (!http_request_build_secondary_key(s, res->secondary_key_signature)) {
        shctx_lock(shctx_ptr(cache));
        sec_entry =
            secondary_entry_exist(cache, res, s->txn->cache_secondary_hash);
...
enum act_parse_ret parse_cache_use(const char **args, int *orig_arg,
                                   struct proxy *proxy, struct act_rule *rule,
                                   char **err) {
  rule->action = ACT_CUSTOM;
...
int cfg_parse_cache(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "cache") == 0) { /* new cache section */
    ...
    if (tmp_cache_config == NULL) {
      ...
      if (strlen(args[1]) > 32) {
        ha_warning("parsing [%s:%d]: cache name is limited to 32 characters, "
                   "truncate to '%s'.\n",
    ...
    if (maxsize > (UINT_MAX >> 20)) {
      ha_warning("parsing [%s:%d]: \"total-max-size\" (%s) must not be greater "
                 "than %u\n",
    ...
    if (!*args[1]) {
      ha_warning("parsing [%s:%d]: '%s' expects a maximum file size parameter "
                 "in bytes.\n",
                 file, linenum, args[0]);
      err_code |= ERR_WARN;
    }
    ...
    if (!*args[1]) {
      ha_warning("parsing [%s:%d]: '%s' expects 0 or 1 (disable or enable vary "
                 "processing).\n",
                 file, linenum, args[0]);
      err_code |= ERR_WARN;
    }
...
int cfg_post_parse_section_cache() {
  ...
  if (tmp_cache_config) {
    ...
    } else if (tmp_cache_config->maxobjsz >
               tmp_cache_config->maxblocks * CACHE_BLOCKSIZE / 2) {
      ha_alert("\"max-object-size\" is limited to an half of "
               "\"total-max-size\" => %u\n",
               tmp_cache_config->maxblocks * CACHE_BLOCKSIZE / 2);
      err_code |= ERR_FATAL | ERR_ALERT;
      goto out;
 ...
 */
static int accept_encoding_normalizer(struct ist full_value, char *buf,
                                      unsigned int *buf_len) {
  ...
  while (count < (ACCEPT_ENCODING_MAX_ENTRIES - 1) &&
         (comma = istchr(full_value, ',')) != NULL) {
...
static int http_request_prebuild_full_secondary_key(struct stream *s) {
  ...
  for (idx = 0;
       idx < sizeof(vary_information) / sizeof(*vary_information) && !retval;
       ++idx) {
    info = &vary_information[idx];

    ctx.blk = NULL;
    if (info->norm_fn != NULL &&
        http_find_header(htx, info->hdr_name, &ctx, 1)) {
      retval = info->norm_fn(ctx.value, &txn->cache_secondary_hash[offset],
                             &hash_length);
      offset += hash_length;
    } else {
      /* Fill hash with 0s. */
...
http_request_reduce_secondary_key(unsigned int vary_signature,
  ...
  for (vary_idx = 0; vary_idx < vary_info_count; ++vary_idx) {
    ...
    for (offset = 0; offset < vary_info->hash_length;
...
static int parse_cache_flt(char **args, int *cur_arg, struct proxy *px,
  ...
  list_for_each_entry_safe(f, back, &px->filter_configs, list) {
    ...
    if (!(cconf->flags & CACHE_FLT_F_IMPLICIT_DECL)) {
      cconf = NULL;
      memprintf(err,
                "%s: multiple explicit declarations of the cache filter '%s'",
                px->id, name);
      goto error;
    }
...
static int cli_parse_show_cache(char **args, char *payload,
  ...
  list_for_each_entry_from(cache, &caches, list) {
    ...
    if (!next_key) {
      chunk_printf(&trash, "%p: %s (shctx:%p, available blocks:%d)\n", cache,
                   cache->id, shctx_ptr(cache), shctx_ptr(cache)->nbav);
    ...
    while (1) {
      ...
      chunk_appendf(&trash, " size:%u (%u blocks), refcount:%u, expire:%d\n",
                    ...
                    block_ptr(entry)->refcount,
                    entry->expire - (int)now.tv_sec);
 ...
 */
static int smp_fetch_res_cache_hit(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  smp->data.type = SMP_T_BOOL;
  smp->data.u.sint =
      (smp->strm ? (smp->strm->target == &http_cache_applet.obj_type) : 0);
 ...
 */
static int smp_fetch_res_cache_name(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
...
static struct action_kw_list http_res_actions = {
    .kw = {{"cache-store", parse_cache_store}, {NULL, NULL}}};

INITCALL1(STG_REGISTER, http_res_keywords_register, &http_res_actions);

static struct action_kw_list http_req_actions = {
    .kw = {{"cache-use", parse_cache_use}, {NULL, NULL}}};
...
static struct sample_fetch_kw_list sample_fetch_keywords = {
    ILH,
    {
        {"res.cache_hit", smp_fetch_res_cache_hit, 0, NULL, SMP_T_BOOL,
         SMP_USE_HRSHP, SMP_VAL_RESPONSE},
        {"res.cache_name", smp_fetch_res_cache_name, 0, NULL, SMP_T_STR,
         SMP_USE_HRSHP, SMP_VAL_RESPONSE},
        {/* END */},
    }};
