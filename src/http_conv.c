...
static int sample_conv_http_date(const struct arg *args, struct sample *smp,
  ...
  if (args && args[1].type == ARGT_SINT && args[1].data.sint != TIME_UNIT_S) {
    temp->data =
        snprintf(temp->area, temp->size - temp->data,
                 "%s, %02d %s %04d %02d:%02d:%02d.%d GMT", day[tm->tm_wday],
                 tm->tm_mday, mon[tm->tm_mon], 1900 + tm->tm_year, tm->tm_hour,
                 tm->tm_min, tm->tm_sec, sec_frac);
  } else {
    temp->data =
        snprintf(temp->area, temp->size - temp->data,
                 "%s, %02d %s %04d %02d:%02d:%02d GMT", day[tm->tm_wday],
                 tm->tm_mday, mon[tm->tm_mon], 1900 + tm->tm_year, tm->tm_hour,
                 tm->tm_min, tm->tm_sec);
  }
...
/* Arguments: The list of expected value, the number of parts returned and the
 * separator */
static int sample_conv_q_preferred(const struct arg *args, struct sample *smp,
                                   void *private) {
...
static int smp_conv_req_capture(const struct arg *args, struct sample *smp,
  ...
  for (hdr = fe->req_cap, i = fe->nb_req_cap - 1; hdr != NULL && i != idx;
       i--, hdr = hdr->next)
    ;
  if (!hdr)
    return 0;
...
static int smp_conv_res_capture(const struct arg *args, struct sample *smp,
                                void *private) {
  struct proxy *fe;
  int idx, i;
  struct cap_hdr *hdr;
  ...
  /* Look for the original configuration. */
  for (hdr = fe->rsp_cap, i = fe->nb_rsp_cap - 1; hdr != NULL && i != idx;
       i--, hdr = hdr->next)
    ;
  if (!hdr)
    return 0;
...
static struct sample_conv_kw_list sample_conv_kws = {
    ILH,
    {
        ...
        {"language", sample_conv_q_preferred, ARG2(1, STR, STR), NULL,
         SMP_T_STR, SMP_T_STR},
        {"capture-req", smp_conv_req_capture, ARG1(1, SINT), NULL, SMP_T_STR,
         SMP_T_STR},
        {"capture-res", smp_conv_res_capture, ARG1(1, SINT), NULL, SMP_T_STR,
         SMP_T_STR},
        {"url_dec", sample_conv_url_dec, ARG1(0, SINT), NULL, SMP_T_STR,
         SMP_T_STR},
        {NULL, NULL, 0, 0, 0},
    }};
