...
struct proxy defproxy =
    {}; /* fake proxy used to assign default values on all instances */
...
struct cfg_kw_list cfg_keywords = {.list = LIST_HEAD_INIT(cfg_keywords.list)};
 ...
 */
int str2listener(char *str, struct proxy *curproxy, struct bind_conf *bind_conf,
  ...
  while (next && *next) {
    ...
    ss2 = str2sa_range(
        str, NULL, &port, &end, &fd, &proto, err,
        curproxy == global.stats_fe ? NULL : global.unix_bind.prefix, NULL,
        PA_O_RESOLVE | PA_O_PORT_OK | PA_O_PORT_MAND | PA_O_PORT_RANGE |
 ...
 * will be replaced with an error message.
 */
int str2receiver(char *str, struct proxy *curproxy, struct bind_conf *bind_conf,
                 const char *file, int line, char **err) {
  struct protocol *proto;
  char *next, *dupstr;
  int port, end;
 ...
 * message may also be null, it will simply not be produced (useful to check
 * only). <msg> and <err_code> are only affected on error.
 */
int too_many_args_idx(int maxarg, int index, char **args, char **msg,
                      int *err_code) {
  int i;
  ...
  if (msg) {
    *msg = NULL;
    ...
    memprintf(msg, "'%s' cannot handle unexpected argument '%s'.", *msg,
              args[index + maxarg + 1]);
  }
  if (err_code)
 ...
 * Return 0 if the number of argument is correct, otherwise emit an alert and
 * return 1 Fill err_code with an ERR_ALERT and an ERR_FATAL
 */
int alertif_too_many_args_idx(int maxarg, int index, const char *file,
                              int linenum, char **args, int *err_code) {
  char *kw = NULL;
  int i;
 ...
 */
int alertif_too_many_args(int maxarg, const char *file, int linenum,
                          char **args, int *err_code) {
...
int warnif_cond_conflicts(const struct acl_cond *cond, unsigned int where,
  ...
  if (acl) {
    if (acl->name && *acl->name)
      ha_warning("parsing [%s:%d] : acl '%s' will never match because it only "
                 "involves keywords that are incompatible with '%s'\n",
                 file, line, acl->name, sample_ckp_names(where));
    else
      ha_warning("parsing [%s:%d] : anonymous acl will never match because it "
                 "uses keyword '%s' which is incompatible with '%s'\n",
                 file, line,
  ...
  if (acl->name && *acl->name)
    ha_warning("parsing [%s:%d] : acl '%s' involves keywords '%s' which is "
               "incompatible with '%s'\n",
               file, line, acl->name, kw, sample_ckp_names(where));
  else
    ha_warning("parsing [%s:%d] : anonymous acl involves keyword '%s' which is "
               "incompatible with '%s'\n",
 ...
 */
unsigned long parse_cpu_set(const char **args, unsigned long *cpu_set,
                            char **err) {
  int cur_arg = 0;
 ...
 * or "peer" lines.
 */
static int init_peers_frontend(const char *file, int linenum, const char *id,
                               struct peers *peers) {
  struct proxy *p;
 ...
 * if already present.
 */
static struct bind_conf *bind_conf_uniq_alloc(struct proxy *p, const char *file,
                                              int line, const char *arg,
 ...
 */
static struct peer *cfg_peers_add_peer(struct peers *peers, const char *file,
                                       int linenum, const char *id, int local) {
  struct peer *p;
...
int cfg_parse_peers(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "bind") == 0 || strcmp(args[0], "default-bind") == 0) {
    ...
    bind_conf = bind_conf_uniq_alloc(curpeers->peers_fe, file, linenum, NULL,
                                     xprt_get(XPRT_RAW));
    if (*args[0] == 'b') {
      struct listener *l;

      if (peer_line) {
        ha_alert("parsing [%s:%d] : mixing \"peer\" and \"bind\" line is "
                 "forbidden\n",
                 file, linenum);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      if (!str2listener(args[1], curpeers->peers_fe, bind_conf, file, linenum,
                        &errmsg)) {
        if (errmsg && *errmsg) {
          indent_msg(&errmsg, 2);
          ...
          ha_alert("parsing [%s:%d] : '%s %s' : error encountered while "
                   "parsing listening address %s.\n",
      ...
      l->options |=
          LI_O_UNLIMITED; /* don't make the peers subject to global limits */
    ...
    err_code |=
        parse_server(file, linenum, args, curpeers->peers_fe, NULL, 0, 1, 1);
  } else if (strcmp(args[0], "log") == 0) {
    if (init_peers_frontend(file, linenum, NULL, curpeers) != 0) {
      err_code |= ERR_ALERT | ERR_ABORT;
      goto out;
    }
    if (!parse_logsrv(args, &curpeers->peers_fe->logsrvs, (kwm == KWM_NO),
                      &errmsg)) {
    ...
    if (err) {
      ha_alert("parsing [%s:%d] : character '%c' is not permitted in '%s' name "
               "'%s'.\n",
    ...
    for (curpeers = cfg_peers; curpeers != NULL; curpeers = curpeers->next) {
      ...
      if (strcmp(curpeers->id, args[1]) == 0) {
        ha_alert("Parsing [%s:%d]: peers section '%s' has the same name as "
                 "another peers section declared at %s:%d.\n",
    ...
    if (cfg_peers->local && !cfg_peers->local->id && local_peer) {
      ...
      if (local_peer && cfg_peers->local) {
        ha_alert("parsing [%s:%d] : '%s %s' : local peer name already "
                 "referenced at %s:%d. %s\n",
                 file, linenum, args[0], args[1], curpeers->peers_fe->conf.file,
                 curpeers->peers_fe->conf.line, cfg_peers->local->id);
        err_code |= ERR_FATAL;
        goto out;
      }
      newpeer =
          cfg_peers_add_peer(curpeers, file, linenum, args[1], local_peer);
      if (!newpeer) {
        err_code |= ERR_ALERT | ERR_ABORT;
        goto out;
      }
     ...
     */
    if (init_peers_frontend(file, newpeer->local ? linenum : -1,
    ...
    bind_conf = bind_conf_uniq_alloc(curpeers->peers_fe, file, linenum, args[2],
                                     xprt_get(XPRT_RAW));
    ...
    if (!str2listener(args[2], curpeers->peers_fe, bind_conf, file, linenum,
      ...
      if (errmsg && *errmsg) {
        ...
        ha_alert("parsing [%s:%d] : '%s %s' : error encountered while parsing "
                 "listening address %s.\n",
                 file, linenum, args[0], args[1], args[2]);
      err_code |= ERR_FATAL;
      goto out;
    ...
    if (other) {
      ha_alert("parsing [%s:%d] : stick-table name '%s' conflicts with table "
               "declared in %s '%s' at %s:%d.\n",
               file, linenum, args[1],
               other->proxy ? proxy_cap_str(other->proxy->cap) : "peers",
               other->proxy ? other->id : other->peers.p->id, other->conf.file,
               other->conf.line);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    err_code |= parse_stick_table(file, linenum, args, t, id, id + prefix_len,
                                  curpeers);
    ...
    curpeers->disabled = 1;
  } else if (!strcmp(args[0], "enabled")) { /* enables this peers section (used
                                               to revert a disabled default) */
    curpeers->disabled = 0;
  } else if (*args[0] != 0) {
    ha_alert("parsing [%s:%d] : unknown keyword '%s' in '%s' section\n", file,
             linenum, args[0], cursection);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  }
...
int cfg_parse_resolvers(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "resolvers") == 0) { /* new resolvers section */
    ...
    list_for_each_entry(curr_resolvers, &dns_resolvers, list) {
      ...
      if (strcmp(curr_resolvers->id, args[1]) == 0) {
        ha_alert("Parsing [%s:%d]: resolvers '%s' has same name as another "
                 "resolvers (declared at %s:%d).\n",
                 file, linenum, args[1], curr_resolvers->conf.file,
    ...
    if (!*args[2]) {
      ha_alert("parsing [%s:%d] : '%s' expects <name> and <addr>[:<port>] as "
               "arguments.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    err = invalid_char(args[1]);
    if (err) {
      ha_alert("parsing [%s:%d] : character '%c' is not permitted in server "
               "name '%s'.\n",
    ...
    list_for_each_entry(newnameserver, &curr_resolvers->nameservers, list) {
      ...
      if (strcmp(newnameserver->id, args[1]) == 0) {
        ha_alert("Parsing [%s:%d]: nameserver '%s' has same name as another "
                 "nameserver (declared at %s:%d).\n",
                 file, linenum, args[1], newnameserver->conf.file,
                 newnameserver->conf.line);
        err_code |= ERR_ALERT | ERR_FATAL;
      }
    }
    ...
    sk = str2sa_range(
        args[2], NULL, &port1, &port2, NULL, NULL, &errmsg, NULL, NULL,
        PA_O_RESOLVE | PA_O_PORT_OK | PA_O_PORT_MAND | PA_O_DGRAM);
    if (!sk) {
      ha_alert("parsing [%s:%d] : '%s %s' : %s\n", file, linenum, args[0],
               args[1], errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
    ...
    while (fgets(resolv_line, LINESIZE, f) != NULL) {
      ...
      if (address == NULL) {
        ha_warning("parsing [/etc/resolv.conf:%d] : nameserver line is missing "
                   "address.\n",
                   resolv_linenum);
        err_code |= ERR_WARN;
        continue;
      ...
      list_for_each_entry(newnameserver, &curr_resolvers->nameservers, list) {
        if (strcmp(newnameserver->id, address) == 0) {
          ha_warning("Parsing [/etc/resolv.conf:%d] : generated name for "
                     "/etc/resolv.conf nameserver '%s' conflicts with another "
                     "nameserver (declared at %s:%d), it appears to be a "
                     "duplicate and will be excluded.\n",
                     resolv_linenum, address, newnameserver->conf.file,
                     newnameserver->conf.line);
          err_code |= ERR_WARN;
          duplicate_name = 1;
        }
      ...
      if (!str2ip2(address, sk, 1)) {
        ha_warning("parsing [/etc/resolv.conf:%d] : address '%s' could not be "
                   "recognized, nameserver will be excluded.\n",
      ...
      if (!proto || !proto->connect) {
        ha_warning("parsing [/etc/resolv.conf:%d] : '%s' : connect() not "
  ...
  resolv_out:
    ...
    if (!*args[2]) {
      ha_alert("parsing [%s:%d] : '%s' expects an <event> and a <time> as "
               "arguments.\n",
               file, linenum, args[0]);
      ha_alert("<event> can be either 'valid', 'nx', 'refused', 'timeout', or "
               "'other'\n");
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (res == PARSE_TIME_OVER) {
      ha_alert("parsing [%s:%d]: timer overflow in argument <%s> to <%s>, "
               "maximum value is 2147483647 ms (~24.8 days).\n",
               file, linenum, args[1], args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else if (res == PARSE_TIME_UNDER) {
      ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to <%s>, "
               "minimum non-null value is 1 ms.\n",
               file, linenum, args[1], args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else if (res) {
      ha_alert(
          "parsing [%s:%d]: unexpected character '%c' in argument to <%s>.\n",
    ...
    if (strcmp(args[1], "nx") == 0)
      ...
      ha_alert("parsing [%s:%d] : '%s' unknown <event>: '%s', expects either "
               "'nx', 'timeout', 'valid', 'obsolete' or 'other'.\n",
    ...
    if (i < DNS_HEADER_SIZE || i > DNS_MAX_UDP_MESSAGE) {
      ha_alert("parsing [%s:%d] : '%s' must be between %d and %d inclusive "
               "(was %s).\n",
               file, linenum, args[0], DNS_HEADER_SIZE, DNS_MAX_UDP_MESSAGE,
               args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    ha_alert("parsing [%s:%d] : '%s' directive is not supported anymore (it "
             "never appeared in a stable release).\n",
             file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
    ...
    if (!*args[1]) {
      ha_alert("parsing [%s:%d] : '%s' expects 'retry' or 'resolve' and <time> "
               "as arguments.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
      ...
      if (res == PARSE_TIME_OVER) {
        ha_alert("parsing [%s:%d]: timer overflow in argument <%s> to <%s %s>, "
                 "maximum value is 2147483647 ms (~24.8 days).\n",
        ...
        ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to <%s "
                 "%s>, minimum non-null value is 1 ms.\n",
                 file, linenum, args[2], args[0], args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      } else if (res) {
        ha_alert("parsing [%s:%d]: unexpected character '%c' in argument to "
                 "<%s %s>.\n",
...
int cfg_parse_mailers(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "mailers") == 0) { /* new mailers section */
    ...
    for (curmailers = mailers; curmailers != NULL;
      ...
      if (strcmp(curmailers->id, args[1]) == 0) {
        ha_alert("Parsing [%s:%d]: mailers section '%s' has the same name as "
                 "another mailers section declared at %s:%d.\n",
                 file, linenum, args[1], curmailers->conf.file,
                 curmailers->conf.line);
        err_code |= ERR_ALERT | ERR_FATAL;
      }
    }
    ...
    curmailers->timeout.mail =
        DEF_MAILALERTTIME; /* XXX: Would like to Skip to the next alert, if any,
                            * ASAP. But need enough time so that timeouts don't
                            * occur during tcp procssing. For now just us an
                            * arbitrary default. */
    ...
    sk = str2sa_range(args[2], NULL, &port1, &port2, NULL, &proto, &errmsg,
                      NULL, NULL,
    ...
    if (proto->sock_prot != IPPROTO_TCP) {
      ha_alert("parsing [%s:%d] : '%s %s' : TCP not supported for this address "
               "family.\n",
               file, linenum, args[0], args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!*args[1]) {
      ha_alert(
          "parsing [%s:%d] : '%s' expects 'mail' and <time> as arguments.\n",
          file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
...
int cfg_parse_netns(const char *file, int linenum, char **args, int kwm) {
#ifdef USE_NS
  const char *err;
  ...
  if (!strcmp(item, "namespace_list")) {
    ...
    while (*(current = args[idx++])) {
      ...
      if (err) {
        ha_alert("parsing [%s:%d]: character '%c' is not permitted in '%s' "
                 "name '%s'.\n",
  ...
  return 0;
#else
  ha_alert("parsing [%s:%d]: namespace support is not compiled in.", file,
           linenum);
  return ERR_ALERT | ERR_FATAL;
#endif
}
  ...
  if (!strcmp(args[0], "userlist")) { /* new userlist */
    ...
    for (ag = userlist->groups; ag; ag = ag->next)
      if (!strcmp(ag->name, args[1])) {
        ha_warning("parsing [%s:%d]: ignoring duplicated group '%s' in "
                   "userlist '%s'.\n",
    ...
    for (newuser = userlist->users; newuser; newuser = newuser->next)
      if (!strcmp(newuser->user, args[1])) {
        ha_warning("parsing [%s:%d]: ignoring duplicated user '%s' in userlist "
                   "'%s'.\n",
                   file, linenum, args[1], userlist->name);
        err_code |= ERR_ALERT;
        goto out;
    ...
    while (*args[cur_arg]) {
      if (!strcmp(args[cur_arg], "password")) {
        ...
        if (!crypt("", args[cur_arg + 1])) {
          ha_alert("parsing [%s:%d]: the encrypted password used for user '%s' "
                   "is not supported by crypt(3).\n",
                   file, linenum, newuser->user);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        }
#else
        ha_warning("parsing [%s:%d]: no crypt(3) support compiled, encrypted "
        ...
        ha_alert("parsing [%s:%d]: '%s' only supports 'password', "
                 "'insecure-password' and 'groups' options.\n",
                 file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
...
int cfg_parse_scope(const char *file, int linenum, char *line) {
  ...
  if (err) {
    ha_alert(
        "parsing [%s:%d] : character '%c' is not permitted in a scope name.\n",
        file, linenum, *err);
    err_code |= ERR_ALERT | ERR_ABORT;
    goto out;
  }
  ...
  while (isspace((unsigned char)*line))
    line++;
  if (*line && *line != '#' && *line != '\n' && *line != '\r') {
    ha_alert("parsing [%s:%d] : character '%c' is not permitted after scope "
             "declaration.\n",
             file, linenum, *line);
    err_code |= ERR_ALERT | ERR_ABORT;
    goto out;
...
int cfg_parse_track_sc_num(unsigned int *track_sc_num, const char *arg,
                           const char *end, char **errmsg) {
  const char *p;
  unsigned int num;
  ...
  if (num >= MAX_SESS_STKCTR) {
    memprintf(errmsg,
              "%u track-sc number exceeding "
              "%d (MAX_SESS_STKCTR-1) value",
              num, MAX_SESS_STKCTR - 1);
    return -1;
  }
...
next_line:
    ...
    while (1) {
      ...
      err = parse_line(line, outline, &outlen, args, &arg,
                       PARSE_OPT_ENV | PARSE_OPT_DQUOTE | PARSE_OPT_SQUOTE |
                           PARSE_OPT_BKSLASH | PARSE_OPT_SHARP |
                           PARSE_OPT_WORD_EXPAND,
                       &errptr);
      ...
      if (err & PARSE_ERR_QUOTE) {
        ...
        ha_alert("parsing [%s:%d]: unmatched quote at position %d:\n"
                 "  %s\n  %*s\n",
                 file, linenum, (int)(errptr - thisline + 1), line,
                 (int)(newpos + 1), "^");
        err_code |= ERR_ALERT | ERR_FATAL;
        fatal++;
        goto next_line;
      ...
      if (err & PARSE_ERR_BRACE) {
        ...
        ha_alert("parsing [%s:%d]: unmatched brace in environment variable "
                 "name at position %d:\n"
                 "  %s\n  %*s\n",
                 file, linenum, (int)(errptr - thisline + 1), line,
                 (int)(newpos + 1), "^");
      ...
      if (err & PARSE_ERR_VARNAME) {
        ...
        ha_alert("parsing [%s:%d]: forbidden first char in environment "
                 "variable name at position %d:\n"
                 "  %s\n  %*s\n",
                 file, linenum, (int)(errptr - thisline + 1), line,
                 (int)(newpos + 1), "^");
        err_code |= ERR_ALERT | ERR_FATAL;
      ...
      if (err & PARSE_ERR_HEX) {
        ...
        ha_alert("parsing [%s:%d]: truncated or invalid hexadecimal sequence "
                 "at position %d:\n"
                 "  %s\n  %*s\n",
                 file, linenum, (int)(errptr - thisline + 1), line,
                 (int)(newpos + 1), "^");
      ...
      if (err & PARSE_ERR_WRONG_EXPAND) {
        ...
        ha_alert("parsing [%s:%d]: truncated or invalid word expansion "
      ...
      if (err & PARSE_ERR_TOOMANY) {
        ...
        ha_alert("parsing [%s:%d]: too many words, truncating after word %d, "
                 "position %ld: <%s>.\n",
                 file, linenum, MAX_LINE_ARGS,
                 (long)(args[MAX_LINE_ARGS - 1] - outline + 1),
                 args[MAX_LINE_ARGS - 1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        fatal++;
        goto next_line;
      }
  ...
  if (missing_lf != -1) {
    ha_alert("parsing [%s:%d]: Missing LF on last line, file might have been "
             "truncated at position %d.\n",
...
void propagate_processes(struct proxy *from, struct proxy *to) {
  ...
  if (to) {
    ...
    if (from->bind_proc && (from->bind_proc & to->bind_proc) == from->bind_proc)
      return;

    if (!from->bind_proc && !to->bind_proc)
      return;

    to->bind_proc = from->bind_proc ? (to->bind_proc | from->bind_proc) : 0;
...
int check_config_validity() {
  ...
  if (global.nbproc > 1 && global.nbthread > 1) {
    ha_alert(
        "config : cannot enable multiple processes if multiple threads are "
        "configured. Please use either nbproc or nbthread but not both.\n");
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  }

  pool_head_requri =
      create_pool("requri", global.tune.requri_len, MEM_F_SHARED);

  pool_head_capture =
      create_pool("capture", global.tune.cookie_len, MEM_F_SHARED);
  ...
  for (curproxy = proxies_list; curproxy; curproxy = curproxy->next) {
    ...
    if (curproxy->bind_proc) {
      ...
      if (!curproxy->bind_proc && nbproc == 1) {
        ha_warning(
            "Proxy '%s': the process specified on the 'bind-process' directive "
            "refers to a process number that is higher than global.nbproc. The "
            "proxy has been forced to run on process 1 only.\n",
            curproxy->id);
        curproxy->bind_proc = 1;
      } else if (!curproxy->bind_proc && nbproc > 1) {
        ha_warning("Proxy '%s': all processes specified on the 'bind-process' "
                   "directive refer to numbers that are all higher than "
                   "global.nbproc. The directive was ignored and the proxy "
                   "will run on all processes.\n",
                   curproxy->id);
    ...
    list_for_each_entry(bind_conf, &curproxy->conf.bind, by_fe) {
      ...
      if (curproxy->mode == PR_MODE_HTTP && global.tune.bufsize < 16384) {
        ...
        /* check NPN */
        if (bind_conf->ssl_conf.npn_str &&
            strstr(bind_conf->ssl_conf.npn_str, "\002h2")) {
          ha_alert(
              "config : HTTP frontend '%s' enables HTTP/2 via NPN at [%s:%d], "
              "so global.tune.bufsize must be at least 16384 bytes (%d now).\n",
              curproxy->id, bind_conf->file, bind_conf->line,
              global.tune.bufsize);
          cfgerr++;
        }
#endif
        ...
        if (bind_conf->ssl_conf.alpn_str &&
            strstr(bind_conf->ssl_conf.alpn_str, "\002h2")) {
          ha_alert(
              "config : HTTP frontend '%s' enables HTTP/2 via ALPN at [%s:%d], "
              "so global.tune.bufsize must be at least 16384 bytes (%d now).\n",
              curproxy->id, bind_conf->file, bind_conf->line,
              global.tune.bufsize);
          cfgerr++;
      ...
      if (!(mask & all_threads_mask)) {
        ...
        ha_warning(
            "Proxy '%s': the thread range specified on the 'process' directive "
            "of 'bind %s' at [%s:%d] only refers to thread numbers out of the "
            "range defined by the global 'nbthread' directive. The thread "
            "numbers were remapped to existing threads instead (mask 0x%lx).\n",
      ...
      mask = proc_mask(bind_conf->settings.bind_proc) &
             proc_mask(curproxy->bind_proc);
      if (!(mask & all_proc_mask)) {
        mask = proc_mask(curproxy->bind_proc) & all_proc_mask;
        nbproc = my_popcountl(bind_conf->settings.bind_proc);
        bind_conf->settings.bind_proc =
            proc_mask(bind_conf->settings.bind_proc) & mask;

        if (!bind_conf->settings.bind_proc && nbproc == 1) {
          ha_warning("Proxy '%s': the process number specified on the "
                     "'process' directive of 'bind %s' at [%s:%d] refers to a "
                     "process not covered by the proxy. This has been fixed by "
          ...
          ha_warning("Proxy '%s': the process range specified on the 'process' "
                     "directive of 'bind %s' at [%s:%d] only refers to "
                     "processes not covered by the proxy. The directive was "
                     "ignored so that all of the proxy's processes are used.\n",
                     curproxy->id, bind_conf->arg, bind_conf->file,
                     bind_conf->line);
    ...
    case PR_MODES:
      ...
      ha_alert(
          "config : %s '%s' cannot use peers or syslog mode for this proxy. "
          "NOTE: PLEASE REPORT THIS TO DEVELOPERS AS YOU'RE NOT SUPPOSED TO BE "
          "ABLE TO CREATE A CONFIGURATION TRIGGERING THIS!\n",
    ...
    if (curproxy != global.stats_fe && (curproxy->cap & PR_CAP_FE) &&
      ...
      ha_warning("config : %s '%s' has no 'bind' directive. Please declare it "
                 "as a backend if this was intended.\n",
                 proxy_type_str(curproxy), curproxy->id);
      err_code |= ERR_WARN;
    }
    ...
    if (curproxy->cap & PR_CAP_BE) {
      if (curproxy->lbprm.algo & BE_LB_KIND) {
        if (curproxy->options & PR_O_TRANSP) {
          ha_alert("config : %s '%s' cannot use both transparent and balance "
                   "mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
          cfgerr++;
        }
          ...
          ha_alert(
              "config : %s '%s' needs at least 1 server in balance mode.\n",
              proxy_type_str(curproxy), curproxy->id);
          cfgerr++;
        }
#endif
        else if (curproxy->options & PR_O_DISPATCH) {
          ha_warning("config : dispatch address of %s '%s' will be ignored in "
                     "balance mode.\n",
                     proxy_type_str(curproxy), curproxy->id);
          err_code |= ERR_WARN;
        }
      } else if (!(curproxy->options &
                   (PR_O_TRANSP | PR_O_DISPATCH | PR_O_HTTP_PROXY))) {
    ...
    if ((curproxy->tcpcheck_rules.flags & TCPCHK_RULES_UNUSED_HTTP_RS)) {
      ha_warning("config : %s '%s' uses http-check rules without 'option "
                 "httpchk', so the rules are ignored.\n",
                 proxy_type_str(curproxy), curproxy->id);
      err_code |= ERR_WARN;
    }

    if ((curproxy->options2 & PR_O2_CHK_ANY) == PR_O2_TCPCHK_CHK &&
        (curproxy->tcpcheck_rules.flags & TCPCHK_RULES_PROTO_CHK) !=
            TCPCHK_RULES_HTTP_CHK) {
      if (curproxy->options & PR_O_DISABLE404) {
        ha_warning("config : '%s' will be ignored for %s '%s' (requires "
                   "'option httpchk').\n",
    ...
    if ((curproxy->options2 & PR_O2_CHK_ANY) == PR_O2_EXT_CHK) {
      if (!global.external_check) {
        ha_alert("Proxy '%s' : '%s' unable to find required "
                 "'global.external-check'.\n",
                 curproxy->id, "option external-check");
        cfgerr++;
      }
      if (!curproxy->check_command) {
        ha_alert("Proxy '%s' : '%s' unable to find required 'external-check "
                 "command'.\n",
                 curproxy->id, "option external-check");
        cfgerr++;
      }
      if (!(global.tune.options & GTUNE_INSECURE_FORK)) {
        ha_warning("Proxy '%s' : 'insecure-fork-wanted' not enabled in the "
                   "global section, '%s' will likely fail.\n",
                   curproxy->id, "option external-check");
        err_code |= ERR_WARN;
      }
    }
    ...
    if (curproxy->email_alert.set) {
      if (!(curproxy->email_alert.mailers.name && curproxy->email_alert.from &&
        ...
        ha_warning(
            "config : 'email-alert' will be ignored for %s '%s' (the presence "
            "any of "
            "'email-alert from', 'email-alert level' 'email-alert mailers', "
            "'email-alert myhostname', or 'email-alert to' "
            "requires each of 'email-alert from', 'email-alert mailers' and "
            "'email-alert to' "
            "to be present).\n",
            proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
    ...
    if (curproxy->check_command) {
      ...
      if ((curproxy->options2 & PR_O2_CHK_ANY) != PR_O2_EXT_CHK) {
        ha_warning("config : '%s' will be ignored for %s '%s' (requires "
                   ...
                   "external-check command", proxy_type_str(curproxy),
                   curproxy->id);
        err_code |= ERR_WARN;
        clear = 1;
      }
      if (curproxy->check_command[0] != '/' && !curproxy->check_path) {
        ha_alert("Proxy '%s': '%s' does not have a leading '/' and "
    ...
    if (curproxy->check_path) {
      if ((curproxy->options2 & PR_O2_CHK_ANY) != PR_O2_EXT_CHK) {
        ha_warning("config : '%s' will be ignored for %s '%s' (requires "
                   "'option external-check').\n",
                   "external-check path", proxy_type_str(curproxy),
                   curproxy->id);
        err_code |= ERR_WARN;
      ...
      if (!target) {
        ...
        cfgerr++;
                 ...
                 !(curproxy->mode == PR_MODE_TCP &&
                   target->mode == PR_MODE_HTTP)) {

        ha_alert("%s %s '%s' (%s:%d) tries to use incompatible %s %s '%s' "
                 "(%s:%d) as its default backend (see 'mode').\n",
                 proxy_mode_str(curproxy->mode), proxy_type_str(curproxy),
                 curproxy->id, curproxy->conf.file, curproxy->conf.line,
                 proxy_mode_str(target->mode), proxy_type_str(target),
                 target->id, target->conf.file, target->conf.line);
        cfgerr++;
      } else {
        ...
        if (curproxy->srv) {
          ha_warning("In proxy '%s', the 'default_backend' rule always has "
                     "precedence over the servers, which will never be used.\n",
                     curproxy->id);
          err_code |= ERR_WARN;
        }
      }
    ...
    list_for_each_entry(rule, &curproxy->switching_rules, list) {
      ...
      err = NULL;
      if (!parse_logformat_string(pxname, curproxy, &rule->be.expr, 0,
                                  SMP_VAL_FE_HRQ_HDR, &err)) {
        ha_alert(
            "Parsing [%s:%d]: failed to parse use_backend rule '%s' : %s.\n",
      ...
      if (!LIST_ISEMPTY(&rule->be.expr)) {
        ...
        /* Only one element in the list, a simple string: free the expression
         * and fall back to static rule
         */
        LIST_DEL(&node->list);
        free(node->arg);
        free(node);
      ...
      if (!target) {
        ...
        ha_alert("%s %s '%s' (%s:%d) tries to use incompatible %s %s '%s' "
                 "(%s:%d) in a 'use_backend' rule (see 'mode').\n",
                 proxy_mode_str(curproxy->mode), proxy_type_str(curproxy),
                 curproxy->id, curproxy->conf.file, curproxy->conf.line,
                 proxy_mode_str(target->mode), proxy_type_str(target),
                 target->id, target->conf.file, target->conf.line);
    ...
    list_for_each_entry(srule, &curproxy->server_rules, list) {
      ...
      /* We try to parse the string as a log format expression. If the result of
       * the parsing is only one entry containing a single string, then it's a
       * standard string corresponding to a static rule, thus the parsing is
       * cancelled and we fall back to setting srv.ptr.
       */
      server_name = srule->srv.name;
      LIST_INIT(&srule->expr);
      curproxy->conf.args.ctx = ARGC_USRV;
      err = NULL;
      if (!parse_logformat_string(server_name, curproxy, &srule->expr, 0,
                                  SMP_VAL_FE_HRQ_HDR, &err)) {
        ha_alert("Parsing [%s:%d]; use-server rule failed to parse log-format "
                 "'%s' : %s.\n",
      ...
      if (!target) {
        ha_alert("config : %s '%s' : unable to find server '%s' referenced in "
                 "a 'use-server' rule.\n",
    ...
    list_for_each_entry(mrule, &curproxy->sticking_rules, list) {
      ...
      if (!target) {
        ha_alert("Proxy '%s': unable to find stick-table '%s'.\n", curproxy->id,
                 mrule->table.name ? mrule->table.name : curproxy->id);
        cfgerr++;
      } else if (!stktable_compatible_sample(mrule->expr, target->type)) {
        ha_alert("Proxy '%s': type of fetch not usable with type of "
                 "stick-table '%s'.\n",
                 curproxy->id,
                 mrule->table.name ? mrule->table.name : curproxy->id);
        cfgerr++;
      } else if (target->proxy &&
                 curproxy->bind_proc & ~target->proxy->bind_proc) {
        ha_alert("Proxy '%s': stick-table '%s' referenced 'stick-store' rule "
                 "not present on all processes covered by proxy '%s'.\n",
    ...
    if (curproxy->table && curproxy->table->peers.name) {
      ...
      if (!curpeers) {
        ...
        ha_alert("Proxy '%s': unable to find local peer '%s' in peers section "
                 "'%s'.\n",
    ...
    if (curproxy->uri_auth && curproxy->uri_auth != defproxy.uri_auth &&
      ...
      ha_alert("%s '%s': stats 'auth'/'realm' and 'http-request' can't be used "
               "at the same time.\n",
               "proxy", curproxy->id);
      cfgerr++;
    ...
    if (curproxy->uri_auth && curproxy->uri_auth->userlist &&
      ...
      int i = 0;
      /* build the ACL condition from scratch. We're relying on anonymous ACLs
       * for that */
      ...
      rule = parse_http_req_cond(uri_auth_compat_req,
                                 "internal-stats-auth-compat", 0, curproxy);
      if (!rule) {
        cfgerr++;
        break;
      }
  ...
  out_uri_auth_compat:
    ...
    if (curproxy->conf.logformat_string) {
      ...
      err = NULL;
      if (!parse_logformat_string(curproxy->conf.logformat_string, curproxy,
                                  &curproxy->logformat,
    ...
    if (curproxy->conf.logformat_sd_string) {
      ...
      err = NULL;
      if (!parse_logformat_string(curproxy->conf.logformat_sd_string, curproxy,
                                  &curproxy->logformat_sd,
                                  LOG_OPT_MANDATORY | LOG_OPT_MERGE_SPACES,
                                  SMP_VAL_FE_LOG_END, &err)) {
        ...
        free(err);
        cfgerr++;
      } else if (!add_to_logformat_list(NULL, NULL, LF_SEPARATOR,
                                        &curproxy->logformat_sd, &err)) {
        ha_alert("Parsing [%s:%d]: failed to parse log-format-sd : %s.\n",
                 curproxy->conf.lfs_file, curproxy->conf.lfs_line, err);
        free(err);
        cfgerr++;
    ...
    if (curproxy->conf.uniqueid_format_string) {
      ...
      err = NULL;
      if (!parse_logformat_string(
              curproxy->conf.uniqueid_format_string, curproxy,
              &curproxy->format_unique_id, LOG_OPT_HTTP | LOG_OPT_MERGE_SPACES,
        ...
        (((curproxy->cap & PR_CAP_FE) && !curproxy->timeout.client) ||
         ((curproxy->cap & PR_CAP_BE) && (curproxy->srv) &&
          (!curproxy->timeout.connect ||
           (!curproxy->timeout.server &&
            (curproxy->mode == PR_MODE_HTTP || !curproxy->timeout.tunnel)))))) {
    ...
    /* Historically, the tarpit and queue timeouts were inherited from
     * contimeout. We must still support older configurations, so let's find out
     * whether those parameters have been set or must be copied from
     * contimeouts.
     */
    ...
    if ((curproxy->tcpcheck_rules.flags & TCPCHK_RULES_UNUSED_TCP_RS)) {
      ha_warning("config : %s '%s' uses tcp-check rules without 'option "
                 "tcp-check', so the rules are ignored.\n",
                 proxy_type_str(curproxy), curproxy->id);
      err_code |= ERR_WARN;
    }
    ...
    if (curproxy->capture_len >= global.tune.cookie_len) {
      ha_warning(
          "config : truncating capture length to %d bytes for %s '%s'.\n",
    ...
    if (curproxy->nb_req_cap) {
      curproxy->req_cap_pool = create_pool(
          "ptrcap", curproxy->nb_req_cap * sizeof(char *), MEM_F_SHARED);
    }

    if (curproxy->nb_rsp_cap) {
      curproxy->rsp_cap_pool = create_pool(
          "ptrcap", curproxy->nb_rsp_cap * sizeof(char *), MEM_F_SHARED);
    }
    ...
    case PR_SRV_STATE_FILE_GLOBAL:
      if (!global.server_state_file) {
        ha_warning("config : backend '%s' configured to load server state file "
                   "from global section 'server-state-file' directive. "
    ...
    for (newsrv = curproxy->srv; newsrv; newsrv = newsrv->next) {
      ...
      for (other_srv = curproxy->srv; other_srv && other_srv != newsrv;
           other_srv = other_srv->next) {
        if (!other_srv->puid && strcmp(other_srv->id, newsrv->id) == 0) {
          ha_alert("parsing [%s:%d] : %s '%s', another server named '%s' was "
                   "already defined at line %d, please use distinct names.\n",
    ...
    while (newsrv != NULL) {
      ...
      if ((newsrv->flags & SRV_F_FASTOPEN) &&
        ...
        ha_warning("parsing [%s:%d] : %s '%s': server '%s' has tfo activated, "
                   "the backend should be configured with at least "
                   "'conn-failure', 'empty-response' and 'response-timeout' or "
                   "we wouldn't be able to retry the connection on failure.\n",
                   newsrv->conf.file, newsrv->conf.line,
                   proxy_type_str(curproxy), curproxy->id, newsrv->id);
      ...
      if (newsrv->trackit) {
        ...
        if (pname) {
          ...
          if (!px) {
            ha_alert("config : %s '%s', server '%s': unable to find required "
                     "proxy '%s' for tracking.\n",
                     proxy_type_str(curproxy), curproxy->id, newsrv->id, pname);
            cfgerr++;
            goto next_srv;
          }
        } else
        ...
        if (!srv) {
          ha_alert("config : %s '%s', server '%s': unable to find required "
                   "server '%s' for tracking.\n",
                   proxy_type_str(curproxy), curproxy->id, newsrv->id, sname);
          cfgerr++;
          goto next_srv;
        }
        ...
        if (!srv->do_check && !srv->do_agent && !srv->track && !srv->trackit) {
          ha_alert(
              "config : %s '%s', server '%s': unable to use %s/%s for "
              "tracking as it does not have any check nor agent enabled.\n",
              proxy_type_str(curproxy), curproxy->id, newsrv->id, px->id,
              srv->id);
          cfgerr++;
          goto next_srv;
        }
        ...
        if (newsrv == srv || loop) {
          ha_alert("config : %s '%s', server '%s': unable to track %s/%s as it "
                   ...
                   proxy_type_str(curproxy), curproxy->id, newsrv->id, px->id,
                   srv->id, px->id, newsrv == srv ? srv->id : loop->id);
          cfgerr++;
          goto next_srv;
        }
    ...
    if ((curproxy->mode == PR_MODE_TCP || curproxy->mode == PR_MODE_HTTP) &&
        ...
        (!LIST_ISEMPTY(&curproxy->logformat) ||
         !LIST_ISEMPTY(&curproxy->logformat_sd))) {
      ha_warning("config : log format ignored for %s '%s' since it has no log "
                 "address.\n",
                 proxy_type_str(curproxy), curproxy->id);
      err_code |= ERR_WARN;
    }
    ...
    if (curproxy->mode != PR_MODE_HTTP) {
      ...
      if (curproxy->uri_auth) {
        ha_warning("config : 'stats' statement ignored for %s '%s' as it "
                   "requires HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
      ...
      if (curproxy->capture_name) {
        ha_warning("config : 'capture' statement ignored for %s '%s' as it "
                   "requires HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
      }
      ...
      if (!LIST_ISEMPTY(&curproxy->http_req_rules)) {
        ha_warning("config : 'http-request' rules ignored for %s '%s' as they "
                   "require HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
      }
      ...
      if (!LIST_ISEMPTY(&curproxy->http_res_rules)) {
        ha_warning("config : 'http-response' rules ignored for %s '%s' as they "
                   "require HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
      }
      ...
      if (!LIST_ISEMPTY(&curproxy->redirect_rules)) {
        ha_warning("config : 'redirect' rules ignored for %s '%s' as they "
                   "require HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id);
        err_code |= ERR_WARN;
      }
      ...
      if (curproxy->options & (PR_O_FWDFOR | PR_O_FF_ALWAYS)) {
        ha_warning("config : 'option %s' ignored for %s '%s' as it requires "
                   "HTTP mode.\n",
      ...
      for (optnum = 0; cfg_opts[optnum].name; optnum++) {
        if (cfg_opts[optnum].mode == PR_MODE_HTTP &&
          ...
          ha_warning("config : 'option %s' ignored for %s '%s' as it requires "
                     "HTTP mode.\n",
                     cfg_opts[optnum].name, proxy_type_str(curproxy),
                     curproxy->id);
          err_code |= ERR_WARN;
          curproxy->options &= ~cfg_opts[optnum].val;
        }
      ...
      for (optnum = 0; cfg_opts2[optnum].name; optnum++) {
        if (cfg_opts2[optnum].mode == PR_MODE_HTTP &&
          ...
          ha_warning("config : 'option %s' ignored for %s '%s' as it requires "
                     "HTTP mode.\n",
                     cfg_opts2[optnum].name, proxy_type_str(curproxy),
                     curproxy->id);
          err_code |= ERR_WARN;
          curproxy->options2 &= ~cfg_opts2[optnum].val;
        }
      ...
      if (curproxy->conn_src.bind_hdr_occ) {
        ...
        ha_warning("config : %s '%s' : ignoring use of header %s as source IP "
                   "in non-HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id,
                   curproxy->conn_src.bind_hdr_name);
        err_code |= ERR_WARN;
      }
#endif
    }
    ...
    while (newsrv != NULL) {
      if ((curproxy->mode != PR_MODE_HTTP) && newsrv->rdr_len) {
        ha_alert("config : %s '%s' : server cannot have cookie or redirect "
                 "prefix in non-HTTP mode.\n",
                 proxy_type_str(curproxy), curproxy->id);
        cfgerr++;
      }

      if ((curproxy->mode != PR_MODE_HTTP) && newsrv->cklen) {
        ha_warning("config : %s '%s' : ignoring cookie for server '%s' as HTTP "
                   "mode is disabled.\n",
                   proxy_type_str(curproxy), curproxy->id, newsrv->id);
        err_code |= ERR_WARN;
      }

      if ((newsrv->flags & SRV_F_MAPPORTS) &&
          (curproxy->options2 & PR_O2_RDPC_PRST)) {
        ha_warning(
            "config : %s '%s' : RDP cookie persistence will not work for "
            "server '%s' because it lacks an explicit port number.\n",
            proxy_type_str(curproxy), curproxy->id, newsrv->id);
        err_code |= ERR_WARN;
      }
      ...
      if (curproxy->mode != PR_MODE_HTTP && newsrv->conn_src.bind_hdr_occ) {
        ...
        ha_warning("config : %s '%s' : server %s cannot use header %s as "
                   "source IP in non-HTTP mode.\n",
                   proxy_type_str(curproxy), curproxy->id, newsrv->id,
                   newsrv->conn_src.bind_hdr_name);
        err_code |= ERR_WARN;
      }
#endif
    ...
    if (curproxy->cap & PR_CAP_FE) {
      ...
      if (!LIST_ISEMPTY(&curproxy->filter_configs)) {
        curproxy->fe_req_ana |=
            AN_REQ_FLT_START_FE | AN_REQ_FLT_XFER_DATA | AN_REQ_FLT_END;
        curproxy->fe_rsp_ana |=
            AN_RES_FLT_START_FE | AN_RES_FLT_XFER_DATA | AN_RES_FLT_END;
        if (curproxy->mode == PR_MODE_HTTP) {
    ...
    if (curproxy->cap & PR_CAP_BE) {
      ...
      if (curproxy->mode == PR_MODE_HTTP) {
        curproxy->be_req_ana |=
            AN_REQ_WAIT_HTTP | AN_REQ_HTTP_INNER | AN_REQ_HTTP_PROCESS_BE;
      ...
      if (!LIST_ISEMPTY(&curproxy->filter_configs)) {
        curproxy->be_req_ana |=
            AN_REQ_FLT_START_BE | AN_REQ_FLT_XFER_DATA | AN_REQ_FLT_END;
        curproxy->be_rsp_ana |=
            AN_RES_FLT_START_BE | AN_RES_FLT_XFER_DATA | AN_RES_FLT_END;
        if (curproxy->mode == PR_MODE_HTTP) {
    ...
    list_for_each_entry(bind_conf, &curproxy->conf.bind, by_fe) {
       ...
       */
      mux_ent = conn_get_best_mux_entry(bind_conf->mux_proto->token,
                                        PROTO_SIDE_FE, mode);

      if (!mux_ent || !isteq(mux_ent->token, bind_conf->mux_proto->token)) {
        ha_alert("config : %s '%s' : MUX protocol '%.*s' is not usable for "
                 "'bind %s' at [%s:%d].\n",
                 proxy_type_str(curproxy), curproxy->id,
                 (int)bind_conf->mux_proto->token.len,
                 bind_conf->mux_proto->token.ptr, bind_conf->arg,
    ...
    for (newsrv = curproxy->srv; newsrv; newsrv = newsrv->next) {
       ...
       * on first pass. Let's adjust it now.
       */
      mux_ent = conn_get_best_mux_entry(newsrv->mux_proto->token, PROTO_SIDE_BE,
                                        mode);
      ...
      if (!mux_ent || !isteq(mux_ent->token, newsrv->mux_proto->token)) {
        ha_alert("config : %s '%s' : MUX protocol '%.*s' is not usable for "
                 ...
                 newsrv->mux_proto->token.ptr, newsrv->id, newsrv->conf.file,
                 newsrv->conf.line);
        cfgerr++;
      }
    ...
    for (newsrv = curproxy->srv; newsrv; newsrv = newsrv->next) {
      ...
      newsrv->available_conns =
          calloc(global.nbthread, sizeof(*newsrv->available_conns));

      if (!newsrv->available_conns) {
        ha_alert("parsing [%s:%d] : failed to allocate idle connections for "
                 "server '%s'.\n",
      ...
      if (newsrv->max_idle_conns != 0) {
        if (idle_conn_task == NULL) {
          ...
          for (i = 0; i < global.nbthread; i++) {
            ...
            if (!idle_conns[i].cleanup_task)
              goto err;
            idle_conns[i].cleanup_task->process =
        ...
        newsrv->idle_conns = calloc((unsigned short)global.nbthread,
                                    sizeof(*newsrv->idle_conns));
        ...
        newsrv->safe_conns =
            calloc(global.nbthread, sizeof(*newsrv->safe_conns));
        ...
        newsrv->curr_idle_thr =
            calloc(global.nbthread, sizeof(*newsrv->curr_idle_thr));
        if (!newsrv->curr_idle_thr)
          goto err;
        continue;
      err:
        ha_alert("parsing [%s:%d] : failed to allocate idle connection tasks "
                 "for server '%s'.\n",
                 newsrv->conf.file, newsrv->conf.line, newsrv->id);
        cfgerr++;
        continue;
  ...
  if (global.nbproc > 1 && global.stats_fe) {
    ...
    if (&bind_conf->by_fe != &global.stats_fe->conf.bind) {
      ha_warning("stats socket will not work as expected in multi-process mode "
                 "(nbproc > 1), you should force process binding globally "
                 "using 'stats bind-process' or per socket using the 'process' "
                 "attribute.\n");
    }
  }
  ...
  for (curproxy = proxies_list; curproxy; curproxy = curproxy->next) {
    ...
    list_for_each_entry(listener, &curproxy->conf.listeners, by_fe) {
      ...
      nbproc = my_popcountl(curproxy->bind_proc &
                            (listener->bind_conf->settings.bind_proc
                                 ? listener->bind_conf->settings.bind_proc
                                 : curproxy->bind_proc) &
                            all_proc_mask);
      ...
      if (!listener->maxaccept)
        listener->maxaccept =
            global.tune.maxaccept ? global.tune.maxaccept : 64;
    ...
    if (atleast2(curproxy->bind_proc & all_proc_mask)) {
      if (curproxy->uri_auth) {
        ...
        if (!LIST_ISEMPTY(&curproxy->uri_auth->admin_rules)) {
          ha_warning("Proxy '%s': stats admin will not work correctly in "
                     "multi-process mode.\n",
                     curproxy->id);
        }
      }
      if (!LIST_ISEMPTY(&curproxy->sticking_rules)) {
        ha_warning("Proxy '%s': sticking rules will not work correctly in "
                   "multi-process mode.\n",
                   curproxy->id);
      }
    }
    ...
    if (curproxy->task) {
      ...
      ha_alert("Proxy '%s': no more memory when trying to allocate the "
               "management task\n",
               curproxy->id);
      cfgerr++;
    }
  ...
  if (cfg_peers) {
    ...
    while (*last) {
      ...
      if (curpeers->disabled) {
        ...
        ha_warning(
            "Removing incomplete section 'peers %s' (no peer named '%s').\n",
        ...
        /* Initializes the transport layer of the server part of all the peers
         * belonging to <curpeers> section if required. Note that ->srv is used
         * by the local peer of a new process to connect to the local peer of an
         * old process.
         */
        p = curpeers->remote;
        while (p) {
          if (p->srv) {
            if (p->srv->use_ssl == 1 && xprt_get(XPRT_SSL) &&
  ...
  list_for_each_entry(curr_resolvers, &dns_resolvers, list) {
    if (LIST_ISEMPTY(&curr_resolvers->nameservers)) {
      ha_warning(
          "config : resolvers '%s' [%s:%d] has no nameservers configured!\n",
