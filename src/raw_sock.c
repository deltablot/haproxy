...
/* A pipe contains 16 segments max, and it's common to see segments of 1448
 * bytes because of timestamps. Use this as a hint for not looping on splice().
 */
 ...
 */
int raw_sock_to_pipe(struct connection *conn, void *xprt_ctx, struct pipe *pipe,
                     unsigned int count) {
  int ret;
  int retval = 0;
  ...
  if (unlikely(!(fdtab[conn->handle.fd].ev & FD_POLL_IN))) {
    ...
    if ((fdtab[conn->handle.fd].ev & FD_POLL_ERR) &&
 ...
 */
int raw_sock_from_pipe(struct connection *conn, void *xprt_ctx,
                       struct pipe *pipe) {
  int ret, done;
 ...
 * error without errno, it's pending and can be retrieved via
 * getsockopt(SO_ERROR).
 */
static size_t raw_sock_to_buf(struct connection *conn, void *xprt_ctx,
 ...
 * caller is responsible for this. It's up to the caller to update the buffer's
 * contents based on the return value.
 */
static size_t raw_sock_from_buf(struct connection *conn, void *xprt_ctx,
 ...
 */
static int raw_sock_subscribe(struct connection *conn, void *xprt_ctx,
 ...
 */
static int raw_sock_unsubscribe(struct connection *conn, void *xprt_ctx,
                                int event_type, struct wait_event *es) {
  return conn_unsubscribe(conn, xprt_ctx, event_type, es);
}

/* We can't have an underlying XPRT, so just return -1 to signify failure */
static int raw_sock_remove_xprt(struct connection *conn, void *xprt_ctx,
                                void *toremove_ctx,
                                const struct xprt_ops *newops, void *newctx) {
...
__attribute__((constructor)) static void __raw_sock_init(void) {
