...
int cfg_parse_global(const char *file, int linenum, char **args, int kwm) {
  ...
  if (!strcmp(args[0], "global")) { /* new section */
    ...
    if (*args[1]) {
      if (!strcmp(args[1], "no-exit-on-failure")) {
        ...
        ha_alert("parsing [%s:%d] : '%s' only supports 'no-exit-on-failure' "
                 "option.\n",
                 file, linenum, args[0]);
    ...
    global.tune.options &= ~GTUNE_USE_POLL;
                 ...
                 args[0],
                 "busy-polling")) { /* "no busy-polling" or "busy-polling" */
    if (alertif_too_many_args(0, file, linenum, args, &err_code))
      goto out;
    if (kwm == KWM_NO)
      ...
      global.tune.options |= GTUNE_BUSY_POLLING;
                 ...
                 args[0],
                 "set-dumpable")) { /* "no set-dumpable" or "set-dumpable" */
    if (alertif_too_many_args(0, file, linenum, args, &err_code))
      goto out;
    if (kwm == KWM_NO)
      ...
      global.tune.options |= GTUNE_SET_DUMPABLE;
                     ...
                     "insecure-fork-wanted")) { /* "no insecure-fork-wanted" or
    ...
    if (kwm == KWM_NO)
      ...
      global.tune.options |= GTUNE_INSECURE_FORK;
                 ...
                 args[0],
                 "insecure-setuid-wanted")) { /* "no insecure-setuid-wanted" or
    ...
    if (/*max < -1 || */ max > INT_MAX) {
      ha_alert("parsing [%s:%d] : '%s' expects -1 or an integer from 0 to "
               "INT_MAX.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (global.tune.buf_limit &&
        global.tune.buf_limit <= global.tune.reserved_bufs)
      global.tune.buf_limit = global.tune.reserved_bufs + 1;
    ...
    global.tune.bufsize =
        (global.tune.bufsize + 2 * sizeof(void *) - 1) & -(2 * sizeof(void *));
    ...
    if (alertif_too_many_args(1, file, linenum, args, &err_code))
      goto out;
    if (*(args[1]) == 0) {
      ha_alert("parsing [%s:%d] : '%s' expects a timer value between 0 and "
               "65535 ms.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (global.tune.max_http_hdr < 1 || global.tune.max_http_hdr > 32767) {
      ha_alert("parsing [%s:%d] : '%s' expects a numeric value between 1 and "
               "32767\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (*args[1]) {
      ...
      if (global.tune.comp_maxlevel < 1 || global.tune.comp_maxlevel > 9) {
        ha_alert(
            "parsing [%s:%d] : '%s' expects a numeric value between 1 and 9\n",
            file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
    ...
    if (strl2irc(args[1], strlen(args[1]), &global.uid) != 0) {
      ha_warning(
          "parsing [%s:%d] :  uid: string '%s' is not a number.\n   | You "
          "might want to use the 'user' parameter to use a system user name.\n",
          file, linenum, args[1]);
      err_code |= ERR_WARN;
      goto out;
    }
    ...
    if (strl2irc(args[1], strlen(args[1]), &global.gid) != 0) {
      ha_warning("parsing [%s:%d] :  gid: string '%s' is not a number.\n   | "
                 "You might want to use the 'group' parameter to use a system "
                 "group name.\n",
                 file, linenum, args[1]);
      err_code |= ERR_WARN;
      goto out;
  ...
  /* user/group name handling */
    ...
    if (ha_user != NULL) {
      ...
      ha_alert("parsing [%s:%d] : cannot find user id for '%s' (%d:%s)\n", file,
               linenum, args[1], errno, strerror(errno));
      err_code |= ERR_ALERT | ERR_FATAL;
    }
    ...
    if (global.gid != 0) {
      ha_alert(
          "parsing [%s:%d] : gid/group was already specified. Continuing.\n",
          file, linenum);
      err_code |= ERR_ALERT;
      goto out;
    }
  ...
  /* end of user/group name handling*/
    ...
    if (global.maxconn > SYSTEM_MAXCONN && cfg_maxconn <= SYSTEM_MAXCONN) {
      ha_alert("parsing [%s:%d] : maxconn value %d too high for this "
    ...
    if (alertif_too_many_args(1, file, linenum, args, &err_code))
      goto out;
    if (*(args[1]) == 0) {
      ha_alert("parsing [%s:%d] : '%s' expects an integer argument between 0 "
               "and 100.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!i || args[1][i]) {
      ha_alert(
          "parsing [%s:%d]: '%s' requires valid node name - non-empty string"
          " with digits(0-9), letters(A-Z, a-z), dot(.), hyphen(-) or "
          "underscode(_).\n",
          file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    while (*(args[cur_arg])) {
      if (!strcmp(args[cur_arg], "prefix")) {
        if (global.unix_bind.prefix != NULL) {
          ha_alert("parsing [%s:%d] : unix-bind '%s' already specified. "
                   "Continuing.\n",
        ...
        if (*(args[cur_arg + 1]) == 0) {
          ha_alert("parsing [%s:%d] : unix_bind '%s' expects a path as an "
                   "argument.\n",
                   file, linenum, args[cur_arg]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
      ...
      ha_alert("parsing [%s:%d] : '%s' only supports the 'prefix', 'mode', "
               "'uid', 'gid', 'user' and 'group' options.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (!parse_logsrv(args, &global.logsrvs, (kwm == KWM_NO), &errmsg)) {
      ...
      goto out;
                 ...
                 args[0],
                 "log-send-hostname")) { /* set the hostname in syslog header */
    char *name;
    ...
    global.log_send_hostname = strdup(name);
                     ...
                     "server-state-base")) { /* path base where HAProxy can find
                                                server state files */
    ...
    if (!*(args[1])) {
      ha_alert(
          "parsing [%s:%d] : '%s' expects one argument: a directory path.\n",
          file, linenum, args[0]);
      err_code |= ERR_FATAL;
      goto out;
    }
    ...
    global.server_state_base = strdup(args[1]);
                     ...
                     "server-state-file")) { /* path to the file where HAProxy
                                                can load the server states */
    ...
    chunk_initlen(&global.log_tag, strdup(args[1]), strlen(args[1]),
                  strlen(args[1]));
    if (b_orig(&global.log_tag) == NULL) {
      ...
      goto out;
                     ...
                     "spread-checks")) { /* random time between checks (0-50) */
    if (alertif_too_many_args(1, file, linenum, args, &err_code))
      goto out;
    ...
    if (global.spread_checks < 0 || global.spread_checks > 50) {
      ha_alert("parsing [%s:%d]: 'spread-checks' needs a positive value in "
               "range 0..50.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
    }
  } else if (!strcmp(args[0], "max-spread-checks")) { /* maximum time between
    ...
    if (err == PARSE_TIME_OVER) {
      ...
      ha_alert("parsing [%s:%d]: unsupported character '%c' in '%s' (wants an "
               "integer delay).\n",
...
#else
    ha_alert("parsing [%s:%d] : '%s' is not enabled, please check build "
             "options for USE_CPU_AFFINITY.\n",
             file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
#endif /* ! USE_CPU_AFFINITY */
  } else if (strcmp(args[0], "setenv") == 0 ||
             strcmp(args[0], "presetenv") == 0) {
    ...
    if (setenv(args[1], args[2], (args[0][0] == 's')) != 0) {
      ha_alert("parsing [%s:%d]: '%s' failed on variable '%s' : %s.\n", file,
               linenum, args[0], args[1], strerror(errno));
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    for (arg = 1; *args[arg]; arg++) {
      if (unsetenv(args[arg]) != 0) {
        ha_alert("parsing [%s:%d]: '%s' failed on variable '%s' : %s.\n", file,
                 linenum, args[0], args[arg], strerror(errno));
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
    }
    ...
    while (*env) {
      ...
      if (!*args[arg]) {
        ...
        if (!delim || delim - *env >= trash.size) {
          ha_alert(
              "parsing [%s:%d]: '%s' failed to unset invalid variable '%s'.\n",
        ...
        if (unsetenv(trash.area) != 0) {
          ha_alert(
              "parsing [%s:%d]: '%s' failed to unset variable '%s' : %s.\n",
        ...
        env++;
                 ...
                 args[0],
                 "strict-limits")) { /* "no strict-limits" or "strict-limits" */
    if (alertif_too_many_args(0, file, linenum, args, &err_code))
      goto out;
    if (kwm == KWM_NO)
    ...
    if (global.localpeer_cmdline != 0) {
      ha_warning("parsing [%s:%d] : '%s' ignored since it is already set by "
                 "using the '-L' "
    ...
    if (cfg_peers) {
      ha_warning("parsing [%s:%d] : '%s' ignored since it is used after "
                 "'peers' section.\n",
                 file, linenum, args[0]);
      err_code |= ERR_WARN;
      goto out;
    ...
    list_for_each_entry(kwl, &cfg_keywords.list, list) {
      for (index = 0; kwl->kw[index].kw != NULL; index++) {
        ...
        if (strcmp(kwl->kw[index].kw, args[0]) == 0) {
          rc = kwl->kw[index].parse(args, CFG_GLOBAL, NULL, NULL, file, linenum,
                                    &errmsg);
          if (rc < 0) {
