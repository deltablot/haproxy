...
struct ebpt_node *ebpt_lookup_le(struct eb_root *root, void *x) {
  ...
  while (1) {
    if ((eb_gettag(troot) == EB_LEAF)) {
       ...
       */
      node = container_of(eb_untag(troot, EB_LEAF), struct ebpt_node,
                          node.branches);
      if (node->key <= x)
        return node;
      /* return prev */
    ...
    node =
        container_of(eb_untag(troot, EB_NODE), struct ebpt_node, node.branches);
    ...
    if (node->node.bit < 0) {
      ...
      if (node->key <= x) {
        ...
        return container_of(eb_untag(troot, EB_LEAF), struct ebpt_node,
                            node.branches);
      }
      /* return prev */
    ...
    troot = node->node.branches
                .b[((ptr_t)x >> node->node.bit) & EB_NODE_BRANCH_MASK];
  }
