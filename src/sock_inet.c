 ...
 */
int sock_inet4_addrcmp(const struct sockaddr_storage *a,
 ...
 */
int sock_inet6_addrcmp(const struct sockaddr_storage *a,
                       const struct sockaddr_storage *b) {
 ...
 * according to the various supported socket options. The socket's address
 * family must be passed in <family>.
 */
...
int sock_inet_bind_receiver(struct receiver *rx, char **errmsg) {
  ...
  if (!ext && rx->settings->interface) {
    if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, rx->settings->interface,
  ...
  if (!ext && bind(fd, (struct sockaddr *)&addr_inet,
                   rx->proto->fam->sock_addrlen) == -1) {
    err |= ERR_RETRYABLE | ERR_ALERT;
