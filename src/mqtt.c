...
uint8_t mqtt_cpt_flags[MQTT_CPT_ENTRIES] = {
    ...
    /* MQTT_CPT_PUBLISH flags can have different values (DUP, QoS, RETAIN), must
     * be check more carefully
     */
...
const struct ist mqtt_fields_string[MQTT_FN_ENTRIES] = {
    ...
    [MQTT_FN_PROTOCOL_VERSION] =
        IST("protocol_version"), /* MQTT 3.1.1: protocol_level */
...
const uint64_t mqtt_fields_per_packet[MQTT_CPT_ENTRIES] = {
    ...
    [MQTT_CPT_CONNECT] =
        MQTT_FN_BIT_PROTOCOL_NAME | MQTT_FN_BIT_PROTOCOL_VERSION |
        ...
        MQTT_FN_BIT_REQUEST_RESPONSE_INFORMATION |
        ...
        MQTT_FN_BIT_CLIENT_IDENTIFIER | MQTT_FN_BIT_DELAY_INTERVAL |
        MQTT_FN_BIT_PAYLOAD_FORMAT_INDICATOR |
        ...
        MQTT_FN_BIT_WILL_PAYLOAD | MQTT_FN_BIT_USERNAME | MQTT_FN_BIT_PASSWORD,

    [MQTT_CPT_CONNACK] =
        MQTT_FN_BIT_FLAGS | MQTT_FN_BIT_PROTOCOL_VERSION |
        ...
        MQTT_FN_BIT_ASSIGNED_CLIENT_IDENTIFIER |
        MQTT_FN_BIT_TOPIC_ALIAS_MAXIMUM | MQTT_FN_BIT_REASON_STRING |
        MQTT_FN_BIT_WILDCARD_SUBSCRIPTION_AVAILABLE |
        MQTT_FN_BIT_SUBSCRIPTION_IDENTIFIERS_AVAILABLE |
        MQTT_FN_BIT_SHARED_SUBSCRIPTION_AVAILABLE |
        MQTT_FN_BIT_SERVER_KEEPALIVE | MQTT_FN_BIT_RESPONSE_INFORMATION |
        MQTT_FN_BIT_SERVER_REFERENCE | MQTT_FN_BIT_USER_PROPERTY |
        MQTT_FN_BIT_AUTHENTICATION_METHOD | MQTT_FN_BIT_AUTHENTICATION_DATA,

    [MQTT_CPT_PUBLISH] = MQTT_FN_BIT_PAYLOAD_FORMAT_INDICATOR |
                         MQTT_FN_BIT_MESSAGE_EXPIRY_INTERVAL |
                         MQTT_FN_BIT_CONTENT_TYPE | MQTT_FN_BIT_RESPONSE_TOPIC |
                         MQTT_FN_BIT_CORRELATION_DATA |
                         MQTT_FN_BIT_SUBSCRIPTION_IDENTIFIER |
                         MQTT_FN_BIT_TOPIC_ALIAS | MQTT_FN_BIT_USER_PROPERTY,

    [MQTT_CPT_PUBACK] = MQTT_FN_BIT_REASON_CODE | MQTT_FN_BIT_REASON_STRING |
                        MQTT_FN_BIT_USER_PROPERTY,

    [MQTT_CPT_PUBREC] = MQTT_FN_BIT_REASON_CODE | MQTT_FN_BIT_REASON_STRING |
                        MQTT_FN_BIT_USER_PROPERTY,

    [MQTT_CPT_PUBREL] = MQTT_FN_BIT_REASON_CODE | MQTT_FN_BIT_REASON_STRING |
                        MQTT_FN_BIT_USER_PROPERTY,

    [MQTT_CPT_PUBCOMP] = MQTT_FN_BIT_REASON_CODE | MQTT_FN_BIT_REASON_STRING |
                         MQTT_FN_BIT_USER_PROPERTY,

    [MQTT_CPT_SUBSCRIBE] =
        MQTT_FN_BIT_SUBSCRIPTION_IDENTIFIER | MQTT_FN_BIT_USER_PROPERTY,
    ...
    [MQTT_CPT_AUTH] = MQTT_FN_BIT_AUTHENTICATION_METHOD |
                      MQTT_FN_BIT_AUTHENTICATION_DATA |
 ...
 * 1   |     0      |
 * +-------+-----------+-----------+-----------+---------+----------+----------+---------+------------+
 * | field |          MQTT Control Packet Type           | Flags specific to
 * each Control Packet type |
 * +-------+---------------------------------------------+--------------------------------------------+
 *
 * On success, <ptk> is updated with the packet type and flags and the new
 * parser state is returned. On error, IST_NULL is returned.
 */
static inline struct ist mqtt_read_fixed_hdr(struct ist parser,
                                             struct mqtt_pkt *pkt) {
 ...
 * the integer is stored in <*i>, if provided, and the new parser state is
...
struct ist mqtt_field_value(struct ist msg, int type, int fieldname_id) {
  ...
  case MQTT_CPT_CONNECT:
    ...
    case MQTT_FN_PAYLOAD_FORMAT_INDICATOR:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connect.payload.will_props.payload_format_indicator))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_MESSAGE_EXPIRY_INTERVAL:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connect.payload.will_props.message_expiry_interval))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_SESSION_EXPIRY_INTERVAL:
      if (mpkt.data.connect.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(
              trash, mpkt.data.connect.var_hdr.props.session_expiry_interval))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_REQUEST_PROBLEM_INFORMATION:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connect.var_hdr.props.request_problem_information))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_DELAY_INTERVAL:
      if ((mpkt.data.connect.var_hdr.protocol_version != MQTT_VERSION_5_0) ||
          !(mpkt.data.connect.var_hdr.flags & MQTT_CONNECT_FL_WILL))
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connect.payload.will_props.delay_interval))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_REQUEST_RESPONSE_INFORMATION:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connect.var_hdr.props.request_response_information))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_RECEIVE_MAXIMUM:
      if (mpkt.data.connect.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connect.var_hdr.props.receive_maximum))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_TOPIC_ALIAS_MAXIMUM:
      if (mpkt.data.connect.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connect.var_hdr.props.topic_alias_maximum))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_MAXIMUM_PACKET_SIZE:
      if (mpkt.data.connect.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connect.var_hdr.props.maximum_packet_size))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
  ...
  case MQTT_CPT_CONNACK:
    ...
    case MQTT_FN_SESSION_EXPIRY_INTERVAL:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(
              trash, mpkt.data.connack.var_hdr.props.session_expiry_interval))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_SERVER_KEEPALIVE:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connack.var_hdr.props.server_keepalive))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_RECEIVE_MAXIMUM:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connack.var_hdr.props.receive_maximum))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_TOPIC_ALIAS_MAXIMUM:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connack.var_hdr.props.topic_alias_maximum))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_RETAIN_AVAILABLE:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connack.var_hdr.props.retain_available))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_MAXIMUM_PACKET_SIZE:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash,
                         mpkt.data.connack.var_hdr.props.maximum_packet_size))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_WILDCARD_SUBSCRIPTION_AVAILABLE:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connack.var_hdr.props.wildcard_subscription_available))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
    ...
    case MQTT_FN_SUBSCRIPTION_IDENTIFIERS_AVAILABLE:
      if (mpkt.data.connack.var_hdr.protocol_version != MQTT_VERSION_5_0)
        goto not_found_or_invalid;
      if (!mqtt_uint2str(trash, mpkt.data.connack.var_hdr.props
    ...
    case MQTT_FN_SHARED_SUBSCRIPTION_AVAILABLE:
      ...
      if (!mqtt_uint2str(
              trash,
              mpkt.data.connack.var_hdr.props.shared_subsription_available))
        goto not_found_or_invalid;
      res = ist2(trash->area, trash->data);
      goto end;
...
static int mqtt_parse_connect(struct ist parser, struct mqtt_pkt *mpkt) {
  ...
  parser =
      mqtt_read_1byte_int(parser, &mpkt->data.connect.var_hdr.protocol_version);
  if (!isttest(parser))
    goto end;
  ...
  if (!isttest(parser) ||
      (mpkt->data.connect.var_hdr.flags & MQTT_CONNECT_FL_RESERVED))
    goto end;

  /* if WILL flag must be set to have WILL_QOS flag or WILL_RETAIN set */
  if ((mpkt->data.connect.var_hdr.flags &
       (MQTT_CONNECT_FL_WILL | MQTT_CONNECT_FL_WILL_QOS |
        MQTT_CONNECT_FL_WILL_RETAIN)) == MQTT_CONNECT_FL_WILL_QOS)
    goto end;
    ...
    while (istlen(props) > 0) {
      ...
      case MQTT_PROP_SESSION_EXPIRY_INTERVAL:
        if (fields & MQTT_FN_BIT_SESSION_EXPIRY_INTERVAL)
          goto end;
        props = mqtt_read_4byte_int(
            istnext(props),
            &mpkt->data.connect.var_hdr.props.session_expiry_interval);
      ...
      case MQTT_PROP_RECEIVE_MAXIMUM:
        if (fields & MQTT_FN_BIT_RECEIVE_MAXIMUM)
          goto end;
        props = mqtt_read_2byte_int(
            istnext(props), &mpkt->data.connect.var_hdr.props.receive_maximum);
        /* cannot be 0 */
      ...
      case MQTT_PROP_USER_PROPERTIES:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.var_hdr.props.user_props[user_prop_idx].name);
        if (!isttest(props))
          goto end;
        props = mqtt_read_string(
            props,
            &mpkt->data.connect.var_hdr.props.user_props[user_prop_idx].value);
        ++user_prop_idx;
        break;
      ...
      case MQTT_PROP_AUTHENTICATION_METHOD:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.var_hdr.props.authentication_method);
      ...
      case MQTT_PROP_AUTHENTICATION_DATA:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.var_hdr.props.authentication_data);
  ...
  /* parsing payload
   *
   * Content of payload is realted to flags parsed above and the field order is
   * pre-defined: Client Identifier, Will Topic, Will Message, User Name,
   * Password
   */
  /* read client identifier */
  parser =
      mqtt_read_string(parser, &mpkt->data.connect.payload.client_identifier);
  ...
  if ((mpkt->data.connect.var_hdr.protocol_version == MQTT_VERSION_5_0) &&
    ...
    while (istlen(props) > 0) {
      ...
      case MQTT_PROP_CONTENT_TYPE:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.payload.will_props.content_type);
      ...
      case MQTT_PROP_RESPONSE_TOPIC:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.payload.will_props.response_topic);
      ...
      case MQTT_PROP_CORRELATION_DATA:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.payload.will_props.correlation_data);
      ...
      case MQTT_PROP_USER_PROPERTIES:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connect.payload.will_props.user_props[user_prop_idx]
                 .name);
        if (!isttest(props))
          goto end;
        props = mqtt_read_string(props, &mpkt->data.connect.payload.will_props
                                             .user_props[user_prop_idx]
                                             .value);
        ++user_prop_idx;
        break;
  ...
  if (mpkt->data.connack.var_hdr.protocol_version == MQTT_VERSION_5_0) {
    ...
    while (istlen(props) > 0) {
      ...
      case MQTT_PROP_SESSION_EXPIRY_INTERVAL:
        ...
        props = mqtt_read_4byte_int(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.session_expiry_interval);
        fields |= MQTT_FN_BIT_SESSION_EXPIRY_INTERVAL;
        break;
      ...
      case MQTT_PROP_RECEIVE_MAXIMUM:
        if (fields & MQTT_FN_BIT_RECEIVE_MAXIMUM)
          goto end;
        props = mqtt_read_2byte_int(
            istnext(props), &mpkt->data.connack.var_hdr.props.receive_maximum);
        /* cannot be 0 */
      ...
      case MQTT_PROP_MAXIMUM_QOS:
        if (fields & MQTT_FN_BIT_MAXIMUM_QOS)
          goto end;
        props = mqtt_read_1byte_int(
            istnext(props), &mpkt->data.connack.var_hdr.props.maximum_qos);
        /* can have only 2 values: 0 or 1 */
      ...
      case MQTT_PROP_RETAIN_AVAILABLE:
        if (fields & MQTT_FN_BIT_RETAIN_AVAILABLE)
          goto end;
        props = mqtt_read_1byte_int(
            istnext(props), &mpkt->data.connack.var_hdr.props.retain_available);
        /* can have only 2 values: 0 or 1 */
      ...
      case MQTT_PROP_ASSIGNED_CLIENT_IDENTIFIER:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.assigned_client_identifier);
        if (!istlen(
                mpkt->data.connack.var_hdr.props.assigned_client_identifier))
          goto end;
      ...
      case MQTT_PROP_REASON_STRING:
        if (fields & MQTT_FN_BIT_REASON_STRING)
          goto end;
        props = mqtt_read_string(
            istnext(props), &mpkt->data.connack.var_hdr.props.reason_string);
      ...
      case MQTT_PROP_WILDCARD_SUBSCRIPTION_AVAILABLE:
        ...
        props = mqtt_read_1byte_int(
            ...
            &mpkt->data.connack.var_hdr.props.wildcard_subscription_available);
        /* can have only 2 values: 0 or 1 */
        if (mpkt->data.connack.var_hdr.props.wildcard_subscription_available >
            1)
          goto end;
      ...
      case MQTT_PROP_SUBSCRIPTION_IDENTIFIERS_AVAILABLE:
        if (fields & MQTT_FN_BIT_SUBSCRIPTION_IDENTIFIER)
          goto end;
        props = mqtt_read_1byte_int(istnext(props),
                                    &mpkt->data.connack.var_hdr.props
                                         .subscription_identifiers_available);
        /* can have only 2 values: 0 or 1 */
        if (mpkt->data.connack.var_hdr.props
      ...
      case MQTT_PROP_SERVER_KEEPALIVE:
        if (fields & MQTT_FN_BIT_SERVER_KEEPALIVE)
          goto end;
        props = mqtt_read_2byte_int(
            istnext(props), &mpkt->data.connack.var_hdr.props.server_keepalive);
      ...
      case MQTT_PROP_RESPONSE_INFORMATION:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.response_information);
      ...
      case MQTT_PROP_SERVER_REFERENCE:
        if (fields & MQTT_FN_BIT_SERVER_REFERENCE)
          goto end;
        props = mqtt_read_string(
            istnext(props), &mpkt->data.connack.var_hdr.props.server_reference);
      ...
      case MQTT_PROP_USER_PROPERTIES:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.user_props[user_prop_idx].name);
        if (!isttest(props))
          goto end;
        props = mqtt_read_string(
            props,
            &mpkt->data.connack.var_hdr.props.user_props[user_prop_idx].value);
        ++user_prop_idx;
        break;
      ...
      case MQTT_PROP_AUTHENTICATION_METHOD:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.authentication_method);
        fields |= MQTT_FN_BIT_AUTHENTICATION_METHOD;
        break;
      ...
      case MQTT_PROP_AUTHENTICATION_DATA:
        ...
        props = mqtt_read_string(
            istnext(props),
            &mpkt->data.connack.var_hdr.props.authentication_data);
        fields |= MQTT_FN_BIT_AUTHENTICATION_DATA;
        break;
 ...
 *   https://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc398718028
 *
 * For now, due to HAProxy limitation, only validation of CONNECT and CONNACK
...
int mqtt_validate_message(const struct ist msg, struct mqtt_pkt *mpkt) {
  ...
  /* Now parsing the variable header and payload, which is based on the packet
   * type */
