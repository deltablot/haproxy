...
void ha_thread_dump(struct buffer *buf, int thr, int calling_tid) {
  ...
  chunk_appendf(buf,
                "%c%cThread %-2u: id=0x%llx act=%d glob=%d wq=%d rq=%d tl=%d "
                "tlsz=%d rqsz=%d\n"
                "             stuck=%d prof=%d",
                (thr == calling_tid) ? '*' : ' ', stuck ? '>' : ' ', thr + 1,
                ha_get_pthread_id(thr), thread_has_tasks(),
                ...
                task_per_thread[thr].rqueue_size, stuck,
                !!(task_profiling_mask & thr_bit));

  chunk_appendf(buf, " harmless=%d wantrdv=%d",
  ...
  if (stuck) {
    ...
    for (j = 0; j < nptrs || dump < 2; j++) {
      ...
      if (dump == 2) {
        /* dumping */
        if (addr == run_poll_loop || addr == main ||
            addr == run_tasks_from_lists) {
          dump = 3;
          *buf = bak;
          break;
        }
 ...
 */
void ha_task_dump(struct buffer *buf, const struct task *task,
                  const char *pfx) {
  ...
  if (TASK_IS_TASKLET(task))
    chunk_appendf(buf, "%p (tasklet) calls=%u\n", task, task->calls);
  else
    chunk_appendf(buf, "%p (task) calls=%u last=%llu%s\n", task, task->calls,
                  task->call_date
                      ? (unsigned long long)(now_mono_time() - task->call_date)
                      : 0,
  ...
  hlua = NULL;
  if (s && (hlua = s->hlua)) {
    chunk_appendf(buf, "%sCurrent executing Lua from a stream analyser -- ",
                  pfx);
...
/* parse a "debug dev exit" command. It always returns 1, though it should never
 * return. */
static int debug_parse_cli_exit(char **args, char *payload,
...
static int debug_parse_cli_close(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  int fd;
...
static int debug_parse_cli_delay(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
...
static int debug_parse_cli_log(char **args, char *payload,
                               struct appctx *appctx, void *private) {
  int arg;
...
static int debug_parse_cli_loop(char **args, char *payload,
                                struct appctx *appctx, void *private) {
...
/* parse a "debug dev panic" command. It always returns 1, though it should
 * never return. */
static int debug_parse_cli_panic(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  if (!cli_has_level(appctx, ACCESS_LVL_ADMIN))
    return 1;
...
static int debug_parse_cli_exec(char **args, char *payload,
                                struct appctx *appctx, void *private) {
  int pipefd[2];
  int arg;
  int pid;
  ...
  if (fcntl(pipefd[0], F_SETFD,
            fcntl(pipefd[0], F_GETFD, FD_CLOEXEC) | FD_CLOEXEC) == -1)
    goto fail_fcntl;

  if (fcntl(pipefd[1], F_SETFD,
            fcntl(pipefd[1], F_GETFD, FD_CLOEXEC) | FD_CLOEXEC) == -1)
    goto fail_fcntl;
  ...
  while (1) {
    size_t ret =
        read(pipefd[0], trash.area + trash.data, trash.size - 20 - trash.data);
    if (ret <= 0)
      break;
    trash.data += ret;
...
static int debug_parse_cli_hex(char **args, char *payload,
                               struct appctx *appctx, void *private) {
...
static int debug_parse_cli_tkill(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  int thr = 0;
  int sig = SIGABRT;
...
static int debug_parse_cli_write(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  unsigned long len;
 ...
 */
static int debug_parse_cli_stream(char **args, char *payload,
                                  struct appctx *appctx, void *private) {
  ...
  if (!*args[3]) {
    return cli_err(
        ...
        "     <obj>   = {strm | strm.f | sif.f | sif.s | sif.x | sib.f | sib.s "
        "| sib.x |\n"
        "                txn.f | req.f | req.r | req.w | res.f | res.r | "
        "res.w}\n"
        "     <op>    = {'' (show) | '=' (assign) | '^' (xor) | '+' (or) | '-' "
        "(andnot)}\n"
        "     <value> = 'now' | 64-bit dec/hex integer (0x prefix supported)\n"
        "     'wake' wakes the stream asssigned to 'strm' (default: "
        "current)\n");
  }
  ...
  for (arg = 3; *args[arg]; arg++) {
    ...
    if (isteq(name, ist("strm"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s;
      size = sizeof(s);
    } else if (isteq(name, ist("strm.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->flags;
      size = sizeof(s->flags);
    } else if (isteq(name, ist("txn.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->txn->flags;
      size = sizeof(s->txn->flags);
    } else if (isteq(name, ist("req.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->req.flags;
      size = sizeof(s->req.flags);
    } else if (isteq(name, ist("res.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->res.flags;
      size = sizeof(s->res.flags);
    } else if (isteq(name, ist("req.r"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->req.rex;
      size = sizeof(s->req.rex);
    } else if (isteq(name, ist("res.r"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->res.rex;
      size = sizeof(s->res.rex);
    } else if (isteq(name, ist("req.w"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->req.wex;
      size = sizeof(s->req.wex);
    } else if (isteq(name, ist("res.w"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->res.wex;
      size = sizeof(s->res.wex);
    } else if (isteq(name, ist("sif.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[0].flags;
      size = sizeof(s->si[0].flags);
    } else if (isteq(name, ist("sib.f"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[1].flags;
      size = sizeof(s->si[1].flags);
    } else if (isteq(name, ist("sif.x"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[0].exp;
      size = sizeof(s->si[0].exp);
    } else if (isteq(name, ist("sib.x"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[1].exp;
      size = sizeof(s->si[1].exp);
    } else if (isteq(name, ist("sif.s"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[0].state;
      size = sizeof(s->si[0].state);
    } else if (isteq(name, ist("sib.s"))) {
      ptr = (!s || !may_access(s)) ? NULL : &s->si[1].state;
      size = sizeof(s->si[1].state);
      ...
      return cli_dynerr(
          appctx, memprintf(&msg, "Unsupported field name: '%s'.\n", word));

    /* read previous value */
    if ((s || ptr == &s) && ptr && may_access(ptr) &&
        may_access(ptr + size - 1)) {
      if (size == 8)
        old = read_u64(ptr);
      else if (size == 4)
        old = read_u32(ptr);
      ...
      memprintf(&msg, "%sSkipping inaccessible pointer %p for field '%.*s'.\n",
    ...
    if (end[1] && *endarg) {
      if (strcmp(end + 1, "now") == 0)
        ...
        memprintf(&msg, "%sIgnoring unparsable value '%s' for field '%.*s'.\n",
    ...
    switch (*end) {
    case '\0': /* show */
      memprintf(&msg, "%s%.*s=%#llx ", msg ? msg : "", (int)(end - word), word,
                old);
...
static struct task *debug_task_handler(struct task *t, void *ctx,
                                       unsigned short state) {
  unsigned long *tctx =
      ctx; // [0] = #tasks, [1] = inter, [2+] = { tl | (tsk+1) }
...
static struct task *debug_tasklet_handler(struct task *t, void *ctx,
                                          unsigned short state) {
  unsigned long *tctx =
      ctx; // [0] = #tasks, [1] = inter, [2+] = { tl | (tsk+1) }
  unsigned long rnd;
 ...
 * debug dev sched {task|tasklet} [count=<count>] [mask=<mask>]
 * [single=<single>] [inter=<inter>]
 */
static int debug_parse_cli_sched(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  int arg;
  void *ptr;
  int size;
  ...
  for (arg = 4; *args[arg]; arg++) {
    ...
    if (isteq(name, ist("count"))) {
      ...
      return cli_dynerr(appctx,
                        memprintf(&msg, "Unsupported setting: '%s'.\n", word));
...
static int debug_parse_cli_memstats(char **args, char *payload,
                                    struct appctx *appctx, void *private) {
...
static int debug_iohandler_memstats(struct appctx *appctx) {
  ...
  for (ptr = appctx->ctx.cli.p0; ptr != appctx->ctx.cli.p1; ptr++) {
    ...
    switch (ptr->type) {
    case MEM_STATS_TYPE_CALLOC:
      type = "CALLOC";
      break;
    case MEM_STATS_TYPE_FREE:
      type = "FREE";
      break;
    case MEM_STATS_TYPE_MALLOC:
      type = "MALLOC";
      break;
    case MEM_STATS_TYPE_REALLOC:
      type = "REALLOC";
      break;
    case MEM_STATS_TYPE_STRDUP:
      type = "STRDUP";
      break;
    default:
      type = "UNSET";
...
static struct cli_kw_list cli_kws = {
    ...
    {{{"debug", "dev", "close", NULL},
     ...
     {{"debug", "dev", "loop", NULL},
      ...
      ACCESS_EXPERT},
#if defined(DEBUG_MEM_STATS)
     {{"debug", "dev", "memstats", NULL},
      "debug dev memstats [reset|all] : dump/reset memory statistics",
      debug_parse_cli_memstats,
      debug_iohandler_memstats,
      NULL,
      NULL,
      ACCESS_EXPERT},
#endif
     {{"debug", "dev", "panic", NULL},
      ...
      NULL,
      ACCESS_EXPERT},
     {{"debug", "dev", "sched", NULL},
      "debug dev sched ...         : stress the scheduler",
      debug_parse_cli_sched,
      NULL,
      NULL,
      NULL,
      ACCESS_EXPERT},
     {{"debug", "dev", "stream", NULL},
     ...
     {{"debug", "dev", "tkill", NULL},
      ...
      NULL,
      ACCESS_EXPERT},
     {{"debug", "dev", "write", NULL},
      "debug dev write [size]      : write that many bytes",
      debug_parse_cli_write,
      NULL,
      NULL,
      NULL,
      ACCESS_EXPERT},
     {{"show", "threads", NULL, NULL},
