...
static int hlua_panic_ljmp(lua_State *L) {
  WILL_LJMP(longjmp(safe_ljmp_env, 1));
}
...
/* This is a NULL-terminated list of lua file which are referenced to load per
 * thread */
...
#define SET_SAFE_LJMP(__HLUA) SET_SAFE_LJMP_L((__HLUA)->T, __HLUA)

#define RESET_SAFE_LJMP(__HLUA) RESET_SAFE_LJMP_L((__HLUA)->T, __HLUA)
...
__LJMP static const char *hlua_traceback(lua_State *L) {
  ...
  while (lua_getstack(L, level++, &ar)) {
     ...
     * and what; 'l': fills in the field currentline; 'n': fills in the field
     * name and namewhat; 't': fills in the field istailcall;
     */
     ...
     */
    if (*ar.namewhat != '\0' &&
        ar.name != NULL) /* is there a name from code? */
      chunk_appendf(msg, "%s '%s'", ar.namewhat, ar.name); /* use it */

    else if (*ar.what ==
             'm') /* "main", the code is not executed in a function */
...
static int hlua_lua2arg(lua_State *L, int ud, struct arg *arg) {
  ...
  case LUA_TSTRING:
    ...
    /* We don't know the actual size of the underlying allocation, so be
     * conservative. */
    arg->data.str.size =
        arg->data.str.data + 1; /* count the terminating null byte */
    arg->data.str.head = 0;
    break;
...
static int hlua_lua2smp(lua_State *L, int ud, struct sample *smp) {
  ...
  case LUA_TSTRING:
     ...
     * conservative. */
    smp->data.u.str.size =
        smp->data.u.str.data + 1; /* count the terminating null byte */
...
__LJMP int hlua_lua2arg_check(lua_State *L, int first, struct arg *argp,
  ...
  while (1) {
    ...
    if ((mask & ARGT_MASK) == ARGT_STOP && argp[idx].type != ARGT_STOP) {
    ...
    case ARGT_USR:
      ...
      if (!ul) {
        msg = lua_pushfstring(L, "unable to find userlist '%s'",
 ...
 */
__LJMP void hlua_yieldk(lua_State *L, int nresults, int ctx, lua_KFunction k,
...
void hlua_hook(lua_State *L, lua_Debug *ar) {
   ...
   */
  lua_sethook(hlua->T, hlua_hook, LUA_MASKRET | LUA_MASKCOUNT,
              hlua_nb_instruction);
}
...
resume_execution:
  ...
  case LUA_ERRRUN:
    ...
    if (msg)
      lua_pushfstring(lua->T, "[state-id %d] runtime error: %s from %s",
                      lua->state_id, msg, trace);
    else
      lua_pushfstring(lua->T, "[state-id %d] unknown runtime error from %s",
                      lua->state_id, trace);
    ret = HLUA_E_ERRMSG;
    break;
  ...
  case LUA_ERRERR:
    ...
    lua_pop(lua->T, 1);
    if (msg)
      lua_pushfstring(lua->T, "[state-id %d] message handler error: %s",
                      lua->state_id, msg);
    else
      lua_pushfstring(lua->T, "[state-id %d] message handler error",
                      lua->state_id);
    ret = HLUA_E_ERRMSG;
    break;
...
__LJMP static int hlua_map_new(struct lua_State *L) {
  ...
  case PAT_MATCH_REG:
    ...
    break;
  case PAT_MATCH_INT:
    conv.in_type = SMP_T_SINT;
    break;
  case PAT_MATCH_IP:
    conv.in_type = SMP_T_ADDR;
    break;
  default:
...
__LJMP static inline int _hlua_map_lookup(struct lua_State *L, int str) {
  ...
  if (desc->pat.expect_type == SMP_T_SINT) {
    ...
    smp.data.type = SMP_T_STR;
    smp.flags = SMP_F_CONST;
    smp.data.u.str.area = (char *)MAY_LJMP(
        luaL_checklstring(L, 2, (size_t *)&smp.data.u.str.data));
...
connection_empty:
  ...
  if (!notification_new(&hlua->com, &appctx->ctx.hlua_cosocket.wake_on_read,
                        hlua->task)) {
    xref_unlock(&socket->xref, peer);
...
__LJMP static int hlua_socket_receive(struct lua_State *L) {
  ...
  if (lua_gettop(L) < 1 || lua_gettop(L) > 3)
    WILL_LJMP(luaL_error(
        L, "The 'receive' function requires between 1 and 3 arguments."));
 ...
 */
static int hlua_socket_write_yield(struct lua_State *L, int status,
                                   lua_KContext ctx) {
  struct hlua_socket *socket;
  struct hlua *hlua;
  struct appctx *appctx;
...
hlua_socket_write_yield_return:
  if (!notification_new(&hlua->com, &appctx->ctx.hlua_cosocket.wake_on_write,
                        hlua->task)) {
    xref_unlock(&socket->xref, peer);
    WILL_LJMP(luaL_error(L, "out of memory"));
  }
...
#define SOCKET_INFO_MAX_LEN                                                    \
  sizeof("[0000:0000:0000:0000:0000:0000:0000:0000]:12345")
__LJMP static inline int hlua_socket_info(struct lua_State *L,
...
__LJMP static int hlua_socket_connect_yield(struct lua_State *L, int status,
                                            lua_KContext ctx) {
  struct hlua_socket *socket = MAY_LJMP(hlua_checksocket(L, 1));
  struct hlua *hlua;
  struct xref *peer;
...
__LJMP static int hlua_socket_connect(struct lua_State *L) {
  ...
  /* Parse ip address. */
  addr = str2sa_range(ip, NULL, &low, &high, NULL, NULL, NULL, NULL, NULL,
                      PA_O_PORT_OK | PA_O_STREAM);
  if (!addr) {
    xref_unlock(&socket->xref, peer);
    WILL_LJMP(
        luaL_error(L, "connect: cannot parse destination address '%s'", ip));
  }
...
__LJMP static int hlua_socket_setoption(struct lua_State *L) { return 0; }
...
__LJMP static int hlua_socket_settimeout(struct lua_State *L) {
  ...
  if (dtmout > INT_MAX) /* overflow check */
    WILL_LJMP(luaL_error(L, "settimeout: cannot set values larger than %d ms",
                         INT_MAX));
 ...
 */
__LJMP static int hlua_channel_dup_yield(lua_State *L, int status,
                                         lua_KContext ctx) {
  struct channel *chn;
 ...
 */
__LJMP static int hlua_channel_get_yield(lua_State *L, int status,
                                         lua_KContext ctx) {
  struct channel *chn;
  int ret;
 ...
 * value.
 */
__LJMP static int hlua_channel_getline_yield(lua_State *L, int status,
                                             lua_KContext ctx) {
  char *blk1;
  char *blk2;
  size_t len1;
  ...
  if (ret == 0)
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_channel_getline_yield, TICK_ETERNITY, 0));
 ...
 */
__LJMP static int hlua_channel_append_yield(lua_State *L, int status,
                                            lua_KContext ctx) {
 ...
 */
__LJMP static int hlua_channel_send_yield(lua_State *L, int status,
                                          lua_KContext ctx) {
  struct channel *chn = MAY_LJMP(hlua_checkchannel(L, 1));
  size_t len;
  const char *str = MAY_LJMP(luaL_checklstring(L, 2, &len));
 ...
 */
__LJMP static int hlua_channel_forward_yield(lua_State *L, int status,
                                             lua_KContext ctx) {
  struct channel *chn;
  int len;
  int l;
  ...
  if (l < len) {
    ...
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_channel_forward_yield, TICK_ETERNITY, 0));
  }
...
__LJMP static int hlua_run_sample_fetch(lua_State *L) {
  ...
  if (f->use & SMP_USE_HTTP_ANY && !(hsmp->flags & HLUA_F_MAY_USE_HTTP)) {
    lua_pushfstring(L,
                    "the sample-fetch '%s' needs an HTTP parser which "
                    "is not available in Lua services",
                    f->kw);
    WILL_LJMP(lua_error(L));
  }
...
__LJMP static int hlua_applet_tcp_set_priv(lua_State *L) {
  ...
  hlua->Mref =
      luaL_ref(L, LUA_REGISTRYINDEX); /* pop the previously pushed value. */
 ...
 */
__LJMP static int hlua_applet_tcp_getline_yield(lua_State *L, int status,
                                                lua_KContext ctx) {
  struct hlua_appctx *appctx = MAY_LJMP(hlua_checkapplet_tcp(L, 1));
  ...
  if (ret == 0) {
    si_cant_get(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_tcp_getline_yield, TICK_ETERNITY, 0));
  }
 ...
 * data. This string can be empty.
 */
__LJMP static int hlua_applet_tcp_recv_yield(lua_State *L, int status,
                                             lua_KContext ctx) {
  struct hlua_appctx *appctx = MAY_LJMP(hlua_checkapplet_tcp(L, 1));
  struct stream_interface *si = appctx->appctx->owner;
  ...
  if (ret == 0) {
    si_cant_get(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_tcp_recv_yield, TICK_ETERNITY, 0));
  }
...
__LJMP static int hlua_applet_tcp_recv(lua_State *L) {
  ...
  if (lua_gettop(L) > 2)
    WILL_LJMP(luaL_error(
        L, "The 'recv' function requires between 1 and 2 arguments."));
  if (lua_gettop(L) >= 2) {
    len = MAY_LJMP(luaL_checkinteger(L, 2));
    lua_pop(L, 1);
  }
 ...
 * if the channel is closed.
 */
__LJMP static int hlua_applet_tcp_send_yield(lua_State *L, int status,
                                             lua_KContext ctx) {
  size_t len;
  struct hlua_appctx *appctx = MAY_LJMP(hlua_checkapplet_tcp(L, 1));
  const char *str = MAY_LJMP(luaL_checklstring(L, 2, &len));
  ...
  if (l < len) {
    si_rx_room_blk(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_tcp_send_yield, TICK_ETERNITY, 0));
  }
...
static int hlua_applet_http_new(lua_State *L, struct appctx *ctx) {
  ...
  appctx->appctx->ctx.hlua_apphttp.status =
      200; /* Default status code returned. */
  appctx->appctx->ctx.hlua_apphttp.reason =
      NULL; /* Use default reason based on status */
  appctx->htxn.s = s;
  appctx->htxn.p = px;
...
__LJMP static int hlua_applet_http_getline_yield(lua_State *L, int status,
  ...
  if (!stop) {
    si_cant_get(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_http_getline_yield, TICK_ETERNITY, 0));
  }
...
__LJMP static int hlua_applet_http_recv_yield(lua_State *L, int status,
  ...
  if (len) {
    ...
    si_cant_get(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_http_recv_yield, TICK_ETERNITY, 0));
  }
 ...
 * if the channel is closed.
 */
__LJMP static int hlua_applet_http_send_yield(lua_State *L, int status,
                                              lua_KContext ctx) {
  struct hlua_appctx *appctx = MAY_LJMP(hlua_checkapplet_http(L, 1));
  struct stream_interface *si = appctx->appctx->owner;
  ...
  snd_yield:
    ...
    si_rx_room_blk(si);
    MAY_LJMP(
        hlua_yieldk(L, 0, 0, hlua_applet_http_send_yield, TICK_ETERNITY, 0));
  }
...
__LJMP static int hlua_applet_http_send(lua_State *L) {
  ...
  if (!(appctx->appctx->ctx.hlua_apphttp.flags & APPLET_HDR_SENT)) {
    hlua_pusherror(
        L, "Lua: 'send' you must call start_response() before sending data.");
    WILL_LJMP(lua_error(L));
  }
...
__LJMP static int hlua_applet_http_addheader(lua_State *L) {
  ...
  if (ret != LUA_TTABLE) {
    hlua_pusherror(L,
                   "Lua: 'add_header' internal error: AppletHTTP['response'] "
                   "is expected as an array. %s found",
                   lua_typename(L, ret));
    WILL_LJMP(lua_error(L));
  }
  ...
  if (ret == LUA_TNIL) {
    ...
    lua_pop(
        L,
        1); /* remove the nil. The "response" table is the top of the stack. */
    ...
    hlua_pusherror(
        L,
        "Lua: 'add_header' internal error: AppletHTTP['response']['%s'] "
        "is expected as an array. %s found",
        name, lua_typename(L, ret));
    WILL_LJMP(lua_error(L));
  }
...
__LJMP static int hlua_applet_http_send_response(lua_State *L) {
  ...
  status =
      ultoa_r(appctx->appctx->ctx.hlua_apphttp.status, trash.area, trash.size);
  ...
  while (lua_next(L, -2) != 0) {
    ...
    if (lua_type(L, -2) != LUA_TSTRING) {
      hlua_pusherror(L,
                     "Lua applet http '%s': AppletHTTP['response'][] element "
    ...
    if (lua_type(L, -1) != LUA_TTABLE) {
      hlua_pusherror(L,
                     "Lua applet http '%s': AppletHTTP['response']['%s'] "
                     "element must be an table. got %s.\n",
                     appctx->appctx->rule->arg.hlua_rule->fcn->name, name,
    ...
    while (lua_next(L, -2) != 0) {
      ...
      if (lua_type(L, -2) != LUA_TNUMBER) {
        hlua_pusherror(L,
                       "Lua applet http '%s': AppletHTTP['response']['%s'][] "
                       "element must be a number. got %s.\n",
                       appctx->appctx->rule->arg.hlua_rule->fcn->name, name,
                       lua_typename(L, lua_type(L, -2)));
        WILL_LJMP(lua_error(L));
      }
      ...
      if (lua_type(L, -1) != LUA_TSTRING) {
        hlua_pusherror(L,
                       "Lua applet http '%s': AppletHTTP['response']['%s'][%d] "
                       "element must be a string. got %s.\n",
                       appctx->appctx->rule->arg.hlua_rule->fcn->name, name, id,
                       lua_typename(L, lua_type(L, -1)));
        WILL_LJMP(lua_error(L));
      }
      ...
      if (!htx_add_header(htx, ist2(name, nlen), ist2(value, vlen))) {
        hlua_pusherror(L,
                       "Lua applet http '%s': Failed to add header '%s' in the "
                       "response.\n",
                       appctx->appctx->rule->arg.hlua_rule->fcn->name, name);
        WILL_LJMP(lua_error(L));
      }
    next:
      /* Remove the array from the stack, and get next element with a remaining
       * string. */
      lua_pop(L, 1);
    }
  ...
  if ((flags & (HTX_SL_F_VER_11 | HTX_SL_F_XFER_LEN)) == HTX_SL_F_VER_11 &&
    ...
    if (!htx_add_header(htx, ist("transfer-encoding"), ist("chunked"))) {
      hlua_pusherror(L,
                     "Lua applet http '%s': Failed to add header "
                     "'transfer-encoding' in the response.\n",
                     appctx->appctx->rule->arg.hlua_rule->fcn->name);
      WILL_LJMP(lua_error(L));
    }
  }
 ...
 */
__LJMP static int hlua_applet_http_start_response_yield(lua_State *L,
                                                        int status,
                                                        lua_KContext ctx) {
  struct hlua_appctx *appctx = MAY_LJMP(hlua_checkapplet_http(L, 1));
  struct stream_interface *si = appctx->appctx->owner;
  ...
  if (co_data(res)) {
    si_rx_room_blk(si);
    MAY_LJMP(hlua_yieldk(L, 0, 0, hlua_applet_http_start_response_yield,
                         TICK_ETERNITY, 0));
  }
...
__LJMP static int hlua_http_get_headers(lua_State *L, struct http_msg *msg) {
  ...
  for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
    ...
    case LUA_TNIL:
      ...
      lua_rawset(L,
                 -3); /* index new table with header name (pop the values). */
      break;
 ...
 */
__LJMP static inline int hlua_http_rep_hdr(lua_State *L, struct http_msg *msg,
                                           int full) {
  size_t name_len;
  ...
  htx = htxbuf(&msg->chn->buf);
  http_replace_hdrs(chn_strm(msg->chn), htx, ist2(name, name_len), value, re,
                    full);
  regex_free(re);
  return 0;
}
...
__LJMP static inline int hlua_http_add_hdr(lua_State *L, struct http_msg *msg) {
  ...
  lua_pushboolean(
      L, http_add_header(htx, ist2(name, name_len), ist2(value, value_len)));
  return 0;
}
...
static int hlua_http_req_set_meth(lua_State *L) {
  ...
  lua_pushboolean(
      L, http_req_replace_stline(0, name, name_len, htxn->p, htxn->s) != -1);
  return 1;
}
...
static int hlua_http_req_set_path(lua_State *L) {
  ...
  lua_pushboolean(
      L, http_req_replace_stline(1, name, name_len, htxn->p, htxn->s) != -1);
  return 1;
}
...
static int hlua_http_req_set_query(lua_State *L) {
  ...
  lua_pushboolean(L, http_req_replace_stline(2, trash.area, trash.data, htxn->p,
                                             htxn->s) != -1);
  return 1;
}
...
static int hlua_http_req_set_uri(lua_State *L) {
  ...
  lua_pushboolean(
      L, http_req_replace_stline(3, name, name_len, htxn->p, htxn->s) != -1);
  return 1;
}
...
static int hlua_txn_reply_new(lua_State *L) {
  ...
  if (lua_istable(L, 2)) {
    /* load headers from the table argument at index 2. If it is a table, copy
     * it. */
...
__LJMP static int hlua_txn_reply_add_header(lua_State *L) {
  ...
  ret = lua_getfield(L, 1, "headers");
  if (ret != LUA_TTABLE) {
    hlua_pusherror(L, "Reply['headers'] is expected to a an array. %s found",
                   lua_typename(L, ret));
    WILL_LJMP(lua_error(L));
  }
  ...
  if (ret == LUA_TNIL) {
    ...
    lua_pop(
        L,
        1); /* remove the nil. The "headers" table is the top of the stack. */
    ...
    hlua_pusherror(
        L, "Reply['headers']['%s'] is expected to be an array. %s found", name,
        lua_typename(L, ret));
    WILL_LJMP(lua_error(L));
  }
...
__LJMP static int hlua_txn_reply_del_header(lua_State *L) {
  ...
  ret = lua_getfield(L, 1, "headers");
  if (ret != LUA_TTABLE) {
    hlua_pusherror(L, "Reply['headers'] is expected to be an array. %s found",
                   lua_typename(L, ret));
    WILL_LJMP(lua_error(L));
  }
...
__LJMP static int hlua_yield(lua_State *L) {
  MAY_LJMP(
      hlua_yieldk(L, 0, 0, hlua_yield_yield, TICK_ETERNITY, HLUA_CTRLYIELD));
  return 0;
}
 ...
 */
struct task *hlua_process_task(struct task *task, void *context,
 ...
 * resume converters.
 */
static int hlua_sample_conv_wrapper(const struct arg *arg_p, struct sample *smp,
                                    void *private) {
  ...
  if (!stream->hlua) {
    ...
    if (!stream->hlua) {
      SEND_ERR(stream->be,
               "Lua converter '%s': can't initialize Lua context.\n",
               fcn->name);
      return 0;
    }
  ...
  if (!HLUA_IS_RUNNING(stream->hlua)) {
    ...
    /* Restore the function in the stack. */
    lua_rawgeti(stream->hlua->T, LUA_REGISTRYINDEX,
                fcn->function_ref[stream->hlua->state_id]);
  ...
  case HLUA_E_YIELD:
    SEND_ERR(stream->be,
             "Lua converter '%s': yield functions like core.tcp() or "
             "core.sleep() are not allowed.\n",
             fcn->name);
    return 0;
 ...
 */
static int hlua_sample_fetch_wrapper(const struct arg *arg_p,
  ...
  if (!stream->hlua) {
    ...
    if (!stream->hlua) {
      SEND_ERR(stream->be,
               "Lua sample-fetch '%s': can't initialize Lua context.\n",
               fcn->name);
      return 0;
    }
    if (!hlua_ctx_init(stream->hlua, fcn_ref_to_stack_id(fcn), stream->task,
...
static enum act_return hlua_action(struct act_rule *rule, struct proxy *px,
  ...
  if (!s->hlua) {
    ...
    if (!hlua_ctx_init(s->hlua, fcn_ref_to_stack_id(rule->arg.hlua_rule->fcn),
                       s->task, 0)) {
      SEND_ERR(px, "Lua action '%s': can't initialize Lua context.\n",
               rule->arg.hlua_rule->fcn->name);
      goto end;
  ...
  if (!HLUA_IS_RUNNING(s->hlua)) {
    ...
    /* Restore the function in the stack. */
    lua_rawgeti(s->hlua->T, LUA_REGISTRYINDEX,
                rule->arg.hlua_rule->fcn->function_ref[s->hlua->state_id]);
  ...
  case HLUA_E_OK:
    ...
    if (act_ret == ACT_RET_YIELD) {
      ...
      if (dir == SMP_OPT_DIR_REQ)
        s->req.analyse_exp = tick_first(
            (tick_is_expired(s->req.analyse_exp, now_ms) ? 0
                                                         : s->req.analyse_exp),
            s->hlua->wake_time);
      else
        s->res.analyse_exp = tick_first(
            (tick_is_expired(s->res.analyse_exp, now_ms) ? 0
                                                         : s->res.analyse_exp),
            s->hlua->wake_time);
    }
    goto end;
  ...
  err_yield:
    ...
    SEND_ERR(px,
             "Lua function '%s': aborting Lua processing on expired timeout.\n",
             rule->arg.hlua_rule->fcn->name);
    goto end;
...
struct task *hlua_applet_wakeup(struct task *t, void *context,
                                unsigned short state) {
...
static int hlua_applet_tcp_init(struct appctx *ctx, struct proxy *px,
                                struct stream *strm) {
  struct stream_interface *si = ctx->owner;
  struct hlua *hlua;
  struct task *task;
   ...
   * Lua initialization cause 5% performances loss.
   */
  if (!hlua_ctx_init(hlua, fcn_ref_to_stack_id(ctx->rule->arg.hlua_rule->fcn),
                     task, 0)) {
  ...
  /* Restore the function in the stack. */
  lua_rawgeti(hlua->T, LUA_REGISTRYINDEX,
              ctx->rule->arg.hlua_rule->fcn->function_ref[hlua->state_id]);
 ...
 * the applet.
 */
static int hlua_applet_http_init(struct appctx *ctx, struct proxy *px,
                                 struct stream *strm) {
  struct stream_interface *si = ctx->owner;
  struct http_txn *txn;
  struct hlua *hlua;
...
void hlua_applet_http_fct(struct appctx *ctx) {
  ...
  if (!HLUA_IS_RUNNING(hlua) && !(ctx->ctx.hlua_apphttp.flags & APPLET_DONE)) {
    struct htx_blk *blk;
 ...
 * tolerated.
 */
static enum act_parse_ret action_register_lua(const char **args, int *cur_arg,
                                              struct proxy *px,
  ...
  rule->arg.hlua_rule->args =
      calloc(fcn->nargs + 1, sizeof(*rule->arg.hlua_rule->args));
...
action_register_service_http(const char **args, int *cur_arg, struct proxy *px,
...
__LJMP static int hlua_register_action(lua_State *L) {
  ...
  if (lua_gettop(L) < 3 || lua_gettop(L) > 4)
    WILL_LJMP(
        luaL_error(L, "'register_action' needs between 3 and 4 arguments"));
  ...
  if (lua_type(L, 2) != LUA_TTABLE)
    WILL_LJMP(luaL_error(
        L, "register_action: second argument must be a table of strings"));

  /* Third argument : lua function. */
  ref = MAY_LJMP(hlua_checkfunction(L, 3));

  /* Fourth argument : number of mandatory arguments expected on the
   * configuration line. */
  if (lua_gettop(L) >= 4)
  ...
  while (lua_next(L, 2) != 0) {
    ...
    if (strcmp(lua_tostring(L, -1), "tcp-req") == 0)
      ...
      WILL_LJMP(luaL_error(L,
                           "Lua action environment '%s' is unknown. "
...
static enum act_parse_ret
action_register_service_tcp(const char **args, int *cur_arg, struct proxy *px,
                            struct act_rule *rule, char **err) {
  struct hlua_function *fcn = rule->kw->private;
...
__LJMP static int hlua_register_service(lua_State *L) {
  ...
  if (strcmp(env, "tcp") == 0)
    ...
    WILL_LJMP(luaL_error(L,
                         "Lua service environment '%s' is unknown. "
 ...
 */
static int hlua_cli_parse_fct(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  struct hlua *hlua;
  struct hlua_function *fcn;
  int i;
...
__LJMP static int hlua_register_cli(lua_State *L) {
  ...
  while (lua_next(L, 1) != 0) {
    if (index >= CLI_PREFIX_KW_NB)
      WILL_LJMP(luaL_argerror(
          L, 1, "1st argument must be a table with a maximum of 5 entries"));
    if (lua_type(L, -1) != LUA_TSTRING)
      WILL_LJMP(luaL_argerror(
          L, 1, "1st argument must be a table filled with strings"));
...
static int hlua_session_timeout(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  return hlua_read_timeout(args, section_type, curpx, defpx, file, line, err,
                           &hlua_timeout_session);
}
...
static int hlua_applet_timeout(char **args, int section_type,
                               struct proxy *curpx, struct proxy *defpx,
                               const char *file, int line, char **err) {
  return hlua_read_timeout(args, section_type, curpx, defpx, file, line, err,
                           &hlua_timeout_applet);
...
static int hlua_parse_maxmem(char **args, int section_type, struct proxy *curpx,
  ...
  if (*(args[1]) == 0) {
    memprintf(err,
              "'%s' expects an integer argument (Lua memory size in MB).\n",
              args[0]);
    return -1;
  }
  hlua_global_allocator.limit = strtoll(args[1], &error, 10) * 1024L * 1024L;
  if (*error != '\0') {
    memprintf(err, "%s: invalid number %s (error at '%c')", args[0], args[1],
              *error);
    return -1;
  }
  return 0;
 ...
 * and execute an lua file during the parsing of the HAProxy configuration file.
 * It is the main lua entry point.
 *
 * This function runs with the HAProxy keywords API. It returns -1 if an error
 * occurs, otherwise it returns 0.
 *
 * In some error case, LUA set an error message in top of the stack. This
 * function returns this error message in the HAProxy logs and pop it from the
 * stack.
 *
 * This function can fail with an abort() due to an Lua critical error.
 * We are in the configuration parsing process of HAProxy, this abort() is
...
static int hlua_load_per_thread(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  int len;
  ...
  per_thread_load =
      realloc(per_thread_load, (len + 2) * sizeof(*per_thread_load));
  if (per_thread_load == NULL) {
    memprintf(err, "out of memory error");
    return -1;
  }
...
/* Prepend the given <path> followed by a semicolon to the `package.<type>`
 * variable in the given <ctx>.
 */
...
static int hlua_config_prepend_path(char **args, int section_type,
                                    struct proxy *curpx, struct proxy *defpx,
                                    const char *file, int line, char **err) {
  char *path;
  char *type = "path";
  ...
  if (*args[2]) {
    if (strcmp(args[2], "path") != 0 && strcmp(args[2], "cpath") != 0) {
      memprintf(err, "'%s' expects <type> to either be 'path' or 'cpath'",
                args[0]);
      return -1;
    }
    type = args[2];
...
int hlua_post_init() {
  ...
  for (hlua_state_id = 2; hlua_state_id < global.nbthread + 1;
       hlua_state_id++) {
    ...
    for (i = 0; per_thread_load && per_thread_load[i]; i++) {
      ret =
          hlua_load_state(per_thread_load[i], hlua_states[hlua_state_id], &err);
      if (ret != 0) {
        ha_alert("Lua init: %s\n", err);
        return 0;
      }
  ...
  for (hlua_state_id = 1; hlua_state_id < global.nbthread + 1;
       hlua_state_id++) {
  ...
  list_for_each_entry(fcn, &referenced_functions, l) {
    ...
    if ((fcn->function_ref[0] == -1) == (ret < 0)) {
      ha_alert("Lua function '%s' is referenced both ins shared Lua context "
               "(throught lua-load) "
               "and per-thread Lua context (throught lua-load-per-thread). "
               "these two context "
               "exclusive.\n",
               fcn->name);
      errors++;
...
void hlua_init(void) {
  ...
  socket_proxy.timeout.connect =
      5000; /* By default the timeout connection is 5s. */
  ...
  socket_tcp.consecutive_errors_limit =
      socket_proxy.defsrv.consecutive_errors_limit;
  ...
  socket_tcp.check.health =
      socket_tcp.check.rise; /* socket, but will fall down at first failure */
  ...
  socket_tcp.agent.health =
      socket_tcp.agent.rise; /* socket, but will fall down at first failure */
  ...
  socket_ssl.consecutive_errors_limit =
      socket_proxy.defsrv.consecutive_errors_limit;
  ...
  socket_ssl.check.health =
      socket_ssl.check.rise; /* socket, but will fall down at first failure */
  ...
  socket_ssl.agent.health =
      socket_ssl.agent.rise; /* socket, but will fall down at first failure */
  ...
  for (idx = 0; args[idx] != NULL; idx++) {
    if ((kw = srv_find_kw(args[idx])) !=
        NULL) { /* Maybe it's registered server keyword */
      /*
       *
