 ...
 */
int warnif_rule_after_tcp_sess(struct proxy *proxy, const char *file, int line,
                               const char *arg) {
  if (!LIST_ISEMPTY(&proxy->tcp_req.l5_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'tcp-request "
               "session' rule will still be processed before.\n",
               file, line, arg);
    return 1;
  }
  return 0;
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_tcp_cont(struct proxy *proxy, const char *file, int line,
                               const char *arg) {
  if (!LIST_ISEMPTY(&proxy->tcp_req.inspect_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'tcp-request "
               "content' rule will still be processed before.\n",
               file, line, arg);
    return 1;
  }
  return 0;
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_monitor(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
  if (!LIST_ISEMPTY(&proxy->mon_fail_cond)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'monitor fail' "
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_http_req(struct proxy *proxy, const char *file, int line,
                               const char *arg) {
  if (!LIST_ISEMPTY(&proxy->http_req_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after an 'http-request' "
               "rule will still be processed before.\n",
               file, line, arg);
    return 1;
  }
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_redirect(struct proxy *proxy, const char *file, int line,
                               const char *arg) {
  if (!LIST_ISEMPTY(&proxy->redirect_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'redirect' rule "
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_use_backend(struct proxy *proxy, const char *file,
                                  int line, const char *arg) {
  if (!LIST_ISEMPTY(&proxy->switching_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'use_backend' "
               "rule will still be processed before.\n",
               file, line, arg);
    return 1;
  }
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_use_server(struct proxy *proxy, const char *file,
                                 int line, const char *arg) {
  if (!LIST_ISEMPTY(&proxy->server_rules)) {
    ha_warning("parsing [%s:%d] : a '%s' rule placed after a 'use-server' rule "
               "will still be processed before.\n",
               file, line, arg);
    return 1;
  }
...
int warnif_misplaced_redirect(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
...
int warnif_misplaced_http_req(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
...
int warnif_misplaced_monitor(struct proxy *proxy, const char *file, int line,
                             const char *arg) {
...
int warnif_misplaced_tcp_cont(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
...
int warnif_misplaced_tcp_sess(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
...
int warnif_misplaced_tcp_conn(struct proxy *proxy, const char *file, int line,
                              const char *arg) {
...
int cfg_parse_listen(const char *file, int linenum, char **args, int kwm) {
  ...
  if (rc != PR_CAP_NONE) { /* new proxy */
    if (!*args[1]) {
      ha_alert(
          "parsing [%s:%d] : '%s' expects an <id> argument and\n"
    ...
    curproxy = (rc & PR_CAP_FE) ? proxy_fe_by_name(args[1])
                                : proxy_be_by_name(args[1]);
    if (curproxy) {
      ha_alert("Parsing [%s:%d]: %s '%s' has the same name as %s '%s' declared "
               "at %s:%d.\n",
               file, linenum, proxy_cap_str(rc), args[1],
               proxy_type_str(curproxy), curproxy->id, curproxy->conf.file,
               curproxy->conf.line);
      err_code |= ERR_ALERT | ERR_FATAL;
    }
    ...
    if (curproxy) {
      ha_alert("Parsing [%s:%d]: %s '%s' has the same name as log forward "
               "section '%s' declared at %s:%d.\n",
               file, linenum, proxy_cap_str(rc), args[1], curproxy->id,
    ...
    if (alertif_too_many_args(1, file, linenum, args, &err_code)) {
      if (curproxy->cap & PR_CAP_FE)
        ha_alert("parsing [%s:%d] : please use the 'bind' keyword for "
                 "listening addresses.\n",
                 file, linenum);
      goto out;
    }
    ...
    if (curproxy->cap & PR_CAP_BE) {
      ...
      curproxy->tcpcheck_rules.flags =
          (defproxy.tcpcheck_rules.flags & ~TCPCHK_RULES_UNUSED_RS);
      ...
      if (!LIST_ISEMPTY(&defproxy.tcpcheck_rules.preset_vars)) {
        if (!dup_tcpcheck_vars(&curproxy->tcpcheck_rules.preset_vars,
          ...
          ha_alert(
              "parsing [%s:%d] : failed to duplicate tcpcheck preset-vars\n",
              file, linenum);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        }
...
#endif
      curproxy->load_server_state_from_file =
          defproxy.load_server_state_from_file;
    ...
    if (curproxy->cap & PR_CAP_FE) {
      ...
      if (curproxy->conf.logformat_string &&
        ...
        curproxy->conf.logformat_string =
            strdup(curproxy->conf.logformat_string);
      ...
      /* get either a pointer to the logformat string for RFC5424
       * structured-data or a copy of it */
      ...
      if (curproxy->conf.logformat_sd_string &&
        ...
        curproxy->conf.logformat_sd_string =
            strdup(curproxy->conf.logformat_sd_string);
    ...
    curproxy->conf.uniqueid_format_string =
        defproxy.conf.uniqueid_format_string;
    if (curproxy->conf.uniqueid_format_string)
      curproxy->conf.uniqueid_format_string =
          strdup(curproxy->conf.uniqueid_format_string);
    ...
    if (isttest(defproxy.header_unique_id)) {
      ...
      if (!isttest(copy)) {
        ha_alert("parsing [%s:%d] : failed to allocate memory for "
                 "unique-id-header\n",
                 file, linenum);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
    ...
    goto out;
                     ...
                     "defaults")) { /* use this one to assign default values */
  ...
  if (!strcmp(args[0], "server") || !strcmp(args[0], "default-server") ||
    ...
    if (!*(args[1])) {
      ha_alert("parsing [%s:%d] : '%s' expects "
               "{<path>|[addr1]:port1[-end1]}{,[addr]:port[-end]}... as "
               "arguments.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    bind_conf =
        bind_conf_alloc(curproxy, file, linenum, args[1], xprt_get(XPRT_RAW));
    ...
    if (!str2listener(args[1], curproxy, bind_conf, file, linenum, &errmsg)) {
      if (errmsg && *errmsg) {
        ...
        ha_alert("parsing [%s:%d] : '%s' : error encountered while parsing "
                 "listening address '%s'.\n",
    ...
    while (*(args[cur_arg])) {
      ...
      if (kw) {
        ...
        if (!kw->parse) {
          ha_alert("parsing [%s:%d] : '%s %s' : '%s' option is not implemented "
                   "in this version (check build options).\n",
    ...
    goto out;
                     ...
                     "monitor-net")) { /* set the range of IPs to ignore */
    ha_alert("parsing [%s:%d] : 'monitor-net' doesn't exist anymore. Please "
             "use 'http-request return status 200 if { src %s }' instead.\n",
             file, linenum, args[1]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
    ...
    if (!strcmp(args[1], "http"))
      curproxy->mode = PR_MODE_HTTP;
    else if (!strcmp(args[1], "tcp"))
      curproxy->mode = PR_MODE_TCP;
    else if (!strcmp(args[1], "health")) {
      ha_alert("parsing [%s:%d] : 'mode health' doesn't exist anymore. Please "
               "use 'http-request return status 200' instead.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else {
    ...
    if (node) {
      ...
      ha_alert("parsing [%s:%d]: %s %s reuses same custom id as %s %s "
               "(declared at %s:%d).\n",
               file, linenum, proxy_type_str(curproxy), curproxy->id,
               proxy_type_str(target), target->id, target->conf.file,
               target->conf.line);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (alertif_too_many_args(0, file, linenum, args, &err_code))
      goto out;
    curproxy->disabled = 0;
  } else if (!strcmp(args[0], "bind-process")) { /* enable this proxy only on
                                                    some processes */
    int cur_arg = 1;
    unsigned long set = 0;
    ...
    err = invalid_char(args[1]);
    if (err) {
      ha_alert("parsing [%s:%d] : character '%c' is not permitted in acl name "
               "'%s'.\n",
    ...
    if (strcasecmp(args[1], "or") == 0) {
      ha_alert("parsing [%s:%d] : acl name '%s' will never match. 'or' is used "
               "to express a "
    ...
    if (parse_acl((const char **)args + 1, &curproxy->acl, &errmsg,
      ...
      ha_alert(
          "parsing [%s:%d] : error detected while parsing ACL '%s' : %s.\n",
          file, linenum, args[1], errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
  } else if (!strcmp(args[0],
                     "dynamic-cookie-key")) { /* Dynamic cookies secret key */
      ...
      if (!strcmp(args[cur_arg], "rewrite")) {
        ...
        if (!strchr(args[cur_arg + 1], '.')) {
          ...
          ha_warning("parsing [%s:%d]: domain '%s' contains no embedded dot,"
                     " this configuration may not work properly (see "
                     "RFC6265#5.2.3).\n",
        ...
        if (err) {
          ha_alert("parsing [%s:%d]: character '%c' is not permitted in domain "
                   "name '%s'.\n",
        ...
        if (!curproxy->cookie_domain) {
           ...
           */
          char *new_ptr;
          int new_len = strlen(curproxy->cookie_domain) + strlen("; domain=") +
                        strlen(args[cur_arg + 1]) + 1;
          new_ptr = malloc(new_len);
          snprintf(new_ptr, new_len, "%s; domain=%s", curproxy->cookie_domain,
                   args[cur_arg + 1]);
          free(curproxy->cookie_domain);
        ...
        if (!*args[cur_arg + 1]) {
          ha_alert("parsing [%s:%d]: '%s' expects <idletime> in seconds as "
                   "argument.\n",
                   file, linenum, args[cur_arg]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        if (!*args[cur_arg + 1]) {
          ha_alert("parsing [%s:%d]: '%s' expects <lifetime> in seconds as "
                   "argument.\n",
                   file, linenum, args[cur_arg]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        cur_arg++;
                     ...
                     args[cur_arg],
                     "dynamic")) { /* Dynamic persistent cookies secret key */

        if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[cur_arg],
                         NULL))
          err_code |= ERR_WARN;
        ...
        while (*val) {
          if (iscntrl((unsigned char)*val) || *val == ';') {
            ha_alert("parsing [%s:%d]: character '%%x%02X' is not permitted in "
                     "attribute value.\n",
                     file, linenum, *val);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
        ...
        if (!curproxy->cookie_attrs)
          ...
          memprintf(&curproxy->cookie_attrs, "%s; %s", curproxy->cookie_attrs,
                    args[cur_arg + 1]);
        cur_arg++;
      }

      else {
        ha_alert("parsing [%s:%d] : '%s' supports 'rewrite', 'insert', "
                 "'prefix', 'indirect', 'nocache', 'postonly', 'domain', "
                 "'maxidle', 'dynamic', 'maxlife' and 'attr' options.\n",
                 file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
    ...
    if (!POWEROF2(curproxy->ck_opts & (PR_CK_RW | PR_CK_IND))) {
      ha_alert("parsing [%s:%d] : cookie 'rewrite' and 'indirect' modes are "
               "incompatible.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
    }
    ...
    if (!POWEROF2(curproxy->ck_opts & (PR_CK_RW | PR_CK_INS | PR_CK_PFX))) {
      ha_alert("parsing [%s:%d] : cookie 'rewrite', 'insert' and 'prefix' "
               "modes are incompatible.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
    }

    if ((curproxy->ck_opts & (PR_CK_PSV | PR_CK_INS | PR_CK_IND)) ==
        PR_CK_PSV) {
      ha_alert("parsing [%s:%d] : cookie 'preserve' requires at least 'insert' "
               "or 'indirect'.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
    }
    ...
    if (!strncmp(args[1], "rdp-cookie", 10)) {
      ...
      if (*(args[1] + 10) == '(') { /* cookie name */
        ...
        if (!end || end == beg) {
          ha_alert("parsing [%s:%d] : persist rdp-cookie(name)' requires an "
                   "rdp cookie name.\n",
                   file, linenum);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        ha_alert("parsing [%s:%d] : persist rdp-cookie(name)' requires an rdp "
                 "cookie name.\n",
                 file, linenum);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
    ...
    ha_alert(
        "parsing [%s:%d] : '%s' is not supported anymore since HAProxy 1.6.\n",
        file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
    ...
    if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0], NULL))
      err_code |= ERR_WARN;
    if (!strcmp(args[1], "global")) { /* use the file pointed to by global
                                         server-state-file directive */
      curproxy->load_server_state_from_file = PR_SRV_STATE_FILE_GLOBAL;
    } else if (!strcmp(args[1],
                       "local")) { /* use the server-state-file-name variable to
                                      locate the server-state file */
      curproxy->load_server_state_from_file = PR_SRV_STATE_FILE_LOCAL;
    } else if (!strcmp(args[1], "none")) { /* don't use server-state-file
                                              directive for this backend */
      curproxy->load_server_state_from_file = PR_SRV_STATE_FILE_NONE;
    } else {
      ha_alert("parsing [%s:%d] : '%s' expects 'global', 'local' or 'none'. "
               "Got '%s'\n",
               file, linenum, args[0], args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0], NULL))
      err_code |= ERR_WARN;
    if (*(args[1]) == 0) {
      ha_alert("parsing [%s:%d] : '%s' expects 'use-backend-name' or a string. "
               "Got no argument\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!strcmp(args[1], "cookie")) { /* name of a cookie to capture */
      if (curproxy == &defproxy) {
        ha_alert(
            "parsing [%s:%d] : '%s %s' not allowed in 'defaults' section.\n",
            file, linenum, args[0], args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
      ...
      if (*(args[4]) == 0) {
        ha_alert("parsing [%s:%d] : '%s' expects 'cookie' <cookie_name> 'len' "
                 "<len>.\n",
                 file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      if (*(args[3]) == 0 || strcmp(args[4], "len") != 0 || *(args[5]) == 0) {
        ha_alert("parsing [%s:%d] : '%s %s' expects 'header' <header_name> "
                 "'len' <len>.\n",
                 file, linenum, args[0], args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      ha_alert("parsing [%s:%d] : '%s' expects 'cookie' or 'request header' or "
               "'response header'.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (*(args[1]) == 0) {
      ha_alert("parsing [%s:%d] : '%s' expects an integer argument (dispatch "
               "counts for one).\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
  ...
  } else if (!strcmp(args[0], "http-request")) { /* request access control:
                                                    allow/deny/auth */
    ...
    if (!LIST_ISEMPTY(&curproxy->http_req_rules) &&
        ...
        (LIST_PREV(&curproxy->http_req_rules, struct act_rule *, list)->flags &
         ACT_FLAG_FINAL)) {
      ha_warning("parsing [%s:%d]: previous '%s' action is final and has no "
                 "condition attached, further entries are NOOP.\n",
                 file, linenum, args[0]);
      err_code |= ERR_WARN;
    }

    rule =
        parse_http_req_cond((const char **)args + 1, file, linenum, curproxy);
    ...
    err_code |= warnif_cond_conflicts(
        rule->cond,
    ...
    if (!LIST_ISEMPTY(&curproxy->http_res_rules) &&
        ...
        (LIST_PREV(&curproxy->http_res_rules, struct act_rule *, list)->flags &
         ACT_FLAG_FINAL)) {
      ha_warning("parsing [%s:%d]: previous '%s' action is final and has no "
                 "condition attached, further entries are NOOP.\n",
                 file, linenum, args[0]);
    ...
    rule =
        parse_http_res_cond((const char **)args + 1, file, linenum, curproxy);
    ...
    if (!LIST_ISEMPTY(&curproxy->http_after_res_rules) &&
        !LIST_PREV(&curproxy->http_after_res_rules, struct act_rule *, list)
             ->cond &&
        (LIST_PREV(&curproxy->http_after_res_rules, struct act_rule *, list)
             ->flags &
         ACT_FLAG_FINAL)) {
    ...
    rule = parse_http_after_res_cond((const char **)args + 1, file, linenum,
                                     curproxy);
  ...
  } else if (!strcmp(args[0], "http-send-name-header")) { /* send server name in
                                                             request header */
    ...
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. Use 'http-request deny' which uses the exact "
             "same syntax.\n",
             file, linenum, args[0]);
    ...
    if ((rule = http_parse_redirect_rule(file, linenum, curproxy,
                                         (const char **)args + 1, &errmsg, 0,
                                         0)) == NULL) {
      ha_alert("parsing [%s:%d] : error detected in %s '%s' while parsing "
               "redirect rule : %s.\n",
    ...
    if (strcmp(args[2], "if") == 0 || strcmp(args[2], "unless") == 0) {
      if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                                 (const char **)args + 2, &errmsg)) == NULL) {
        ha_alert("parsing [%s:%d] : error detected while parsing switching "
                 "rule : %s.\n",
                 file, linenum, errmsg);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      err_code |=
          warnif_cond_conflicts(cond, SMP_VAL_FE_SET_BCK, file, linenum);
    } else if (*args[2]) {
      ha_alert("parsing [%s:%d] : unexpected keyword '%s' after switching "
               "rule, only 'if' and 'unless' are allowed.\n",
               file, linenum, args[2]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (strcmp(args[2], "if") != 0 && strcmp(args[2], "unless") != 0) {
      ha_alert("parsing [%s:%d] : '%s' requires either 'if' or 'unless' "
    ...
    if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                               (const char **)args + 2, &errmsg)) == NULL) {
      ha_alert("parsing [%s:%d] : error detected while parsing switching rule "
               ": %s.\n",
               file, linenum, errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                               (const char **)args + 1, &errmsg)) == NULL) {
      ha_alert(
          "parsing [%s:%d] : error detected while parsing a '%s' rule : %s.\n",
          file, linenum, args[0], errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (curproxy == &defproxy) {
      ha_alert("parsing [%s:%d] : 'stick-table' is not supported in 'defaults' "
               "section.\n",
               file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    expr = sample_parse_expr(args, &myidx, file, linenum, &errmsg,
    ...
    if (flags & STK_ON_RSP) {
      if (!(expr->fetch->val & SMP_VAL_BE_STO_RUL)) {
        ha_alert(
            "parsing [%s:%d] : '%s': fetch method '%s' extracts information "
            "from '%s', none of which is available for 'store-response'.\n",
      ...
      if (!(expr->fetch->val & SMP_VAL_BE_SET_SRV)) {
        ha_alert(
            "parsing [%s:%d] : '%s': fetch method '%s' extracts information "
            "from '%s', none of which is available during request.\n",
            file, linenum, args[0], expr->fetch->kw,
            sample_src_names(expr->fetch->use));
        err_code |= ERR_ALERT | ERR_FATAL;
        free(expr);
    ...
    if (strcmp(args[myidx], "if") == 0 || strcmp(args[myidx], "unless") == 0) {
      if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                                 (const char **)args + myidx, &errmsg)) ==
          NULL) {
        ha_alert("parsing [%s:%d] : '%s': error detected while parsing "
                 "sticking condition : %s.\n",
                 file, linenum, args[0], errmsg);
        err_code |= ERR_ALERT | ERR_FATAL;
        free(expr);
        goto out;
    ...
    if (flags & STK_ON_RSP)
      err_code |=
          warnif_cond_conflicts(cond, SMP_VAL_BE_STO_RUL, file, linenum);
    else
      err_code |=
          warnif_cond_conflicts(cond, SMP_VAL_BE_SET_SRV, file, linenum);
    ...
    if (!*args[1]) {
      ...
      if (strcmp(args[2], "if") != 0 && strcmp(args[2], "unless") != 0) {
        ha_alert("parsing [%s:%d] : '%s %s' requires either 'if' or 'unless' "
                 "followed by a condition.\n",
                 file, linenum, args[0], args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                                 (const char **)args + 2, &errmsg)) == NULL) {
        ha_alert("parsing [%s:%d] : error detected while parsing a '%s %s' "
                 "rule : %s.\n",
      ...
      if (err == PARSE_TIME_OVER) {
        ha_alert(
            "parsing [%s:%d]: timer overflow in argument <%s> to stats refresh "
            "interval, maximum value is 2147483647 s (~68 years).\n",
            file, linenum, args[2]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      } else if (err == PARSE_TIME_UNDER) {
        ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to stats "
                 "refresh interval, minimum non-null value is 1 s.\n",
                 file, linenum, args[2]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      } else if (err) {
    ...
    } else if (!strcmp(args[1], "http-request")) { /* request access control:
                                                      allow/deny/auth */
      struct act_rule *rule;
      ...
      if (!LIST_ISEMPTY(&curproxy->uri_auth->http_req_rules) &&
          !LIST_PREV(&curproxy->uri_auth->http_req_rules, struct act_rule *,
                     list)
               ->cond) {
        ha_warning("parsing [%s:%d]: previous '%s' action has no condition "
                   "attached, further entries are NOOP.\n",
                   file, linenum, args[0]);
        err_code |= ERR_WARN;
      }

      rule =
          parse_http_req_cond((const char **)args + 2, file, linenum, curproxy);
      ...
      if (*args[2]) {
        ...
        if (!i || args[2][i]) {
          ha_alert(
              "parsing [%s:%d]: '%s %s' invalid node name - should be a string"
              "with digits(0-9), letters(A-Z, a-z), hyphen(-) or "
              "underscode(_).\n",
              file, linenum, args[0], args[1]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
      ...
      if (!*args[2] && !global.desc)
        ha_warning("parsing [%s:%d]: '%s' requires a parameter or 'desc' to be "
    ...
    stats_error_parsing:
      ha_alert("parsing [%s:%d]: %s '%s', expects 'admin', 'uri', 'realm', "
               "'auth', 'scope', 'enable', 'hide-version', 'show-node', "
    ...
    for (optnum = 0; cfg_opts[optnum].name; optnum++) {
      if (!strcmp(args[1], cfg_opts[optnum].name)) {
        if (cfg_opts[optnum].cap == PR_CAP_NONE) {
          ha_alert("parsing [%s:%d]: option '%s' is not supported due to build "
                   "options.\n",
        ...
        if (warnifnotcap(curproxy, cfg_opts[optnum].cap, file, linenum, args[1],
                         NULL)) {
          err_code |= ERR_WARN;
          goto out;
        }
    ...
    for (optnum = 0; cfg_opts2[optnum].name; optnum++) {
      if (!strcmp(args[1], cfg_opts2[optnum].name)) {
        ...
        if (alertif_too_many_args_idx(0, 1, file, linenum, args, &err_code))
          goto out;
        if (warnifnotcap(curproxy, cfg_opts2[optnum].cap, file, linenum,
                         args[1], NULL)) {
          err_code |= ERR_WARN;
          goto out;
        }
    ...
    if (strcmp(args[1], "httpclose") == 0 ||
        strcmp(args[1], "forceclose") == 0) {
      if (strcmp(args[1], "forceclose") == 0) {
        if (!already_warned(WARN_FORCECLOSE_DEPRECATED))
          ha_warning(
              "parsing [%s:%d]: keyword '%s' is deprecated in favor of "
              "'httpclose', and will not be supported by future versions.\n",
              file, linenum, args[1]);
        err_code |= ERR_WARN;
      }
      if (alertif_too_many_args_idx(0, 1, file, linenum, args, &err_code))
      ...
      ha_warning("parsing [%s:%d]: the option '%s' is deprecated and will be "
    ...
    if (kwm != KWM_STD) {
      ha_alert("parsing [%s:%d]: negation/default is not supported for option "
               "'%s'.\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!strcmp(args[1], "httplog")) {
      ...
      if (curproxy->conf.logformat_string && curproxy == &defproxy) {
        ...
        if (logformat == clf_http_log_format)
          clflogformat = " clf";
        ha_warning("parsing [%s:%d]: 'option httplog%s' overrides previous "
                   "'%s' in 'defaults' section.\n",
      ...
      if (curproxy != &defproxy && !(curproxy->cap & PR_CAP_FE)) {
        ha_warning("parsing [%s:%d] : backend '%s' : 'option httplog' "
                   "directive is ignored in backends.\n",
                   file, linenum, curproxy->id);
        err_code |= ERR_WARN;
      }
      ...
      if (curproxy->conf.logformat_string && curproxy == &defproxy) {
        ...
        if (curproxy->conf.logformat_string == default_http_log_format)
          ...
          oldlogformat = "option httplog clf";
        ha_warning("parsing [%s:%d]: 'option tcplog' overrides previous '%s' "
                   "in 'defaults' section.\n",
      ...
      if (curproxy != &defproxy && !(curproxy->cap & PR_CAP_FE)) {
        ha_warning("parsing [%s:%d] : backend '%s' : 'option tcplog' directive "
      ...
      if (warnifnotcap(curproxy, PR_CAP_BE | PR_CAP_FE, file, linenum, args[1],
                       NULL))
        err_code |= ERR_WARN;
      ...
      err_code |=
          proxy_parse_httpchk_opt(args, 0, curproxy, &defproxy, file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "ssl-hello-chk")) {
      err_code |= proxy_parse_ssl_hello_chk_opt(args, 0, curproxy, &defproxy,
                                                file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "smtpchk")) {
      err_code |=
          proxy_parse_smtpchk_opt(args, 0, curproxy, &defproxy, file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "pgsql-check")) {
      err_code |= proxy_parse_pgsql_check_opt(args, 0, curproxy, &defproxy,
                                              file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "redis-check")) {
      err_code |= proxy_parse_redis_check_opt(args, 0, curproxy, &defproxy,
                                              file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "mysql-check")) {
      err_code |= proxy_parse_mysql_check_opt(args, 0, curproxy, &defproxy,
                                              file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "ldap-check")) {
      err_code |= proxy_parse_ldap_check_opt(args, 0, curproxy, &defproxy, file,
                                             linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "spop-check")) {
      err_code |= proxy_parse_spop_check_opt(args, 0, curproxy, &defproxy, file,
                                             linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "tcp-check")) {
      err_code |= proxy_parse_tcp_check_opt(args, 0, curproxy, &defproxy, file,
                                            linenum);
      if (err_code & ERR_FATAL)
        goto out;
    } else if (!strcmp(args[1], "external-check")) {
      err_code |= proxy_parse_external_check_opt(args, 0, curproxy, &defproxy,
                                                 file, linenum);
      if (err_code & ERR_FATAL)
        goto out;
      ...
      /* insert x-forwarded-for field, but not for the IP address listed as an
       * except. set default options (ie: bitfield, header name, etc)
       */
      ...
      /* loop to go through arguments - start at 2, since 0+1 = "option"
       * "forwardfor" */
      cur_arg = 2;
      while (*(args[cur_arg])) {
        if (!strcmp(args[cur_arg], "except")) {
          ...
          if (!*(args[cur_arg + 1]) ||
              !str2net(args[cur_arg + 1], 1, &curproxy->except_net,
                       &curproxy->except_mask)) {
            ha_alert("parsing [%s:%d] : '%s %s %s' expects <address>[/mask] as "
                     "argument.\n",
                     file, linenum, args[0], args[1], args[cur_arg]);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
          ...
          if (*(args[cur_arg + 1]) == 0) {
            ha_alert("parsing [%s:%d] : '%s %s %s' expects <header_name> as "
                     "argument.\n",
                     file, linenum, args[0], args[1], args[cur_arg]);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
      ...
      /* insert x-original-to field, but not for the IP address listed as an
       * except. set default options (ie: bitfield, header name, etc)
       */
      ...
      while (*(args[cur_arg])) {
        if (!strcmp(args[cur_arg], "except")) {
          /* suboption except - needs additional argument for it */
          if (!*(args[cur_arg + 1]) ||
              !str2net(args[cur_arg + 1], 1, &curproxy->except_to,
                       &curproxy->except_mask_to)) {
            ha_alert("parsing [%s:%d] : '%s %s %s' expects <address>[/mask] as "
                     "argument.\n",
                     file, linenum, args[0], args[1], args[cur_arg]);
            err_code |= ERR_ALERT | ERR_FATAL;
    ...
    ha_alert("parsing [%s:%d] : keyword '%s' directive is not supported "
             "anymore since HAProxy 2.1. Use 'option redispatch'.\n",
             file, linenum, args[0]);
    ...
    if (strcmp(args[1], "never") == 0) {
      ...
      ha_alert("parsing [%s:%d] : '%s' only supports 'never', 'safe', "
               "'aggressive', 'always'.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (strcmp(args[1], "fail") == 0) {
      ...
      err_code |=
          warnif_misplaced_monitor(curproxy, file, linenum, "monitor fail");
      if ((cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                                 (const char **)args + 2, &errmsg)) == NULL) {
        ha_alert("parsing [%s:%d] : error detected while parsing a '%s %s' "
                 "condition : %s.\n",
    ...
    if (warnifnotcap(curproxy, PR_CAP_FE, file, linenum, args[0],
                     " Maybe you want 'fullconn' instead ?"))
      err_code |= ERR_WARN;
    ...
    if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0],
                     " Maybe you want 'maxconn' instead ?"))
      err_code |= ERR_WARN;
    ...
    if (err == PARSE_TIME_OVER) {
      ha_alert("parsing [%s:%d]: timer overflow in argument <%s> to grace "
               "time, maximum value is 2147483647 ms (~24.8 days).\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else if (err == PARSE_TIME_UNDER) {
      ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to grace "
               "time, minimum non-null value is 1 ms.\n",
               file, linenum, args[1]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else if (err) {
    ...
    ha_warning("parsing [%s:%d]: the '%s' is deprecated and will be removed in "
               "a future version.\n",
               file, linenum, args[0]);
    ...
    sk = str2sa_range(args[1], NULL, &port1, &port2, NULL, NULL, &errmsg, NULL,
                      NULL,
    ...
    curproxy->options |= PR_O_DISPATCH;
                     ...
                     "balance")) { /* set balancing with optional algorithm */
    if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0], NULL))
      err_code |= ERR_WARN;
     ...
     *
     * The default hash function is sdbm for map-based and sdbm+avalanche for
     * consistent.
     */
    curproxy->lbprm.algo &=
        ~(BE_LB_HASH_TYPE | BE_LB_HASH_FUNC | BE_LB_HASH_MOD);
    ...
    if (strcmp(args[1], "consistent") == 0) { /* use consistent hashing */
      ...
      ha_alert(
          "parsing [%s:%d] : experimental feature '%s %s' is not supported "
          "anymore, please use '%s map-based sdbm avalanche' instead.\n",
          file, linenum, args[0], args[1], args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    } else {
      ha_alert("parsing [%s:%d] : '%s' only supports 'consistent' and "
               "'map-based'.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (!*args[2]) {
      ...
      if (!strcmp(args[2], "sdbm")) {
        ...
        ha_alert("parsing [%s:%d] : '%s' only supports 'sdbm', 'djb2', "
                 "'crc32', or 'wt6' hash functions.\n",
                 file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
      ...
      if (!strcmp(args[3], "avalanche")) {
        ...
        ha_alert("parsing [%s:%d] : '%s' only supports 'avalanche' as a "
                 "modifier for hash functions.\n",
                 file, linenum, args[0]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
    ...
    if (curproxy->lbprm.hash_balance_factor != 0 &&
        curproxy->lbprm.hash_balance_factor <= 100) {
    ...
    if (*(args[2])) {
      ha_alert("parsing [%s:%d] : %s expects only one argument, don't forget "
               "to escape spaces!\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    ...
    if (copy == NULL) {
      ha_alert(
          "parsing [%s:%d] : failed to allocate memory for unique-id-header\n",
          file, linenum);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
    ...
    if (curproxy->conf.logformat_string && curproxy == &defproxy) {
      ...
      if (curproxy->conf.logformat_string == default_http_log_format)
        ...
        oldlogformat = "option httplog clf";
      ha_warning("parsing [%s:%d]: 'log-format' overrides previous '%s' in "
                 "'defaults' section.\n",
                 file, linenum, oldlogformat);
    }
    if (curproxy->conf.logformat_string != default_http_log_format &&
     ...
     */
    if (curproxy != &defproxy && !(curproxy->cap & PR_CAP_FE)) {
      ha_warning("parsing [%s:%d] : backend '%s' : 'log-format' directive is "
                 "ignored in backends.\n",
                 file, linenum, curproxy->id);
      err_code |= ERR_WARN;
    }
     ...
     */
    if (curproxy != &defproxy && !(curproxy->cap & PR_CAP_FE)) {
      ha_warning("parsing [%s:%d] : backend '%s' : 'log-format-sd' directive "
                 "is ignored in backends.\n",
                 file, linenum, curproxy->id);
      err_code |= ERR_WARN;
    }
    ...
    chunk_initlen(&curproxy->log_tag, strdup(args[1]), strlen(args[1]),
                  strlen(args[1]));
    ...
    if (!parse_logsrv(args, &curproxy->logsrvs, (kwm == KWM_NO), &errmsg)) {
      ...
      goto out;
                     ...
                     "source")) { /* address to which we bind when connecting */
    int cur_arg;
    int port1, port2;
    struct sockaddr_storage *sk;
    ...
    if (!*args[1]) {
      ha_alert("parsing [%s:%d] : '%s' expects <addr>[:<port>], and optionally "
               "'%s' <addr>, and '%s' <name>.\n",
    ...
    while (*(args[cur_arg])) {
      if (!strcmp(args[cur_arg], "usesrc")) { /* address to use outside */
#if defined(CONFIG_HAP_TRANSPARENT)
        if (!*args[cur_arg + 1]) {
          ha_alert("parsing [%s:%d] : '%s' expects <addr>[:<port>], 'client', "
                   "or 'clientip' as argument.\n",
                   file, linenum, "usesrc");
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        }
        ...
        if (!strcmp(args[cur_arg + 1], "client")) {
          ...
          if (curproxy->conn_src.bind_hdr_occ < -MAX_HDR_HISTORY) {
            ha_alert("parsing [%s:%d] : usesrc hdr_ip(name,num) does not "
                     "support negative"
        ...
        ha_alert("parsing [%s:%d] : '%s' not allowed here because support for "
                 "TPROXY was not compiled in.\n",
                 file, linenum, "usesrc");
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
#endif
      ...
      ha_alert("parsing [%s:%d] : '%s' only supports optional keywords '%s' "
               "and '%s'.\n",
      ...
      goto out;
                 ...
                 args[0],
                 "usesrc")) { /* address to use outside: needs "source" first */
    ...
    goto out;
             ...
             !strcmp(args[0],
                     "reqrep")) { /* replace request header from a regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-request replace-path', 'http-request replace-uri' or "
             "'http-request replace-header' instead.\n",
             file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0],
                     "reqdel")) { /* delete request header from a regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqdeny")) { /* deny a request if a header
                                               matches this regex */
    ha_alert(
        "parsing [%s:%d] : The '%s' not supported anymore since HAProxy 2.1. "
        "Use 'http-request deny' instead.\n",
        file, linenum, args[0]);
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqpass")) { /* pass this header without allowing
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqallow")) { /* allow a request if a header
                                                matches this regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqtarpit")) { /* tarpit a request if a header
                                                 matches this regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqirep")) { /* replace request header from a
                                               regex, ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "reqidel")) { /* delete request header from a
                                               regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-request del-header' instead.\n",
    ...
    goto out;
                     ...
                     "reqideny")) { /* deny a request if a header matches this
                                       regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-request deny' instead.\n",
    ...
    goto out;
                     ...
                     "reqipass")) { /* pass this header without allowing or
                                       denying the request */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1.\n",
             file, linenum, args[0]);
    ...
    goto out;
                     ...
                     "reqiallow")) { /* allow a request if a header matches this
                                        regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-request allow' instead.\n",
    ...
    goto out;
                     ...
                     "reqitarpit")) { /* tarpit a request if a header matches
                                         this regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-request tarpit' instead.\n",
             file, linenum, args[0]);
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "srvexp") ||
             !strcmp(args[0],
                     "rsprep")) { /* replace response header from a regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    goto out;
                     ...
                     "rspdel")) { /* delete response header from a regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    goto out;
                     ...
                     "rspdeny")) { /* block response header from a regex */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "rspirep")) { /* replace response header from a
                                               regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-response replace-header' instead.\n",
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "rspidel")) { /* delete response header from a
                                               regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
    ...
    err_code |= ERR_ALERT | ERR_FATAL;
    goto out;
  } else if (!strcmp(args[0], "rspideny")) { /* block response header from a
                                                regex ignoring case */
    ha_alert("parsing [%s:%d] : The '%s' directive is not supported anymore "
             "since HAProxy 2.1. "
             "Use 'http-response deny' instead.\n",
    ...
    list_for_each_entry(kwl, &cfg_keywords.list, list) {
      for (index = 0; kwl->kw[index].kw != NULL; index++) {
        ...
        if (strcmp(kwl->kw[index].kw, args[0]) == 0) {
          ...
          rc = kwl->kw[index].parse(args, CFG_LISTEN, curproxy, &defproxy, file,
                                    linenum, &errmsg);
          if (rc < 0) {
