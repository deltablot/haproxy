...
/******************************/
#define PEERS_F_RESYNC_LOCAL                                                   \
  0x00000001 /* Learn from local finished or no more needed */
#define PEERS_F_RESYNC_REMOTE                                                  \
  0x00000002 /* Learn from remote finished or no more needed */
#define PEERS_F_RESYNC_ASSIGN                                                  \
  0x00000004 /* A peer was assigned to learn our lesson */
#define PEERS_F_RESYNC_PROCESS                                                 \
  0x00000008 /* The assigned peer was requested for resync */
#define PEERS_F_DONOTSTOP                                                      \
  0x00010000 /* Main table sync task block process during soft stop            \
...
#define PEER_F_TEACH_FINISHED                                                  \
  0x00000008 /* Teach conclude, (wait for confirm) */
#define PEER_F_TEACH_COMPLETE                                                  \
  0x00000010 /* All that we know already taught to current peer, used only for \
                a local peer */
#define PEER_F_LEARN_ASSIGN                                                    \
  0x00000100 /* Current peer was assigned for a lesson */
#define PEER_F_LEARN_NOTUP2DATE                                                \
  0x00000200 /* Learn from peer finished but peer is not up to date */
#define PEER_F_ALIVE 0x20000000     /* Used to flag a peer a alive. */
#define PEER_F_HEARTBEAT 0x40000000 /* Heartbeat message to send. */
#define PEER_F_DWNGRD                                                          \
  0x80000000 /* When this flag is enabled, we must downgrade the supported     \
                version announced during peer sessions. */

#define PEER_TEACH_RESET                                                       \
  ~(PEER_F_TEACH_PROCESS |                                                     \
    PEER_F_TEACH_FINISHED) /* PEER_F_TEACH_COMPLETE should never be reset */
...
static int peer_net_key_type[SMP_TYPES] = {
    [SMP_T_SINT] = PEER_KT_SINT, [SMP_T_IPV4] = PEER_KT_IPV4,
    [SMP_T_IPV6] = PEER_KT_IPV6, [SMP_T_STR] = PEER_KT_STR,
...
static int peer_int_key_type[PEER_KT_TYPES] = {
    [PEER_KT_SINT] = SMP_T_SINT, [PEER_KT_IPV4] = SMP_T_IPV4,
    [PEER_KT_IPV6] = SMP_T_IPV6, [PEER_KT_STR] = SMP_T_STR,
...
/* 3 bytes */
#define PEER_ENC_3BYTES_MIN                                                    \
  ((1ULL << 11) | PEER_ENC_2BYTES_MIN) /*              0x8f0 (or 2288) */
/* 4 bytes */
#define PEER_ENC_4BYTES_MIN                                                    \
  ((1ULL << 18) | PEER_ENC_3BYTES_MIN) /*            0x408f0 (or 264432) */
/* 5 bytes */
#define PEER_ENC_5BYTES_MIN                                                    \
  ((1ULL << 25) | PEER_ENC_4BYTES_MIN) /*          0x20408f0 (or 33818864) */
/* 6 bytes */
#define PEER_ENC_6BYTES_MIN                                                    \
  ((1ULL << 32) | PEER_ENC_5BYTES_MIN) /*        0x1020408f0 (or 4328786160)   \
                                        */
/* 7 bytes */
#define PEER_ENC_7BYTES_MIN                                                    \
  ((1ULL << 39) |                                                              \
   PEER_ENC_6BYTES_MIN) /*       0x81020408f0 (or 554084600048) */
/* 8 bytes */
#define PEER_ENC_8BYTES_MIN                                                    \
  ((1ULL << 46) |                                                              \
   PEER_ENC_7BYTES_MIN) /*     0x4081020408f0 (or 70922828777712) */
/* 9 bytes */
#define PEER_ENC_9BYTES_MIN                                                    \
  ((1ULL << 53) |                                                              \
   PEER_ENC_8BYTES_MIN) /*   0x204081020408f0 (or 9078122083518704) */
/* 10 bytes */
#define PEER_ENC_10BYTES_MIN                                                   \
  ((1ULL << 60) |                                                              \
   PEER_ENC_9BYTES_MIN) /* 0x10204081020408f0 (or 1161999626690365680) */
...
enum {
  PEER_SESS_ST_ACCEPT =
      0, /* Initial state for session create by an accept, must be zero! */
  ...
  PEER_SESS_ST_SENDSUCCESS, /* Send ret code 200 (success) and wait for message
                             */
  PEER_SESS_ST_CONNECT,   /* Initial state for session create on a connect, push
                             presentation into buffer */
...
#define PEERS_EV_UPDTMSG (1 << 0)
    {.mask = PEERS_EV_UPDTMSG,
     .name = "updtmsg",
     .desc = "update message received"},
};
...
struct trace_source trace_peers = {
    ...
    .desc = "Peers protocol",
    .arg_def =
        TRC_ARG1_CONN, /* TRACE()'s first argument is always a connection */
 ...
 * Return the number of written bytes written to build this messages if
 * succeeded, 0 if not.
 */
static int peer_prepare_hellomsg(char *msg, size_t size,
  ...
  peer = p->hello.peer;
  min_ver =
      (peer->flags & PEER_F_DWNGRD) ? PEER_DWNGRD_MINOR_VER : PEER_MINOR_VER;
  /* Prepare headers */
  ret = snprintf(msg, size, PEER_SESSION_PROTO_NAME " %u.%u\n%s\n%s %d %d\n",
                 PEER_MAJOR_VER, min_ver, peer->id, localpeer, (int)getpid(),
                 relative_pid);
  if (ret >= size)
    return 0;
 ...
 * succeeded, 0 if not.
 */
static int peer_prepare_status_successmsg(char *msg, size_t size,
                                          struct peer_prep_params *p) {
  int ret;
 ...
 * succeeded, 0 if not.
 */
static int peer_prepare_status_errormsg(char *msg, size_t size,
                                        struct peer_prep_params *p) {
  int ret;
  unsigned int st1;
 ...
 * Always successful.
 */
static inline void peer_set_update_msg_type(char *msg_type, int use_identifier,
                                            int use_timed) {
  if (use_timed) {
    if (use_identifier)
...
/*
 * This prepare the data update message on the stick session <ts>, <st> is the
 * considered stick table. <msg> is a buffer of <size> to receive data message
 * content If function returns 0, the caller should consider we were unable to
 * encode this message (TODO: check size)
 */
static int peer_prepare_updatemsg(char *msg, size_t size,
                                  struct peer_prep_params *p) {
  uint32_t netinteger;
  ...
  if (!st->last_pushed || updateid < st->last_pushed ||
      ((updateid - st->last_pushed) != 1)) {
    use_identifier = 1;
  }
 ...
 * If function returns 0, the caller should consider we were unable to encode
 * this message (TODO: check size)
 */
static int peer_prepare_switchmsg(char *msg, size_t size,
...
/*
 * This prepare the acknowledge message on the stick session <ts>, <st> is the
 * considered stick table. <msg> is a buffer of <size> to receive data message
 * content If function returns 0, the caller should consider we were unable to
 * encode this message (TODO: check size)
 */
static int peer_prepare_ackmsg(char *msg, size_t size,
                               struct peer_prep_params *p) {
  unsigned short datalen;
  char *cursor, *datamsg;
  uint32_t netinteger;
 ...
 */
static int peer_get_version(const char *str, unsigned int *maj_ver,
...
/*
 * Parse a line terminated by an optional '\r' character, followed by a
 * mandatory
 * '\n' character.
 ...
 * Return 0 if the message could not be built modifying the appcxt st0 to
 * PEER_SESS_ST_END value. Returns -1 if there was not enough room left to send
 * the message, any other negative returned value must  be considered as an
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_msg(
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_switchmsg(struct shared_table *st,
                                      struct appctx *appctx) {
  struct peer_prep_params p = {
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_ackmsg(struct shared_table *st,
                                   struct appctx *appctx) {
  struct peer_prep_params p = {
      .ack.shared_table = st,
  };
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_updatemsg(struct shared_table *st,
                                      struct appctx *appctx, struct stksess *ts,
                                      unsigned int updateid, int use_identifier,
                                      int use_timed) {
  struct peer_prep_params p = {
      .updt =
          {
 ...
 * 0 if not.
 */
static int peer_prepare_control_msg(char *msg, size_t size,
                                    struct peer_prep_params *p) {
 ...
 * PEER_SESS_ST_END value. Returns -1 if there was not enough room left to send
 * the message, any other negative returned value must  be considered as an
 * error with an appctx st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_resync_reqmsg(struct appctx *appctx) {
  struct peer_prep_params p = {
      .control.head =
          {
              PEER_MSG_CLASS_CONTROL,
              PEER_MSG_CTRL_RESYNCREQ,
          },
  };
...
static inline int peer_send_resync_finishedmsg(struct appctx *appctx,
  ...
  p.control.head[1] =
      (peers->flags & PEERS_RESYNC_STATEMASK) == PEERS_RESYNC_FINISHED
          ? PEER_MSG_CTRL_RESYNCFINISHED
          : PEER_MSG_CTRL_RESYNCPARTIAL;
 ...
 * Return 0 if the message could not be built modifying the appctx st0 to
 * PEER_SESS_ST_END value. Returns -1 if there was not enough room left to send
 * the message, any other negative returned value must  be considered as an
 * error with an appctx st0 returned value equal to PEER_SESS_ST_END.
 */
 ...
 * 0 if not.
 */
static int peer_prepare_error_msg(char *msg, size_t size,
                                  struct peer_prep_params *p) {
 ...
 * <st> shared stick-table when a lesson must be taught a peer
 * (PEER_F_LEARN_ASSIGN flag set).
 */
static inline struct stksess *
peer_teach_process_stksess_lookup(struct shared_table *st) {
  struct eb32_node *eb;
 ...
 */
static inline struct stksess *
peer_teach_stage1_stksess_lookup(struct shared_table *st) {
  struct eb32_node *eb;
 ...
 */
static inline struct stksess *
peer_teach_stage2_stksess_lookup(struct shared_table *st) {
  struct eb32_node *eb;
...
/*
 * Generic function to emit update messages for <st> stick-table when a lesson
 * must be taught to the peer <p>. <locked> must be set to 1 if the shared table
 * <st> is already locked when entering this function, 0 if not.
 *
 * This function temporary unlock/lock <st> when it sends stick-table updates or
 * when decrementing its refcount in case of any error when it sends this
 * updates.
 *
 * Return 0 if any message could not be built modifying the appcxt st0 to
 * PEER_SESS_ST_END value. Returns -1 if there was not enough room left to send
 * the message, any other negative returned value must  be considered as an
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END. If it
 * returns 0 or -1, this function leave <st> locked if already locked when
 * entering this function unlocked if not already locked when entering this
 * function.
 */
static inline int peer_send_teachmsgs(
    struct appctx *appctx, struct peer *p,
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_teach_process_msgs(struct appctx *appctx,
                                               struct peer *p,
                                               struct shared_table *st) {
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_teach_stage1_msgs(struct appctx *appctx,
                                              struct peer *p,
                                              struct shared_table *st) {
 ...
 * error with an appcxt st0 returned value equal to PEER_SESS_ST_END.
 */
static inline int peer_send_teach_stage2_msgs(struct appctx *appctx,
                                              struct peer *p,
                                              struct shared_table *st) {
...
/*
 * Function used to parse a stick-table update message after it has been
 * received by <p> peer with <msg_cur> as address of the pointer to the position
 * in the receipt buffer with <msg_end> being position of the end of the
 * stick-table message. Update <msg_curr> accordingly to the peer protocol specs
 * if no peer protocol error was encountered. <exp> must be set if the
 * stick-table entry expires. <updt> must be set for  PEER_MSG_STKT_UPDATE or
 * PEER_MSG_STKT_UPDATE_TIMED stick-table messages, in this case the stick-table
 * update message is received with a stick-table update ID. <totl> is the length
 * of the stick-table update message computed upon receipt.
 */
static int peer_treat_updatemsg(struct appctx *appctx, struct peer *p, int updt,
                                int exp, char **msg_cur, char *msg_end,
                                int msg_len, int totl) {
  struct stream_interface *si = appctx->owner;
  ...
  if (exp) {
    ...
    if (*msg_cur + expire_sz > msg_end) {
      TRACE_PROTO("malformed message", PEERS_EV_UPDTMSG, NULL, p, *msg_cur);
      TRACE_PROTO("malformed message", PEERS_EV_UPDTMSG, NULL, p, msg_end,
                  &expire_sz);
      goto malformed_exit;
    }
    ...
    case STD_T_DICT: {
      ...
      if (*msg_cur == end) {
        ...
        if (id > dc->max_entries) {
          TRACE_PROTO("malformed message", PEERS_EV_UPDTMSG, NULL, p, NULL,
                      &id);
          goto malformed_unlock;
        }
        ...
        if (!*msg_cur || *msg_cur + value_len > end ||
          ...
          TRACE_PROTO("malformed message", PEERS_EV_UPDTMSG, NULL, p, *msg_cur,
                      &value_len);
          TRACE_PROTO("malformed message", PEERS_EV_UPDTMSG, NULL, p, end,
                      &chunk->size);
          goto malformed_unlock;
        }
 ...
 * has been received by <p> peer with <msg_cur> as address of the pointer to the
 * position in the receipt buffer with <msg_end> being the position of the end
 * of the stick-table message. Update <msg_curr> accordingly to the peer
 * protocol specs if no peer protocol error was encountered. Return 1 if
 * succeeded, 0 if not with the appctx state st0 set to PEER_SESS_ST_ERRPROTO.
 */
...
/*
 * Function used to parse a stick-table switch message after it has been
 * received by <p> peer with <msg_cur> as address of the pointer to the position
 * in the receipt buffer with <msg_end> being the position of the end of the
 * stick-table message. Update <msg_curr> accordingly to the peer protocol specs
 * if no peer protocol error was encountered. Return 1 if succeeded, 0 if not
 * with the appctx state st0 set to PEER_SESS_ST_ERRPROTO.
 */
...
/*
 * Function used to parse a stick-table definition message after it has been
 * received by <p> peer with <msg_cur> as address of the pointer to the position
 * in the receipt buffer with <msg_end> being the position of the end of the
 * stick-table message. Update <msg_curr> accordingly to the peer protocol specs
 * if no peer protocol error was encountered. <totl> is the length of the
 * stick-table update message computed upon receipt. Return 1 if succeeded, 0 if
 * not with the appctx state st0 set to PEER_SESS_ST_ERRPROTO.
 */
 ...
 * Returns 1 if there was no error, if not, returns 0 if not enough data were
 * available, -1 if there was an error updating the appctx state st0
 * accordingly.
 */
static inline int peer_recv_msg(struct appctx *appctx, char *msg_head,
                                size_t msg_head_sz, uint32_t *msg_len,
                                int *totl) {
  int reql;
  struct stream_interface *si = appctx->owner;
  char *cur;
  ...
  *msg_len = intdecode(&cur, cur + reql);
  if (!cur) {
    ...
    /* malformed message */
    TRACE_PROTO("malformed message: too large length encoding",
                PEERS_EV_UPDTMSG);
    appctx->st0 = PEER_SESS_ST_ERRPROTO;
    return -1;
  }
 ...
 */
static inline int peer_treat_awaited_msg(struct appctx *appctx,
                                         struct peer *peer,
  ...
  if (msg_head[0] == PEER_MSG_CLASS_CONTROL) {
    if (msg_head[1] == PEER_MSG_CTRL_RESYNCREQ) {
      ...
      if (peer->flags & PEER_F_LEARN_ASSIGN) {
        ...
        peers->resync_timeout =
            tick_add(now_ms, MS_TO_TICKS(PEER_RESYNC_TIMEOUT));
        task_wakeup(peers->sync_task, TASK_WOKEN_MSG);
      }
      peer->confirm++;
    ...
    if (msg_head[1] == PEER_MSG_STKT_DEFINE) {
      ...
      update = msg_head[1] == PEER_MSG_STKT_UPDATE ||
               msg_head[1] == PEER_MSG_STKT_UPDATE_TIMED;
      expire = msg_head[1] == PEER_MSG_STKT_UPDATE_TIMED ||
               msg_head[1] == PEER_MSG_STKT_INCUPDATE_TIMED;
      if (!peer_treat_updatemsg(appctx, peer, update, expire, msg_cur, msg_end,
                                msg_len, totl))
        return 0;
 ...
 */
static inline int peer_getline_last(struct appctx *appctx,
                                    struct peer **curpeer) {
  char *p;
  int reql;
  struct peer *peer;
  ...
  switchstate:
    ...
    case PEER_SESS_ST_GETPEER: {
      ...
      if (curpeer->appctx && curpeer->appctx != appctx) {
         ...
         */
        curpeer->reconnect =
            tick_add(now_ms, MS_TO_TICKS(50 + ha_random() % 2000));
    ...
    case PEER_SESS_ST_WAITMSG: {
      ...
      reql = peer_recv_msg(appctx, (char *)msg_head, sizeof msg_head, &msg_len,
                           &totl);
      if (reql <= 0) {
        if (reql == -1)
          goto switchstate;
      ...
      msg_end += msg_len;
      if (!peer_treat_awaited_msg(appctx, curpeer, msg_head, &msg_cur, msg_end,
                                  msg_len, totl))
        goto switchstate;
 ...
 * automatically)
 */
static struct appctx *peer_session_create(struct peers *peers,
                                          struct peer *peer) {
 ...
 */
static struct task *process_peer_sync(struct task *task, void *context,
  ...
  if (!stopping) {
          ...
          if (((peers->flags & PEERS_RESYNC_STATEMASK) ==
            ...
            for (st = ps->tables; st; st = st->next) {
              if ((int)(st->last_pushed - st->table->localupdate) < 0) {
                ...
                ps->heartbeat =
                    tick_add(now_ms, MS_TO_TICKS(PEER_HEARTBEAT_TIMEOUT));
            ...
            if (!update_to_push) {
              if (tick_is_expired(ps->reconnect, now_ms)) {
                if (ps->flags & PEER_F_ALIVE) {
                  ...
                  ps->reconnect =
                      tick_add(now_ms, MS_TO_TICKS(PEER_RECONNECT_TIMEOUT));
                } else {
                  ps->reconnect =
                      tick_add(now_ms, MS_TO_TICKS(50 + ha_random() % 2000));
    ...
    if ((peers->flags & PEERS_RESYNC_STATEMASK) != PEERS_RESYNC_FINISHED) {
      /* Resync not finished*/
      /* reschedule task to resync timeout if not expired, to ended resync if
       * needed */
    ...
    if (ps->flags & PEER_F_TEACH_COMPLETE) {
      ...
      if (ps->statuscode == 0 || ps->statuscode == PEER_SESS_SC_SUCCESSCODE ||
          ps->statuscode == PEER_SESS_SC_CONNECTEDCODE ||
...
/*
 * Look for the dictionary entry with the value of <i> in <d> cache of
 * dictionary entries used upon transmission. Return the entry if found, NULL if
 * not.
 */
 ...
 * error message.
 */
static int cli_parse_show_peers(char **args, char *payload,
 ...
 * if not. Dedicated to be called by cli_io_handler_show_peers() cli I/O
 * handler.
 */
static int peers_dump_head(struct buffer *msg, struct stream_interface *si,
                           struct peers *peers) {
  struct tm tm;
  ...
  chunk_appendf(
      msg,
      "%p: [%02d/%s/%04d:%02d:%02d:%02d] id=%s disabled=%d flags=0x%x "
      "resync_timeout=%s task_calls=%u\n",
      peers, tm.tm_mday, monthname[tm.tm_mon], tm.tm_year + 1900, tm.tm_hour,
      tm.tm_min, tm.tm_sec, peers->id, peers->disabled, peers->flags,
      peers->resync_timeout
          ? tick_is_expired(peers->resync_timeout, now_ms)
                ? "<PAST>"
 ...
 * handler.
 */
static int peers_dump_peer(struct buffer *msg, struct stream_interface *si,
                           struct peer *peer) {
  ...
  chunk_appendf(msg, "  %p: id=%s(%s,%s) addr=%s:%d last_status=%s", peer,
                peer->id, peer->local ? "local" : "remote",
                peer->appctx ? "active" : "inactive", pn,
                get_host_port(&peer->addr), statuscode_str(peer->statuscode));

  chunk_appendf(msg, " last_hdshk=%s\n",
                peer->last_hdshk
                    ? human_time(TICKS_TO_MS(now_ms - peer->last_hdshk),
                                 TICKS_TO_MS(1000))
                    : "<NEVER>");
  ...
  chunk_appendf(msg,
                " confirm=%u tx_hbt=%u rx_hbt=%u no_hbt=%u new_conn=%u "
                "proto_err=%u coll=%u\n",
                peer->confirm, peer->tx_hbt, peer->rx_hbt, peer->no_hbt,
                peer->new_conn, peer->proto_err, peer->coll);
  ...
  chunk_appendf(&trash, " appctx:%p st0=%d st1=%d task_calls=%u", appctx,
                appctx->st0, appctx->st1, appctx->t ? appctx->t->calls : 0);
...
table_info:
  if (peer->remote_table)
    chunk_appendf(&trash,
                  ...
                  peer->remote_table, peer->remote_table->table->id,
                  peer->remote_table->local_id, peer->remote_table->remote_id);

  if (peer->last_local_table)
    chunk_appendf(
        &trash, "\n        last_local_table:%p id=%s local_id=%d remote_id=%d",
        peer->last_local_table, peer->last_local_table->table->id,
        peer->last_local_table->local_id, peer->last_local_table->remote_id);
  ...
  if (peer->tables) {
    ...
    for (st = peer->tables; st; st = st->next) {
      ...
      chunk_appendf(&trash,
                    ...
                    st, st->local_id, st->remote_id, st->flags,
      ...
      chunk_appendf(&trash,
                    ...
                    t, t->id, t->update, t->localupdate, t->commitupdate,
                    t->syncing);
      chunk_appendf(&trash, "\n        TX dictionary cache:");
      count = 0;
      ...
      for (i = 0; i < dcache->max_entries; i++) {
        ...
        chunk_appendf(&trash, "  %3u -> %s", i,
                      dcache->rx[i].de ? (char *)dcache->rx[i].de->value.key
                                       : "-");
        count &= 0x3;
      }
    }
...
static struct cli_kw_list cli_kws = {
    {},
    {
        {
            {"show", "peers", NULL},
            "show peers [peers section]: dump some information about all the "
            "peers or this peers section",
