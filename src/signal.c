...
int signal_queue_len; /* length of signal queue, <= MAX_SIGNAL (1 entry per
                         signal max) */
...
DECLARE_STATIC_POOL(pool_head_sig_handlers, "sig_handlers",
                    sizeof(struct sig_handler));
...
void signal_handler(int sig) {
  if (sig < 0 || sig >= MAX_SIGNAL) {
    ...
    signal(sig, SIG_IGN);
    qfprintf(stderr,
             "Received unhandled signal %d. Signal has been disabled.\n", sig);
    return;
  }
 ...
 */
struct sig_handler *signal_register_task(int sig, struct task *task,
                                         int reason) {
  struct sig_handler *sh;
