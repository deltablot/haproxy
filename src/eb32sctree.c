 ...
 */
struct eb32sc_node *eb32sc_insert_dup(struct eb_node *sub, struct eb_node *new,
                                      unsigned long scope) {
 ...
 */
struct eb32sc_node *eb32sc_insert(struct eb_root *root, struct eb32sc_node *new,
                                  unsigned long scope) {
  ...
  while (1) {
    if (eb_gettag(troot) == EB_LEAF) {
      /* insert above a leaf */
      old = container_of(eb_untag(troot, EB_LEAF), struct eb32sc_node,
                         node.branches);
    ...
    old = container_of(eb_untag(troot, EB_NODE), struct eb32sc_node,
                       node.branches);
 ...
 */
struct eb32sc_node *eb32sc_lookup_ge(struct eb_root *root, u32 x,
                                     unsigned long scope) {
  ...
  while (1) {
    if ((eb_gettag(troot) == EB_LEAF)) {
       ...
       */
      node = container_of(eb_untag(troot, EB_LEAF), struct eb32sc_node,
                          node.branches);
    ...
    node = container_of(eb_untag(troot, EB_NODE), struct eb32sc_node,
                        node.branches);
    ...
    if (((x ^ node->key) >> node->node.bit) >= EB_NODE_BRANCHES) {
       ...
       */
      if ((node->node_s & scope) &&
          (node->key >> node->node.bit) > (x >> node->node.bit))
        troot = eb_dotag(&node->node.branches, EB_LEFT);
      else
        troot = node->node.node_p;
      break;
 ...
 */
struct eb32sc_node *eb32sc_lookup_ge_or_first(struct eb_root *root, u32 x,
                                              unsigned long scope) {
  struct eb32sc_node *eb32;
  eb_troot_t *troot;
  ...
  while (1) {
    if ((eb_gettag(troot) == EB_LEAF)) {
       ...
       * node or its next one if the former is too small.
       */
      eb32 = container_of(eb_untag(troot, EB_LEAF), struct eb32sc_node,
                          node.branches);
    ...
    eb32 = container_of(eb_untag(troot, EB_NODE), struct eb32sc_node,
                        node.branches);
    ...
    if (((x ^ eb32->key) >> eb32->node.bit) >= EB_NODE_BRANCHES) {
       ...
       */
      if ((eb32->node_s & scope) &&
          (eb32->key >> eb32->node.bit) > (x >> eb32->node.bit))
        troot = eb_dotag(&eb32->node.branches, EB_LEFT);
      else
        troot = eb32->node.node_p;
      break;
...
void eb32sc_delete(struct eb32sc_node *eb32) {
  ...
  for (pside = 0; pside <= 1; pside++) {
    if (eb_gettag(parent->branches.b[pside]) == EB_NODE) {
      ...
      scope |= container_of(eb_untag(parent->branches.b[pside], EB_NODE),
                            struct eb32sc_node, node.branches)
                   ->node_s;
    } else {
      eb_root_to_node(eb_untag(parent->branches.b[pside], EB_LEAF))->leaf_p =
          eb_dotag(&parent->branches, pside);
      scope |= container_of(eb_untag(parent->branches.b[pside], EB_LEAF),
                            struct eb32sc_node, node.branches)
                   ->leaf_s;
    }
  }
