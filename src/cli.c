...
static char *cli_gen_usage_msg(struct appctx *appctx) {
  ...
  list_for_each_entry(kw_list, &cli_keywords.list, list) {
    ...
    while (kw->str_kw[0]) {
      ...
      if (appctx->applet == &mcli_applet &&
          !(kw->level & (ACCESS_MASTER_ONLY | ACCESS_MASTER)))
        goto next_kw;
 ...
 */
static struct proxy *alloc_stats_fe(const char *name, const char *file,
                                    int line) {
  struct proxy *fe;
 ...
 */
static int stats_parse_global(char **args, int section_type,
  ...
  if (!strcmp(args[1], "socket")) {
    ...
    if (*args[2] == 0) {
      memprintf(err,
                "'%s %s' in global section expects an address or a path to a "
                "UNIX socket",
                args[0], args[1]);
      return -1;
    }
    ...
    if (!global.stats_fe) {
      if ((global.stats_fe = alloc_stats_fe("GLOBAL", file, line)) == NULL) {
        memprintf(err, "'%s %s' : out of memory trying to allocate a frontend",
                  args[0], args[1]);
        return -1;
      }
    }

    bind_conf = bind_conf_alloc(global.stats_fe, file, line, args[2],
                                xprt_get(XPRT_RAW));
    ...
    while (*args[cur_arg]) {
      ...
      if (kw) {
        if (!kw->parse) {
          memprintf(err,
                    "'%s %s' : '%s' option is not implemented in this version "
                    "(check build options).",
      ...
      memprintf(err, "'%s %s' : unknown keyword '%s'.%s%s", args[0], args[1],
                args[cur_arg], err && *err ? " Registered keywords :" : "",
                err && *err ? *err : "");
      return -1;
    }
    ...
    if (res == PARSE_TIME_OVER) {
      memprintf(err,
                "timer overflow in argument '%s' to '%s %s' (maximum value is "
                "2147483647 ms or ~24.8 days)",
                args[2], args[0], args[1]);
      return -1;
    } else if (res == PARSE_TIME_UNDER) {
      memprintf(err,
                "timer underflow in argument '%s' to '%s %s' (minimum non-null "
                "value is 1 ms)",
                args[2], args[0], args[1]);
      return -1;
    } else if (res) {
      memprintf(err, "'%s %s' : unexpected character '%c'", args[0], args[1],
                *res);
      return -1;
    }
  ...
  } else if (!strcmp(args[1], "bind-process")) { /* enable the socket only on
                                                    some processes */
    int cur_arg = 2;
    unsigned long set = 0;
    ...
    memprintf(err,
              "'%s' only supports 'socket', 'maxconn', 'bind-process' and "
              "'timeout' (got '%s')",
              args[0], args[1]);
    return -1;
  }
    ...
    list_for_each_entry(bind_conf, &frontend->conf.bind, by_fe) {
      ...
      list_for_each_entry(l, &bind_conf->listeners, by_bind) {
        ...
        if (l->rx.addr.ss_family == AF_UNIX) {
          ...
          chunk_appendf(trash, "sockpair@%d",
                        ((struct sockaddr_in *)&l->rx.addr)->sin_addr.s_addr);
        }
      }
    }
...
/* Returns severity_output for the current session if set, or default for the
 * socket */
 ...
 * 1 if the request was understood, otherwise zero. It is called with
 * appctx->st0 set to CLI_ST_GETREQ and presets ->st2 to 0 so that parsers don't
 * have to do it. It will possilbly leave st0 to CLI_ST_CALLBACK if the keyword
 * needs to have its own I/O handler called again. Most of the time, parsers
 * will only set st0 to CLI_ST_PRINT and put their message to be displayed into
 * cli.msg. If a keyword parser is NULL and an I/O handler is declared, the I/O
 * handler will automatically be used.
 */
...
/* prepends then outputs the argument msg with a syslog-type severity depending
 * on severity_output value */
static int cli_output_msg(struct channel *chn, const char *msg, int severity,
                          int severity_output) {
  struct buffer *tmp;
  ...
  while (1) {
    if (appctx->st0 == CLI_ST_INIT) {
      ...
      if (appctx->st1 & APPCTX_CLI_ST1_PAYLOAD) {
         ...
         * to a gathering mode.
         */
        if (!strcmp(appctx->chunk->area + appctx->chunk->data -
                        strlen(PAYLOAD_PATTERN),
                    PAYLOAD_PATTERN))
          appctx->st1 |= APPCTX_CLI_ST1_PAYLOAD;
        else {
      ...
      case CLI_ST_PRINT_FREE: /* print dyn error in err, free */
        if (appctx->st0 == CLI_ST_PRINT || appctx->st0 == CLI_ST_PRINT_ERR) {
          sev = appctx->st0 == CLI_ST_PRINT_ERR ? LOG_ERR
                                                : appctx->ctx.cli.severity;
          msg = appctx->ctx.cli.msg;
        } else if (appctx->st0 == CLI_ST_PRINT_DYN ||
                   appctx->st0 == CLI_ST_PRINT_FREE) {
          sev = appctx->st0 == CLI_ST_PRINT_FREE ? LOG_ERR
                                                 : appctx->ctx.cli.severity;
        ...
        if (cli_output_msg(res, msg, sev, cli_get_severity_output(appctx)) !=
            -1) {
      ...
      /* The post-command prompt is either LF alone or LF + '> ' in interactive
       * mode */
      ...
      /* If the output functions are still there, it means they require more
       * room. */
      ...
      if (((res->flags & (CF_SHUTW | CF_SHUTW_NOW))) ||
          (!(appctx->st1 & APPCTX_CLI_ST1_PROMPT) && !co_data(req) &&
           (!(appctx->st1 & APPCTX_CLI_ST1_PAYLOAD)))) {
        appctx->st0 = CLI_ST_END;
        continue;
      }
...
out:
  DPRINTF(stderr,
          "%s@%d: st=%d, rqf=%x, rpf=%x, rqh=%lu, rqs=%lu, rh=%lu, rs=%lu\n",
          __FUNCTION__, __LINE__, si->state, req->flags, res->flags,
          ci_data(req), co_data(req), ci_data(res), co_data(res));
}
...
static int cli_io_handler_show_fd(struct appctx *appctx) {
  ...
  while (fd >= 0 && fd < global.maxsock) {
    ...
    chunk_printf(
        &trash,
        "  %5d : st=0x%02x(R:%c%c W:%c%c) ev=0x%02x(%c%c%c%c%c) [%c%c] "
        "tmask=0x%lx umask=0x%lx owner=%p iocb=%p(",
        fd, fdt.state, (fdt.state & FD_EV_READY_R) ? 'R' : 'r',
        (fdt.state & FD_EV_ACTIVE_R) ? 'A' : 'a',
        (fdt.state & FD_EV_READY_W) ? 'R' : 'r',
        (fdt.state & FD_EV_ACTIVE_W) ? 'A' : 'a', fdt.ev,
        (fdt.ev & FD_POLL_HUP) ? 'H' : 'h', (fdt.ev & FD_POLL_ERR) ? 'E' : 'e',
        (fdt.ev & FD_POLL_OUT) ? 'O' : 'o', (fdt.ev & FD_POLL_PRI) ? 'P' : 'p',
        (fdt.ev & FD_POLL_IN) ? 'I' : 'i', fdt.linger_risk ? 'L' : 'l',
        fdt.cloned ? 'C' : 'c', fdt.thread_mask, fdt.update_mask, fdt.owner,
        fdt.iocb);
    ...
    if (!fdt.owner) {
      ...
      chunk_appendf(&trash, ") l.st=%s fe=%s", listener_state_str(li),
...
#define SHOW_AVG(t, x)                                                         \
  ...
  chunk_appendf(&trash, "date_now: %lu.%06lu\n", (long)now.tv_sec,
                (long)now.tv_usec);
  chunk_appendf(&trash, "ctxsw:");
  SHOW_TOT(thr, activity[thr].ctxsw);
  chunk_appendf(&trash, "tasksw:");
  SHOW_TOT(thr, activity[thr].tasksw);
  chunk_appendf(&trash, "empty_rq:");
  SHOW_TOT(thr, activity[thr].empty_rq);
  chunk_appendf(&trash, "long_rq:");
  SHOW_TOT(thr, activity[thr].long_rq);
  chunk_appendf(&trash, "loops:");
  SHOW_TOT(thr, activity[thr].loops);
  chunk_appendf(&trash, "wake_tasks:");
  SHOW_TOT(thr, activity[thr].wake_tasks);
  chunk_appendf(&trash, "wake_signal:");
  SHOW_TOT(thr, activity[thr].wake_signal);
  chunk_appendf(&trash, "poll_io:");
  SHOW_TOT(thr, activity[thr].poll_io);
  chunk_appendf(&trash, "poll_exp:");
  SHOW_TOT(thr, activity[thr].poll_exp);
  chunk_appendf(&trash, "poll_drop_fd:");
  SHOW_TOT(thr, activity[thr].poll_drop_fd);
  chunk_appendf(&trash, "poll_dead_fd:");
  SHOW_TOT(thr, activity[thr].poll_dead_fd);
  chunk_appendf(&trash, "poll_skip_fd:");
  SHOW_TOT(thr, activity[thr].poll_skip_fd);
  chunk_appendf(&trash, "conn_dead:");
  SHOW_TOT(thr, activity[thr].conn_dead);
  chunk_appendf(&trash, "stream_calls:");
  SHOW_TOT(thr, activity[thr].stream_calls);
  chunk_appendf(&trash, "pool_fail:");
  SHOW_TOT(thr, activity[thr].pool_fail);
  chunk_appendf(&trash, "buf_wait:");
  SHOW_TOT(thr, activity[thr].buf_wait);
  chunk_appendf(&trash, "cpust_ms_tot:");
  SHOW_TOT(thr, activity[thr].cpust_total / 2);
  chunk_appendf(&trash, "cpust_ms_1s:");
  SHOW_TOT(thr, read_freq_ctr(&activity[thr].cpust_1s) / 2);
  chunk_appendf(&trash, "cpust_ms_15s:");
  SHOW_TOT(thr, read_freq_ctr_period(&activity[thr].cpust_15s, 15000) / 2);
  chunk_appendf(&trash, "avg_loop_us:");
  SHOW_AVG(thr, swrate_avg(activity[thr].avg_loop_us, TIME_STATS_SAMPLES));
  chunk_appendf(&trash, "accepted:");
  SHOW_TOT(thr, activity[thr].accepted);
  chunk_appendf(&trash, "accq_pushed:");
  SHOW_TOT(thr, activity[thr].accq_pushed);
  chunk_appendf(&trash, "accq_full:");
  SHOW_TOT(thr, activity[thr].accq_full);
#ifdef USE_THREAD
  chunk_appendf(&trash, "accq_ring:");
  SHOW_TOT(thr, (accept_queue_rings[thr].tail - accept_queue_rings[thr].head +
                 ACCEPT_QUEUE_SIZE) %
                    ACCEPT_QUEUE_SIZE);
  chunk_appendf(&trash, "fd_takeover:");
  SHOW_TOT(thr, activity[thr].fd_takeover);
#endif

#if defined(DEBUG_DEV)
  /* keep these ones at the end */
  chunk_appendf(&trash, "ctr0:");
  SHOW_TOT(thr, activity[thr].ctr0);
  chunk_appendf(&trash, "ctr1:");
  SHOW_TOT(thr, activity[thr].ctr1);
  chunk_appendf(&trash, "ctr2:");
  SHOW_TOT(thr, activity[thr].ctr2);
#endif
      ...
      list_for_each_entry(bind_conf, &global.stats_fe->conf.bind, by_fe) {
        ...
        list_for_each_entry(l, &bind_conf->listeners, by_bind) {
          ...
          if (bind_conf->settings.bind_proc != 0) {
            int pos;
            for (pos = 0; pos < 8 * sizeof(bind_conf->settings.bind_proc);
                 pos++) {
        ...
        appctx->ctx.cli.p0 =
            &bind_conf->by_fe; /* store the latest list node dumped */
      }
    }
    /* fall through */
  default:
 ...
 */
static int cli_parse_show_env(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  extern char **environ;
  char **var;
  ...
  if (*args[2]) {
    ...
    for (; *var; var++) {
      if (strncmp(*var, args[2], len) == 0 && (*var)[len] == '=')
        break;
    }
    if (!*var)
 ...
 */
static int cli_parse_show_fd(char **args, char *payload, struct appctx *appctx,
                             void *private) {
  if (!cli_has_level(appctx, ACCESS_LVL_OPER))
    return 1;
...
static int cli_parse_set_timeout(char **args, char *payload,
...
static int cli_parse_set_maxconn_global(char **args, char *payload,
                                        struct appctx *appctx, void *private) {
  int v;
...
static int cli_parse_set_severity_output(char **args, char *payload,
                                         struct appctx *appctx, void *private) {
  if (*args[2] && set_severity_output(&appctx->cli_severity_output, args[2]))
    return 0;

  return cli_err(appctx,
                 "one of 'none', 'number', 'string' is a required argument\n");
}

/* show the level of the current CLI session */
static int cli_parse_show_lvl(char **args, char *payload, struct appctx *appctx,
                              void *private) {
...
static int cli_parse_set_lvl(char **args, char *payload, struct appctx *appctx,
                             void *private) {
...
static int cli_parse_expert_mode(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  if (!cli_has_level(appctx, ACCESS_LVL_ADMIN))
    return 1;
...
int cli_parse_default(char **args, char *payload, struct appctx *appctx,
                      void *private) {
  return 0;
}

/* parse a "set rate-limit" command. It always returns 1. */
static int cli_parse_set_ratelimit(char **args, char *payload,
                                   struct appctx *appctx, void *private) {
  int v;
  int *res;
  int mul = 1;
  ...
  if (strcmp(args[2], "connections") == 0 && strcmp(args[3], "global") == 0)
    ...
    return cli_err(
        ...
        "   - 'connections global' to set the per-process maximum connection "
        "rate\n"
        "   - 'sessions global' to set the per-process maximum session rate\n"
#ifdef USE_OPENSSL
        "   - 'ssl-sessions global' to set the per-process maximum SSL session "
        "rate\n"
#endif
        "   - 'http-compression global' to set the per-process maximum "
...
static int bind_parse_expose_fd(char **args, int cur_arg, struct proxy *px,
  ...
  if (!strcmp(args[cur_arg + 1], "listeners")) {
    ...
    memprintf(err, "'%s' only supports 'listeners' (got '%s')", args[cur_arg],
              args[cur_arg + 1]);
    return ERR_ALERT | ERR_FATAL;
  }
...
static int bind_parse_level(char **args, int cur_arg, struct proxy *px,
                            struct bind_conf *conf, char **err) {
  if (!*args[cur_arg + 1]) {
  ...
  if (!strcmp(args[cur_arg + 1], "user")) {
    ...
    memprintf(err,
              "'%s' only supports 'user', 'operator', and 'admin' (got '%s')",
...
static int bind_parse_severity_output(char **args, int cur_arg,
  ...
  if (set_severity_output(&conf->severity_output, args[cur_arg + 1]))
    ...
    memprintf(err,
              "'%s' only supports 'none', 'number', and 'string' (got '%s')",
              args[cur_arg], args[cur_arg + 1]);
    return ERR_ALERT | ERR_FATAL;
  }
}
...
static int _getsocks(char **args, char *payload, struct appctx *appctx,
                     void *private) {
  char *cmsgbuf = NULL;
...
static int cli_parse_simple(char **args, char *payload, struct appctx *appctx,
                            void *private) {
  if (*args[0] == 'h')
    /* help */
    cli_gen_usage_msg(appctx);
...
int pcli_find_and_exec_kw(struct stream *s, char **args, int argl,
                          char **errmsg, int *next_pid) {
  if (argl < 1)
    return 0;
 ...
 */
int pcli_parse_request(struct stream *s, struct channel *req, char **errmsg,
                       int *next_pid) {
...
int pcli_wait_for_response(struct stream *s, struct channel *rep, int an_bit) {
  ...
  if (s->pcli_flags & PCLI_F_PAYLOAD) {
    s->req.analysers |= AN_REQ_WAIT_CLI;
    s->res.analysers &= ~AN_RES_WAIT_CLI;
    s->req.flags |= CF_WAKE_ONCE; /* need to be called again if there is some
                                     command left in the request */
    return 0;
  }
  ...
  if ((rep->flags & (CF_SHUTR | CF_READ_NULL))) {
    ...
    s->logs.t_handshake =
        0; /* There are no handshake in keep alive connection. */
    s->logs.t_idle = -1;
    ...
    s->logs.t_data = -1;
    s->logs.t_close = 0;
    s->logs.prx_queue_pos =
        0; /* we get the number of pending conns before us */
    ...
    s->si[1].flags &=
        SI_FL_ISBACK |
        SI_FL_DONT_WAKE; /* we're in the context of process_stream */
    s->req.flags &=
        ~(CF_SHUTW | CF_SHUTW_NOW | CF_AUTO_CONNECT | CF_WRITE_ERROR |
          CF_STREAMER | CF_STREAMER_FAST | CF_NEVER_WAIT | CF_WROTE_DATA);
    s->res.flags &=
        ~(CF_SHUTR | CF_SHUTR_NOW | CF_READ_ATTACHED | CF_READ_ERROR |
          CF_READ_NOEXP | CF_STREAMER | CF_STREAMER_FAST | CF_WRITE_PARTIAL |
          CF_NEVER_WAIT | CF_WROTE_DATA | CF_READ_NULL);
    s->flags &= ~(SF_DIRECT | SF_ASSIGNED | SF_ADDR_SET | SF_BE_ASSIGNED |
                  SF_FORCE_PRST | SF_IGNORE_PRST);
...
int mworker_cli_proxy_create() {
  ...
  list_for_each_entry(child, &proc_list, list) {
    ...
    if ((sk = str2sa_range(msg, &port, &port1, &port2, NULL, &proto, &errmsg,
                           NULL, NULL, PA_O_STREAM)) == 0) {
      goto error;
    }
    free(msg);
    msg = NULL;
...
int mworker_cli_proxy_new_listener(char *line) {
  ...
  if (!str2listener(args[0], mworker_proxy, bind_conf, "master-socket", 0,
                    &err)) {
  ...
  while (*args[cur_arg]) {
    ...
    memprintf(&err, "'%s %s' : unknown keyword '%s'.%s%s", args[0], args[1],
              args[cur_arg], err ? " Registered keywords :" : "",
              err ? err : "");
    goto err;
  }
  ...
  list_for_each_entry(l, &bind_conf->listeners, by_bind) {
    ...
    /* don't make the peers subject to global limits and don't close it in the
     * master */
...
int mworker_cli_sockpair_new(struct mworker_proc *mworker_proc, int proc) {
  ...
  if (!global.stats_fe) {
    if ((global.stats_fe = alloc_stats_fe("GLOBAL", "master-socket", 0)) ==
        NULL) {
  ...
  bind_conf->level &= ~ACCESS_LVL_MASK;
  bind_conf->level |=
      ACCESS_LVL_ADMIN; /* TODO: need to lower the rights with a CLI keyword*/

  bind_conf->settings.bind_proc = 1UL << proc;
  global.stats_fe->bind_proc = 0; /* XXX: we should be careful with that, it can
...
static struct cli_kw_list cli_kws = {
    ...
    {{{"help", NULL}, NULL, cli_parse_simple, NULL},
     ...
     {{"set", "rate-limit", NULL},
      ...
      NULL},
     {{"set", "severity-output", NULL},
      "set severity-output [none|number|string] : set presence of severity "
      "level in feedback information",
...
static struct bind_kw_list bind_kws = {
    ...
    {},
    {
        ...
        {"expose-fd", bind_parse_expose_fd,
         1}, /* set the unix socket expose fd rights */
        {"severity-output", bind_parse_severity_output,
         1}, /* set the severity output format */
        {NULL, NULL, 0},
    }};
