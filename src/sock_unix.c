 ...
 */
int sock_unix_addrcmp(const struct sockaddr_storage *a,
  ...
  if (bu->sun_path[idx] != 0) {
    for (idx2 = dot + 1; idx2 && isdigit((unsigned char)bu->sun_path[idx2]);
         idx2++)
      ;
...
int sock_unix_bind_receiver(struct receiver *rx, char **errmsg) {
  ...
  if (path[0]) {
    ...
    if (unlink(tempname) < 0 && errno != ENOENT) {
      err |= ERR_FATAL | ERR_ALERT;
      memprintf(errmsg, "error when trying to unlink previous UNIX socket (%s)",
                strerror(errno));
      goto bind_return;
    }
    ...
    if (link(path, backname) < 0 && errno != ENOENT) {
      err |= ERR_FATAL | ERR_ALERT;
      memprintf(errmsg,
                "error when trying to preserve previous UNIX socket (%s)",
                strerror(errno));
      goto bind_return;
    }
  ...
  fd = socket(rx->proto->fam->sock_domain, rx->proto->sock_type,
              rx->proto->sock_prot);
  if (fd < 0) {
    err |= ERR_FATAL | ERR_ALERT;
...
fd_ready:
  ...
  if (!ext && path[0] &&
      (((rx->settings->ux.uid != -1 || rx->settings->ux.gid != -1) &&
       ...
       (rx->settings->ux.mode != 0 &&
        chmod(tempname, rx->settings->ux.mode) == -1))) {
    err |= ERR_FATAL | ERR_ALERT;
    memprintf(errmsg, "cannot change UNIX socket ownership (%s)",
              strerror(errno));
  ...
  if (!ext && path[0] && rename(tempname, path) < 0) {
    err |= ERR_FATAL | ERR_ALERT;
    memprintf(errmsg, "cannot switch final and temporary UNIX sockets (%s)",
              strerror(errno));
    goto err_rename;
  }
