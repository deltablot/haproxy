...
#define H2_CF_MUX_MALLOC                                                       \
  0x00000001 // mux blocked on lack of connection's mux buffer
#define H2_CF_MUX_MFULL                                                        \
  0x00000002 // mux blocked on connection's mux buffer full
 ...
 * received data.
 */
#define H2_CF_DEM_DALLOC                                                       \
  0x00000004 // demux blocked on lack of connection's demux buffer
#define H2_CF_DEM_DFULL                                                        \
  0x00000008 // demux blocked on connection's demux buffer full

#define H2_CF_DEM_MBUSY                                                        \
  0x00000010 // demux blocked on connection's mux side busy
#define H2_CF_DEM_MROOM                                                        \
  0x00000020 // demux blocked on lack of room in mux buffer
#define H2_CF_DEM_SALLOC                                                       \
  0x00000040 // demux blocked on lack of stream's request buffer
#define H2_CF_DEM_SFULL                                                        \
  0x00000080 // demux blocked on stream request buffer full
#define H2_CF_DEM_TOOMANY                                                      \
  0x00000100 // demux blocked waiting for some conn_streams to leave
#define H2_CF_DEM_BLOCK_ANY                                                    \
  0x000001F0 // aggregate of the demux flags above except DALLOC/DFULL
...
#define H2_CF_WAIT_FOR_HS                                                      \
  0x00004000 // We did check that at least a stream was waiting for handshake
...
struct h2c {
  ...
  struct list blocked_list; /* list of streams blocked for other reasons (e.g.
                               sfctl, dep) */
...
#define H2_SF_BLK_MROOM                                                        \
  0x00000020 // blocked waiting for room in the mux (must be in send list)
#define H2_SF_BLK_MFCTL                                                        \
  0x00000040 // blocked due to mux fctl (must be in fctl list)
#define H2_SF_BLK_SFCTL                                                        \
  0x00000080 // blocked due to stream fctl (must be in blocked list)
...
#define H2_SF_NOTIFIED                                                         \
  0x00000800 // a paused stream was notified to try to send again
#define H2_SF_HEADERS_SENT                                                     \
  0x00001000 // a HEADERS frame was sent for this stream
#define H2_SF_OUTGOING_DATA 0x00002000 // set whenever we've seen outgoing data

#define H2_SF_HEADERS_RCVD                                                     \
  0x00004000 // a HEADERS frame was received for this stream
...
struct h2s {
  ...
  struct wait_event *subs; /* recv wait_event the conn_stream associated is
                              waiting on (via h2_subscribe) */
  struct list
      list; /* To be used when adding in h2c->send_list or h2c->fctl_lsit */
  struct tasklet
      *shut_tl; /* deferred shutdown tasklet, to retry to send an RST after we
                 * failed to, in case there's no other subscription to do it */
};
...
#define H2_EV_H2C_FCTL (1ULL << 3)
    {.mask = H2_EV_H2C_FCTL,
     .name = "h2c_fctl",
     .desc = "H2 connection flow-controlled"},
#define H2_EV_H2C_BLK (1ULL << 4)
    {.mask = H2_EV_H2C_BLK, .name = "h2c_blk", .desc = "H2 connection blocked"},
#define H2_EV_H2C_WAKE (1ULL << 5)
    {.mask = H2_EV_H2C_WAKE,
     .name = "h2c_wake",
     .desc = "H2 connection woken up"},
#define H2_EV_H2C_END (1ULL << 6)
    {.mask = H2_EV_H2C_END,
     .name = "h2c_end",
     .desc = "H2 connection terminated"},
#define H2_EV_H2C_ERR (1ULL << 7)
    {.mask = H2_EV_H2C_ERR,
     .name = "h2c_err",
     .desc = "error on H2 connection"},
#define H2_EV_RX_FHDR (1ULL << 8)
    {.mask = H2_EV_RX_FHDR,
     .name = "rx_fhdr",
     .desc = "H2 frame header received"},
#define H2_EV_RX_FRAME (1ULL << 9)
    {.mask = H2_EV_RX_FRAME,
     .name = "rx_frame",
     .desc = "receipt of any H2 frame"},
#define H2_EV_RX_EOI (1ULL << 10)
    {.mask = H2_EV_RX_EOI,
     .name = "rx_eoi",
     .desc = "receipt of end of H2 input (ES or RST)"},
#define H2_EV_RX_PREFACE (1ULL << 11)
    {.mask = H2_EV_RX_PREFACE,
     .name = "rx_preface",
     .desc = "receipt of H2 preface"},
#define H2_EV_RX_DATA (1ULL << 12)
    {.mask = H2_EV_RX_DATA,
     .name = "rx_data",
     .desc = "receipt of H2 DATA frame"},
#define H2_EV_RX_HDR (1ULL << 13)
    {.mask = H2_EV_RX_HDR,
     .name = "rx_hdr",
     .desc = "receipt of H2 HEADERS frame"},
#define H2_EV_RX_PRIO (1ULL << 14)
    {.mask = H2_EV_RX_PRIO,
     .name = "rx_prio",
     .desc = "receipt of H2 PRIORITY frame"},
#define H2_EV_RX_RST (1ULL << 15)
    {.mask = H2_EV_RX_RST,
     .name = "rx_rst",
     .desc = "receipt of H2 RST_STREAM frame"},
#define H2_EV_RX_SETTINGS (1ULL << 16)
    {.mask = H2_EV_RX_SETTINGS,
     .name = "rx_settings",
     .desc = "receipt of H2 SETTINGS frame"},
#define H2_EV_RX_PUSH (1ULL << 17)
    {.mask = H2_EV_RX_PUSH,
     .name = "rx_push",
     .desc = "receipt of H2 PUSH_PROMISE frame"},
#define H2_EV_RX_PING (1ULL << 18)
    {.mask = H2_EV_RX_PING,
     .name = "rx_ping",
     .desc = "receipt of H2 PING frame"},
#define H2_EV_RX_GOAWAY (1ULL << 19)
    {.mask = H2_EV_RX_GOAWAY,
     .name = "rx_goaway",
     .desc = "receipt of H2 GOAWAY frame"},
#define H2_EV_RX_WU (1ULL << 20)
    {.mask = H2_EV_RX_WU,
     .name = "rx_wu",
     .desc = "receipt of H2 WINDOW_UPDATE frame"},
#define H2_EV_RX_CONT (1ULL << 21)
    {.mask = H2_EV_RX_CONT,
     .name = "rx_cont",
     .desc = "receipt of H2 CONTINUATION frame"},
#define H2_EV_TX_FRAME (1ULL << 22)
    {.mask = H2_EV_TX_FRAME,
     .name = "tx_frame",
     .desc = "transmission of any H2 frame"},
#define H2_EV_TX_EOI (1ULL << 23)
    {.mask = H2_EV_TX_EOI,
     .name = "tx_eoi",
     .desc = "transmission of H2 end of input (ES or RST)"},
#define H2_EV_TX_PREFACE (1ULL << 24)
    {.mask = H2_EV_TX_PREFACE,
     .name = "tx_preface",
     .desc = "transmission of H2 preface"},
#define H2_EV_TX_DATA (1ULL << 25)
    {.mask = H2_EV_TX_DATA,
     .name = "tx_data",
     .desc = "transmission of H2 DATA frame"},
#define H2_EV_TX_HDR (1ULL << 26)
    {.mask = H2_EV_TX_HDR,
     .name = "tx_hdr",
     .desc = "transmission of H2 HEADERS frame"},
#define H2_EV_TX_PRIO (1ULL << 27)
    {.mask = H2_EV_TX_PRIO,
     .name = "tx_prio",
     .desc = "transmission of H2 PRIORITY frame"},
#define H2_EV_TX_RST (1ULL << 28)
    {.mask = H2_EV_TX_RST,
     .name = "tx_rst",
     .desc = "transmission of H2 RST_STREAM frame"},
#define H2_EV_TX_SETTINGS (1ULL << 29)
    {.mask = H2_EV_TX_SETTINGS,
     .name = "tx_settings",
     .desc = "transmission of H2 SETTINGS frame"},
#define H2_EV_TX_PUSH (1ULL << 30)
    {.mask = H2_EV_TX_PUSH,
     .name = "tx_push",
     .desc = "transmission of H2 PUSH_PROMISE frame"},
#define H2_EV_TX_PING (1ULL << 31)
    {.mask = H2_EV_TX_PING,
     .name = "tx_ping",
     .desc = "transmission of H2 PING frame"},
#define H2_EV_TX_GOAWAY (1ULL << 32)
    {.mask = H2_EV_TX_GOAWAY,
     .name = "tx_goaway",
     .desc = "transmission of H2 GOAWAY frame"},
#define H2_EV_TX_WU (1ULL << 33)
    {.mask = H2_EV_TX_WU,
     .name = "tx_wu",
     .desc = "transmission of H2 WINDOW_UPDATE frame"},
#define H2_EV_TX_CONT (1ULL << 34)
    {.mask = H2_EV_TX_CONT,
     .name = "tx_cont",
     .desc = "transmission of H2 CONTINUATION frame"},
...
#define H2_EV_H2S_FCTL (1ULL << 38)
    {.mask = H2_EV_H2S_FCTL,
     .name = "h2s_fctl",
     .desc = "H2 stream flow-controlled"},
...
#define H2_VERB_MINIMAL 2
    {.name = "minimal",
     .desc = "report only h2c/h2s state and flags, no real decoding"},
#define H2_VERB_SIMPLE 3
    {.name = "simple",
     .desc = "add request/response status line or frame info when available"},
#define H2_VERB_ADVANCED 4
    {.name = "advanced",
     .desc = "add header fields or frame decoding when available"},
...
static struct name_desc h2_stats[] = {
    ...
    [H2_ST_CONN_PROTO_ERR] = {.name = "h2_detected_conn_protocol_errors",
                              .desc =
                                  "Total number of connection protocol errors"},
    ...
    [H2_ST_GOAWAY_RESP] = {.name = "h2_goaway_resp",
                           .desc =
                               "Total number of GOAWAY sent on detected error"},
...
static struct h2_counters {
  ...
  long long
      rst_stream_resp;   /* total number of RST_STREAM frame sent on error */
...
static struct task *h2_timeout_task(struct task *t, void *context,
...
static int h2c_decode_headers(struct h2c *h2c, struct buffer *rxbuf,
                              uint32_t *flags, unsigned long long *body_len);
static int h2_frt_transfer_data(struct h2s *h2s);
static struct task *h2_deferred_shut(struct task *t, void *ctx,
                                     unsigned short state);
...
static inline const char *h2s_st_to_str(enum h2_ss st) {
  ...
  case H2_SS_IDLE:
    return "IDL"; // idle
  case H2_SS_RLOC:
    return "RSL"; // reserved local
  case H2_SS_RREM:
    return "RSR"; // reserved remote
  case H2_SS_OPEN:
    return "OPN"; // open
  case H2_SS_HREM:
    return "HCR"; // half-closed remote
  case H2_SS_HLOC:
    return "HCL"; // half-closed local
  case H2_SS_ERROR:
    return "ERR"; // error
  case H2_SS_CLOSED:
    return "CLO"; // closed
...
static void h2_trace(enum trace_level level, uint64_t mask,
  ...
  if (src->verbosity > H2_VERB_CLEAN) {
    chunk_appendf(&trace_buf, " : h2c=%p(%c,%s)", h2c,
                  conn_is_back(conn) ? 'B' : 'F', h2c_st_to_str(h2c->st0));

    if (h2c->errcode)
      chunk_appendf(&trace_buf, " err=%s/%02x", h2_err_str(h2c->errcode),
                    h2c->errcode);

    if (h2c->dsi >= 0 && (mask & (H2_EV_RX_FRAME | H2_EV_RX_FHDR)) ==
                             (H2_EV_RX_FRAME | H2_EV_RX_FHDR)) {
      chunk_appendf(&trace_buf, " dft=%s/%02x dfl=%d", h2_ft_str(h2c->dft),
                    h2c->dff, h2c->dfl);
    }
    ...
    if (h2s) {
      ...
      chunk_appendf(&trace_buf, " h2s=%p(%d,%s)", h2s, h2s->id,
                    h2s_st_to_str(h2s->st));
      if (h2s->id && h2s->errcode)
        chunk_appendf(&trace_buf, " err=%s/%02x", h2_err_str(h2s->errcode),
                      h2s->errcode);
    }
  }
  ...
  if ((mask == (H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_STRM_NEW) ||
       mask == (H2_EV_RX_FRAME | H2_EV_RX_HDR)) &&
      buf)
  ...
  if (level == TRACE_LEVEL_USER && src->verbosity != H2_VERB_MINIMAL && htx &&
      (pos = htx_get_head(htx)) != -1) {
    ...
    if (type == HTX_BLK_REQ_SL)
      chunk_appendf(&trace_buf, " : [%d] H2 REQ: %.*s %.*s %.*s",
                    h2s ? h2s->id : h2c->dsi, HTX_SL_P1_LEN(sl),
...
static inline int h2c_may_expire(const struct h2c *h2c) {
  return eb_is_empty(&h2c->streams_by_id) || br_data(h2c->mbuf) ||
         !LIST_ISEMPTY(&h2c->blocked_list) || !LIST_ISEMPTY(&h2c->fctl_list) ||
...
static __inline int h2c_is_dead(const struct h2c *h2c) {
  if (eb_is_empty(&h2c->streams_by_id) &&  /* don't close if streams exist */
      ((h2c->conn->flags & CO_FL_ERROR) || /* errors close immediately */
       ...
       (!(h2c->conn->owner)) || /* Nobody's left to take care of the connection,
                                   drop it now */
       (!br_data(h2c->mbuf) &&  /* mux buffer empty, also process clean events
                                   below */
...
static inline int h2_recv_allowed(const struct h2c *h2c) {
  if (b_data(&h2c->dbuf) == 0 &&
      (h2c->st0 >= H2_CS_ERROR || h2c->conn->flags & CO_FL_ERROR ||
       conn_xprt_read0_pending(h2c->conn)))
    return 0;
...
static int h2_buf_available(void *target) {
  ...
  if ((h2c->flags & H2_CF_MUX_MALLOC) &&
      b_alloc_margin(br_tail(h2c->mbuf), 0)) {
  ...
  if ((h2c->flags & H2_CF_DEM_SALLOC) && (h2s = h2c_st_by_id(h2c, h2c->dsi)) &&
      h2s->cs && b_alloc_margin(&h2s->rxbuf, 0)) {
...
static int h2_avail_streams(struct connection *conn) {
  ...
  if (ret1 > 0 && srv && srv->max_reuse >= 0) {
    ret2 = h2c->stream_cnt <= srv->max_reuse
               ? srv->max_reuse - h2c->stream_cnt + 1
               : 0;
    ret1 = MIN(ret1, ret2);
  }
  return ret1;
...
static int h2_init(struct connection *conn, struct proxy *prx,
  ...
  if (conn_is_back(conn)) {
    ...
    h2c->px_counters =
        EXTRA_COUNTERS_GET(prx->extra_counters_be, &h2_stats_module);
  } else {
    ...
    h2c->px_counters =
        EXTRA_COUNTERS_GET(prx->extra_counters_fe, &h2_stats_module);
  }
...
fail_no_h2c:
  ...
  TRACE_DEVEL("leaving in error",
              H2_EV_H2C_NEW | H2_EV_H2C_END | H2_EV_H2C_ERR);
  return -1;
}
...
static inline __maybe_unused int h2c_mux_busy(const struct h2c *h2c,
...
h2_get_buf_bytes(void *dst, size_t bytes, const struct buffer *b, int o) {
  readv_bytes(dst, bytes, b_peek(b, o), b_wrap(b) - b_peek(b, o), b_orig(b));
}

static inline __maybe_unused uint16_t h2_get_n16(const struct buffer *b,
                                                 int o) {
  return readv_n16(b_peek(b, o), b_wrap(b) - b_peek(b, o), b_orig(b));
}

static inline __maybe_unused uint32_t h2_get_n32(const struct buffer *b,
                                                 int o) {
  return readv_n32(b_peek(b, o), b_wrap(b) - b_peek(b, o), b_orig(b));
}

static inline __maybe_unused uint64_t h2_get_n64(const struct buffer *b,
                                                 int o) {
 ...
 */
static __maybe_unused int h2_peek_frame_hdr(const struct buffer *b, int o,
                                            struct h2_fh *h) {
  uint64_t w;
...
static inline __maybe_unused int h2_get_frame_hdr(struct buffer *b,
                                                  struct h2_fh *h) {
  int ret;
...
static int h2c_frt_recv_preface(struct h2c *h2c) {
  ...
  if (unlikely(ret1 <= 0)) {
    ...
    if (ret1 < 0 || conn_xprt_read0_pending(h2c->conn)) {
      TRACE_ERROR("I/O error or short read", H2_EV_RX_FRAME | H2_EV_RX_PREFACE,
                  h2c->conn);
  ...
  for (offset = 0; offset < h2c->dfl; offset += 6) {
    ...
    case H2_SETTINGS_MAX_FRAME_SIZE:
      if (arg < 16384 || arg > 16777215) { // RFC7540#6.5.2
        TRACE_ERROR("MAX_FRAME_SIZE out of range",
    ...
    case H2_SETTINGS_ENABLE_PUSH:
      if (arg < 0 || arg > 1) { // RFC7540#6.5.2
        TRACE_ERROR("ENABLE_PUSH out of range",
                    H2_EV_RX_FRAME | H2_EV_RX_SETTINGS, h2c->conn);
        error = H2_ERR_PROTOCOL_ERROR;
        HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
        goto fail;
...
static int h2c_handle_window_update(struct h2c *h2c, struct h2s *h2s) {
  ...
  if (h2c->dsi != 0) {
    ...
    if (!inc) {
      TRACE_ERROR("stream WINDOW_UPDATE inc=0", H2_EV_RX_FRAME | H2_EV_RX_WU,
                  h2c->conn, h2s);
      error = H2_ERR_PROTOCOL_ERROR;
    ...
    if (!inc) {
      TRACE_ERROR("conn WINDOW_UPDATE inc=0", H2_EV_RX_FRAME | H2_EV_RX_WU,
                  h2c->conn);
      error = H2_ERR_PROTOCOL_ERROR;
      HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
      goto conn_err;
...
conn_err:
  h2c_error(h2c, error);
out0:
  TRACE_DEVEL("leaving on missing data or error", H2_EV_RX_FRAME | H2_EV_RX_WU,
              h2c->conn);
  return 0;
...
strm_err:
  ...
  TRACE_DEVEL("leaving on stream error", H2_EV_RX_FRAME | H2_EV_RX_WU,
              h2c->conn);
  return 0;
}
...
static int h2c_handle_goaway(struct h2c *h2c) {
  ...
  /* process full frame only */
  if (b_data(&h2c->dbuf) < h2c->dfl) {
    TRACE_DEVEL("leaving on missing data", H2_EV_RX_FRAME | H2_EV_RX_GOAWAY,
                h2c->conn);
    return 0;
  }
...
static int h2c_handle_priority(struct h2c *h2c) {
  ...
  /* process full frame only */
  if (b_data(&h2c->dbuf) < h2c->dfl) {
    TRACE_DEVEL("leaving on missing data", H2_EV_RX_FRAME | H2_EV_RX_PRIO,
                h2c->conn);
    return 0;
  }
  ...
  if (h2_get_n32(&h2c->dbuf, 0) == h2c->dsi) {
    ...
    TRACE_ERROR("PRIORITY depends on itself", H2_EV_RX_FRAME | H2_EV_RX_WU,
                h2c->conn);
    h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
    HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
...
static int h2c_handle_rst_stream(struct h2c *h2c, struct h2s *h2s) {
  ...
  if (b_data(&h2c->dbuf) < h2c->dfl) {
    TRACE_DEVEL("leaving on missing data",
                H2_EV_RX_FRAME | H2_EV_RX_RST | H2_EV_RX_EOI, h2c->conn, h2s);
    return 0;
  }

  /* late RST, already handled */
  if (h2s->st == H2_SS_CLOSED) {
    TRACE_DEVEL("leaving on stream closed",
                H2_EV_RX_FRAME | H2_EV_RX_RST | H2_EV_RX_EOI, h2c->conn, h2s);
    return 1;
  }
...
static struct h2s *h2c_frt_handle_headers(struct h2c *h2c, struct h2s *h2s) {
  ...
  if (h2s->st != H2_SS_IDLE) {
    ...
    error = H2_ERR_PROTOCOL_ERROR;
    TRACE_ERROR("HEADERS on invalid stream ID", H2_EV_RX_FRAME | H2_EV_RX_HDR,
                h2c->conn);
    HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
    sess_log(h2c->conn->owner);
    goto conn_err;
  ...
  TRACE_USER("rcvd H2 request ", H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_STRM_NEW,
             h2c->conn, 0, &rxbuf);
  return h2s;
...
out:
  ...
  TRACE_DEVEL("leaving on missing data or error", H2_EV_RX_FRAME | H2_EV_RX_HDR,
              h2c->conn, h2s);
  return NULL;
...
send_rst:
  ...
  TRACE_USER("rejected H2 request",
             H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_STRM_NEW | H2_EV_STRM_END,
             h2c->conn, 0, &rxbuf);
  TRACE_DEVEL("leaving on error", H2_EV_RX_FRAME | H2_EV_RX_HDR, h2c->conn,
              h2s);
  return h2s;
}
...
static struct h2s *h2c_bck_handle_headers(struct h2c *h2c, struct h2s *h2s) {
  ...
  if (h2s->st != H2_SS_OPEN && h2s->st != H2_SS_HLOC) {
    /* RFC7540#5.1 */
    TRACE_ERROR("response HEADERS in invalid state",
                H2_EV_RX_FRAME | H2_EV_RX_HDR, h2c->conn, h2s);
  ...
  if (error <= 0) {
    ...
    TRACE_ERROR("couldn't decode response HEADERS",
                H2_EV_RX_FRAME | H2_EV_RX_HDR, h2c->conn, h2s);
  ...
  TRACE_USER("rcvd H2 response", H2_EV_RX_FRAME | H2_EV_RX_HDR, h2c->conn, 0,
             &h2s->rxbuf);
...
send_rst:
  ...
  TRACE_USER("rejected H2 response",
             H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_STRM_NEW | H2_EV_STRM_END,
             h2c->conn, 0, &rxbuf);
  TRACE_DEVEL("leaving on error", H2_EV_RX_FRAME | H2_EV_RX_HDR, h2c->conn,
              h2s);
...
static int h2c_frt_handle_data(struct h2c *h2c, struct h2s *h2s) {
  ...
  if ((h2s->flags & H2_SF_DATA_CLEN) && (h2c->dfl - h2c->dpl) > h2s->body_len) {
    /* RFC7540#8.1.2 */
    TRACE_ERROR("DATA frame larger than content-length",
                H2_EV_RX_FRAME | H2_EV_RX_DATA, h2c->conn, h2s);
    error = H2_ERR_PROTOCOL_ERROR;
    HA_ATOMIC_ADD(&h2c->px_counters->strm_proto_err, 1);
    goto strm_err;
  }
  ...
  if (h2c->dff & H2_F_DATA_END_STREAM) {
    ...
    if (h2s->flags & H2_SF_DATA_CLEN && h2s->body_len) {
      /* RFC7540#8.1.2 */
      TRACE_ERROR("ES on DATA frame before content-length",
                  H2_EV_RX_FRAME | H2_EV_RX_DATA, h2c->conn, h2s);
      error = H2_ERR_PROTOCOL_ERROR;
      HA_ATOMIC_ADD(&h2c->px_counters->strm_proto_err, 1);
      goto strm_err;
...
static int h2_frame_check_vs_state(struct h2c *h2c, struct h2s *h2s) {
  ...
  if (h2s->st == H2_SS_IDLE && h2c->dft != H2_FT_HEADERS &&
      h2c->dft != H2_FT_PRIORITY) {
    /* RFC7540#5.1: any frame other than HEADERS or PRIORITY in
     * this state MUST be treated as a connection error
     */
    TRACE_ERROR("invalid frame type for IDLE state",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR, h2c->conn, h2s);
    h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
  ...
  if (h2s->st == H2_SS_IDLE && (h2c->flags & H2_CF_IS_BACK)) {
    ...
    TRACE_ERROR("invalid frame type for IDLE state (back)",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR, h2c->conn, h2s);
    h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
    HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
    TRACE_DEVEL("leaving in error (idle&back)",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR, h2c->conn,
                h2s);
    return 0;
  }
  ...
  if (h2s->st == H2_SS_HREM && h2c->dft != H2_FT_WINDOW_UPDATE &&
    ...
    if (h2_ft_bit(h2c->dft) & H2_FT_HDR_MASK) {
      TRACE_ERROR("invalid frame type for HREM state",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR, h2c->conn, h2s);
      h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
      HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
    } else {
    ...
    TRACE_DEVEL("leaving in error (hrem&!wu&!rst&!prio)",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR, h2c->conn,
                h2s);
    return 0;
  }
  ...
  if (h2s->st == H2_SS_CLOSED && h2c->dsi) {
    if (!(h2c->flags & H2_CF_IS_BACK) && h2_ft_bit(h2c->dft) & H2_FT_HDR_MASK) {
      ...
      TRACE_DEVEL("leaving in error (closed&hdrmask)",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR, h2c->conn,
                  h2s);
      return 0;
    }
    ...
    if (h2s->flags & H2_SF_RST_RCVD &&
        !(h2_ft_bit(h2c->dft) &
          (H2_FT_HDR_MASK | H2_FT_RST_STREAM_BIT | H2_FT_PRIORITY_BIT |
      ...
      h2s_error(h2s, H2_ERR_STREAM_CLOSED);
      h2c->st0 = H2_CS_FRAME_E;
      TRACE_DEVEL("leaving in error (rst_rcvd&!hdrmask)",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR, h2c->conn,
                  h2s);
      return 0;
    }
    ...
    if (h2s->id && !(h2s->flags & H2_SF_RST_SENT)) {
       ...
       * a connection error
       */
      if (h2c->dft != H2_FT_RST_STREAM && h2c->dft != H2_FT_PRIORITY &&
          h2c->dft != H2_FT_WINDOW_UPDATE) {
        h2c_error(h2c, H2_ERR_STREAM_CLOSED);
        TRACE_DEVEL("leaving in error (rst_sent&!rst&!prio&!wu)",
                    H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR, h2c->conn,
                    h2s);
        return 0;
      }
...
static void h2_process_demux(struct h2c *h2c) {
  ...
  if (unlikely(h2c->st0 < H2_CS_FRAME_H)) {
    if (h2c->st0 == H2_CS_PREFACE) {
      ...
      if (unlikely(h2c_frt_recv_preface(h2c) <= 0)) {
        ...
        if (h2c->st0 == H2_CS_ERROR) {
          TRACE_PROTO("failed to receive preface",
                      H2_EV_RX_PREFACE | H2_EV_PROTO_ERR, h2c->conn);
    ...
    if (h2c->st0 == H2_CS_SETTINGS1) {
       ...
       * without an ACK.
       */
      TRACE_STATE("expecting settings",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_SETTINGS,
                  h2c->conn);
      if (!h2_get_frame_hdr(&h2c->dbuf, &hdr)) {
        /* RFC7540#3.5: a GOAWAY frame MAY be omitted */
        if (h2c->st0 == H2_CS_ERROR) {
          TRACE_ERROR("failed to receive settings",
                      H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_SETTINGS |
                          H2_EV_PROTO_ERR,
                      h2c->conn);
          h2c->st0 = H2_CS_ERROR2;
      ...
      if (hdr.sid || hdr.ft != H2_FT_SETTINGS || hdr.ff & H2_F_SETTINGS_ACK) {
        /* RFC7540#3.5: a GOAWAY frame MAY be omitted */
        TRACE_ERROR("unexpected frame type or flags",
                    H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_SETTINGS |
                        H2_EV_PROTO_ERR,
                    h2c->conn);
        h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
      ...
      if ((int)hdr.len < 0 || (int)hdr.len > global.tune.bufsize) {
        /* RFC7540#3.5: a GOAWAY frame MAY be omitted */
        TRACE_ERROR("invalid settings frame length",
                    H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_SETTINGS |
                        H2_EV_PROTO_ERR,
                    h2c->conn);
  ...
  while (1) {
    ...
    if (h2c->st0 >= H2_CS_ERROR) {
      TRACE_STATE("end of connection reported", H2_EV_RX_FRAME | H2_EV_RX_EOI,
                  h2c->conn);
      break;
    }
    ...
    if (h2c->st0 == H2_CS_FRAME_H) {
      ...
      TRACE_STATE("expecting H2 frame header", H2_EV_RX_FRAME | H2_EV_RX_FHDR,
                  h2c->conn);
      if (!h2_peek_frame_hdr(&h2c->dbuf, 0, &hdr))
        break;

      if ((int)hdr.len < 0 || (int)hdr.len > global.tune.bufsize) {
        TRACE_ERROR("invalid H2 frame length",
                    H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR,
                    h2c->conn);
        h2c_error(h2c, H2_ERR_FRAME_SIZE_ERROR);
        if (!h2c->nb_streams && !(h2c->flags & H2_CF_IS_BACK)) {
          /* only log if no other stream can report the error */
      ...
      if (h2_ft_bit(hdr.ft) & H2_FT_PADDED_MASK && hdr.ff & H2_F_PADDED) {
         ...
         */
        if (hdr.len < 1) {
          TRACE_ERROR("invalid H2 padded frame length",
                      H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR,
                      h2c->conn);
          h2c_error(h2c, H2_ERR_FRAME_SIZE_ERROR);
          if (!(h2c->flags & H2_CF_IS_BACK))
        ...
        if (padlen > hdr.len) {
          TRACE_ERROR("invalid H2 padding length",
                      H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_PROTO_ERR,
                      h2c->conn);
    ...
    new_frame:
      ...
      h2c->dff = hdr.ff;
      h2c->dpl = padlen;
      TRACE_STATE("rcvd H2 frame header, switching to FRAME_P state",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR, h2c->conn);
      h2c->st0 = H2_CS_FRAME_P;

      /* check for minimum basic frame format validity */
      ret =
          h2_frame_check(h2c->dft, 1, h2c->dsi, h2c->dfl, global.tune.bufsize);
    ...
    if (tmp_h2s != h2s && h2s && h2s->cs &&
        (b_data(&h2s->rxbuf) || h2c_read0_pending(h2c) ||
         h2s->st == H2_SS_CLOSED || (h2s->flags & H2_SF_ES_RCVD) ||
         (h2s->cs->flags & (CS_FL_ERROR | CS_FL_ERR_PENDING | CS_FL_EOS)))) {
      /* we may have to signal the upper layers */
      TRACE_DEVEL("notifying stream before switching SID",
                  H2_EV_RX_FRAME | H2_EV_STRM_WAKE, h2c->conn, h2s);
    ...
    if (h2c->st0 == H2_CS_FRAME_E ||
      ...
      TRACE_PROTO("stream error reported", H2_EV_RX_FRAME | H2_EV_PROTO_ERR,
                  h2c->conn, h2s);
      goto strm_err;
    }
    ...
    case H2_FT_SETTINGS:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 SETTINGS frame",
                    H2_EV_RX_FRAME | H2_EV_RX_SETTINGS, h2c->conn, h2s);
        ret = h2c_handle_settings(h2c);
      }
      HA_ATOMIC_ADD(&h2c->px_counters->settings_rcvd, 1);

      if (h2c->st0 == H2_CS_FRAME_A) {
        TRACE_PROTO("sending H2 SETTINGS ACK frame",
                    H2_EV_TX_FRAME | H2_EV_RX_SETTINGS, h2c->conn, h2s);
    ...
    case H2_FT_PING:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 PING frame", H2_EV_RX_FRAME | H2_EV_RX_PING,
                    h2c->conn, h2s);
        ret = h2c_handle_ping(h2c);
      }

      if (h2c->st0 == H2_CS_FRAME_A) {
        TRACE_PROTO("sending H2 PING ACK frame",
                    H2_EV_TX_FRAME | H2_EV_TX_SETTINGS, h2c->conn, h2s);
    ...
    case H2_FT_WINDOW_UPDATE:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 WINDOW_UPDATE frame",
                    H2_EV_RX_FRAME | H2_EV_RX_WU, h2c->conn, h2s);
    ...
    case H2_FT_CONTINUATION:
       ...
       */
      TRACE_ERROR("received unexpected H2 CONTINUATION frame",
                  H2_EV_RX_FRAME | H2_EV_RX_CONT | H2_EV_H2C_ERR, h2c->conn,
                  h2s);
      h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
      if (!(h2c->flags & H2_CF_IS_BACK))
        sess_log(h2c->conn->owner);
    ...
    case H2_FT_HEADERS:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 HEADERS frame", H2_EV_RX_FRAME | H2_EV_RX_HDR,
                    h2c->conn, h2s);
        if (h2c->flags & H2_CF_IS_BACK)
    ...
    case H2_FT_DATA:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 DATA frame", H2_EV_RX_FRAME | H2_EV_RX_DATA,
                    h2c->conn, h2s);
      ...
      if (h2c->st0 == H2_CS_FRAME_A) {
        TRACE_PROTO("sending stream WINDOW_UPDATE frame",
                    H2_EV_TX_FRAME | H2_EV_TX_WU, h2c->conn, h2s);
    ...
    case H2_FT_PRIORITY:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 PRIORITY frame",
                    H2_EV_RX_FRAME | H2_EV_RX_PRIO, h2c->conn, h2s);
    ...
    case H2_FT_RST_STREAM:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 RST_STREAM frame",
                    H2_EV_RX_FRAME | H2_EV_RX_RST | H2_EV_RX_EOI, h2c->conn,
                    h2s);
    ...
    case H2_FT_GOAWAY:
      if (h2c->st0 == H2_CS_FRAME_P) {
        TRACE_PROTO("receiving H2 GOAWAY frame",
                    H2_EV_RX_FRAME | H2_EV_RX_GOAWAY, h2c->conn, h2s);
  ...
  strm_err:
    ...
    if (h2s->st == H2_SS_ERROR) {
      TRACE_STATE("stream error, switching to FRAME_E",
                  H2_EV_RX_FRAME | H2_EV_H2S_ERR, h2c->conn, h2s);
      h2c->st0 = H2_CS_FRAME_E;
    }

    if (h2c->st0 == H2_CS_FRAME_E) {
      TRACE_PROTO("sending H2 RST_STREAM frame",
                  H2_EV_TX_FRAME | H2_EV_TX_RST | H2_EV_TX_EOI, h2c->conn, h2s);
      ret = h2c_send_rst_stream(h2c, h2s);
    }

    /* error or missing data condition met above ? */
    if (ret <= 0) {
      TRACE_DEVEL("insufficient data to proceed", H2_EV_RX_FRAME, h2c->conn,
                  h2s);
      break;
    }
    ...
    if (h2c->st0 != H2_CS_FRAME_H) {
      if (h2c->dfl)
        TRACE_DEVEL("skipping remaining frame payload", H2_EV_RX_FRAME,
                    h2c->conn, h2s);
      ret = MIN(b_data(&h2c->dbuf), h2c->dfl);
      b_del(&h2c->dbuf, ret);
      h2c->dfl -= ret;
      if (!h2c->dfl) {
        TRACE_STATE("switching to FRAME_H", H2_EV_RX_FRAME | H2_EV_RX_FHDR,
                    h2c->conn);
  ...
  if (h2c->rcvd_c > 0 &&
    ...
    TRACE_PROTO("sending H2 WINDOW_UPDATE frame", H2_EV_TX_FRAME | H2_EV_TX_WU,
                h2c->conn);
...
fail:
  ...
  if (h2s && h2s->cs &&
    ...
    /* we may have to signal the upper layers */
    TRACE_DEVEL("notifying stream before switching SID",
                H2_EV_RX_FRAME | H2_EV_H2S_WAKE, h2c->conn, h2s);
    h2s->cs->flags |= CS_FL_RCV_MORE;
    h2s_notify_recv(h2s);
  }

  if (old_iw != h2c->miw) {
    TRACE_STATE("notifying streams about SFCTL increase",
                H2_EV_RX_FRAME | H2_EV_H2S_WAKE, h2c->conn);
...
static int h2_process_mux(struct h2c *h2c) {
  ...
  if (h2c->rcvd_s > 0 && !(h2c->flags & (H2_CF_MUX_MFULL | H2_CF_MUX_MALLOC)) &&
      h2c_send_strm_wu(h2c) < 0)
    goto fail;

  if (h2c->rcvd_c > 0 && !(h2c->flags & (H2_CF_MUX_MFULL | H2_CF_MUX_MALLOC)) &&
...
static int h2_recv(struct h2c *h2c) {
  ...
  if (max && !ret && h2_recv_allowed(h2c)) {
    TRACE_DATA("failed to receive data, subscribing", H2_EV_H2C_RECV,
               h2c->conn);
    conn->xprt->subscribe(conn, conn->xprt_ctx, SUB_RETRY_RECV,
                          &h2c->wait_event);
  } else if (ret)
    TRACE_DATA("received data", H2_EV_H2C_RECV, h2c->conn, 0, 0,
               (void *)(long)ret);
  ...
  while (!done) {
    ...
    for (buf = br_head(h2c->mbuf); b_size(buf); buf = br_del_head(h2c->mbuf)) {
      if (b_data(buf)) {
        ...
        sent = 1;
        TRACE_DATA("sent data", H2_EV_H2C_SEND, h2c->conn, 0, buf,
                   (void *)(long)ret);
        b_del(buf, ret);
        if (b_data(buf)) {
          done = 1;
...
schedule:
  if (!(conn->flags & CO_FL_ERROR) &&
      !(h2c->wait_event.events & SUB_RETRY_SEND)) {
...
static int h2_process(struct h2c *h2c) {
  ...
  if (unlikely(h2c->proxy->disabled) && !(h2c->flags & H2_CF_IS_BACK)) {
     ...
     */
    TRACE_STATE("proxy stopped, sending GOAWAY",
                H2_EV_H2C_WAKE | H2_EV_TX_FRAME, conn);
  ...
  if ((conn->flags & CO_FL_SOCK_WR_SH) || h2c->st0 == H2_CS_ERROR2 ||
      (h2c->flags & H2_CF_GOAWAY_FAILED) ||
      (h2c->st0 != H2_CS_ERROR && !br_data(h2c->mbuf) &&
  ...
  if (h2c->task) {
    if (h2c_may_expire(h2c))
      h2c->task->expire = tick_add(
          now_ms, h2c->last_sid < 0 ? h2c->timeout : h2c->shut_timeout);
    else
...
do_leave:
  ...
  if (br_data(h2c->mbuf) && !(h2c->flags & H2_CF_GOAWAY_FAILED) &&
    ...
    for (buf = br_head(h2c->mbuf); b_size(buf); buf = br_del_head(h2c->mbuf)) {
      if (b_data(buf)) {
        int ret = h2c->conn->xprt->snd_buf(h2c->conn, h2c->conn->xprt_ctx, buf,
                                           b_data(buf), 0);
        if (!ret)
          break;
        b_del(buf, ret);
...
static struct conn_stream *h2_attach(struct connection *conn,
  ...
  if (!cs) {
    TRACE_DEVEL("leaving on CS allocation failure",
                H2_EV_H2S_NEW | H2_EV_H2S_ERR, conn);
    return NULL;
  }
...
h2_get_first_cs(const struct connection *conn) {
  struct h2c *h2c = conn->ctx;
  struct h2s *h2s;
  struct eb32_node *node;
...
static int h2_ctl(struct connection *conn, enum mux_ctl_type mux_ctl,
                  void *output) {
  int ret = 0;
  struct h2c *h2c = conn->ctx;
...
static void h2_detach(struct conn_stream *cs) {
   ...
   */
  if (!(cs->conn->flags & CO_FL_ERROR) && (h2c->st0 < H2_CS_ERROR) &&
      (h2s->flags & (H2_SF_BLK_MBUSY | H2_SF_BLK_MROOM | H2_SF_BLK_MFCTL)) &&
      ((h2s->flags & (H2_SF_WANT_SHUTR | H2_SF_WANT_SHUTW)) || h2s->subs)) {
    TRACE_DEVEL("leaving on stream blocked", H2_EV_STRM_END | H2_EV_H2S_BLK,
                h2c->conn, h2s);
    return;
  }
      ...
      if (h2c->conn->flags & CO_FL_PRIVATE) {
        ...
        if (!session_add_conn(sess, h2c->conn, h2c->conn->target)) {
          ...
          if (eb_is_empty(&h2c->streams_by_id)) {
            ...
            TRACE_DEVEL("leaving on error after killing outgoing connection",
                        H2_EV_STRM_END | H2_EV_H2C_ERR);
            return;
          }
        }
        if (eb_is_empty(&h2c->streams_by_id)) {
          if (session_check_idle_conn(h2c->conn->owner, h2c->conn) != 0) {
            /* At this point either the connection is destroyed, or it's been
             * added to the server idle list, just stop */
            TRACE_DEVEL("leaving without reusable idle connection",
                        H2_EV_STRM_END);
            return;
          }
        }
        ...
        if (eb_is_empty(&h2c->streams_by_id)) {
          ...
          return;
                   ...
                   h2_avail_streams(h2c->conn) > 0 &&
                   objt_server(h2c->conn->target) &&
                   !LIST_ADDED(&h2c->conn->session_list)) {
          LIST_ADD(&__objt_server(h2c->conn->target)->available_conns[tid],
                   mt_list_to_list(&h2c->conn->list));
        }
      }
    }
  }
  ...
  if (h2c_is_dead(h2c)) {
    ...
    TRACE_DEVEL("leaving and killing dead connection", H2_EV_STRM_END,
                h2c->conn);
    h2_release(h2c);
  } else if (h2c->task) {
    if (h2c_may_expire(h2c))
      ...
      h2c->task->expire = TICK_ETERNITY;
    task_queue(h2c->task);
    TRACE_DEVEL("leaving, refreshing connection's timeout", H2_EV_STRM_END,
                h2c->conn);
  } else
...
static void h2_do_shutr(struct h2s *h2s) {
  ...
  if ((h2s->flags & H2_SF_KILL_CONN) &&
    ...
    TRACE_STATE("stream wants to kill the connection", H2_EV_STRM_SHUT,
                h2c->conn, h2s);
...
next_frame:
  ...
  if (unlikely(!(h2c->dff & H2_F_HEADERS_END_HEADERS))) {
    ...
    TRACE_STATE("EH missing, expecting continuation frame",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_HDR, h2c->conn);
    ...
    if (hdr.ft != H2_FT_CONTINUATION) {
      ...
      TRACE_STATE("not continuation!",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_HDR |
                      H2_EV_RX_CONT | H2_EV_H2C_ERR | H2_EV_PROTO_ERR,
                  h2c->conn);
      h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
      HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
      goto fail;
    ...
    if (hdr.sid != h2c->dsi) {
      ...
      TRACE_STATE("different stream ID!",
                  H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_HDR |
                      H2_EV_RX_CONT | H2_EV_H2C_ERR | H2_EV_PROTO_ERR,
                  h2c->conn);
      h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
     ...
     */
    b_move(&h2c->dbuf, b_peek_ofs(&h2c->dbuf, h2c->dfl + hole + 9), clen,
           -(h2c->dpl + hole + 9));
    h2c->dfl += clen - h2c->dpl;
    hole += h2c->dpl + 9;
    h2c->dpl = 0;
    TRACE_STATE("waiting for next continuation frame",
                H2_EV_RX_FRAME | H2_EV_RX_FHDR | H2_EV_RX_CONT | H2_EV_RX_HDR,
                h2c->conn);
    goto next_frame;
  }
  ...
  if (wrap < h2c->dfl) {
    ...
    if (!copy) {
      TRACE_DEVEL("failed to allocate temporary buffer",
                  H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_ERR, h2c->conn);
      h2c_error(h2c, H2_ERR_INTERNAL_ERROR);
      goto fail;
    }
  ...
  if (h2c->dff & H2_F_HEADERS_PRIORITY) {
    if (read_n32(hdrs) == h2c->dsi) {
      ...
      TRACE_STATE("invalid stream dependency!",
                  H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_ERR |
                      H2_EV_PROTO_ERR,
                  h2c->conn);
      h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
      HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
    ...
    if (flen < 5) {
      TRACE_STATE("frame too short for priority!",
                  H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_ERR |
                      H2_EV_PROTO_ERR,
                  h2c->conn);
      h2c_error(h2c, H2_ERR_FRAME_SIZE_ERROR);
  ...
  if (!h2_get_buf(h2c, rxbuf)) {
    TRACE_STATE("waiting for h2c rxbuf allocation",
                H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_BLK, h2c->conn);
    h2c->flags |= H2_CF_DEM_SALLOC;
    goto leave;
  }
  ...
  if (!htx_is_empty(htx)) {
    TRACE_STATE("waiting for room in h2c rxbuf",
                H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_BLK, h2c->conn);
  ...
  if (outlen < 0) {
    TRACE_STATE("failed to decompress HPACK",
                H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_ERR | H2_EV_PROTO_ERR,
                h2c->conn);
  ...
  if (outlen < 0) {
    ...
    TRACE_STATE("request headers too large",
                H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2S_ERR | H2_EV_PROTO_ERR,
                h2c->conn);
    goto fail;
  }
  ...
  if ((h2c->dff & H2_F_HEADERS_END_STREAM)) {
    ...
    if (!htx_add_endof(htx, HTX_BLK_EOM)) {
      TRACE_STATE("failed to append HTX EOM block into rxbuf",
...
trailers:
  ...
  if (!(h2c->dff & H2_F_HEADERS_END_STREAM)) {
    ...
    TRACE_STATE("missing EH on trailers frame",
                H2_EV_RX_FRAME | H2_EV_RX_HDR | H2_EV_H2C_ERR | H2_EV_PROTO_ERR,
                h2c->conn);
    h2c_error(h2c, H2_ERR_PROTOCOL_ERROR);
    HA_ATOMIC_ADD(&h2c->px_counters->conn_proto_err, 1);
...
static int h2_frt_transfer_data(struct h2s *h2s) {
  ...
  if (!csbuf) {
    h2c->flags |= H2_CF_DEM_SALLOC;
    TRACE_STATE("waiting for an h2s rxbuf",
                H2_EV_RX_FRAME | H2_EV_RX_DATA | H2_EV_H2S_BLK, h2c->conn, h2s);
    goto fail;
  }
...
try_again:
  ...
  TRACE_DATA("move some data to h2s rxbuf", H2_EV_RX_FRAME | H2_EV_RX_DATA,
             h2c->conn, h2s, 0, (void *)(long)sent);
...
end_transfer:
  ...
  if (h2c->dff & H2_F_DATA_END_STREAM) {
    htx->flags |=
        HTX_FL_EOI; /* no more data are expected. Only EOM remains to add now */
    if (!htx_add_endof(htx, HTX_BLK_EOM)) {
      TRACE_STATE("h2s rxbuf is full, failed to add EOM",
                  H2_EV_RX_FRAME | H2_EV_RX_DATA | H2_EV_H2S_BLK, h2c->conn,
                  h2s);
      h2c->flags |= H2_CF_DEM_SFULL;
      goto fail;
    }
...
static size_t h2s_frt_make_resp_headers(struct h2s *h2s, struct htx *htx) {
  ...
  if (h2c_mux_busy(h2c, h2s)) {
    TRACE_STATE("mux output busy", H2_EV_TX_FRAME | H2_EV_TX_HDR, h2c->conn,
                h2s);
    h2s->flags |= H2_SF_BLK_MBUSY;
  ...
  if (h2s->status < 100 || h2s->status > 999) {
    TRACE_ERROR("will not encode an invalid status code",
                H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_H2S_ERR, h2c->conn, h2s);
    goto fail;
  }
  ...
  while ((idx = htx_get_next(htx, idx)) != -1) {
    ...
    if (unlikely(hdr >= sizeof(list) / sizeof(list[0]) - 1)) {
      TRACE_ERROR("too many headers",
                  H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_H2S_ERR, h2c->conn,
                  h2s);
      goto fail;
    }
...
retry:
  if (!h2_get_buf(h2c, mbuf)) {
    ...
    TRACE_STATE("waiting for room in output buffer",
                H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_H2S_BLK, h2c->conn, h2s);
    return 0;
  }
  ...
  TRACE_USER("sent H2 response", H2_EV_TX_FRAME | H2_EV_TX_HDR, h2c->conn, h2s,
             htx);
  ...
  if (es_now) {
    h2s->flags |= H2_SF_ES_SENT;
    TRACE_PROTO("setting ES on HEADERS frame", H2_EV_TX_FRAME | H2_EV_TX_HDR,
                h2c->conn, h2s, htx);
    if (h2s->st == H2_SS_OPEN)
      h2s->st = H2_SS_HLOC;
    else
...
full:
  ...
  h2s->flags |= H2_SF_BLK_MROOM;
  ret = 0;
  TRACE_STATE("mux buffer full", H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_H2S_BLK,
              h2c->conn, h2s);
  goto end;
fail:
...
static size_t h2s_bck_make_req_headers(struct h2s *h2s, struct htx *htx) {
  ...
  if (unlikely(uri.len == 0)) {
    TRACE_ERROR("no URI in HTX request",
                H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_H2S_ERR, h2c->conn, h2s);
    goto fail;
  }
  ...
  while ((idx = htx_get_next(htx, idx)) != -1) {
    ...
    if ((h2c->flags & H2_CF_IS_BACK) && h2c->proxy->server_id_hdr_name &&
        isteq(list[hdr].n, ist2(h2c->proxy->server_id_hdr_name,
...
retry:
  ...
  if (unlikely(sl->info.req.meth == HTTP_METH_CONNECT)) {
    ...
    if (!scheme.len) {
       ...
       */
      if ((sl->flags & (HTX_SL_F_HAS_SCHM | HTX_SL_F_SCHM_HTTP)) ==
          (HTX_SL_F_HAS_SCHM | HTX_SL_F_SCHM_HTTP))
        scheme = ist("http");
      else
        scheme = ist("https");
    }
  ...
  for (hdr = 0; hdr < sizeof(list) / sizeof(list[0]); hdr++) {
    ...
    /* these ones do not exist in H2 and must be dropped. */
    if (isteq(n, ist("connection")) || (auth.len && isteq(n, ist("host"))) ||
        isteq(n, ist("proxy-connection")) || isteq(n, ist("keep-alive")) ||
        isteq(n, ist("upgrade")) || isteq(n, ist("transfer-encoding")))
      continue;
  ...
  /* commit the H2 response */
  TRACE_USER("sent H2 request", H2_EV_TX_FRAME | H2_EV_TX_HDR, h2c->conn, h2s,
             htx);
  b_add(mbuf, outbuf.data);
  h2s->flags |= H2_SF_HEADERS_SENT;
  h2s->st = H2_SS_OPEN;
...
static size_t h2s_frt_make_resp_data(struct h2s *h2s, struct buffer *buf,
  ...
  if (h2c_mux_busy(h2c, h2s)) {
    TRACE_STATE("mux output busy", H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn,
                h2s);
    h2s->flags |= H2_SF_BLK_MBUSY;
...
retry:
  if (!h2_get_buf(h2c, mbuf)) {
    ...
    h2s->flags |= H2_SF_BLK_MROOM;
    TRACE_STATE("waiting for room in output buffer",
                H2_EV_TX_FRAME | H2_EV_TX_DATA | H2_EV_H2S_BLK, h2c->conn, h2s);
    goto end;
  }
   ...
   * from the HTX blocks.
   */
  if (unlikely(fsize == count && htx_nbblks(htx) == 1 && type == HTX_BLK_DATA &&
               fsize <= h2s_mws(h2s) && fsize <= h2c->mws &&
               fsize <= h2c->mfs)) {
    ...
    if (b_data(mbuf)) {
      ...
      if (fsize + 9 <= b_room(mbuf) &&
        ...
        TRACE_STATE("small data present in output buffer, appending",
                    H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn, h2s);
        goto copy;
      }
      ...
      h2s->flags |= H2_SF_BLK_MROOM;
      TRACE_STATE(
          "too large data present in output buffer, waiting for emptiness",
          H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn, h2s);
      goto end;
    }
     ...
     * frame header there.
     */
    *mbuf = b_make(buf->area, buf->size, sizeof(struct htx) + blk->addr - 9,
                   fsize + 9);
    ...
    TRACE_PROTO("sent H2 DATA frame (zero-copy)",
                H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn, h2s);
    goto end;
  }
...
copy:
  ...
  if (outbuf.size < 9) {
    ...
    h2c->flags |= H2_CF_MUX_MFULL;
    h2s->flags |= H2_SF_BLK_MROOM;
    TRACE_STATE("output buffer full", H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn,
                h2s);
    goto end;
  }
  ...
  if (h2s_mws(h2s) <= 0) {
    ...
    TRACE_STATE("stream window <=0, flow-controlled",
                H2_EV_TX_FRAME | H2_EV_TX_DATA | H2_EV_H2S_FCTL, h2c->conn,
                h2s);
    goto end;
  }
  ...
  if (fsize + 9 > outbuf.size) {
     ...
     * buffer now.
     */
    if (b_space_wraps(mbuf) && (fsize + 9 <= b_room(mbuf)) &&
    ...
    if (fsize <= 0) {
      ...
      h2c->flags |= H2_CF_MUX_MFULL;
      h2s->flags |= H2_SF_BLK_MROOM;
      TRACE_STATE("output buffer full", H2_EV_TX_FRAME | H2_EV_TX_DATA,
                  h2c->conn, h2s);
      goto end;
    }
  }
  ...
  if (h2c->mws <= 0) {
    ...
    TRACE_STATE("connection window <=0, stream flow-controlled",
                H2_EV_TX_FRAME | H2_EV_TX_DATA | H2_EV_H2C_FCTL, h2c->conn,
                h2s);
    goto end;
  }
...
send_empty:
  ...
  if (fsize == bsize) {
    htx_remove_blk(htx, blk);
    if (fsize) {
      TRACE_DEVEL("more data available, trying to send another frame",
                  H2_EV_TX_FRAME | H2_EV_TX_DATA, h2c->conn, h2s);
      goto new_frame;
    }
  } else {
  ...
  if (es_now) {
    ...
    h2s->flags |= H2_SF_ES_SENT;
    TRACE_PROTO("ES flag set on outgoing frame",
                H2_EV_TX_FRAME | H2_EV_TX_DATA | H2_EV_TX_EOI, h2c->conn, h2s);
  }
...
retry:
  ...
  for (idx = 0; idx < hdr; idx++) {
    ...
    if (isteq(list[idx].n, ist("host")) ||
        ...
        isteq(list[idx].n, ist("upgrade")) || isteq(list[idx].n, ist("te")) ||
  ...
  /* commit the H2 response */
  TRACE_PROTO("sent H2 trailers HEADERS frame",
              H2_EV_TX_FRAME | H2_EV_TX_HDR | H2_EV_TX_EOI, h2c->conn, h2s);
  b_add(mbuf, outbuf.data);
  h2s->flags |= H2_SF_ES_SENT;
 ...
 */
static int h2_subscribe(struct conn_stream *cs, int event_type,
  ...
  if (event_type & SUB_RETRY_SEND) {
    ...
    if (!(h2s->flags & H2_SF_BLK_SFCTL) && !LIST_ADDED(&h2s->list)) {
      if (h2s->flags & H2_SF_BLK_MFCTL)
        LIST_ADDQ(&h2c->fctl_list, &h2s->list);
      else
        LIST_ADDQ(&h2c->send_list, &h2s->list);
 ...
 */
static int h2_unsubscribe(struct conn_stream *cs, int event_type,
                          struct wait_event *es) {
  struct h2s *h2s = cs->ctx;
...
static size_t h2_rcv_buf(struct conn_stream *cs, struct buffer *buf,
 ...
 */
static size_t h2_snd_buf(struct conn_stream *cs, struct buffer *buf,
                         size_t count, int flags) {
  struct h2s *h2s = cs->ctx;
  size_t total = 0;
  size_t ret;
   ...
   */
  if (!(h2s->flags & H2_SF_NOTIFIED) && (!LIST_ISEMPTY(&h2s->h2c->send_list) ||
                                         !LIST_ISEMPTY(&h2s->h2c->fctl_list))) {
    TRACE_DEVEL("other streams already waiting, going to the queue and leaving",
                H2_EV_H2S_SEND | H2_EV_H2S_BLK, h2s->h2c->conn, h2s);
    return 0;
  }
  h2s->flags &= ~H2_SF_NOTIFIED;

  if (h2s->h2c->st0 < H2_CS_FRAME_H) {
    TRACE_DEVEL("connection not ready, leaving", H2_EV_H2S_SEND | H2_EV_H2S_BLK,
                h2s->h2c->conn, h2s);
    return 0;
  }
  ...
  if (h2s->h2c->st0 >= H2_CS_ERROR) {
    cs->flags |= CS_FL_ERROR;
    TRACE_DEVEL("connection is in error, leaving in error",
                H2_EV_H2S_SEND | H2_EV_H2S_BLK | H2_EV_H2S_ERR | H2_EV_STRM_ERR,
                h2s->h2c->conn, h2s);
    return 0;
  }
  ...
  while (h2s->st < H2_SS_HLOC && !(h2s->flags & H2_SF_BLK_ANY) && count &&
         !htx_is_empty(htx)) {
    idx = htx_get_head(htx);
    blk = htx_get_blk(htx, idx);
  ...
  if (h2s->st == H2_SS_ERROR || h2s->flags & H2_SF_RST_RCVD) {
    TRACE_DEVEL("reporting RST/error to the app-layer stream",
                H2_EV_H2S_SEND | H2_EV_H2S_ERR | H2_EV_STRM_ERR, h2s->h2c->conn,
                h2s);
    cs_set_error(cs);
   ...
   * the conn_stream.
   */
  if (conn_xprt_read0_pending(h2s->h2c->conn) && !b_data(&h2s->h2c->dbuf) &&
      (h2s->flags & (H2_SF_BLK_SFCTL | H2_SF_BLK_MFCTL))) {
    TRACE_DEVEL("fctl with shutr, reporting error to app-layer",
                H2_EV_H2S_SEND | H2_EV_STRM_SEND | H2_EV_STRM_ERR,
                h2s->h2c->conn, h2s);
  ...
  if (total > 0 && !(h2s->flags & H2_SF_BLK_SFCTL) &&
      !(h2s->flags & (H2_SF_WANT_SHUTR | H2_SF_WANT_SHUTW))) {
    /* Ok we managed to send something, leave the send_list if we were still
     * there */
    LIST_DEL_INIT(&h2s->list);
  }
...
static void h2_show_fd(struct buffer *msg, struct connection *conn) {
  ...
  list_for_each_entry(h2s, &h2c->fctl_list, list) fctl_cnt++;

  list_for_each_entry(h2s, &h2c->send_list, list) send_cnt++;
  ...
  chunk_appendf(msg,
                " h2c.st0=%s .err=%d .maxid=%d .lastid=%d .flg=0x%04x"
                " .nbst=%u .nbcs=%u .fctl_cnt=%d .send_cnt=%d .tree_cnt=%d"
                " .orph_cnt=%d .sub=%d .dsi=%d .dbuf=%u@%p+%u/%u .msi=%d"
                " .mbuf=[%u..%u|%u],h=[%u@%p+%u/%u],t=[%u@%p+%u/%u]",
                h2c_st_to_str(h2c->st0), h2c->errcode, h2c->max_id,
                h2c->last_sid, h2c->flags, h2c->nb_streams, h2c->nb_cs,
                fctl_cnt, send_cnt, tree_cnt, orph_cnt, h2c->wait_event.events,
                h2c->dsi, (unsigned int)b_data(&h2c->dbuf), b_orig(&h2c->dbuf),
                (unsigned int)b_head_ofs(&h2c->dbuf),
                (unsigned int)b_size(&h2c->dbuf), h2c->msi,
                br_head_idx(h2c->mbuf), br_tail_idx(h2c->mbuf),
                br_size(h2c->mbuf), (unsigned int)b_data(hmbuf), b_orig(hmbuf),
  ...
  if (h2s) {
    chunk_appendf(
        msg, " last_h2s=%p .id=%d .st=%s.flg=0x%04x .rxbuf=%u@%p+%u/%u .cs=%p",
        h2s, h2s->id, h2s_st_to_str(h2s->st), h2s->flags,
        (unsigned int)b_data(&h2s->rxbuf), b_orig(&h2s->rxbuf),
        (unsigned int)b_head_ofs(&h2s->rxbuf),
        (unsigned int)b_size(&h2s->rxbuf), h2s->cs);
    if (h2s->cs)
      chunk_appendf(msg, " .cs.flg=0x%08x .cs.data=%p", h2s->cs->flags,
                    h2s->cs->data);
  }
}
...
static int h2_takeover(struct connection *conn, int orig_tid) {
  ...
  h2c->wait_event.tasklet->process = h2_io_cb;
  h2c->wait_event.tasklet->context = h2c;
  h2c->conn->xprt->subscribe(h2c->conn, h2c->conn->xprt_ctx, SUB_RETRY_RECV,
                             &h2c->wait_event);
...
static int h2_parse_header_table_size(char **args, int section_type,
  ...
  if (h2_settings_header_table_size < 4096 ||
      h2_settings_header_table_size > 65536) {
    memprintf(err, "'%s' expects a numeric value between 4096 and 65536.",
              args[0]);
    return -1;
  }
  return 0;
...
static int h2_parse_initial_window_size(char **args, int section_type,
                                        struct proxy *curpx,
...
static int h2_parse_max_frame_size(char **args, int section_type,
                                   struct proxy *curpx, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  if (too_many_args(1, args, err, NULL))
    return -1;
  ...
  if (h2_settings_max_frame_size < 16384 ||
      h2_settings_max_frame_size > 16777215) {
    memprintf(err, "'%s' expects a numeric value between 16384 and 16777215.",
              args[0]);
    return -1;
  }
  return 0;
...
static struct mux_proto_list mux_proto_h2 = {.token = IST("h2"),
                                             .mode = PROTO_MODE_HTTP,
                                             .side = PROTO_SIDE_BOTH,
                                             .mux = &h2_ops};
 ...
 */
static int init_h2() {
  pool_head_hpack_tbl = create_pool("hpack_tbl", h2_settings_header_table_size,
