...
static struct task *mux_pt_io_cb(struct task *t, void *tctx,
...
static int mux_pt_wake(struct connection *conn) {
  ...
  if ((conn->flags & (CO_FL_EARLY_DATA | CO_FL_EARLY_SSL_HS |
                      CO_FL_WAIT_XPRT)) == CO_FL_EARLY_DATA)
...
static struct conn_stream *mux_pt_attach(struct connection *conn,
  ...
  if (ctx->wait_event.events)
    conn->xprt->unsubscribe(ctx->conn, conn->xprt_ctx, SUB_RETRY_RECV,
                            &ctx->wait_event);
  cs = cs_new(conn, conn->target);
  if (!cs)
    goto fail;
...
mux_pt_get_first_cs(const struct connection *conn) {
  struct mux_pt_ctx *ctx = conn->ctx;
  struct conn_stream *cs = ctx->cs;
 ...
 */
static size_t mux_pt_rcv_buf(struct conn_stream *cs, struct buffer *buf,
  ...
  ret =
      cs->conn->xprt->rcv_buf(cs->conn, cs->conn->xprt_ctx, buf, count, flags);
...
static size_t mux_pt_snd_buf(struct conn_stream *cs, struct buffer *buf,
  ...
  ret =
      cs->conn->xprt->snd_buf(cs->conn, cs->conn->xprt_ctx, buf, count, flags);
 ...
 */
static int mux_pt_subscribe(struct conn_stream *cs, int event_type,
                            struct wait_event *es) {
  return cs->conn->xprt->subscribe(cs->conn, cs->conn->xprt_ctx, event_type,
                                   es);
}
 ...
 */
static int mux_pt_unsubscribe(struct conn_stream *cs, int event_type,
                              struct wait_event *es) {
  return cs->conn->xprt->unsubscribe(cs->conn, cs->conn->xprt_ctx, event_type,
                                     es);
}

#if defined(USE_LINUX_SPLICE)
/* Send and get, using splicing */
static int mux_pt_rcv_pipe(struct conn_stream *cs, struct pipe *pipe,
                           unsigned int count) {
  int ret;
...
static int mux_pt_ctl(struct connection *conn, enum mux_ctl_type mux_ctl,
                      void *output) {
  int ret = 0;
  switch (mux_ctl) {
  case MUX_STATUS:
...
static struct mux_proto_list mux_proto_pt = {.token = IST(""),
                                             .mode = PROTO_MODE_TCP,
