...
#define MAKE_KEY(class, offset)                                                \
  (((u32)(class + 0x7ff) << 20) | ((u32)(now_ms + offset) & 0xfffff))
...
unsigned int srv_dynamic_maxconn(const struct server *s) {
  ...
  if (s->proxy->beconn >= s->proxy->fullconn)
    ...
    max = MAX(s->minconn, s->proxy->beconn * s->maxconn / s->proxy->fullconn);
...
static struct pendconn *pendconn_first(struct eb_root *pendconns) {
  ...
  if (!node2 || KEY_CLASS(node2->key) != KEY_CLASS(node->key)) {
 ...
 * the pendconn itself. If the stream was already marked as served, its flag is
...
struct pendconn *pendconn_add(struct stream *strm) {
  ...
  if (srv) {
    ...
    while (new_max > old_max) {
      if (likely(
              _HA_ATOMIC_CAS(&px->be_counters.nbpend_max, &old_max, new_max)))
        break;
    }
    __ha_barrier_atomic_store();
...
static enum act_return action_set_priority_class(struct act_rule *rule,
                                                 struct proxy *px,
                                                 struct session *sess,
...
static enum act_parse_ret parse_set_priority_class(const char **args, int *arg,
                                                   struct proxy *px,
...
static enum act_parse_ret parse_set_priority_offset(const char **args, int *arg,
                                                    struct proxy *px,
                                                    struct act_rule *rule,
                                                    char **err) {
  unsigned int where = 0;
...
static struct action_kw_list tcp_cont_kws = {
    ILH,
...
static int smp_fetch_priority_class(const struct arg *args, struct sample *smp,
...
static int smp_fetch_priority_offset(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  if (!smp->strm)
    return 0;
