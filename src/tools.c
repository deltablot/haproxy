...
static inline const char *__invalid_char(const char *name, int (*f)(int)) {
  ...
  while (*name) {
    if (!f((unsigned char)*name) && *name != '.' && *name != '_' &&
        *name != '-')
      return name;
 ...
 * IPv6 addresses, the last colon ':' is mandatory even when no port is
...
struct sockaddr_storage *str2sa_range(const char *str, int *port, int *low,
  ...
  if (!*str2) {
    memprintf(
        err,
        "'%s' resolves to an empty address (environment variable missing?)\n",
        str);
    goto out;
  }
  ...
  if (ss.ss_family == AF_CUST_SOCKPAIR) {
    ...
    /* when abstract==1, we skip the first zero and copy all bytes except the
     * trailing zero */
    memset(un->sun_path, 0, sizeof(un->sun_path));
    if (prefix_path_len)
      memcpy(un->sun_path, pfx, prefix_path_len);
    memcpy(un->sun_path + prefix_path_len + abstract, str2,
           adr_len + 1 - abstract);
  } else { /* IPv4 and IPv6 */
  ...
  if (proto || (opts & PA_O_CONNECT)) {
     ...
     */
    new_proto = protocol_lookup(ss.ss_family, sock_type == SOCK_DGRAM,
    ...
    if ((opts & PA_O_CONNECT) && new_proto && !new_proto->connect) {
      memprintf(err,
                "connect() not supported for this protocol family %d used by "
                "address '%s'",
                ss.ss_family, str);
      goto out;
    }
 ...
 */
int str2net(const char *str, int resolve, struct in_addr *addr,
            struct in_addr *mask) {
...
int url2sa(const char *url, int ulen, struct sockaddr_storage *addr,
  ...
  if (*curr == '[') {
    ...
    p = trash.area;
    for (end = curr; end < url + ulen && *end != ']'; end++, p++)
      *p = *end;
    if (*end != ']')
      return -1;
    *p = '\0';
    ...
    /* Try IPv6 decoding. */
    if (!inet_pton(AF_INET6, trash.area,
    ...
    if (ret) {
      ...
      /* The IPv4 and IPv6 decoding fails, maybe the url contain name. Try to
       * execute synchronous DNS request only if HAProxy is in the start state.
       */

      /* look for : or / or end */
      for (end = curr; end < url + ulen && *end != '/' && *end != ':'; end++)
        ;
...
char *encode_string(char *start, char *stop, const char escape, const long *map,
                    const char *string) {
  if (start < stop) {
 ...
 */
char *encode_chunk(char *start, char *stop, const char escape, const long *map,
 ...
 * completes.
 */
char *escape_string(char *start, char *stop, const char escape, const long *map,
                    const char *string) {
  if (start < stop) {
    stop--; /* reserve one byte for the final '\0' */
    while (start < stop && *string != '\0') {
 ...
 */
char *escape_chunk(char *start, char *stop, const char escape, const long *map,
                   const struct buffer *chunk) {
  char *str = chunk->area;
  char *end = chunk->area + chunk->data;
...
unsigned int str2ui(const char *s) { return __str2ui(s); }

unsigned int str2uic(const char *s) { return __str2uic(s); }
 ...
 * returns it. The <s> pointer is adjusted to point to the first unread char.
 * The function automatically stops at <end>. If the number overflows, the
 * 2^64-1 value is returned.
 */
 ...
 * it. The <s> pointer is adjusted to point to the first unread char. The
 * function automatically stops at <end>. Il the number is bigger than 2^63-2,
 * the 2^63-1 value is returned. If the number is lowest than -2^63-1, the -2^63
 * value is returned.
 */
...
int strl2irc(const char *s, int len, int *ret) {
  ...
  if (*s != '-') {
    ...
    while (len-- > 0) {
      ...
      if (i > INT_MAX / 10)
        return 1; /* check for multiply overflow */
      i = i * 10;
      if (i + j < i)
        return 1; /* check for addition overflow */
      i = i + j;
    }
  } else {
    /* negative number */
    ...
    while (--len > 0) {
      ...
      i = i * 10;
      if (i - j > i)
        return 1; /* check for subtract overflow */
      i = i - j;
    }
  }
  *ret = i;
 ...
 */
const char *parse_time_err(const char *text, unsigned *ret,
                           unsigned unit_flags) {
  ...
  if (*(++text) != '\0') {
    ha_warning("unexpected character '%c' after the timer value '%s', only "
               "(us=microseconds,ms=milliseconds,s=seconds,m=minutes,h=hours,d="
               "days) are supported."
 ...
 */
int parse_binary(const char *source, char **binstr, int *binstrlen,
                 char **err) {
  int len;
  const char *p = source;
  int i, j;
 ...
 */
const void *my_memmem(const void *haystack, size_t haystacklen,
...
size_t my_memspn(const void *str, size_t len, const void *accept,
                 size_t acceptlen) {
  size_t ret = 0;
...
/* get length of the initial segment consisting entirely of bytes not in
 * <rejcet> */
size_t my_memcspn(const void *str, size_t len, const void *reject,
                  size_t rejectlen) {
  size_t ret = 0;
...
void eb32sc_to_file(FILE *file, struct eb_root *root,
                    const struct eb32sc_node *subj, int op, const char *desc) {
  ...
  fprintf(file,
          "  node [fontname=\"fixed\" fontsize=8 shape=\"box\" "
          "style=\"filled\" color=\"black\" fillcolor=\"white\"];\n"
          "  edge [fontname=\"fixed\" fontsize=8 style=\"solid\" "
          "color=\"magenta\" dir=\"forward\"];\n"
          "  \"%lx_n\" [label=\"root\\n%lx\"]\n",
  ...
  while (node) {
    if (node->node.node_p) {
      ...
      fprintf(file,
              "  \"%lx_n\" [label=\"%lx\\nkey=%u\\nscope=%lx\\nbit=%d\" "
              "fillcolor=\"lightskyblue1\" %s];\n",
              (long)node, (long)node, node->key, node->node_s, node->node.bit,
              (node == subj) ? (op < 0 ? "color=\"red\" style=\"dashed\""
                                       : op > 0 ? "color=\"red\"" : "")
                             : "");

      fprintf(file, "  \"%lx_n\" -> \"%lx_n\" [taillabel=\"%c\"];\n",
              (long)node, (long)eb_root_to_node(eb_clrtag(node->node.node_p)),
 ...
 * by one of the functions above. Also, apparently this function does not
 * support hosts above 255 and requires exactly 4 octets. The destination is
...
unsigned int full_hash(unsigned int a) { return __full_hash(a); }
 ...
 * above.
 */
void mask_prep_rank_map(unsigned long m, unsigned long *a, unsigned long *b,
                        unsigned long *c, unsigned long *d) {
  *a = m - ((m >> 1) & ~0UL / 3);
  *b = (*a & ~0UL / 5) + ((*a >> 2) & ~0UL / 5);
  *c = (*b + (*b >> 4)) & ~0UL / 0x11;
 ...
 */
int in_net_ipv4(const void *addr, const struct in_addr *mask,
 ...
 * while the two other ones must.
 */
int in_net_ipv6(const void *addr, const struct in6_addr *mask,
...
/* RFC 4291 prefix */
const char rfc4291_pfx[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
 ...
 */
struct sockaddr_storage *ipcpy(struct sockaddr_storage *source,
  ...
  case AF_INET6:
    memcpy(((struct sockaddr_in6 *)dest)->sin6_addr.s6_addr,
           ((struct sockaddr_in6 *)source)->sin6_addr.s6_addr,
 ...
 */
char *date2str_log(char *dst, const struct tm *tm, const struct timeval *date,
                   size_t size) {
  ...
  *dst++ = '.';
  ...
  dst = utoa_pad((unsigned int)(date->tv_usec / 1000) % 1000, dst,
                 4); // milliseconds
  if (!dst)
    return NULL;
  *dst = '\0';
 ...
 * ignored). Retrieved from glibc 2.18 source code.
 */
 ...
 * tm). Offsets are kept in an internal cache for better performances.
 */
...
time_t my_timegm(const struct tm *tm) {
   ...
   */
  uint64_t extra = (0ULL << 0 * 5) + (3ULL << 1 * 5) +
                   (3ULL << 2 * 5) + /* Jan, Feb, Mar, */
                   (6ULL << 3 * 5) + (8ULL << 4 * 5) +
                   (11ULL << 5 * 5) + /* Apr, May, Jun, */
                   (13ULL << 6 * 5) + (16ULL << 7 * 5) +
                   (19ULL << 8 * 5) + /* Jul, Aug, Sep, */
                   (21ULL << 9 * 5) + (24ULL << 10 * 5) +
                   (26ULL << 11 * 5); /* Oct, Nov, Dec, */
 ...
 */
static inline int parse_http_dayname(const char **date, int *len,
                                     struct tm *tm) {
  if (*len < 3)
    return 0;
 ...
 * This array must be alphabetically sorted
 */
static inline int parse_http_monthname(const char **date, int *len,
                                       struct tm *tm) {
  if (*len < 3)
    return 0;
  switch (STR2I3((*date)[0], (*date)[1], (*date)[2])) {
 ...
 * This array must be alphabetically sorted
 */
static inline int parse_http_ldayname(const char **date, int *len,
                                      struct tm *tm) {
...
/* This function parses exactly 1 digit and returns the numeric value in
 * "digit". */
...
/* This function parses exactly 2 digits and returns the numeric value in
 * "digit". */
...
/* This function parses exactly 4 digits and returns the numeric value in
 * "digit". */
...
int parse_imf_date(const char *date, int len, struct tm *tm) {
  ...
  RET0_UNLESS(parse_expect_char(&date, &len, ' ')); /* expect SP */
  RET0_UNLESS(parse_strcmp(&date, &len, "GMT",
                           3)); /* GMT = %x47.4D.54 ; "GMT", case-sensitive */
  tm->tm_isdst = -1;
  return 1;
}
...
/* Used to add <level> spaces before each line of <out>, unless there is only
 * one line. The input argument is automatically freed and reassigned. The
 * result will have to be freed by the caller. It also supports being passed a
 * NULL which results in the same output. Example of use : parse(cmd, &err);
 * (callee: memprintf(&err, ...)) fprintf(stderr, "Parser said: %s\n",
 * indent_error(&err)); free(err);
 */
char *indent_msg(char **out, int level) {
  ...
  /* copy each line, prefixed with LF and <level> spaces, and without the
   * trailing LF */
  while (*in) {
    *p++ = '\n';
    memset(p, ' ', level);
 ...
 */
int append_prefixed_str(struct buffer *out, const char *in, const char *pfx,
                        char eol, int first) {
  int bol, lf;
...
const char *strnistr(const char *str1, int len_str1, const char *str2,
                     int len_str2) {
  ...
  if (str1 == NULL ||
      len_str1 ==
          0) // search pattern into an empty string => search is not found
    return NULL;
  ...
  if (len_str1 <
      len_str2) // pattern is longer than string => search is not found
    return NULL;

  for (tmp1 = 0, start = (char *)str1, pptr = (char *)str2, slen = len_str1,
      plen = len_str2;
    ...
    while (toupper((unsigned char)*sptr) == toupper((unsigned char)*pptr)) {
      ...
      if (*pptr == '\0' || tmp2 == len_str2) /* end of pattern found */
        return start;
      if (*sptr == '\0' || tmp2 == len_str1) /* end of string found and the
                                                pattern is not fully found */
        return NULL;
    }
  }
  return NULL;
 ...
 * Other non-printable chars are encoded "\xHH". Space, '\', and '=' are also
 * escaped. Print stopped if null char or <bsize> is reached, or if no more
 * place in the chunk.
 */
...
void dump_hex(struct buffer *out, const char *pfx, const void *buf, int len,
  ...
  for (i = 0; i < start + len; i += 16) {
    chunk_appendf(out, (sizeof(void *) == 4) ? "%s%8p: " : "%s%16p: ", pfx,
                  d + i);
 ...
 */
void dump_addr_and_bytes(struct buffer *buf, const char *pfx, const void *addr,
                         int n) {
  int ok = 0;
  int i;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (ok)
      chunk_appendf(buf, "%02x%s", ((uint8_t *)addr)[i],
                    (i < n - 1) ? " " : "]");
    else
...
static int dladdr_and_size(const void *addr, Dl_info *dli, size_t *size) {
  int ret;
#if (__GLIBC__ > 2 ||                                                          \
     (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 3)) // most detailed one
  const ElfW(Sym) * sym;
...
void ha_generate_uuid(struct buffer *output) {
  ...
  chunk_printf(output, "%8.8x-%4.4x-%4.4x-%4.4x-%12.12llx", rnd[0],
               rnd[1] & 0xFFFF,
               ((rnd[1] >> 16u) & 0xFFF) |
                   0x4000, // highest 4 bits indicate the uuid version
               (rnd[2] & 0x3FFF) |
                   0x8000, // the highest 2 bits indicate the UUID variant (10),
 ...
 */
uint32_t parse_line(char *in, char *out, size_t *outlen, char **args,
                    int *nbargs, uint32_t opts, char **errptr) {
  char *quote = NULL;
  char *brace = NULL;
  ...
  while (1) {
    if (*in >= '-' && *in != '\\') {
      ...
      break;
               ...
               (opts &
                PARSE_OPT_DQUOTE)) { /* double quote outside single quotes */
      if (dquote) {
        dquote = 0;
        quote = NULL;
      } else {
      ...
      continue;
               ...
               (opts &
                PARSE_OPT_SQUOTE)) { /* single quote outside double quotes */
      if (squote) {
        squote = 0;
        quote = NULL;
      } else {
       ...
       * double quotes for some of them. Other combinations left unchanged (eg:
       * \1).
       */
      char tosend = *in;
      ...
      case '\'':
        /* escaping of "'" only outside single quotes and only if single quotes
         * are supported */
      ...
      case '"':
        /* escaping of '"' only outside single quotes and only if double quotes
         * are supported */
    ...
    } else if (*in == '$' && (opts & PARSE_OPT_ENV) &&
               (dquote || !(opts & PARSE_OPT_DQUOTE))) {
...
/* This is used to sanitize an input line that's about to be used for error
 * reporting. It will adjust <line> to print approximately <width> chars around
 * <pos>, trying to preserve the beginning, with leading or trailing "..." when
 * the line is truncated. If non-printable chars are present in the output. It
 * returns the new offset <pos> in the modified line. Non-printable characters
 * are replaced with '?'. <width> must be at least 6 to support two "..."
 * otherwise the result is undefined. The line itself must have at least 7 chars
