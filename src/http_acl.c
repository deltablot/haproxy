 ...
 */
static int pat_parse_meth(const char *text, struct pattern *pattern, int mflags,
                          char **err) {
  int len, meth;
...
static struct pattern *pat_match_meth(struct sample *smp,
                                      struct pattern_expr *expr, int fill) {
  int icase;
  ...
  list_for_each_entry(lst, &expr->patterns, list) {
    ...
    if ((icase && strncasecmp(pattern->ptr.str, smp->data.u.meth.str.area,
                              smp->data.u.meth.str.data) == 0) ||
        (!icase && strncmp(pattern->ptr.str, smp->data.u.meth.str.area,
                           smp->data.u.meth.str.data) == 0))
      return pattern;
  }
  return NULL;
...
static struct acl_kw_list acl_kws = {
    ILH,
    {
         ...
         * of tree storage). The basic match is PAT_MATCH_STR, but the
         * indexation and delete functions are relative to the list management.
         * The parse and match method are related to the corresponding fetch
         * methods. This is very particular ACL declaration mode.
         */
        {"http_auth_group", NULL, PAT_MATCH_STR, NULL, pat_idx_list_str, NULL,
         NULL, pat_match_auth},
        {"method", NULL, PAT_MATCH_STR, pat_parse_meth, pat_idx_list_str, NULL,
         NULL, pat_match_meth},
