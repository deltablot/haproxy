 ...
 * <domain>, <type> and <protocol> and returns the FD or -1 in case of error
 * (check errno).
 */
int my_socketat(const struct netns_entry *ns, int domain, int type,
                int protocol) {
  int sock;
  ...
  if (default_namespace >= 0 && ns &&
      setns(default_namespace, CLONE_NEWNET) == -1) {
    if (sock >= 0)
      close(sock);
    return -1;
  }
