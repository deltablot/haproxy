...
static void _do_poll(struct poller *p, int exp, int wake) {
  ...
  for (i = 0; i < (maxfd + FD_SETSIZE - 1) / (8 * sizeof(int)); i++) {
    readnotnull |= (*(((int *)tmp_evts[DIR_RD]) + i) =
                        *(((int *)fd_evts[DIR_RD]) + i)) != 0;
    writenotnull |= (*(((int *)tmp_evts[DIR_WR]) + i) =
                         *(((int *)fd_evts[DIR_WR]) + i)) != 0;
  }
  ...
  status = select(maxfd, readnotnull ? tmp_evts[DIR_RD] : NULL,
                  writenotnull ? tmp_evts[DIR_WR] : NULL, NULL, &delta);
  ...
  for (fds = 0; (fds * BITS_PER_INT) < maxfd; fds++) {
    if ((((int *)(tmp_evts[DIR_RD]))[fds] | ((int *)(tmp_evts[DIR_WR]))[fds]) ==
        0)
      continue;

    for (count = BITS_PER_INT, fd = fds * BITS_PER_INT; count && fd < maxfd;
         count--, fd++) {
      unsigned int n = 0;
...
static int init_select_per_thread() {
  ...
  fd_set_bytes =
      sizeof(fd_set) * (global.maxsock + FD_SETSIZE - 1) / FD_SETSIZE;
 ...
 */
__attribute__((constructor)) static void _do_register(void) {
  struct poller *p;
