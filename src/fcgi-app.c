...
DECLARE_STATIC_POOL(pool_head_fcgi_flt_ctx, "fcgi_flt_ctx",
                    sizeof(struct fcgi_flt_ctx));
DECLARE_STATIC_POOL(pool_head_fcgi_param_rule, "fcgi_param_rule",
                    sizeof(struct fcgi_param_rule));
DECLARE_STATIC_POOL(pool_head_fcgi_hdr_rule, "fcgi_hdr_rule",
                    sizeof(struct fcgi_hdr_rule));
...
static int fcgi_flt_check(struct proxy *px, struct flt_conf *fconf) {
  ...
  list_for_each_entry(f, &px->filter_configs, list) {
    if (f->id == http_comp_flt_id || f->id == cache_store_flt_id)
      ...
      ha_alert(
          "config : proxy '%s' : only one fcgi-app supported per backend.\n",
          px->id);
      goto err;
  ...
  list_for_each_entry_safe(crule, back, &fcgi_conf->app->conf.rules, list) {
    ...
    if (rule->type == FCGI_RULE_SET_PARAM ||
        rule->type == FCGI_RULE_UNSET_PARAM)
...
static int fcgi_flt_http_headers(struct stream *s, struct filter *filter,
                                 struct http_msg *msg) {
  ...
  if (msg->chn->flags & CF_ISRESP) {
    ...
    if (sl &&
        (sl->flags & (HTX_SL_F_XFER_LEN | HTX_SL_F_CLEN | HTX_SL_F_CHNK)) ==
            HTX_SL_F_XFER_LEN &&
      ...
      for (blk = htx_get_first_blk(htx); blk;
      ...
      if (http_add_header(htx, ist("content-length"),
                          ist2(trash.area, end - trash.area)))
...
/*********************** FCGI Config parsing ******************************/
/**************************************************************************/
static int parse_fcgi_flt(char **args, int *cur_arg, struct proxy *px,
...
static int proxy_parse_use_fcgi_app(char **args, int section,
...
static int cfg_fcgi_apps_postparser() {
  ...
  for (px = proxies_list; px; px = px->next) {
    ...
    if (px->mode == PR_MODE_TCP && fcgi_conf) {
      ha_alert("config : proxy '%s': FCGI application cannot be used in "
               "non-HTTP mode.\n",
               px->id);
    ...
    for (srv = px->srv; srv; srv = srv->next) {
      if (srv->mux_proto && isteq(srv->mux_proto->token, ist("fcgi"))) {
        ...
        if (fcgi_conf)
          continue;
        ha_alert("config : proxy '%s': FCGI server '%s' has no FCGI app "
                 "configured.\n",
                 px->id, srv->id);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto end;
    ...
    if (fcgi_conf && !nb_fcgi_srv) {
      ha_alert("config : proxy '%s': FCGI app configured but no FCGI server "
               "found.\n",
               px->id);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto end;
    ...
    list_for_each_entry(logsrv, &curapp->logsrvs, list) {
      if (logsrv->type == LOG_TARGET_BUFFER) {
        ...
        if (!sink || sink->type != SINK_TYPE_BUFFER) {
          ha_alert("config : fcgi-app '%s' : log server uses unknown ring "
                   "named '%s'.\n",
...
static int fcgi_app_add_rule(struct fcgi_app *curapp, enum fcgi_rule_type type,
                             char *name, char *value, struct acl_cond *cond,
                             char **err) {
...
static int cfg_parse_fcgi_app(const char *file, int linenum, char **args,
                              int kwm) {
  ...
  if (!strcmp(args[0], "fcgi-app")) { /* new fcgi-app */
    ...
    for (curapp = fcgi_apps; curapp != NULL; curapp = curapp->next) {
      if (!strcmp(curapp->name, args[1])) {
        ha_alert("Parsing [%s:%d]: fcgi-app section '%s' has the same name as "
                 "another one declared at %s:%d.\n",
    ...
    if (!fcgi_app_add_rule(curapp, FCGI_RULE_SET_PARAM, "REMOTE_USER",
                           "%[http_auth_user]", NULL, &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_SET_PARAM, "AUTH_TYPE",
                           "%[http_auth_type]", NULL, &errmsg)) {
    ...
    if (!fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "connection", NULL, NULL,
                           &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "keep-alive", NULL, NULL,
                           &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "authorization", NULL,
                           NULL, &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "proxy", NULL, NULL,
                           &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "proxy-authorization",
                           NULL, NULL, &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "proxy-authenticate",
                           NULL, NULL, &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "te", NULL, NULL,
                           &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "trailers", NULL, NULL,
                           &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "transfer-encoding",
                           NULL, NULL, &errmsg) ||
        !fcgi_app_add_rule(curapp, FCGI_RULE_HIDE_HDR, "upgrade", NULL, NULL,
                           &errmsg)) {
      ha_alert("parsing [%s:%d] : '%s' : %s.\n", file, linenum, args[1],
               errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
    ...
    if (!*(args[1]) || !*(args[2])) {
      ha_alert(
          "parsing [%s:%d] : '%s' expects <name> and <value> as arguments.\n",
          file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
    }
  ...
  parse_cond_rule:
    ...
    if (strcmp(args[0], "if") == 0)
      cond = parse_acl_cond((const char **)args + 1, &curapp->acls, ACL_COND_IF,
                            &errmsg, &curapp->conf.args, file, linenum);
    else if (strcmp(args[0], "unless") == 0)
      cond = parse_acl_cond((const char **)args + 1, &curapp->acls,
                            ACL_COND_UNLESS, &errmsg, &curapp->conf.args, file,
                            linenum);
    if (!cond) {
      ha_alert("parsing [%s:%d] : '%s' : %s.\n", file, linenum, name, errmsg);
      err_code |= ERR_ALERT | ERR_FATAL;
    }
  add_rule:
    ...
    if (!*(args[1])) {
      ...
      if (kwm != KWM_STD) {
        ha_alert("parsing [%s:%d]: negation/default is not supported for "
                 "option '%s'.\n",
                 file, linenum, args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      ...
      if (!curapp->maxreqs) {
        ha_alert("parsing [%s:%d]: option '%s' expects a strictly positive "
                 "integer argument.\n",
                 file, linenum, args[1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
...
static struct sample_fetch_kw_list sample_fetch_keywords = {
    ILH,
    {{"fcgi.docroot", smp_fetch_fcgi_docroot, 0, NULL, SMP_T_STR,
      SMP_USE_HRQHV},
