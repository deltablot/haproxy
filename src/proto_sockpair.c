...
static int sockpair_bind_listener(struct listener *listener, char *errmsg,
                                  int errlen);
...
int sockpair_bind_receiver(struct receiver *rx, char **errmsg) {
  ...
  fd_insert(rx->fd, rx->owner, rx->iocb,
            thread_mask(rx->settings->bind_thread) & all_threads_mask);
  return err;
