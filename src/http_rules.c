...
struct act_rule *parse_http_req_cond(const char **args, const char *file,
  ...
  if (((custom = action_http_req_custom(args[0])) != NULL)) {
    ...
    rule->kw = custom;
    if (custom->parse(args, &cur_arg, proxy, rule, &errmsg) ==
        ACT_RET_PRS_ERR) {
      ha_alert("parsing [%s:%d] : error detected in %s '%s' while parsing "
               "'http-request %s' rule : %s.\n",
    ...
    ha_alert("parsing [%s:%d]: 'http-request' expects %s%s, but got '%s'%s.\n",
             file, linenum, *trash.area ? ", " : "", trash.area, args[0],
             *args[0] ? "" : " (missing argument)");
    goto out_err;
  }
  ...
  if (strcmp(args[cur_arg], "if") == 0 ||
    ...
    if ((cond = build_acl_cond(file, linenum, &proxy->acl, proxy,
                               args + cur_arg, &errmsg)) == NULL) {
      ha_alert("parsing [%s:%d] : error detected while parsing an "
               "'http-request %s' condition : %s.\n",
    ...
    ha_alert(
        "parsing [%s:%d]: 'http-request %s' expects"
...
struct act_rule *parse_http_res_cond(const char **args, const char *file,
                                     int linenum, struct proxy *proxy) {
  struct act_rule *rule;
  struct action_kw *custom = NULL;
  int cur_arg;
  ...
  if (strcmp(args[cur_arg], "if") == 0 ||
    ...
    ha_alert(
        "parsing [%s:%d]: 'http-response %s' expects"
        " either 'if' or 'unless' followed by a condition but found '%s'.\n",
        file, linenum, args[0], args[cur_arg]);
    goto out_err;
  }
...
struct act_rule *parse_http_after_res_cond(const char **args, const char *file,
                                           int linenum, struct proxy *proxy) {
  struct act_rule *rule;
  struct action_kw *custom = NULL;
  int cur_arg;
  ...
  if (((custom = action_http_after_res_custom(args[0])) != NULL)) {
    ...
    ha_alert("parsing [%s:%d]: 'http-after-response' expects %s%s, but got "
             "'%s'%s.\n",
             file, linenum, *trash.area ? ", " : "", trash.area, args[0],
             *args[0] ? "" : " (missing argument)");
    goto out_err;
  ...
  if (strcmp(args[cur_arg], "if") == 0 ||
    ...
    if ((cond = build_acl_cond(file, linenum, &proxy->acl, proxy,
                               args + cur_arg, &errmsg)) == NULL) {
      ha_alert("parsing [%s:%d] : error detected while parsing an "
               "'http-after-response %s' condition : %s.\n",
               file, linenum, args[0], errmsg);
      free(errmsg);
      goto out_err;
    }
    ...
    ha_alert(
        "parsing [%s:%d]: 'http-after-response %s' expects"
        " either 'if' or 'unless' followed by a condition but found '%s'.\n",
        file, linenum, args[0], args[cur_arg]);
    goto out_err;
  }
...
/* Parses a redirect rule. Returns the redirect rule on success or NULL on
 * error, with <err> filled with the error message. If <use_fmt> is not null,
 * builds a dynamic log-format rule instead of a static string. Parameter <dir>
 * indicates the direction of the rule, and equals 0 for request, non-zero for
 * responses.
 */
struct redirect_rule *http_parse_redirect_rule(const char *file, int linenum,
  ...
  while (*(args[cur_arg])) {
    if (strcmp(args[cur_arg], "location") == 0) {
      ...
      if (code < 301 || code > 308 || (code > 303 && code < 307)) {
        memprintf(errmsg,
                  "'%s': unsupported HTTP code '%s' (must be one of 301, 302, "
                  "303, 307 or 308)",
      ...
      flags |= REDIRECT_FLAG_APPEND_SLASH;
               ...
               strcmp(args[cur_arg], "unless") == 0) {
      cond = build_acl_cond(file, linenum, &curproxy->acl, curproxy,
                            (const char **)args + cur_arg, errmsg);
      if (!cond) {
      ...
      memprintf(errmsg,
                "expects 'code', 'prefix', 'location', 'scheme', 'set-cookie', "
                "'clear-cookie', 'drop-query' or 'append-slash' (was '%s')",
                args[cur_arg]);
      return NULL;
    }
    cur_arg++;
  ...
  if (type == REDIRECT_TYPE_NONE) {
    memprintf(errmsg,
              "redirection type expected ('prefix', 'location', or 'scheme')");
    return NULL;
  }
  ...
  if (!use_fmt) {
    ...
    if (!(type == REDIRECT_TYPE_PREFIX && destination[0] == '/' &&
      ...
      if (!parse_logformat_string(
              destination, curproxy, &rule->rdr_fmt, LOG_OPT_HTTP,
              dir ? (curproxy->cap & PR_CAP_FE) ? SMP_VAL_FE_HRS_HDR
                                                : SMP_VAL_BE_HRS_HDR
  ...
  if (cookie) {
    /* depending on cookie_set, either we want to set the cookie, or to clear
     * it. a clear consists in appending "; path=/; Max-Age=0;" at the end.
     */
...
__attribute__((constructor)) static void __http_rules_init(void) {}
