  ...
  for (curuserlist = userlist; curuserlist; curuserlist = curuserlist->next) {
    ...
    for (curuser = curuserlist->users; curuser; curuser = curuser->next) {
      ...
      while ((group = strtok(group ? NULL : curuser->u.groups_names, ","))) {
        ...
        if (!grl) {
          ha_alert("userlist '%s': no more memory when trying to allocate the "
                   "user groups.\n",
                   curuserlist->name);
          free(groups);
          return ERR_ALERT | ERR_FATAL;
...
#ifdef DEBUG_AUTH
  fprintf(stderr, "cfg: user=%s, pass=%s, flags=%X, groups=", u->user, u->pass,
          u->flags);
...
struct pattern *pat_match_auth(struct sample *smp, struct pattern_expr *expr,
                               int fill) {
...
REGISTER_BUILD_OPTS(
    "Encrypted password support via crypt(3): " CRYPT_STATE_MSG);
