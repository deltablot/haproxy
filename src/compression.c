...
static int identity_add_data(struct comp_ctx *comp_ctx, const char *in_data,
...
static int rfc195x_add_data(struct comp_ctx *comp_ctx, const char *in_data,
                            int in_len, struct buffer *out);
...
static int deflate_add_data(struct comp_ctx *comp_ctx, const char *in_data,
                            int in_len, struct buffer *out);
...
const struct comp_algo comp_algos[] = {
    {"identity", 8, "identity", 8, identity_init, identity_add_data,
     identity_flush, identity_finish, identity_end},
#if defined(USE_SLZ)
    {"deflate", 7, "deflate", 7, rfc1950_init, rfc195x_add_data, rfc195x_flush,
     rfc195x_finish, rfc195x_end},
    {"raw-deflate", 11, "deflate", 7, rfc1951_init, rfc195x_add_data,
     rfc195x_flush, rfc195x_finish, rfc195x_end},
    {"gzip", 4, "gzip", 4, rfc1952_init, rfc195x_add_data, rfc195x_flush,
     rfc195x_finish, rfc195x_end},
#elif defined(USE_ZLIB)
    {"deflate", 7, "deflate", 7, deflate_init, deflate_add_data, deflate_flush,
     deflate_finish, deflate_end},
    {"raw-deflate", 11, "deflate", 7, raw_def_init, deflate_add_data,
     deflate_flush, deflate_finish, deflate_end},
    {"gzip", 4, "gzip", 4, gzip_init, deflate_add_data, deflate_flush,
     deflate_finish, deflate_end},
#endif /* USE_ZLIB */
...
static inline int init_comp_ctx(struct comp_ctx **comp_ctx) {
  ...
  if (global.maxzlibmem > 0 &&
      (global.maxzlibmem - zlib_used_memory) < sizeof(struct comp_ctx))
    return -1;
#endif
 ...
 */
static int identity_init(struct comp_ctx **comp_ctx, int level) { return 0; }
 ...
 */
static int identity_end(struct comp_ctx **comp_ctx) { return 0; }
 ...
 */
static int rfc195x_flush_or_finish(struct comp_ctx *comp_ctx,
                                   struct buffer *out, int finish) {
  ...
  if ((global.comp_rate_lim > 0 && (read_freq_ctr(&global.comp_bps_out) >
                                    global.comp_rate_lim)) || /* rate */
...
static void *alloc_zlib(void *opaque, unsigned int items, unsigned int size) {
  ...
  if (global.maxzlibmem > 0 &&
      (global.maxzlibmem - zlib_used_memory) < (long)(items * size))
    goto end;
  ...
  case 0:
    if (zlib_pool_deflate_state == NULL) {
      ...
      if (zlib_pool_deflate_state == NULL)
        zlib_pool_deflate_state =
            create_pool("zlib_state", size * items, MEM_F_SHARED);
  ...
  case 1:
    if (zlib_pool_window == NULL) {
      ...
      if (zlib_pool_window == NULL)
        zlib_pool_window =
            create_pool("zlib_window", size * items, MEM_F_SHARED);
      HA_SPIN_UNLOCK(COMP_POOL_LOCK, &comp_pool_lock);
    }
  ...
  case 4:
    if (zlib_pool_pending_buf == NULL) {
      HA_SPIN_LOCK(COMP_POOL_LOCK, &comp_pool_lock);
      if (zlib_pool_pending_buf == NULL)
        zlib_pool_pending_buf =
            create_pool("zlib_pending_buf", size * items, MEM_F_SHARED);
      HA_SPIN_UNLOCK(COMP_POOL_LOCK, &comp_pool_lock);
    }
...
static int gzip_init(struct comp_ctx **comp_ctx, int level) {
  ...
  if (deflateInit2(strm, level, Z_DEFLATED, global_tune_zlibwindowsize + 16,
                   global_tune_zlibmemlevel, Z_DEFAULT_STRATEGY) != Z_OK) {
    deinit_comp_ctx(comp_ctx);
    return -1;
  }
...
static int raw_def_init(struct comp_ctx **comp_ctx, int level) {
  ...
  if (deflateInit2(strm, level, Z_DEFLATED, -global_tune_zlibwindowsize,
                   global_tune_zlibmemlevel, Z_DEFAULT_STRATEGY) != Z_OK) {
    deinit_comp_ctx(comp_ctx);
    return -1;
  }
...
static int deflate_flush_or_finish(struct comp_ctx *comp_ctx,
                                   struct buffer *out, int flag) {
  int ret;
  int out_len = 0;
  z_stream *strm = &comp_ctx->strm;
...
static int zlib_parse_global_memlevel(char **args, int section_type,
...
static int zlib_parse_global_windowsize(char **args, int section_type,
                                        struct proxy *curpx,
...
__attribute__((constructor)) static void __comp_fetch_init(void) {
#ifdef USE_SLZ
  slz_make_crc_table();
...
#else
  memprintf(&ptr, "Built without compression support (neither USE_ZLIB nor "
                  "USE_SLZ are set).");
#endif
  memprintf(&ptr, "%s\nCompression algorithms supported :", ptr);

  for (i = 0; comp_algos[i].cfg_name; i++)
    memprintf(&ptr, "%s%s %s(\"%s\")", ptr, (i == 0 ? "" : ","),
              comp_algos[i].cfg_name, comp_algos[i].ua_name);
