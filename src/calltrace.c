 ...
 *
 * It is also possible to emit comments using the calltrace() function which
 * uses the printf() format. Such comments are then inserted by replacing the
 * caller pointer with a sharp ('#') like this :
 *
...
static char
    line[128]; /* more than enough for a message (9+1+6+1+3+1+18+1+1+18+1+1) */
 ...
 * by 100 (using adds and shifts). The trick is that dividing by 100M is
 * equivalent to multiplying by 1/100M, which approximates to 1441151881/2^57.
 * All local variables fit in registers on x86. This version outputs two digits
 * per round. <min_pairs> indicates the minimum number of pairs of digits that
 * have to be emitted, which might be left-padded with zeroes. It returns the
 * pointer to the ending '\0'.
 */
...
static void make_line(void *from, void *to, int level, char dir, long ret) {
  ...
  if (!use_tsc)
    fprintf(log, "%u.%06u %d %p %c %p\n", (unsigned int)now_ptr->tv_sec,
            (unsigned int)now_ptr->tv_usec, level, from, dir, to);
  else
 ...
 */
__attribute__((format(printf, 1, 2))) void calltrace(char *fmt, ...) {
  va_list ap;
  ...
  if (!use_tsc)
    fprintf(log, "%u.%06u %d # ", (unsigned int)now_ptr->tv_sec,
            (unsigned int)now_ptr->tv_usec, level + 1);
  else
    fprintf(log, "%llx %d # ", rdtsc(), level + 1);
