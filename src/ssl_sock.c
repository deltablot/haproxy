...
static struct eb_root cert_issuer_tree =
    EB_ROOT; /* issuers tree from "issuers-chain-path" */
...
DECLARE_STATIC_POOL(ssl_sock_ctx_pool, "ssl_sock_ctx_pool",
                    sizeof(struct ssl_sock_ctx));
...
static void ssl_fill_stats(void *data, struct field *stats) {
  ...
  stats[SSL_ST_FAILED_HANDSHAKE] =
      mkf_u64(FN_COUNTER, counters->failed_handshake);
}
...
static struct stats_module ssl_stats_module = {
    ...
    .domain_flags = MK_STATS_PROXY_DOMAIN(STATS_PX_CAP_FE | STATS_PX_CAP_LI |
                                          STATS_PX_CAP_BE | STATS_PX_CAP_SRV),
    .clearable = 1,
};
...
static int ha_ssl_gets(BIO *h, char *buf, int size) { return 0; }
...
static int ha_ssl_free(BIO *data) { return 1; }

#if defined(USE_THREAD) && (HA_OPENSSL_VERSION_NUMBER < 0x10100000L)

static HA_RWLOCK_T *ssl_rwlocks;

unsigned long ssl_id_function(void) { return (unsigned long)tid; }
...
/* SSL_CTX_load_verify_locations substitute, internally call
...
/*
   ...
   compatibility. Return a shared ca_list: SSL_dup_CA_list must be used before
   set it on SSL_CTX.
*/
  ...
  if (ca_e->ca_list == NULL) {
    ...
    for (i = 0; i < sk_X509_OBJECT_num(objs); i++) {
      ...
      for (node = eb64_lookup(&ca_name_tree, key), ca_name = NULL;
           node && ca_name == NULL; node = eb64_next(node)) {
      ...
      xn = X509_NAME_dup(xn);
      if (!ca_name || !xn || !sk_X509_NAME_push(skn, xn)) {
        free(ca_name);
        X509_NAME_free(xn);
...
static char *x509v3_ext_names[X509V3_EXT_SIZE] = {
    "basicConstraints",       "nsComment", "subjectKeyIdentifier",
    "authorityKeyIdentifier", "keyUsage",
};
static char *x509v3_ext_values[X509V3_EXT_SIZE] = {
    "CA:FALSE", "\"OpenSSL Generated Certificate\"", "hash",
    "keyid,issuer:always", "nonRepudiation,digitalSignature,keyEncipherment"};
 ...
 */
const char *SSL_SOCK_KEYTYPE_NAMES[] = {"dsa", "ecdsa", "rsa"};
...
static void ssl_sock_parse_heartbeat(struct connection *conn, int write_p,
                                     int version, int content_type,
                                     const void *buf, size_t len, SSL *ssl);
#endif
static void ssl_sock_parse_clienthello(struct connection *conn, int write_p,
                                       int version, int content_type,
                                       const void *buf, size_t len, SSL *ssl);
  ...
  if (unlikely(global.mode & MODE_DEBUG)) {
    while (1) {
      ...
      fprintf(stderr, "fd[%04x] OpenSSL error[0x%lx] %s: %s\n",
              (unsigned short)conn->handle.fd, ret, ERR_func_error_string(ret),
...
void ssl_async_fd_free(int fd) {
  ...
  if (num_all_fds > 32) {
    send_log(NULL, LOG_EMERG,
             "haproxy: openssl returns too many async fds. It seems a bug. "
             "Process may crash\n");
    return;
  }
...
static long asn1_generalizedtime_to_epoch(ASN1_GENERALIZEDTIME *d) {
  long epoch;
  char *p, *end;
  const unsigned short month_offset[12] = {0,   31,  59,  90,  120, 151,
                                           181, 212, 243, 273, 304, 334};
  int year, month;

  if (!d || (d->type != V_ASN1_GENERALIZEDTIME))
    return -1;
  ...
  if (end - p < 4)
    return -1;
  year =
      1000 * (p[0] - '0') + 100 * (p[1] - '0') + 10 * (p[2] - '0') + p[3] - '0';
  p += 4;
  if (end - p < 2)
    return -1;
  ...
  if (month < 1 || month > 12)
    return -1;
  /* Compute the number of seconds since 1 jan 1970 and the beginning of current
     month We consider leap years and the current month (<marsh or not) */
  epoch = (((year - 1970) * 365) +
           ((year - (month < 3)) / 4 - (year - (month < 3)) / 100 +
            (year - (month < 3)) / 400) -
...
nosec:
  if (p[0] == 'Z') {
    ...
    return epoch - ((10 * (p[1] - '0') + p[2] - '0') * 60 * 60 +
                    (10 * (p[3] - '0') + p[4] - '0')) *
                       60;
  } else if (p[0] == '-') {
    if (end - p != 5)
 ...
 * containers tree (using as index the ASN1 form of the OCSP Certificate ID
 * extracted from the response) and exits on error if not found. Finally, If an
 * OCSP response is already present in the container, it will be overwritten.
 *
...
static int ssl_sock_load_ocsp_response(struct buffer *ocsp_response,
  ...
  resp =
      d2i_OCSP_RESPONSE(NULL, (const unsigned char **)&p, ocsp_response->data);
  if (!resp) {
  ...
  if (count_sr > 1) {
    memprintf(
        err,
        "OCSP response ignored because contains multiple single responses (%d)",
        count_sr);
    goto out;
  }
  ...
  if (cid) {
    if (OCSP_id_cmp(id, cid)) {
      memprintf(err, "OCSP single response: Certificate ID does not match "
  ...
  if (!ocsp) {
    ...
    ocsp = (struct certificate_ocsp *)ebmb_lookup(&cert_ocsp_tree, key,
                                                  OCSP_MAX_CERTID_ASN1_LENGTH);
    if (!ocsp) {
      memprintf(err, "OCSP single response: Certificate ID does not match any "
  ...
  ocsp->expire =
      asn1_generalizedtime_to_epoch(nextupd) - OCSP_MAX_RESPONSE_TIME_SKEW;
...
static int ssl_tlsext_ticket_key_cb(SSL *s, unsigned char key_name[16],
  ...
  if (enc) {
    ...
    if (ref->key_size_bits == 128) {
      ...
      if (!EVP_EncryptInit_ex(ectx, EVP_aes_128_cbc(), NULL,
                              keys[head].key_128.aes_key, iv))
        goto end;

      HMAC_Init_ex(hctx, keys[head].key_128.hmac_key, 16,
                   TLS_TICKET_HASH_FUNCT(), NULL);
      ret = 1;
    } else if (ref->key_size_bits == 256) {

      if (!EVP_EncryptInit_ex(ectx, EVP_aes_256_cbc(), NULL,
                              keys[head].key_256.aes_key, iv))
        goto end;

      HMAC_Init_ex(hctx, keys[head].key_256.hmac_key, 32,
                   TLS_TICKET_HASH_FUNCT(), NULL);
      ret = 1;
    }
  } else {
  ...
  found:
    if (ref->key_size_bits == 128) {
      HMAC_Init_ex(hctx, keys[(head + i) % TLS_TICKETS_NO].key_128.hmac_key, 16,
                   TLS_TICKET_HASH_FUNCT(), NULL);
      if (!EVP_DecryptInit_ex(ectx, EVP_aes_128_cbc(), NULL,
                              keys[(head + i) % TLS_TICKETS_NO].key_128.aes_key,
                              iv))
        goto end;
      /* 2 for key renewal, 1 if current key is still valid */
      ret = i ? 2 : 1;
    } else if (ref->key_size_bits == 256) {
      HMAC_Init_ex(hctx, keys[(head + i) % TLS_TICKETS_NO].key_256.hmac_key, 32,
                   TLS_TICKET_HASH_FUNCT(), NULL);
      if (!EVP_DecryptInit_ex(ectx, EVP_aes_256_cbc(), NULL,
                              keys[(head + i) % TLS_TICKETS_NO].key_256.aes_key,
                              iv))
        goto end;
      /* 2 for key renewal, 1 if current key is still valid */
      ret = i ? 2 : 1;
...
struct tls_keys_ref *tlskeys_ref_lookup(const char *filename) {
  ...
  list_for_each_entry(ref, &tlskeys_reference,
                      list) if (ref->filename &&
                                strcmp(filename, ref->filename) ==
                                    0) return ref;
  return NULL;
}
...
struct tls_keys_ref *tlskeys_ref_lookupid(int unique_id) {
  ...
  list_for_each_entry(ref, &tlskeys_reference,
                      list) if (ref->unique_id == unique_id) return ref;
  return NULL;
}
...
int ssl_sock_update_tlskey_ref(struct tls_keys_ref *ref,
  ...
  memcpy((char *)(ref->tlskeys +
                  ((ref->tls_ticket_enc_index + 2) % TLS_TICKETS_NO)),
...
int ssl_sock_ocsp_stapling_cbk(SSL *ssl, void *arg) {
  ...
  if (ocsp_arg->is_single && ocsp_arg->single_kt == key_type)
    ...
    /* For multiple certs per context, we have to find the correct OCSP response
  ...
  if (!ocsp || !ocsp->response.area || !ocsp->response.data ||
...
#if ((defined SSL_CTRL_SET_TLSEXT_STATUS_REQ_CB &&                             \
 ...
 */
static int ssl_sock_load_ocsp(SSL_CTX *ctx,
                              const struct cert_key_and_chain *ckch,
                              STACK_OF(X509) * chain) {
  X509 *x, *issuer;
  ...
  iocsp = (struct certificate_ocsp *)ebmb_insert(&cert_ocsp_tree, &ocsp->key,
                                                 OCSP_MAX_CERTID_ASN1_LENGTH);
  if (iocsp == ocsp)
    ocsp = NULL;
...
#define SSL_CTX_get_tlsext_status_cb(ctx, cb)                                  \
  *cb = (void (*)(void))ctx->tlsext_status_cb;
  ...
  if (!callback) {
    ...
    SSL_CTX_set_ex_data(
        ctx, ocsp_ex_index,
        cb_arg); /* we use the ex_data instead of the cb_arg function here, so
                    we can use the cleanup callback to free */

  } else {
    /*
     * If the ctx has a status CB, then we have previously set an OCSP staple
     * for this ctx Update that cb_arg with the new cert's staple
     */
    struct ocsp_cbk_arg *cb_arg;
    ...
    /*
     * The following few lines will convert cb_arg from a single ocsp to multi
     * ocsp the order of operations below matter, take care when changing it
     */
  ...
  if (ssl_sock_load_ocsp_response(ckch->ocsp_response, iocsp, cid, &warn)) {
    memprintf(&warn, "Loading: %s. Content will be ignored",
              warn ? warn : "failure");
    ha_warning("%s.\n", warn);
  }
...
static int ssl_sock_load_ocsp(SSL_CTX *ctx,
                              const struct cert_key_and_chain *ckch,
                              STACK_OF(X509) * chain) {
  return SSL_CTX_set_ocsp_response(ctx,
                                   (const uint8_t *)ckch->ocsp_response->area,
                                   ckch->ocsp_response->data);
}
#endif
...
int ssl_sock_sctl_add_cbk(SSL *ssl, unsigned ext_type,
...
int ssl_sock_sctl_parse_cbk(SSL *s, unsigned int ext_type,
...
static int ssl_sock_load_sctl(SSL_CTX *ctx, struct buffer *sctl) {
  ...
  if (!SSL_CTX_add_server_custom_ext(ctx, CT_EXTENSION_TYPE,
                                     ssl_sock_sctl_add_cbk, NULL, sctl,
                                     ssl_sock_sctl_parse_cbk, NULL))
    goto out;
...
void ssl_sock_infocbk(const SSL *ssl, int where, int ret) {
  ...
  if (where & SSL_CB_HANDSHAKE_START) {
    ...
    if ((conn->flags &
         (CO_FL_WAIT_L6_CONN | CO_FL_EARLY_SSL_HS | CO_FL_EARLY_DATA)) == 0) {
...
int ssl_sock_bind_verifycbk(int ok, X509_STORE_CTX *x_store) {
  ...
  ssl =
      X509_STORE_CTX_get_ex_data(x_store, SSL_get_ex_data_X509_STORE_CTX_idx());
  ...
  if (depth > 0) {
    ...
    if (err < 64 &&
        __objt_listener(conn->target)->bind_conf->ca_ignerr & (1ULL << err)) {
  ...
  if (err < 64 &&
      __objt_listener(conn->target)->bind_conf->crt_ignerr & (1ULL << err)) {
    ssl_sock_dump_errors(conn);
    ERR_clear_error();
    return 1;
  }
...
void ssl_sock_msgcbk(int write_p, int version, int content_type,
...
static int ssl_sock_srv_select_protos(SSL *s, unsigned char **out,
                                      unsigned char *outlen,
                                      const unsigned char *in,
                                      unsigned int inlen, void *arg) {
  struct server *srv = arg;

  if (SSL_select_next_proto(out, outlen, in, inlen,
                            (unsigned char *)srv->ssl_ctx.npn_str,
...
static int ssl_sock_advertise_alpn_protos(SSL *s, const unsigned char **out,
  ...
  if (SSL_select_next_proto(
          (unsigned char **)out, outlen, (const unsigned char *)conf->alpn_str,
...
int ssl_sock_add_san_ext(X509V3_CTX *ctx, X509 *cert, const char *servername) {
  ...
  san_ext =
      X509V3_EXT_nconf_nid(conf, ctx, NID_subject_alt_name, san_name->area);
  if (!san_ext) {
    failure = 1;
    goto cleanup;
  }
...
static SSL_CTX *ssl_sock_do_create_cert(const char *servername,
  ...
  ASN1_INTEGER_set(X509_get_serialNumber(newcrt),
                   _HA_ATOMIC_ADD(&ssl_ctx_serial, 1));
  ...
  if (X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
                                 (const unsigned char *)servername, -1, -1,
                                 0) != 1) {
    X509_NAME_free(name);
    goto mkcert_error;
  }
  ...
  for (i = 0; i < X509V3_EXT_SIZE; i++) {
    ...
    if (!(ext = X509V3_EXT_nconf(ctmp, &ctx, x509v3_ext_names[i],
                                 x509v3_ext_values[i])))
      goto mkcert_error;
  ...
  {
    const char *ecdhe = (bind_conf->ssl_conf.ecdhe ? bind_conf->ssl_conf.ecdhe
                                                   : ECDHE_DEFAULT_CURVE);
    EC_KEY *ecc;
    int nid;
...
SSL_CTX *ssl_sock_create_cert(struct connection *conn, const char *servername,
                              unsigned int key) {
...
SSL_CTX *ssl_sock_assign_generated_cert(unsigned int key,
                                        struct bind_conf *bind_conf, SSL *ssl) {
...
SSL_CTX *ssl_sock_get_generated_cert(unsigned int key,
...
int ssl_sock_set_generated_cert(SSL_CTX *ssl_ctx, unsigned int key,
                                struct bind_conf *bind_conf) {
  struct lru64 *lru = NULL;
 ...
 * cert's refcount is maintained regardless of the cert's presence in the LRU
 * cache.
 */
static int ssl_sock_generate_certificate(const char *servername,
                                         struct bind_conf *bind_conf,
                                         SSL *ssl) {
  X509 *cacert = bind_conf->ca_sign_ckch->cert;
  SSL_CTX *ssl_ctx = NULL;
...
static int ssl_sock_generate_certificate_from_conn(struct bind_conf *bind_conf,
                                                   SSL *ssl) {
  unsigned int key;
  struct connection *conn = SSL_get_ex_data(ssl, ssl_app_data_index);
...
struct methodVersions methodVersions[] = {
    ...
    {SSL_OP_NO_SSLv3, MC_SSL_O_NO_SSLV3, ctx_set_SSLv3_func, ssl_set_SSLv3_func,
     "SSLv3"}, /* CONF_SSLV3 */
    {SSL_OP_NO_TLSv1, MC_SSL_O_NO_TLSV10, ctx_set_TLSv10_func,
     ssl_set_TLSv10_func, "TLSv1.0"}, /* CONF_TLSV10 */
    {SSL_OP_NO_TLSv1_1, MC_SSL_O_NO_TLSV11, ctx_set_TLSv11_func,
     ssl_set_TLSv11_func, "TLSv1.1"}, /* CONF_TLSV11 */
    {SSL_OP_NO_TLSv1_2, MC_SSL_O_NO_TLSV12, ctx_set_TLSv12_func,
     ssl_set_TLSv12_func, "TLSv1.2"}, /* CONF_TLSV12 */
...
static int ssl_sock_switchctx_cbk(SSL *ssl, int *al, void *arg) {
...
#else
  if (SSL_client_hello_get0_ext(ssl, TLSEXT_TYPE_server_name, &extension_data,
                                &extension_len)) {
#endif
    /*
  ...
  if (has_ecdsa_sig) { /* in very rare case: has ecdsa sign but not a ECDSA
                          cipher */
    ...
    for (n = node; n; n = ebmb_next_dup(n)) {
      ...
      if (!container_of(n, struct sni_ctx, name)->neg) {
        ...
        if (i == 1 && wildp) { /* wildcard */
          /* If this is a wildcard, look for an exclusion on the same crt-list
           * line */
          sni = container_of(n, struct sni_ctx, name);
          list_for_each_entry(sni_tmp, &sni->ckch_inst->sni_ctx, by_ckch_inst) {
            if (sni_tmp->neg &&
                (!strcmp((const char *)sni_tmp->name.key, trash.area))) {
              skip = 1;
              break;
            }
          }
...
static int ckch_inst_add_cert_sni(SSL_CTX *ctx, struct ckch_inst *ckch_inst,
                                  struct bind_conf *s,
                                  struct ssl_bind_conf *conf,
                                  struct pkey_info kinfo, char *name,
                                  int order) {
  struct sni_ctx *sc;
  int wild = 0, neg = 0;
...
/*
 * Insert the sni_ctxs that are listed in the ckch_inst, in the bind_conf's
 * sni_ctx tree This function can't return an error.
 *
 * *CAUTION*: The caller must lock the sni tree if called in multithreading mode
 */
void ssl_sock_load_cert_sni(struct ckch_inst *ckch_inst,
                            struct bind_conf *bind_conf) {
 ...
 */
#ifndef OPENSSL_NO_DH
static int ssl_sock_load_dh_params(SSL_CTX *ctx,
                                   const struct cert_key_and_chain *ckch,
  ...
  if (ckch && ckch->dh) {
    ...
    if (!SSL_CTX_set_tmp_dh(ctx, dh)) {
      ...
      memprintf(
          err,
          "%s, SSL library will use an automatically generated DH parameter.\n",
          err && *err ? *err : "");
#else
    ...
    if (!SSL_CTX_set_tmp_dh(ctx, global_dh)) {
      memprintf(err,
                "%sunable to use the global DH parameter for certificate '%s'",
                err && *err ? *err : "", path);
#if defined(SSL_CTX_set_dh_auto)
      SSL_CTX_set_dh_auto(ctx, 1);
      memprintf(
    ...
    if (global_ssl.default_dh_param && global_ssl.default_dh_param <= 1024) {
      ...
      if (local_dh_1024 == NULL) {
        memprintf(err,
                  "%sunable to load default 1024 bits DH parameter for "
                  "certificate '%s'.\n",
                  err && *err ? *err : "", path);
      ...
      if (!SSL_CTX_set_tmp_dh(ctx, local_dh_1024)) {
        ...
        SSL_CTX_set_dh_auto(ctx, 1);
        memprintf(err,
                  "%s, SSL library will use an automatically generated DH "
                  "parameter.\n",
                  err && *err ? *err : "");
#else
        memprintf(err, "%s, DH ciphers won't be available.\n",
 ...
 * the operation succeed.
 */
static int ssl_sock_put_ckch_into_ctx(const char *path,
                                      const struct cert_key_and_chain *ckch,
  ...
  if (!SSL_CTX_set1_chain(ctx, find_chain)) {
    memprintf(err,
              "%sunable to load chain certificate into SSL Context '%s'. Make "
              "sure you are linking against Openssl >= 1.0.2.\n",
              err && *err ? *err : "", path);
    errcode |= ERR_ALERT | ERR_FATAL;
    goto end;
  }
  ...
  { /* legacy compat (< openssl 1.0.2) */
    ...
    while ((ca = sk_X509_shift(chain)))
      if (!SSL_CTX_add_extra_chain_cert(ctx, ca)) {
        memprintf(err,
                  "%sunable to load chain certificate into SSL Context '%s'.\n",
                  err && *err ? *err : "", path);
        X509_free(ca);
  ...
  if (global_ssl.skip_self_issued_ca) {
    if (!SSL_CTX_build_cert_chain(ctx, SSL_BUILD_CHAIN_FLAG_NO_ROOT |
  ...
  if (ckch->ocsp_response) {
    if (ssl_sock_load_ocsp(ctx, ckch, find_chain) < 0) {
      memprintf(err,
                "%s '%s.ocsp' is present and activates OCSP but it is "
                "impossible to compute the OCSP certificate ID (maybe the "
 ...
 *     ERR_WARN if a warning is available into err
 */
int ckch_inst_new_load_store(const char *path, struct ckch_store *ckchs,
                             struct bind_conf *bind_conf,
                             struct ssl_bind_conf *ssl_conf, char **sni_filter,
                             int fcount, struct ckch_inst **ckchi, char **err) {
  SSL_CTX *ctx;
  int i;
  int order = 0;
  X509_NAME *xname;
  ...
  *ckchi = NULL;
  ...
  if (fcount) {
    while (fcount--) {
      order = ckch_inst_add_cert_sni(ctx, ckch_inst, bind_conf, ssl_conf, kinfo,
    ...
    if (names) {
      for (i = 0; i < sk_GENERAL_NAME_num(names); i++) {
        ...
        if (name->type == GEN_DNS) {
          if (ASN1_STRING_to_UTF8((unsigned char **)&str, name->d.dNSName) >=
              0) {
            order = ckch_inst_add_cert_sni(ctx, ckch_inst, bind_conf, ssl_conf,
                                           kinfo, str, order);
            OPENSSL_free(str);
            if (order < 0) {
              memprintf(err, "%sunable to create a sni context.\n",
  ...
  if (bind_conf->default_ctx) {
    memprintf(
        err,
        "%sthis version of openssl cannot load multiple SSL certificates.\n",
        err && *err ? *err : "");
    errcode |= ERR_ALERT | ERR_FATAL;
    goto error;
  }
  ...
  SSL_CTX_free(ctx); /* we need to free the ctx since we incremented the
                        refcount where it's used */
...
static int ssl_sock_load_ckchs(const char *path, struct ckch_store *ckchs,
  ...
  errcode |= ckch_inst_new_load_store(path, ckchs, bind_conf, ssl_conf,
                                      sni_filter, fcount, ckch_inst, err);
 ...
 *  - store the content of the file in a crtlist structure with crtlist_entry
 * structures
 ...
 *  Returns a set of ERR_* flags possibly with an error in <err>.
 */
int ssl_sock_load_cert_list_file(char *file, int dir,
                                 struct bind_conf *bind_conf,
                                 struct proxy *curproxy, char **err) {
  ...
  list_for_each_entry(entry, &crtlist->ord_entries, by_crtlist) {
    ...
    cfgerr |=
        ssl_sock_load_ckchs(store->path, store, bind_conf, entry->ssl_conf,
                            entry->filters, entry->fcount, &ckch_inst, err);
    if (cfgerr & ERR_CODE) {
      memprintf(err, "error processing line %d in file '%s' : %s",
                entry->linenum, file, *err);
      goto error;
    }
...
error : {
  ...
  if (lastentry) {
    list_for_each_entry(entry, &crtlist->ord_entries, by_crtlist) {
      if (entry == lastentry) /* last entry we tried to generate, no need to go
                                 further */
        break;

      list_for_each_entry_safe(inst, s_inst, &entry->ckch_inst,
                               by_crtlist_entry) {
...
int ssl_sock_load_cert(char *path, struct bind_conf *bind_conf, char **err) {
  ...
  if ((ckchs = ckchs_lookup(path))) {
    /* we found the ckchs in the tree, we can use it directly */
    cfgerr |= ssl_sock_load_ckchs(path, ckchs, bind_conf, NULL, NULL, 0,
                                  &ckch_inst, err);
    found++;
    ...
    if (S_ISDIR(buf.st_mode) == 0) {
      ...
      cfgerr |= ssl_sock_load_cert_list_file(path, 1, bind_conf,
    ...
    if (global_ssl.extra_files & SSL_GF_BUNDLE) {
      ...
      for (n = 0; n < SSL_SOCK_NUM_KEYTYPES; n++) {
        ...
        if ((ckchs = ckchs_lookup(fp))) {
          cfgerr |= ssl_sock_load_ckchs(fp, ckchs, bind_conf, NULL, NULL, 0,
                                        &ckch_inst, err);
          found++;
        } else {
          if (stat(fp, &buf) == 0) {
      ...
      if (found) {
        memprintf(err,
                  "%sCan't load '%s'. Loading a multi certificates bundle "
...
static int ssl_sock_initial_ctx(struct bind_conf *bind_conf) {
  SSL_CTX *ctx = NULL;
  long options = SSL_OP_ALL | /* all known workarounds for bugs */
                 SSL_OP_NO_SSLv2 | SSL_OP_NO_COMPRESSION |
                 SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE |
                 SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION |
                 SSL_OP_PRIORITIZE_CHACHA | SSL_OP_CIPHER_SERVER_PREFERENCE;
  long mode = SSL_MODE_ENABLE_PARTIAL_WRITE |
              SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER | SSL_MODE_RELEASE_BUFFERS |
              SSL_MODE_SMALL_BUFFERS;
  struct tls_version_filter *conf_ssl_methods =
      &bind_conf->ssl_conf.ssl_methods;
  int i, min, max, hole;
  ...
  if (conf_ssl_methods->flags &&
      (conf_ssl_methods->min || conf_ssl_methods->max))
    ha_warning(
        "Proxy '%s': no-sslv3/no-tlsv1x are ignored for bind '%s' at [%s:%d]. "
        "Use only 'ssl-min-ver' and 'ssl-max-ver' to fix.\n",
        bind_conf->frontend->id, bind_conf->arg, bind_conf->file,
        bind_conf->line);
  else
  ...
  if (!min) {
    if (!max || (max >= default_min_ver)) {
      ...
      ha_warning(
          "Proxy '%s': Ambiguous configuration for bind '%s' at [%s:%d]: the "
          "ssl-min-ver value is not configured and the ssl-max-ver value is "
          "lower than the default ssl-min-ver value (%s). "
          "Setting the ssl-min-ver to %s. Use 'ssl-min-ver' to fix this.\n",
          bind_conf->frontend->id, bind_conf->arg, bind_conf->file,
          bind_conf->line, methodVersions[default_min_ver].name,
          methodVersions[max].name);
      min = max;
    }
  }
  /* Real min and max should be determinate with configuration and openssl's
   * capabilities */
  if (min)
  ...
  for (i = CONF_TLSV_MIN; i <= CONF_TLSV_MAX; i++)
    ...
    if (methodVersions[i].option && !(flags & methodVersions[i].flag)) {
      if (min) {
        if (hole) {
          ha_warning("Proxy '%s': SSL/TLS versions range not contiguous for "
                     "bind '%s' at [%s:%d]. "
                     "Hole find for %s. Use only 'ssl-min-ver' and "
                     "'ssl-max-ver' to fix.\n",
                     bind_conf->frontend->id, bind_conf->arg, bind_conf->file,
                     bind_conf->line, methodVersions[hole].name);
          hole = 0;
        }
        max = i;
      } else {
  ...
  if (!min) {
    ha_alert("Proxy '%s': all SSL/TLS versions are disabled for bind '%s' at "
             "[%s:%d].\n",
             bind_conf->frontend->id, bind_conf->arg, bind_conf->file,
             bind_conf->line);
    cfgerr += 1;
...
#else /* openssl >= 1.1.0 */
  /* set the max_version is required to cap TLS version or activate new TLS
   * (v1.3) */
...
static inline void sh_ssl_sess_free_blocks(struct shared_block *first,
...
static inline struct shared_block *
sh_ssl_sess_first_block(struct sh_ssl_sess_hdr *sh_ssl_sess) {
  return (struct shared_block *)((unsigned char *)sh_ssl_sess -
                                 ((struct shared_block *)NULL)->data);
}
 ...
 */
static int sh_ssl_sess_store(unsigned char *s_id, unsigned char *data,
                             int data_len) {
  struct shared_block *first;
  struct sh_ssl_sess_hdr *sh_ssl_sess, *oldsh_ssl_sess;

  first = shctx_row_reserve_hot(ssl_shctx, NULL,
                                data_len + sizeof(struct sh_ssl_sess_hdr));
  if (!first) {
...
err:
  ...
  SSL_SESSION_set1_id_context(sess, (const unsigned char *)SHCTX_APPNAME,
                              strlen(SHCTX_APPNAME));

  return 0; /* do not increment session reference count */
}

/* SSL callback used on lookup an existing session cause none found in internal
 * cache */
SSL_SESSION *sh_ssl_sess_get_cb(SSL *ssl,
                                __OPENSSL_110_CONST__ unsigned char *key,
  ...
  *do_copy = 0;
  ...
  shctx_row_data_get(ssl_shctx, first, data, sizeof(struct sh_ssl_sess_hdr),
                     first->len - sizeof(struct sh_ssl_sess_hdr));
  ...
  /* decode ASN1 session */
  p = data;
  sess = d2i_SSL_SESSION(NULL, (const unsigned char **)&p,
                         first->len - sizeof(struct sh_ssl_sess_hdr));
...
void ssl_set_shctx(SSL_CTX *ctx) {
  SSL_CTX_set_session_id_context(ctx, (const unsigned char *)SHCTX_APPNAME,
                                 strlen(SHCTX_APPNAME));
 ...
 * We only need to copy the secret as there is a sample fetch for the
 * ClientRandom
 */
...
void SSL_CTX_keylog(const SSL *ssl, const char *line) {
  ...
  } else if (strncmp(line, "CLIENT_EARLY_TRAFFIC_SECRET ",
                     strlen("CLIENT_EARLY_TRAFFIC_SECRET ")) == 0) {
    if (keylog->client_early_traffic_secret)
      goto error;
    keylog->client_early_traffic_secret = dst;

  } else if (strncmp(line, "CLIENT_HANDSHAKE_TRAFFIC_SECRET ",
                     strlen("CLIENT_HANDSHAKE_TRAFFIC_SECRET ")) == 0) {
    if (keylog->client_handshake_traffic_secret)
      goto error;
    keylog->client_handshake_traffic_secret = dst;

  } else if (strncmp(line, "SERVER_HANDSHAKE_TRAFFIC_SECRET ",
                     strlen("SERVER_HANDSHAKE_TRAFFIC_SECRET ")) == 0) {
    if (keylog->server_handshake_traffic_secret)
      goto error;
    keylog->server_handshake_traffic_secret = dst;

  } else if (strncmp(line, "CLIENT_TRAFFIC_SECRET_0 ",
                     strlen("CLIENT_TRAFFIC_SECRET_0 ")) == 0) {
    if (keylog->client_traffic_secret_0)
      goto error;
    keylog->client_traffic_secret_0 = dst;

  } else if (strncmp(line, "SERVER_TRAFFIC_SECRET_0 ",
                     strlen("SERVER_TRAFFIC_SECRET_0 ")) == 0) {
    if (keylog->server_traffic_secret_0)
      goto error;
    keylog->server_traffic_secret_0 = dst;

  } else if (strncmp(line, "EARLY_EXPORTER_SECRET ",
                     strlen("EARLY_EXPORTER_SECRET ")) == 0) {
    if (keylog->early_exporter_secret)
      goto error;
    keylog->early_exporter_secret = dst;

  } else if (strncmp(line, "EXPORTER_SECRET ", strlen("EXPORTER_SECRET ")) ==
             0) {
 ...
 */
int ssl_sock_prepare_ctx(struct bind_conf *bind_conf,
                         struct ssl_bind_conf *ssl_conf, SSL_CTX *ctx,
                         char **err) {
  ...
  if (ssl_conf) {
    ...
    /* Real min and max should be determinate with configuration and openssl's
     * capabilities */
    min = conf_ssl_methods->min ? conf_ssl_methods->min
                                : bind_conf->ssl_conf.ssl_methods.min;
    max = conf_ssl_methods->max ? conf_ssl_methods->max
                                : bind_conf->ssl_conf.ssl_methods.max;
    if (min)
      flags |= (methodVersions[min].flag - 1);
    if (max)
      flags |= ~((methodVersions[max].flag << 1) - 1);
    ...
    if (!min) {
      memprintf(err,
                "%sProxy '%s': all SSL/TLS versions are disabled for bind '%s' "
                "at [%s:%d].\n",
                err && *err ? *err : "", bind_conf->frontend->id,
                bind_conf->arg, bind_conf->file, bind_conf->line);
      cfgerr |= ERR_ALERT | ERR_FATAL;
    }
  }

  switch ((ssl_conf && ssl_conf->verify) ? ssl_conf->verify
                                         : bind_conf->ssl_conf.verify) {
  ...
  if (verify & SSL_VERIFY_PEER) {
    char *ca_file = (ssl_conf && ssl_conf->ca_file)
                        ? ssl_conf->ca_file
                        : bind_conf->ssl_conf.ca_file;
    char *ca_verify_file = (ssl_conf && ssl_conf->ca_verify_file)
                               ? ssl_conf->ca_verify_file
                               : bind_conf->ssl_conf.ca_verify_file;
    char *crl_file = (ssl_conf && ssl_conf->crl_file)
                         ? ssl_conf->crl_file
                         : bind_conf->ssl_conf.crl_file;
    if (ca_file || ca_verify_file) {
      /* set CAfile to verify */
      if (ca_file && !ssl_set_verify_locations_file(ctx, ca_file)) {
        memprintf(err,
                  "%sProxy '%s': unable to set CA file '%s' for bind '%s' at "
                  "[%s:%d].\n",
                  err && *err ? *err : "", curproxy->id, ca_file,
                  bind_conf->arg, bind_conf->file, bind_conf->line);
        cfgerr |= ERR_ALERT | ERR_FATAL;
      }
      if (ca_verify_file &&
          !ssl_set_verify_locations_file(ctx, ca_verify_file)) {
        memprintf(err,
                  "%sProxy '%s': unable to set CA-no-names file '%s' for bind "
                  "'%s' at [%s:%d].\n",
                  err && *err ? *err : "", curproxy->id, ca_verify_file,
                  bind_conf->arg, bind_conf->file, bind_conf->line);
        cfgerr |= ERR_ALERT | ERR_FATAL;
      }
      if (ca_file && !((ssl_conf && ssl_conf->no_ca_names) ||
                       bind_conf->ssl_conf.no_ca_names)) {
        /* set CA names for client cert request, function returns void */
        SSL_CTX_set_client_CA_list(
            ctx, SSL_dup_CA_list(ssl_get_client_ca_file(ca_file)));
      }
    } else {
      memprintf(err,
                "%sProxy '%s': verify is enabled but no CA file specified for "
                "bind '%s' at [%s:%d].\n",
                err && *err ? *err : "", curproxy->id, bind_conf->arg,
    ...
    if (crl_file) {
      ...
      if (!ssl_set_cert_crl_file(store, crl_file)) {
        memprintf(err,
                  "%sProxy '%s': unable to configure CRL file '%s' for bind "
                  "'%s' at [%s:%d].\n",
                  err && *err ? *err : "", curproxy->id, crl_file,
                  bind_conf->arg, bind_conf->file, bind_conf->line);
        cfgerr |= ERR_ALERT | ERR_FATAL;
      } else {
  ...
  if (bind_conf->keys_ref) {
    if (!SSL_CTX_set_tlsext_ticket_key_cb(ctx, ssl_tlsext_ticket_key_cb)) {
      memprintf(err,
                "%sProxy '%s': unable to set callback for TLS ticket "
                "validation for bind '%s' at [%s:%d].\n",
                err && *err ? *err : "", curproxy->id, bind_conf->arg,
                bind_conf->file, bind_conf->line);
      cfgerr |= ERR_ALERT | ERR_FATAL;
    }
  ...
  ssl_set_shctx(ctx);
  conf_ciphers = (ssl_conf && ssl_conf->ciphers) ? ssl_conf->ciphers
                                                 : bind_conf->ssl_conf.ciphers;
  if (conf_ciphers && !SSL_CTX_set_cipher_list(ctx, conf_ciphers)) {
    memprintf(err,
              "%sProxy '%s': unable to set SSL cipher list to '%s' for bind "
              "'%s' at [%s:%d].\n",
              err && *err ? *err : "", curproxy->id, conf_ciphers,
              bind_conf->arg, bind_conf->file, bind_conf->line);
    cfgerr |= ERR_ALERT | ERR_FATAL;
  }
  ...
  conf_ciphersuites = (ssl_conf && ssl_conf->ciphersuites)
                          ? ssl_conf->ciphersuites
                          : bind_conf->ssl_conf.ciphersuites;
  if (conf_ciphersuites && !SSL_CTX_set_ciphersuites(ctx, conf_ciphersuites)) {
    memprintf(err,
              "%sProxy '%s': unable to set TLS 1.3 cipher suites to '%s' for "
              "bind '%s' at [%s:%d].\n",
              err && *err ? *err : "", curproxy->id, conf_ciphersuites,
              bind_conf->arg, bind_conf->file, bind_conf->line);
    cfgerr |= ERR_ALERT | ERR_FATAL;
  }
#endif
  ...
  if (global_ssl.default_dh_param == 0 && global_dh == NULL &&
      (ssl_dh_ptr_index == -1 ||
  ...
  if (ssl_conf_cur)
    SSL_CTX_set_next_protos_advertised_cb(ctx, ssl_sock_advertise_npn_protos,
                                          ssl_conf_cur);
#endif
#ifdef TLSEXT_TYPE_application_layer_protocol_negotiation
  ssl_conf_cur = NULL;
  ...
  if (conf_curves) {
    if (!SSL_CTX_set1_curves_list(ctx, conf_curves)) {
      memprintf(err,
                "%sProxy '%s': unable to set SSL curves list to '%s' for bind "
                "'%s' at [%s:%d].\n",
                err && *err ? *err : "", curproxy->id, conf_curves,
                bind_conf->arg, bind_conf->file, bind_conf->line);
      cfgerr |= ERR_ALERT | ERR_FATAL;
    }
  ...
  if (!conf_curves) {
...
#if (HA_OPENSSL_VERSION_NUMBER >= 0x10101000L)
    const char *ecdhe =
        (ssl_conf && ssl_conf->ecdhe)
            ? ssl_conf->ecdhe
            : (bind_conf->ssl_conf.ecdhe ? bind_conf->ssl_conf.ecdhe : NULL);
    ...
    if (!i || ((ecdh = EC_KEY_new_by_curve_name(i)) == NULL)) {
      memprintf(err,
                "%sProxy '%s': unable to set elliptic named curve to '%s' for "
                "bind '%s' at [%s:%d].\n",
                err && *err ? *err : "", curproxy->id, ecdhe, bind_conf->arg,
                bind_conf->file, bind_conf->line);
      cfgerr |= ERR_ALERT | ERR_FATAL;
    } else {
...
static int ssl_sock_srv_hostcheck(const char *pattern, const char *hostname) {
  const char *pattern_wildcard, *pattern_left_label_end,
      *hostname_left_label_end;
  ...
  if (hostname_left_label_end - hostname <
      (pattern_left_label_end - pattern) - 1)
    return 0;
  ...
  if ((prefixlen && (strncasecmp(pattern, hostname, prefixlen) != 0)) ||
      (suffixlen &&
       (strncasecmp(pattern_wildcard + 1, hostname_left_label_end - suffixlen,
                    suffixlen) != 0)))
    return 0;
...
static int ssl_sock_srv_verifycbk(int ok, X509_STORE_CTX *ctx) {
  ...
  i = -1;
  while (!ok && (i = X509_NAME_get_index_by_NID(cert_subject, NID_commonName,
                                                i)) != -1) {
...
int ssl_sock_prepare_srv_ctx(struct server *srv) {
  ...
  if (!srv->ssl_ctx.reused_sess) {
    if ((srv->ssl_ctx.reused_sess = calloc(
             1, global.nbthread * sizeof(*srv->ssl_ctx.reused_sess))) == NULL) {
      ha_alert("Proxy '%s', server '%s' [%s:%d] out of memory.\n", curproxy->id,
               srv->id, srv->conf.file, srv->conf.line);
      cfgerr++;
      return cfgerr;
    }
  }
  ...
  if (conf_ssl_methods->flags &&
      (conf_ssl_methods->min || conf_ssl_methods->max))
    ha_warning(
        "config : %s '%s': no-sslv3/no-tlsv1x are ignored for server '%s'. "
        "Use only 'ssl-min-ver' and 'ssl-max-ver' to fix.\n",
        proxy_type_str(curproxy), curproxy->id, srv->id);
  else
    flags = conf_ssl_methods->flags;
  ...
  for (i = CONF_TLSV_MIN; i <= CONF_TLSV_MAX; i++)
    ...
    if (methodVersions[i].option && !(flags & methodVersions[i].flag)) {
      if (min) {
        if (hole) {
          ha_warning("config : %s '%s': SSL/TLS versions range not contiguous "
                     "for server '%s'. "
                     "Hole find for %s. Use only 'ssl-min-ver' and "
                     "'ssl-max-ver' to fix.\n",
                     proxy_type_str(curproxy), curproxy->id, srv->id,
  ...
  if (!min) {
    ha_alert("config : %s '%s': all SSL/TLS versions are disabled for server "
             "'%s'.\n",
             proxy_type_str(curproxy), curproxy->id, srv->id);
    cfgerr += 1;
  }
  ...
  if (srv->ssl_ctx.client_crt) {
    if (SSL_CTX_use_PrivateKey_file(srv->ssl_ctx.ctx, srv->ssl_ctx.client_crt,
                                    SSL_FILETYPE_PEM) <= 0) {
      ha_alert("config : %s '%s', server '%s': unable to load SSL private key "
               "from PEM file '%s'.\n",
               proxy_type_str(curproxy), curproxy->id, srv->id,
               srv->ssl_ctx.client_crt);
      cfgerr++;
    } else if (SSL_CTX_use_certificate_chain_file(
                   srv->ssl_ctx.ctx, srv->ssl_ctx.client_crt) <= 0) {
      ha_alert("config : %s '%s', server '%s': unable to load ssl certificate "
               "from PEM file '%s'.\n",
               proxy_type_str(curproxy), curproxy->id, srv->id,
               srv->ssl_ctx.client_crt);
      cfgerr++;
    } else if (SSL_CTX_check_private_key(srv->ssl_ctx.ctx) <= 0) {
      ha_alert("config : %s '%s', server '%s': inconsistencies between private "
               "key and certificate loaded from PEM file '%s'.\n",
               proxy_type_str(curproxy), curproxy->id, srv->id,
               srv->ssl_ctx.client_crt);
      cfgerr++;
    }
  ...
  SSL_CTX_set_verify(srv->ssl_ctx.ctx, verify,
                     (srv->ssl_ctx.verify_host || (verify & SSL_VERIFY_PEER))
                         ? ssl_sock_srv_verifycbk
                         : NULL);
  if (verify & SSL_VERIFY_PEER) {
    if (srv->ssl_ctx.ca_file) {
      /* set CAfile to verify */
      if (!ssl_set_verify_locations_file(srv->ssl_ctx.ctx,
                                         srv->ssl_ctx.ca_file)) {
        ha_alert(
            "Proxy '%s', server '%s' [%s:%d] unable to set CA file '%s'.\n",
            curproxy->id, srv->id, srv->conf.file, srv->conf.line,
            srv->ssl_ctx.ca_file);
        cfgerr++;
      }
    } else {
      if (global.ssl_server_verify == SSL_SERVER_VERIFY_REQUIRED)
        ha_alert("Proxy '%s', server '%s' [%s:%d] verify is enabled by default "
                 ...
                 "use 'ssl-server-verify none' in the global section to "
                 "disable server-side verifications by default.\n",
                 curproxy->id, srv->id, srv->conf.file, srv->conf.line);
      else
        ha_alert("Proxy '%s', server '%s' [%s:%d] verify is enabled but no CA "
                 "file specified.\n",
                 curproxy->id, srv->id, srv->conf.file, srv->conf.line);
      cfgerr++;
    }
    ...
    if (srv->ssl_ctx.crl_file) {
      ...
      if (!ssl_set_cert_crl_file(store, srv->ssl_ctx.crl_file)) {
        ha_alert("Proxy '%s', server '%s' [%s:%d] unable to configure CRL file "
                 "'%s'.\n",
                 curproxy->id, srv->id, srv->conf.file, srv->conf.line,
  ...
  SSL_CTX_set_session_cache_mode(srv->ssl_ctx.ctx,
  ...
  if (srv->ssl_ctx.ciphers &&
    ...
    ha_alert("Proxy '%s', server '%s' [%s:%d] : unable to set SSL cipher list "
             "to '%s'.\n",
             curproxy->id, srv->id, srv->conf.file, srv->conf.line,
             srv->ssl_ctx.ciphers);
    cfgerr++;
  ...
  if (srv->ssl_ctx.ciphersuites &&
    ...
    ha_alert("Proxy '%s', server '%s' [%s:%d] : unable to set TLS 1.3 cipher "
             "suites to '%s'.\n",
             curproxy->id, srv->id, srv->conf.file, srv->conf.line,
  ...
  if (srv->ssl_ctx.alpn_str)
    SSL_CTX_set_alpn_protos(ctx, (unsigned char *)srv->ssl_ctx.alpn_str,
...
int ssl_sock_prepare_all_ctx(struct bind_conf *bind_conf) {
  ...
  if (!bind_conf->initial_ctx) {
    ...
    errcode |=
        ssl_sock_prepare_ctx(bind_conf, NULL, bind_conf->initial_ctx, &errmsg);
  }
  if (bind_conf->default_ctx)
    errcode |= ssl_sock_prepare_ctx(bind_conf, bind_conf->default_ssl_conf,
                                    bind_conf->default_ctx, &errmsg);
...
int ssl_sock_prepare_bind_conf(struct bind_conf *bind_conf) {
  ...
  if (!bind_conf->is_ssl) {
    if (bind_conf->default_ctx) {
      ha_warning("Proxy '%s': A certificate was specified but SSL was not "
                 "enabled on bind '%s' at [%s:%d] (use 'ssl').\n",
  ...
  if (!bind_conf->default_ctx) {
    if (bind_conf->strict_sni && !bind_conf->generate_certs) {
      ha_warning("Proxy '%s': no SSL certificate specified for bind '%s' at "
                 "[%s:%d], ssl connections will fail (use 'crt').\n",
                 px->id, bind_conf->arg, bind_conf->file, bind_conf->line);
    } else {
      ha_alert("Proxy '%s': no SSL certificate specified for bind '%s' at "
               "[%s:%d] (use 'crt').\n",
               px->id, bind_conf->arg, bind_conf->file, bind_conf->line);
      return -1;
    }
  ...
  if (!ssl_shctx && global.tune.sslcachesize) {
    alloc_ctx =
        shctx_init(&ssl_shctx, global.tune.sslcachesize,
                   sizeof(struct sh_ssl_sess_hdr) + SHSESS_BLOCK_MIN_SIZE, -1,
                   sizeof(*sh_ssl_sess_tree),
                   ((global.nbthread > 1) ||
                    (!global_ssl.private_cache && (global.nbproc > 1)))
                       ? 1
                       : 0);
    if (alloc_ctx <= 0) {
      if (alloc_ctx == SHCTX_E_INIT_LOCK)
        ha_alert("Unable to initialize the lock for the shared SSL session "
                 "cache. You can retry using the global statement "
                 "'tune.ssl.force-private-cache' but it could increase CPU "
                 "usage due to renegotiations if nbproc > 1.\n");
      else
 ...
 * may be NULL, in which case nothing is done. The default_ctx is nullified too.
 */
  ...
  retry_connect:
    ...
    if (__objt_server(conn->target)->ssl_ctx.reused_sess[tid].ptr) {
      const unsigned char *ptr =
          __objt_server(conn->target)->ssl_ctx.reused_sess[tid].ptr;
      SSL_SESSION *sess = d2i_SSL_SESSION(
          NULL, &ptr,
          __objt_server(conn->target)->ssl_ctx.reused_sess[tid].size);
    ...
    if (__objt_listener(conn->target)->bind_conf->ssl_conf.early_data) {
      ...
      SSL_set_max_early_data(ctx->ssl,
                             ...
                             (!b_is_null(&ctx->early_buf))
                                 ? global.tune.bufsize - global.tune.maxrewrite
                                 : 0);
    }
#endif
...
static int ssl_sock_handshake(struct connection *conn, unsigned int flag) {
  ...
  case OBJ_TYPE_SERVER:
    ...
    counters_px =
        EXTRA_COUNTERS_GET(srv->proxy->extra_counters_be, &ssl_stats_module);
    break;
  ...
  if (conn->flags & CO_FL_EARLY_SSL_HS) {
    ...
    while (1) {
      ret = SSL_read_early_data(ctx->ssl, b_tail(&ctx->early_buf),
                                b_room(&ctx->early_buf), &read_data);
  ...
  if (!(conn->flags & CO_FL_WAIT_L6_CONN) &&
      SSL_renegotiate_pending(ctx->ssl)) {
    char c;
...
#else
          /* access packet_length directly in OpenSSL <= 1.0.2; SSL_state() is
           * broken */
 ...
 */
static int ssl_subscribe(struct connection *conn, void *xprt_ctx,
 ...
 */
static int ssl_unsubscribe(struct connection *conn, void *xprt_ctx,
                           int event_type, struct wait_event *es) {
  struct ssl_sock_ctx *ctx = xprt_ctx;
 ...
 */
static int ssl_add_xprt(struct connection *conn, void *xprt_ctx,
                        void *toadd_ctx, const struct xprt_ops *toadd_ops,
                        void **oldxprt_ctx,
                        const struct xprt_ops **oldxprt_ops) {
  struct ssl_sock_ctx *ctx = xprt_ctx;
 ...
 * XPRT.
 */
static int ssl_remove_xprt(struct connection *conn, void *xprt_ctx,
                           void *toremove_ctx, const struct xprt_ops *newops,
                           void *newctx) {
  struct ssl_sock_ctx *ctx = xprt_ctx;
  ...
  return (ctx->xprt->remove_xprt(conn, ctx->xprt_ctx, toremove_ctx, newops,
                                 newctx));
}

static struct task *ssl_sock_io_cb(struct task *t, void *context,
 ...
 */
static size_t ssl_sock_to_buf(struct connection *conn, void *xprt_ctx,
 ...
 * function does not call the connection's polling update function, so the
 * caller is responsible for this. The buffer's output is not adjusted, it's up
 * to the caller to take care of this. It's up to the caller to update the
 * buffer's contents based on the return value.
 */
static size_t ssl_sock_from_buf(struct connection *conn, void *xprt_ctx,
  ...
  while (count) {
    ...
    try
      = b_contig_data(buf, done);
    if (try > count)
      try
        = count;

    if (!(flags & CO_SFL_STREAMER) &&
        !(ctx->xprt_st & SSL_SOCK_SEND_UNLIMITED) && global_ssl.max_record &&
        try > global_ssl.max_record) {
      try
        = global_ssl.max_record;
    } else {
    ...
    if (!SSL_is_init_finished(ctx->ssl) && conn_is_back(conn)) {
      ...
      if (try + ctx->sent_early_data > max_early) {
        try
          -= (try + ctx->sent_early_data) - max_early;
        if (try <= 0) {
          conn->flags |= CO_FL_SSL_WAIT_HS | CO_FL_WAIT_L6_CONN;
          tasklet_wakeup(ctx->wait_event.tasklet);
          break;
      ...
      ret =
          SSL_write_early_data(ctx->ssl, b_peek(buf, done), try, &written_data);
      if (ret == 1) {
        ret = written_data;
        ctx->sent_early_data += ret;
        if (objt_server(conn->target)) {
          conn->flags |=
              CO_FL_SSL_WAIT_HS | CO_FL_WAIT_L6_CONN | CO_FL_EARLY_DATA;
...
static void ssl_sock_close(struct connection *conn, void *xprt_ctx) {
  ...
  if (ctx) {
    if (ctx->wait_event.events != 0)
      ctx->xprt->unsubscribe(ctx->conn, ctx->xprt_ctx, ctx->wait_event.events,
                             &ctx->wait_event);
    if (ctx->subs) {
      ctx->subs->events = 0;
      tasklet_wakeup(ctx->subs->tasklet);
...
void ssl_sock_set_alpn(struct connection *conn, const unsigned char *alpn,
                       int len) {
#ifdef TLSEXT_TYPE_application_layer_protocol_negotiation
  struct ssl_sock_ctx *ctx;
...
int ssl_sock_get_remote_common_name(struct connection *conn,
  ...
  const struct buffer find_cn_chunk = {.area = (char *)&find_cn,
 ...
 */
static int ssl_sock_get_alpn(const struct connection *conn, void *xprt_ctx,
...
#ifdef TLSEXT_TYPE_application_layer_protocol_negotiation
  SSL_get0_alpn_selected(ctx->ssl, (const unsigned char **)str,
                         (unsigned *)len);
  if (*str)
    return 1;
#endif
#if defined(OPENSSL_NPN_NEGOTIATED) && !defined(OPENSSL_NO_NEXTPROTONEG)
  SSL_get0_next_proto_negotiated(ctx->ssl, (const unsigned char **)str,
                                 (unsigned *)len);
  if (*str)
    return 1;
#endif
...
int ssl_load_global_issuer_from_BIO(BIO *in, char *fp, char **err) {
  ...
  if (!chain) {
    memprintf(err,
              "unable to load issuers-chain %s : pem certificate not found.\n",
              fp);
    goto end;
  }
  if (!skid) {
    memprintf(
        err,
        "unable to load issuers-chain %s : SubjectKeyIdentifier not found.\n",
        fp);
    goto end;
  }
  if (!name) {
  ...
  for (node = eb64_lookup(&cert_issuer_tree, key); node;
       node = eb64_next(node)) {
    issuer = container_of(node, typeof(*issuer), node);
    if (!X509_NAME_cmp(
            name, X509_get_subject_name(sk_X509_value(issuer->chain, 0)))) {
...
struct issuer_chain *ssl_get0_issuer_chain(X509 *cert) {
  ...
  if (akid && akid->keyid) {
    ...
    for (node = eb64_lookup(&cert_issuer_tree, hk); node;
         node = eb64_next(node)) {
 ...
 */
#if (defined SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB && TLS_TICKETS_NO > 0)
static inline struct tls_keys_ref *
tlskeys_list_get_next(struct tls_keys_ref *getnext, struct list *end) {
...
static inline struct tls_keys_ref *
tlskeys_ref_lookup_ref(const char *reference) {
  int id;
  char *error;
...
/* dumps all tls keys. Relies on cli.i0 (non-null = only list file names),
 * cli.i1 (next index to be dumped), and cli.p0 (next key reference).
 */
static int cli_io_handler_tlskeys_files(struct appctx *appctx) {
  ...
  case STAT_ST_INIT:
    ...
    if (appctx->ctx.cli.p0 == NULL) {
      appctx->ctx.cli.p0 =
          LIST_ELEM(&tlskeys_reference, struct tls_keys_ref *, list);
      appctx->ctx.cli.p0 =
  ...
  case STAT_ST_LIST:
    while (appctx->ctx.cli.p0) {
      ...
      if (appctx->io_handler == cli_io_handler_tlskeys_entries) {
        ...
        while (appctx->ctx.cli.i1 < TLS_TICKETS_NO) {
          ...
          if (ref->key_size_bits == 128) {
            t2->data = a2base64(
                (char *)(ref->tlskeys +
                         (head + 2 + appctx->ctx.cli.i1) % TLS_TICKETS_NO),
                sizeof(struct tls_sess_key_128), t2->area, t2->size);
            chunk_appendf(&trash, "%d.%d %s\n", ref->unique_id,
                          appctx->ctx.cli.i1, t2->area);
          } else if (ref->key_size_bits == 256) {
            t2->data = a2base64(
                (char *)(ref->tlskeys +
                         (head + 2 + appctx->ctx.cli.i1) % TLS_TICKETS_NO),
                sizeof(struct tls_sess_key_256), t2->area, t2->size);
            chunk_appendf(&trash, "%d.%d %s\n", ref->unique_id,
                          appctx->ctx.cli.i1, t2->area);
          } else {
      ...
      if (appctx->ctx.cli.i0 ==
          0) /* don't display everything if not necessary */
        break;
...
static int cli_parse_show_tlskeys(char **args, char *payload,
  ...
  if (args[2][0] == '*') {
    ...
    if (!appctx->ctx.cli.p0)
      return cli_err(appctx,
                     "'show tls-keys' unable to locate referenced filename\n");
  }
  appctx->io_handler = cli_io_handler_tlskeys_entries;
  return 0;
}

static int cli_parse_set_tlskeys(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  struct tls_keys_ref *ref;
  int ret;
  ...
  if (!*args[3] || !*args[4])
    return cli_err(appctx, "'set ssl tls-key' expects a filename and the new "
                           "TLS key in base64 encoding.\n");

  ref = tlskeys_ref_lookup_ref(args[3]);
  if (!ref)
    return cli_err(appctx,
                   "'set ssl tls-key' unable to locate referenced filename\n");

  ret = base64dec(args[4], strlen(args[4]), trash.area, trash.size);
  if (ret < 0)
    return cli_err(
        appctx, "'set ssl tls-key' received invalid base64 encoded TLS key.\n");
...
static int cli_parse_set_ocspresponse(char **args, char *payload,
                                      struct appctx *appctx, void *private) {
#if (defined SSL_CTRL_SET_TLSEXT_STATUS_REQ_CB && !defined OPENSSL_NO_OCSP)
  char *err = NULL;
  int i, j, ret;
...
#else
  return cli_err(appctx, "HAProxy was compiled against a version of OpenSSL "
                         "that doesn't support OCSP stapling.\n");
#endif
}
...
#if (defined SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB && TLS_TICKETS_NO > 0)
        {{"show", "tls-keys", NULL},
         "show tls-keys [id|*]: show tls keys references or dump tls ticket "
         "keys when id specified",
         cli_parse_show_tlskeys,
         NULL},
        {{"set", "ssl", "tls-key", NULL},
         "set ssl tls-key [id|keyfile] <tlskey>: set the next TLS key for the "
         "<id> or <keyfile> listener to <tlskey>",
         cli_parse_set_tlskeys,
         NULL},
#endif
        {{"set", "ssl", "ocsp-response", NULL},
         NULL,
         cli_parse_set_ocspresponse,
         NULL},
        {{NULL}, NULL, NULL, NULL}}};
...
static enum act_parse_ret ssl_parse_wait_for_hs(const char **args,
                                                int *orig_arg, struct proxy *px,
...
static struct action_kw_list http_req_actions = {
    ILH, {{"wait-for-handshake", ssl_parse_wait_for_hs}, {/* END */}}};
...
static void ssl_sock_sctl_free_func(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
  ...
  if (ptr) {
    ...
    free(ptr);
      ...
      !defined OPENSSL_NO_OCSP) &&                                             \
     !defined OPENSSL_IS_BORINGSSL)
static void ssl_sock_ocsp_free_func(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
                                    int idx, long argl, void *argp) {
  struct ocsp_cbk_arg *ocsp_arg;
...
static void ssl_sock_capture_free_func(void *parent, void *ptr,
...
#ifdef HAVE_OPENSSL_KEYLOG
static void ssl_sock_keylog_free_func(void *parent, void *ptr,
                                      CRYPTO_EX_DATA *ad, int idx, long argl,
                                      void *argp) {
  struct ssl_keylog *keylog;
...
__attribute__((constructor)) static void __ssl_sock_init(void) {
  ...
  if (global_ssl.listen_default_ciphers)
    global_ssl.listen_default_ciphers =
  ...
  ssl_locking_init();
     ...
     !defined OPENSSL_IS_BORINGSSL)
  sctl_ex_index =
      SSL_CTX_get_ex_new_index(0, NULL, NULL, NULL, ssl_sock_sctl_free_func);
#endif
     ...
     !defined OPENSSL_IS_BORINGSSL)
  ocsp_ex_index =
      SSL_CTX_get_ex_new_index(0, NULL, NULL, NULL, ssl_sock_ocsp_free_func);
#endif

  ssl_app_data_index = SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
  ssl_capture_ptr_index =
      SSL_get_ex_new_index(0, NULL, NULL, NULL, ssl_sock_capture_free_func);
#ifdef HAVE_OPENSSL_KEYLOG
  ssl_keylog_index =
      SSL_get_ex_new_index(0, NULL, NULL, NULL, ssl_sock_keylog_free_func);
#endif
#ifndef OPENSSL_NO_ENGINE
...
__attribute__((destructor)) static void __ssl_sock_deinit(void) {
#if (defined SSL_CTRL_SET_TLSEXT_HOSTNAME &&                                   \
     !defined SSL_NO_GENERATE_CERTIFICATES)
  if (ssl_ctx_lru_tree) {
...
#if (HA_OPENSSL_VERSION_NUMBER >= 0x00907000L) &&                              \
    (HA_OPENSSL_VERSION_NUMBER < 0x10100000L)
