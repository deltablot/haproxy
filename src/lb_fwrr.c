    ...
    if (srv == p->lbprm.fbck) {
      ...
      do {
        srv2 = srv2->next;
               ...
               !((srv2->flags & SRV_F_BACKUP) && srv_willbe_usable(srv2)));
      p->lbprm.fbck = srv2;
    }
  } else {
...
static void fwrr_set_server_status_up(struct server *srv) {
  ...
  srv->npos =
      grp->curr_pos +
      (grp->next_weight + grp->curr_weight - grp->curr_pos) / srv->next_eweight;
  fwrr_queue_srv(srv);
...
static void fwrr_update_server_weight(struct server *srv) {
  ...
  if (srv->lb_tree == grp->init) {
    ...
    fwrr_get_srv(srv);
    srv->npos =
        grp->curr_pos + (grp->next_weight + grp->curr_weight - grp->curr_pos) /
                            srv->next_eweight;
    fwrr_queue_srv(srv);
  } else {
 ...
 * The lbprm's lock must be held. The server's lock is not used.
 */
static inline void fwrr_queue_by_weight(struct eb_root *root,
                                        struct server *s) {
...
void fwrr_init_server_groups(struct proxy *p) {
  ...
  p->lbprm.fwrr.act.curr = p->lbprm.fwrr.act.t0 = p->lbprm.fwrr.act.t1 =
      init_head;
  ...
  p->lbprm.fwrr.bck.curr = p->lbprm.fwrr.bck.t0 = p->lbprm.fwrr.bck.t1 =
      init_head;
  ...
  for (srv = p->srv; srv; srv = srv->next) {
    if (!srv_currently_usable(srv))
      continue;
    fwrr_queue_by_weight((srv->flags & SRV_F_BACKUP) ? p->lbprm.fwrr.bck.init
...
static void fwrr_queue_srv(struct server *s) {
  ...
  if (!srv_willbe_usable(s)) {
  ...
  } else if (s->next_eweight <= 0 || s->npos >= 2 * grp->curr_weight ||
    ...
    s->lb_node.key = SRV_UWGHT_RANGE * s->npos +
                     (unsigned)(SRV_EWGHT_MAX + s->rweight - s->next_eweight) /
                         BE_WEIGHT_SCALE;
...
static void fwrr_get_srv(struct server *s) {
  struct proxy *p = s->proxy;
  struct fwrr_group *grp =
      (s->flags & SRV_F_BACKUP) ? &p->lbprm.fwrr.bck : &p->lbprm.fwrr.act;
 ...
 */
static inline void fwrr_update_position(struct fwrr_group *grp,
                                        struct server *s) {
 ...
 */
struct server *fwrr_get_next_server(struct proxy *p,
