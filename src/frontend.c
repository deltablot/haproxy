...
int frontend_accept(struct stream *s) {
  ...
  if ((fe->mode == PR_MODE_TCP || fe->mode == PR_MODE_HTTP) &&
    ...
    if (likely(!LIST_ISEMPTY(&fe->logformat))) {
      ...
      send_log(fe, LOG_INFO,
               "Connect from unknown source to listener %d (%s/%s)\n", l->luid,
      ...
      case AF_INET6:
        ...
        send_log(fe, LOG_INFO, "Connect from %s:%d to %s:%d (%s/%s)\n", pn,
                 get_host_port(conn->src), sn, port, fe->id,
                 (fe->mode == PR_MODE_HTTP) ? "HTTP" : "TCP");
        break;
      case AF_UNIX:
  ...
  if (unlikely((global.mode & MODE_DEBUG) && conn &&
    ...
    if (!conn_get_src(conn)) {
      chunk_printf(&trash,
                   "%08x:%s.accept(%04x)=%04x from [listener:%d] ALPN=%s\n",
                   s->uniq_id, fe->id, (unsigned short)l->rx.fd,
                   (unsigned short)conn->handle.fd, l->luid, alpn);
    } else
      switch (addr_to_str(conn->src, pn, sizeof(pn))) {
      case AF_INET:
      case AF_INET6:
        chunk_printf(&trash, "%08x:%s.accept(%04x)=%04x from [%s:%d] ALPN=%s\n",
                     s->uniq_id, fe->id, (unsigned short)l->rx.fd,
                     (unsigned short)conn->handle.fd, pn,
                     get_host_port(conn->src), alpn);
        break;
      case AF_UNIX:
        /* UNIX socket, only the destination is known */
        chunk_printf(&trash,
                     "%08x:%s.accept(%04x)=%04x from [unix:%d] ALPN=%s\n",
                     s->uniq_id, fe->id, (unsigned short)l->rx.fd,
                     (unsigned short)conn->handle.fd, l->luid, alpn);
        break;
      }
...
static int smp_fetch_fe_id(const struct arg *args, struct sample *smp,
...
static int smp_fetch_fe_name(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
...
static int smp_fetch_fe_defbe(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
...
/* set temp integer to the number of HTTP requests per second reaching the
 * frontend. Accepts exactly 1 argument. Argument is a frontend, other types
 * will cause an undefined behaviour.
 */
static int smp_fetch_fe_req_rate(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
 ...
 * will cause an undefined behaviour.
 */
static int smp_fetch_fe_sess_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 */
static int smp_fetch_fe_conn(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
...
static int smp_fetch_fe_client_timeout(const struct arg *args,
