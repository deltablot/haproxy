...
static struct {
  ...
  char information_list_separator; /* the separator used in request to separate
                                      values */
  struct list
      information_list; /* the list of WURFL data to return into request */
  void *handle;         /* the handle to WURFL engine */
  struct eb_root
      btree; /* btree containing info (name/type) on WURFL data to return */
} global_wurfl = {
    .data_file = NULL,
    .cache_size = NULL,
...
static const char *ha_wurfl_retrieve_header(const char *header_name,
                                            const void *wh);
static const char *ha_wurfl_get_wurfl_root_id(wurfl_handle wHandle,
                                              wurfl_device_handle dHandle);
static const char *ha_wurfl_get_wurfl_id(wurfl_handle wHandle,
                                         wurfl_device_handle dHandle);
static const char *ha_wurfl_get_wurfl_isdevroot(wurfl_handle wHandle,
                                                wurfl_device_handle dHandle);
static const char *ha_wurfl_get_wurfl_useragent(wurfl_handle wHandle,
                                                wurfl_device_handle dHandle);
static const char *ha_wurfl_get_wurfl_api_version(wurfl_handle wHandle,
                                                  wurfl_device_handle dHandle);
static const char *
ha_wurfl_get_wurfl_engine_target(wurfl_handle wHandle,
                                 wurfl_device_handle dHandle);
static const char *ha_wurfl_get_wurfl_info(wurfl_handle wHandle,
                                           wurfl_device_handle dHandle);
static const char *
ha_wurfl_get_wurfl_last_load_time(wurfl_handle wHandle,
                                  wurfl_device_handle dHandle);
static const char *
ha_wurfl_get_wurfl_normalized_useragent(wurfl_handle wHandle,
                                        wurfl_device_handle dHandle);
static const char *
ha_wurfl_get_wurfl_useragent_priority(wurfl_handle wHandle,
                                      wurfl_device_handle dHandle);
static const char *(*ha_wurfl_get_property_callback(char *name))(
    wurfl_handle wHandle, wurfl_device_handle dHandle);
...
static const struct {
  ...
  const char *(*func)(wurfl_handle wHandle, wurfl_device_handle dHandle);
    ...
    {"wurfl_engine_target",
     ha_wurfl_get_wurfl_engine_target}, // kept for backward conf file compat
    ...
    {"wurfl_useragent_priority",
     ha_wurfl_get_wurfl_useragent_priority}, // kept for backward conf file
                                             // compat
};
 ...
 */
static int ha_wurfl_cfg_data_file(char **args, int section_type,
...
static int ha_wurfl_cfg_cache(char **args, int section_type,
                              struct proxy *curpx, struct proxy *defpx,
                              const char *file, int line, char **err) {
  if (*(args[1]) == 0) {
    memprintf(err, "WURFL: %s expects a value.\n", args[0]);
...
static int ha_wurfl_cfg_engine_mode(char **args, int section_type,
                                    struct proxy *curpx, struct proxy *defpx,
                                    const char *file, int line, char **err) {
...
static int ha_wurfl_cfg_information_list_separator(
    char **args, int section_type, struct proxy *curpx, struct proxy *defpx,
    const char *file, int line, char **err) {
  if (*(args[1]) == 0) {
...
static int ha_wurfl_cfg_information_list(char **args, int section_type,
                                         struct proxy *curpx,
...
static int ha_wurfl_cfg_patch_file_list(char **args, int section_type,
                                        struct proxy *curpx,
                                        struct proxy *defpx, const char *file,
                                        int line, char **err) {
  int argIdx = 1;
...
static int ha_wurfl_cfg_useragent_priority(char **args, int section_type,
                                           struct proxy *curpx,
                                           struct proxy *defpx,
...
static int ha_wurfl_init(void) {
  ...
  if (LIST_ISEMPTY(&global_wurfl.information_list)) {
    ha_warning("WURFL: missing wurfl-information-list parameter in global "
               "configuration\n");
    return ERR_WARN;
  } else {
    ...
    list_for_each_entry(wi, &global_wurfl.information_list, list) {
      ...
      if (ebst_lookup(&global_wurfl.btree, wi->data.name) == NULL) {
        if ((wi->data.func_callback = (PROP_CALLBACK_FUNC)
                 ha_wurfl_get_property_callback(wi->data.name)) != NULL) {
          wi->data.type = HA_WURFL_DATA_TYPE_PROPERTY;
#ifdef WURFL_DEBUG
          ha_notice("WURFL: [%s] is a valid wurfl data [property]\n",
                    wi->data.name);
#endif
        } else if (wurfl_has_virtual_capability(global_wurfl.handle,
                                                wi->data.name)) {
          ...
          // by default a cap type is assumed to be and we control it on engine
          // load
          wi->data.type = HA_WURFL_DATA_TYPE_CAP;

          if (wurfl_add_requested_capability(global_wurfl.handle,
                                             wi->data.name) != WURFL_OK) {
            ha_warning("WURFL: capability filtering failed - %s\n",
                       wurfl_get_error_message(global_wurfl.handle));
            return ERR_WARN;
          }

          ha_notice("WURFL: [%s] treated as wurfl capability. Will check its "
                    "validity later, on engine load\n",
                    wi->data.name);
        }
        ...
        if (wn == NULL) {
          ha_warning(
              "WURFL: Error allocating memory for information tree element.\n");
          return ERR_WARN;
        }
  ...
  if (!LIST_ISEMPTY(&global_wurfl.patch_file_list)) {
    ...
    list_for_each_entry(wp, &global_wurfl.patch_file_list, list) {
      if (wurfl_add_patch(global_wurfl.handle, wp->patch_file_path) !=
          WURFL_OK) {
  ...
  if (global_wurfl.cache_size != NULL) {
    if (strpbrk(global_wurfl.cache_size, ",") != NULL) {
      wurfl_result_code = wurfl_set_cache_provider(
          global_wurfl.handle, WURFL_CACHE_PROVIDER_DOUBLE_LRU,
          global_wurfl.cache_size);
    } else {
      if (strcmp(global_wurfl.cache_size, "0")) {
        wurfl_result_code = wurfl_set_cache_provider(global_wurfl.handle,
                                                     WURFL_CACHE_PROVIDER_LRU,
                                                     global_wurfl.cache_size);
      } else {
        wurfl_result_code = wurfl_set_cache_provider(
            global_wurfl.handle, WURFL_CACHE_PROVIDER_NONE, 0);
      }
    }
...
static int ha_wurfl_get_all(const struct arg *args, struct sample *smp,
  ...
  list_for_each_entry(wi, &global_wurfl.information_list, list) {
    ...
    case HA_WURFL_DATA_TYPE_UNKNOWN:
      ha_wurfl_log("WURFL: %s is of an %s type\n", wi->data.name,
                   HA_WURFL_DATA_TYPE_UNKNOWN_STRING);
#ifdef WURFL_HEADER_WITH_DETAILS
      // write WURFL property type and name before its value...
      chunk_appendf(temp, "%s=%s", HA_WURFL_DATA_TYPE_UNKNOWN_STRING,
                    wi->data.name);
#endif
      break;
    case HA_WURFL_DATA_TYPE_CAP:
      ha_wurfl_log("WURFL: %s is a %s\n", wi->data.name,
                   HA_WURFL_DATA_TYPE_CAP_STRING);
#ifdef WURFL_HEADER_WITH_DETAILS
      // write WURFL property type and name before its value...
      chunk_appendf(temp, "%s=%s=", HA_WURFL_DATA_TYPE_CAP_STRING,
                    wi->data.name);
#endif
      chunk_appendf(temp, "%s",
                    wurfl_device_get_capability(dHandle, wi->data.name));
      break;
    case HA_WURFL_DATA_TYPE_VCAP:
      ha_wurfl_log("WURFL: %s is a %s\n", wi->data.name,
                   HA_WURFL_DATA_TYPE_VCAP_STRING);
#ifdef WURFL_HEADER_WITH_DETAILS
      // write WURFL property type and name before its value...
      chunk_appendf(temp, "%s=%s=", HA_WURFL_DATA_TYPE_VCAP_STRING,
                    wi->data.name);
#endif
      chunk_appendf(
          temp, "%s",
          wurfl_device_get_virtual_capability(dHandle, wi->data.name));
      break;
    case HA_WURFL_DATA_TYPE_PROPERTY:
      ha_wurfl_log("WURFL: %s is a %s\n", wi->data.name,
                   HA_WURFL_DATA_TYPE_PROPERTY_STRING);
#ifdef WURFL_HEADER_WITH_DETAILS
      // write WURFL property type and name before its value...
      chunk_appendf(temp, "%s=%s=", HA_WURFL_DATA_TYPE_PROPERTY_STRING,
                    wi->data.name);
#endif
      chunk_appendf(temp, "%s",
                    wi->data.func_callback(global_wurfl.handle, dHandle));
      break;
    }
...
static int ha_wurfl_get(const struct arg *args, struct sample *smp,
                        const char *kw, void *private) {
  wurfl_device_handle dHandle;
  struct buffer *temp;
...
#endif
        chunk_appendf(temp, "%s",
                      wurfl_device_get_capability(dHandle, wn->name));
        break;
      case HA_WURFL_DATA_TYPE_VCAP:
        ha_wurfl_log("WURFL: %s is a %s\n", wn->name,
                     HA_WURFL_DATA_TYPE_VCAP_STRING);
...
#endif
        chunk_appendf(temp, "%s",
                      wurfl_device_get_virtual_capability(dHandle, wn->name));
        break;
      case HA_WURFL_DATA_TYPE_PROPERTY:
        ha_wurfl_log("WURFL: %s is a %s\n", wn->name,
                     HA_WURFL_DATA_TYPE_PROPERTY_STRING);
...
#endif
        chunk_appendf(temp, "%s",
                      wn->func_callback(global_wurfl.handle, dHandle));
        break;
      }
...
static struct cfg_kw_list wurflcfg_kws = {
    {},
    {
        ...
        {CFG_GLOBAL, "wurfl-information-list-separator",
        ...
        {CFG_GLOBAL, "wurfl-useragent-priority",
...
static struct sample_fetch_kw_list fetch_kws = {
    ILH,
    {
        {"wurfl-get-all", ha_wurfl_get_all, 0, NULL, SMP_T_STR, SMP_USE_HRQHV},
        {"wurfl-get", ha_wurfl_get,
         ARG12(1, STR, STR, STR, STR, STR, STR, STR, STR, STR, STR, STR, STR),
         NULL, SMP_T_STR, SMP_USE_HRQHV},
        {NULL, NULL, 0, 0, 0},
    }};
...
static const char *(*ha_wurfl_get_property_callback(char *name))(
  ...
  while (begin <= end) {
    ...
    if ((cond = strcmp(wurfl_properties_function_map[position].name, name)) ==
        0) {
...
static const char *ha_wurfl_retrieve_header(const char *header_name,
  ...
  // We could skip this check since ha_wurfl_retrieve_header is called from
  // inside ha_wurfl_get()/ha_wurfl_get_all() that already perform the same
  // check We choose to keep it in case ha_wurfl_retrieve_header will be called
  // directly
  htx = smp_prefetch_htx(smp, chn, NULL, 1);
  if (!htx) {
    return NULL;
  ...
  // If 4th param is set, it works on full-line headers in whose comma is not a
  // delimiter but is part of the syntax
...
static void ha_wurfl_register_build_options() {
  ...
  memprintf(&ptr, "Built with WURFL support (%sversion %s)",
            strcmp(ver, "1.11.2.100") ? "" : "dummy library ", ver);
