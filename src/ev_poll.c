...
static void _do_poll(struct poller *p, int exp, int wake) {
  ...
  for (fds = 0; (fds * 8 * sizeof(**fd_evts)) < maxfd; fds++) {
    ...
    for (count = 0, fd = fds * 8 * sizeof(**fd_evts);
         count < 8 * sizeof(**fd_evts) && fd < maxfd; count++, fd++) {
      sr = (rn >> count) & 1;
      sw = (wn >> count) & 1;
      if ((sr | sw)) {
        if (!fdtab[fd].owner) {
        ...
        poll_events[nbfd].events =
            (sr ? (POLLIN | POLLRDHUP) : 0) | (sw ? POLLOUT : 0);
        nbfd++;
      }
    }
  }
...
static int _do_init(struct poller *p) {
  ...
  p->private = NULL;
  fd_evts_bytes = (global.maxsock + sizeof(**fd_evts) * 8 - 1) /
                  (sizeof(**fd_evts) * 8) * sizeof(**fd_evts);
 ...
 */
static int _do_test(struct poller *p) { return 1; }
