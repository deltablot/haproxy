...
static int get_http_auth(struct sample *smp, struct htx *htx) {
  ...
  if (!p || p == ctx.value.ptr) /* if no space was found or if the space is the
                                   first character */
    return 0;
  len = p - ctx.value.ptr;
  ...
  if (!strncasecmp("Basic", auth_method.area, auth_method.data)) {
    ...
    len = base64dec(txn->auth.method_data.area, txn->auth.method_data.data,
...
struct htx *smp_prefetch_htx(struct sample *smp, struct channel *chn,
  ...
  if (chn && (((smp->opt & SMP_OPT_DIR) == SMP_OPT_DIR_REQ &&
               (chn->flags & CF_ISRESP)) ||
              ((smp->opt & SMP_OPT_DIR) == SMP_OPT_DIR_RES &&
               !(chn->flags & CF_ISRESP))))
    return 0;
  ...
  if (!s || !chn) {
    if (check) {
      ...
      if (vol && !sl) {
        /* The start-line was already forwarded, it is too late to fetch
         * anything */
        return NULL;
      }
      goto end;
  ...
  if (IS_HTX_STRM(s)) {
    ...
    h1m_init_req(&h1m);
    ret = h1_headers_to_hdr_list(b_head(buf), b_stop(buf), hdrs,
                                 sizeof(hdrs) / sizeof(hdrs[0]), &h1m, &h1sl);
    if (ret <= 0) {
      /* Invalid or too big*/
    ...
    sl = htx_add_stline(htx, HTX_BLK_REQ_SL, flags, h1sl.rq.m, h1sl.rq.u,
                        h1sl.rq.v);
 ...
 */
static int smp_fetch_meth(const struct arg *args, struct sample *smp,
...
static int smp_fetch_rqver(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
...
static int smp_fetch_stver(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
...
static int smp_fetch_stcode(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  struct channel *chn = SMP_RES_CHN(smp);
  struct check *check = objt_check(smp->sess->origin);
  struct htx *htx = smp_prefetch_htx(smp, chn, check, 1);
...
static int smp_fetch_uniqueid(const struct arg *args, struct sample *smp,
  ...
  unique_id =
      stream_generate_unique_id(smp->strm, &smp->sess->fe->format_unique_id);
 ...
 */
static int smp_fetch_hdrs(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
 ...
 */
static int smp_fetch_hdrs_bin(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
 ...
 */
static int smp_fetch_body(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  ...
  if (!finished &&
      (check || (chn && !channel_full(chn, global.tune.maxrewrite) &&
 ...
 * has been waited for using http-buffer-request.
 */
static int smp_fetch_body_len(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
 ...
 */
static int smp_fetch_body_size(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
...
static int smp_fetch_url(const struct arg *args, struct sample *smp,
                         const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
...
static int smp_fetch_url_ip(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
...
static int smp_fetch_url_port(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
 ...
 */
static int smp_fetch_fhdr(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  ...
  if (http_get_htx_fhdr(htx, name, occ, ctx, &smp->data.u.str.area,
                        &smp->data.u.str.data))
    return 1;
  smp->flags &= ~SMP_F_NOT_LAST;
  return 0;
 ...
 * returns full lines instead (useful for User-Agent or Date for example).
 */
static int smp_fetch_fhdr_cnt(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
...
static int smp_fetch_hdr_names(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
 ...
 */
static int smp_fetch_hdr(const struct arg *args, struct sample *smp,
                         const char *kw, void *private) {
  /* possible keywords: req.hdr / hdr, res.hdr / shdr */
  struct channel *chn =
      ((kw[0] == 'h' || kw[2] == 'q') ? SMP_REQ_CHN(smp) : SMP_RES_CHN(smp));
  struct check *check =
      ((kw[0] == 's' || kw[2] == 's') ? objt_check(smp->sess->origin) : NULL);
  struct htx *htx = smp_prefetch_htx(smp, chn, check, 1);
  struct http_hdr_ctx *ctx = smp->ctx.a[0];
  struct ist name;
  int occ = 0;
  ...
  smp->data.type = SMP_T_STR;
  smp->flags |= SMP_F_VOL_HDR | SMP_F_CONST;
  if (http_get_htx_hdr(htx, name, occ, ctx, &smp->data.u.str.area,
                       &smp->data.u.str.data))
    return 1;
 ...
 */
static int smp_fetch_chn_hdr(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
 ...
 */
static int smp_fetch_hdr_cnt(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
 ...
 */
static int smp_fetch_hdr_val(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
  int ret = smp_fetch_hdr(args, smp, kw, private);

  if (ret > 0) {
    smp->data.type = SMP_T_SINT;
    smp->data.u.sint = strl2ic(smp->data.u.str.area, smp->data.u.str.data);
  }
 ...
 */
static int smp_fetch_hdr_ip(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  int ret;
  ...
  while ((ret = smp_fetch_hdr(args, smp, kw, private)) > 0) {
    if (smp->data.u.str.data < temp->size - 1) {
      memcpy(temp->area, smp->data.u.str.area, smp->data.u.str.data);
 ...
 * for 'pathq' keyword.
 */
static int smp_fetch_path(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
 ...
 */
static int smp_fetch_base(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
 ...
 * ('/'). This means that '*' will not be added, resulting in exactly the first
 * Host entry. If no Host header is found, then the path is used. The resulting
 * value is hashed using the path hash followed by a full avalanche hash and
 * provides a 32-bit integer value. This fetch is useful for tracking per-path
 * activity on high-traffic sites without having to store whole paths.
 */
static int smp_fetch_base32(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
 ...
 */
static int smp_fetch_base32_src(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  struct buffer *temp;
  ...
  *(unsigned int *)temp->area = htonl(smp->data.u.sint);
  ...
  case AF_INET:
    memcpy(temp->area + temp->data,
           &((struct sockaddr_in *)cli_conn->src)->sin_addr, 4);
    temp->data += 4;
    break;
  case AF_INET6:
    memcpy(temp->area + temp->data,
           &((struct sockaddr_in6 *)cli_conn->src)->sin6_addr, 16);
    temp->data += 16;
    break;
  default:
    return 0;
 ...
 */
static int smp_fetch_query(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct htx_sl *sl;
...
static int smp_fetch_proto_http(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
 ...
 */
static int smp_fetch_http_first_req(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  if (!smp->strm)
    return 0;
 ...
 */
static int smp_fetch_http_auth_type(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct http_txn *txn;
 ...
 * get_http_auth()
 */
static int smp_fetch_http_auth_user(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct http_txn *txn;
 ...
 * get_http_auth()
 */
static int smp_fetch_http_auth_pass(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct http_txn *txn;
...
static int smp_fetch_http_auth(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
...
/* Accepts exactly 1 argument of type userlist */
static int smp_fetch_http_auth_grp(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
 ...
 */
static int smp_fetch_capture_req_hdr(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct proxy *fe;
  int idx;
 ...
 * the "capture" option in the configuration file
 */
static int smp_fetch_capture_res_hdr(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct proxy *fe;
  int idx;
  ...
  if (idx > (fe->nb_rsp_cap - 1) || smp->strm->res_cap == NULL ||
      smp->strm->res_cap[idx] == NULL)
    return 0;
...
/* Extracts the METHOD in the HTTP request, the txn->uri should be filled before
 * the call */
static int smp_fetch_capture_req_method(const struct arg *args,
...
/* Extracts the path in the HTTP request, the txn->uri should be filled before
 * the call  */
static int smp_fetch_capture_req_uri(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct http_txn *txn;
  struct ist path;
  const char *ptr;
 ...
 */
static int smp_fetch_capture_req_ver(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct http_txn *txn;
 ...
 * as a string (either "HTTP/1.0" or "HTTP/1.1").
 */
static int smp_fetch_capture_res_ver(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct http_txn *txn;
 ...
 */
static int smp_fetch_cookie(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  /* possible keywords: req.cookie / cookie / cook, res.cookie / scook /
   * set-cookie */
  struct channel *chn =
      ((kw[0] == 'c' || kw[2] == 'q') ? SMP_REQ_CHN(smp) : SMP_RES_CHN(smp));
  struct check *check =
      ((kw[0] == 's' || kw[2] == 's') ? objt_check(smp->sess->origin) : NULL);
  struct htx *htx = smp_prefetch_htx(smp, chn, check, 1);
  ...
  hdr = (!(check || (chn && chn->flags & CF_ISRESP)) ? ist("Cookie")
                                                     : ist("Set-Cookie"));
  ...
  while (1) {
    ...
    smp->flags |= SMP_F_CONST;
    smp->ctx.a[0] =
        http_extract_cookie_value(smp->ctx.a[0], smp->ctx.a[1], cook, cook_l,
 ...
 */
static int smp_fetch_chn_cookie(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
 ...
 */
static int smp_fetch_cookie_cnt(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  ...
  while (1) {
    ...
    smp->data.type = SMP_T_STR;
    smp->flags |= SMP_F_CONST;
    while ((val_beg = http_extract_cookie_value(
                val_beg, val_end, cook, cook_l,
                (smp->opt & SMP_OPT_DIR) == SMP_OPT_DIR_REQ,
                &smp->data.u.str.area, &smp->data.u.str.data))) {
      cnt++;
    }
 ...
 */
static int smp_fetch_cookie_val(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
 ...
 */
static int smp_fetch_param(char delim, const char *name, int name_len,
  ...
  if (!http_find_next_url_param(chunks, name, name_len, &vstart, &vend, delim))
    return 0;
   ...
   */
  smp->data.type = SMP_T_STR;
  if (chunks[2] && vstart >= chunks[0] && vstart <= chunks[1] &&
      vend >= chunks[2] && vend <= chunks[3]) {
    /* Wrapped case. */
    temp = get_trash_chunk();
    memcpy(temp->area, vstart, chunks[1] - vstart);
    memcpy(temp->area + (chunks[1] - vstart), chunks[2], vend - chunks[2]);
    smp->data.u.str.area = temp->area;
 ...
 */
static int smp_fetch_url_param(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  char delim = '?';
  const char *name;
  ...
  if (!smp->ctx.a[0]) { // first call, find the query string
    ...
    sl = http_get_stline(htx);
    smp->ctx.a[0] =
        http_find_param_list(HTX_SL_REQ_UPTR(sl), HTX_SL_REQ_ULEN(sl), delim);
    if (!smp->ctx.a[0])
      return 0;
 ...
 */
static int smp_fetch_body_param(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  const char *name;
  int name_len;
...
/* Return the signed integer value for the specified url parameter (see
 * url_param above).
 */
static int smp_fetch_url_param_val(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
 ...
 * ('/'). This means that '*' will not be added, resulting in exactly the first
 * Host entry. If no Host header is found, then the path is used. The resulting
 * value is hashed using the url hash followed by a full avalanche hash and
 * provides a 32-bit integer value. This fetch is useful for tracking per-URL
 * activity on high-traffic sites without having to store whole paths. this
 * differs from the base32 functions in that it includes the url parameters as
 * well as the path
 */
static int smp_fetch_url32(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
  struct channel *chn = SMP_REQ_CHN(smp);
  struct htx *htx = smp_prefetch_htx(smp, chn, NULL, 1);
  struct http_hdr_ctx ctx;
 ...
 * 8 bytes would still work.
 */
static int smp_fetch_url32_src(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  struct buffer *temp;
  struct connection *cli_conn = objt_conn(smp->sess->origin);
...
static struct sample_fetch_kw_list sample_fetch_keywords = {
    ILH,
    {
        ...
        {"capture.req.hdr", smp_fetch_capture_req_hdr, ARG1(1, SINT), NULL,
         SMP_T_STR, SMP_USE_HRQHP},

        /* retrieve these captures from the HTTP logs */
        {"capture.req.method", smp_fetch_capture_req_method, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHP},
        {"capture.req.uri", smp_fetch_capture_req_uri, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHP},
        {"capture.req.ver", smp_fetch_capture_req_ver, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHP},

        {"capture.res.hdr", smp_fetch_capture_res_hdr, ARG1(1, SINT), NULL,
         SMP_T_STR, SMP_USE_HRSHP},
        {"capture.res.ver", smp_fetch_capture_res_ver, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHP},
         ...
         */
        {"cook", smp_fetch_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"cookie", smp_fetch_chn_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"cook_cnt", smp_fetch_cookie_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"cook_val", smp_fetch_cookie_val, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
         ...
         */
        {"hdr", smp_fetch_chn_hdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"hdr_cnt", smp_fetch_hdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"hdr_ip", smp_fetch_hdr_ip, ARG2(0, STR, SINT), val_hdr, SMP_T_IPV4,
         SMP_USE_HRQHV},
        {"hdr_val", smp_fetch_hdr_val, ARG2(0, STR, SINT), val_hdr, SMP_T_SINT,
         SMP_USE_HRQHV},

        {"http_auth_type", smp_fetch_http_auth_type, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"http_auth_user", smp_fetch_http_auth_user, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"http_auth_pass", smp_fetch_http_auth_pass, 0, NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"http_auth", smp_fetch_http_auth, ARG1(1, USR), NULL, SMP_T_BOOL,
         SMP_USE_HRQHV},
        {"http_auth_group", smp_fetch_http_auth_grp, ARG1(1, USR), NULL,
         SMP_T_STR, SMP_USE_HRQHV},
        {"http_first_req", smp_fetch_http_first_req, 0, NULL, SMP_T_BOOL,
         SMP_USE_HRQHP},
        ...
        {"req.proto_http", smp_fetch_proto_http, 0, NULL, SMP_T_BOOL,
         SMP_USE_HRQHP},
        {"req_proto_http", smp_fetch_proto_http, 0, NULL, SMP_T_BOOL,
         SMP_USE_HRQHP},
        ...
        {"req.body_len", smp_fetch_body_len, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"req.body_size", smp_fetch_body_size, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"req.body_param", smp_fetch_body_param, ARG1(0, STR), NULL, SMP_T_BIN,
         SMP_USE_HRQHV},
        ...
        {"res.body_len", smp_fetch_body_len, 0, NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"res.body_size", smp_fetch_body_size, 0, NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        ...
        /* explicit req.{cook,hdr} are used to force the fetch direction to be
           request-only */
        {"req.cook", smp_fetch_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"req.cook_cnt", smp_fetch_cookie_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"req.cook_val", smp_fetch_cookie_val, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},

        {"req.fhdr", smp_fetch_fhdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRQHV},
        {"req.fhdr_cnt", smp_fetch_fhdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"req.hdr", smp_fetch_hdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRQHV},
        {"req.hdr_cnt", smp_fetch_hdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRQHV},
        {"req.hdr_ip", smp_fetch_hdr_ip, ARG2(0, STR, SINT), val_hdr,
         SMP_T_IPV4, SMP_USE_HRQHV},
        {"req.hdr_names", smp_fetch_hdr_names, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"req.hdr_val", smp_fetch_hdr_val, ARG2(0, STR, SINT), val_hdr,
         SMP_T_SINT, SMP_USE_HRQHV},

        /* explicit req.{cook,hdr} are used to force the fetch direction to be
           response-only */
        {"res.cook", smp_fetch_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRSHV},
        {"res.cook_cnt", smp_fetch_cookie_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"res.cook_val", smp_fetch_cookie_val, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},

        {"res.fhdr", smp_fetch_fhdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRSHV},
        {"res.fhdr_cnt", smp_fetch_fhdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"res.hdr", smp_fetch_hdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRSHV},
        {"res.hdr_cnt", smp_fetch_hdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"res.hdr_ip", smp_fetch_hdr_ip, ARG2(0, STR, SINT), val_hdr,
         SMP_T_IPV4, SMP_USE_HRSHV},
        {"res.hdr_names", smp_fetch_hdr_names, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRSHV},
        {"res.hdr_val", smp_fetch_hdr_val, ARG2(0, STR, SINT), val_hdr,
         SMP_T_SINT, SMP_USE_HRSHV},

        /* scook is valid only on the response and is used for ACL compatibility
         */
        {"scook", smp_fetch_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRSHV},
        {"scook_cnt", smp_fetch_cookie_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"scook_val", smp_fetch_cookie_val, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"set-cookie", smp_fetch_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_HRSHV}, /* deprecated */

        /* shdr is valid only on the response and is used for ACL compatibility
         */
        {"shdr", smp_fetch_hdr, ARG2(0, STR, SINT), val_hdr, SMP_T_STR,
         SMP_USE_HRSHV},
        {"shdr_cnt", smp_fetch_hdr_cnt, ARG1(0, STR), NULL, SMP_T_SINT,
         SMP_USE_HRSHV},
        {"shdr_ip", smp_fetch_hdr_ip, ARG2(0, STR, SINT), val_hdr, SMP_T_IPV4,
         SMP_USE_HRSHV},
        {"shdr_val", smp_fetch_hdr_val, ARG2(0, STR, SINT), val_hdr, SMP_T_SINT,
         SMP_USE_HRSHV},
        ...
        {"url_param", smp_fetch_url_param, ARG2(0, STR, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV},
        {"urlp", smp_fetch_url_param, ARG2(0, STR, STR), NULL, SMP_T_STR,
         SMP_USE_HRQHV},
