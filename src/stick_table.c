...
static int (*smp_fetch_src)(const struct arg *, struct sample *, const char *,
                            void *);
 ...
 */
void stksess_setkey(struct stktable *t, struct stksess *ts,
 ...
 */
struct stksess *__stktable_lookup_key(struct stktable *t,
                                      struct stktable_key *key) {
  struct ebmb_node *eb;

  if (t->type == SMP_T_STR)
    eb = ebst_lookup_len(&t->keys, key->key,
                         key->key_len + 1 < t->key_size ? key->key_len
                                                        : t->key_size - 1);
  else
 ...
 */
struct stksess *stktable_lookup_key(struct stktable *t,
                                    struct stktable_key *key) {
  struct stksess *ts;
 ...
 */
void __stktable_touch_with_exp(struct stktable *t, struct stksess *ts,
  ...
  if (t->sync_task) {
    if (local) {
      ...
      if (!ts->upd.node.leaf_p || (int)(t->commitupdate - ts->upd.key) >= 0 ||
...
/* Update the expiration timer for <ts> but do not touch its expiration node.
 * The table's expiration timer is updated using the date of expiration coming
 * from <t> stick-table configuration. The node will be also inserted into the
 * update tree if needed, at a position considering the update is coming from a
 * remote node
 */
void stktable_touch_remote(struct stktable *t, struct stksess *ts,
                           int decrefcnt) {
  HA_SPIN_LOCK(STK_TABLE_LOCK, &t->lock);
 ...
 * The table's expiration timer is updated using the date of expiration coming
 * from <t> stick-table configuration. The node will be also inserted into the
 * update tree if needed, at a position considering the update was made locally
 */
void stktable_touch_local(struct stktable *t, struct stksess *ts,
                          int decrefcnt) {
 ...
 */
struct stksess *__stktable_get_entry(struct stktable *table,
                                     struct stktable_key *key) {
  struct stksess *ts;
 ...
 */
struct stksess *stktable_get_entry(struct stktable *table,
                                   struct stktable_key *key) {
  struct stksess *ts;
 ...
 */
struct stksess *__stktable_set_entry(struct stktable *table,
                                     struct stksess *nts) {
  struct stksess *ts;
 ...
 * This function locks the table, and the refcount of the entry is increased.
 */
struct stksess *stktable_set_entry(struct stktable *table,
                                   struct stksess *nts) {
  struct stksess *ts;
 ...
 */
static struct task *process_table_expire(struct task *task, void *context,
...
/* Perform minimal stick table intializations, report 0 in case of error, 1 if
 * OK. */
int stktable_init(struct stktable *t) {
  if (t->size) {
    ...
    t->pool = create_pool("sticktables",
                          sizeof(struct stksess) +
                              round_ptr_size(t->data_size) + t->key_size,
                          MEM_F_SHARED);
 ...
 */
int stktable_parse_type(char **args, int *myidx, unsigned long *type,
                        size_t *key_size) {
...
/*
 * Parse a line with <linenum> as number in <file> configuration file to
 * configure the stick-table with <t> as address and  <id> as ID. <peers>
 * provides the "peers" section pointer only if this function is called from a
 * "peers" section. <nid> is the stick-table name which is sent over the
 * network. It must be equal to <id> if this stick-table is parsed from a proxy
 * section, and prefixed by <peers> "peers" section name followed by a '/'
 * character if parsed from a "peers" section. This is the responsibility of the
 * caller to check this. Return an error status with ERR_* flags set if
 * required, 0 if no error was encountered.
 */
int parse_stick_table(const char *file, int linenum, char **args,
                      struct stktable *t, char *id, char *nid,
                      struct peers *peers) {
  int err_code = 0;
  int idx = 1;
  unsigned int val;
  ...
  /* Store the "peers" section if this function is called from a "peers"
   * section. */
  if (peers) {
    t->peers.p = peers;
    idx++;
  ...
  while (*args[idx]) {
    ...
    if (strcmp(args[idx], "size") == 0) {
      ...
      if (!*(args[idx])) {
        ha_alert("parsing [%s:%d] : %s: missing argument after '%s'.\n", file,
                 linenum, args[0], args[idx - 1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
      if ((err = parse_size_err(args[idx], &t->size))) {
        ha_alert("parsing [%s:%d] : %s: unexpected character '%c' in argument "
                 "of '%s'.\n",
    ...
    /* This argument does not exit in "peers" section. */
      ...
      if (err == PARSE_TIME_OVER) {
        ha_alert("parsing [%s:%d]: %s: timer overflow in argument <%s> to "
                 "<%s>, maximum value is 2147483647 ms (~24.8 days).\n",
                 file, linenum, args[0], args[idx], args[idx - 1]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      } else if (err == PARSE_TIME_UNDER) {
        ha_alert("parsing [%s:%d]: %s: timer underflow in argument <%s> to "
      ...
      while (*nw) {
        ...
        while (*nw && *nw != ',') {
          if (*nw == '(') {
            *nw = 0;
            ...
            while (*nw != ')') {
              if (!*nw) {
                ha_alert("parsing [%s:%d] : %s: missing closing parenthesis "
                         "after store option '%s'.\n",
        ...
        case PE_NONE:
          break;
        case PE_EXIST:
          ha_warning("parsing [%s:%d]: %s: store option '%s' already enabled, "
                     "ignored.\n",
                     file, linenum, args[0], cw);
          err_code |= ERR_WARN;
          break;
        ...
        case PE_ARG_MISSING:
          ha_alert(
              "parsing [%s:%d] : %s: missing argument to store option '%s'.\n",
              file, linenum, args[0], cw);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;

        case PE_ARG_NOT_USED:
          ha_alert("parsing [%s:%d] : %s: unexpected argument to store option "
                   "'%s'.\n",
                   file, linenum, args[0], cw);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        default:
          ha_alert("parsing [%s:%d] : %s: error when processing store option "
                   "'%s'.\n",
                   file, linenum, args[0], cw);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
 ...
 * <expr> on request or response considering the <opt> parameter. Returns either
 * NULL if no key could be extracted, or a pointer to the converted result
 * stored in static_table_key in format <table_type>. If <smp> is not NULL, it
 * will be reset and its flags will be initialized so that the caller gets a
 * copy of the input sample, and knows why it was not accepted (eg:
 * SMP_F_MAY_CHANGE is present without SMP_OPT_FINAL). The output will be usable
 * like this :
 *
 ...
 */
int stktable_compatible_sample(struct sample_expr *expr,
...
struct stktable_data_type stktable_data_types[STKTABLE_DATA_TYPES] = {
    ...
    [STKTABLE_DT_GPC0_RATE] = {.name = "gpc0_rate",
                               .std_type = STD_T_FRQP,
                               .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_CONN_CNT] = {.name = "conn_cnt", .std_type = STD_T_UINT},
    [STKTABLE_DT_CONN_RATE] = {.name = "conn_rate",
                               .std_type = STD_T_FRQP,
                               .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_CONN_CUR] = {.name = "conn_cur", .std_type = STD_T_UINT},
    [STKTABLE_DT_SESS_CNT] = {.name = "sess_cnt", .std_type = STD_T_UINT},
    [STKTABLE_DT_SESS_RATE] = {.name = "sess_rate",
                               .std_type = STD_T_FRQP,
                               .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_HTTP_REQ_CNT] = {.name = "http_req_cnt",
                                  .std_type = STD_T_UINT},
    [STKTABLE_DT_HTTP_REQ_RATE] = {.name = "http_req_rate",
                                   .std_type = STD_T_FRQP,
                                   .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_HTTP_ERR_CNT] = {.name = "http_err_cnt",
                                  .std_type = STD_T_UINT},
    [STKTABLE_DT_HTTP_ERR_RATE] = {.name = "http_err_rate",
                                   .std_type = STD_T_FRQP,
                                   .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_BYTES_IN_CNT] = {.name = "bytes_in_cnt",
                                  .std_type = STD_T_ULL},
    [STKTABLE_DT_BYTES_IN_RATE] = {.name = "bytes_in_rate",
                                   .std_type = STD_T_FRQP,
                                   .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_BYTES_OUT_CNT] = {.name = "bytes_out_cnt",
                                   .std_type = STD_T_ULL},
    [STKTABLE_DT_BYTES_OUT_RATE] = {.name = "bytes_out_rate",
                                    .std_type = STD_T_FRQP,
                                    .arg_type = ARG_T_DELAY},
    [STKTABLE_DT_GPC1] = {.name = "gpc1", .std_type = STD_T_UINT},
    [STKTABLE_DT_GPC1_RATE] = {.name = "gpc1_rate",
                               .std_type = STD_T_FRQP,
                               .arg_type = ARG_T_DELAY},
 ...
 */
int stktable_register_data_store(int idx, const char *name, int std_type,
                                 int arg_type) {
  if (idx < 0) {
 ...
 */
static int sample_conv_table_bytes_in_rate(const struct arg *arg_p,
...
/* Casts sample <smp> to the type of the table specified in arg(0), and looks
 * it up into this table. Returns the cumulated number of connections for the
 ...
 */
static int sample_conv_table_conn_cnt(const struct arg *arg_p,
                                      struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * table, <not found> is returned.
 */
static int sample_conv_table_conn_cur(const struct arg *arg_p,
                                      struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_conn_rate(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_bytes_out_rate(const struct arg *arg_p,
                                            struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_gpt0(const struct arg *arg_p, struct sample *smp,
                                  void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_gpc0_rate(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_gpc1(const struct arg *arg_p, struct sample *smp,
                                  void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_gpc1_rate(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
  ...
  if (ptr)
    smp->data.u.sint =
        read_freq_ctr_period(&stktable_data_cast(ptr, gpc1_rate),
 ...
 */
static int sample_conv_table_http_err_cnt(const struct arg *arg_p,
                                          struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_http_err_rate(const struct arg *arg_p,
                                           struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * table, <not found> is returned.
 */
static int sample_conv_table_http_req_cnt(const struct arg *arg_p,
                                          struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * is returned.
 */
static int sample_conv_table_http_req_rate(const struct arg *arg_p,
                                           struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
...
/* Casts sample <smp> to the type of the table specified in arg(0), and looks
 * it up into this table. Returns the volume of datareceived from clients in
 * kbytes if the key is present in the table, otherwise zero, so that
 * comparisons can be easily performed. If the inspected parameter is not stored
 * in the table, <not found> is returned.
 */
static int sample_conv_table_kbytes_in(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_kbytes_out(const struct arg *arg_p,
                                        struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * <not found> is returned.
 */
static int sample_conv_table_server_id(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * table, <not found> is returned.
 */
static int sample_conv_table_sess_cnt(const struct arg *arg_p,
                                      struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 * is returned.
 */
static int sample_conv_table_sess_rate(const struct arg *arg_p,
                                       struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
 ...
 */
static int sample_conv_table_trackers(const struct arg *arg_p,
                                      struct sample *smp, void *private) {
  struct stktable *t;
  struct stktable_key *key;
  struct stksess *ts;
...
static enum act_return action_inc_gpc0(struct act_rule *rule, struct proxy *px,
  ...
  if (ts) {
    ...
    /* First, update gpc0_rate if it's tracked. Second, update its gpc0 if
     * tracked. */
 ...
 */
static enum act_parse_ret parse_inc_gpc0(const char **args, int *arg,
                                         struct proxy *px,
  ...
  if (*cmd_name == '\0') {
    ...
    if (*cmd_name != '(') {
      memprintf(err, "invalid stick table track ID. Expects %s(<Track ID>)",
                args[*arg - 1]);
      return ACT_RET_PRS_ERR;
    }
    cmd_name++; /* jump the '(' */
    rule->arg.gpc.sc =
        strtol(cmd_name, &error, 10); /* Convert stick table id. */
    if (*error != ')') {
      memprintf(err, "invalid stick table track ID. Expects %s(<Track ID>)",
                args[*arg - 1]);
      return ACT_RET_PRS_ERR;
...
static enum act_return action_inc_gpc1(struct act_rule *rule, struct proxy *px,
  ...
  if (ts) {
    ...
    /* First, update gpc1_rate if it's tracked. Second, update its gpc1 if
     * tracked. */
 ...
 * expression to execute.
 */
static enum act_parse_ret parse_inc_gpc1(const char **args, int *arg,
                                         struct proxy *px,
                                         struct act_rule *rule, char **err) {
  const char *cmd_name = args[*arg - 1];
  char *error;
  ...
  if (ptr) {
    if (!rule->arg.gpt.expr)
      ...
      case ACT_F_TCP_REQ_SES:
        smp_opt_dir = SMP_OPT_DIR_REQ;
        break;
      case ACT_F_TCP_REQ_CNT:
        smp_opt_dir = SMP_OPT_DIR_REQ;
        break;
      case ACT_F_TCP_RES_CNT:
        smp_opt_dir = SMP_OPT_DIR_RES;
        break;
      case ACT_F_HTTP_REQ:
        smp_opt_dir = SMP_OPT_DIR_REQ;
        break;
      ...
      default:
        send_log(px, LOG_ERR,
                 "stick table: internal error while setting gpt0.");
        if (!(global.mode & MODE_QUIET) || (global.mode & MODE_VERBOSE))
          ha_alert(
              "stick table: internal error while executing setting gpt0.\n");
        return ACT_RET_CONT;
      }

      /* Fetch and cast the expression. */
      smp = sample_fetch_as_type(px, sess, s, smp_opt_dir | SMP_OPT_FINAL,
                                 rule->arg.gpt.expr, SMP_T_SINT);
      if (!smp) {
        send_log(
            px, LOG_WARNING,
            "stick table: invalid expression or data type while setting gpt0.");
        if (!(global.mode & MODE_QUIET) || (global.mode & MODE_VERBOSE))
          ha_alert("stick table: invalid expression or data type while setting "
                   "gpt0.\n");
        return ACT_RET_CONT;
      }
 ...
 * expression to execute.
 */
static enum act_parse_ret parse_set_gpt0(const char **args, int *arg,
                                         struct proxy *px,
                                         struct act_rule *rule, char **err)

{
  ...
  if (*error != '\0') {
    rule->arg.gpt.expr =
        sample_parse_expr((char **)args, arg, px->conf.args.file,
    ...
    switch (rule->from) {
    case ACT_F_TCP_REQ_SES:
      smp_val = SMP_VAL_FE_SES_ACC;
      break;
    case ACT_F_TCP_REQ_CNT:
      smp_val = SMP_VAL_FE_REQ_CNT;
      break;
    case ACT_F_TCP_RES_CNT:
      smp_val = SMP_VAL_BE_RES_CNT;
      break;
    case ACT_F_HTTP_REQ:
      smp_val = SMP_VAL_FE_HRQ_HDR;
      break;
    case ACT_F_HTTP_RES:
      smp_val = SMP_VAL_BE_HRS_HDR;
      break;
    default:
      memprintf(
          err,
    ...
    if (!(rule->arg.gpt.expr->fetch->val & smp_val)) {
      memprintf(err,
                "fetch method '%s' extracts information from '%s', none of "
...
/* set temp integer to the number of used entries in the table pointed to by
 * expr. Accepts exactly 1 argument of type table.
 */
static int smp_fetch_table_cnt(const struct arg *args, struct sample *smp,
...
/* set temp integer to the number of free entries in the table pointed to by
 * expr. Accepts exactly 1 argument of type table.
 */
static int smp_fetch_table_avl(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  struct stktable *t;
 ...
 */
struct stkctr *smp_fetch_sc_stkctr(struct session *sess, struct stream *strm,
                                   const struct arg *args, const char *kw,
                                   struct stkctr *stkctr) {
 ...
 * src_clr_gpc*.
 */
struct stkctr *smp_create_src_stkctr(struct session *sess, struct stream *strm,
                                     const struct arg *args, const char *kw,
                                     struct stkctr *stkctr) {
  struct stktable_key *key;
  struct connection *conn = objt_conn(sess->origin);
 ...
 */
static int smp_fetch_sc_tracked(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
 ...
 */
static int smp_fetch_sc_get_gpt0(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr_entry(stkctr)) {
    ...
    ptr = stktable_data_ptr(stkctr->table, stkctr_entry(stkctr),
                            STKTABLE_DT_GPT0);
    if (!ptr) {
      if (stkctr == &tmpstkctr)
        stktable_release(stkctr->table, stkctr_entry(stkctr));
 ...
 * zero is returned if the key is new.
 */
static int smp_fetch_sc_get_gpc0(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 * zero is returned if the key is new.
 */
static int smp_fetch_sc_get_gpc1(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_gpc0_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 * Value zero is returned if the key is new.
 */
static int smp_fetch_sc_gpc1_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_inc_gpc0(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr && stkctr_entry(stkctr)) {
     ...
     */
    ptr1 = stktable_data_ptr(stkctr->table, stkctr_entry(stkctr),
                             STKTABLE_DT_GPC0_RATE);
    ptr2 = stktable_data_ptr(stkctr->table, stkctr_entry(stkctr),
                             STKTABLE_DT_GPC0);
    if (ptr1 || ptr2) {
      ...
      /* If data was modified, we need to touch to re-schedule sync */
      stktable_touch_local(stkctr->table, stkctr_entry(stkctr),
                           (stkctr == &tmpstkctr) ? 1 : 0);
 ...
 */
static int smp_fetch_sc_inc_gpc1(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_clr_gpc0(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_clr_gpc1(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 * "src_conn_cnt" only.
 */
static int smp_fetch_sc_conn_cnt(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 * only.
 */
static int smp_fetch_sc_conn_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
...
/* set temp integer to the number of connections from the stream's source
 * address in the table pointed to by expr, after updating it. Accepts exactly 1
 * argument of type table.
 */
static int smp_fetch_src_updt_conn_cnt(const struct arg *args,
                                       struct sample *smp, const char *kw,
                                       void *private) {
 ...
 * "src_conn_cur" only.
 */
static int smp_fetch_sc_conn_cur(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 * "src_sess_cnt" only.
 */
static int smp_fetch_sc_sess_cnt(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_sess_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_http_req_cnt(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_http_req_rate(const struct arg *args,
                                      struct sample *smp, const char *kw,
                                      void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr_entry(stkctr) != NULL) {
    ...
    smp->data.u.sint = read_freq_ctr_period(
        &stktable_data_cast(ptr, http_req_rate),
 ...
 */
static int smp_fetch_sc_http_err_cnt(const struct arg *args, struct sample *smp,
                                     const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_http_err_rate(const struct arg *args,
                                      struct sample *smp, const char *kw,
                                      void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr_entry(stkctr) != NULL) {
    ...
    smp->data.u.sint = read_freq_ctr_period(
        &stktable_data_cast(ptr, http_err_rate),
 ...
 */
static int smp_fetch_sc_kbytes_in(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_bytes_in_rate(const struct arg *args,
                                      struct sample *smp, const char *kw,
                                      void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr_entry(stkctr) != NULL) {
    ...
    smp->data.u.sint = read_freq_ctr_period(
        &stktable_data_cast(ptr, bytes_in_rate),
 ...
 */
static int smp_fetch_sc_kbytes_out(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
 ...
 */
static int smp_fetch_sc_bytes_out_rate(const struct arg *args,
                                       struct sample *smp, const char *kw,
                                       void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr_entry(stkctr) != NULL) {
    ...
    smp->data.u.sint = read_freq_ctr_period(
        &stktable_data_cast(ptr, bytes_out_rate),
 ...
 */
static int smp_fetch_sc_trackers(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  struct stkctr tmpstkctr;
  struct stkctr *stkctr;
  ...
  if (stkctr == &tmpstkctr) {
    smp->data.u.sint =
        stkctr_entry(stkctr) ? (stkctr_entry(stkctr)->ref_cnt - 1) : 0;
    stktable_release(stkctr->table, stkctr_entry(stkctr));
  } else {
...
static int table_dump_head_to_buffer(struct buffer *msg,
  ...
  chunk_appendf(msg, "# table: %s, type: %s, size:%d, used:%d\n", t->id,
                stktable_types[t->type].kw, t->size, t->current);
  ...
  if (target &&
    ...
    chunk_appendf(msg,
                  "# contents not dumped due to insufficient privileges\n");
...
static int table_dump_entry_to_buffer(struct buffer *msg,
  ...
  chunk_appendf(msg, " use=%d exp=%d", entry->ref_cnt - 1,
                tick_remain(now_ms, entry->expire));
  ...
  for (dt = 0; dt < STKTABLE_DATA_TYPES; dt++) {
    ...
    if (stktable_data_types[dt].arg_type == ARG_T_DELAY)
      chunk_appendf(msg, " %s(%d)=", stktable_data_types[dt].name,
                    t->data_arg[dt].u);
    else
...
static int table_process_entry_per_key(struct appctx *appctx, char **args) {
  ...
  default:
    ...
    case STK_CLI_ACT_SHOW:
      return cli_err(appctx, "Showing keys from tables of type other than ip, "
                             "ipv6, string and integer is not supported\n");
    case STK_CLI_ACT_CLR:
      return cli_err(appctx, "Removing keys from tables of type other than ip, "
                             "ipv6, string and integer is not supported\n");
    case STK_CLI_ACT_SET:
      return cli_err(appctx, "Inserting keys into tables of type other than "
                             "ip, ipv6, string and integer is not supported\n");
    default:
  ...
  case STK_CLI_ACT_SET:
    ...
    for (cur_arg = 5; *args[cur_arg]; cur_arg += 2) {
      ...
      if (!*args[cur_arg + 1] ||
          strl2llrc(args[cur_arg + 1], strlen(args[cur_arg + 1]), &value) !=
              0) {
...
static int table_prepare_data_request(struct appctx *appctx, char **args) {
  ...
  if (appctx->ctx.table.action != STK_CLI_ACT_SHOW &&
      appctx->ctx.table.action != STK_CLI_ACT_CLR)
    return cli_err(appctx, "content-based lookup is only supported with the "
                           "\"show\" and \"clear\" actions\n");

  for (i = 0; i < STKTABLE_FILTER_LEN; i++) {
    if (i > 0 && !*args[3 + 3 * i]) // number of filter entries can be less than
                                    // STKTABLE_FILTER_LEN
      break;
    /* condition on stored data value */
    appctx->ctx.table.data_type[i] =
        stktable_get_data_type(args[3 + 3 * i] + 5);
    if (appctx->ctx.table.data_type[i] < 0)
      return cli_dynerr(
          appctx,
          memprintf(&err, "Filter entry #%i: Unknown data type\n", i + 1));

    if (!((struct stktable *)appctx->ctx.table.target)
             ->data_ofs[appctx->ctx.table.data_type[i]])
      return cli_dynerr(
          appctx,
          memprintf(&err,
                    "Filter entry #%i: Data type not stored in this table\n",
                    i + 1));

    appctx->ctx.table.data_op[i] = get_std_op(args[4 + 3 * i]);
    if (appctx->ctx.table.data_op[i] < 0)
      return cli_dynerr(
          appctx, memprintf(&err,
                            "Filter entry #%i: Require and operator among "
                            "\"eq\", \"ne\", \"le\", \"ge\", \"lt\", \"gt\"\n",
                            i + 1));

    if (!*args[5 + 3 * i] || strl2llrc(args[5 + 3 * i], strlen(args[5 + 3 * i]),
                                       &appctx->ctx.table.value[i]) != 0)
      return cli_dynerr(appctx, memprintf(&err,
                                          "Filter entry #%i: Require a valid "
  ...
  if (*args[3 + 3 * i]) {
    return cli_dynerr(
        ...
        memprintf(
            &err,
            "Detected extra data in filter, %ith word of input, after '%s'\n",
            3 + 3 * i + 1, args[2 + 3 * i]));
  }
...
static int cli_parse_table_req(char **args, char *payload,
  ...
  appctx->ctx.table.action =
      (long)private; // keyword argument, one of STK_CLI_ACT_*
...
err_args:
  ...
  case STK_CLI_ACT_SHOW:
    return cli_err(appctx,
                   "Optional argument only supports \"data.<store_data_type>\" "
                   "<operator> <value> and key <key>\n");
  case STK_CLI_ACT_CLR:
    return cli_err(appctx,
                   "Required arguments: <table> \"data.<store_data_type>\" "
                   "<operator> <value> or <table> key <key>\n");
  case STK_CLI_ACT_SET:
    return cli_err(appctx, "Required arguments: <table> key <key> "
                           "[data.<store_data_type> <value>]*\n");
  default:
    return cli_err(appctx, "Unknown action\n");
  }
}
 ...
 * on the action stored in appctx->private). It returns 0 if the output buffer
  ...
  while (appctx->st2 != STAT_ST_FIN) {
    ...
    case STAT_ST_INFO:
      ...
      if (appctx->ctx.table.t->size) {
        if (show && !table_dump_head_to_buffer(&trash, si, appctx->ctx.table.t,
                                               appctx->ctx.table.target))
          return 0;

        if (appctx->ctx.table.target && (strm_li(s)->bind_conf->level &
                                         ACCESS_LVL_MASK) >= ACCESS_LVL_OPER) {
    ...
    case STAT_ST_LIST:
      ...
      if (appctx->ctx.table.data_type[0] >= 0) {
        ...
        for (i = 0; i < STKTABLE_FILTER_LEN; i++) {
          ...
          ptr = stktable_data_ptr(appctx->ctx.table.t, appctx->ctx.table.entry,
                                  dt);
      ...
      if (show && !skip_entry &&
          !table_dump_entry_to_buffer(&trash, si, appctx->ctx.table.t,
...
static struct cli_kw_list cli_kws = {
    ...
    {{{"clear", "table", NULL},
      ...
      (void *)STK_CLI_ACT_CLR},
     {{"set", "table", NULL},
      "set table [id] : update or create a table entry's data",
      cli_parse_table_req,
      cli_io_handler_table,
      NULL,
      (void *)STK_CLI_ACT_SET},
     {{"show", "table", NULL},
      "show table [id]: report table usage stats or dump this table's contents",
      cli_parse_table_req,
      cli_io_handler_table,
      cli_release_show_table,
      (void *)STK_CLI_ACT_SHOW},
     {
         {},
     }}};
...
//	{ "table_avl",          smp_fetch_table_avl,         ARG1(1,TAB), NULL,
//SMP_T_SINT, SMP_USE_INTRN, }, 	{ "table_cnt",          smp_fetch_table_cnt,
//ARG1(1,TAB),      NULL, SMP_T_SINT, SMP_USE_INTRN, }, 	{ /* END */ },
//}};
/* Note: must not be declared <const> as its list will be overwritten.
 * Please take care of keeping this list alphabetically sorted.
 */
static struct sample_fetch_kw_list smp_fetch_keywords = {
    ILH,
    {
        ...
        {"sc_clr_gpc1", smp_fetch_sc_clr_gpc1, ARG2(1, SINT, TAB), NULL,
        ...
        {"sc_get_gpc1", smp_fetch_sc_get_gpc1, ARG2(1, SINT, TAB), NULL,
         SMP_T_SINT, SMP_USE_INTRN},
        {
            "sc_gpc0_rate",
...
static struct sample_conv_kw_list sample_conv_kws = {
    ILH,
    {
        {"in_table", sample_conv_in_table, ARG1(1, TAB), NULL, SMP_T_ANY,
         SMP_T_BOOL},
        {"table_bytes_in_rate", sample_conv_table_bytes_in_rate, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_bytes_out_rate", sample_conv_table_bytes_out_rate, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_conn_cnt", sample_conv_table_conn_cnt, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_conn_cur", sample_conv_table_conn_cur, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_conn_rate", sample_conv_table_conn_rate, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_gpt0", sample_conv_table_gpt0, ARG1(1, TAB), NULL, SMP_T_ANY,
         SMP_T_SINT},
        {"table_gpc0", sample_conv_table_gpc0, ARG1(1, TAB), NULL, SMP_T_ANY,
         SMP_T_SINT},
        {"table_gpc1", sample_conv_table_gpc1, ARG1(1, TAB), NULL, SMP_T_ANY,
         SMP_T_SINT},
        {"table_gpc0_rate", sample_conv_table_gpc0_rate, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_gpc1_rate", sample_conv_table_gpc1_rate, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_http_err_cnt", sample_conv_table_http_err_cnt, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_http_err_rate", sample_conv_table_http_err_rate, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_http_req_cnt", sample_conv_table_http_req_cnt, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_http_req_rate", sample_conv_table_http_req_rate, ARG1(1, TAB),
         NULL, SMP_T_ANY, SMP_T_SINT},
        {"table_kbytes_in", sample_conv_table_kbytes_in, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_kbytes_out", sample_conv_table_kbytes_out, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_server_id", sample_conv_table_server_id, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_sess_cnt", sample_conv_table_sess_cnt, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_sess_rate", sample_conv_table_sess_rate, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {"table_trackers", sample_conv_table_trackers, ARG1(1, TAB), NULL,
         SMP_T_ANY, SMP_T_SINT},
        {/* END */},
    }};
