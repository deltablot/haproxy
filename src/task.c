 ...
 */
DECLARE_POOL(pool_head_notification, "notification",
             sizeof(struct notification));

unsigned int nb_tasks = 0;
volatile unsigned long global_tasks_mask =
    0; /* Mask of threads with tasks in the global runqueue */
unsigned int tasks_run_queue = 0;
unsigned int tasks_run_queue_cur = 0; /* copy of the run queue size */
unsigned int nb_tasks_cur = 0;        /* copy of the tasks count */
unsigned int niced_tasks = 0; /* number of niced tasks in the run queue */

THREAD_LOCAL struct task_per_thread *sched =
    &task_per_thread[0]; /* scheduler context for the current thread */
...
#endif
  {
    int nb = ((void *)root - (void *)&task_per_thread[0].rqueue) /
             sizeof(task_per_thread[0]);
  ...
  for (queue = 0; queue < TL_CLASSES;) {
    ...
    if (global.tune.options & GTUNE_SCHED_LOW_LATENCY) {
      if (unlikely(sched->tl_class_mask & budget_mask & ((1 << queue) - 1))) {
         ...
         */
        queue =
            (sched->tl_class_mask & 1) ? 0 : (sched->tl_class_mask & 2) ? 1 : 2;
        continue;
      }

      if (unlikely(queue > TL_URGENT && budget_mask & (1 << TL_URGENT) &&
      ...
      if (unlikely(
              queue > TL_NORMAL && budget_mask & (1 << TL_NORMAL) &&
...
not_done_yet:
  ...
  if ((tt->tl_class_mask & (1 << TL_NORMAL)) || (sched->rqueue_size > 0) ||
  ...
  for (queue = 0; queue < TL_CLASSES; queue++)
    max[queue] =
        ((unsigned)max_processed * max[queue] + max_total - 1) / max_total;
  ...
  while (picked < budget) {
    ...
    if (!lrq) {
      lrq = eb32sc_lookup_ge(&tt->rqueue, rqueue_ticks - TIMER_LOOK_BACK,
                             tid_bit);
      if (unlikely(!lrq))
...
struct work_list *work_list_create(int nbthread,
                                   struct task *(*fct)(struct task *, void *,
                                                       unsigned short),
                                   void *arg) {
  struct work_list *wl;
  int i;
...
static int cfg_parse_tune_sched_low_latency(char **args, int section_type,
                                            struct proxy *curpx,
                                            struct proxy *defpx,
