 ...
 * it does not exist.
 */
static inline struct vars *get_vars(struct session *sess, struct stream *strm,
 ...
 */
void var_accounting_diff(struct vars *vars, struct session *sess,
                         struct stream *strm, int size) {
  switch (vars->scope) {
  case SCOPE_REQ:
  case SCOPE_RES:
    if (strm)
 ...
 */
static int var_accounting_add(struct vars *vars, struct session *sess,
                              struct stream *strm, int size) {
  switch (vars->scope) {
  case SCOPE_REQ:
  case SCOPE_RES:
    if (var_reqres_limit && strm &&
        strm->vars_reqres.size + size > var_reqres_limit)
      return 0;
    /* fall through */
  case SCOPE_TXN:
...
void vars_prune(struct vars *vars, struct session *sess, struct stream *strm) {
  ...
  list_for_each_entry_safe(var, tmp, &vars->head, l) { size += var_clear(var); }
...
static char *register_name(const char *name, int len, enum vars_scope *scope,
  ...
  if (len > 5 && strncmp(name, "proc.", 5) == 0) {
    ...
    *scope = SCOPE_CHECK;
    ...
    memprintf(err,
              "invalid variable name '%s'. A variable name must be start by "
              "its scope. "
              "The scope can be 'proc', 'sess', 'txn', 'req', 'res' or 'check'",
              name);
    return res;
  }
...
static inline struct var *var_get(struct vars *vars, const char *name) {
  ...
  list_for_each_entry(var, &vars->head, l) if (var->name == name) return var;
  return NULL;
}

/* Returns 0 if fails, else returns 1. */
static int smp_fetch_var(const struct arg *args, struct sample *smp,
 ...
 */
static int sample_store(struct vars *vars, const char *name,
                        struct sample *smp) {
  struct var *var;
  ...
  if (var) {
    ...
    if (var->data.type == SMP_T_STR || var->data.type == SMP_T_BIN) {
      free(var->data.u.str.area);
      var_accounting_diff(vars, smp->sess, smp->strm, -var->data.u.str.data);
    } else if (var->data.type == SMP_T_METH &&
               var->data.u.meth.meth == HTTP_METH_OTHER) {
      free(var->data.u.meth.str.area);
  ...
  case SMP_T_BIN:
    if (!var_accounting_add(vars, smp->sess, smp->strm, smp->data.u.str.data)) {
      var->data.type =
          SMP_T_BOOL; /* This type doesn't use additional memory. */
      return 0;
    }
    var->data.u.str.area = malloc(smp->data.u.str.data);
    if (!var->data.u.str.area) {
      var_accounting_diff(vars, smp->sess, smp->strm, -smp->data.u.str.data);
      var->data.type =
          SMP_T_BOOL; /* This type doesn't use additional memory. */
      return 0;
    }
    var->data.u.str.data = smp->data.u.str.data;
    memcpy(var->data.u.str.area, smp->data.u.str.area, var->data.u.str.data);
    break;
  case SMP_T_METH:
    ...
    if (!var_accounting_add(vars, smp->sess, smp->strm,
...
/* Returns 0 if fails, else returns 1. Note that stream may be null for
 * SCOPE_SESS. */
...
/* Returns 0 if fails, else returns 1. Note that stream may be null for
 * SCOPE_SESS. */
static inline int sample_clear_stream(const char *name, enum vars_scope scope,
                                      struct sample *smp) {
  struct vars *vars;
  struct var *var;
  unsigned int size = 0;
...
/* Returns 0 if fails, else returns 1. */
static int smp_conv_store(const struct arg *args, struct sample *smp,
                          void *private) {
  return sample_store_stream(args[0].data.var.name, args[0].data.var.scope,
                             smp);
}

/* Returns 0 if fails, else returns 1. */
static int smp_conv_clear(const struct arg *args, struct sample *smp,
                          void *private) {
  return sample_clear_stream(args[0].data.var.name, args[0].data.var.scope,
                             smp);
}
...
int vars_check_arg(struct arg *arg, char **err) {
  ...
  name = register_name(arg->data.str.area, arg->data.str.data, &scope, 1, err);
  if (!name)
    return 0;
 ...
 * Returns zero on failure and non-zero otherwise.
 */
int vars_unset_by_name_ifexist(const char *name, size_t len,
                               struct sample *smp) {
  enum vars_scope scope;
...
static enum act_return action_store(struct act_rule *rule, struct proxy *px,
  ...
  if (!sample_process(px, sess, s, dir | SMP_OPT_FINAL, rule->arg.vars.expr,
                      &smp))
    return ACT_RET_CONT;
 ...
 */
static enum act_parse_ret parse_store(const char **args, int *arg,
  ...
  if (*var_name != '(') {
    memprintf(err,
              "invalid variable '%s'. Expects 'set-var(<var-name>)' or "
              "'unset-var(<var-name>)'",
              args[*arg - 1]);
  ...
  rule->arg.vars.name =
      register_name(var_name, var_len, &rule->arg.vars.scope, 1, err);
...
static int vars_max_size_global(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  return vars_max_size(args, section_type, curpx, defpx, file, line, err,
                       &var_global_limit);
}

static int vars_max_size_proc(char **args, int section_type,
                              struct proxy *curpx, struct proxy *defpx,
                              const char *file, int line, char **err) {
  return vars_max_size(args, section_type, curpx, defpx, file, line, err,
                       &var_proc_limit);
...
static int vars_max_size_sess(char **args, int section_type,
                              struct proxy *curpx, struct proxy *defpx,
                              const char *file, int line, char **err) {
  return vars_max_size(args, section_type, curpx, defpx, file, line, err,
                       &var_sess_limit);
...
static int vars_max_size_reqres(char **args, int section_type,
                                struct proxy *curpx, struct proxy *defpx,
                                const char *file, int line, char **err) {
  return vars_max_size(args, section_type, curpx, defpx, file, line, err,
                       &var_reqres_limit);
...
static int vars_max_size_check(char **args, int section_type,
                               struct proxy *curpx, struct proxy *defpx,
                               const char *file, int line, char **err) {
  return vars_max_size(args, section_type, curpx, defpx, file, line, err,
                       &var_check_limit);
...
static struct action_kw_list tcp_req_sess_kws = {
    {},
    {{"set-var", parse_store, 1}, {"unset-var", parse_store, 1}, {/* END */}}};
...
static struct action_kw_list tcp_check_kws = {
    ILH,
    {{"set-var", parse_store, 1}, {"unset-var", parse_store, 1}, {/* END */}}};
