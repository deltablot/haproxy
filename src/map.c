 ...
 */
int sample_load_map(struct arg *arg, struct sample_conv *conv, const char *file,
                    int line, char **err) {
  struct map_descriptor *desc;
  ...
  case SMP_T_STR:
    desc->pat.parse_smp = map_parse_str;
    break;
  case SMP_T_SINT:
    desc->pat.parse_smp = map_parse_int;
    break;
  case SMP_T_ADDR:
    desc->pat.parse_smp = map_parse_ip;
    break;
  default:
    memprintf(err,
              "map: internal haproxy error: no default parse case for the "
              "input type <%d>.",
              conv->out_type);
    free(desc);
    return 0;
  ...
  /* Load map. */
  if (!pattern_read_from_file(&desc->pat, PAT_REF_MAP, arg[0].data.str.area,
                              PAT_MF_NO_DNS, 1, err, file, line))
    return 0;
  ...
  if (pat) {
    if (pat->data) {
      ...
      if ((long)private == PAT_MATCH_REGM) {
        ...
        str = get_trash_chunk();
        len = exp_replace(str->area, str->size, tmptrash->area,
                          pat->data->u.str.area, (regmatch_t *)smp->ctx.a[0]);
 ...
 * nodes.
 */
static inline struct pat_ref *pat_list_get_next(struct pat_ref *getnext,
                                                struct list *end,
                                                unsigned int flags) {
...
static inline struct pat_ref *pat_ref_lookup_ref(const char *reference) {
  int id;
  char *error;
 ...
 * each reference.
 */
static inline struct pattern_expr *
pat_expr_get_next(struct pattern_expr *getnext, struct list *end) {
...
static int cli_io_handler_pats_list(struct appctx *appctx) {
  ...
  case STAT_ST_INIT:
    ...
    appctx->ctx.map.ref = pat_list_get_next(
  ...
  case STAT_ST_LIST:
    while (appctx->ctx.map.ref) {
      ...
      /* get next list entry and check the end of the list */
      appctx->ctx.map.ref =
          pat_list_get_next(appctx->ctx.map.ref, &pattern_reference,
...
static int cli_io_handler_map_lookup(struct appctx *appctx) {
  ...
  case STAT_ST_INIT:
    ...
    appctx->ctx.map.expr =
        LIST_ELEM(&appctx->ctx.map.ref->pat, struct pattern_expr *, list);
    appctx->ctx.map.expr =
        pat_expr_get_next(appctx->ctx.map.expr, &appctx->ctx.map.ref->pat);
    appctx->st2 = STAT_ST_LIST;
    /* fall through */
  ...
  case STAT_ST_LIST:
    ...
    while (appctx->ctx.map.expr) {
      ...
      if (appctx->ctx.map.expr->pat_head->match &&
        ...
        pat = appctx->ctx.map.expr->pat_head->match(&sample,
                                                    appctx->ctx.map.expr, 1);
      else
        pat = NULL;
      ...
      for (match_method = 0; match_method < PAT_MATCH_NUM; match_method++)
        if (appctx->ctx.map.expr->pat_head->match ==
            pat_match_fcts[match_method])
          break;
      if (match_method >= PAT_MATCH_NUM)
        chunk_appendf(&trash, "type=unknown(%p)",
                      appctx->ctx.map.expr->pat_head->match);
      else
        ...
        if (appctx->ctx.map.display_flags == PAT_REF_MAP) {
          if (pat->data && pat->ref && pat->ref->sample)
            chunk_appendf(&trash, ", value=\"%s\", type=\"%s\"",
                          pat->ref->sample, smp_to_type[pat->data->type]);
          else
...
static int cli_parse_get_map(char **args, char *payload, struct appctx *appctx,
                             void *private) {
...
static int cli_parse_show_map(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  if (strcmp(args[1], "map") == 0 || strcmp(args[1], "acl") == 0) {
...
static int cli_parse_set_map(char **args, char *payload, struct appctx *appctx,
                             void *private) {
  if (strcmp(args[1], "map") == 0) {
    ...
    if (!*args[2] || !*args[3] || !*args[4])
      return cli_err(appctx, "'set map' expects three parameters: map "
...
static int map_add_key_value(struct appctx *appctx, const char *key,
...
static int cli_parse_add_map(char **args, char *payload, struct appctx *appctx,
                             void *private) {
  if (strcmp(args[1], "map") == 0 || strcmp(args[1], "acl") == 0) {
    int ret;
    char *err;
    ...
    if (!payload) {
      ...
      while (payload < end) {
        ...
        if (!*payload && appctx->ctx.map.display_flags == PAT_REF_MAP)
          return cli_dynerr(
              appctx, memprintf(&err, "Missing value for key '%s'.\n", key));
...
static int cli_parse_del_map(char **args, char *payload, struct appctx *appctx,
                             void *private) {
  if (args[1][0] == 'm')
    appctx->ctx.map.display_flags = PAT_REF_MAP;
  else
...
static int cli_parse_clear_map(char **args, char *payload,
 ...
 *
 * For the map_*_int keywords, the output is declared as SMP_T_UINT, but the
 * converter function can provide SMP_T_UINT, SMP_T_SINT or SMP_T_BOOL depending
 * on how the patterns found in the file can be parsed.
 *
 * For the map_*_ip keyword, the output is declared as SMP_T_IPV4, but the
 * converter function can provide SMP_T_IPV4 or SMP_T_IPV6 depending on the
 * patterns found in the file.
 *
 * The map_* keywords only emit strings.
 *
 * The output type is only used during the configuration parsing. It is used for
...
static struct sample_conv_kw_list sample_conv_kws = {
    ILH,
    {
        {"map", sample_conv_map, ARG2(1, STR, STR), sample_load_map, SMP_T_STR,
         SMP_T_STR, (void *)PAT_MATCH_STR},
