...
void sock_unbind(struct receiver *rx) {
   ...
   *   - worker process unbinding from a worker's FD with socket transfer
   * enabled => keep
  ...
  if (!stopping && !master && !(rx->flags & RX_F_MWORKER) &&
      (global.tune.options & GTUNE_SOCKET_TRANSFER))
    return;

  if (!stopping && master && rx->flags & RX_F_INHERITED)
    return;
...
int sock_get_old_sockets(const char *unixsocket) {
  ...
  if (got_fd != fd_nb) {
    ha_warning(
        "We didn't get the expected number of sockets (expecting %d got %d)\n",
        fd_nb, got_fd);
    goto out;
  }
  ...
  for (cur_fd = 0; cur_fd < got_fd; cur_fd++) {
    ...
    socklen = sizeof(val);
    if (getsockopt(fd, SOL_SOCKET, SO_TYPE, &val, &socklen) == 0 &&
        val == SOCK_DGRAM)
    ...
    if (xfer_sock->addr.ss_family == AF_INET6 &&
        getsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &val, &socklen) == 0 &&
        val > 0)
...
int sock_find_compatible_fd(const struct receiver *rx) {
  ...
  while (xfer_sock) {
    if ((options == xfer_sock->options) &&
        ...
        (!if_namelen ||
         strcmp(rx->settings->interface, xfer_sock->iface) == 0) &&
#ifdef USE_NS
        (!ns_namelen ||
         strcmp(rx->settings->netns->node.key, xfer_sock->namespace) == 0) &&
#endif
...
void sock_conn_iocb(int fd) {
  ...
  flags = conn->flags &
          ~CO_FL_ERROR; /* ensure to call the wake handler upon error */
