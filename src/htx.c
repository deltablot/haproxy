...
struct htx htx_empty = {
    .size = 0, .data = 0, .head = -1, .tail = -1, .first = -1};
...
static struct htx_blk *htx_reserve_nxblk(struct htx *htx, uint32_t blksz) {
  ...
  tailroom =
      (!htx->head_addr ? htx_pos_to_addr(htx, tail) - htx->tail_addr : 0);
 ...
 */
static int htx_prepare_blk_expansion(struct htx *htx, struct htx_blk *blk,
                                     int32_t delta) {
 ...
 */
struct htx_blk *htx_add_blk(struct htx *htx, enum htx_blk_type type,
                            uint32_t blksz) {
  struct htx_blk *blk;
...
struct htx_ret htx_find_offset(struct htx *htx, uint32_t offset) {
  ...
  for (blk = htx_get_head_blk(htx); blk && offset;
...
struct htx_blk *htx_replace_blk_value(struct htx *htx, struct htx_blk *blk,
                                      const struct ist old,
                                      const struct ist new) {
  struct ist n, v;
  int32_t delta;
  int ret;
  ...
  if (ret == 1) { /* Replace in place */
    if (delta <= 0) {
      ...
      memmove(old.ptr + new.len, old.ptr + old.len,
 ...
 */
struct htx_sl *htx_replace_stline(struct htx *htx, struct htx_blk *blk,
                                  const struct ist p1, const struct ist p2,
                                  const struct ist p3) {
  enum htx_blk_type type;
  struct htx_sl *sl;
 ...
 */
struct htx_sl *htx_add_stline(struct htx *htx, enum htx_blk_type type,
                              unsigned int flags, const struct ist p1,
 ...
 * NULL is returned. */
struct htx_blk *htx_add_all_headers(struct htx *htx,
 ...
 * NULL is returned. */
struct htx_blk *htx_add_all_trailers(struct htx *htx,
                                     const struct http_hdr *hdrs) {
  int i;
...
struct htx_blk *htx_add_last_data(struct htx *htx, struct ist data) {
  ...
  for (pblk = htx_get_prev_blk(htx, blk); pblk;
       pblk = htx_get_prev_blk(htx, pblk)) {
...
void htx_move_blk_before(struct htx *htx, struct htx_blk **blk,
                         struct htx_blk **ref) {
  struct htx_blk *cblk, *pblk;

  cblk = *blk;
  for (pblk = htx_get_prev_blk(htx, cblk); pblk;
       pblk = htx_get_prev_blk(htx, pblk)) {
    /* Swap .addr and .info fields */
    cblk->addr ^= pblk->addr;
    pblk->addr ^= cblk->addr;
