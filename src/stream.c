...
static const struct trace_event strm_trace_events[] = {
    ...
    {.mask = STRM_EV_STRM_ERR,
     .name = "strm_err",
     .desc = "error during stream processing"},
    {.mask = STRM_EV_STRM_ANA, .name = "strm_ana", .desc = "stream analyzers"},
    {.mask = STRM_EV_STRM_PROC,
     .name = "strm_proc",
     .desc = "stream processing"},

    {.mask = STRM_EV_SI_ST,
     .name = "si_state",
     .desc = "processing stream-interface states"},

    {.mask = STRM_EV_HTTP_ANA, .name = "http_ana", .desc = "HTTP analyzers"},
    {.mask = STRM_EV_HTTP_ERR,
     .name = "http_err",
     .desc = "error during HTTP analyzis"},

    {.mask = STRM_EV_TCP_ANA, .name = "tcp_ana", .desc = "TCP analyzers"},
    {.mask = STRM_EV_TCP_ERR,
     .name = "tcp_err",
     .desc = "error during TCP analyzis"},
    {}};
...
#define STRM_VERB_ADVANCED 4
    {.name = "advanced",
     .desc = "add info on channel's buffer for data and developer levels only"},
...
static void strm_trace(enum trace_level level, uint64_t mask,
  ...
  chunk_appendf(&trace_buf, " : [%u,%s]", s->uniq_id,
                ((s->flags & SF_HTX) ? "HTX" : "TCP"));
  ...
  chunk_appendf(&trace_buf, " SI=(%s,%s)", si_state_str(si_f->state),
                si_state_str(si_b->state));

  /* If txn is defined, HTTP req/rep states */
  if (txn)
    chunk_appendf(&trace_buf, " HTTP=(%s,%s)",
                  h1_msg_state_str(txn->req.msg_state),
                  h1_msg_state_str(txn->rsp.msg_state));
  if (msg)
    chunk_appendf(&trace_buf, " %s",
                  ((msg->chn->flags & CF_ISRESP) ? "RESPONSE" : "REQUEST"));
  ...
  if (txn) {
    chunk_appendf(&trace_buf,
                  " - t=%p s=(%p,0x%08x) txn.flags=0x%08x, "
                  "http.flags=(0x%08x,0x%08x) status=%d",
                  task, s, s->flags, txn->flags, txn->req.flags, txn->rsp.flags,
                  txn->status);
  } else {
    chunk_appendf(&trace_buf,
                  " - t=%p s=(%p,0x%08x) si_f=(%p,0x%08x,0x%x) "
                  "si_b=(%p,0x%08x,0x%x) retries=%d",
                  task, s, s->flags, si_f, si_f->flags, si_f->err_type, si_b,
                  si_b->flags, si_b->err_type, si_b->conn_retries);
  }
  ...
  if (src->verbosity == STRM_VERB_SIMPLE || txn) {
    ...
    chunk_appendf(&trace_buf,
                  " req=(%p .fl=0x%08x .ana=0x%08x .exp(r,w,a)=(%u,%u,%u) "
                  ".o=%lu .tot=%llu .to_fwd=%u)",
                  req, req->flags, req->analysers, req->rex, req->wex,
                  req->analyse_exp, (long)req->output, req->total,
                  req->to_forward);
    chunk_appendf(&trace_buf,
                  " res=(%p .fl=0x%08x .ana=0x%08x .exp(r,w,a)=(%u,%u,%u) "
                  ".o=%lu .tot=%llu .to_fwd=%u)",
                  res, res->flags, res->analysers, res->rex, res->wex,
                  res->analyse_exp, (long)res->output, res->total,
                  res->to_forward);
  }
  ...
  if (s->flags & SF_HTX) {
    ...
    chunk_appendf(&trace_buf, " htx=(%u/%u#%u, %u/%u#%u)", rqhtx->data,
                  rqhtx->size, htx_nbblks(rqhtx), rphtx->data, rphtx->size,
                  htx_nbblks(rphtx));
  } else {
    chunk_appendf(
        &trace_buf, " buf=(%u@%p+%u/%u, %u@%p+%u/%u)",
...
struct stream *stream_new(struct session *sess, enum obj_type *origin,
  ...
  if (!b_is_null(input)) {
    ...
    *input = BUF_NULL;
    s->req.total =
        (IS_HTX_STRM(s) ? htxbuf(&s->req.buf)->data : b_data(&s->req.buf));
...
static void stream_free(struct stream *s) {
  ...
  if (objt_server(
          s->target)) { /* there may be requests left pending in queue */
  ...
  if (s->dns_ctx.dns_requester) {
    __decl_thread(struct dns_resolvers *resolvers =
                      s->dns_ctx.parent->arg.dns.resolvers);
  ...
  list_for_each_entry_safe(bref, back, &s->back_refs, users) {
    ...
    if (s->list.n != &streams)
      LIST_ADDQ(&LIST_ELEM(s->list.n, struct stream *, list)->back_refs,
                &bref->users);
    bref->ref = s->list.n;
  }
  LIST_DEL(&s->list);
...
int stream_set_timeout(struct stream *s, enum act_timeout_name name,
                       int timeout) {
  switch (name) {
...
static void back_establish(struct stream *s) {
  ...
  if (si->flags & SI_FL_ERR && req->flags & CF_WROTE_DATA) {
    ...
    DBG_TRACE_STATE("read/write error",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
  }
  ...
  if (s->be->mode ==
      PR_MODE_TCP) { /* let's allow immediate data connection in this case */
    /* if the user wants to log as soon as possible, without counting
     * bytes from the server, then this is the right moment. */
    if (!LIST_ISEMPTY(&strm_fe(s)->logformat) &&
    ...
    rep->flags |=
        CF_READ_DONTWAIT; /* a single read is enough to get response headers */
  }
  ...
  if (rep->flags & (CF_SHUTR | CF_SHUTW)) {
    si->state = SI_ST_DIS;
    DBG_TRACE_STATE("response channel shutdwn for read/write",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
  }
...
static void sess_set_term_flags(struct stream *s) {
  if (!(s->flags & SF_FINST_MASK)) {
    if (s->si[1].state == SI_ST_INI) {
      ...
      s->flags |= SF_FINST_Q;
    else if (si_state_in(s->si[1].state, SI_SB_REQ | SI_SB_TAR | SI_SB_ASS |
                                             SI_SB_CON | SI_SB_CER | SI_SB_RDY))
      s->flags |= SF_FINST_C;
...

{
  ...
  if (rule->from != ACT_F_HTTP_REQ) {
    if (sess->fe ==
        s->be) /* report it if the request was intercepted by the frontend */
 ...
 */
static int process_switching_rules(struct stream *s, struct channel *req,
                                   int an_bit) {
  ...
  if (!(s->flags & SF_BE_ASSIGNED)) {
    ...
    list_for_each_entry(rule, &fe->switching_rules, list) {
      ...
      if (rule->cond) {
        ret = acl_exec_cond(rule->cond, fe, sess, s,
  ...
  /* we don't want to run the TCP or HTTP filters again if the backend has not
   * changed */
  if (fe == s->be) {
   ...
   * or ignored persistence rule, and report that in the stream.
   */
  list_for_each_entry(prst_rule, &s->be->persist_rules, list) {
    int ret = 1;

    if (prst_rule->cond) {
      ret = acl_exec_cond(prst_rule->cond, s->be, sess, s,
                          SMP_OPT_DIR_REQ | SMP_OPT_FINAL);
      ret = acl_pass(ret);
 ...
 */
static int process_server_rules(struct stream *s, struct channel *req,
                                int an_bit) {
 ...
 * they won't match. It always returns 1.
 */
static int process_sticking_rules(struct stream *s, struct channel *req,
                                  int an_bit) {
  struct proxy *px = s->be;
  struct session *sess = s->sess;
  ...
  list_for_each_entry(rule, &px->sticking_rules, list) {
    ...
    if (rule->cond) {
      ret = acl_exec_cond(rule->cond, px, sess, s,
                          SMP_OPT_DIR_REQ | SMP_OPT_FINAL);
      ret = acl_pass(ret);
      if (rule->cond->pol == ACL_COND_UNLESS)
        ret = !ret;
    ...
    if (ret) {
      ...
      key =
          stktable_fetch_key(rule->table.t, px, sess, s,
                             SMP_OPT_DIR_REQ | SMP_OPT_FINAL, rule->expr, NULL);
      if (!key)
        continue;
 ...
 * they won't match. It always returns 1.
 */
static int process_store_rules(struct stream *s, struct channel *rep,
                               int an_bit) {
  struct proxy *px = s->be;
  struct session *sess = s->sess;
  struct sticking_rule *rule;
  ...
  list_for_each_entry(rule, &px->storersp_rules, list) {
    ...
    if (ret) {
      ...
      key =
          stktable_fetch_key(rule->table.t, px, sess, s,
                             SMP_OPT_DIR_RES | SMP_OPT_FINAL, rule->expr, NULL);
      if (!key)
        continue;
  ...
  for (i = 0; i < s->store_count; i++) {
    ...
    if (objt_server(s->target) &&
        objt_server(s->target)->flags & SRV_F_NON_STICK) {
      stksess_free(s->store[i].table, s->store[i].ts);
      s->store[i].ts = NULL;
      continue;
    }
...
#define FLT_ANALYZE(strm, chn, fun, list, back, flag, ...)                     \
  {                                                                            \
    if ((list) & (flag)) {                                                     \
      ...
      UPDATE_ANALYSERS((chn)->analysers, (list), (back), (flag));              \
    }                                                                          \
  }
 ...
 */
struct task *process_stream(struct task *t, void *context,
  ...
  if (rate >= 100000 &&
      s->call_rate.prev_ctr) { // make sure to wait at least a full second
  ...
  if (unlikely(s->pending_events & TASK_WOKEN_TIMER)) {
    ...
    if (unlikely((req->flags & (CF_SHUTW | CF_WRITE_TIMEOUT)) ==
                 CF_WRITE_TIMEOUT)) {
      si_b->flags |= SI_FL_NOLINGER;
      si_shutw(si_b);
    }

    if (unlikely((req->flags & (CF_SHUTR | CF_READ_TIMEOUT)) ==
                 CF_READ_TIMEOUT)) {
    ...
    if (unlikely((res->flags & (CF_SHUTW | CF_WRITE_TIMEOUT)) ==
                 CF_WRITE_TIMEOUT)) {
      si_f->flags |= SI_FL_NOLINGER;
      si_shutw(si_f);
    }
...
resync_request:
  ...
  if (((req->flags & ~rqf_last) & CF_MASK_ANALYSER) ||
      ...
      si_f->state != rq_prod_last || si_b->state != rq_cons_last ||
    ...
    if (si_state_in(si_f->state, SI_SB_EST | SI_SB_DIS | SI_SB_CLO)) {
      ...
      while (ana_list && max_loops--) {
         ...
         */
        ANALYZE(s, req, flt_start_analyze, ana_list, ana_back,
                AN_REQ_FLT_START_FE);
        FLT_ANALYZE(s, req, tcp_inspect_request, ana_list, ana_back,
                    AN_REQ_INSPECT_FE);
        FLT_ANALYZE(s, req, http_wait_for_request, ana_list, ana_back,
                    AN_REQ_WAIT_HTTP);
        FLT_ANALYZE(s, req, http_wait_for_request_body, ana_list, ana_back,
                    AN_REQ_HTTP_BODY);
        FLT_ANALYZE(s, req, http_process_req_common, ana_list, ana_back,
                    AN_REQ_HTTP_PROCESS_FE, sess->fe);
        FLT_ANALYZE(s, req, process_switching_rules, ana_list, ana_back,
        ...
        FLT_ANALYZE(s, req, tcp_inspect_request, ana_list, ana_back,
                    AN_REQ_INSPECT_BE);
        FLT_ANALYZE(s, req, http_process_req_common, ana_list, ana_back,
                    AN_REQ_HTTP_PROCESS_BE, s->be);
        FLT_ANALYZE(s, req, http_process_tarpit, ana_list, ana_back,
                    AN_REQ_HTTP_TARPIT);
        FLT_ANALYZE(s, req, process_server_rules, ana_list, ana_back,
                    AN_REQ_SRV_RULES);
        FLT_ANALYZE(s, req, http_process_request, ana_list, ana_back,
                    AN_REQ_HTTP_INNER);
        FLT_ANALYZE(s, req, tcp_persist_rdp_cookie, ana_list, ana_back,
                    AN_REQ_PRST_RDP_COOKIE);
        FLT_ANALYZE(s, req, process_sticking_rules, ana_list, ana_back,
                    AN_REQ_STICKING_RULES);
        ANALYZE(s, req, flt_analyze_http_headers, ana_list, ana_back,
                AN_REQ_FLT_HTTP_HDRS);
        ANALYZE(s, req, http_request_forward_body, ana_list, ana_back,
                AN_REQ_HTTP_XFER_BODY);
        ANALYZE(s, req, pcli_wait_for_request, ana_list, ana_back,
                AN_REQ_WAIT_CLI);
        ANALYZE(s, req, flt_xfer_data, ana_list, ana_back,
...
resync_response:
  ...
  if (((res->flags & ~rpf_last) & CF_MASK_ANALYSER) ||
      ...
      si_f->state != rp_cons_last || si_b->state != rp_prod_last ||
      s->pending_events & TASK_WOKEN_MSG) {
    ...
    if (si_state_in(si_b->state, SI_SB_EST | SI_SB_DIS | SI_SB_CLO)) {
      ...
      while (ana_list && max_loops--) {
        /* Warning! ensure that analysers are always placed in ascending order!
         */
        ANALYZE(s, res, flt_start_analyze, ana_list, ana_back,
                AN_RES_FLT_START_FE);
        ANALYZE(s, res, flt_start_analyze, ana_list, ana_back,
                AN_RES_FLT_START_BE);
        FLT_ANALYZE(s, res, tcp_inspect_response, ana_list, ana_back,
                    AN_RES_INSPECT);
        FLT_ANALYZE(s, res, http_wait_for_response, ana_list, ana_back,
                    AN_RES_WAIT_HTTP);
        FLT_ANALYZE(s, res, process_store_rules, ana_list, ana_back,
                    AN_RES_STORE_RULES);
        FLT_ANALYZE(s, res, http_process_res_common, ana_list, ana_back,
                    AN_RES_HTTP_PROCESS_BE, s->be);
        ANALYZE(s, res, flt_analyze_http_headers, ana_list, ana_back,
                AN_RES_FLT_HTTP_HDRS);
        ANALYZE(s, res, http_response_forward_body, ana_list, ana_back,
                AN_RES_HTTP_XFER_BODY);
        ANALYZE(s, res, pcli_wait_for_response, ana_list, ana_back,
                AN_RES_WAIT_CLI);
        ANALYZE(s, res, flt_xfer_data, ana_list, ana_back,
  ...
  if (unlikely(!(s->flags & SF_ERR_MASK))) {
    ...
    } else if (res->flags & (CF_READ_ERROR | CF_READ_TIMEOUT | CF_WRITE_ERROR |
                             CF_WRITE_TIMEOUT)) {
   ...
   */
  if (unlikely((!req->analysers || (req->analysers == AN_REQ_FLT_END &&
                                    !(req->flags & CF_FLT_ANALYZE))) &&
  ...
  if (!(req->flags & (CF_KERN_SPLICING | CF_SHUTR)) && req->to_forward &&
      (global.tune.options & GTUNE_USE_SPLICE) &&
      (objt_cs(si_f->end) && __objt_cs(si_f->end)->conn->xprt &&
       __objt_cs(si_f->end)->conn->xprt->rcv_pipe &&
       __objt_cs(si_f->end)->conn->mux &&
       __objt_cs(si_f->end)->conn->mux->rcv_pipe) &&
      (objt_cs(si_b->end) && __objt_cs(si_b->end)->conn->xprt &&
       __objt_cs(si_b->end)->conn->xprt->snd_pipe &&
       __objt_cs(si_b->end)->conn->mux &&
       __objt_cs(si_b->end)->conn->mux->snd_pipe) &&
  ...
  /* it's possible that an upper layer has requested a connection setup or
   * abort. There are 2 situations where we decide to establish a new connection
   * :
    ...
    if (!(req->flags & CF_SHUTW)) {
      if ((req->flags & CF_AUTO_CONNECT) || !channel_is_empty(req)) {
        ...
        if ((s->be->retry_type & ~PR_RE_CONN_FAILED) &&
            (s->be->mode == PR_MODE_HTTP) && !(si_b->flags & SI_FL_D_L7_RETRY))
  ...
  if (si_state_in(si_b->state, SI_SB_REQ | SI_SB_QUE | SI_SB_TAR | SI_SB_ASS)) {
    ...
    do {
      ...
      srv = objt_server(s->target);
      if (si_b->state == SI_ST_ASS && srv && srv->rdr_len &&
  ...
  /* first, let's check if the request buffer needs to shutdown(write), which
   * may happen either because the input is closed or because we want to force a
   * close once the server has begun to respond. If a half-closed timeout is
   * set, we adjust the other side's timeout as well.
   */
  if (unlikely((req->flags & (CF_SHUTW | CF_SHUTW_NOW | CF_AUTO_CLOSE |
                              CF_SHUTR)) == (CF_AUTO_CLOSE | CF_SHUTR))) {
  ...
  if (unlikely((req->flags & (CF_SHUTW | CF_SHUTR | CF_SHUTR_NOW)) ==
                   CF_SHUTW &&
               !req->analysers))
   ...
   * recent call to channel_abort().
   */
  if (unlikely((!res->analysers || (res->analysers == AN_RES_FLT_END &&
                                    !(res->flags & CF_FLT_ANALYZE))) &&
  ...
  if (!(res->flags & (CF_KERN_SPLICING | CF_SHUTR)) && res->to_forward &&
      (global.tune.options & GTUNE_USE_SPLICE) &&
      (objt_cs(si_f->end) && __objt_cs(si_f->end)->conn->xprt &&
       __objt_cs(si_f->end)->conn->xprt->snd_pipe &&
       __objt_cs(si_f->end)->conn->mux &&
       __objt_cs(si_f->end)->conn->mux->snd_pipe) &&
      (objt_cs(si_b->end) && __objt_cs(si_b->end)->conn->xprt &&
       __objt_cs(si_b->end)->conn->xprt->rcv_pipe &&
       __objt_cs(si_b->end)->conn->mux &&
       __objt_cs(si_b->end)->conn->mux->rcv_pipe) &&
      (pipes_used < global.maxpipes) &&
  ...
  if (unlikely((res->flags & (CF_SHUTW | CF_SHUTW_NOW | CF_AUTO_CLOSE |
                              CF_SHUTR)) == (CF_AUTO_CLOSE | CF_SHUTR))) {
  ...
  if (unlikely((res->flags & (CF_SHUTW | CF_SHUTR | CF_SHUTR_NOW)) ==
               CF_SHUTW) &&
      !res->analysers)
    channel_shutr_now(res);
  ...
  if (unlikely((global.mode & MODE_DEBUG) &&
    ...
    if (si_b->state == SI_ST_CLO && si_b->prev_state == SI_ST_EST) {
      chunk_printf(&trash, "%08x:%s.srvcls[%04x:%04x]\n", s->uniq_id, s->be->id,
                   objt_cs(si_f->end)
                       ? (unsigned short)objt_cs(si_f->end)->conn->handle.fd
                       : -1,
                   objt_cs(si_b->end)
                       ? (unsigned short)objt_cs(si_b->end)->conn->handle.fd
                       : -1);
      DISGUISE(write(1, trash.area, trash.data));
    }

    if (si_f->state == SI_ST_CLO && si_f->prev_state == SI_ST_EST) {
  ...
  if (likely((si_f->state != SI_ST_CLO) ||
             !si_state_in(si_b->state, SI_SB_INI | SI_SB_CLO))) {
  ...
  if (unlikely((global.mode & MODE_DEBUG) &&
               (!(global.mode & MODE_QUIET) || (global.mode & MODE_VERBOSE)))) {
    chunk_printf(
        &trash, "%08x:%s.closed[%04x:%04x]\n", s->uniq_id, s->be->id,
        objt_cs(si_f->end) ? (unsigned short)objt_cs(si_f->end)->conn->handle.fd
                           : -1,
        objt_cs(si_b->end) ? (unsigned short)objt_cs(si_b->end)->conn->handle.fd
                           : -1);
    DISGUISE(write(1, trash.area, trash.data));
  }
...
void stream_update_time_stats(struct stream *s) {
  ...
  if (srv) {
    samples_window = (((s->be->mode == PR_MODE_HTTP)
                           ? srv->counters.p.http.cum_req
                           : srv->counters.cum_lbconn) > TIME_STATS_SAMPLES)
                         ? TIME_STATS_SAMPLES
                         : 0;
  ...
  samples_window = (((s->be->mode == PR_MODE_HTTP)
                         ...
                         : s->be->be_counters.cum_lbconn) > TIME_STATS_SAMPLES)
                       ? TIME_STATS_SAMPLES
                       : 0;
...
void stream_dump(struct buffer *buf, const struct stream *s, const char *pfx,
  ...
  if (s->obj_type != OBJ_TYPE_STREAM) {
    chunk_appendf(buf, "%sstrm=%p [invalid type=%d(%s)]%c", pfx, s, s->obj_type,
                  obj_type_name(&s->obj_type), eol);
    return;
  }
  ...
  chunk_appendf(
      ...
      "",
      pfx, s, src, s->sess->fe->id, s->be->id, dst, eol, pfx, req->flags,
      req->analysers, res->flags, res->analysers, si_state_str(si_f->state),
      si_f->flags, si_state_str(si_b->state), si_b->flags, eol, pfx, acf,
      acf ? acf->st0 : 0, csf, csf ? csf->flags : 0, eol, pfx, acb,
      acb ? acb->st0 : 0, csb, csb ? csb->flags : 0, eol, pfx, cof,
      cof ? cof->flags : 0, conn_get_mux_name(cof), cof ? cof->ctx : 0,
      conn_get_xprt_name(cof), cof ? cof->xprt_ctx : 0, conn_get_ctrl_name(cof),
      cof ? cof->handle.fd : 0, eol, pfx, cob, cob ? cob->flags : 0,
      conn_get_mux_name(cob), cob ? cob->ctx : 0, conn_get_xprt_name(cob),
      cob ? cob->xprt_ctx : 0, conn_get_ctrl_name(cob),
      cob ? cob->handle.fd : 0, eol);
}
 ...
 *
 * If an ID is already stored within the stream nothing happens existing unique
 ID is
 * returned.
 */
...
stream_parse_use_service(const char **args, int *cur_arg, struct proxy *px,
 ...
 */
static int stats_dump_full_strm_to_buffer(struct stream_interface *si,
                                          struct stream *strm) {
  ...
  if (appctx->ctx.sess.section > 0 && appctx->ctx.sess.uid != strm->uniq_id) {
    ...
    chunk_appendf(&trash,
                  "  *** session terminated while we were watching it ***\n");
  ...
  case 1:
    ...
    chunk_appendf(
        &trash, "%p: [%02d/%s/%04d:%02d:%02d:%02d.%06d] id=%u proto=%s", strm,
        tm.tm_mday, monthname[tm.tm_mon], tm.tm_year + 1900, tm.tm_hour,
        tm.tm_min, tm.tm_sec, (int)(strm->logs.accept_date.tv_usec),
        strm->uniq_id, strm_li(strm) ? strm_li(strm)->rx.proto->name : "?");

    conn = objt_conn(strm_orig(strm));
    switch (conn && conn_get_src(conn) ? addr_to_str(conn->src, pn, sizeof(pn))
                                       : AF_UNSPEC) {
    case AF_INET:
    case AF_INET6:
    ...
    chunk_appendf(
        &trash, "  frontend=%s (id=%u mode=%s), listener=%s (id=%u)",
    ...
    switch (conn && conn_get_dst(conn) ? addr_to_str(conn->dst, pn, sizeof(pn))
                                       : AF_UNSPEC) {
    case AF_INET:
    case AF_INET6:
    ...
    if (strm->be->cap & PR_CAP_BE)
      chunk_appendf(&trash, "  backend=%s (id=%u mode=%s)", strm->be->id,
    ...
    chunk_appendf(
        ...
        strm->task, strm->task->state, strm->task->nice, strm->task->calls,
        read_freq_ctr(&strm->call_rate),
        strm->task->expire
            ? tick_is_expired(strm->task->expire, now_ms)
                  ? "<PAST>"
                  : human_time(TICKS_TO_MS(strm->task->expire - now_ms),
                               TICKS_TO_MS(1000))
            : "<NEVER>",
        strm->task->thread_mask, task_in_rq(strm->task) ? ", running" : "");

    chunk_appendf(&trash, " age=%s)\n",
                  human_time(now.tv_sec - strm->logs.accept_date.tv_sec, 1));

    if (strm->txn)
      chunk_appendf(&trash,
                    "  txn=%p flags=0x%x meth=%d status=%d req.st=%s rsp.st=%s "
                    "req.f=0x%02x rsp.f=0x%02x\n",
                    strm->txn, strm->txn->flags, strm->txn->meth,
                    strm->txn->status,
                    h1_msg_state_str(strm->txn->req.msg_state),
                    h1_msg_state_str(strm->txn->rsp.msg_state),
                    strm->txn->req.flags, strm->txn->rsp.flags);

    chunk_appendf(&trash,
                  "  si[0]=%p (state=%s flags=0x%02x endp0=%s:%p exp=%s "
                  "et=0x%03x sub=%d)\n",
                  &strm->si[0], si_state_str(strm->si[0].state),
                  strm->si[0].flags, obj_type_name(strm->si[0].end),
                  obj_base_ptr(strm->si[0].end),
                  strm->si[0].exp
                      ? tick_is_expired(strm->si[0].exp, now_ms)
                            ? "<PAST>"
    ...
    chunk_appendf(&trash,
                  "  si[1]=%p (state=%s flags=0x%02x endp1=%s:%p exp=%s "
                  "et=0x%03x sub=%d)\n",
                  &strm->si[1], si_state_str(strm->si[1].state),
                  strm->si[1].flags, obj_type_name(strm->si[1].end),
                  obj_base_ptr(strm->si[1].end),
                  strm->si[1].exp
                      ? tick_is_expired(strm->si[1].exp, now_ms)
                            ? "<PAST>"
    ...
    if ((cs = objt_cs(strm->si[0].end)) != NULL) {
      ...
      chunk_appendf(&trash,
                    ...
                    conn, conn_get_ctrl_name(conn), conn_get_xprt_name(conn),
                    conn_get_mux_name(conn), cs_get_data_name(cs),
                    obj_type_name(conn->target), obj_base_ptr(conn->target));

      chunk_appendf(
          &trash,
          "      flags=0x%08x fd=%d fd.state=%02x updt=%d fd.tmask=0x%lx\n",
          conn->flags, conn->handle.fd,
          conn->handle.fd >= 0 ? fdtab[conn->handle.fd].state : 0,
          conn->handle.fd >= 0
              ? !!(fdtab[conn->handle.fd].update_mask & tid_bit)
              : 0,
      ...
      chunk_appendf(&trash,
                    ...
                    "nice=%d calls=%u rate=%u cpu=%llu lat=%llu\n",
                    tmpctx, tmpctx->st0, tmpctx->st1, tmpctx->st2,
                    tmpctx->applet->name, tmpctx->thread_mask, tmpctx->t->nice,
                    tmpctx->t->calls, read_freq_ctr(&tmpctx->call_rate),
                    (unsigned long long)tmpctx->t->cpu_time,
                    (unsigned long long)tmpctx->t->lat_time);
    }
    ...
    chunk_appendf(&trash,
                  ...
                  "      an_exp=%s",
                  &strm->req, strm->req.flags, strm->req.analysers,
    ...
    chunk_appendf(
        &trash, " rex=%s",
        strm->req.rex
            ? human_time(TICKS_TO_MS(strm->req.rex - now_ms), TICKS_TO_MS(1000))
            : "<NEVER>");
    ...
    chunk_appendf(
        ...
        strm->req.wex
            ? human_time(TICKS_TO_MS(strm->req.wex - now_ms), TICKS_TO_MS(1000))
            : "<NEVER>",
        &strm->req.buf, b_orig(&strm->req.buf),
        (unsigned int)co_data(&strm->req),
        (unsigned int)ci_head_ofs(&strm->req),
        (unsigned int)ci_data(&strm->req), (unsigned int)strm->req.buf.size);

    if (IS_HTX_STRM(strm)) {
      struct htx *htx = htxbuf(&strm->req.buf);

      chunk_appendf(&trash,
                    "      htx=%p flags=0x%x size=%u data=%u used=%u wrap=%s "
                    "extra=%llu\n",
    ...
    chunk_appendf(&trash,
                  ...
                  "      an_exp=%s",
                  &strm->res, strm->res.flags, strm->res.analysers,
    ...
    chunk_appendf(
        &trash, " rex=%s",
        strm->res.rex
            ? human_time(TICKS_TO_MS(strm->res.rex - now_ms), TICKS_TO_MS(1000))
            : "<NEVER>");
    ...
    chunk_appendf(
        ...
        "      buf=%p data=%p o=%u p=%u i=%u size=%u\n",
        strm->res.wex
            ? human_time(TICKS_TO_MS(strm->res.wex - now_ms), TICKS_TO_MS(1000))
            : "<NEVER>",
        &strm->res.buf, b_orig(&strm->res.buf),
        (unsigned int)co_data(&strm->res),
        (unsigned int)ci_head_ofs(&strm->res),
        (unsigned int)ci_data(&strm->res), (unsigned int)strm->res.buf.size);
...
static int cli_parse_show_sess(char **args, char *payload,
  ...
  case STAT_ST_LIST:
    ...
    while (appctx->ctx.sess.bref.ref != si_strm(appctx->owner)->list.n) {
      ...
      if (appctx->ctx.sess.target) {
        if (appctx->ctx.sess.target != (void *)-1 &&
            appctx->ctx.sess.target != curr_strm)
          goto next_sess;
      ...
      chunk_appendf(&trash, "%p: proto=%s", curr_strm,
                    strm_li(curr_strm) ? strm_li(curr_strm)->rx.proto->name
                                       : "?");

      conn = objt_conn(strm_orig(curr_strm));
      switch (conn && conn_get_src(conn)
                  ? addr_to_str(conn->src, pn, sizeof(pn))
                  : AF_UNSPEC) {
      case AF_INET:
      case AF_INET6:
        chunk_appendf(
            &trash, " src=%s:%d fe=%s be=%s srv=%s", pn,
            get_host_port(conn->src), strm_fe(curr_strm)->id,
            (curr_strm->be->cap & PR_CAP_BE) ? curr_strm->be->id : "<NONE>",
            objt_server(curr_strm->target) ? objt_server(curr_strm->target)->id
                                           : "<none>");
        break;
      case AF_UNIX:
        chunk_appendf(
            &trash, " src=unix:%d fe=%s be=%s srv=%s", strm_li(curr_strm)->luid,
      ...
      chunk_appendf(
          &trash, " ts=%02x age=%s calls=%u rate=%u cpu=%llu lat=%llu",
          ...
          (unsigned long long)curr_strm->task->cpu_time,
          (unsigned long long)curr_strm->task->lat_time);

      chunk_appendf(
          &trash, " rq[f=%06xh,i=%u,an=%02xh,rx=%s", curr_strm->req.flags,
          (unsigned int)ci_data(&curr_strm->req), curr_strm->req.analysers,
          curr_strm->req.rex
              ? human_time(TICKS_TO_MS(curr_strm->req.rex - now_ms),
                           TICKS_TO_MS(1000))
              : "");

      chunk_appendf(&trash, ",wx=%s",
                    curr_strm->req.wex
      ...
      chunk_appendf(
          &trash, " rp[f=%06xh,i=%u,an=%02xh,rx=%s", curr_strm->res.flags,
          (unsigned int)ci_data(&curr_strm->res), curr_strm->res.analysers,
          curr_strm->res.rex
      ...
      chunk_appendf(&trash, " s0=[%d,%1xh,fd=%d,ex=%s]", curr_strm->si[0].state,
                    curr_strm->si[0].flags, conn ? conn->handle.fd : -1,
                    curr_strm->si[0].exp
      ...
      chunk_appendf(&trash, " s1=[%d,%1xh,fd=%d,ex=%s]", curr_strm->si[1].state,
                    curr_strm->si[1].flags, conn ? conn->handle.fd : -1,
                    curr_strm->si[1].exp
    ...
    if (appctx->ctx.sess.target && appctx->ctx.sess.target != (void *)-1) {
      ...
      if (appctx->ctx.sess.section > 0)
        chunk_appendf(
            &trash, "  *** session terminated while we were watching it ***\n");
      else
...
static int cli_parse_shutdown_session(char **args, char *payload,
                                      struct appctx *appctx, void *private) {
...
static int cli_parse_shutdown_sessions_server(char **args, char *payload,
                                              struct appctx *appctx,
                                              void *private) {
  struct server *sv;
...
static struct action_kw_list stream_tcp_keywords = {
    ILH, {{"use-service", stream_parse_use_service}, {/* END */}}};

INITCALL1(STG_REGISTER, tcp_req_cont_keywords_register, &stream_tcp_keywords);

static struct action_kw_list stream_http_keywords = {
    ILH, {{"use-service", stream_parse_use_service}, {/* END */}}};

INITCALL1(STG_REGISTER, http_req_keywords_register, &stream_http_keywords);

static int smp_fetch_cur_server_timeout(const struct arg *args,
                                        struct sample *smp, const char *km,
                                        void *private) {
...
static int smp_fetch_cur_tunnel_timeout(const struct arg *args,
                                        struct sample *smp, const char *km,
                                        void *private) {
  smp->flags = SMP_F_VOL_TXN;
  smp->data.type = SMP_T_SINT;
