...
struct eb64_node *eb64_lookup_le(struct eb_root *root, u64 x) {
  ...
  while (1) {
    if ((eb_gettag(troot) == EB_LEAF)) {
       ...
       */
      node = container_of(eb_untag(troot, EB_LEAF), struct eb64_node,
                          node.branches);
      if (node->key <= x)
        return node;
      /* return prev */
    ...
    node =
        container_of(eb_untag(troot, EB_NODE), struct eb64_node, node.branches);
    ...
    if (node->node.bit < 0) {
      ...
      if (node->key <= x) {
        ...
        return container_of(eb_untag(troot, EB_LEAF), struct eb64_node,
                            node.branches);
      }
      /* return prev */
