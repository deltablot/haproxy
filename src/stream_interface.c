...
static void stream_int_chk_rcv(struct stream_interface *si) {
  ...
  DPRINTF(stderr, "%s: si=%p, si->state=%d ic->flags=%08x oc->flags=%08x\n",
          __FUNCTION__, si, si->state, ic->flags, si_oc(si)->flags);
...
static void stream_int_chk_snd(struct stream_interface *si) {
  ...
  DPRINTF(stderr, "%s: si=%p, si->state=%d ic->flags=%08x oc->flags=%08x\n",
          __FUNCTION__, si, si->state, si_ic(si)->flags, oc->flags);
 ...
 * itself. It also pre-initializes the applet's context and returns it (or NULL
 * in case it could not be allocated).
 */
struct appctx *si_register_handler(struct stream_interface *si,
                                   struct applet *app) {
 ...
 * further, otherwise it returns non-zero and removes itself from the
 * connection's flags (the bit is provided in <flag> by the caller). It is
 * designed to be called by the connection handler and relies on it to commit
 * polling changes. Note that it can emit a PROXY line by relying on the other
 * end's address when the connection is attached to a stream interface, or by
 * resolving the local address otherwise (also called a LOCAL line).
 */
int conn_si_send_proxy(struct connection *conn, unsigned int flag) {
  ...
  if (conn->send_proxy_ofs) {
    ...
    if (cs && cs->data_cb == &si_conn_cb) {
      ...
      ret = make_proxy_line(trash.area, trash.size, objt_server(conn->target),
                            remote_cs ? remote_cs->conn : NULL, strm);
    } else {
     ...
     */
    ret = conn_ctrl_send(
        conn, trash.area + ret + conn->send_proxy_ofs, -conn->send_proxy_ofs,
...
static void stream_int_notify(struct stream_interface *si) {
   ...
   */
  if (!channel_is_empty(ic) && (sio->flags & SI_FL_WAIT_DATA) &&
      (!(ic->flags & CF_EXPECT_MORE) || c_full(ic) || ci_data(ic) == 0 ||
       ic->pipe)) {
  ...
  if (/* changes on the production side */
    ...
    task->expire = tick_first(
        ...
        tick_first(tick_first(ic->rex, ic->wex), tick_first(oc->rex, oc->wex)));
...
int si_cs_send(struct conn_stream *cs) {
  ...
  if (co_data(oc)) {
    ...
    if ((!(oc->flags & (CF_NEVER_WAIT | CF_SEND_DONTWAIT)) &&
         ((oc->to_forward && oc->to_forward != CHN_INFINITE_FORWARD) ||
          ...
          (IS_HTX_STRM(si_strm(si)) && (!(oc->flags & (CF_EOI | CF_SHUTR)) &&
                                        htx_expect_more(htxbuf(&oc->buf)))))) ||
        ((oc->flags & CF_ISRESP) &&
         ((oc->flags & (CF_AUTO_CLOSE | CF_SHUTW_NOW)) ==
    ...
    if ((si->flags & SI_FL_L7_RETRY) && !b_data(&si->l7_buffer)) {
      ...
      if (!s->txn || (s->txn->req.msg_state != HTTP_MSG_DONE))
        ...
        if (b_is_null(&si->l7_buffer))
          ...
          memcpy(b_orig(&si->l7_buffer), b_orig(&oc->buf), b_size(&oc->buf));
    ...
    if (ret > 0) {
      ...
      if (!co_data(oc)) {
        /* Always clear both flags once everything has been sent, they're
         * one-shot */
...
void si_update_rx(struct stream_interface *si) {
  ...
  if (!channel_is_empty(ic)) {
    ...
    /* (re)start reading and update timeout. Note: we don't recompute the
     * timeout everytime we get here, otherwise it would risk never to expire.
     * We only update it if is was not yet set. The stream socket handler will
     * already have updated it if there has been a completed I/O.
     */
    si_rx_room_rdy(si);
  }
 ...
 */
void si_update_both(struct stream_interface *si_f,
                    struct stream_interface *si_b) {
  struct channel *req = si_ic(si_f);
  struct channel *res = si_oc(si_f);

  req->flags &= ~(CF_READ_NULL | CF_READ_PARTIAL | CF_READ_ATTACHED |
                  CF_WRITE_NULL | CF_WRITE_PARTIAL);
  res->flags &= ~(CF_READ_NULL | CF_READ_PARTIAL | CF_READ_ATTACHED |
                  CF_WRITE_NULL | CF_WRITE_PARTIAL);
...
abort_splice:
  ...
  /* Important note : if we're called with POLL_IN|POLL_HUP, it means the read
   * polling was enabled, which implies that the recv buffer was not full. So we
   * have a guarantee that if such an event is not handled above in splice, it
   * will be handled here by recv().
   */
  while ((cs->flags & CS_FL_RCV_MORE) ||
    ...
    ret = cs->conn->mux->rcv_buf(cs, &ic->buf, max,
                                 flags | (co_data(ic) ? CO_RFL_BUF_WET : 0));
...
do_close:
  /* OK we completely close the socket here just as if we went through
   * si_shut[rw]() */
  cs_close(cs);
