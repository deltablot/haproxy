 ...
 */
int crtlist_parse_line(char *line, char **crt_path, struct crtlist_entry *entry,
  ...
  while (*line) {
    if (isspace((unsigned char)*line)) {
      ...
      *line = 0;
      ...
      if (!arg) {
        memprintf(err, "parsing [%s:%d]: file must start with a cert", file,
                  linenum);
        cfgerr |= ERR_ALERT | ERR_FATAL;
        goto error;
      }
  ...
  *crt_path = args[0];
  ...
  while (cur_arg < ssl_e) {
    ...
    for (i = 0; ssl_bind_kws[i].kw != NULL; i++) {
      if (strcmp(ssl_bind_kws[i].kw, args[cur_arg]) == 0) {
        ...
        if (cur_arg + 1 + ssl_bind_kws[i].skip > ssl_e) {
          memprintf(err, "parsing [%s:%d]: ssl args out of '[]' for %s", file,
    ...
    if (!cfgerr && !newarg) {
      memprintf(err, "parsing [%s:%d]: unknown ssl keyword %s", file, linenum,
                args[cur_arg]);
      cfgerr |= ERR_ALERT | ERR_FATAL;
      goto error;
    }
...
/* This function parse a crt-list file and store it in a struct crtlist, each
 * line is a crtlist_entry structure Fill the <crtlist> argument with a pointer
 * to a new crtlist struct
 *
 * This function tries to open and store certificate files.
 */
int crtlist_parse_file(char *file, struct bind_conf *bind_conf,
                       struct proxy *curproxy, struct crtlist **crtlist,
                       char **err) {
  struct crtlist *newlist;
  ...
  while (fgets(thisline, sizeof(thisline), f) != NULL) {
    ...
    cfgerr |=
        crtlist_parse_line(thisline, &crt_path, entry, file, linenum, err);
    if (cfgerr & ERR_CODE)
      goto error;
    ...
    if (*crt_path != '/' && global_ssl.crt_base) {
      if ((strlen(global_ssl.crt_base) + 1 + strlen(crt_path)) > MAXPATHLEN) {
        memprintf(err, "parsing [%s:%d]: '%s' : path too long", file, linenum,
                  crt_path);
        cfgerr |= ERR_ALERT | ERR_FATAL;
        goto error;
      }
    ...
    if (ckchs == NULL) {
      if (stat(crt_path, &buf) == 0) {
        ...
        int n = 0;
        struct crtlist_entry *entry_dup =
            entry; /* use the previous created entry */
        for (n = 0; n < SSL_SOCK_NUM_KEYTYPES; n++) {
          struct stat buf;
          int ret;

          ret = snprintf(fp, sizeof(fp), "%s.%s", crt_path,
                         SSL_SOCK_KEYTYPE_NAMES[n]);
          if (ret > sizeof(fp))
            continue;
          ...
          entry_dup =
              NULL; /* the entry was used, we need a new one next round */
        }
      ...
      if (!found) {
        memprintf(err,
                  "%sunable to stat SSL certificate from file '%s' : %s.\n",
  ...
  if (missing_lf != -1) {
    memprintf(err,
              "parsing [%s:%d]: Missing LF on last line, file might have been "
...
/* This function reads a directory and stores it in a struct crtlist, each file
 * is a crtlist_entry structure Fill the <crtlist> argument with a pointer to a
 * new crtlist struct
 *
 * This function tries to open and store certificate files.
 */
int crtlist_load_cert_dir(char *path, struct bind_conf *bind_conf,
                          struct crtlist **crtlist, char **err) {
  struct crtlist *dir;
  ...
  if (n < 0) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (end && (!strcmp(end, ".issuer") || !strcmp(end, ".ocsp") ||
                  !strcmp(end, ".sctl") || !strcmp(end, ".key")))
        goto ignore_entry;
 ...
 */
static void dump_crtlist_sslconf(struct buffer *buf,
                                 const struct ssl_bind_conf *conf) {
  int space = 0;
  ...
  if (conf->ssl_methods_cfg.min) {
    if (space)
      chunk_appendf(buf, " ");
    chunk_appendf(buf, "ssl-min-ver %s",
                  methodVersions[conf->ssl_methods_cfg.min].name);
    space++;
  }
  ...
  if (conf->ssl_methods_cfg.max) {
    if (space)
      chunk_appendf(buf, " ");
    chunk_appendf(buf, "ssl-max-ver %s",
                  methodVersions[conf->ssl_methods_cfg.max].name);
    space++;
  }
...
static void dump_crtlist_filters(struct buffer *buf,
                                 struct crtlist_entry *entry) {
  int i;
...
static int cli_parse_dump_crtlist(char **args, char *payload,
  ...
  if (mode == 's' && !*args[4])
    return cli_err(
        appctx, "'show ssl crt-list -n' expects a filename or a directory\n");

  if (filename && *filename) {

    /* strip trailing slashes, including first one */
    for (end = filename + strlen(filename) - 1; end >= filename && *end == '/';
         end--)
      *end = 0;
 ...
 * crt-list and generates the ckch_insts for each bind_conf that uses this
 * crt-list
 *
...
static int cli_io_handler_add_crtlist(struct appctx *appctx) {
  ...
  while (1) {
    ...
    case SETCERT_ST_INIT:
      ...
      chunk_printf(trash, "Inserting certificate '%s' in crt-list '%s'",
                   store->path, crtlist->node.key);
      if (ci_putchk(si_ic(si), trash) == -1) {
        si_rx_room_blk(si);
        goto yield;
      }
    ...
    case SETCERT_ST_GEN:
      bind_conf_node =
          appctx->ctx.cli.p2; /* get the previous ptr from the yield */
      ...
      for (; bind_conf_node; bind_conf_node = bind_conf_node->next) {
        ...
        errcode |= ckch_inst_new_load_store(store->path, store, bind_conf,
                                            entry->ssl_conf, entry->filters,
                                            entry->fcount, &new_inst, &err);
        if (errcode & ERR_CODE)
          goto error;

        /* we need to initialize the SSL_CTX generated */
        /* this iterate on the newly generated SNIs in the new instance to
         * prepare their SSL_CTX */
        list_for_each_entry(sni, &new_inst->sni_ctx, by_ckch_inst) {
          if (!sni->order) { /* we initialized only the first SSL_CTX because
                                it's the same in the other sni_ctx's */
            errcode |= ssl_sock_prepare_ctx(bind_conf, new_inst->ssl_conf,
                                            sni->ctx, &err);
            if (errcode & ERR_CODE)
              goto error;
          }
 ...
 */
static int cli_parse_add_crtlist(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  int cfgerr = 0;
  struct ckch_store *store;
  char *err = NULL;
  ...
  if (!*args[3] || (!payload && !*args[4]))
    return cli_err(
        appctx,
        "'add ssl crtlist' expects a filename and a certificate name\n");

  crtlist_path = args[3];

  /* strip trailing slashes, including first one */
  for (end = crtlist_path + strlen(crtlist_path) - 1;
       end >= crtlist_path && *end == '/'; end--)
    *end = 0;
  ...
  if (!cert_path) {
    memprintf(
        &err,
        "'add ssl crtlist' should contain the certificate name in the payload");
    cfgerr |= ERR_ALERT | ERR_FATAL;
    goto error;
  }
  ...
  if (eb_gettag(crtlist->entries.b[EB_RGHT])) {
    ...
    if (!slash) {
      memprintf(&err,
                "'%s' is a directory, certificate path '%s' must contain the "
                "directory path",
                (char *)crtlist->node.key, cert_path);
      goto error;
    }
  ...
  /* this is supposed to be a directory (EB_ROOT_UNIQUE), so no ssl_conf are
   * allowed */
  if ((entry->ssl_conf || entry->filters) &&
      eb_gettag(crtlist->entries.b[EB_RGHT])) {
    memprintf(
        &err,
        "this is a directory, SSL configuration and filters are not allowed");
    goto error;
  }
...
static int cli_parse_del_crtlist(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  struct ckch_store *store;
  char *err = NULL;
  ...
  if (!*args[3] || !*args[4])
    return cli_err(
        appctx,
        "'del ssl crtlist' expects a filename and a certificate name\n");
  ...
  if (!ebpt) {
    memprintf(&err, "certificate '%s' can't be found in crt-list '%s'!",
              cert_path, crtlist_path);
    goto error;
  }

  /* list the line number of entries for errors in err, and select the right
   * ebpt */
  ...
  if (!entry) {
    memprintf(&err,
              "found a certificate '%s' but the line number is incorrect, "
              "please specify a correct line number preceded by colons (%s)!",
              cert_path, err ? err : NULL);
    goto error;
  }
  ...
  if (linenum == 0 && ebpt_next_dup(&entry->node)) {
    memprintf(&err,
              "found the certificate '%s' in several entries, please specify a "
              "line number preceded by colons (%s)!",
              cert_path, err ? err : NULL);
    goto error;
  }
...
static struct cli_kw_list cli_kws = {
    {},
    {{{"add", "ssl", "crt-list", NULL},
      "add ssl crt-list <filename> <certfile> [options] : add a line "
      "<certfile> to a crt-list <filename>",
      cli_parse_add_crtlist,
      cli_io_handler_add_crtlist,
      cli_release_add_crtlist},
     {{"del", "ssl", "crt-list", NULL},
      "del ssl crt-list <filename> <certfile[:line]> : delete a line "
      "<certfile> in a crt-list <filename>",
      cli_parse_del_crtlist,
      NULL,
      NULL},
     {{"show", "ssl", "crt-list", NULL},
      "show ssl crt-list [-n] [<filename>] : show the list of crt-lists or the "
      "content of a crt-list <filename>",
