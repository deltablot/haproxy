...
static int hlua_fcn_pushunsigned(lua_State *L, unsigned int val) {
     ...
     ((LUA_MAXINTEGER == LONG_MAX) && (__WORDSIZE == 64)))
...
void hlua_class_function(lua_State *L, const char *name,
 ...
 */
static int hlua_parse_date(lua_State *L,
                           int (*fcn)(const char *, int, struct tm *)) {
  const char *str;
  size_t len;
  struct tm tm;
  time_t time;
...
static void hlua_stktable_entry(lua_State *L, struct stktable *t,
                                struct stksess *ts) {
  int dt;
  void *ptr;
  ...
  case LUA_TTABLE:
    ...
    while (lua_next(L, 2) != 0) {
      ...
      if (filter_count >= STKTABLE_FILTER_LEN)
        return hlua_error(L, "Filter table too large (len > %d)",
                          STKTABLE_FILTER_LEN);

      if (lua_type(L, -1) != LUA_TTABLE || lua_rawlen(L, -1) != 3)
        return hlua_error(L,
                          "Filter table entry must be a triplet: "
                          "{\"data_col\", \"op\", val} (entry #%d)",
                          filter_count + 1);
      ...
      while (lua_next(L, -2) != 0) {
        ...
        case 0:
          if (lua_type(L, -1) != LUA_TSTRING)
            return hlua_error(
                L, "Filter table data column must be string (entry #%d)",
                filter_count + 1);

          dt = stktable_get_data_type((char *)lua_tostring(L, -1));
          if (dt < 0 || t->data_ofs[dt] == 0)
            return hlua_error(L,
                              "Filter table data column not present in stick "
                              "table (entry #%d)",
                              filter_count + 1);
        ...
        case 1:
          if (lua_type(L, -1) != LUA_TSTRING)
            return hlua_error(
                L, "Filter table operator must be string (entry #%d)",
                filter_count + 1);

          op = get_std_op(lua_tostring(L, -1));
          if (op < 0)
            return hlua_error(L, "Unknown operator in filter table (entry #%d)",
                              filter_count + 1);
...
int hlua_server_get_addr(lua_State *L) {
  ...
  case AF_INET:
    inet_ntop(AF_INET, &((struct sockaddr_in *)&srv->addr)->sin_addr, addr,
              INET_ADDRSTRLEN);
    ...
    break;
  case AF_INET6:
    inet_ntop(AF_INET6, &((struct sockaddr_in6 *)&srv->addr)->sin6_addr, addr,
              INET6_ADDRSTRLEN);
    luaL_addstring(&b, addr);
    luaL_addstring(&b, ":");
    snprintf(addr, INET_ADDRSTRLEN, "%d", srv->svc_port);
