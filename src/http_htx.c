...
struct conf_errors {
  ...
  union {
    ...
    struct {
      ...
      char status[HTTP_ERR_SIZE]; /* list of status to import (0: ignore, 1:
                                     implicit import, 2: explicit import) */
...
struct htx_sl *http_get_stline(struct htx *htx) {
  ...
  BUG_ON(htx_get_blk_type(blk) != HTX_BLK_REQ_SL &&
         htx_get_blk_type(blk) != HTX_BLK_RES_SL);
...
static int __http_find_header(const struct htx *htx, const void *pattern,
                              struct http_hdr_ctx *ctx, int flags) {
...
int http_find_header(const struct htx *htx, const struct ist name,
  ...
  return __http_find_header(
      htx, &name, ctx, HTTP_FIND_FL_MATCH_STR | (full ? HTTP_FIND_FL_FULL : 0));
}

/* Header names must match <name>. Same than http_find_header */
int http_find_str_header(const struct htx *htx, const struct ist name,
                         struct http_hdr_ctx *ctx, int full) {
  return __http_find_header(
      htx, &name, ctx, HTTP_FIND_FL_MATCH_STR | (full ? HTTP_FIND_FL_FULL : 0));
}
...
int http_find_pfx_header(const struct htx *htx, const struct ist prefix,
                         struct http_hdr_ctx *ctx, int full) {
  return __http_find_header(htx, &prefix, ctx,
                            HTTP_FIND_FL_MATCH_PFX |
                                (full ? HTTP_FIND_FL_FULL : 0));
}

/* Header names must end with <suffix> */
int http_find_sfx_header(const struct htx *htx, const struct ist suffix,
                         struct http_hdr_ctx *ctx, int full) {
  return __http_find_header(htx, &suffix, ctx,
                            HTTP_FIND_FL_MATCH_SFX |
                                (full ? HTTP_FIND_FL_FULL : 0));
}
/* Header names must contain <sub> */
int http_find_sub_header(const struct htx *htx, const struct ist sub,
                         struct http_hdr_ctx *ctx, int full) {
  return __http_find_header(
      htx, &sub, ctx, HTTP_FIND_FL_MATCH_SUB | (full ? HTTP_FIND_FL_FULL : 0));
}

/* Header names must match <re> regex*/
int http_match_header(const struct htx *htx, const struct my_regex *re,
                      struct http_hdr_ctx *ctx, int full) {
  return __http_find_header(
      htx, re, ctx, HTTP_FIND_FL_MATCH_REG | (full ? HTTP_FIND_FL_FULL : 0));
}
...
int http_add_header(struct htx *htx, const struct ist n, const struct ist v) {
  ...
  for (prev = htx_get_prev(htx, htx->tail); prev != htx->first;
       prev = htx_get_prev(htx, prev)) {
 ...
 */
int http_replace_stline(struct htx *htx, const struct ist p1,
...
int http_replace_req_query(struct htx *htx, const struct ist query) {
  ...
  uri = ist2(temp->area + meth.len + vsn.len,
             uri.len - q.len + query.len - offset);
 ...
 */
int http_replace_header_value(struct htx *htx, struct http_hdr_ctx *ctx,
 ...
 * http_find_header function. The value is removed, as well as surrounding
 * commas if any. If the removed value was alone, the whole header is removed.
 * The <ctx> is always updated accordingly, as well as the HTX message <htx>. It
 ...
 * returned.
 */
int http_update_authority(struct htx *htx, struct htx_sl *sl,
                          const struct ist host) {
  struct buffer *temp = get_trash_chunk();
  ...
  chunk_memcat(temp, authority.ptr + authority.len,
               uri.ptr + uri.len - (authority.ptr + authority.len));
  uri = ist2(temp->area + meth.len + vsn.len,
             host.len + uri.len - authority.len); /* uri */
  ...
  if (!uri.len || uri.ptr[0] == '/' || uri.ptr[0] == '*') {
    ...
    if (sl->info.req.meth != HTTP_METH_CONNECT) {
      ...
      if (uri.len > 4 && (uri.ptr[0] | 0x20) == 'h')
        sl->flags |=
            ((uri.ptr[4] == ':') ? HTX_SL_F_SCHM_HTTP : HTX_SL_F_SCHM_HTTPS);
...
unsigned int http_get_htx_hdr(const struct htx *htx, const struct ist hdr,
                              int occ, struct http_hdr_ctx *ctx, char **vptr,
                              size_t *vlen) {
...
int http_str_to_htx(struct buffer *buf, struct ist raw, char **errmsg) {
  ...
  ret = h1_headers_to_hdr_list(raw.ptr, raw.ptr + raw.len, hdrs,
                               sizeof(hdrs) / sizeof(hdrs[0]), &h1m, &h1sl);
  if (ret <= 0) {
    memprintf(errmsg, "unabled to parse headers (error offset: %d)",
              h1m.err_pos);
    goto error;
  }

  if (unlikely(h1sl.st.v.len != 8)) {
    memprintf(errmsg, "invalid http version (%.*s)", (int)h1sl.st.v.len,
              h1sl.st.v.ptr);
    goto error;
  }
  ...
  if (h1sl.st.status < 200 &&
      (h1sl.st.status == 100 || h1sl.st.status >= 102)) {
  ...
  if ((flags & HTX_SL_F_CLEN) && h1m.body_len != (raw.len - ret)) {
    memprintf(
        errmsg,
        "payload size does not match the announced content-length (%lu != %lu)",
        (unsigned long)(raw.len - ret), (unsigned long)h1m.body_len);
    goto error;
  }

  htx = htx_from_buf(buf);
  sl = htx_add_stline(htx, HTX_BLK_RES_SL, flags, h1sl.st.v, h1sl.st.c,
                      h1sl.st.r);
...
static int http_htx_init(void) {
  ...
  for (rc = 0; rc < HTTP_ERR_SIZE; rc++) {
    if (!http_err_msgs[rc]) {
      ha_alert(
          "Internal error: no default message defined for HTTP return code %d",
          rc);
    ...
    if (!http_str_to_htx(&chk, raw, &errmsg)) {
      ha_alert("Internal error: invalid default message for HTTP return code "
               "%d: %s.\n",
...
struct buffer *http_load_errorfile(const char *file, char **errmsg) {
  ...
  if (stat.st_size <= global.tune.bufsize)
    ...
    ha_warning(
        "custom error message file '%s' larger than %d bytes. Truncating.\n",
 ...
 */
struct buffer *http_load_errormsg(const char *key, const struct ist msg,
                                  char **errmsg) {
  struct buffer *buf = NULL;
  struct buffer chk;
  struct ebpt_node *node;
 ...
 */
struct buffer *http_parse_errorfile(int status, const char *file,
                                    char **errmsg) {
  struct buffer *buf = NULL;
  int rc;
 ...
 * message.
 */
struct buffer *http_parse_errorloc(int errloc, int status, const char *url,
                                   char **errmsg) {
  static const char *HTTP_302 = "HTTP/1.1 302 Found\r\n"
                                "Cache-Control: no-cache\r\n"
                                "Content-length: 0\r\n"
                                "Location: "; /* not terminated since it will be
                                                 concatenated with the URL */
  static const char *HTTP_303 = "HTTP/1.1 303 See Other\r\n"
                                "Cache-Control: no-cache\r\n"
                                "Content-length: 0\r\n"
                                "Location: "; /* not terminated since it will be
                                                 concatenated with the URL */
 ...
 * filled.
 */
int http_check_http_reply(struct http_reply *reply, struct proxy *px,
                          char **errmsg) {
  ...
  list_for_each_entry(http_errs, &http_errors_list, list) {
    if (strcmp(http_errs->id, reply->body.http_errors) == 0) {
      ...
      reply->body.reply =
          http_errs->replies[http_get_status_idx(reply->status)];
      if (!reply->body.reply)
  ...
  if (&http_errs->list == &http_errors_list) {
    memprintf(errmsg,
              "unknown http-errors section '%s' referenced by an http reply ",
 ...
 *   - HTTP_REPLY_ERRMSG   : implicit error message depending on the status code
 * or explicit one
 *   - HTTP_REPLY_ERRFILES : points on an http-errors section (resolved during
 * post-parsing)
...
struct http_reply *http_parse_http_reply(const char **args, int *orig_arg,
                                         struct proxy *px, int default_status,
                                         char **errmsg) {
  struct logformat_node *lf, *lfb;
  ...
  if (px->conf.args.ctx == ARGC_HERR)
    ...
    cap =
        ((px->conf.args.ctx == ARGC_HRQ)
             ...
             : ((px->cap & PR_CAP_BE) ? SMP_VAL_BE_HRS_HDR
                                      : SMP_VAL_FE_HRS_HDR));
  ...
  while (*args[cur_arg]) {
    if (strcmp(args[cur_arg], "status") == 0) {
      cur_arg++;
      if (!*args[cur_arg]) {
        memprintf(errmsg, "'%s' expects <status_code> as argument",
                  args[cur_arg - 1]);
        goto error;
      }
      ...
      cur_arg++;
      if (!*args[cur_arg]) {
        memprintf(errmsg, "'%s' expects <ctype> as argument",
                  args[cur_arg - 1]);
        goto error;
      }
      free(reply->ctype);
      ...
      if (reply->type != HTTP_REPLY_EMPTY) {
        memprintf(errmsg, "unexpected '%s' argument, '%s' already defined",
                  args[cur_arg], act_arg);
        goto error;
      }
      ...
      if (strcasecmp(args[cur_arg], "content-length") == 0 ||
        ...
        ha_warning(
            "parsing [%s:%d] : header '%s' always ignored by the http reply.\n",
      ...
      if (!parse_logformat_string(args[cur_arg + 1], px, &hdr->value,
                                  LOG_OPT_HTTP, cap, errmsg))
        goto error;
  ...
  if (reply->type == HTTP_REPLY_EMPTY) { /* no payload */
    if (reply->ctype) {
      ha_warning("parsing [%s:%d] : content-type '%s' ignored by the http "
                 "reply because"
  ...
  } else if (reply->type == HTTP_REPLY_ERRFILES ||
             reply->type == HTTP_REPLY_ERRMSG) { /* errorfiles or errorfile */
    ...
    if (reply->type != HTTP_REPLY_ERRMSG || !reply->body.errmsg) {
      ...
      if (rc >= HTTP_ERR_SIZE) {
        memprintf(errmsg,
                  "status code '%d' not handled by default with '%s' argument.",
    ...
    if (!LIST_ISEMPTY(&reply->hdrs)) {
      ha_warning("parsing [%s:%d] : hdr parameters ignored by the http reply "
                 "when used "
                 "with an erorrfile.\n",
      ...
      list_for_each_entry_safe(hdr, hdrb, &reply->hdrs, list) {
        ...
        free(hdr);
             ...
             HTTP_REPLY_RAW) { /* explicit parameter using 'file' parameter*/
    ...
    if (!reply->ctype && objlen) {
      memprintf(errmsg, "a content type must be defined when non-empty payload "
                        "is configured");
      goto error;
    }
    ...
    if (b_room(&reply->body.obj) < global.tune.maxrewrite) {
      ha_warning("parsing [%s:%d] : http reply payload runs over the buffer "
                 "space reserved to headers rewriting."
                 " It may lead to internal errors if strict rewriting mode is "
                 "enabled.\n",
                 px->conf.args.file, px->conf.args.line);
    }
  } else if (reply->type ==
             HTTP_REPLY_LOGFMT) { /* log-format payload using 'lf-file' of
                                     'lf-string' parameter */
    ...
    if (!reply->ctype) {
      memprintf(errmsg,
                "a content type must be defined with a log-format payload");
      goto error;
    }
    if (!parse_logformat_string(obj, px, &reply->body.fmt, LOG_OPT_HTTP, cap,
                                errmsg))
      goto error;
...
static int proxy_parse_http_error(char **args, int section, struct proxy *curpx,
  ...
  reply =
      http_parse_http_reply((const char **)args, &cur_arg, curpx, 0, errmsg);
  if (!reply) {
    memprintf(errmsg, "%s : %s", args[0], *errmsg);
    goto error;
  ...
  if (rc >= HTTP_ERR_SIZE) {
    memprintf(errmsg, "%s: status code '%d' not handled.", args[0],
              reply->status);
    goto error;
  }
  if (*args[cur_arg]) {
...
static int proxy_check_errors(struct proxy *px) {
  ...
  list_for_each_entry_safe(conf_err, conf_err_back, &px->conf.errors, list) {
    if (conf_err->type == 1) {
      ...
      if (px->replies[rc]->type == HTTP_REPLY_ERRMSG &&
          !px->replies[rc]->body.errmsg)
        px->replies[rc] = NULL;
    } else {
      /* errorfiles */
      list_for_each_entry(http_errs, &http_errors_list, list) {
      ...
      if (&http_errs->list == &http_errors_list) {
        ha_alert("config : proxy '%s': unknown http-errors section '%s' (at "
                 "%s:%d).\n",
                 px->id, conf_err->info.errorfiles.name, conf_err->file,
                 conf_err->line);
...
static int post_check_errors() {
  ...
  while (node) {
    ...
    if (htx_free_data_space(htx) < global.tune.maxrewrite) {
      ha_warning(
          "config: errorfile '%s' runs over the buffer space"
...
int proxy_dup_default_conf_errors(struct proxy *curpx, struct proxy *defpx,
                                  char **errmsg) {
  ...
  list_for_each_entry(conf_err, &defpx->conf.errors, list) {
    ...
    if (conf_err->type == 1) {
      ...
      new_conf_err->info.errorfiles.name =
          strdup(conf_err->info.errorfiles.name);
      if (!new_conf_err->info.errorfiles.name) {
        memprintf(errmsg,
                  "unable to duplicate default errors (out of memory).");
        goto out;
      }
      memcpy(&new_conf_err->info.errorfiles.status,
             &conf_err->info.errorfiles.status,
 ...
 * indicators.
 */
static int cfg_parse_http_errors(const char *file, int linenum, char **args,
                                 int kwm) {
  ...
  if (strcmp(args[0], "http-errors") == 0) { /* new errors section */
    ...
    list_for_each_entry(curr_errs, &http_errors_list, list) {
      ...
      if (strcmp(curr_errs->id, args[1]) == 0) {
        ha_alert("parsing [%s:%d]: http-errors section '%s' already exists "
                 "(declared at %s:%d).\n",
                 file, linenum, args[1], curr_errs->conf.file,
                 curr_errs->conf.line);
        err_code |= ERR_ALERT | ERR_FATAL;
      }
    }
    ...
    if (*(args[1]) == 0 || *(args[2]) == 0) {
      ha_alert("parsing [%s:%d] : %s: expects <status_code> and <file> as "
               "arguments.\n",
               file, linenum, args[0]);
      err_code |= ERR_ALERT | ERR_FATAL;
      goto out;
...
static int smp_fetch_is_htx(const struct arg *arg_p, struct sample *smp,
...
static int smp_fetch_htx_nbblks(const struct arg *arg_p, struct sample *smp,
                                const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
 ...
 * sample direction. */
static int smp_fetch_htx_size(const struct arg *arg_p, struct sample *smp,
                              const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
...
/* Returns the data size of an HTX message. The channel is chosen depending on
 * the sample direction. */
static int smp_fetch_htx_data(const struct arg *arg_p, struct sample *smp,
                              const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
 ...
 * depending on the sample direction. */
static int smp_fetch_htx_used(const struct arg *arg_p, struct sample *smp,
                              const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
 ...
 * depending on the sample direction. */
static int smp_fetch_htx_free(const struct arg *arg_p, struct sample *smp,
                              const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
...
static int smp_fetch_htx_free_data(const struct arg *arg_p, struct sample *smp,
                                   const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
 ...
 * the sample direction. */
static int smp_fetch_htx_has_eom(const struct arg *arg_p, struct sample *smp,
                                 const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
 ...
 * direction. */
static int smp_fetch_htx_blk_type(const struct arg *arg_p, struct sample *smp,
                                  const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
  enum htx_blk_type type;
...
static int smp_fetch_htx_blk_size(const struct arg *arg_p, struct sample *smp,
                                  const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
  struct htx_blk *blk;
  ...
  if (pos == -1)
    ...
    blk =
        ((pos >= htx->head && pos <= htx->tail) ? htx_get_blk(htx, pos) : NULL);
 ...
 * the sample direction. */
static int smp_fetch_htx_blk_stline(const struct arg *arg_p, struct sample *smp,
                                    const char *kw, void *private) {
  struct buffer *temp;
  struct channel *chn;
  struct htx *htx;
  ...
  if (!blk || (htx_get_blk_type(blk) != HTX_BLK_REQ_SL &&
               htx_get_blk_type(blk) != HTX_BLK_RES_SL)) {
 ...
 * supported or "head", "tail" or "first". The channel is chosen depending on
 * the sample direction. */
static int smp_fetch_htx_blk_hdrname(const struct arg *arg_p,
  ...
  if (!blk || (htx_get_blk_type(blk) != HTX_BLK_HDR &&
               htx_get_blk_type(blk) != HTX_BLK_TLR)) {
    smp->data.u.str.size = 0;
    smp->data.u.str.area = "";
    smp->data.u.str.data = 0;
  } else {
 ...
 * supported or "head", "tail" or "first". The channel is chosen depending on
 * the sample direction. */
static int smp_fetch_htx_blk_hdrval(const struct arg *arg_p, struct sample *smp,
                                    const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
  struct htx_blk *blk;
 ...
 * direction. */
static int smp_fetch_htx_blk_data(const struct arg *arg_p, struct sample *smp,
                                  const char *kw, void *private) {
  struct channel *chn;
  struct htx *htx;
  struct htx_blk *blk;
...
int val_blk_arg(struct arg *arg, char **err_msg) {
  if (arg[0].type != ARGT_STR || !arg[0].data.str.data) {
    memprintf(err_msg, "a block position is expected (> 0) or a special block "
                       "name (head, tail, first)");
    return 0;
  }
  if (arg[0].data.str.data == 4 && !strncmp(arg[0].data.str.area, "head", 4)) {
    ...
    arg[0].data.sint = -1;
  } else if (arg[0].data.str.data == 4 &&
             !strncmp(arg[0].data.str.area, "tail", 4)) {
    chunk_destroy(&arg[0].data.str);
    arg[0].type = ARGT_SINT;
    arg[0].data.sint = -2;
  } else if (arg[0].data.str.data == 5 &&
             !strncmp(arg[0].data.str.area, "first", 5)) {
    chunk_destroy(&arg[0].data.str);
    arg[0].type = ARGT_SINT;
    arg[0].data.sint = -3;
  } else {
...
static struct sample_fetch_kw_list sample_fetch_keywords = {
    ILH,
    {
        {"internal.strm.is_htx", smp_fetch_is_htx, 0, NULL, SMP_T_BOOL,
         SMP_USE_L6REQ},

        {"internal.htx.nbblks", smp_fetch_htx_nbblks, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.size", smp_fetch_htx_size, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.data", smp_fetch_htx_data, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.used", smp_fetch_htx_used, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.free", smp_fetch_htx_free, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.free_data", smp_fetch_htx_free_data, 0, NULL, SMP_T_SINT,
         SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx.has_eom", smp_fetch_htx_has_eom, 0, NULL, SMP_T_BOOL,
         SMP_USE_HRQHV | SMP_USE_HRSHV},

        {"internal.htx_blk.type", smp_fetch_htx_blk_type, ARG1(1, STR),
         val_blk_arg, SMP_T_STR, SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx_blk.size", smp_fetch_htx_blk_size, ARG1(1, STR),
         val_blk_arg, SMP_T_SINT, SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx_blk.start_line", smp_fetch_htx_blk_stline, ARG1(1, STR),
         val_blk_arg, SMP_T_STR, SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx_blk.hdrname", smp_fetch_htx_blk_hdrname, ARG1(1, STR),
         val_blk_arg, SMP_T_STR, SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx_blk.hdrval", smp_fetch_htx_blk_hdrval, ARG1(1, STR),
         val_blk_arg, SMP_T_STR, SMP_USE_HRQHV | SMP_USE_HRSHV},
        {"internal.htx_blk.data", smp_fetch_htx_blk_data, ARG1(1, STR),
         val_blk_arg, SMP_T_BIN, SMP_USE_HRQHV | SMP_USE_HRSHV},
