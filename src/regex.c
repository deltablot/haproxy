...
THREAD_LOCAL regmatch_t
    pmatch[MAX_MATCH]; /* rm_so, rm_eo for regular expressions */

int exp_replace(char *dst, unsigned int dst_size, char *src, const char *str,
...
/* returns NULL if the replacement string <str> is valid, or the pointer to the
 * first error */
  ...
  while (*str) {
    if (*str == '\\') {
      ...
      if (!*str)
        ...
        ha_warning("'\\%c' : deprecated use of a backslash before something "
                   "not '\\','x' or a digit.\n",
                   *str);
        err = NULL;
      }
...
int regex_exec_match(const struct my_regex *preg, const char *subject,
...
#if defined(USE_PCRE) || defined(USE_PCRE_JIT) || defined(USE_PCRE2) ||        \
    defined(USE_PCRE2_JIT)
  int ret;
#ifdef USE_PCRE2
  PCRE2_SIZE *matches;
  pcre2_match_data *pm;
  ...
  if (flags & REG_NOTBOL)
    ...
    /* The value returned by pcre_exec()/pcre2_match() is one more than the
     * highest numbered pair that has been set. For example, if two substrings
     * have been captured, the returned value is 3. If there are no capturing
     * subpatterns, the return value from a successful match is 1, indicating
     * that just the first pair of offsets has been set.
     *
     * It seems that this function returns 0 if it detects more matches than
     * available space in the matches array.
     */
#ifdef USE_PCRE2
...
#else
  ret = pcre_exec(preg->reg, preg->extra, subject, strlen(subject), 0, options,
                  matches, enmatch * 3);
...
int regex_exec_match2(const struct my_regex *preg, char *subject, int length,
  ...
  if (flags & REG_NOTBOL)
    ...
    /* The value returned by pcre_exec()/pcre2_(jit)_match() is one more than
     * the highest numbered pair that has been set. For example, if two
     * substrings have been captured, the returned value is 3. If there are no
...
#ifdef USE_PCRE2
  pm = pcre2_match_data_create_from_pattern(preg->reg, NULL);
  ret = preg->mfn(preg->reg, (PCRE2_SPTR)subject, (PCRE2_SIZE)length, 0,
                  options, pm, NULL);
  ...
  matches = pcre2_get_ovector_pointer(pm);
#else
  ret = pcre_exec(preg->reg, preg->extra, subject, length, 0, options, matches,
                  enmatch * 3);
  if (ret < 0)
    return 0;
#endif
...
struct my_regex *regex_comp(const char *str, int cs, int cap, char **err) {
  ...
  if (!regex->reg) {
    memprintf(err, "regex '%s' is invalid (error=%s, erroffset=%d)", str, error,
              erroffset);
    goto out_fail_alloc;
  }
  ...
  regex->reg = pcre2_compile((PCRE2_SPTR)str, PCRE2_ZERO_TERMINATED, flags,
                             &errn, &erroffset, NULL);
  if (!regex->reg) {
    pcre2_get_error_message(errn, error, sizeof(error));
    memprintf(err, "regex '%s' is invalid (error=%s, erroffset=%zu)", str,
              error, erroffset);
    goto out_fail_alloc;
  }
...
#ifdef USE_PCRE
  memprintf(&ptr, "Built with PCRE version : %s",
            (HAP_XSTRING(Z PCRE_PRERELEASE)[1] == 0)
                ? HAP_XSTRING(PCRE_MAJOR.PCRE_MINOR PCRE_DATE)
                : HAP_XSTRING(PCRE_MAJOR.PCRE_MINOR)
                      HAP_XSTRING(PCRE_PRERELEASE PCRE_DATE));
...
#ifdef USE_PCRE2
  memprintf(&ptr, "Built with PCRE2 version : %s",
            (HAP_XSTRING(Z PCRE2_PRERELEASE)[1] == 0)
                ? HAP_XSTRING(PCRE2_MAJOR.PCRE2_MINOR PCRE2_DATE)
                : HAP_XSTRING(PCRE2_MAJOR.PCRE2_MINOR)
                      HAP_XSTRING(PCRE2_PRERELEASE PCRE2_DATE));
