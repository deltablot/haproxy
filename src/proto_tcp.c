...
static int tcp_bind_listener(struct listener *listener, char *errmsg,
                             int errlen);
 ...
 * which case we try to bind <remote>. <flags> is a 2-bit field consisting of :
 ...
 */
int tcp_bind_socket(int fd, int flags, struct sockaddr_storage *local,
  ...
  if (flags) {
    ...
    case AF_INET6:
      ...
      if (flags & 2)
        ((struct sockaddr_in6 *)&bind_addr)->sin6_port =
 ...
 * source port). conn->target may point either to a valid server or to a
 * backend, depending on conn->target. Only OBJ_TYPE_PROXY and OBJ_TYPE_SERVER
 * are supported. The <data> parameter is a boolean indicating whether there are
 * data waiting for being sent or not, in order to adjust data write polling and
 * on some platforms, the ability to avoid an empty initial ACK. The <flags>
 * argument allows the caller to force using a delayed ACK when establishing the
 * connection
 *   - 0 = no delayed ACK unless data are advertised and backend has
 * tcp-smart-connect
 *   - CONNECT_DELACK_SMART_CONNECT = delayed ACK if backend has
 * tcp-smart-connect, regardless of data
...
int tcp_connect_server(struct connection *conn, int flags) {
  ...
  if (fd == -1) {
    ...
    if (errno == ENFILE) {
      ...
      send_log(be, LOG_EMERG,
               "Proxy %s reached system FD limit (maxsock=%d). Please check "
               "system tunables.\n",
               be->id, global.maxsock);
    } else if (errno == EMFILE) {
      conn->err_code = CO_ER_PROC_FDLIM;
      send_log(be, LOG_EMERG,
               "Proxy %s reached process FD limit (maxsock=%d). Please check "
               "'ulimit-n' and restart.\n",
               be->id, global.maxsock);
    } else if (errno == ENOBUFS || errno == ENOMEM) {
      conn->err_code = CO_ER_SYS_MEMLIM;
      send_log(be, LOG_EMERG,
               "Proxy %s reached system memory limit (maxsock=%d). Please "
  ...
  if (fd >= global.maxsock) {
     ...
     */
    ha_alert(
        "socket(): not enough free sockets. Raise -n argument. Giving up.\n");
    close(fd);
...
#ifdef TCP_KEEPCNT
    if (be->srvtcpka_cnt)
      setsockopt(fd, IPPROTO_TCP, TCP_KEEPCNT, &be->srvtcpka_cnt,
                 sizeof(be->srvtcpka_cnt));
#endif

#ifdef TCP_KEEPIDLE
    if (be->srvtcpka_idle)
      setsockopt(fd, IPPROTO_TCP, TCP_KEEPIDLE, &be->srvtcpka_idle,
                 sizeof(be->srvtcpka_idle));
#endif

#ifdef TCP_KEEPINTVL
    if (be->srvtcpka_intvl)
      setsockopt(fd, IPPROTO_TCP, TCP_KEEPINTVL, &be->srvtcpka_intvl,
                 sizeof(be->srvtcpka_intvl));
#endif
  }
  ...
  if (src) {
    ...
    if (src->iface_name)
      setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, src->iface_name,
                 src->iface_len + 1);
#endif
    ...
    if (src->sport_range) {
      ...
      setsockopt(fd, SOL_IP, IP_BIND_ADDRESS_NO_PORT,
                 (const void *)&bind_address_no_port, sizeof(int));
#endif
    ...
    if (unlikely(ret != 0)) {
      ...
      if (ret == 1) {
        ha_alert("Cannot bind to source address before connect() for backend "
                 "%s. Aborting.\n",
                 be->id);
        send_log(
            be, LOG_EMERG,
        ...
        ha_alert("Cannot bind to tproxy source address before connect() for "
                 "backend %s. Aborting.\n",
                 be->id);
        send_log(be, LOG_EMERG,
                 "Cannot bind to tproxy source address before connect() for "
                 "backend %s.\n",
                 be->id);
      }
      conn->flags |= CO_FL_ERROR;
  ...
  if (flags & (CONNECT_DELACK_ALWAYS) ||
      ((flags & CONNECT_DELACK_SMART_CONNECT || (flags & CONNECT_HAS_DATA) ||
        conn->send_proxy_ofs) &&
  ...
  if (srv && srv->tcp_ut)
    setsockopt(fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &srv->tcp_ut,
               sizeof(srv->tcp_ut));
#endif
  ...
  if (global.tune.server_sndbuf)
    setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &global.tune.server_sndbuf,
               sizeof(global.tune.server_sndbuf));

  if (global.tune.server_rcvbuf)
    setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &global.tune.server_rcvbuf,
  ...
  if (connect(fd, (const struct sockaddr *)addr, get_addr_len(addr)) == -1) {
    if (errno == EINPROGRESS || errno == EALREADY) {
      ...
      conn->flags &= ~CO_FL_WAIT_L4_CONN;
    } else if (errno == EAGAIN || errno == EADDRINUSE ||
               errno == EADDRNOTAVAIL) {
      char *msg;
      ...
      // (errno == ECONNREFUSED || errno == ENETUNREACH || errno == EACCES ||
      // errno == EPERM)
...
int tcp_bind_listener(struct listener *listener, char *errmsg, int errlen) {
  ...
  if (listener->options & LI_O_NOLINGER)
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_LINGER, &tmplinger, &len) == 0 &&
      ...
      setsockopt(fd, SOL_SOCKET, SO_LINGER, &tmplinger, sizeof(tmplinger));
    }
  }

#if defined(TCP_MAXSEG)
  if (listener->maxseg > 0) {
    if (setsockopt(fd, IPPROTO_TCP, TCP_MAXSEG, &listener->maxseg,
                   sizeof(listener->maxseg)) == -1) {
      msg = "cannot set MSS";
      err |= ERR_WARN;
    }
  } else {
    ...
    if (defaultmss > 0 && tmpmaxseg != defaultmss &&
  ...
  if (listener->tcp_ut) {
    if (setsockopt(fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &listener->tcp_ut,
                   sizeof(listener->tcp_ut)) == -1) {
      msg = "cannot set TCP User Timeout";
      err |= ERR_WARN;
    }
  } else
    setsockopt(fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &zero, sizeof(zero));
#endif
#if defined(TCP_DEFER_ACCEPT)
  if (listener->options & LI_O_DEF_ACCEPT) {
    /* defer accept by up to one second */
    int accept_delay = 1;
    if (setsockopt(fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &accept_delay,
                   sizeof(accept_delay)) == -1) {
      msg = "cannot enable DEFER_ACCEPT";
      err |= ERR_WARN;
    }
  } else
    setsockopt(fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &zero, sizeof(zero));
#endif
  ...
  if (listener->options & LI_O_TCP_FO) {
    ...
    if (getsockopt(fd, IPPROTO_TCP, TCP_FASTOPEN, &qlen, &len) == 0 &&
        qlen != 0) {
      if (setsockopt(fd, IPPROTO_TCP, TCP_FASTOPEN, &zero, sizeof(zero)) ==
          -1) {
...
tcp_return:
  if (msg && errlen) {
    ...
    snprintf(errmsg, errlen, "%s [%s:%d]", msg, pn,
 ...
 * still be valid.
 */
static void tcp_disable_listener(struct listener *l) { fd_stop_recv(l->rx.fd); }
