...
int tcp_inspect_request(struct stream *s, struct channel *req, int an_bit) {
  ...
  list_for_each_entry(rule, &s->be->tcp_req.inspect_rules, list) {
    ...
    if (rule->cond) {
      ret =
          acl_exec_cond(rule->cond, s->be, sess, s, SMP_OPT_DIR_REQ | partial);
        ...
        case ACT_RET_YIELD:
          ...
          if (partial & SMP_OPT_FINAL) {
            send_log(s->be, LOG_WARNING,
                     "Internal error: yield not allowed if the inspect-delay "
                     "expired "
...
missing_data:
  ...
  req->analyse_exp = tick_first(
      (tick_is_expired(req->analyse_exp, now_ms) ? 0 : req->analyse_exp),
      s->rules_exp);
  DBG_TRACE_DEVEL("waiting for more data", STRM_EV_STRM_ANA | STRM_EV_TCP_ANA,
                  s);
  return 0;
...
abort:
  ...
  if (!(s->flags & SF_FINST_MASK))
    s->flags |= SF_FINST_R;
  DBG_TRACE_DEVEL("leaving on error|deny|abort",
                  STRM_EV_STRM_ANA | STRM_EV_TCP_ANA | STRM_EV_TCP_ERR, s);
  return 0;
}
...
int tcp_inspect_response(struct stream *s, struct channel *rep, int an_bit) {
  ...
  list_for_each_entry(rule, &s->be->tcp_rep.inspect_rules, list) {
    ...
    if (rule->cond) {
      ret =
          acl_exec_cond(rule->cond, s->be, sess, s, SMP_OPT_DIR_RES | partial);
      if (ret == ACL_TEST_MISS)
        goto missing_data;
...
missing_data:
  ...
  rep->analyse_exp = tick_first(
      (tick_is_expired(rep->analyse_exp, now_ms) ? 0 : rep->analyse_exp),
      s->rules_exp);
  DBG_TRACE_DEVEL("waiting for more data", STRM_EV_STRM_ANA | STRM_EV_TCP_ANA,
                  s);
  return 0;
 ...
 * any data. This only works on connection-based client-facing stream
 * interfaces.
 */
int tcp_exec_l4_rules(struct session *sess) {
  ...
  list_for_each_entry(rule, &sess->fe->tcp_req.l4_rules, list) {
    ...
    if (rule->cond) {
      ret = acl_exec_cond(rule->cond, sess->fe, sess, NULL,
      ...
      if (rule->action_ptr) {
        ...
        case ACT_RET_YIELD:
          ...
          send_log(sess->fe, LOG_WARNING,
                   "Internal error: yield not allowed with tcp-request "
                   "connection actions.");
          /* fall through */
        case ACT_RET_STOP:
        case ACT_RET_DONE:
    ...
    if (ret) {
      ...
      if (rule->action_ptr) {
        ...
        case ACT_RET_YIELD:
           ...
           */
          send_log(sess->fe, LOG_WARNING,
                   "Internal error: yield not allowed with tcp-request session "
                   "actions.");
          /* fall through */
        case ACT_RET_STOP:
        case ACT_RET_DONE:
...
static int tcp_parse_response_rule(char **args, int arg, int section_type,
                                   ...
                                   unsigned int where, const char *file,
                                   int line) {
  if (curpx == defpx || !(curpx->cap & PR_CAP_BE)) {
    memprintf(err, "%s %s is only allowed in 'backend' sections", args[0],
              args[1]);
    return -1;
  }
  ...
  if (strcmp(args[arg], "accept") == 0) {
    ...
    if (kw) {
      ...
      memprintf(err,
                "'%s %s' expects 'accept', 'close', 'reject', %s in %s '%s' "
                "(got '%s')",
                args[0], args[1], trash.area, proxy_type_str(curpx), curpx->id,
                args[arg]);
      return -1;
    }
  }
  ...
  if (strcmp(args[arg], "if") == 0 || strcmp(args[arg], "unless") == 0) {
    if ((rule->cond = build_acl_cond(file, line, &curpx->acl, curpx,
                                     (const char **)args + arg, err)) == NULL) {
      memprintf(err,
                "'%s %s %s' : error detected in %s '%s' while parsing '%s' "
                "condition : %s",
    ...
    memprintf(
        err, "'%s %s %s' only accepts 'if' or 'unless', in %s '%s' (got '%s')",
 ...
 */
static enum act_return tcp_action_track_sc(struct act_rule *rule,
                                           struct proxy *px,
                                           struct session *sess,
  ...
  if (rule->from == ACT_F_TCP_REQ_CNT) { /* L7 rules: use the stream */
    ...
    key = stktable_fetch_key(t, s->be, sess, s, opt, rule->arg.trk_ctr.expr,
                             &smp);
    ...
    key = stktable_fetch_key(t, sess->fe, sess, NULL, opt,
 ...
 * ACT_RET_ERR is returned.
 */
static enum act_return tcp_action_capture(struct act_rule *rule,
                                          struct proxy *px,
                                          struct session *sess,
                                          struct stream *s, int flags) {
  struct sample *key;
  ...
  key =
      sample_fetch_as_type(s->be, sess, s, opt, rule->arg.cap.expr, SMP_T_STR);
  if (!key)
    goto end;
...
static int tcp_parse_request_rule(char **args, int arg, int section_type,
                                  ...
                                  int line) {
  if (curpx == defpx) {
    memprintf(err, "%s %s is not allowed in 'defaults' sections", args[0],
              args[1]);
    return -1;
  }
  ...
  if (!strcmp(args[arg], "accept")) {
    ...
    if (!(curpx->cap & PR_CAP_FE)) {
      memprintf(err, "'%s %s %s' : proxy '%s' has no frontend capability",
                args[0], args[1], args[kw], curpx->id);
      return -1;
    }

    if (!(where & SMP_VAL_FE_REQ_CNT)) {
      memprintf(err, "'%s %s' is not allowed in '%s %s' rules in %s '%s'",
                args[arg], args[arg + 1], args[0], args[1],
    ...
    expr =
        sample_parse_expr(args, &arg, file, line, err, &curpx->conf.args, NULL);
    if (!expr) {
    ...
    if (!(expr->fetch->val & where)) {
      memprintf(err,
                "'%s %s %s' : fetch method '%s' extracts information from "
                "'%s', none of which is available here",
                args[0], args[1], args[kw], args[arg - 1],
    ...
    if (strcmp(args[arg], "len") == 0) {
      arg++;
      if (!args[arg]) {
        memprintf(err, "'%s %s %s' : missing length value", args[0], args[1],
                  args[kw]);
        release_sample_expr(expr);
        return -1;
      }
      ...
      len = atoi(args[arg]);
      if (len <= 0) {
        memprintf(err, "'%s %s %s' : length must be > 0", args[0], args[1],
                  args[kw]);
        release_sample_expr(expr);
        return -1;
      }
    ...
    if (!len) {
      memprintf(err, "'%s %s %s' : a positive 'len' argument is mandatory",
    ...
    if (strcmp(args[arg], "table") == 0) {
      arg++;
      if (!args[arg]) {
        memprintf(err, "'%s %s %s' : missing table name", args[0], args[1],
                  args[kw]);
        release_sample_expr(expr);
        return -1;
      }
    ...
    if (strcmp(args[arg + 1], "layer4") != 0) {
      memprintf(err, "'%s %s %s' only supports 'layer4' in %s '%s' (got '%s')",
                args[0], args[1], args[arg], proxy_type_str(curpx), curpx->id,
                args[arg + 1]);
      return -1;
    }
    ...
    if (kw) {
      ...
      memprintf(err,
                ...
                "'track-sc%d', %s "
                "in %s '%s' (got '%s').\n",
                args[0], args[1], MAX_SESS_STKCTR - 1, trash.area,
...
static int tcp_parse_tcp_rep(char **args, int section_type, struct proxy *curpx,
  ...
  if (!*args[1]) {
    memprintf(err, "missing argument for '%s' in %s '%s'", args[0],
              proxy_type_str(curpx), curpx->id);
    return -1;
  }
  ...
  if (strcmp(args[1], "inspect-delay") == 0) {
    ...
    if (!*args[2] || (ptr = parse_time_err(args[2], &val, TIME_UNIT_MS))) {
      ...
      if (ptr == PARSE_TIME_OVER)
        memprintf(err,
                  "%s (timer overflow in '%s', maximum value is 2147483647 ms "
                  "or ~24.8 days)",
                  *err, args[2]);
      else if (ptr == PARSE_TIME_UNDER)
        memprintf(
            err, "%s (timer underflow in '%s', minimum non-null value is 1 ms)",
            *err, args[2]);
      else if (ptr)
    ...
    if (curpx->tcp_rep.inspect_delay) {
      memprintf(err, "ignoring %s %s (was already defined) in %s '%s'", args[0],
                args[1], proxy_type_str(curpx), curpx->id);
      return 1;
    }
  ...
  if (strcmp(args[1], "content") == 0) {
    ...
    if (tcp_parse_response_rule(args, arg, section_type, curpx, defpx, rule,
                                err, where, file, line) < 0)
      goto error;
    ...
    if (acl) {
      if (acl->name && *acl->name)
        memprintf(err,
                  "acl '%s' will never match in '%s %s' because it only "
                  "involves keywords that are incompatible with '%s'",
                  acl->name, args[0], args[1], sample_ckp_names(where));
      else
        memprintf(err,
                  "anonymous acl will never match in '%s %s' because it uses "
                  "keyword '%s' which is incompatible with '%s'",
                  args[0], args[1],
      ...
      warn++;
               ...
               acl_cond_kw_conflicts(rule->cond, where, &acl, &kw)) {
      if (acl->name && *acl->name)
        memprintf(
            err,
        ...
        memprintf(err,
                  "anonymous acl involves keyword '%s' which is incompatible "
                  "with '%s'",
...
static int tcp_parse_tcp_req(char **args, int section_type, struct proxy *curpx,
  ...
  if (strcmp(args[1], "content") == 0) {
    ...
    if (tcp_parse_request_rule(args, arg, section_type, curpx, defpx, rule, err,
                               where, file, line) < 0)
      goto error;
    ...
    if (curpx == defpx)
      memprintf(err,
                "'%s' expects 'inspect-delay', 'connection', or 'content' in "
                "defaults section (got '%s')",
                args[0], args[1]);
    else
      memprintf(err,
                "'%s' expects 'inspect-delay', 'connection', or 'content' in "
