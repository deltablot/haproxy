...
struct xprt_ops *registered_xprt[XPRT_ENTRIES] = {
    NULL,
};
 ...
 */
int conn_ctrl_send(struct connection *conn, const void *buf, int len,
                   int flags) {
...
int conn_recv_proxy(struct connection *conn, int flag) {
  ...
  if (!sockaddr_alloc(&conn->src, NULL, 0) ||
      !sockaddr_alloc(&conn->dst, NULL, 0))
    goto fail;
...
not_v1:
  ...
  case 0x01: /* PROXY command */
    ...
    case 0x11: /* TCPv4 */
      ...
      ((struct sockaddr_in *)conn->src)->sin_family = AF_INET;
      ((struct sockaddr_in *)conn->src)->sin_addr.s_addr =
          hdr_v2->addr.ip4.src_addr;
      ((struct sockaddr_in *)conn->src)->sin_port = hdr_v2->addr.ip4.src_port;
      ((struct sockaddr_in *)conn->dst)->sin_family = AF_INET;
      ((struct sockaddr_in *)conn->dst)->sin_addr.s_addr =
    ...
    case 0x21: /* TCPv6 */
      ...
      ((struct sockaddr_in6 *)conn->src)->sin6_family = AF_INET6;
      memcpy(&((struct sockaddr_in6 *)conn->src)->sin6_addr,
             hdr_v2->addr.ip6.src_addr, 16);
      ((struct sockaddr_in6 *)conn->src)->sin6_port = hdr_v2->addr.ip6.src_port;
      ((struct sockaddr_in6 *)conn->dst)->sin6_family = AF_INET6;
      memcpy(&((struct sockaddr_in6 *)conn->dst)->sin6_addr,
...
int conn_recv_netscaler_cip(struct connection *conn, int flag) {
   ...
   * it does not match */
  if (ntohl(read_u32(line)) !=
      __objt_listener(conn->target)->bind_conf->ns_cip_magic)
    goto bad_magic;
...
int conn_send_socks4_proxy_request(struct connection *conn) {
  ...
  if (conn->send_proxy_ofs < 0) {
     ...
     */
    ret = conn_ctrl_send(
        conn, ((char *)(&req_line)) + (sizeof(req_line) + conn->send_proxy_ofs),
        -conn->send_proxy_ofs,
        (conn->subs && conn->subs->events & SUB_RETRY_SEND) ? CO_SFL_MSG_MORE
                                                            : 0);
...
int conn_recv_socks4_proxy_response(struct connection *conn) {
  ...
  while (1) {
    /* SOCKS4 Proxy will response with 8 bytes, 0x00 | 0x5A | 0x00 0x00 | 0x00
     * 0x00 0x00 0x00 Try to peek into it, before all 8 bytes ready.
     */
    ...
    if (ret == 0) {
      ...
      DPRINTF(stderr,
              "SOCKS PROXY HS FD[%04X]: Received ret[%d], errno[%d], looks "
              "like the socket has been closed or shutdown for send\n",
    ...
    if (ret > 0) {
      if (ret == SOCKS4_HS_RSP_LEN) {
        DPRINTF(stderr,
                "SOCKS PROXY HS FD[%04X]: Received 8 bytes, the response is "
                "[%02X|%02X|%02X %02X|%02X %02X %02X %02X]\n",
                conn->handle.fd, line[0], line[1], line[2], line[3], line[4],
                line[5], line[6], line[7]);
      } else {
        DPRINTF(stderr,
                "SOCKS PROXY HS FD[%04X]: Received ret[%d], first byte is "
                "[%02X], last bye is [%02X]\n",
   ...
   *the client 93: request rejected because the client program and identd report
   *different user-ids The remaining fields are ignored.
   */
  if (line[1] != 90) {
    conn->flags &= ~CO_FL_SOCKS4_RECV;

    DPRINTF(stderr,
            "SOCKS PROXY HS FD[%04X]: FAIL, the response is [%02X|%02X|%02X "
            "%02X|%02X %02X %02X %02X]\n",
            conn->handle.fd, line[0], line[1], line[2], line[3], line[4],
            line[5], line[6], line[7]);
    if (conn->err_code == CO_ER_NONE) {
...
int make_proxy_line(char *buf, int buf_len, struct server *srv,
                    struct connection *remote, struct stream *strm) {
  int ret = 0;
...
int make_proxy_line_v1(char *buf, int buf_len, struct sockaddr_storage *src,
  ...
  if (src->ss_family == AF_INET && dst->ss_family == AF_INET) {
    protocol = "TCP4";
    if (!inet_ntop(AF_INET, &((struct sockaddr_in *)src)->sin_addr, src_str,
                   sizeof(src_str)))
      return 0;
    src_port = ((struct sockaddr_in *)src)->sin_port;
    if (!inet_ntop(AF_INET, &((struct sockaddr_in *)dst)->sin_addr, dst_str,
                   sizeof(dst_str)))
      return 0;
  ...
  ret = snprintf(buf, buf_len, "PROXY %s %s %s %u %u\r\n", protocol, src_str,
                 dst_str, ntohs(src_port), ntohs(dst_port));
  if (ret >= buf_len)
    return 0;
...
int make_proxy_line_v2(char *buf, int buf_len, struct server *srv,
  ...
  if (!src || !dst ||
      (!pp2_never_send_local &&
       conn_is_back(remote)) // locally initiated connection
      || (src->ss_family != AF_INET && src->ss_family != AF_INET6) ||
      (dst->ss_family != AF_INET && dst->ss_family != AF_INET6)) {
    ...
    /* IPv6 for at least one of src and dst */
      ...
      if (src->ss_family == AF_INET) {
        ...
        memcpy(hdr->addr.ip6.src_addr, &((struct sockaddr_in6 *)src)->sin6_addr,
               16);
      ...
      if (dst->ss_family == AF_INET) {
        ...
        memcpy(hdr->addr.ip6.dst_addr, &((struct sockaddr_in6 *)dst)->sin6_addr,
               16);
  ...
  if (remote && conn_get_alpn(remote, &value, &value_len)) {
    if ((buf_len - ret) < sizeof(struct tlv))
      return 0;
    ret +=
        make_tlv(&buf[ret], (buf_len - ret), PP2_TYPE_ALPN, value_len, value);
  }
  ...
  if (srv->pp_opts & SRV_PP_V2_AUTHORITY) {
    ...
    if (value) {
      if ((buf_len - ret) < sizeof(struct tlv))
        return 0;
      ret += make_tlv(&buf[ret], (buf_len - ret), PP2_TYPE_AUTHORITY, value_len,
                      value);
    }
  }
  ...
  if (strm && (srv->pp_opts & SRV_PP_V2_UNIQUE_ID)) {
    ...
    struct ist unique_id =
        stream_generate_unique_id(strm, &sess->fe->format_unique_id);
    ...
    if (value_len >= 0) {
      if ((buf_len - ret) < sizeof(struct tlv))
        return 0;
      ret += make_tlv(&buf[ret], (buf_len - ret), PP2_TYPE_UNIQUE_ID, value_len,
                      value);
    }
  }
    ...
    if (ssl_sock_is_ssl(remote)) {
      ...
      if (value) {
        ssl_tlv_len +=
            make_tlv(&buf[ret + ssl_tlv_len], (buf_len - ret - ssl_tlv_len),
                     PP2_SUBTYPE_SSL_VERSION, strlen(value), value);
      }
      ...
      if (srv->pp_opts & SRV_PP_V2_SSL_CN) {
        ...
        if (ssl_sock_get_remote_common_name(remote, cn_trash) > 0) {
          ssl_tlv_len +=
              make_tlv(&buf[ret + ssl_tlv_len], (buf_len - ret - ssl_tlv_len),
                       PP2_SUBTYPE_SSL_CN, cn_trash->data, cn_trash->area);
        }
      }
      if (srv->pp_opts & SRV_PP_V2_SSL_KEY_ALG) {
        ...
        if (ssl_sock_get_pkey_algo(remote, pkey_trash) > 0) {
          ssl_tlv_len += make_tlv(
              &buf[ret + ssl_tlv_len], (buf_len - ret - ssl_tlv_len),
              PP2_SUBTYPE_SSL_KEY_ALG, pkey_trash->data, pkey_trash->area);
        }
      }
      if (srv->pp_opts & SRV_PP_V2_SSL_SIG_ALG) {
        ...
        if (value) {
          ssl_tlv_len +=
              make_tlv(&buf[ret + ssl_tlv_len], (buf_len - ret - ssl_tlv_len),
                       PP2_SUBTYPE_SSL_SIG_ALG, strlen(value), value);
        }
      }
      if (srv->pp_opts & SRV_PP_V2_SSL_CIPHER) {
        ...
        if (value) {
          ssl_tlv_len +=
              make_tlv(&buf[ret + ssl_tlv_len], (buf_len - ret - ssl_tlv_len),
                       PP2_SUBTYPE_SSL_CIPHER, strlen(value), value);
        }
      }
    }
  ...
  if (remote && (remote->proxy_netns)) {
    if ((buf_len - ret) < sizeof(struct tlv))
      return 0;
    ret +=
        make_tlv(&buf[ret], (buf_len - ret), PP2_TYPE_NETNS,
...
static int cfg_parse_pp2_never_send_local(char **args, int section_type,
                                          struct proxy *curpx,
 ...
 */
static int smp_fetch_fc_http_major(const struct arg *args, struct sample *smp,
  ...
  smp->data.u.sint =
      (conn && strcmp(conn_get_mux_name(conn), "H2") == 0) ? 2 : 1;
  return 1;
}

/* fetch if the received connection used a PROXY protocol header */
int smp_fetch_fc_rcvd_proxy(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  struct connection *conn;
