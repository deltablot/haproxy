...
const char *smp_to_type[SMP_TYPES] = {
    [SMP_T_ANY] = "any",   [SMP_T_BOOL] = "bool", [SMP_T_SINT] = "sint",
    [SMP_T_ADDR] = "addr", [SMP_T_IPV4] = "ipv4", [SMP_T_IPV6] = "ipv6",
    [SMP_T_STR] = "str",   [SMP_T_BIN] = "bin",   [SMP_T_METH] = "meth",
};
...
const unsigned int fetch_cap[SMP_SRC_ENTRIES] = {
    ...
    [SMP_SRC_L5CLI] =
        (SMP_VAL___________ | SMP_VAL_FE_SES_ACC | SMP_VAL_FE_REQ_CNT |
         SMP_VAL_FE_HRQ_HDR | SMP_VAL_FE_HRQ_BDY | SMP_VAL_FE_SET_BCK |
         SMP_VAL_BE_REQ_CNT | SMP_VAL_BE_HRQ_HDR | SMP_VAL_BE_HRQ_BDY |
         SMP_VAL_BE_SET_SRV | SMP_VAL_BE_SRV_CON | SMP_VAL_BE_RES_CNT |
         SMP_VAL_BE_HRS_HDR | SMP_VAL_BE_HRS_BDY | SMP_VAL_BE_STO_RUL |
    ...
    [SMP_SRC_L6REQ] =
        (SMP_VAL___________ | SMP_VAL___________ | SMP_VAL_FE_REQ_CNT |
         SMP_VAL_FE_HRQ_HDR | SMP_VAL_FE_HRQ_BDY | SMP_VAL_FE_SET_BCK |
         SMP_VAL_BE_REQ_CNT | SMP_VAL_BE_HRQ_HDR | SMP_VAL_BE_HRQ_BDY |
 ...
 * initialized.
 */
inline int smp_expr_output_type(struct sample_expr *expr) {
...
const char *sample_src_names(unsigned int use) {
  ...
  for (bit = 0; bit < SMP_SRC_ENTRIES; bit++) {
    ...
    trash.data +=
        snprintf(trash.area + trash.data, trash.size - trash.data, "%s%s",
                 (use & ((1 << bit) - 1)) ? "," : "", fetch_src_names[bit]);
  }
  return trash.area;
}
...
/*
 * Registers the sample format coverstion keyword list <pckl> as a list of valid
 * keywords for next parsing sessions.
 */
 ...
 * the list is reached.
 */
struct sample_fetch *sample_fetch_getnext(struct sample_fetch *current,
                                          int *idx) {
  struct sample_fetch_kw_list *kwl;
...
/*
 * Returns the pointer on sample format conversion keyword structure identified
 * by string of <len> in buffer <kw>.
 *
 */
...
int smp_dup(struct sample *smp) {
  ...
  case SMP_T_STR:
    ...
    trash->data = smp->data.type == SMP_T_STR ? smp->data.u.str.data
                                              : smp->data.u.meth.str.data;
    if (trash->data > trash->size - 1)
      trash->data = trash->size - 1;

    memcpy(trash->area,
           smp->data.type == SMP_T_STR ? smp->data.u.str.area
...
int c_none(struct sample *smp) { return 1; }
...
sample_cast_fct sample_casts[SMP_TYPES][SMP_TYPES] = {
    /*            to:  ANY     BOOL       SINT       ADDR        IPV4      IPV6
       STR         BIN         METH */
    /* from:  ANY */ {
        c_none,
        c_none,
 ...
 */
struct sample_expr *sample_parse_expr(char **str, int *idx, const char *file,
                                      int line, char **err_msg,
                                      struct arg_list *al, char **endptr) {
  ...
  al->kw = expr->fetch->kw;
  al->conv = NULL;
  if (make_arg_list(endw, -1, fetch->arg_mask, &expr->arg_p, err_msg, &endt,
                    &err_arg, al) < 0) {
  ...
  while (1) {
    ...
    argcnt = make_arg_list(endw, -1, conv->arg_mask, &conv_expr->arg_p, err_msg,
                           &endt, &err_arg, al);
    if (argcnt < 0) {
      memprintf(err_msg, "invalid arg %d in converter '%s' : %s", err_arg + 1,
                ckw, *err_msg);
      goto out_error;
    }
    ...
    if (conv->val_args &&
        !conv->val_args(conv_expr->arg_p, conv, file, line, err_msg)) {
...
/*
 * Process a fetch + format conversion of defined by the sample expression
 * <expr> on request or response considering the <opt> parameter. Returns a
 * pointer on a typed sample structure containing the result or NULL if sample
 * is not found or when format conversion failed. If <p> is not null, function
 * returns results in structure pointed by <p>. If <p> is null, functions
 * returns a pointer on a static sample structure.
 *
...
struct sample *sample_process(struct proxy *px, struct session *sess,
  ...
  if (!expr->fetch->process(expr->arg_p, p, expr->fetch->kw,
                            expr->fetch->private))
    return NULL;
...
int smp_resolve_args(struct proxy *p) {
  ...
  list_for_each_entry_safe(cur, bak, &p->conf.args.list, list) {
    ...
    switch (cur->ctx) {
    case ARGC_STK:
      where = "in stick rule in";
      break;
    case ARGC_TRK:
      where = "in tracking rule in";
      break;
    case ARGC_LOG:
      where = "in log-format string in";
      break;
    case ARGC_LOGSD:
      where = "in log-format-sd string in";
      break;
    case ARGC_HRQ:
      where = "in http-request header format string in";
      break;
    case ARGC_HRS:
      where = "in http-response header format string in";
      break;
    case ARGC_UIF:
      where = "in unique-id-format string in";
      break;
    case ARGC_RDR:
      where = "in redirect format string in";
      break;
    case ARGC_CAP:
      where = "in capture rule in";
      break;
    case ARGC_ACL:
      ctx = "ACL keyword";
      break;
    case ARGC_SRV:
      where = "in server directive in";
      break;
    case ARGC_SPOE:
      where = "in spoe-message directive in";
      break;
    case ARGC_HERR:
      where = "in http-error directive in";
      break;
    case ARGC_OT:
      where = "in ot-scope directive in";
      break;
    }
    ...
    case ARGT_SRV:
      if (!arg->data.str.data) {
        ha_alert("parsing [%s:%d] : missing server name in arg %d of %s%s%s%s "
                 "'%s' %s proxy '%s'.\n",
                 cur->file, cur->line, cur->arg_pos + 1, conv_pre, conv_ctx,
      ...
      if (sname) {
        *sname++ = '\0';
        ...
        if (!px) {
          ha_alert("parsing [%s:%d] : unable to find proxy '%s' referenced in "
                   "arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                   cur->file, cur->line, pname, cur->arg_pos + 1, conv_pre,
                   conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
          cfgerr++;
          break;
        }
      } else
      ...
      if (!srv) {
        ha_alert("parsing [%s:%d] : unable to find server '%s' in proxy '%s', "
                 "referenced in arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, sname, pname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
      }
    ...
    case ARGT_FE:
      ...
      if (!px) {
        ha_alert("parsing [%s:%d] : unable to find frontend '%s' referenced in "
                 "arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, pname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
      ...
      if (!(px->cap & PR_CAP_FE)) {
        ha_alert("parsing [%s:%d] : proxy '%s', referenced in arg %d of "
                 "%s%s%s%s '%s' %s proxy '%s', has not frontend capability.\n",
                 cur->file, cur->line, pname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
    ...
    case ARGT_BE:
      ...
      if (!px) {
        ha_alert("parsing [%s:%d] : unable to find backend '%s' referenced in "
                 "arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, pname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
      ...
      if (!(px->cap & PR_CAP_BE)) {
        ha_alert("parsing [%s:%d] : proxy '%s', referenced in arg %d of "
                 "%s%s%s%s '%s' %s proxy '%s', has not backend capability.\n",
                 cur->file, cur->line, pname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
    ...
    case ARGT_TAB:
      ...
      if (!t) {
        ha_alert("parsing [%s:%d] : unable to find table '%s' referenced in "
                 "arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, stktname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
      }
      ...
      if (!t->size) {
        ha_alert("parsing [%s:%d] : no table in proxy '%s' referenced in arg "
                 "%d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, stktname, cur->arg_pos + 1, conv_pre,
                 conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
      ...
      if (t->proxy && (p->bind_proc & ~t->proxy->bind_proc)) {
        ha_alert("parsing [%s:%d] : stick-table '%s' not present on all "
                 "processes covered by proxy '%s'.\n",
    ...
    case ARGT_USR:
      if (!arg->data.str.data) {
        ha_alert("parsing [%s:%d] : missing userlist name in arg %d of "
                 "%s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, cur->arg_pos + 1, conv_pre, conv_ctx,
                 conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
      ...
      if (!ul) {
        ha_alert("parsing [%s:%d] : unable to find userlist '%s' referenced in "
                 "arg %d of %s%s%s%s '%s' %s proxy '%s'.\n",
                 cur->file, cur->line, arg->data.str.area, cur->arg_pos + 1,
                 conv_pre, conv_ctx, conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
        break;
      }
    ...
    case ARGT_REG:
      if (!arg->data.str.data) {
        ha_alert("parsing [%s:%d] : missing regex in arg %d of %s%s%s%s '%s' "
                 "%s proxy '%s'.\n",
                 cur->file, cur->line, cur->arg_pos + 1, conv_pre, conv_ctx,
                 conv_pos, ctx, cur->kw, where, p->id);
        cfgerr++;
      ...
      if (!(reg = regex_comp(arg->data.str.area, !(rflags & REG_ICASE),
                             1 /* capture substr */, &err))) {
        ha_alert("parsing [%s:%d] : error in regex '%s' in arg %d of %s%s%s%s "
                 "'%s' %s proxy '%s' : %s.\n",
                 cur->file, cur->line, arg->data.str.area, cur->arg_pos + 1,
                 conv_pre, conv_ctx, conv_pos, ctx, cur->kw, where, p->id, err);
        cfgerr++;
        continue;
      }
...
static int sample_conv_debug(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  int i;
  struct sample tmp;
  struct buffer *buf;
...
static int sample_conv_base642bin(const struct arg *arg_p, struct sample *smp,
  ...
  trash->data = 0;
  bin_len = base64dec(smp->data.u.str.area, smp->data.u.str.data, trash->area,
                      trash->size);
  if (bin_len < 0)
    return 0;
...
static int sample_conv_bin2base64(const struct arg *arg_p, struct sample *smp,
  ...
  trash->data = 0;
  b64_len = a2base64(smp->data.u.str.area, smp->data.u.str.data, trash->area,
                     trash->size);
  if (b64_len < 0)
    return 0;
...
static int sample_conv_sha1(const struct arg *arg_p, struct sample *smp,
                            void *private) {
  blk_SHA_CTX ctx;
  struct buffer *trash = get_trash_chunk();
...
static int sample_conv_sha2(const struct arg *arg_p, struct sample *smp,
                            void *private) {
  struct buffer *trash = get_trash_chunk();
  int bits = 256;
...
static inline int sample_conv_var2smp_str(const struct arg *arg,
                                          struct sample *smp) {
  switch (arg->type) {
  case ARGT_STR:
    smp->data.type = SMP_T_STR;
 ...
 * base64 encoded */
static int sample_conv_aes_gcm_dec(const struct arg *arg_p, struct sample *smp,
                                   void *private) {
  ...
  /* Initialise key */
  if (!EVP_DecryptInit_ex(ctx, NULL, NULL, (unsigned char *)smp_trash->area,
                          NULL))
    goto err;

  if (!EVP_DecryptUpdate(
          ctx, (unsigned char *)smp_trash->area, (int *)&smp_trash->data,
          (unsigned char *)smp->data.u.str.area, (int)smp->data.u.str.data))
    goto err;

  dec_size = base64dec(aead_tag.data.u.str.area, aead_tag.data.u.str.data,
                       smp_trash_alloc->area, smp_trash_alloc->size);
  if (dec_size < 0)
    goto err;
  ...
  ret = EVP_DecryptFinal_ex(ctx,
                            (unsigned char *)smp_trash->area + smp_trash->data,
                            (int *)&smp_trash->data);
...
static int sample_conv_crypto_digest(const struct arg *args, struct sample *smp,
                                     void *private) {
  struct buffer *trash = get_trash_chunk();
...
static int sample_conv_crypto_hmac(const struct arg *args, struct sample *smp,
  ...
  dec_size = base64dec(key.data.u.str.area, key.data.u.str.data,
                       key_trash->area, key_trash->size);
  if (dec_size < 0)
    goto err;

  md = (unsigned char *)trash->area;
  md_len = trash->size;
  if (!HMAC(evp, key_trash->area, dec_size,
            (const unsigned char *)smp->data.u.str.area, smp->data.u.str.data,
            md, &md_len))
    goto err;
...
static int sample_conv_djb2(const struct arg *arg_p, struct sample *smp,
                            void *private) {
  smp->data.u.sint = hash_djb2(smp->data.u.str.area, smp->data.u.str.data);
  if (arg_p && arg_p->data.sint)
...
static int sample_conv_length(const struct arg *arg_p, struct sample *smp,
                              void *private) {
...
static int sample_conv_ipmask(const struct arg *args, struct sample *smp,
                              void *private) {
  ...
  if (smp->data.type == SMP_T_IPV4) {
    ...
    write_u64(&smp->data.u.ipv6.s6_addr[0],
              read_u64(&smp->data.u.ipv6.s6_addr[0]) &
                  read_u64(&args[1].data.ipv6.s6_addr[0]));
    write_u64(&smp->data.u.ipv6.s6_addr[8],
              read_u64(&smp->data.u.ipv6.s6_addr[8]) &
                  read_u64(&args[1].data.ipv6.s6_addr[8]));
    smp->data.type = SMP_T_IPV6;
  }
...
static int sample_conv_ltime(const struct arg *args, struct sample *smp,
                             void *private) {
  struct buffer *temp;
  /* With high numbers, the date returned can be negative, the 55 bits mask
   * prevent this. */
...
static int sample_conv_sdbm(const struct arg *arg_p, struct sample *smp,
                            void *private) {
  smp->data.u.sint = hash_sdbm(smp->data.u.str.area, smp->data.u.str.data);
  if (arg_p && arg_p->data.sint)
    smp->data.u.sint = full_hash(smp->data.u.sint);
  smp->data.type = SMP_T_SINT;
  return 1;
...
static int sample_conv_wt6(const struct arg *arg_p, struct sample *smp,
                           void *private) {
  smp->data.u.sint = hash_wt6(smp->data.u.str.area, smp->data.u.str.data);
  if (arg_p && arg_p->data.sint)
    smp->data.u.sint = full_hash(smp->data.u.sint);
  smp->data.type = SMP_T_SINT;
  return 1;
 ...
 */
static int sample_conv_xxh32(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  unsigned int seed;
  ...
  if (arg_p && arg_p->data.sint)
    ...
    seed = 0;
  smp->data.u.sint = XXH32(smp->data.u.str.area, smp->data.u.str.data, seed);
  smp->data.type = SMP_T_SINT;
  return 1;
}
 ...
 * The seed of the hash defaults to 0 but can be changd in argument 1.
 */
static int sample_conv_xxh64(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  ...
  if (arg_p && arg_p->data.sint)
    ...
    seed = 0;
  smp->data.u.sint =
      (long long int)XXH64(smp->data.u.str.area, smp->data.u.str.data, seed);
  smp->data.type = SMP_T_SINT;
  return 1;
}
...
static int sample_conv_crc32(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  smp->data.u.sint = hash_crc32(smp->data.u.str.area, smp->data.u.str.data);
  if (arg_p && arg_p->data.sint)
    smp->data.u.sint = full_hash(smp->data.u.sint);
  smp->data.type = SMP_T_SINT;
  return 1;
...
static int sample_conv_crc32c(const struct arg *arg_p, struct sample *smp,
                              void *private) {
  smp->data.u.sint = hash_crc32c(smp->data.u.str.area, smp->data.u.str.data);
  if (arg_p && arg_p->data.sint)
    smp->data.u.sint = full_hash(smp->data.u.sint);
  smp->data.type = SMP_T_SINT;
  return 1;
...
static int sample_conv_json(const struct arg *arg_p, struct sample *smp,
  ...
  while (p < smp->data.u.str.area + smp->data.u.str.data) {
    ...
    if (input_type == IT_ASCII) {
      ...
      /* Read input as UTF8. */
      ret = utf8_next(p, smp->data.u.str.data - (p - smp->data.u.str.area), &c);
      ...
      if (input_type == IT_UTF8S && utf8_return_code(ret) != UTF8_CODE_OK)
        continue;
      if (input_type == IT_UTF8P &&
          utf8_return_code(ret) & (UTF8_CODE_INVRANGE | UTF8_CODE_BADSEQ))
        return 0;
      if (input_type == IT_UTF8PS &&
          utf8_return_code(ret) & (UTF8_CODE_INVRANGE | UTF8_CODE_BADSEQ))
        continue;
...
static int sample_conv_bytes(const struct arg *arg_p, struct sample *smp,
  ...
  if ((arg_p[1].type == ARGT_SINT) &&
      (arg_p[1].data.sint < smp->data.u.str.data))
...
/* This sample function is designed to a return selected part of a string
 * (field). First arg is the index of the field (start at 1) Second arg is a
 * char list of separators (type string)
 */
...
static int sample_conv_regsub_check(struct arg *args, struct sample_conv *conv,
  ...
  while (len) {
    if (*p == 'i') {
      ...
      memprintf(err, "invalid regex flag '%c', only 'i' and 'g' are supported",
                *p);
      return 0;
    }
    p++;
...
static int sample_conv_regsub(const struct arg *arg_p, struct sample *smp,
  ...
  while (1) {
    ...
    if ((arg_p[0].type_flags & ARGF_REG_GLOB) || !(flag & REG_NOTBOL)) {
      ...
      found =
          regex_exec_match2(reg, start, end - start, MAX_MATCH, pmatch, flag);
    }

    if (!found)
      pmatch[0].rm_so = end - start;

    /* copy the heading non-matching part (which may also be the tail if nothing
     * matches) */
 ...
 */
static inline int sample_conv_var2smp(const struct arg *arg,
                                      struct sample *smp) {
  switch (arg->type) {
  case ARGT_SINT:
    smp->data.type = SMP_T_SINT;
 ...
 */
static int sample_conv_concat(const struct arg *arg_p, struct sample *smp,
                              void *private) {
  struct buffer *trash;
  struct sample tmp;
  int max;
  ...
  if (arg_p[1].type == ARGT_VAR && vars_get_by_desc(&arg_p[1].data.var, &tmp) &&
    ...
    if (max) {
      memcpy(trash->area + trash->data, tmp.data.u.str.area, max);
      trash->data += max;
      trash->area[trash->data] = 0;
    }
  }
 ...
 */
static int sample_conv_secure_memcmp(const struct arg *arg_p,
...
/* Takes a boolean as input. Returns the first argument if that boolean is true
 * and the second argument otherwise.
 */
static int sample_conv_iif(const struct arg *arg_p, struct sample *smp,
                           void *private) {
  smp->data.type = SMP_T_STR;
  smp->flags |= SMP_F_CONST;
...
/*
 * Extract the field value of an input binary sample. Takes a mandatory
 * argument: the protocol buffers field identifier (dotted notation) internally
 * represented as an array of unsigned integers and its size. Return 1 if the
 * field was found, 0 if not.
 */
static int sample_conv_ungrpc(const struct arg *arg_p, struct sample *smp,
                              void *private) {
  unsigned char *pos;
  size_t grpc_left;
...
static int sample_conv_protobuf(const struct arg *arg_p, struct sample *smp,
                                void *private) {
  unsigned char *pos;
  size_t left;
...
static int sample_conv_protobuf_check(struct arg *args,
                                      struct sample_conv *conv,
                                      const char *file, int line, char **err) {
  if (!args[1].type) {
 ...
 */
static int sample_conv_fix_tag_value(const struct arg *arg_p,
                                     struct sample *smp, void *private) {
  struct ist value;
 ...
 */
static int sample_conv_fix_value_check(struct arg *args,
                                       struct sample_conv *conv,
                                       const char *file, int line, char **err) {
  struct ist str;
  unsigned int tag;
 ...
 */
static int sample_conv_fix_is_valid(const struct arg *arg_p, struct sample *smp,
                                    void *private) {
  struct ist msg;
 ...
 */
static int sample_conv_mqtt_field_value(const struct arg *arg_p,
                                        struct sample *smp, void *private) {
 ...
 *
 * Args[0] will be turned into a MQTT_CPT_* value for direct maching when
 * parsing a packet.
 */
static int sample_conv_mqtt_field_value_check(struct arg *args,
                                              struct sample_conv *conv,
  ...
  fieldname_id = mqtt_check_type_fieldname(
      type, ist2(args[1].data.str.area, args[1].data.str.data));
  if (fieldname_id == MQTT_FN_INVALID) {
    memprintf(err, "Unknown MQTT field name '%s' for packet type '%s'",
              args[1].data.str.area, args[0].data.str.area);
    return 0;
  }
 ...
 */
static int sample_conv_mqtt_is_valid(const struct arg *arg_p,
                                     struct sample *smp, void *private) {
  struct ist msg;
 ...
 * the variable name and its scope.
 */
...
/**/
static int sample_conv_htonl(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  struct buffer *tmp;
  uint32_t n;
...
/**/
static int sample_conv_cut_crlf(const struct arg *arg_p, struct sample *smp,
                                void *private) {
  char *p;
  size_t l;
...
/**/
static int sample_conv_ltrim(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  char *delimiters, *p;
  size_t dlen, l;
...
/**/
static int sample_conv_rtrim(const struct arg *arg_p, struct sample *smp,
                             void *private) {
  char *delimiters, *p;
  size_t dlen, l;
...
static int smp_fetch_true(const struct arg *args, struct sample *smp,
...
static int smp_fetch_false(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
  smp->data.type = SMP_T_BOOL;
  smp->data.u.sint = 0;
  return 1;
...
static int smp_fetch_env(const struct arg *args, struct sample *smp,
                         const char *kw, void *private) {
  char *env;
 ...
 * an optional string representing the unit of the result: "s" for seconds, "ms"
 * for milliseconds and "us" for microseconds. Returns 0 on error and non-zero
 * if OK.
 */
 ...
 */
static int smp_fetch_date(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  ...
  else if (args && args[1].type == ARGT_SINT &&
...
static int smp_fetch_date_us(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
  smp->data.u.sint = date.tv_usec;
  smp->data.type = SMP_T_SINT;
  smp->flags |= SMP_F_VOL_TEST | SMP_F_MAY_CHANGE;
...
static int smp_fetch_hostname(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
  smp->data.type = SMP_T_STR;
  smp->flags = SMP_F_CONST;
...
static int smp_fetch_nbproc(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  smp->data.type = SMP_T_SINT;
...
static int smp_fetch_proc(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
  smp->data.type = SMP_T_SINT;
...
static int smp_fetch_thread(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = tid;
  return 1;
 ...
 */
static int smp_fetch_rand(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
...
static int smp_fetch_stopping(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
  smp->data.type = SMP_T_BOOL;
...
static int smp_fetch_cpu_calls(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  if (!smp->strm)
    return 0;
...
/* returns the average number of nanoseconds spent processing the stream per
 * call */
static int smp_fetch_cpu_ns_avg(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  if (!smp->strm)
    return 0;
  ...
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = smp->strm->task->calls
                         ? smp->strm->task->cpu_time / smp->strm->task->calls
                         : 0;
  return 1;
}

/* returns the total number of nanoseconds spent processing the stream */
static int smp_fetch_cpu_ns_tot(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  if (!smp->strm)
    return 0;
...
/* returns the average number of nanoseconds per call spent waiting for other
 * tasks to be processed */
static int smp_fetch_lat_ns_avg(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  if (!smp->strm)
    return 0;
  ...
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = smp->strm->task->calls
                         ? smp->strm->task->lat_time / smp->strm->task->calls
                         : 0;
  return 1;
}

/* returns the total number of nanoseconds per call spent waiting for other
 * tasks to be processed */
static int smp_fetch_lat_ns_tot(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  if (!smp->strm)
    return 0;
...
static int smp_fetch_const_str(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  smp->flags |= SMP_F_CONST;
  smp->data.type = SMP_T_STR;
...
static int smp_fetch_const_bool(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  smp->data.type = SMP_T_BOOL;
  smp->data.u.sint = args[0].data.sint;
  return 1;
...
static int smp_fetch_const_int(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = args[0].data.sint;
  return 1;
...
static int smp_fetch_const_ipv4(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  smp->data.type = SMP_T_IPV4;
...
static int smp_fetch_const_ipv6(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  smp->data.type = SMP_T_IPV6;
...
static int smp_fetch_const_bin(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  smp->flags |= SMP_F_CONST;
  smp->data.type = SMP_T_BIN;
  smp->data.u.str.area = args[0].data.str.area;
...
static int smp_fetch_const_meth(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  smp->data.type = SMP_T_METH;
  if (args[0].type == ARGT_SINT) {
    smp->flags &= ~SMP_F_CONST;
...
static int smp_fetch_uuid(const struct arg *args, struct sample *smp,
                          const char *kw, void *private) {
...
static struct sample_fetch_kw_list smp_kws = {
    ILH,
    {
        ...
        {"date", smp_fetch_date, ARG2(0, SINT, STR), smp_check_date_unit,
        ...
        {"stopping", smp_fetch_stopping, 0, NULL, SMP_T_BOOL, SMP_USE_INTRN},
        {"uuid", smp_fetch_uuid, ARG1(0, SINT), smp_check_uuid, SMP_T_STR,
         SMP_USE_INTRN},

        {"cpu_calls", smp_fetch_cpu_calls, 0, NULL, SMP_T_SINT, SMP_USE_INTRN},
        {"cpu_ns_avg", smp_fetch_cpu_ns_avg, 0, NULL, SMP_T_SINT,
         SMP_USE_INTRN},
        {"cpu_ns_tot", smp_fetch_cpu_ns_tot, 0, NULL, SMP_T_SINT,
         SMP_USE_INTRN},
        {"lat_ns_avg", smp_fetch_lat_ns_avg, 0, NULL, SMP_T_SINT,
         SMP_USE_INTRN},
        {"lat_ns_tot", smp_fetch_lat_ns_tot, 0, NULL, SMP_T_SINT,
         SMP_USE_INTRN},

        {"str", smp_fetch_const_str, ARG1(1, STR), NULL, SMP_T_STR,
         SMP_USE_INTRN},
        {"bool", smp_fetch_const_bool, ARG1(1, STR), smp_check_const_bool,
         SMP_T_BOOL, SMP_USE_INTRN},
        {"int", smp_fetch_const_int, ARG1(1, SINT), NULL, SMP_T_SINT,
         SMP_USE_INTRN},
        {"ipv4", smp_fetch_const_ipv4, ARG1(1, IPV4), NULL, SMP_T_IPV4,
         SMP_USE_INTRN},
        {"ipv6", smp_fetch_const_ipv6, ARG1(1, IPV6), NULL, SMP_T_IPV6,
         SMP_USE_INTRN},
...
static struct sample_conv_kw_list sample_conv_kws = {
    ILH,
    {
        {"debug", sample_conv_debug, ARG2(0, STR, STR), smp_check_debug,
         SMP_T_ANY, SMP_T_ANY},
        ...
        {"crc32", sample_conv_crc32, ARG1(0, SINT), NULL, SMP_T_BIN,
         SMP_T_SINT},
        {"crc32c", sample_conv_crc32c, ARG1(0, SINT), NULL, SMP_T_BIN,
         SMP_T_SINT},
        ...
        {"xxh64", sample_conv_xxh64, ARG1(0, SINT), NULL, SMP_T_BIN,
         SMP_T_SINT},
        {"json", sample_conv_json, ARG1(1, STR), sample_conv_json_check,
         SMP_T_STR, SMP_T_STR},
...
#ifdef USE_OPENSSL
        {"sha2", sample_conv_sha2, ARG1(0, SINT), smp_check_sha2, SMP_T_BIN,
         SMP_T_BIN},
#if (HA_OPENSSL_VERSION_NUMBER >= 0x1000100fL)
        {"aes_gcm_dec", sample_conv_aes_gcm_dec, ARG4(4, SINT, STR, STR, STR),
         check_aes_gcm, SMP_T_BIN, SMP_T_BIN},
#endif
        {"digest", sample_conv_crypto_digest, ARG1(1, STR), check_crypto_digest,
         SMP_T_BIN, SMP_T_BIN},
        {"hmac", sample_conv_crypto_hmac, ARG2(2, STR, STR), check_crypto_hmac,
         SMP_T_BIN, SMP_T_BIN},
#endif
        {"concat", sample_conv_concat, ARG3(1, STR, STR, STR), smp_check_concat,
         SMP_T_STR, SMP_T_STR},
        ...
        {"protobuf", sample_conv_protobuf, ARG2(1, PBUF_FNUM, STR),
         sample_conv_protobuf_check, SMP_T_BIN, SMP_T_BIN},

        /* FIX converters */
        {"fix_is_valid", sample_conv_fix_is_valid, 0, NULL, SMP_T_BIN,
         SMP_T_BOOL},
        {"fix_tag_value", sample_conv_fix_tag_value, ARG1(1, STR),
         sample_conv_fix_value_check, SMP_T_BIN, SMP_T_BIN},

        /* MQTT converters */
        {"mqtt_is_valid", sample_conv_mqtt_is_valid, 0, NULL, SMP_T_BIN,
         SMP_T_BOOL},
        {"mqtt_field_value", sample_conv_mqtt_field_value, ARG2(2, STR, STR),
         sample_conv_mqtt_field_value_check, SMP_T_BIN, SMP_T_STR},

        {"iif", sample_conv_iif, ARG2(2, STR, STR), NULL, SMP_T_BOOL,
         SMP_T_STR},

        {"and", sample_conv_binary_and, ARG1(1, STR), check_operator,
         SMP_T_SINT, SMP_T_SINT},
        {"or", sample_conv_binary_or, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
        ...
        {"add", sample_conv_arith_add, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
        {"sub", sample_conv_arith_sub, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
        {"mul", sample_conv_arith_mul, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
        {"div", sample_conv_arith_div, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
        {"mod", sample_conv_arith_mod, ARG1(1, STR), check_operator, SMP_T_SINT,
         SMP_T_SINT},
