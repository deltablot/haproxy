...
DECLARE_STATIC_POOL(xprt_handshake_ctx_pool, "xprt_handshake_ctx_pool",
                    sizeof(struct xprt_handshake_ctx));

/* This XPRT doesn't take care of sending or receiving data, once its handshake
 * is done, it just removes itself
 */
static size_t xprt_handshake_from_buf(struct connection *conn, void *xprt_ctx,
                                      const struct buffer *buf, size_t count,
                                      int flags) {
  return 0;
}

static size_t xprt_handshake_to_buf(struct connection *conn, void *xprt_ctx,
                                    struct buffer *buf, size_t count,
                                    int flags) {
  return 0;
}

static struct task *xprt_handshake_io_cb(struct task *t, void *bctx,
...
out:
   ...
   * connection error
   * */
  if ((conn->flags & CO_FL_ERROR) || !(conn->flags & CO_FL_HANDSHAKE)) {
    int ret = 0;
    int woke = 0;
    int was_conn_ctx = 0;
    ...
    if (conn->xprt_ctx == ctx) {
      ...
      conn->xprt->remove_xprt(conn, conn->xprt_ctx, ctx, ctx->xprt,
                              ctx->xprt_ctx);
 ...
 */
static int xprt_handshake_subscribe(struct connection *conn, void *xprt_ctx,
 ...
 */
static int xprt_handshake_unsubscribe(struct connection *conn, void *xprt_ctx,
                                      int event_type, struct wait_event *es) {
  struct xprt_handshake_ctx *ctx = xprt_ctx;
 ...
 */
static int xprt_handshake_add_xprt(struct connection *conn, void *xprt_ctx,
                                   void *toadd_ctx,
                                   const struct xprt_ops *toadd_ops,
                                   void **oldxprt_ctx,
 ...
 * XPRT.
 */
static int xprt_handshake_remove_xprt(struct connection *conn, void *xprt_ctx,
                                      void *toremove_ctx,
                                      const struct xprt_ops *newops,
                                      void *newctx) {
  struct xprt_handshake_ctx *ctx = xprt_ctx;
...
__attribute__((constructor)) static void __xprt_handshake_init(void) {
