 ...
 *
 * Copyright (C) 2018 HAProxy Technologies, Christopher Faulet
 * <cfaulet@haproxy.com>
 *
...
static void http_capture_headers(struct htx *htx, char **cap,
                                 struct cap_hdr *cap_hdr);
...
static void http_debug_stline(const char *dir, struct stream *s,
                              const struct htx_sl *sl);
static void http_debug_hdr(const char *dir, struct stream *s,
                           const struct ist n, const struct ist v);

static enum rule_result http_req_get_intercept_rule(struct proxy *px,
                                                    struct list *rules,
                                                    struct stream *s);
static enum rule_result http_res_get_intercept_rule(struct proxy *px,
                                                    struct list *rules,
                                                    struct stream *s);

static void http_manage_client_side_cookies(struct stream *s,
                                            struct channel *req);
static void http_manage_server_side_cookies(struct stream *s,
                                            struct channel *res);

static int http_stats_check_uri(struct stream *s, struct http_txn *txn,
                                struct proxy *backend);
static int http_handle_stats(struct stream *s, struct channel *req);

static int http_handle_expect_hdr(struct stream *s, struct htx *htx,
                                  struct http_msg *msg);
...
int http_wait_for_request(struct stream *s, struct channel *req, int an_bit) {
  ...
  stream_inc_http_req_ctr(s);
  proxy_inc_fe_req_ctr(sess->listener,
                       sess->fe); /* one more valid request for this FE */
  ...
  if (unlikely((global.mode & MODE_DEBUG) &&
    ...
    for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
      ...
      http_debug_hdr("clihdr", s, htx_get_blk_name(htx, blk),
  ...
  if (txn->meth == HTTP_METH_GET || txn->meth == HTTP_METH_HEAD)
    s->flags |= SF_REDIRECTABLE;
           ...
           isteqi(htx_sl_req_meth(sl), ist("PRI"))) {
   ...
   */
  if (unlikely(
          (sess->fe->monitor_uri_len != 0) &&
          ((*sess->fe->monitor_uri == '/' &&
            isteq(http_get_path(htx_sl_req_uri(sl)),
                  ist2(sess->fe->monitor_uri, sess->fe->monitor_uri_len))) ||
           isteq(htx_sl_req_uri(sl),
                 ist2(sess->fe->monitor_uri, sess->fe->monitor_uri_len))))) {
    /*
    ...
    list_for_each_entry(cond, &sess->fe->mon_fail_cond, list) {
      int ret = acl_exec_cond(cond, sess->fe, sess, s,
   ...
   * CONNECT ip:port.
   */
  if ((sess->fe->options2 & PR_O2_USE_PXHDR) && *HTX_SL_REQ_UPTR(sl) != '/' &&
      *HTX_SL_REQ_UPTR(sl) != '*')
   ...
   */
  if (likely(txn->meth == HTTP_METH_GET || txn->meth == HTTP_METH_HEAD ||
             txn->meth == HTTP_METH_OPTIONS || txn->meth == HTTP_METH_TRACE))
...
return_prx_cond:
  ...
  DBG_TRACE_DEVEL("leaving on error",
                  STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA | STRM_EV_HTTP_ERR, s,
                  txn);
  return 0;
}
 ...
 * error, ...).
 */
int http_process_req_common(struct stream *s, struct channel *req, int an_bit,
                            struct proxy *px) {
  struct session *sess = s->sess;
  struct http_txn *txn = s->txn;
  struct http_msg *msg = &txn->req;
  ...
  if (!LIST_ISEMPTY(&px->http_req_rules)) {
    ...
    case HTTP_RULE_RES_DONE: /* OK, but terminate request processing (eg:
                                redirect) */
      goto done;
  ...
  if (!s->target && http_stats_check_uri(s, txn, px)) {
    ...
    if (verdict ==
        HTTP_RULE_RES_ABRT) /* stats auth / stats http-request auth */
      goto return_prx_cond;
  ...
  if (unlikely(objt_applet(s->target))) {
    ...
    /* enable the minimally required analyzers to handle keep-alive and
     * compression on the HTTP response */
    req->analysers &=
  ...
  req->analyse_exp = TICK_ETERNITY;
done_without_exp
    : /* done with this analyser, but don't reset the analyse_exp. */
  req->analysers &= ~an_bit;
  DBG_TRACE_LEAVE(STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA, s, txn);
  return 1;
...
return_prx_yield:
  channel_dont_connect(req);
  DBG_TRACE_DEVEL("waiting for more data", STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA,
                  s, txn);
  return 0;
}
...
int http_process_request(struct stream *s, struct channel *req, int an_bit) {
   ...
   *
   * A unique ID is generated even when it is not sent to ensure that the ID can
   * make use of fetches only available in the HTTP request processing stage.
   */
  ...
  if ((sess->fe->options | s->be->options) & PR_O_FWDFOR) {
    ...
    struct ist hdr = ist2(s->be->fwdfor_hdr_len ? s->be->fwdfor_hdr_name
                                                : sess->fe->fwdfor_hdr_name,
                          s->be->fwdfor_hdr_len ? s->be->fwdfor_hdr_len
                                                : sess->fe->fwdfor_hdr_len);
    ...
    if (!((sess->fe->options | s->be->options) & PR_O_FF_ALWAYS) &&
       ...
       */
    } else if (cli_conn && conn_get_src(cli_conn) &&
               cli_conn->src->ss_family == AF_INET) {
      /* Add an X-Forwarded-For header unless the source IP is
       * in the 'except' network range.
       */
      if ((!sess->fe->except_mask.s_addr ||
           (((struct sockaddr_in *)cli_conn->src)->sin_addr.s_addr &
            sess->fe->except_mask.s_addr) != sess->fe->except_net.s_addr) &&
          (!s->be->except_mask.s_addr ||
           (((struct sockaddr_in *)cli_conn->src)->sin_addr.s_addr &
            s->be->except_mask.s_addr) != s->be->except_net.s_addr)) {
        unsigned char *pn =
            (unsigned char *)&((struct sockaddr_in *)cli_conn->src)->sin_addr;
        ...
        if (unlikely(!http_add_header(htx, hdr, ist2(trash.area, trash.data))))
          goto return_int_err;
               ...
               cli_conn->src->ss_family == AF_INET6) {
  ...
  if ((sess->fe->options | s->be->options) & PR_O_ORGTO) {
    ...
    if (cli_conn && conn_get_src(cli_conn) &&
        cli_conn->src->ss_family == AF_INET && conn_get_dst(cli_conn)) {
      /* Add an X-Original-To header unless the destination IP is
       * in the 'except' network range.
       */
      if (cli_conn->dst->ss_family == AF_INET &&
          ((!sess->fe->except_mask_to.s_addr ||
            (((struct sockaddr_in *)cli_conn->dst)->sin_addr.s_addr &
             sess->fe->except_mask_to.s_addr) != sess->fe->except_to.s_addr) &&
           (!s->be->except_mask_to.s_addr ||
            (((struct sockaddr_in *)cli_conn->dst)->sin_addr.s_addr &
             s->be->except_mask_to.s_addr) != s->be->except_to.s_addr))) {
        struct ist hdr;
        unsigned char *pn =
            (unsigned char *)&((struct sockaddr_in *)cli_conn->dst)->sin_addr;
...
int http_process_tarpit(struct stream *s, struct channel *req, int an_bit) {
  ...
  http_reply_and_close(
      s, txn->status,
      (!(req->flags & CF_READ_ERROR) ? http_error_message(s) : NULL));
 ...
 */
int http_wait_for_request_body(struct stream *s, struct channel *req,
                               int an_bit) {
  struct session *sess = s->sess;
  struct http_txn *txn = s->txn;
...
http_end:
  ...
  s->logs.tv_request =
      now; /* update the request timer to reflect full request */
  req->analysers &= ~an_bit;
  req->analyse_exp = TICK_ETERNITY;
  DBG_TRACE_LEAVE(STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA, s, txn);
  return 1;
 ...
 */
int http_request_forward_body(struct stream *s, struct channel *req,
                              int an_bit) {
  struct session *sess = s->sess;
  struct http_txn *txn = s->txn;
  struct http_msg *msg = &txn->req;
...
return_prx_cond:
  ...
  req->analysers &= AN_REQ_FLT_END;
  s->res.analysers &= AN_RES_FLT_END; /* we're in data phase, we want to abort
                                         both directions */
  if (!(s->flags & SF_ERR_MASK))
    s->flags |= SF_ERR_PRXCOND;
  if (!(s->flags & SF_FINST_MASK))
    s->flags |=
        ((txn->rsp.msg_state < HTTP_MSG_ERROR) ? SF_FINST_H : SF_FINST_D);
  DBG_TRACE_DEVEL("leaving on error ",
                  STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA | STRM_EV_HTTP_ERR, s,
                  txn);
  return 0;
...
/* Reset the stream and the backend stream_interface to a situation suitable for
...
static __inline int do_l7_retry(struct stream *s, struct stream_interface *si) {
  ...
  res->flags &= ~(CF_READ_ERROR | CF_READ_TIMEOUT | CF_SHUTR | CF_EOI |
                  CF_READ_NULL | CF_SHUTR_NOW);
  res->analysers = 0;
...
next_one:
  if (unlikely(htx_is_empty(htx) || htx->first == -1)) {
    ...
    if (rep->flags & CF_READ_ERROR) {
      ...
      if (conn && conn->err_code == CO_ER_SSL_EARLY_FAILED) {
        if ((s->be->retry_type & PR_RE_EARLY_ERROR) &&
            (si_b->flags & SI_FL_L7_RETRY) && do_l7_retry(s, si_b) == 0) {
          DBG_TRACE_DEVEL("leaving on L7 retry",
                          STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA, s, txn);
          return 0;
        }
    ...
    /* 4: close from server, capture the response if the server has started to
       respond */
    else if (rep->flags & CF_SHUTR) {
      if ((si_b->flags & SI_FL_L7_RETRY) &&
  ...
  if (unlikely((global.mode & MODE_DEBUG) &&
    ...
    for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
      ...
      http_debug_hdr("srvhdr", s, htx_get_blk_name(htx, blk),
                     htx_get_blk_value(htx, blk));
    }
  }
  ...
  if (objt_server(s->target)) {
    if (txn->status >= 100 &&
        (txn->status < 500 || txn->status == 501 || txn->status == 505))
...
return_bad_res:
  ...
  if ((s->be->retry_type & PR_RE_JUNK_REQUEST) &&
      (si_b->flags & SI_FL_L7_RETRY) && do_l7_retry(s, si_b) == 0) {
    DBG_TRACE_DEVEL("leaving on L7 retry", STRM_EV_STRM_ANA | STRM_EV_HTTP_ANA,
                    s, txn);
    return 0;
  }
  txn->status = 502;
 ...
 */
int http_process_res_common(struct stream *s, struct channel *rep, int an_bit,
                            struct proxy *px) {
  struct session *sess = s->sess;
  struct http_txn *txn = s->txn;
  struct http_msg *msg = &txn->rsp;
  ...
  while (1) {
    ...
    if (ret == HTTP_RULE_RES_CONT) {
      ret =
          http_res_get_intercept_rule(cur_proxy, &cur_proxy->http_res_rules, s);
  ...
  if (objt_server(s->target) && (s->be->ck_opts & PR_CK_INS) &&
      ...
      (!(s->flags & SF_DIRECT) ||
       ((s->be->cookie_maxidle || txn->cookie_last_date) &&
        (!txn->cookie_last_date ||
         (txn->cookie_last_date - date.tv_sec) < 0)) ||
       (s->be->cookie_maxlife &&
        !txn->cookie_first_date) || // set the first_date
       (!s->be->cookie_maxlife &&
        txn->cookie_first_date)) && // remove the first_date
    ...
    if (!objt_server(s->target)->cookie) {
      chunk_printf(&trash, "%s=; Expires=Thu, 01-Jan-1970 00:00:01 GMT; path=/",
                   s->be->cookie_name);
    } else {
      chunk_printf(&trash, "%s=%s", s->be->cookie_name,
                   objt_server(s->target)->cookie);
    ...
    if (unlikely(!http_add_header(htx, ist("Set-Cookie"),
                                  ist2(trash.area, trash.data))))
      goto return_int_err;
  ...
  if (((txn->flags & (TX_CACHEABLE | TX_CACHE_COOK | TX_SCK_PRESENT)) ==
    ...
    ha_alert(
        ...
        s->be->id,
        objt_server(s->target) ? objt_server(s->target)->id : "<dispatch>");
    send_log(
        s->be, LOG_ALERT,
        "Blocking cacheable cookie in response from instance %s, server %s.\n",
        s->be->id,
 ...
 * on missing data.
 */
int http_response_forward_body(struct stream *s, struct channel *res,
                               int an_bit) {
  struct session *sess = s->sess;
  struct http_txn *txn = s->txn;
  ...
  if ((txn->meth == HTTP_METH_CONNECT && txn->status == 200) ||
      txn->status == 101 ||
...
ending:
  ...
  if ((txn->meth == HTTP_METH_CONNECT && txn->status == 200) ||
      txn->status == 101 || !(msg->flags & HTTP_MSGF_XFER_LEN)) {
    msg->msg_state = HTTP_MSG_TUNNEL;
    goto ending;
  } else {
    msg->msg_state = HTTP_MSG_DONE;
...
return_error:
  ...
  s->req.analysers &= AN_REQ_FLT_END; /* we're in data phase, we want to abort
                                         both directions */
  if (!(s->flags & SF_FINST_MASK))
    s->flags |= SF_FINST_D;
  DBG_TRACE_DEVEL("leaving on error",
 ...
 */
int http_apply_redirect_rule(struct redirect_rule *rule, struct stream *s,
                             struct http_txn *txn) {
  ...
  case REDIRECT_TYPE_SCHEME: {
    ...
    if (rule->rdr_str) { /* this is an old "redirect" rule */
      ...
      chunk->data += build_logline(s, chunk->area + chunk->data,
                                   chunk->size - chunk->data, &rule->rdr_fmt);
    }
    /* add "://" + host + path */
  ...
  if (!(txn->req.flags & HTTP_MSGF_BODYLESS) &&
  ...
  flags = (HTX_SL_F_IS_RESP | HTX_SL_F_VER_11 | HTX_SL_F_XFER_LEN |
           HTX_SL_F_BODYLESS);
  sl = htx_add_stline(htx, HTX_BLK_RES_SL, flags, ist("HTTP/1.1"), status,
                      reason);
  if (!sl)
    goto fail;
  ...
  if (rule->cookie_len) {
    if (!htx_add_header(htx, ist("Set-Cookie"),
                        ist2(rule->cookie_str, rule->cookie_len)))
      goto fail;
  }
  ...
  if (!(s->flags & SF_FINST_MASK))
    s->flags |=
        ((rule->flags & REDIRECT_FLAG_FROM_REQ) ? SF_FINST_R : SF_FINST_H);
...
int http_replace_hdrs(struct stream *s, struct htx *htx, struct ist name,
  ...
  while (http_find_header(htx, name, &ctx, full)) {
    if (!regex_exec_match2(re, ctx.value.ptr, ctx.value.len, MAX_MATCH, pmatch,
                           0))
      continue;

    output->data =
        exp_replace(output->area, output->size, ctx.value.ptr, str, pmatch);
    if (output->data == -1)
      return -1;
 ...
 */
int http_res_set_status(unsigned int status, struct ist reason,
                        struct stream *s) {
...
int http_eval_after_res_rules(struct stream *s) {
  ...
  /* prune the request variables if not already done and swap to the response
   * variables. */
  ...
  if ((ret == HTTP_RULE_RES_CONT || ret == HTTP_RULE_RES_STOP) &&
      sess->fe != s->be)
    ret = http_res_get_intercept_rule(sess->fe, &sess->fe->http_after_res_rules,
                                      s);

end:
  /* All other codes than CONTINUE, STOP or DONE are forbidden */
  return (ret == HTTP_RULE_RES_CONT || ret == HTTP_RULE_RES_STOP ||
          ret == HTTP_RULE_RES_DONE);
}
  ...
  while (http_find_header(htx, ist("Cookie"), &ctx, 1)) {
    ...
    for (prev = hdr_beg; prev < hdr_end; prev = next) {
      ...
      if (equal < hdr_end && *equal == '=') {
        ...
        /* make val_end point to the first white space or delimiter after the
         * value */
        val_end = next;
      ...
      if (sess->fe->capture_name != NULL && txn->cli_cookie == NULL &&
          ...
          memcmp(att_beg, sess->fe->capture_name, sess->fe->capture_namelen) ==
              0) {
       ...
       */
      if ((att_end - att_beg == s->be->cookie_len) &&
          (s->be->cookie_name != NULL) &&
         ...
         * have the server ID between val_beg and delim, and the original cookie
         * between delim+1 and val_end. Otherwise, delim==val_end :
         *
         * hdr_beg
         * |
         * v
        ...
        if (txn->cookie_first_date && s->be->cookie_maxlife &&
            (((signed)(date.tv_sec - txn->cookie_first_date) >
              (signed)s->be->cookie_maxlife) ||
          ...
          txn->cookie_last_date = 0;
                   ...
                   (((signed)(date.tv_sec - txn->cookie_last_date) >
                     (signed)s->be->cookie_maxidle) ||
        ...
        /* Here, we'll look for the first running server which supports the
         * cookie. This allows to share a same cookie between several servers,
         * for example to dedicate backup servers to specific servers only.
         * However, to prevent clients from sticking to cookie-less backup
         * server when they have incidentely learned an empty cookie, we simply
         * ignore empty cookies and mark them as invalid. The same behaviour is
         * applied when persistence must be ignored.
         */
        ...
        while (srv) {
          if (srv->cookie && (srv->cklen == delim - val_beg) &&
            ...
            if ((srv->cur_state != SRV_ST_STOPPED) ||
                (s->be->options & PR_O_PERSIST) || (s->flags & SF_FORCE_PRST)) {
              /* we found the server and we can use it */
              txn->flags &= ~TX_CK_MASK;
              txn->flags |=
                  (srv->cur_state != SRV_ST_STOPPED) ? TX_CK_VALID : TX_CK_DOWN;
         ...
         * - delete the complete cookie if we're in insert+indirect mode, so
         * that the server never sees it ;
         * - remove the server id from the cookie value, and tag the cookie as
         * an application cookie so that it does not get accidentally removed
         * later, if we're in cookie prefix mode
         */
        if ((s->be->ck_opts & PR_CK_PFX) && (delim != val_end)) {
          ...
          preserve_hdr = 1; /* we want to keep this cookie */
                   ...
                   (s->be->ck_opts & (PR_CK_INS | PR_CK_IND)) ==
                       (PR_CK_INS | PR_CK_IND)) {
          del_from = prev;
        }
      } else {
...
static void http_manage_server_side_cookies(struct stream *s,
  ...
  while (1) {
    ...
    for (prev = hdr_beg; prev < hdr_end; prev = next) {
       ...
       * cookies shorter than the declared name.
       */
      if (sess->fe->capture_name != NULL && txn->srv_cookie == NULL &&
          (val_end - att_beg >= sess->fe->capture_namelen) &&
          memcmp(att_beg, sess->fe->capture_name, sess->fe->capture_namelen) ==
              0) {
        int log_len = val_end - att_beg;
      ...
      if (!(s->flags & SF_IGNORE_PRST) &&
          (att_end - att_beg == s->be->cookie_len) &&
          (s->be->cookie_name != NULL) &&
          (memcmp(att_beg, s->be->cookie_name, att_end - att_beg) == 0)) {
        ...
        if (s->be->ck_opts & PR_CK_PSV) {
          ...
          ctx.lws_before = ctx.lws_after = 0;
          http_replace_header_value(htx, &ctx,
                                    ist2(srv->cookie, srv->cklen + 1));
          delta = srv->cklen + 1;
          sliding = (ctx.value.ptr - val_beg);
          hdr_beg += sliding;
          val_beg += sliding;
 ...
 * s->txn->flags.
 */
void http_check_request_for_cacheability(struct stream *s,
                                         struct channel *req) {
  struct http_txn *txn = s->txn;
  struct htx *htx;
  struct http_hdr_ctx ctx = {.blk = NULL};
 ...
 */
void http_check_response_for_cacheability(struct stream *s,
                                          struct channel *res) {
  struct http_txn *txn = s->txn;
  struct http_hdr_ctx ctx = {.blk = NULL};
  struct htx *htx;
...
/*
 * In a GET, HEAD or POST request, check if the requested URI matches the stats
 * uri for the current backend.
 *
...
static int http_handle_stats(struct stream *s, struct channel *req) {
  ...
  for (h = lookup; h <= end - 8; h++) {
    if (memcmp(h, STAT_SCOPE_INPUT_NAME "=",
               strlen(STAT_SCOPE_INPUT_NAME) + 1) == 0) {
      int itx = 0;
      const char *h2;
  ...
  list_for_each_entry(stats_admin_rule, &uri_auth->admin_rules, list) {
    ...
    if (stats_admin_rule->cond) {
      ret = acl_exec_cond(stats_admin_rule->cond, s->be, sess, s,
                          SMP_OPT_DIR_REQ | SMP_OPT_FINAL);
      ret = acl_pass(ret);
...
void http_perform_server_redirect(struct stream *s,
  ...
  flags = (HTX_SL_F_IS_RESP | HTX_SL_F_VER_11 | HTX_SL_F_XFER_LEN |
           HTX_SL_F_BODYLESS);
  sl = htx_add_stline(htx, HTX_BLK_RES_SL, flags, ist("HTTP/1.1"), ist("302"),
                      ist("Found"));
  if (!sl)
    goto fail;
  ...
  /* FIXME: we should increase a counter of redirects per server and per
   * backend. */
...
static void http_end_request(struct stream *s) {
  ...
  if (txn->req.msg_state == HTTP_MSG_DONE) {
    ...
    if (txn->flags & TX_CON_WANT_TUN) {
      ...
      if (b_data(&chn->buf)) {
        DBG_TRACE_DEVEL("waiting to flush the request", STRM_EV_HTTP_ANA, s,
                        txn);
        return;
      }
...
static void http_end_response(struct stream *s) {
  ...
  if (txn->rsp.msg_state == HTTP_MSG_DONE) {
    ...
    if (txn->flags & TX_CON_WANT_TUN) {
      ...
      chn->flags |= CF_NEVER_WAIT;
      if (b_data(&chn->buf)) {
        DBG_TRACE_DEVEL("waiting to flush the respone", STRM_EV_HTTP_ANA, s,
                        txn);
        return;
      }
...
void http_reply_and_close(struct stream *s, short status,
                          struct http_reply *msg) {
  if (!msg) {
 ...
 */
int http_reply_to_htx(struct stream *s, struct htx *htx,
  ...
  if (reply->type == HTTP_REPLY_ERRMSG) {
    ...
    slflags = (HTX_SL_F_IS_RESP | HTX_SL_F_VER_11 | HTX_SL_F_XFER_LEN |
               HTX_SL_F_CLEN);
    if (!body || !b_data(body))
      slflags |= HTX_SL_F_BODYLESS;
    sl = htx_add_stline(htx, HTX_BLK_RES_SL, slflags, ist("HTTP/1.1"),
    ...
    if (!LIST_ISEMPTY(&reply->hdrs)) {
      ...
      list_for_each_entry(hdr, &reply->hdrs, list) {
        ...
        if (b_data(value) &&
            !htx_add_header(htx, hdr->name,
                            ist2(b_head(value), b_data(value)))) {
    ...
    if (!htx_add_header(htx, ist("content-length"), ist(clen)) ||
        (body && b_data(body) && ctype &&
         !htx_add_header(htx, ist("content-type"), ist(ctype))) ||
        !htx_add_endof(htx, HTX_BLK_EOH) ||
        (body && b_data(body) &&
         !htx_add_data_atonce(htx, ist2(b_head(body), b_data(body)))) ||
...
void http_return_srv_error(struct stream *s, struct stream_interface *si) {
  ...
  if (err_type & SI_ET_QUEUE_ABRT)
    http_server_error(s, si, SF_ERR_CLICL, SF_FINST_Q, http_error_message(s));
...
static int http_reply_100_continue(struct stream *s) {
  ...
  struct htx *htx = htx_from_buf(&res->buf);
  struct htx_sl *sl;
  unsigned int flags = (HTX_SL_F_IS_RESP | HTX_SL_F_VER_11 | HTX_SL_F_XFER_LEN |
                        HTX_SL_F_BODYLESS);
 ...
 */
static size_t http_fmt_req_line(const struct htx_sl *sl, char *str,
                                size_t len) {
...
static void http_debug_stline(const char *dir, struct stream *s,
  ...
  chunk_printf(&trash, "%08x:%s.%s[%04x:%04x]: ", s->uniq_id, s->be->id, dir,
               objt_conn(sess->origin)
                   ? (unsigned short)objt_conn(sess->origin)->handle.fd
                   : -1,
               objt_cs(s->si[1].end)
                   ? (unsigned short)objt_cs(s->si[1].end)->conn->handle.fd
                   : -1);
...
void http_txn_reset_req(struct http_txn *txn) {
  txn->req.flags = 0;
  txn->req.msg_state =
      HTTP_MSG_RQBEFORE; /* at the very beginning of the request */
}

void http_txn_reset_res(struct http_txn *txn) {
  txn->rsp.flags = 0;
  txn->rsp.msg_state =
      HTTP_MSG_RPBEFORE; /* at the very beginning of the response */
}
...
__attribute__((constructor)) static void __http_protocol_init(void) {}
