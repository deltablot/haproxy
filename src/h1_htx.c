...
static size_t h1_eval_htx_size(const struct ist p1, const struct ist p2,
                               const struct ist p3,
  ...
  sz =
      sizeof(struct htx_blk) + sizeof(struct htx_sl) + p1.len + p2.len + p3.len;
 ...
 */
static int h1_postparse_req_hdrs(struct h1m *h1m, union h1_sl *h1sl,
                                 struct htx *htx, struct http_hdr *hdrs,
                                 size_t max) {
  struct htx_sl *sl;
 ...
 * HTX_FL_PARSING_ERROR and filling h1m->err_pos and h1m->err_state fields.
 */
static int h1_postparse_res_hdrs(struct h1m *h1m, union h1_sl *h1sl,
                                 struct htx *htx, struct http_hdr *hdrs,
                                 size_t max) {
  struct htx_sl *sl;
...
int h1_parse_msg_hdrs(struct h1m *h1m, union h1_sl *h1sl, struct htx *dsthtx,
  ...
  ret = h1_headers_to_hdr_list(b_peek(srcbuf, ofs), b_tail(srcbuf), hdrs,
 ...
 */
static int h1_copy_msg_data(struct htx **dsthtx, struct buffer *srcbuf,
                            size_t ofs, size_t count, struct buffer *htxbuf) {
   ...
   */
  if (unlikely(htx_is_empty(tmp_htx) && count == b_data(srcbuf) && !ofs &&
               b_head_ofs(srcbuf) == sizeof(struct htx))) {
...
int h1_parse_msg_tlrs(struct h1m *h1m, struct htx *dsthtx,
  ...
  ret = h1_headers_to_hdr_list(b_peek(srcbuf, ofs), b_tail(srcbuf), hdrs,
                               sizeof(hdrs) / sizeof(hdrs[0]), &tlr_h1m, NULL);
  if (ret <= 0) {
...
int h1_format_htx_reqline(const struct htx_sl *sl, struct buffer *chk) {
  ...
  if (!chunk_memcat(chk, HTX_SL_REQ_MPTR(sl), HTX_SL_REQ_MLEN(sl)) ||
      !chunk_memcat(chk, " ", 1) || !chunk_memcat(chk, uri.ptr, uri.len) ||
      !chunk_memcat(chk, " ", 1))
    goto full;
 ...
 * returns 0.
 */
int h1_format_htx_hdr(const struct ist n, const struct ist v,
                      struct buffer *chk) {
  size_t sz = chk->data;
  ...
  if (!chunk_memcat(chk, n.ptr, n.len) || !chunk_memcat(chk, ": ", 2) ||
      !chunk_memcat(chk, v.ptr, v.len) || !chunk_memcat(chk, "\r\n", 2))
    goto full;
  ...
  if (chunked) {
    ...
    *--beg = '\r';
    ...
    if (!chunk_memcat(chk, beg, end - beg) ||
        !chunk_memcat(chk, data.ptr, data.len) || !chunk_memcat(chk, "\r\n", 2))
      goto full;
  } else {
