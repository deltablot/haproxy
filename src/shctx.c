...
/*
 * Reserve a new row if <first> is null, put it in the hotlist, set the refcount
 * to 1 or append new blocks to the row with <first> as first block if non null.
 *
 * Reserve blocks in the avail list and put them in the hot list
 * Return the first block put in the hot list or NULL if not enough blocks
 * available
 */
 ...
 */
void shctx_row_inc_hot(struct shared_context *shctx,
                       struct shared_block *first) {
...
/*
 * decrement the refcount and move the row at the end of the avail list if it
 * reaches 0.
 */
void shctx_row_dec_hot(struct shared_context *shctx,
                       struct shared_block *first) {
  struct shared_block *block, *sblock;
  int count = 0;
...
int shctx_init(struct shared_context **orig_shctx, int maxblocks, int blocksize,
  ...
  shctx = (struct shared_context *)mmap(
      NULL,
      sizeof(struct shared_context) + extra +
          (maxblocks * (sizeof(struct shared_block) + blocksize)),
