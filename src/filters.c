...
static int handle_analyzer_result(struct stream *s, struct channel *chn,
 ...
 * themselves
 */
static int parse_filter(char **args, int section_type, struct proxy *curpx,
  ...
  if (curpx == defpx) {
    memprintf(err,
              "parsing [%s:%d] : %s is not allowed in a 'default' section.",
              file, line, args[0]);
    return -1;
  }
  if (!strcmp(args[0], "filter")) {
    ...
    if (!*args[1]) {
      memprintf(err, "parsing [%s:%d] : missing argument for '%s' in %s '%s'.",
    ...
    if (kw) {
      if (!kw->parse) {
        memprintf(err,
                  "parsing [%s:%d] : '%s' : "
                  "'%s' option is not implemented in this version (check build "
                  "options).",
      ...
      flt_dump_kws(err);
      indent_msg(err, 4);
      memprintf(err, "'%s' : unknown keyword '%s'.%s%s", args[0], args[cur_arg],
                err && *err ? " Registered keywords :" : "",
                err && *err ? *err : "");
      goto error;
    }
    if (*args[cur_arg]) {
      memprintf(err, "'%s %s' : unknown keyword '%s'.", args[0], args[1],
                args[cur_arg]);
      goto error;
    }
    if (fconf->ops == NULL) {
      memprintf(err, "'%s %s' : no callbacks defined.", args[0], args[1]);
      goto error;
    }
 ...
 */
static int flt_init(struct proxy *proxy) {
 ...
 */
static int flt_init_per_thread(struct proxy *proxy) {
  struct flt_conf *fconf;

  list_for_each_entry(fconf, &proxy->filter_configs, list) {
    if (fconf->ops->init_per_thread &&
        fconf->ops->init_per_thread(proxy, fconf) < 0)
      return ERR_ALERT | ERR_FATAL;
  }
  return 0;
}
...
static int flt_init_all_per_thread() {
  struct proxy *px;
  int err_code = 0;
 ...
 */
int flt_check(struct proxy *proxy) {
  struct flt_conf *fconf;
  int err = 0;
 ...
 * HAProxy is stopped.
 */
void flt_deinit(struct proxy *proxy) {
 ...
 */
void flt_deinit_per_thread(struct proxy *proxy) {
  struct flt_conf *fconf, *back;
...
static void flt_deinit_all_per_thread() {
  struct proxy *px;
...
static int flt_stream_add_filter(struct stream *s, struct flt_conf *fconf,
                                 unsigned int flags) {
  struct filter *f;
...
int flt_stream_start(struct stream *s) {
  ...
  list_for_each_entry(filter, &strm_flt(s)->filters, list) {
    if (FLT_OPS(filter)->stream_start &&
        FLT_OPS(filter)->stream_start(s, filter) < 0)
      return -1;
  }
  return 0;
}
...
/*
 * Calls 'check_timeouts' for all filters attached to a stream. This happens
 * when the stream is woken up because of expired timer.
 */
...
int flt_http_payload(struct stream *s, struct http_msg *msg, unsigned int len) {
  ...
  list_for_each_entry(filter, &strm_flt(s)->filters, list) {
    ...
    if (FLT_OPS(filter)->http_payload) {
      DBG_TRACE_DEVEL(FLT_ID(filter), STRM_EV_HTTP_ANA | STRM_EV_FLT_ANA, s);
      ret = FLT_OPS(filter)->http_payload(s, filter, msg, out + offset,
                                          data - offset);
      if (ret < 0)
        goto end;
 ...
 * otherwise.
 */
int flt_start_analyze(struct stream *s, struct channel *chn,
                      unsigned int an_bit) {
  int ret = 1;
 ...
 */
int flt_pre_analyze(struct stream *s, struct channel *chn,
                    unsigned int an_bit) {
  int ret = 1;
  ...
  RESUME_FILTER_LOOP(s, chn) {
    if (FLT_OPS(filter)->channel_pre_analyze &&
 ...
 */
int flt_post_analyze(struct stream *s, struct channel *chn,
                     unsigned int an_bit) {
  struct filter *filter;
  int ret = 1;
  ...
  list_for_each_entry(filter, &strm_flt(s)->filters, list) {
    if (FLT_OPS(filter)->channel_post_analyze &&
 ...
 * needs to wait, any other value otherwise.
 */
int flt_analyze_http_headers(struct stream *s, struct channel *chn,
                             unsigned int an_bit) {
  struct http_msg *msg;
  int ret = 1;
  ...
  if (HAS_DATA_FILTERS(s, chn)) {
    ...
    list_for_each_entry(f, &strm_flt(s)->filters, list) FLT_OFF(f, chn) = data;
  }
 ...
 * needs to wait, any other value otherwise.
 */
int flt_end_analyze(struct stream *s, struct channel *chn,
                    unsigned int an_bit) {
  int ret = 1;
 ...
 * forwarded bytes.
 */
int flt_tcp_payload(struct stream *s, struct channel *chn, unsigned int len) {
  struct filter *filter;
  ...
  list_for_each_entry(filter, &strm_flt(s)->filters, list) {
    ...
    if (FLT_OPS(filter)->tcp_payload) {
      ...
      ret = FLT_OPS(filter)->tcp_payload(s, filter, chn, out + offset,
                                         data - offset);
      if (ret < 0)
        goto end;
      data = ret + *flt_off - *strm_off;
...
int flt_xfer_data(struct stream *s, struct channel *chn, unsigned int an_bit) {
  ...
  if (chn->flags & CF_SHUTR) {
    if (((s->flags & SF_HTX) && htx_is_empty(htxbuf(&chn->buf))) ||
        c_empty(chn)) {
      ret = 1;
      goto end;
    }
  ...
  DBG_TRACE_DEVEL("waiting for more data",
                  STRM_EV_STRM_ANA | STRM_EV_TCP_ANA | STRM_EV_FLT_ANA, s);
  return 0;
end:
