...
struct extcheck_env {
  ...
  int vmaxlen; /* value maximum length, used to determine the required memory
                  allocation */
};

/* environment variables memory requirement for different types of data */
#define EXTCHK_SIZE_EVAL_INIT                                                  \
  0                          /* size determined during the init phase,         \
...
#define EXTCHK_SIZE_ADDR                                                       \
  INET6_ADDRSTRLEN + 1 /* max string length for an address */
...
enum {
  ...
  EXTCHK_HAPROXY_SERVER_CURCONN, /* the current number of connections on the
                                    server */
...
int init_pid_list(void) {
  ...
  if (!signal_register_fct(SIGCHLD, sigchld_handler, SIGCHLD)) {
    ha_alert("Failed to set signal handler for external health checks: %s. "
             "Aborting.\n",
             strerror(errno));
    return 1;
  }
  ...
  pool_head_pid_list =
      create_pool("pid_list", sizeof(struct pid_list), MEM_F_SHARED);
  if (pool_head_pid_list == NULL) {
    ha_alert("Failed to allocate memory pool for external health checks: %s. "
             "Aborting.\n",
             strerror(errno));
    return 1;
  }
...
/* helper macro to set an environment variable and jump to a specific label on
 * failure. */
#define EXTCHK_SETENV(check, envidx, value, fail)                              \
  {                                                                            \
    if (extchk_setenv(check, envidx, value))                                   \
      goto fail;                                                               \
  }
...
static int extchk_setenv(struct check *check, int idx, const char *value) {
  ...
  if (!check->envp[idx]) {
    ha_alert("Failed to allocate memory for the environment variable '%s'. "
             "Aborting.\n",
             envname);
    return 1;
  }
  ...
  if (ret < 0) {
    ha_alert("Failed to store the environment variable '%s'. Reason : %s. "
             "Aborting.\n",
...
int prepare_external_check(struct check *check) {
  ...
  EXTCHK_SETENV(check, EXTCHK_HAPROXY_PROXY_ID,
                ultoa_r(px->uuid, buf, sizeof(buf)), err);
  ...
  EXTCHK_SETENV(check, EXTCHK_HAPROXY_SERVER_ID,
                ultoa_r(s->puid, buf, sizeof(buf)), err);
  ...
  EXTCHK_SETENV(check, EXTCHK_HAPROXY_SERVER_MAXCONN,
                ultoa_r(s->maxconn, buf, sizeof(buf)), err);
...
static int connect_proc_chk(struct task *t) {
  ...
  if (pid < 0) {
    ha_alert(
        "Failed to fork process for external health check%s: %s. Aborting.\n",
  ...
  if (pid == 0) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &limit) == -1) {
      ...
      ha_warning("External check: failed to restore initial FD limits (cur=%u "
                 "max=%u), using cur=%u max=%u\n",
    ...
    /* Update some environment variables and command args: curconn, server addr
     * and server port */
    EXTCHK_SETENV(check, EXTCHK_HAPROXY_SERVER_CURCONN,
                  ultoa_r(s->cur_sess, buf, sizeof(buf)), fail);
    ...
    *check->argv[4] = 0;
    ...
    ha_alert(
        "Failed to exec process for external health check: %s. Aborting.\n",
        strerror(errno));
  fail:
    exit(-1);
  }
 ...
 */
struct task *process_chk_proc(struct task *t, void *context,
  ...
  if (!(check->state & CHK_ST_INPROGRESS)) {
    ...
    if (check->result == CHK_RES_UNKNOWN) {
      ...
      if (elem->exited) {
        status =
            elem->status; /* Save in case the process exits between use below */
        if (!WIFEXITED(status))
          check->code = -1;
        else
...
int proxy_parse_external_check_opt(char **args, int cur_arg,
