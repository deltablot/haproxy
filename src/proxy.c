...
unsigned int error_snapshot_id =
    0; /* global ID assigned to each error then incremented */
...
const struct cfg_opt cfg_opts[] = {
    ...
    {"http-buffer-request", PR_O_WREQ_BODY, PR_CAP_FE | PR_CAP_BE, 0,
     PR_MODE_HTTP},
...
static int proxy_parse_timeout(char **args, int section, struct proxy *proxy,
  ...
  if (!strcmp(args[0], "client")) {
    ...
    memprintf(err,
              "the '%s' directive is not supported anymore since HAProxy 2.1. "
              "Use 'timeout client'.",
              args[0]);
    return -1;
  ...
  if (*args[1] == 0) {
    memprintf(err, "'timeout %s' expects an integer value (in milliseconds)",
              name);
    return -1;
  }
  ...
  if (res == PARSE_TIME_OVER) {
    memprintf(err,
              "timer overflow in argument '%s' to 'timeout %s' (maximum value "
              "is 2147483647 ms or ~24.8 days)",
              args[1], name);
    return -1;
  } else if (res == PARSE_TIME_UNDER) {
    memprintf(err,
              "timer underflow in argument '%s' to 'timeout %s' (minimum "
  ...
  if (!(proxy->cap & cap)) {
    memprintf(
        err,
        "'timeout %s' will be ignored because %s '%s' has no %s capability",
        name, proxy_type_str(proxy), proxy->id,
        (cap & PR_CAP_BE) ? "backend" : "frontend");
    retval = 1;
  } else if (defpx && *tv != *td) {
    memprintf(err, "overwriting 'timeout %s' which was already specified",
              name);
    retval = 1;
  }
  ...
  if (*args[2] != 0) {
    memprintf(err,
              "'timeout %s' : unexpected extra argument '%s' after value '%s'.",
              name, args[2], args[1]);
    retval = -1;
  }
...
static int proxy_parse_rate_limit(char **args, int section, struct proxy *proxy,
  ...
  if (strcmp(args[1], "sessions") == 0) {
    ...
    memprintf(err, "'%s' only supports 'sessions' (got '%s')", args[0],
              args[1]);
    return -1;
  }
  ...
  if (*args[2] == 0) {
    memprintf(err,
              "'%s %s' expects expects an integer value (in sessions/second)",
              args[0], args[1]);
    return -1;
  }
  ...
  if (*res) {
    memprintf(err, "'%s %s' : unexpected character '%c' in integer value '%s'",
              args[0], args[1], *res, args[2]);
    return -1;
  }

  if (!(proxy->cap & PR_CAP_FE)) {
    memprintf(
        err, "%s %s will be ignored because %s '%s' has no frontend capability",
        args[0], args[1], proxy_type_str(proxy), proxy->id);
    retval = 1;
  } else if (defpx && *tv != *td) {
    memprintf(err, "overwriting %s %s which was already specified", args[0],
              args[1]);
    retval = 1;
  }
 ...
 */
static int proxy_parse_max_ka_queue(char **args, int section,
                                    struct proxy *proxy, struct proxy *defpx,
  ...
  if (*args[1] == 0) {
    memprintf(err, "'%s' expects expects an integer value (or -1 to disable)",
              args[0]);
    return -1;
  }
  ...
  if (*res) {
    memprintf(err, "'%s' : unexpected character '%c' in integer value '%s'",
              args[0], *res, args[1]);
    return -1;
  }
  ...
  if (!(proxy->cap & PR_CAP_BE)) {
    memprintf(err,
              "%s will be ignored because %s '%s' has no backend capability",
              args[0], proxy_type_str(proxy), proxy->id);
    retval = 1;
  }

  /* we store <val+1> so that a user-facing value of -1 is stored as zero
   * (default) */
 ...
 * if there is any error, 1 for warning, otherwise 0. If it does not return
 * zero, it will write an error or warning message into a preallocated buffer
 * returned at <err>. The function must be called with <args> pointing to the
 * first command line word, with <proxy> pointing to the proxy being parsed, and
...
static int proxy_parse_declare(char **args, int section, struct proxy *curpx,
  ...
  if (!(curpx->cap & PR_CAP_FE)) {
    memprintf(err, "'%s' only available in frontend or listen section",
              args[0]);
    return -1;
  }
  ...
  if (!args[1] || !*args[1]) {
    memprintf(err,
              "'%s' needs a second keyword that specify the type of "
              "declaration ('capture')",
              args[0]);
    return -1;
  }
  ...
  if (strcmp(args[1], "capture") == 0) {
    ...
    if (!args[2] || !*args[2] ||
        (strcmp(args[2], "response") != 0 && strcmp(args[2], "request") != 0)) {
      memprintf(err, "'%s %s' requires a direction ('request' or 'response')",
                args[0], args[1]);
      return -1;
    }
    ...
    if (!args[3] || !*args[3] || strcmp(args[3], "len") != 0) {
      memprintf(err, "'%s %s' requires a capture length ('len')", args[0],
                args[1]);
      return -1;
    }
    ...
    if (!args[4] || !*args[4]) {
      memprintf(err,
                "'%s %s': 'len' requires a numeric value that represents the "
                "capture length",
                args[0], args[1]);
      return -1;
    }
    ...
    if (*error != '\0') {
      memprintf(err, "'%s %s': cannot parse the length '%s'.", args[0], args[1],
                args[3]);
      return -1;
    }
    ...
    memprintf(err, "unknown declaration type '%s' (supports 'capture')",
              args[1]);
    return -1;
  }
}
...
static int proxy_parse_retry_on(char **args, int section, struct proxy *curpx,
                                struct proxy *defpx, const char *file, int line,
                                char **err) {
  int i;

  if (!(*args[1])) {
    memprintf(err, "'%s' needs at least one keyword to specify when to retry",
              args[0]);
    return -1;
  }
  ...
  for (i = 1; *(args[i]); i++) {
    if (!strcmp(args[i], "conn-failure"))
      ...
      curpx->retry_type |= PR_RE_CONN_FAILED | PR_RE_DISCONNECTED |
                           PR_RE_TIMEOUT | PR_RE_500 | PR_RE_502 | PR_RE_503 |
                           PR_RE_504 | PR_RE_EARLY_ERROR | PR_RE_JUNK_REQUEST;
...
static int proxy_parse_tcpka_cnt(char **args, int section, struct proxy *proxy,
  ...
  if (!strcmp(args[0], "clitcpka-cnt")) {
    if (!(proxy->cap & PR_CAP_FE)) {
      memprintf(err,
                "%s will be ignored because %s '%s' has no frontend capability",
                args[0], proxy_type_str(proxy), proxy->id);
      retval = 1;
    }
...
static int proxy_parse_tcpka_intvl(char **args, int section,
                                   struct proxy *proxy, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  int retval;
  const char *res;
...
struct proxy *proxy_find_by_name(const char *name, int cap, int table) {
  ...
  if (*name == '#') {
    ...
    for (node = ebis_lookup(&proxy_by_name, name); node;
         node = ebpt_next(node)) {
 ...
 *    ok  --  --  | proxy deleted, but other half with same name still here
 * (before) ok  --  ok  | proxy's ID changed (proxy moved in the config file) ok
 * ok  --  | proxy deleted, but other half with same name still here (after) ok
 * ok  ok  | perfect match
 *
 ...
 */
struct proxy *proxy_find_best_match(int cap, const char *name, int id,
                                    int *diff) {
  struct proxy *byname;
  struct proxy *byid;
  ...
  if (id) {
    ...
    if (byid) {
      if (!name)
        ...
        *diff |= PR_FBM_MISMATCH_NAME |
                 PR_FBM_MISMATCH_PROXYTYPE; /* name and type changed */
...
int proxy_cfg_ensure_no_http(struct proxy *curproxy) {
  ...
  if (curproxy->monitor_uri != NULL) {
    ha_warning("config : monitor-uri will be ignored for %s '%s' (needs 'mode "
               "http').\n",
               proxy_type_str(curproxy), curproxy->id);
  }
  if (curproxy->lbprm.algo & BE_LB_NEED_HTTP) {
    ...
    ha_warning("config : Layer 7 hash not possible for %s '%s' (needs 'mode "
               "http'). Falling back to round robin.\n",
               proxy_type_str(curproxy), curproxy->id);
  }
  if (curproxy->to_log & (LW_REQ | LW_RESP)) {
    curproxy->to_log &= ~(LW_REQ | LW_RESP);
    ha_warning("parsing [%s:%d] : HTTP log/header format not usable with %s "
               "'%s' (needs 'mode http').\n",
  ...
  if (curproxy->conf.logformat_string == default_http_log_format ||
    ...
    ha_warning("parsing [%s:%d] : 'option httplog' not usable with %s '%s' "
...
void proxy_cond_disable(struct proxy *p) {
  ...
  if (p->mode == PR_MODE_TCP || p->mode == PR_MODE_HTTP ||
      p->mode == PR_MODE_SYSLOG)
    ha_warning("Proxy %s stopped (cumulated conns: FE: %lld, BE: %lld).\n",
               p->id, p->fe_counters.cum_conn, p->be_counters.cum_conn);

  if (p->mode == PR_MODE_TCP || p->mode == PR_MODE_HTTP)
    send_log(p, LOG_WARNING,
             "Proxy %s stopped (cumulated conns: FE: %lld, BE: %lld).\n", p->id,
             p->fe_counters.cum_conn, p->be_counters.cum_conn);
...
static int proxy_parse_hard_stop_after(char **args, int section_type,
...
struct task *hard_stop(struct task *t, void *context, unsigned short state) {
  ...
  if (killed) {
    ...
    send_log(NULL, LOG_WARNING,
             "Some tasks resisted to hard-stop, exiting now.\n");
    killed = 2;
  ...
  while (p) {
    if ((p->cap & PR_CAP_FE) && (p->feconn > 0)) {
      ha_warning("Proxy %s hard-stopped (%d remaining conns will be closed).\n",
                 p->id, p->feconn);
      send_log(p, LOG_WARNING,
               "Proxy %s hard-stopped (%d remaining conns will be closed).\n",
               p->id, p->feconn);
    }
    p = p->next;
  }
  list_for_each_entry(s, &streams, list) { stream_shutdown(s, SF_ERR_KILLED); }
...
/*
 * this function disables health-check servers so that the process will quickly
 * be ignored by load balancers. Note that if a proxy was already in the PAUSED
 * state, then its grace time will not be used since it would already not listen
 * anymore to the socket.
 */
void soft_stop(void) {
  struct proxy *p;
  struct task *task;
...
int pause_proxy(struct proxy *p) {
  ...
  list_for_each_entry(l, &p->conf.listeners, by_fe) pause_listener(l);
...
void stop_proxy(struct proxy *p) {
  ...
  list_for_each_entry(l, &p->conf.listeners, by_fe) stop_listener(l, 1, 0, 0);
  ...
  list_for_each_entry(l, &p->conf.listeners, by_fe) {
    if (!resume_listener(l)) {
      ...
      if (port) {
        ...
        send_log(p, LOG_WARNING,
                 "Bind on socket %d busy while trying to enable %s %s.\n",
                 l->luid, proxy_cap_str(p->cap), p->id);
      }
...
int stream_set_backend(struct stream *s, struct proxy *be) {
   ...
   */
  s->req.analysers |=
      be->be_req_ana & ~(strm_li(s) ? strm_li(s)->analysers : 0);
      ...
      if (conn && cs) {
        ...
        if (s->si[0].wait_event.events)
          conn->mux->unsubscribe(cs, s->si[0].wait_event.events,
                                 &s->si[0].wait_event);
        if (conn_upgrade_mux_fe(conn, cs, &s->req.buf, ist(""),
                                PROTO_MODE_HTTP) == -1)
          return 0;
 ...
 */
void proxy_capture_error(
    struct proxy *proxy, int is_back, struct proxy *other_end,
    enum obj_type *target, const struct session *sess, const struct buffer *buf,
    long buf_ofs, unsigned int buf_out, unsigned int err_pos,
      ...
      if (swrule1->be.backend != curproxy->defbe.be) {
        ...
        list_for_each_entry(swrule2, &curproxy->switching_rules, list) {
          if (swrule2 == swrule1) {
            ...
            break;
          } else if (!swrule2->dynamic &&
                     swrule2->be.backend == swrule1->be.backend) {
...
static struct cfg_kw_list cfg_kws = {
    ILH,
    {
        ...
        {CFG_LISTEN, "clitimeout",
         proxy_parse_timeout}, /* This keyword actually fails to parse, this
                                  line remains for better error messages. */
        {CFG_LISTEN, "contimeout",
         proxy_parse_timeout}, /* This keyword actually fails to parse, this
                                  line remains for better error messages. */
        {CFG_LISTEN, "srvtimeout",
         proxy_parse_timeout}, /* This keyword actually fails to parse, this
                                  line remains for better error messages. */
...
/* Expects to find a frontend named <arg> and returns it, otherwise displays
 * various adequate error messages and returns NULL. This function is designed
 * to be used by functions requiring a frontend on the CLI.
 */
 ...
 */
static int cli_parse_show_servers(char **args, char *payload,
 ...
 * These information are all the parameters which may change during HAProxy
 * runtime. By default, we only export to the last known server state file
 * format. These information can be used at next startup to recover same level
 * of server state. It uses the proxy pointer from cli.p0, the proxy's id from
 * cli.i0 and the server's pointer from cli.p1.
 */
static int dump_servers_state(struct stream_interface *si) {
  ...
  /* we don't want to report any state if the backend is not enabled on this
   * process */
  ...
  for (; appctx->ctx.cli.p1 != NULL; appctx->ctx.cli.p1 = srv->next) {
    ...
    case AF_INET:
      inet_ntop(srv->addr.ss_family,
                &((struct sockaddr_in *)&srv->addr)->sin_addr, srv_addr,
                INET_ADDRSTRLEN + 1);
      break;
    case AF_INET6:
      inet_ntop(srv->addr.ss_family,
                &((struct sockaddr_in6 *)&srv->addr)->sin6_addr, srv_addr,
                INET6_ADDRSTRLEN + 1);
      break;
    default:
      memcpy(srv_addr, "-\0", 2);
    ...
    if (appctx->ctx.cli.o0 == 0) {
      ...
      chunk_printf(&trash,
                   ...
                   "%s %d"
                   "\n",
                   px->uuid, px->id, srv->puid, srv->id, srv_addr,
                   srv->cur_state, srv->cur_admin, srv->uweight, srv->iweight,
                   (long int)srv_time_since_last_change, srv->check.status,
                   srv->check.result, srv->check.health, srv->check.state,
                   srv->agent.state, bk_f_forced_id, srv_f_forced_id,
      ...
      chunk_printf(&trash, "%s/%s %d/%d %s %u - %u %u %u %u %u %u %d %u",
                   px->id, srv->id, px->uuid, srv->puid, srv_addr,
                   srv->svc_port, srv->pool_purge_delay, srv->curr_used_conns,
                   srv->max_used_conns, srv->est_need_conns, srv->curr_idle_nb,
                   srv->curr_safe_nb, (int)srv->max_idle_conns,
                   srv->curr_idle_conns);
  ...
  if (appctx->st2 == STAT_ST_HEAD) {
    if (appctx->ctx.cli.o0 == 0)
      ...
      chunk_printf(
          ...
          "need_est unsafe_nb safe_nb idle_lim idle_cur idle_per_thr[%d]\n",
          global.nbthread);
 ...
 */
static int cli_parse_enable_dyncookie_backend(char **args, char *payload,
 ...
 * Grabs the proxy lock and each server's lock.
 */
static int cli_parse_disable_dyncookie_backend(char **args, char *payload,
                                               struct appctx *appctx,
                                               void *private) {
  struct proxy *px;
  struct server *s;
 ...
 * Grabs the proxy lock and each server's lock.
 */
static int cli_parse_set_dyncookie_key_backend(char **args, char *payload,
                                               struct appctx *appctx,
                                               void *private) {
  struct proxy *px;
  struct server *s;
 ...
 */
static int cli_parse_set_maxconn_frontend(char **args, char *payload,
                                          struct appctx *appctx,
                                          void *private) {
  struct proxy *px;
  struct listener *l;
 ...
 * Grabs the proxy lock.
 */
static int cli_parse_shutdown_frontend(char **args, char *payload,
                                       struct appctx *appctx, void *private) {
  struct proxy *px;
 ...
 * Grabs the proxy lock.
 */
static int cli_parse_disable_frontend(char **args, char *payload,
                                      struct appctx *appctx, void *private) {
  struct proxy *px;
  int ret;
  ...
  if (px->disabled)
    return cli_msg(appctx, LOG_NOTICE,
 ...
 * Grabs the proxy lock.
 */
static int cli_parse_enable_frontend(char **args, char *payload,
                                     struct appctx *appctx, void *private) {
  struct proxy *px;
  int ret;
  ...
  if (!ret)
    return cli_err(appctx, "Failed to resume frontend, check logs for precise "
                           "cause (port conflict?).\n");
  return 1;
}
 ...
 * now.
 */
static int cli_parse_show_errors(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
...
static int cli_io_handler_show_errors(struct appctx *appctx) {
  ...
  if (!appctx->ctx.errors.px) {
    ...
    chunk_appendf(
        &trash,
        "Total events captured on [%02d/%s/%04d:%02d:%02d:%02d.%03d] : %u\n",
        tm.tm_mday, monthname[tm.tm_mon], tm.tm_year + 1900, tm.tm_hour,
        tm.tm_min, tm.tm_sec, (int)(date.tv_usec / 1000), error_snapshot_id);
  ...
  while (appctx->ctx.errors.px) {
    ...
    if (appctx->ctx.errors.ptr < 0) {
      ...
      chunk_appendf(&trash, " \n[%02d/%s/%04d:%02d:%02d:%02d.%03d]", tm.tm_mday,
                    monthname[tm.tm_mon], tm.tm_year + 1900, tm.tm_hour,
                    tm.tm_min, tm.tm_sec, (int)(es->when.tv_usec / 1000));
      ...
      case 0:
        chunk_appendf(&trash,
                      ...
                      (es->oe && es->oe->cap & PR_CAP_BE) ? es->oe->id
                                                          : "<NONE>",
      ...
      chunk_appendf(&trash,
                    ...
                    es->srv ? es->srv->puid : -1, es->ev_id, pn, port,
    ...
    if (appctx->ctx.errors.ev_id != es->ev_id) {
      ...
      chunk_appendf(&trash, "  WARNING! update detected on this snapshot, dump "
                            "interrupted. Please re-check!\n");
      if (ci_putchk(si_ic(si), &trash) == -1)
        goto cant_send_unlock;
    ...
    while (appctx->ctx.errors.ptr < es->buf_len &&
           appctx->ctx.errors.ptr < global.tune.bufsize) {
      int newptr;
      int newline;

      newline = appctx->ctx.errors.bol;
      newptr = dump_text_line(&trash, es->buf, global.tune.bufsize, es->buf_len,
                              &newline, appctx->ctx.errors.ptr);
...
static struct cli_kw_list cli_kws = {
    ...
    {{{"disable", "frontend", NULL},
     ...
     {{"set", "maxconn", "frontend", NULL},
      ...
      NULL},
     {{"show", "servers", "conn", NULL},
      "show servers conn [id]: dump server connections status (for backend "
      "<id>)",
      cli_parse_show_servers,
      cli_io_handler_servers_state},
     {{"show", "servers", "state", NULL},
     ...
     {{"shutdown", "frontend", NULL},
      "shutdown frontend : stop a specific frontend",
      cli_parse_shutdown_frontend,
      NULL,
      NULL},
     {{"set", "dynamic-cookie-key", "backend", NULL},
      "set dynamic-cookie-key backend : change a backend secret key for "
      "dynamic cookies",
      cli_parse_set_dyncookie_key_backend,
      NULL},
     {{"enable", "dynamic-cookie", "backend", NULL},
      "enable dynamic-cookie backend : enable dynamic cookies on a specific "
      "backend",
      cli_parse_enable_dyncookie_backend,
      NULL},
     {{"disable", "dynamic-cookie", "backend", NULL},
      "disable dynamic-cookie backend : disable dynamic cookies on a specific "
      "backend",
