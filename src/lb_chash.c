 ...
 */
static inline struct eb32_node *chash_skip_node(struct eb_root *root,
 ...
 * algorithm of Mirrokni, Thorup, and Zadimoghaddam (arxiv:1608.01350), adapted
 * for use with unequal server weights.
 */
int chash_server_is_eligible(struct server *s) {
  /* The total number of slots to allocate is the total number of outstanding
   * requests (including the one we're about to make) times the
   * load-balance-factor, rounded up.
   */
  unsigned tot_slots =
      ((s->proxy->served + 1) * s->proxy->lbprm.hash_balance_factor + 99) / 100;
   ...
   */
  slots += ((s->cumulative_weight + s->cur_eweight) * remainder) /
 ...
 */
struct server *chash_get_next_server(struct proxy *p,
...
void chash_init_server_tree(struct proxy *p) {
  ...
  for (srv = p->srv; srv; srv = srv->next) {
    srv->lb_tree =
        (srv->flags & SRV_F_BACKUP) ? &p->lbprm.chash.bck : &p->lbprm.chash.act;
    ...
    for (node = 0; node < srv->lb_nodes_tot; node++) {
      ...
      srv->lb_nodes[node].node.key =
          full_hash(srv->puid * SRV_EWGHT_RANGE + node);
    }
