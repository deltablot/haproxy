...
struct acl_expr *parse_acl_expr(const char **args, char **err,
                                struct arg_list *al, const char *file,
                                int line) {
  ...
  if (aclkw) {
     ...
     */
    nbargs = make_arg_list(arg, -1, smp->fetch->arg_mask, &smp->arg_p, err,
                           &endt, NULL, al);
    if (nbargs < 0) {
    ...
    while (*arg) {
      ...
      if (*arg && *arg != ',') {
        if (ckw)
          memprintf(err,
                    "ACL keyword '%s' : missing comma after converter '%s'.",
                    aclkw->kw, ckw);
        else
          memprintf(err,
                    "ACL keyword '%s' : missing comma after fetch keyword.",
                    aclkw->kw);
        goto out_free_smp;
      }
      ...
      if (!conv) {
        ...
        memprintf(err, "ACL keyword '%s' : unknown converter '%s'.", aclkw->kw,
                  ckw);
        goto out_free_smp;
      }
      ...
      if (conv->in_type >= SMP_TYPES || conv->out_type >= SMP_TYPES) {
        memprintf(
            err,
            "ACL keyword '%s' : returns type of converter '%s' is unknown.",
            aclkw->kw, ckw);
        goto out_free_smp;
      }
      ...
      argcnt = make_arg_list(endw, -1, conv->arg_mask, &conv_expr->arg_p, err,
                             &arg, &err_arg, al);
      if (argcnt < 0) {
        memprintf(err,
                  "ACL keyword '%s' : invalid arg %d in converter '%s' : %s.",
      ...
      if (conv->val_args &&
          !conv->val_args(conv_expr->arg_p, conv, file, line, err)) {
        memprintf(err,
                  "ACL keyword '%s' : invalid args in converter '%s' : %s.",
                  aclkw->kw, ckw, *err);
        goto out_free_smp;
      }
    }
  ...
  if (aclkw && !acl_conv_found) {
    expr->kw = aclkw->kw;
    expr->pat.parse =
        aclkw->parse ? aclkw->parse : pat_parse_fcts[aclkw->match_type];
    expr->pat.index =
        aclkw->index ? aclkw->index : pat_index_fcts[aclkw->match_type];
    expr->pat.match =
        aclkw->match ? aclkw->match : pat_match_fcts[aclkw->match_type];
    expr->pat.prune =
        aclkw->prune ? aclkw->prune : pat_prune_fcts[aclkw->match_type];
  }
  ...
  if (expr->pat.parse && cur_type != SMP_T_BOOL && !*args[1]) {
    ha_warning("parsing acl keyword '%s' :\n"
               "  no pattern to match against were provided, so this ACL will "
               "never match.\n"
               "  If this is what you intended, please add '--' to get rid of "
               "this warning.\n"
               "  If you intended to match only for existence, please use '-m "
               "found'.\n"
               "  If you wanted to force an int to match as a bool, please use "
               "'-m bool'.\n"
               "\n",
               args[0]);
  }
  ...
  while (**args == '-') {
    if (strcmp(*args, "-i") == 0)
      ...
      if (!expr->pat.parse) {
        memprintf(err,
                  "matching method must be specified first (using '-m') when "
                  "using a sample fetch of this type ('%s')",
                  expr->kw);
        goto out_free_expr;
      }

      if (!pattern_read_from_file(&expr->pat, refflags, args[1], patflags,
                                  load_as_map, err, file, line))
        goto out_free_expr;
      is_loaded = 1;
      args++;
      ...
      if (is_loaded) {
        memprintf(err, "'-m' must only be specified before patterns and files "
                       "in parsing ACL expression");
        goto out_free_expr;
      }
      ...
      if (idx < 0) {
        memprintf(err,
                  "unknown matching method '%s' when parsing ACL expression",
                  args[1]);
        goto out_free_expr;
      }

      /* Note: -m found is always valid, bool/int are compatible,
       * str/bin/reg/len are compatible */
      if (idx != PAT_MATCH_FOUND &&
          !sample_casts[cur_type][pat_match_types[idx]]) {
        memprintf(err,
                  "matching method '%s' cannot be used with fetch keyword '%s'",
                  args[1], expr->kw);
        goto out_free_expr;
      }
      ...
      memprintf(err,
                "'%s' is not a valid ACL option. Please use '--' before any "
  ...
  if (!expr->pat.parse) {
    memprintf(err,
              "matching method must be specified first (using '-m') when using "
              "a sample fetch of this type ('%s')",
              expr->kw);
    goto out_free_expr;
  }

  /* Create displayed reference */
  snprintf(trash.area, trash.size, "acl '%s' file '%s' line %d", expr->kw, file,
           line);
  ...
  while (**args) {
    ...
    if (expr->pat.parse == pat_parse_int ||
      ...
      /* If the pattern contain ':' or '-' character, give it to the parser as
       * is. If no contain ':' and operator is STD_OP_EQ, give it to the parser
       * as is. In other case, try to convert the value according with the
       * operator.
       */
      if (!contain_colon && operator!= STD_OP_EQ) {
        ...
        /* convert the integer minor part for the pat_parse_dotted_ver()
         * function. */
        if (expr->pat.parse == pat_parse_dotted_ver && have_dot) {
          if (strl2llrc(dot + 1, strlen(dot + 1), &minor) != 0) {
            memprintf(err, "'%s' is neither a number nor a supported operator",
                      arg);
            goto out_free_expr;
          }
          if (minor >= 65536) {
        ...
        case STD_OP_GE:
          if (expr->pat.parse == pat_parse_int)
            ...
            snprintf(buffer, NB_LLMAX_STR + NB_LLMAX_STR + 2,
                     "%lld.%lld:", value >> 16, value & 0xffff);
          arg = buffer;
          break;
...
struct acl *parse_acl(const char **args, struct list *known_acl, char **err,
                      struct arg_list *al, const char *file, int line) {
  ...
  if (!strchr(args[1], '(') && *args[2] == '(')
    ha_warning(
        ...
        "  not what you want. Maybe you need to remove the extraneous space "
        "before '('.\n"
        "  If you are really sure this is not an error, please insert '--' "
        "between the\n"
...
const struct {
  ...
  const char *expr[4]; /* put enough for longest expression */
    ...
    {.name = "HTTP_CONTENT",
     .expr = {"hdr_val(content-length)", "gt", "0", ""}},
 ...
 */
static struct acl *find_acl_default(const char *acl_name,
                                    struct list *known_acl, char **err,
                                    struct arg_list *al, const char *file,
                                    int line) {
  __label__ out_return, out_free_acl_expr, out_free_name;
  struct acl *cur_acl;
...
struct acl_cond *prune_acl_cond(struct acl_cond *cond) {
  ...
  list_for_each_entry_safe(suite, tmp_suite, &cond->suites, list) {
    list_for_each_entry_safe(term, tmp_term, &suite->terms, list) free(term);
    free(suite);
  }
  return cond;
}
...
struct acl_cond *parse_acl_cond(const char **args, struct list *known_acl,
                                enum acl_cond_pol pol, char **err,
                                struct arg_list *al, const char *file,
                                int line) {
 ...
 * freeable or NULL.
 */
struct acl_cond *build_acl_cond(const char *file, int line,
                                struct list *known_acl, struct proxy *px,
                                const char **args, char **err) {
 ...
 *         res = !res;
 */
enum acl_test_res acl_exec_cond(struct acl_cond *cond, struct proxy *px,
 ...
 */
const struct acl *acl_cond_conflicts(const struct acl_cond *cond,
                                     unsigned int where) {
  struct acl_term_suite *suite;
  struct acl_term *term;
  struct acl *acl;
 ...
 * returned.
 */
int acl_cond_kw_conflicts(const struct acl_cond *cond, unsigned int where,
                          struct acl const **acl, char const **kw) {
  struct acl_term_suite *suite;
  struct acl_term *term;
  struct acl_expr *expr;
    ...
    list_for_each_entry(expr, &acl->expr, list) {
      if (!strcmp(expr->kw, "http_auth_group")) {
        ...
        if (expr->smp->arg_p->unresolved) {
          ha_alert("Internal bug in proxy %s: %sacl %s %s() makes use of "
                   "unresolved userlist '%s'. Please report this.\n",
        ...
        list_for_each_entry(pexp, &expr->pat.head, list) {
          ...
          list_for_each_entry(pattern, &pexp->expr->patterns, list) {
            ...
            if (!check_group(expr->smp->arg_p->data.usr,
                             pattern->pat.ptr.str)) {
              ha_alert("proxy %s: acl %s %s(): invalid group '%s'.\n", p->id,
                       acl->name, expr->kw, pattern->pat.ptr.str);
              cfgerr++;
            }
          }
        }
...
int init_acl() {
  ...
  list_for_each_entry(kwl, &acl_keywords.list, list) {
    for (index = 0; kwl->kw[index].kw != NULL; index++) {
      ...
      if (!smp) {
        ha_alert("Critical internal error: ACL keyword '%s' relies on sample "
                 "fetch '%s' which was not registered!\n",
