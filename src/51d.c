...
static struct {
  char property_separator; /* the separator to use in the response for the
                              values. this is taken from
                              51degrees-property-separator from config. */
  struct list
      property_names; /* list of properties to load into the data set. this is
                         taken from 51degrees-property-name-list from config. */
  char *data_file_path;
  ...
  fiftyoneDegreesDataSet
      data_set; /* data set used with the pattern and trie detection methods. */
#ifdef FIFTYONEDEGREES_H_PATTERN_INCLUDED
  fiftyoneDegreesWorksetPool *
      pool; /* pool of worksets to avoid creating a new one for each request. */
#endif
  ...
  fiftyoneDegreesDeviceOffsets
      device_offsets; /* Memory used for device offsets. */
#endif
#endif
  int cache_size;
} global_51degrees = {
...
static int _51d_property_name_list(char **args, int section_type,
  ...
  if (*(args[cur_arg]) == 0) {
    memprintf(err, "'%s' expects at least one 51Degrees property name.",
              args[0]);
    return -1;
  }
...
static int _51d_property_separator(char **args, int section_type,
                                   struct proxy *curpx, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  if (*(args[1]) == 0) {
    memprintf(err, "'%s' expects a single character.", args[0]);
    return -1;
  }
  if (strlen(args[1]) > 1) {
    memprintf(err, "'%s' expects a single character, got '%s'.", args[0],
              args[1]);
    return -1;
  }
...
static int _51d_cache_size(char **args, int section_type, struct proxy *curpx,
  ...
  if (global_51degrees.cache_size < 0) {
    memprintf(err, "'%s' expects a positive numeric value, got '%s'.", args[0],
              args[1]);
    return -1;
  }
...
static int _51d_fetch_check(struct arg *arg, char **err_msg) {
  ...
  memprintf(
      err_msg,
      "51Degrees data file is not specified (parameter '51degrees-data-file')");
  return 0;
}
  ...
  for (i = 0; i < global_51degrees.header_count; i++) {
    ...
    if (http_find_header(htx, name, &ctx, 1)) {
      (offsets->firstOffset + offsets->size)->httpHeaderOffset =
          *(global_51degrees.header_offsets + i);
      (offsets->firstOffset + offsets->size)->deviceOffset =
          fiftyoneDegreesGetDeviceOffset(&global_51degrees.data_set,
                                         ctx.value.ptr);
      offsets->size++;
    }
  }
 ...
 */
unsigned long long _51d_req_hash(const struct arg *args,
  ...
  for (i = 0; i < ws->importantHeadersCount; i++) {
    ...
    hash ^= XXH64(ws->importantHeaders[i].headerValue,
                  ws->importantHeaders[i].headerValueLength, seed);
  }
  return hash;
}
#endif
...
static void _51d_process_match(const struct arg *args, struct sample *smp,
  ...
  static void _51d_process_match(const struct arg *args, struct sample *smp,
    ...
    const char **requiredProperties =
        fiftyoneDegreesGetRequiredPropertiesNames(&global_51degrees.data_set);
    int requiredPropertiesCount =
        fiftyoneDegreesGetRequiredPropertiesCount(&global_51degrees.data_set);
#endif
    ...
    /* Loop through property names passed to the filter and fetch them from the
     * dataset. */
    while (args[i].data.str.area) {
      ...
      if (strcmp("Method", args[i].data.str.area) == 0) {
        ...
        for (j = 0; j < ws->dataSet->requiredPropertyCount; j++) {
          property_name = fiftyoneDegreesGetPropertyName(
              ws->dataSet, ws->dataSet->requiredProperties[j]);
          if (strcmp(property_name, args[i].data.str.area) == 0) {
            found = 1;
            fiftyoneDegreesSetValues(ws, j);
            chunk_appendf(
                temp, "%s",
                fiftyoneDegreesGetValueName(ws->dataSet, *ws->values));
            break;
          }
        }
      }
      ...
      for (j = 0; j < requiredPropertiesCount; j++) {
        ...
        if (strcmp(property_name, args[i].data.str.area) == 0 &&
            fiftyoneDegreesGetValueFromOffsets(&global_51degrees.data_set,
                                               offsets, j, valuesBuffer,
                                               1024) > 0) {
          found = 1;
  ...
  static int _51d_fetch(const struct arg *args, struct sample *smp,
    ...
    if (_51d_lru_tree) {
      ...
      lru = lru64_get(_51d_req_hash(args, ws), _51d_lru_tree, (void *)args, 0);
  ...
  static int _51d_conv(const struct arg *args, struct sample *smp,
                       void *private) {
#ifdef FIFTYONEDEGREES_H_PATTERN_INCLUDED
    fiftyoneDegreesWorkset *ws; /* workset for detection */
    struct lru64 *lru = NULL;
    ...
    offsets->firstOffset->deviceOffset = fiftyoneDegreesGetDeviceOffset(
        &global_51degrees.data_set, smp->data.u.str.area);
    offsets->size = 1;
    _51d_process_match(args, smp, offsets);
#endif
  ...
  void _51d_init_http_headers() {
    ...
    global_51degrees.header_names =
        malloc(global_51degrees.header_count * sizeof(struct buffer));
    for (index = 0; index < global_51degrees.header_count; index++) {
      headerName =
          fiftyoneDegreesGetString(ds, ds->httpHeaders[index].headerNameOffset);
      (global_51degrees.header_names + index)->area =
          (char *)&headerName->firstByte;
      (global_51degrees.header_names + index)->data = headerName->length - 1;
      (global_51degrees.header_names + index)->size =
  ...
  void _51d_init_http_headers() {
    ...
    global_51degrees.header_names =
        malloc(global_51degrees.header_count * sizeof(struct buffer));
    global_51degrees.header_offsets =
        malloc(global_51degrees.header_count * sizeof(int32_t));
    for (index = 0; index < global_51degrees.header_count; index++) {
      global_51degrees.header_offsets[index] =
          fiftyoneDegreesGetHttpHeaderNameOffset(ds, index);
      global_51degrees.header_names[index].area =
          (char *)fiftyoneDegreesGetHttpHeaderNamePointer(ds, index);
      global_51degrees.header_names[index].data =
          strlen(global_51degrees.header_names[index].area);
      global_51degrees.header_names[index].size =
          global_51degrees.header_names->data;
    }
  }
#endif
  ...
  static int init_51degrees(void) {
    ...
    if (!LIST_ISEMPTY(&global_51degrees.property_names)) {
      i = 0;
      list_for_each_entry(name, &global_51degrees.property_names, list)++ i;
    ...
    _51d_dataset_status = fiftyoneDegreesInitWithPropertyArray(
        global_51degrees.data_file_path, &global_51degrees.data_set,
        (const char **)_51d_property_list, i);
...
#ifdef FIFTYONEDEGREES_H_PATTERN_INCLUDED
      global_51degrees.pool = fiftyoneDegreesWorksetPoolCreate(
          &global_51degrees.data_set, NULL, global.nbthread);
#endif
...
#ifdef FIFTYONEDEGREES_H_PATTERN_INCLUDED
      chunk_printf(temp, "Corrupt data file. Check that the data file provided "
                         "is uncompressed and Pattern data format.");
#endif
#ifdef FIFTYONEDEGREES_H_TRIE_INCLUDED
      chunk_printf(temp, "Corrupt data file. Check that the data file provided "
                         "is uncompressed and Trie data format.");
#endif
      break;
    case DATA_SET_INIT_STATUS_INCORRECT_VERSION:
#ifdef FIFTYONEDEGREES_H_PATTERN_INCLUDED
      chunk_printf(temp, "Incorrect version. Check that the data file provided "
                         "is uncompressed and Pattern data format.");
#endif
#ifdef FIFTYONEDEGREES_H_TRIE_INCLUDED
      chunk_printf(temp, "Incorrect version. Check that the data file provided "
    ...
    case DATA_SET_INIT_STATUS_POINTER_OUT_OF_BOUNDS:
      chunk_printf(temp,
                   "Allocated continuous memory containing 51Degrees data file "
                   "appears to be smaller than expected. Most likely"
                   " because the data file was not fully loaded into the "
                   "allocated memory.");
      break;
  ...
  static void deinit_51degrees(void) {
    ...
    free(global_51degrees.data_file_path);
    global_51degrees.data_file_path = NULL;
    list_for_each_entry_safe(_51d_prop_name, _51d_prop_nameb,
                             &global_51degrees.property_names, list) {
  ...
  static struct sample_fetch_kw_list sample_fetch_keywords = {
      ILH,
      {
          {"51d.all", _51d_fetch, ARG5(1, STR, STR, STR, STR, STR),
           _51d_fetch_check, SMP_T_STR, SMP_USE_HRQHV},
          {NULL, NULL, 0, 0, 0},
      }};
  ...
  /* Note: must not be declared <const> as its list will be overwritten */
  static struct sample_conv_kw_list conv_kws = {
      ILH,
      {
