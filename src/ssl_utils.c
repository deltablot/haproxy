...
/*
 * Extract the DN in the specified format from the X509_NAME and copy result to
 * a chunk. Currently supports rfc2253 for returning LDAP V3 DNs. Returns 1 if
 * dn entries exist, 0 if no dn entry was found.
 */
 ...
 * Returns 1 if dn entries exits, 0 if no dn entry found or -1 if output is not
 * large enough.
 */
