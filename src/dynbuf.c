...
int init_buffer() {
  ...
  pool_head_buffer =
      create_pool("buffer", global.tune.bufsize, MEM_F_SHARED | MEM_F_EXACT);
  if (!pool_head_buffer)
    return 0;
...
void buffer_dump(FILE *o, struct buffer *b, int from, int to) {
  ...
  fprintf(o, "            orig=%p size=%u head=%u tail=%u data=%u\n", b_orig(b),
          (unsigned int)b_size(b), (unsigned int)b_head_ofs(b),
          (unsigned int)b_tail_ofs(b), (unsigned int)b_data(b));
  ...
  while (from < to) {
    ...
    for (i = 0; (from + i < to) && (i < 16); i++) {
      fprintf(o, "%c",
              isprint((unsigned char)b_orig(b)[from + i]) ? b_orig(b)[from + i]
                                                          : '.');
...
void __offer_buffer(void *from, unsigned int threshold) {
   ...
   * buffers.
   */
  avail = pool_head_buffer->allocated - pool_head_buffer->used -
