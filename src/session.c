...
static struct task *session_expire_embryonic(struct task *t, void *context,
 ...
 */
struct session *session_new(struct proxy *fe, struct listener *li,
...
void session_free(struct session *sess) {
  ...
  list_for_each_entry_safe(srv_list, srv_list_back, &sess->srv_list, srv_list) {
    list_for_each_entry_safe(conn, conn_back, &srv_list->conn_list,
                             session_list) {
...
int session_accept_fd(struct connection *cli_conn) {
  ...
  if ((l->options & LI_O_TCP_L4_RULES) && !tcp_exec_l4_rules(sess)) {
    ...
    setsockopt(cfd, SOL_SOCKET, SO_LINGER, (struct linger *)&nolinger,
...
#ifdef TCP_KEEPCNT
      if (p->clitcpka_cnt)
        setsockopt(cfd, IPPROTO_TCP, TCP_KEEPCNT, &p->clitcpka_cnt,
                   sizeof(p->clitcpka_cnt));
#endif

#ifdef TCP_KEEPIDLE
      if (p->clitcpka_idle)
        setsockopt(cfd, IPPROTO_TCP, TCP_KEEPIDLE, &p->clitcpka_idle,
                   sizeof(p->clitcpka_idle));
#endif
  ...
  if (global.tune.client_sndbuf)
    setsockopt(cfd, SOL_SOCKET, SO_SNDBUF, &global.tune.client_sndbuf,
               sizeof(global.tune.client_sndbuf));

  if (global.tune.client_rcvbuf)
    setsockopt(cfd, SOL_SOCKET, SO_RCVBUF, &global.tune.client_rcvbuf,
               sizeof(global.tune.client_rcvbuf));
...
static void session_kill_embryonic(struct session *sess, unsigned short state) {
  ...
  if (log && (sess->fe->options & PR_O_NULLNOLOG)) {
    ...
    if (!conn->err_code || conn->err_code == CO_ER_PRX_EMPTY ||
  ...
  if (log) {
    ...
    if (err_msg)
      ...
      send_log(sess->fe, level,
               "%s: unknown connection error (code=%d flags=%08x)\n",
