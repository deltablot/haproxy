...
struct pool_head *create_pool(char *name, unsigned int size,
  ...
  if (!pool) {
    ...
    if (!pool) {
      ...
      for (entry = pool_base_start; entry != pool_base_start + MAX_BASE_POOLS;
           entry++) {
        if (!entry->size) {
          pool = entry;
          break;
...
void *__pool_refill_alloc(struct pool_head *pool, unsigned int avail) {
  ...
  while (1) {
    ...
    swrate_add_scaled(&pool->needed_avg, POOL_AVG_SAMPLES, pool->allocated,
                      POOL_AVG_SAMPLES / 4);
...
void pool_gc(struct pool_head *pool_ctx) {
  ...
  list_for_each_entry(entry, &pools, list) {
    while ((int)((volatile int)entry->allocated - (volatile int)entry->used) >
           (int)entry->minavail) {
      struct pool_free_list cmp, new;
...
void pool_destroy_all() {
  ...
  list_for_each_entry_safe(entry, back, &pools, list) pool_destroy(entry);
}
...
#endif
    chunk_appendf(&trash,
                  "  - Pool %s (%u bytes) : %u allocated (%u bytes), %u used, "
                  "needed_avg %u, %u failures, %u users, @%p=%02d%s\n",
                  entry->name, entry->size, entry->allocated,
                  entry->size * entry->allocated, entry->used,
                  swrate_avg(entry->needed_avg, POOL_AVG_SAMPLES),
                  entry->failed, entry->users, entry,
                  (int)pool_get_index(entry),
...
int pool_total_failures() {
  ...
  list_for_each_entry(entry, &pools, list) failed += entry->failed;
  return failed;
}

/* This function returns the total amount of memory allocated in pools (in
 * bytes) */
unsigned long pool_total_allocated() {
  struct pool_head *entry;
  unsigned long allocated = 0;

  list_for_each_entry(entry, &pools, list) allocated +=
...
unsigned long pool_total_used() {
  ...
  list_for_each_entry(entry, &pools, list) used += entry->used * entry->size;
  return used;
}
 ...
 */
void create_pool_callback(struct pool_head **ptr, char *name,
                          unsigned int size) {
...
int mem_should_fail(const struct pool_head *pool) {
  ...
  n = snprintf(&mem_fail_str[mem_fail_cur_idx * MEM_FAIL_MAX_CHAR],
               MEM_FAIL_MAX_CHAR - 2, "%d %.18s %d %d", mem_fail_cur_idx,
               pool->name, ret, tid);
...
static int mem_parse_global_fail_alloc(char **args, int section_type,
...
static struct cfg_kw_list mem_cfg_kws = {
    ILH,
    {
#ifdef DEBUG_FAIL_ALLOC
