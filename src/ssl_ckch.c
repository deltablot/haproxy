...
/* Try to load a sctl from a buffer <buf> if not NULL, or read the file
 * <sctl_path> It fills the ckch->sctl buffer return 0 on success or != 0 on
 * failure */
int ssl_sock_load_sctl_from_file(const char *sctl_path, char *buf,
                                 struct cert_key_and_chain *ckch, char **err) {
  int fd = -1;
  int r = 0;
  int ret = 1;
  struct buffer tmp;
 ...
 */
int ssl_sock_load_ocsp_response_from_file(const char *ocsp_path, char *buf,
 ...
 *      1 on SSL Failure
 */
int ssl_sock_load_files_into_ckch(const char *path,
                                  struct cert_key_and_chain *ckch, char **err) {
  ...
  if (!X509_check_private_key(ckch->cert, ckch->key)) {
    memprintf(
        err,
        "%sinconsistencies between private key and certificate loaded '%s'.\n",
        err && *err ? *err : "", path);
    goto end;
  }
  ...
  if (global_ssl.extra_files & SSL_GF_SCTL) {
    ...
    if (stat(fp->area, &st) == 0) {
      if (ssl_sock_load_sctl_from_file(fp->area, NULL, ckch, err)) {
        memprintf(err,
                  "%s '%s.sctl' is present but cannot be read or parsed'.\n",
                  err && *err ? *err : "", fp->area);
        ret = 1;
        goto end;
      }
 ...
 */
int ssl_sock_load_key_into_ckch(const char *path, char *buf,
                                struct cert_key_and_chain *ckch, char **err) {
  BIO *in = NULL;
  int ret = 1;
  EVP_PKEY *key = NULL;
 ...
 *  Return 0 on success or != 0 on failure
 */
int ssl_sock_load_pem_into_ckch(const char *path, char *buf,
                                struct cert_key_and_chain *ckch, char **err) {
  BIO *in = NULL;
  int ret = 1;
  X509 *ca;
  ...
  /* Read Private Key */
  key = PEM_read_bio_PrivateKey(in, NULL, NULL, NULL);
  /* no need to check for errors here, because the private key could be loaded
   * later */
 ...
 */
void ssl_sock_free_cert_key_and_chain_contents(
    struct cert_key_and_chain *ckch) {
  if (!ckch)
    return;
...
ssl_sock_copy_cert_key_and_chain(struct cert_key_and_chain *src,
 ...
 * return 0 on success or != 0 on failure
 */
int ssl_sock_load_issuer_file_into_ckch(const char *path, char *buf,
                                        struct cert_key_and_chain *ckch,
                                        char **err) {
  int ret = 1;
  BIO *in = NULL;
...
/*
 * This function allocate a ckch_store and populate it with certificates from
 * files.
 */
...
struct {
  const char *ext;
  int type;
  int (*load)(const char *path, char *payload, struct cert_key_and_chain *ckch,
              char **err);
  /* add a parsing callback */
} cert_exts[CERT_TYPE_MAX + 1] = {
    [CERT_TYPE_PEM] =
        {"", CERT_TYPE_PEM,
         &ssl_sock_load_pem_into_ckch}, /* default mode, no extensions */
    [CERT_TYPE_KEY] = {"key", CERT_TYPE_KEY, &ssl_sock_load_key_into_ckch},
#if ((defined SSL_CTRL_SET_TLSEXT_STATUS_REQ_CB &&                             \
      !defined OPENSSL_NO_OCSP) ||                                             \
     defined OPENSSL_IS_BORINGSSL)
    [CERT_TYPE_OCSP] = {"ocsp", CERT_TYPE_OCSP,
                        &ssl_sock_load_ocsp_response_from_file},
#endif
#if (HA_OPENSSL_VERSION_NUMBER >= 0x1000200fL && !defined OPENSSL_NO_TLSEXT && \
     !defined OPENSSL_IS_BORINGSSL)
    [CERT_TYPE_SCTL] = {"sctl", CERT_TYPE_SCTL, &ssl_sock_load_sctl_from_file},
#endif
    [CERT_TYPE_ISSUER] = {"issuer", CERT_TYPE_ISSUER,
...
static int cli_parse_show_cert(char **args, char *payload,
...
error:
  HA_SPIN_UNLOCK(CKCH_LOCK, &ckch_lock);
  return cli_err(appctx, "Can't display the certificate: Not found or the "
                         "certificate is a bundle!\n");
}

/* release function of the  `set ssl cert' command, free things and unlock the
 * spinlock */
static void cli_release_commit_cert(struct appctx *appctx) {
  ...
  if (appctx->st2 != SETCERT_ST_FIN) {
    /* free every new sni_ctx and the new store, which are not in the trees so
     * no spinlock there */
    new_ckchs = appctx->ctx.ssl.new_ckchs;

    /* if the allocation failed, we need to free everything from the temporary
     * list */
...
static int cli_io_handler_commit_cert(struct appctx *appctx) {
  ...
  while (1) {
    ...
    case SETCERT_ST_GEN:
      ...
      /* walk through the old ckch_inst and creates new ckch_inst using the
       * updated ckchs */
      list_for_each_entry_from(ckchi, &old_ckchs->ckch_inst, by_ckchs) {
        ...
        /* it takes a lot of CPU to creates SSL_CTXs, so we yield every 10 CKCH
         * instances */
        if (y >= 10) {
        ...
        errcode |= ckch_inst_new_load_store(
            new_ckchs->path, new_ckchs, ckchi->bind_conf, ckchi->ssl_conf,
            sni_filter, fcount, &new_inst, &err);
        ...
        list_for_each_entry_safe(sc0, sc0s, &new_inst->sni_ctx, by_ckch_inst) {
          if (!sc0->order) { /* we initialized only the first SSL_CTX because
                                it's the same in the other sni_ctx's */
            errcode |= ssl_sock_prepare_ctx(ckchi->bind_conf, ckchi->ssl_conf,
                                            sc0->ctx, &err);
            if (errcode & ERR_CODE)
              goto error;
          }
    ...
    case SETCERT_ST_INSERT:
      ...
      /* get the list of crtlist_entry in the old store, and update the pointers
       * to the store */
      ...
      /* First, we insert every new SNIs in the trees, also replace the
       * default_ctx */
 ...
 */
static int cli_parse_commit_cert(char **args, char *payload,
                                 struct appctx *appctx, void *private) {
  char *err = NULL;
  ...
  if (strcmp(ckchs_transaction.path, args[3]) != 0) {
    memprintf(&err,
              "The ongoing transaction is about '%s' but you are trying to set "
              "'%s'\n",
  ...
  if (ckchs_transaction.new_ckchs->ckch->cert &&
      !ckchs_transaction.new_ckchs->ckch->key) {
    memprintf(&err, "The transaction must contain at least a certificate and a "
                    "private key!\n");
    goto error;
  }

  if (!X509_check_private_key(ckchs_transaction.new_ckchs->ckch->cert,
                              ckchs_transaction.new_ckchs->ckch->key)) {
    memprintf(
        &err,
        "inconsistencies between private key and certificate loaded '%s'.\n",
  ...
  /* we don't unlock there, it will be unlock after the IO handler, in the
   * release handler */
  return 0;
 ...
 */
static int cli_parse_set_cert(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  ...
  if (!*args[3] || !payload)
    return cli_err(
        appctx,
        "'set ssl cert expects a filename and a certificate as a payload\n");
  ...
  if (ckchs_transaction.path) {
    ...
    if (strcmp(ckchs_transaction.path, buf->area) != 0) {
      ...
      /* if the del-ext option is activated we should try to take a look at a
       * ".crt" too. */
      if (type != CERT_TYPE_PEM && global_ssl.extra_files_noext) {
        ...
        if (strcmp(ckchs_transaction.path, buf->area) != 0) {
          ...
          *(b_orig(buf) + b_data(buf) + strlen(".crt")) = '\0';
          ...
          memprintf(&err,
                    "The ongoing transaction is about '%s' but you are trying "
                    "to set '%s'\n",
  ...
  if (!appctx->ctx.ssl.old_ckchs) {
    memprintf(&err,
              "%sCan't replace a certificate which is not referenced by the "
              "configuration!\n",
              err ? err : "");
    errcode |= ERR_ALERT | ERR_FATAL;
    goto end;
...
static int cli_parse_abort_cert(char **args, char *payload,
                                struct appctx *appctx, void *private) {
  char *err = NULL;
  ...
  if (strcmp(ckchs_transaction.path, args[3]) != 0) {
    memprintf(&err,
              "The ongoing transaction is about '%s' but you are trying to "
              "abort a transaction for '%s'\n",
              ckchs_transaction.path, args[3]);
    goto error;
  }

  /* Only free the ckchs there, because the SNI and instances were not generated
   * yet */
  ckch_store_free(ckchs_transaction.new_ckchs);
  ckchs_transaction.new_ckchs = NULL;
...
static int cli_parse_new_cert(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  struct ckch_store *store;
  char *err = NULL;
  char *path;
...
static int cli_parse_del_cert(char **args, char *payload, struct appctx *appctx,
                              void *private) {
  struct ckch_store *store;
  char *err = NULL;
  char *filename;
...
static struct cli_kw_list cli_kws = {
    {},
    {{{"new", "ssl", "cert", NULL},
      "new ssl cert <certfile> : create a new certificate file to be used in a "
      "crt-list or a directory",
      cli_parse_new_cert,
      NULL,
      NULL},
     {{"set", "ssl", "cert", NULL},
     ...
     {{"del", "ssl", "cert", NULL},
      ...
      NULL},
     {{"show", "ssl", "cert", NULL},
      "show ssl cert [<certfile>] : display the SSL certificates used in "
      "memory, or the details of a <certfile>",
      cli_parse_show_cert,
