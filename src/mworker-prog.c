    ...
    if (child->reloads == 0 && (child->options & PROC_O_TYPE_PROG)) {
      ...
      if (reexec && (!(child->options & PROC_O_START_RELOAD))) {
        ...
        list_for_each_entry(old_child, &proc_list, list) {
          if (!(old_child->options & PROC_O_TYPE_PROG) ||
      ...
      if (ret < 0) {
        ...
        if (child->gid != -1) {
          if (getgroups(0, NULL) > 0 && setgroups(0, NULL) == -1)
            ha_warning("[%s.main()] Failed to drop supplementary groups. Using "
                       "'gid'/'group'"
...
int cfg_parse_program(const char *file, int linenum, char **args, int kwm) {
  ...
  if (!strcmp(args[0], "program")) {
    ...
    ext_child->options |=
        PROC_O_START_RELOAD; /* restart the programs by default */
    ext_child->uid = -1;
    ext_child->gid = -1;
    ...
    list_for_each_entry(child, &proc_list, list) {
      if (child->reloads == 0 && (child->options & PROC_O_TYPE_PROG)) {
        if (!strcmp(args[1], child->id)) {
          ha_alert("parsing [%s:%d]: '%s' program section already exists in "
                   "the configuration.\n",
                   file, linenum, args[1]);
          err_code |= ERR_ALERT | ERR_ABORT;
          goto error;
    ...
    if (*(args[1]) == 0) {
      ha_alert("parsing [%s:%d]: '%s' expects a command with optional "
               "arguments separated in words.\n",
               file, linenum, args[0]);
