...
int srv_downtime(const struct server *s) {
  if ((s->cur_state != SRV_ST_STOPPED) ||
      s->last_change >= now.tv_sec) // ignore negative time
    return s->down_time;
...
int srv_getinter(const struct check *check) {
  ...
  if ((check->state & CHK_ST_CONFIGURED) &&
...
static inline void srv_check_for_dup_dyncookie(struct server *s) {
  ...
  for (tmpserv = p->srv; tmpserv != NULL; tmpserv = tmpserv->next) {
    if (tmpserv == s)
      continue;
    if (tmpserv->next_admin & SRV_ADMF_FMAINT)
      continue;
    if (tmpserv->cookie && strcmp(tmpserv->cookie, s->cookie) == 0) {
...
void srv_set_dyncookie(struct server *s) {
  ...
  if ((s->flags & SRV_F_COOKIESET) || !(s->proxy->ck_opts & PR_CK_DYNAMIC) ||
    ...
    for (index = 0; kwl->kw[index].kw != NULL; index++) {
      if (kwl->kw[index].parse ||
        ...
        memprintf(out, "%s[%4s] %s%s%s%s\n", *out ? *out : "", kwl->scope,
                  kwl->kw[index].kw, kwl->kw[index].skip ? " <arg>" : "",
...
static int srv_parse_backup(char **args, int *cur_arg, struct proxy *curproxy,
...
static int srv_parse_cookie(char **args, int *cur_arg, struct proxy *curproxy,
                            struct server *newsrv, char **err) {
  char *arg;
...
static int srv_parse_disabled(char **args, int *cur_arg, struct proxy *curproxy,
                              struct server *newsrv, char **err) {
...
static int srv_parse_enabled(char **args, int *cur_arg, struct proxy *curproxy,
                             struct server *newsrv, char **err) {
  newsrv->next_admin &= ~SRV_ADMF_CMAINT & ~SRV_ADMF_FMAINT;
...
static int srv_parse_max_reuse(char **args, int *cur_arg,
                               struct proxy *curproxy, struct server *newsrv,
                               char **err) {
  char *arg;
...
static int srv_parse_pool_purge_delay(char **args, int *cur_arg,
  ...
  if (res == PARSE_TIME_OVER) {
    memprintf(err,
              "timer overflow in argument '%s' to '%s' (maximum value is "
              "2147483647 ms or ~24.8 days)",
              args[*cur_arg + 1], args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  } else if (res == PARSE_TIME_UNDER) {
    memprintf(err,
              "timer underflow in argument '%s' to '%s' (minimum non-null "
              "value is 1 ms)",
              args[*cur_arg + 1], args[*cur_arg]);
    return ERR_ALERT | ERR_FATAL;
  } else if (res) {
...
static int srv_parse_pool_low_conn(char **args, int *cur_arg,
                                   struct proxy *curproxy,
                                   struct server *newsrv, char **err) {
  char *arg;
...
static int srv_parse_pool_max_conn(char **args, int *cur_arg,
                                   struct proxy *curproxy,
                                   struct server *newsrv, char **err) {
  char *arg;
...
static int srv_parse_id(char **args, int *cur_arg, struct proxy *curproxy,
                        struct server *newsrv, char **err) {
  ...
  if (node) {
    ...
    memprintf(err, "'%s' : custom id %d already used at %s:%d ('server %s')",
              args[*cur_arg], newsrv->puid, target->conf.file,
              target->conf.line, target->id);
    return ERR_ALERT | ERR_FATAL;
  }
...
static int srv_parse_no_tfo(char **args, int *cur_arg, struct proxy *curproxy,
                            struct server *newsrv, char **err) {
...
static int srv_parse_proto(char **args, int *cur_arg, struct proxy *px,
                           struct server *newsrv, char **err) {
  struct ist proto;
  ...
  if (!newsrv->mux_proto) {
    memprintf(err, "'%s' :  unknown MUX protocol '%s'", args[*cur_arg],
              args[*cur_arg + 1]);
    return ERR_ALERT | ERR_FATAL;
  }
  return 0;
...
static int srv_parse_observe(char **args, int *cur_arg, struct proxy *curproxy,
                             struct server *newsrv, char **err) {
  char *arg;
  ...
  if (!strcmp(arg, "none")) {
    ...
    memprintf(err,
              "'%s' expects one of 'none', 'layer4', 'layer7' "
              "but got '%s'\n",
              args[*cur_arg], arg);
    return ERR_ALERT | ERR_FATAL;
  }
...
static int srv_parse_redir(char **args, int *cur_arg, struct proxy *curproxy,
                           struct server *newsrv, char **err) {
  char *arg;
...
static int srv_parse_source(char **args, int *cur_arg, struct proxy *curproxy,
                            struct server *newsrv, char **err) {
  char *errmsg;
  ...
  if (!*args[*cur_arg + 1]) {
    memprintf(
        err,
        "'%s' expects <addr>[:<port>[-<port>]], and optionally '%s' <addr>, "
  ...
  if (port_low != port_high) {
    ...
    newsrv->conn_src.sport_range =
        port_range_alloc_range(port_high - port_low + 1);
  ...
  while (*(args[*cur_arg])) {
    if (!strcmp(args[*cur_arg], "usesrc")) { /* address to use outside */
      ...
      if (!strcmp(args[*cur_arg + 1], "client")) {
        ...
        /* 'sk' is statically allocated (no need to be freed). */
        sk = str2sa_range(
            args[*cur_arg + 1], NULL, &port1, &port2, NULL, NULL, &errmsg, NULL,
            NULL, PA_O_RESOLVE | PA_O_PORT_OK | PA_O_STREAM | PA_O_CONNECT);
        if (!sk) {
          ha_alert("'%s %s' : %s\n", args[*cur_arg], args[*cur_arg + 1],
                   errmsg);
          goto err;
      ...
      *cur_arg += 2;
      ...
      ha_alert("'usesrc' not allowed here because support for TPROXY was not "
               "compiled in.\n");
      goto err;
...
static int srv_parse_stick(char **args, int *cur_arg, struct proxy *curproxy,
                           struct server *newsrv, char **err) {
  newsrv->flags &= ~SRV_F_NON_STICK;
  return 0;
}
...
static int srv_parse_track(char **args, int *cur_arg, struct proxy *curproxy,
                           struct server *newsrv, char **err) {
  char *arg;
...
static int srv_parse_socks4(char **args, int *cur_arg, struct proxy *curproxy,
                            struct server *newsrv, char **err) {
  char *errmsg;
  int port_low, port_high;
  struct sockaddr_storage *sk;
  ...
  sk = str2sa_range(args[*cur_arg + 1], NULL, &port_low, &port_high, NULL, NULL,
                    &errmsg, NULL, NULL,
                    PA_O_RESOLVE | PA_O_PORT_OK | PA_O_PORT_MAND | PA_O_STREAM |
                        PA_O_CONNECT);
  if (!sk) {
    memprintf(err, "'%s %s' : %s\n", args[*cur_arg], args[*cur_arg + 1],
              errmsg);
...
static int srv_parse_tfo(char **args, int *cur_arg, struct proxy *px,
                         struct server *newsrv, char **err) {
  newsrv->flags |= SRV_F_FASTOPEN;
  return 0;
}
...
void srv_shutdown_streams(struct server *srv, int why) {
  ...
  list_for_each_entry_safe(stream, stream_bck, &srv->actconns,
                           by_srv) if (stream->srv_conn == srv)
...
void srv_append_status(struct buffer *msg, struct server *s,
  ...
  if (xferred >= 0) {
    if (s->next_state == SRV_ST_STOPPED)
      chunk_appendf(msg,
                    ". %d active and %d backup servers left.%s"
      ...
      chunk_appendf(msg,
                    ". %d active and %d backup servers online.%s"
...
void srv_set_admin_flag(struct server *s, enum srv_admin mode,
  ...
  /* stop going down if the equivalent flag was already present (forced or
   * inherited) */
...
void srv_clr_admin_flag(struct server *s, enum srv_admin mode) {
  ...
  /* stop going down if the equivalent flag is still present (forced or
   * inherited) */
 ...
 */
static struct srv_kw_list srv_kws = {
    "ALL",
    {},
    {
        {"backup", srv_parse_backup, 0, 1}, /* Flag as backup server */
        {"cookie", srv_parse_cookie, 1, 1}, /* Assign a cookie to the server */
        {"disabled", srv_parse_disabled, 0,
         1}, /* Start the server in 'disabled' state */
        {"enabled", srv_parse_enabled, 0,
         1},                        /* Start the server in 'enabled' state */
        {"id", srv_parse_id, 1, 0}, /* set id# of server */
        {"max-reuse", srv_parse_max_reuse, 1,
         1}, /* Set the max number of requests on a connection, -1 means
                unlimited */
        {"namespace", srv_parse_namespace, 1,
         1}, /* Namespace the server socket belongs to (if supported) */
        {"no-backup", srv_parse_no_backup, 0,
         1}, /* Flag as non-backup server */
        {"no-send-proxy", srv_parse_no_send_proxy, 0,
         1}, /* Disable use of PROXY V1 protocol */
        {"no-send-proxy-v2", srv_parse_no_send_proxy_v2, 0,
         1}, /* Disable use of PROXY V2 protocol */
        {"no-tfo", srv_parse_no_tfo, 0, 1}, /* Disable use of TCP Fast Open */
        {"non-stick", srv_parse_non_stick, 0,
         1}, /* Disable stick-table persistence */
        {"observe", srv_parse_observe, 1,
         1}, /* Enables health adjusting based on observing communication with
                the server */
        {"pool-low-conn", srv_parse_pool_low_conn, 1,
         1}, /* Set the min number of orphan idle connecbefore being allowed to
                pick from other threads */
        {"pool-max-conn", srv_parse_pool_max_conn, 1,
         1}, /* Set the max number of orphan idle connections, 0 means unlimited
              */
        {"pool-purge-delay", srv_parse_pool_purge_delay, 1,
         1}, /* Set the time before we destroy orphan idle connections, defaults
                to 1s */
        {"proto", srv_parse_proto, 1,
         1}, /* Set the proto to use for all outgoing connections */
        {"proxy-v2-options", srv_parse_proxy_v2_options, 1,
         1},                              /* options for send-proxy-v2 */
        {"redir", srv_parse_redir, 1, 1}, /* Enable redirection mode */
        {"send-proxy", srv_parse_send_proxy, 0,
         1}, /* Enforce use of PROXY V1 protocol */
        {"send-proxy-v2", srv_parse_send_proxy_v2, 0,
         1}, /* Enforce use of PROXY V2 protocol */
        {"source", srv_parse_source, -1,
         1}, /* Set the source address to be used to connect to the server */
        {"stick", srv_parse_stick, 0, 1}, /* Enable stick-table persistence */
        {"tfo", srv_parse_tfo, 0, 1},     /* enable TCP Fast Open of server */
        {"track", srv_parse_track, 1,
         1}, /* Set the current state of the server, tracking another one */
        {"socks4", srv_parse_socks4, 1,
         1}, /* Set the socks4 proxy of the server*/
        {NULL, NULL, 0},
    }};
 ...
 * time, and the proxy's algorithm. To be used after updating sv->uweight. The
 * warmup state is automatically disabled if the time is elapsed. If
 * <must_update> is not zero, the update will be propagated immediately.
 *
 * Must be called with the server lock held.
 */
void server_recalc_eweight(struct server *sv, int must_update) {
  ...
  /* We must take care of not pushing the server to full throttle during slow
   * starts. It must also start immediately, at least at the minimal step when
   * leaving maintenance.
   */
  if ((sv->next_state == SRV_ST_STARTING) && (px->lbprm.algo & BE_LB_PROP_DYN))
    w = (px->lbprm.wdiv * (now.tv_sec - sv->last_change) + sv->slowstart) /
        sv->slowstart;
  else
    w = px->lbprm.wdiv;
...
const char *server_parse_weight_change_request(struct server *sv,
  ...
  if (w && w != sv->iweight && !(px->lbprm.algo & BE_LB_PROP_DYN))
    return "Backend is using a static LB algorithm and only accepts weights "
           "'0%' and '100%'.\n";
...
static struct sample_expr *srv_sni_sample_parse_expr(struct server *srv,
  ...
  return sample_parse_expr((char **)args, &idx, file, linenum, err,
                           &px->conf.args, NULL);
}

static int server_parse_sni_expr(struct server *newsrv, struct proxy *px,
                                 char **err) {
  struct sample_expr *expr;

  expr =
      srv_sni_sample_parse_expr(newsrv, px, px->conf.file, px->conf.line, err);
  if (!expr) {
  ...
  if (!(expr->fetch->val & SMP_VAL_BE_SRV_CON)) {
    memprintf(err,
              "error detected while parsing sni expression : "
...
static void display_parser_err(const char *file, int linenum, char **args,
                               int cur_arg, int err_code, char **err) {
  ...
  if (err_code & ERR_WARN && !(err_code & ERR_ALERT))
    ha_warning("parsing [%s:%d] : '%s %s' : %s%s%s%s.\n", file, linenum,
               args[0], args[1], msg, quote, token, quote);
  else
    ha_alert("parsing [%s:%d] : '%s %s' : %s%s%s%s.\n", file, linenum, args[0],
             args[1], msg, quote, token, quote);
}
...
/*
 * Copy <src> server connection source settings to <srv> server everything
 * needed.
 */
...
static void srv_ssl_settings_cpy(struct server *srv, struct server *src) {
  ...
  if (src->ssl_ctx.npn_str) {
    ...
    if (srv->ssl_ctx.npn_str) {
      memcpy(srv->ssl_ctx.npn_str, src->ssl_ctx.npn_str, src->ssl_ctx.npn_len);
 ...
 * May be safely called with a default server as <src> argument (without
 * hostname). Returns -1 in case of any allocation failure, 0 if not.
 */
static int srv_prepare_for_resolution(struct server *srv,
                                      const char *hostname) {
  char *hostname_dn;
  ...
  hostname_dn_len =
      dns_str_to_dn_label(hostname, hostname_len + 1, hostname_dn, trash.size);
...
static void srv_settings_cpy(struct server *srv, struct server *src,
  ...
  memcpy(srv->dns_opts.pref_net, src->dns_opts.pref_net,
...
#ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
static int server_sni_expr_init(const char *file, int linenum, char **args,
                                int cur_arg, struct server *srv,
                                struct proxy *proxy) {
  int ret;
  char *err = NULL;
 ...
 */
static int server_finalize_init(const char *file, int linenum, char **args,
                                int cur_arg, struct server *srv,
                                struct proxy *px) {
#ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
  int ret;
  ...
  if (srv->do_check && srv->trackit) {
    ha_alert("parsing [%s:%d]: unable to enable checks and tracking at the "
             "same time!\n",
             file, linenum);
    return ERR_ALERT | ERR_FATAL;
  }
  ...
  if (srv->do_agent && !srv->agent.port) {
    ha_alert("parsing [%s:%d] : server %s does not have agent port. Agent "
 ...
 */
static int srv_tmpl_parse_range(struct server *srv, const char *arg,
...
static inline void srv_set_id_from_prefix(struct server *srv,
...
static int server_template_init(struct server *srv, struct proxy *px) {
  ...
  for (i = srv->tmpl_info.nb_low + 1; i <= srv->tmpl_info.nb_high; i++) {
    ...
    if (newsrv->sni_expr) {
      newsrv->ssl_ctx.sni =
          srv_sni_sample_parse_expr(newsrv, px, NULL, 0, NULL);
...
int parse_server(const char *file, int linenum, char **args,
                 struct proxy *curproxy, struct proxy *defproxy, int parse_addr,
                 int in_peers_section, int initial_resolve) {
  ...
  if (!strcmp(args[0], "server") || !strcmp(args[0], "peer") ||
    ...
    if (srv && parse_addr) {
      if (!*args[2]) {
        if (in_peers_section) {
          ...
          ha_alert("parsing [%s:%d] : '%s' expects <name> and <addr>[:<port>] "
                   "as arguments.\n",
                   file, linenum, args[0]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
      ...
      if (!*args[3]) {
        ...
        ha_alert("parsing [%s:%d] : '%s' expects <prefix> <nb | range> "
    ...
    if (err) {
      ha_alert(
          "parsing [%s:%d] : character '%c' is not permitted in %s %s '%s'.\n",
    ...
    if (srv_tmpl) {
      ...
      if (srv_tmpl_parse_range(newsrv, args[cur_arg], &tmpl_range_low,
                               &tmpl_range_high) < 0) {
        ha_alert("parsing [%s:%d] : Wrong %s number or range arg '%s'.\n", file,
                 linenum, args[0], args[cur_arg]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
      cur_arg++;
    ...
    if (!defsrv) {
       ...
       * This is a temporary id which will be used for server allocations to
       * come after parsing.
       */
      if (srv)
      ...
      sk = str2sa_range(args[cur_arg], &port, &port1, &port2, NULL, NULL,
                        &errmsg, NULL, &fqdn,
                        (initial_resolve ? PA_O_RESOLVE : 0) | PA_O_PORT_OK |
                            PA_O_PORT_OFS | PA_O_STREAM | PA_O_XPRT |
                            PA_O_CONNECT);
      if (!sk) {
      ...
      if (fqdn) {
        if (fqdn[0] == '_') { /* SRV record */
          ...
          ha_alert(
              "parsing [%s:%d] : Can't create DNS resolution for server '%s'\n",
      ...
      if (!newsrv->srvrq && !newsrv->hostname &&
        ...
        ha_alert("parsing [%s:%d] : Unknown protocol family %d '%s'\n", file,
                 linenum, newsrv->addr.ss_family, args[cur_arg]);
        err_code |= ERR_ALERT | ERR_FATAL;
        goto out;
      }
      ...
      if (!strcmp(args[cur_arg], "init-addr")) {
        ...
        for (p = args[cur_arg + 1]; *p; p = end) {
          ...
          if (!strcmp(p, "libc")) {
                ...
                srv_append_initaddr(&newsrv->init_addr_methods, SRV_IADDR_LIBC);
          } else if (!strcmp(p, "last")) {
            done =
                srv_append_initaddr(&newsrv->init_addr_methods, SRV_IADDR_LAST);
          } else if (!strcmp(p, "none")) {
            done =
                srv_append_initaddr(&newsrv->init_addr_methods, SRV_IADDR_NONE);
          } else if (str2ip2(p, &sa, 0)) {
            if (is_addr(&newsrv->init_addr)) {
              ha_alert("parsing [%s:%d]: '%s' : initial address already "
                       "specified, cannot add '%s'.\n",
            ...
            newsrv->init_addr = sa;
                ...
                srv_append_initaddr(&newsrv->init_addr_methods, SRV_IADDR_IP);
          } else {
            ha_alert("parsing [%s:%d]: '%s' : unknown init-addr method '%s', "
                     "supported methods are 'libc', 'last', 'none'.\n",
                     file, linenum, args[cur_arg], p);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
          }
          if (!done) {
            ha_alert("parsing [%s:%d]: '%s' : too many init-addr methods when "
                     "trying to add '%s'\n",
                     file, linenum, args[cur_arg], p);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
        ...
        for (p = args[cur_arg + 1]; *p; p = end) {
          ...
          if (!strcmp(p, "allow-dup-ip")) {
            ...
            ha_alert("parsing [%s:%d]: '%s' : unknown resolve-opts option "
                     "'%s', supported methods are 'allow-dup-ip', "
                     "'ignore-weight', and 'prevent-dup-ip'.\n",
                     file, linenum, args[cur_arg], p);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
          }
        ...
        if (!strcmp(args[cur_arg + 1], "ipv4"))
          ...
          ha_alert("parsing [%s:%d]: '%s' expects either ipv4 or ipv6 as "
                   "argument.\n",
        ...
        while (*p != '\0') {
          ...
          if (opt->pref_net_nb >= SRV_MAX_PREF_NET) {
            ha_alert("parsing [%s:%d]: '%s' exceed %d networks.\n", file,
                     linenum, args[cur_arg], SRV_MAX_PREF_NET);
            err_code |= ERR_ALERT | ERR_FATAL;
            goto out;
          }
        ...
        if (w < 0 || w > SRV_UWGHT_MAX) {
          ha_alert("parsing [%s:%d] : weight of server %s is not within 0 and "
                   "%d (%d).\n",
        ...
        if (err == PARSE_TIME_OVER) {
          ha_alert(
              "parsing [%s:%d]: timer overflow in argument <%s> to <%s> of "
              "server %s, maximum value is 2147483647 ms (~24.8 days).\n",
              file, linenum, args[cur_arg + 1], args[cur_arg], newsrv->id);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        } else if (err == PARSE_TIME_UNDER) {
          ha_alert("parsing [%s:%d]: timer underflow in argument <%s> to <%s> "
                   "of server %s, minimum non-null value is 1 ms.\n",
                   file, linenum, args[cur_arg + 1], args[cur_arg], newsrv->id);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        } else if (err) {
          ha_alert("parsing [%s:%d] : unexpected character '%c' in 'slowstart' "
                   "argument of server %s.\n",
        ...
        if (!strcmp(args[cur_arg + 1], "shutdown-sessions"))
          ...
          ha_alert("parsing [%s:%d]: '%s' expects 'shutdown-sessions' but got "
                   "'%s'\n",
                   file, linenum, args[cur_arg], args[cur_arg + 1]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        if (!strcmp(args[cur_arg + 1], "shutdown-backup-sessions"))
          ...
          ha_alert("parsing [%s:%d]: '%s' expects 'shutdown-backup-sessions' "
                   "but got '%s'\n",
                   file, linenum, args[cur_arg], args[cur_arg + 1]);
          err_code |= ERR_ALERT | ERR_FATAL;
          goto out;
        ...
        cur_arg += 2;
      } else if (!strcmp(args[cur_arg], "usesrc")) { /* address to use outside:
                                                        needs "source" first */
        ha_alert(
            "parsing [%s:%d] : '%s' only allowed after a '%s' statement.\n",
        ...
        if (kw) {
          ...
          if (!kw->parse) {
            ha_alert("parsing [%s:%d] : '%s %s' : '%s' option is not "
                     "implemented in this version (check build options).\n",
          ...
          if (defsrv && !kw->default_ok) {
            ha_alert("parsing [%s:%d] : '%s %s' : '%s' option is not accepted "
                     "in default-server sections.\n",
                     file, linenum, args[0], args[1], args[cur_arg]);
            if (kw->skip != -1)
              cur_arg += 1 + kw->skip;
            err_code |= ERR_ALERT;
        ...
        ha_alert("parsing [%s:%d] : '%s %s' unknown keyword '%s'.%s%s\n", file,
                 linenum, args[0], args[1], args[cur_arg],
    ...
    if (!defsrv)
      err_code |=
          server_finalize_init(file, linenum, args, cur_arg, newsrv, curproxy);
...
struct server *server_find_best_match(struct proxy *bk, char *name, int id,
                                      int *diff) {
...
static void srv_update_state(struct server *srv, int version, char **params) {
  ...
  case 1:
    ...
    if ((p == params[1]) || errno == EINVAL || errno == ERANGE ||
        (srv_op_state != SRV_ST_STOPPED && srv_op_state != SRV_ST_STARTING &&
         srv_op_state != SRV_ST_RUNNING && srv_op_state != SRV_ST_STOPPING)) {
    ...
    if ((p == params[2]) || errno == EINVAL || errno == ERANGE ||
        (srv_admin_state != 0 && srv_admin_state != SRV_ADMF_FMAINT &&
    ...
    if ((p == params[5]) || errno == EINVAL || errno == ERANGE)
      chunk_appendf(msg, ", invalid srv_last_time_change value '%s'",
                    params[5]);
    ...
    if (fqdn && *fqdn == '-')
      fqdn = NULL;
    if (fqdn &&
        (strlen(fqdn) > DNS_MAX_NAME_SIZE || invalid_domainchar(fqdn))) {
    ...
    /* configuration is the same, let's compate old running state and new conf
       state */
    else {
    ...
    /* We want to apply the previous 'running' weight (srv_uweight) only if
     * there was no change in the configuration: both previous and new iweight
     * are equals
     *
     * It means that a configuration file change has precedence over a unix
     * socket change for server's weight
     *
     * by default, HAProxy applies the following weight when parsing the
     * configuration srv->uweight = srv->iweight
     */
    ...
    if (fqdn && srv->hostname) {
      if (!strcmp(srv->hostname, fqdn)) {
         ...
         * - FQDN changed again from file configuration (with the same previous
         value set from stats socket, from foo1 to foo2 value),
         * - reload for any other reason than a FQDN modification,
         * the configuration file FQDN matches the fqdn server state file value.
         * So we must reset the 'set from stats socket FQDN' flag to be
         consistent with
     ...
     */
      ...
      if (srv->srvrq && strcmp(srv->srvrq->name, srvrecord) != 0) {
        chunk_appendf(
            msg,
            ", SRV record mismatch between configuration ('%s') and state file "
            "('%s) for server '%s'. Previous state not applied",
            srv->srvrq->name, srvrecord, srv->id);
        HA_SPIN_UNLOCK(SERVER_LOCK, &srv->lock);
        goto out;
      }
      ...
      if (srv->srvrq == NULL &&
          (srv->srvrq = find_srvrq_by_name(srvrecord, srv->proxy)) == NULL)
        srv->srvrq = new_dns_srvrq(srv, srvrecord);
      if (srv->srvrq == NULL) {
        chunk_appendf(
            msg, ", can't create or find SRV resolution '%s' for server '%s'",
            srvrecord, srv->id);
        HA_SPIN_UNLOCK(SERVER_LOCK, &srv->lock);
        goto out;
      }
      ...
      if (res == -1) {
        chunk_appendf(
            msg, ", can't allocate memory for DNS resolution for server '%s'",
            srv->id);
        HA_SPIN_UNLOCK(SERVER_LOCK, &srv->lock);
        goto out;
      }
 ...
 * found. If no version is found, then 0 is returned Note that this should be
 * the first read on <f>
 */
static int srv_state_get_version(FILE *f) {
  char buf[2];
  ...
  /* first character of first line of the file must contain the version of the
   * export */
  ...
  ret = atoi(buf);
  if ((ret < SRV_STATE_FILE_VERSION_MIN) || (ret > SRV_STATE_FILE_VERSION_MAX))
    return 0;
...
/* This function parses all the proxies and only take care of the backends
 * (since we're looking for server) For each proxy, it does the following:
 ...
 *
 * If the running backend uuid or id differs from the state file, then HAProxy
 * reports a warning.
 *
...
out_load_server_state_in_tree:
  ...
  /* parse all proxies and load states form tree (global file) or from local
   * file */
  for (curproxy = proxies_list; curproxy != NULL; curproxy = curproxy->next) {
    ...
    case PR_SRV_STATE_FILE_LOCAL:
      ...
      /* absolute path or no base directory provided */
      if ((curproxy->server_state_file_name[0] == '/') ||
          (!global.server_state_base)) {
        ...
        memcpy(localfilepath + localfilepathlen,
               curproxy->server_state_file_name, len);
        localfilepathlen += len;
        localfilepath[localfilepathlen++] = 0;
      }
     ...
     * Backend name can't be wrong since it's used as a key to retrieve the
     * server state line from the tree.
     */
    if (curproxy->load_server_state_from_file == PR_SRV_STATE_FILE_GLOBAL) {
      ...
      /* first character of first line of the file must contain the version of
       * the export */
      ...
      while (fgets(mybuf, SRV_STATE_LINE_MAXLEN, f)) {
        ...
        if (!check_id && !check_name)
          ...
          ha_warning("backend ID mismatch: from server state file: '%s', from "
                     "running config '%d'\n",
                     params[0], bk->uuid);
          send_log(bk, LOG_NOTICE,
                   "backend ID mismatch: from server state file: '%s', from "
                   "running config '%d'\n",
                   params[0], bk->uuid);
        } else if (check_id && !check_name) {
          ha_warning("backend name mismatch: from server state file: '%s', "
                     "from running config '%s'\n",
                     params[1], bk->id);
          send_log(bk, LOG_NOTICE,
                   "backend name mismatch: from server state file: '%s', from "
                   "running config '%s'\n",
                   params[1], bk->id);
          /* if name doesn't match, we still want to update curproxy if the
           * backend id was forced in previous the previous configuration */
          if (!bk_f_forced_id)
            continue;
        }
  ...
  for (node = ebmb_first(&state_file),
      next_node = node ? ebmb_next(node) : NULL;
       node; node = next_node, next_node = node ? ebmb_next(node) : NULL) {
    st = container_of(node, struct state_line, name_name);
 ...
 * updater is a string which contains an information about the requester of the
 * update. updater is used if not NULL.
 *
 * A log line and a stderr warning message is generated based on server's
 * backend options.
 *
 * Must be called with the server lock held.
 */
int update_server_addr(struct server *s, void *ip, int ip_sin_family,
                       const char *updater) {
  ...
  if (1) {
    ...
    case AF_INET:
      inet_ntop(s->addr.ss_family, &((struct sockaddr_in *)&s->addr)->sin_addr,
                oldip, INET_ADDRSTRLEN);
      break;
    case AF_INET6:
      inet_ntop(s->addr.ss_family,
                &((struct sockaddr_in6 *)&s->addr)->sin6_addr, oldip,
                INET6_ADDRSTRLEN);
      break;
    default:
      strcpy(oldip, "(none)");
 ...
 * families.
 *
 * Caller can pass its name through <updater> to get it integrated in the
 * response message returned by the function.
 *
 ...
 * Must be called with the server lock held.
 */
const char *update_server_addr_port(struct server *s, const char *addr,
                                    const char *port, char *updater) {
  struct sockaddr_storage sa;
  ...
  if (addr) {
    ...
    if ((sa.ss_family != AF_INET) && (sa.ss_family != AF_INET6)) {
      chunk_printf(msg, "Update to families other than AF_INET and AF_INET6 "
                        "supported only through configuration file");
      goto out;
    }
    ...
    if ((ret != AF_INET) && (ret != AF_INET6)) {
      chunk_printf(msg, "Update for the current server address family is only "
                        "supported through configuration file");
      goto out;
    }
  ...
  if (port) {
    ...
    if (sign == '-' || (sign == '+')) {
      ...
      if (!(s->flags & SRV_F_MAPPORTS)) {
         ...
         * prevent PORT change if check doesn't have it's dedicated port while
         * switching to port mapping */
        if ((s->check.state & CHK_ST_CONFIGURED) &&
            !(s->flags & SRV_F_CHECKPORT)) {
          chunk_appendf(
              msg, "can't change <port> to port map because it is incompatible "
                   "with current health check port configuration (use 'port' "
                   "statement from the 'server' directive.");
          goto out;
        }
      }
    ...
    if (port_change_required) {
      ...
      /* we also need to update health checks port only if it uses server's
       * realport */
      if ((s->check.state & CHK_ST_CONFIGURED) &&
          !(s->flags & SRV_F_CHECKPORT)) {
...
int snr_update_srv_status(struct server *s, int has_no_ip) {
  ...
  case RSLV_STATUS_VALID:
    ...
    if (has_no_ip) {
      if (s->next_admin & SRV_ADMF_RMAINT)
        return 1;
      srv_set_admin_flag(s, SRV_ADMF_RMAINT, "No IP for server ");
      return 0;
    }
    ...
    chunk_printf(
        &trash,
        "Server %s/%s administratively READY thanks to valid DNS answer",
        s->proxy->id, s->id);
 ...
 *  - update with first new ip found if family is met and current IP is not
 * found returns: 0 on error 1 when no error or safe ignore
 *
 * Must be called with server lock held
 */
int snr_resolution_cb(struct dns_requester *requester,
  ...
  ret = dns_get_ip_from_response(&resolution->response, &s->dns_opts, serverip,
                                 server_sin_family, &firstip,
 ...
 * Must be called with server lock held
 */
struct server *snr_check_ip_callback(struct server *srv, void *ip,
  ...
  for (tmpsrv = be->srv; tmpsrv; tmpsrv = tmpsrv->next) {
     ...
     *   * the hostname used for the resolution by our server is the same than
     * the one used for the server found in the backend
    ...
    if (*ip_family == tmpsrv->addr.ss_family &&
        ((tmpsrv->addr.ss_family == AF_INET &&
          memcmp(ip, &((struct sockaddr_in *)&tmpsrv->addr)->sin_addr, 4) ==
              0) ||
         (tmpsrv->addr.ss_family == AF_INET6 &&
          memcmp(ip, &((struct sockaddr_in6 *)&tmpsrv->addr)->sin6_addr, 16) ==
              0))) {
      HA_SPIN_UNLOCK(SERVER_LOCK, &tmpsrv->lock);
      return tmpsrv;
    }
...
int srv_set_fqdn(struct server *srv, const char *hostname, int dns_locked) {
  ...
  if (resolution && resolution->hostname_dn &&
...
static int srv_iterate_initaddr(struct server *srv) {
  ...
  while (methods) {
    ...
    case SRV_IADDR_NONE:
      ...
      if (return_code) {
        ha_warning("parsing [%s:%d] : 'server %s' : could not resolve address "
                   "'%s', disabling server.\n",
    ...
    case SRV_IADDR_IP:
      ...
      if (return_code) {
        ha_warning("parsing [%s:%d] : 'server %s' : could not resolve address "
                   "'%s', falling back to configured address.\n",
                   srv->conf.file, srv->conf.line, srv->id, name);
      }
      goto out;
  ...
  if (!return_code) {
    ha_alert("parsing [%s:%d] : 'server %s' : no method found to resolve "
             "address '%s'\n",
             srv->conf.file, srv->conf.line, srv->id, name);
  } else {
    ha_alert(
        "parsing [%s:%d] : 'server %s' : could not resolve address '%s'.\n",
        srv->conf.file, srv->conf.line, srv->id, name);
  }
 ...
 * Must be called with the server lock held.
 */
const char *update_server_fqdn(struct server *server, const char *fqdn,
                               const char *updater, int dns_locked) {
  ...
  chunk_appendf(msg, "%s/%s changed its FQDN from %s to %s", server->proxy->id,
                server->id, server->hostname, fqdn);

  if (srv_set_fqdn(server, fqdn, dns_locked) < 0) {
    chunk_reset(msg);
    chunk_appendf(msg, "could not update %s/%s FQDN", server->proxy->id,
                  server->id);
    goto out;
  }
...
/* Expects to find a backend and a server in <arg> under the form
 * <backend>/<server>, and returns the pointer to the server. Otherwise, display
 * adequate error messages on the CLI, sets the CLI's state to CLI_ST_PRINT and
 * returns NULL. This is only used for CLI commands requiring a server name.
...
static int cli_parse_set_server(char **args, char *payload,
  ...
  if (strcmp(args[3], "weight") == 0) {
    ...
    if (strl2irc(args[4], strlen(args[4]), &i) != 0) {
      cli_err(
          appctx,
          "'set server <srv> check-port' expects an integer as argument.\n");
      goto out_unlock;
    }
    if ((i < 0) || (i > 65535)) {
    ...
    char *port = NULL;
    if (strlen(args[4]) == 0) {
      cli_err(appctx, "set server <b>/<s> addr requires an address and "
                      "optionally a port.\n");
      goto out_unlock;
    } else {
      addr = args[4];
...
static int cli_parse_get_weight(char **args, char *payload,
                                struct appctx *appctx, void *private) {
 ...
 * Grabs the server lock.
 */
static int cli_parse_set_weight(char **args, char *payload,
                                struct appctx *appctx, void *private) {
  struct server *sv;
  const char *warning;
 ...
 * Grabs the server lock.
 */
static int cli_parse_set_maxconn_server(char **args, char *payload,
                                        struct appctx *appctx, void *private) {
  struct server *sv;
  const char *warning;
 ...
 * Grabs the server lock.
 */
static int cli_parse_disable_agent(char **args, char *payload,
                                   struct appctx *appctx, void *private) {
  struct server *sv;
 ...
 * Grabs the server lock.
 */
static int cli_parse_disable_health(char **args, char *payload,
                                    struct appctx *appctx, void *private) {
  struct server *sv;
 ...
 * Grabs the server lock.
 */
static int cli_parse_disable_server(char **args, char *payload,
                                    struct appctx *appctx, void *private) {
  struct server *sv;
 ...
 * Grabs the server lock.
 */
static int cli_parse_enable_agent(char **args, char *payload,
                                  struct appctx *appctx, void *private) {
  struct server *sv;
 ...
 * Grabs the server lock.
 */
static int cli_parse_enable_health(char **args, char *payload,
                                   struct appctx *appctx, void *private) {
  struct server *sv;
 ...
 * Grabs the server lock.
 */
static int cli_parse_enable_server(char **args, char *payload,
                                   struct appctx *appctx, void *private) {
  struct server *sv;
...
static struct cli_kw_list cli_kws = {
    ...
    {{{"disable", "agent", NULL},
     ...
     {{"enable", "server", NULL},
      ...
      NULL},
     {{"set", "maxconn", "server", NULL},
      "set maxconn server : change a server's maxconn setting",
      cli_parse_set_maxconn_server,
      NULL},
     {{"set", "server", NULL},
      "set server     : change a server's state, weight, address or ssl",
...
static void srv_update_status(struct server *s) {
  ...
  int srv_was_stopping =
      (s->cur_state == SRV_ST_STOPPING) || (s->cur_admin & SRV_ADMF_DRAIN);
  int log_level;
  ...
  if (!(s->cur_admin & SRV_ADMF_MAINT)) {
    ...
    if ((s->cur_state != SRV_ST_STOPPED) && (s->next_state == SRV_ST_STOPPED)) {
      ...
      if (tmptrash) {
        chunk_printf(tmptrash, "%sServer %s/%s is DOWN",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id);
        ...
        send_email_alert(s, log_level, "%s", tmptrash->area);
    ...
    } else if ((s->cur_state != SRV_ST_STOPPING) &&
               (s->next_state == SRV_ST_STOPPING)) {
      s->last_change = now.tv_sec;
      if (s->proxy->lbprm.set_server_status_down)
        s->proxy->lbprm.set_server_status_down(s);
      ...
      tmptrash = alloc_trash_chunk();
      if (tmptrash) {
        chunk_printf(tmptrash, "%sServer %s/%s is stopping",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id);
      ...
      if (prev_srv_count && s->proxy->srv_bck == 0 && s->proxy->srv_act == 0)
        set_backend_down(s->proxy);
    } else if (((s->cur_state != SRV_ST_RUNNING) &&
                (s->next_state == SRV_ST_RUNNING)) ||
               ((s->cur_state != SRV_ST_STARTING) &&
                (s->next_state == SRV_ST_STARTING))) {
      ...
      if (s->cur_state == SRV_ST_STOPPED &&
          s->last_change < now.tv_sec) // ignore negative times
      ...
      tmptrash = alloc_trash_chunk();
      if (tmptrash) {
        chunk_printf(tmptrash, "%sServer %s/%s is UP",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id);
        ...
        ha_warning("%s.\n", tmptrash->area);
        send_log(s->proxy, LOG_NOTICE, "%s.\n", tmptrash->area);
        send_email_alert(s, LOG_NOTICE, "%s", tmptrash->area);
        free_trash_chunk(tmptrash);
        tmptrash = NULL;
      }
  ...
  *s->op_st_chg.reason = 0;
  ...
  if (!(s->cur_admin & SRV_ADMF_MAINT) && (s->next_admin & SRV_ADMF_MAINT)) {
    ...
    if (s->cur_state == SRV_ST_STOPPED) { /* server was already down */
      ...
      if (tmptrash) {
        ...
        srv_append_status(tmptrash, s, NULL, xferred,
                          (s->next_admin & SRV_ADMF_FMAINT));
  ...
  } else if ((s->cur_admin & SRV_ADMF_MAINT) &&
    ...
    if ((!s->track || s->track->next_state != SRV_ST_STOPPED) &&
        (!(s->agent.state & CHK_ST_ENABLED) ||
         (s->agent.health >= s->agent.rise)) &&
    ...
    if (tmptrash) {
      if (!(s->next_admin & SRV_ADMF_FMAINT) &&
          (s->cur_admin & SRV_ADMF_FMAINT)) {
        chunk_printf(tmptrash,
                     "%sServer %s/%s is %s/%s (leaving forced maintenance)",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id, (s->next_state == SRV_ST_STOPPED) ? "DOWN" : "UP",
                     (s->next_admin & SRV_ADMF_DRAIN) ? "DRAIN" : "READY");
      }
      if (!(s->next_admin & SRV_ADMF_RMAINT) &&
          (s->cur_admin & SRV_ADMF_RMAINT)) {
        chunk_printf(
            tmptrash, "%sServer %s/%s ('%s') is %s/%s (resolves again)",
            s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id, s->id,
            s->hostname, (s->next_state == SRV_ST_STOPPED) ? "DOWN" : "UP",
            (s->next_admin & SRV_ADMF_DRAIN) ? "DRAIN" : "READY");
      }
      if (!(s->next_admin & SRV_ADMF_IMAINT) &&
          (s->cur_admin & SRV_ADMF_IMAINT)) {
        chunk_printf(tmptrash, "%sServer %s/%s is %s/%s (leaving maintenance)",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id, (s->next_state == SRV_ST_STOPPED) ? "DOWN" : "UP",
                     (s->next_admin & SRV_ADMF_DRAIN) ? "DRAIN" : "READY");
      }
    ...
    if (!(s->next_admin & SRV_ADMF_FMAINT) &&
      ...
      if (tmptrash) {
        chunk_printf(tmptrash,
                     "%sServer %s/%s is leaving forced maintenance but remains "
                     "in maintenance",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id);

        if (s->track) /* normally it's mandatory here */
          chunk_appendf(tmptrash, " via %s/%s", s->track->proxy->id,
                        s->track->id);
        ha_warning("%s.\n", tmptrash->area);
        send_log(s->proxy, LOG_NOTICE, "%s.\n", tmptrash->area);
        free_trash_chunk(tmptrash);
    ...
    } else if (!(s->next_admin & SRV_ADMF_IMAINT) &&
               (s->cur_admin & SRV_ADMF_IMAINT)) {
      tmptrash = alloc_trash_chunk();
      if (tmptrash) {
        chunk_printf(tmptrash, "%sServer %s/%s remains in forced maintenance",
                     s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                     s->id);
        ha_warning("%s.\n", tmptrash->area);
        send_log(s->proxy, LOG_NOTICE, "%s.\n", tmptrash->area);
    ...
    /* don't report anything when leaving drain mode and remaining in
     * maintenance */
  ...
  if (!(s->next_admin & SRV_ADMF_MAINT)) {
    if (!(s->cur_admin & SRV_ADMF_DRAIN) && (s->next_admin & SRV_ADMF_DRAIN)) {
      ...
      if (prev_srv_count && s->proxy->srv_bck == 0 && s->proxy->srv_act == 0)
        set_backend_down(s->proxy);
    } else if ((s->cur_admin & SRV_ADMF_DRAIN) &&
               !(s->next_admin & SRV_ADMF_DRAIN)) {
      ...
      if (tmptrash) {
        if (!(s->next_admin & SRV_ADMF_FDRAIN)) {
          chunk_printf(tmptrash, "%sServer %s/%s is %s (leaving forced drain)",
                       s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                       s->id,
                       (s->next_state == SRV_ST_STOPPED) ? "DOWN" : "UP");
        } else {
          chunk_printf(tmptrash, "%sServer %s/%s is %s (leaving drain)",
                       s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                       s->id,
                       (s->next_state == SRV_ST_STOPPED) ? "DOWN" : "UP");
          if (s->track) /* normally it's mandatory here */
      ...
      if (tmptrash) {
        if (!(s->next_admin & SRV_ADMF_FDRAIN)) {
          chunk_printf(tmptrash,
                       "%sServer %s/%s is leaving forced drain but remains in "
                       "drain mode",
                       s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                       s->id);
          ...
          chunk_printf(tmptrash, "%sServer %s/%s remains in forced drain mode",
                       s->flags & SRV_F_BACKUP ? "Backup " : "", s->proxy->id,
                       s->id);
        }
        ha_warning("%s.\n", tmptrash->area);
...
struct task *srv_cleanup_toremove_connections(struct task *task, void *context,
 ...
 */
static int srv_migrate_conns_to_remove(struct mt_list *idle_list,
                                       struct mt_list *toremove_list,
                                       int toremove_nb) {
...
static void srv_cleanup_connections(struct server *srv) {
  ...
  for (i = tid;;) {
    did_remove = 0;
    if (srv_migrate_conns_to_remove(&srv->idle_conns[i],
                                    &idle_conns[i].toremove_conns, -1) > 0)
      did_remove = 1;
    if (srv_migrate_conns_to_remove(&srv->safe_conns[i],
                                    &idle_conns[i].toremove_conns, -1) > 0)
      did_remove = 1;
    if (did_remove)
...
struct task *srv_cleanup_idle_connections(struct task *task, void *context,
  ...
  while (1) {
    ...
    exceed_conns = srv->curr_used_conns + curr_idle -
                   MAX(srv->max_used_conns, srv->est_need_conns);
    ...
    for (i = tid;;) {
      ...
      max_conn = (exceed_conns * srv->curr_idle_thr[i]) / curr_idle + 1;

      j = srv_migrate_conns_to_remove(&srv->idle_conns[i],
                                      &idle_conns[i].toremove_conns, max_conn);
      if (j > 0)
        did_remove = 1;
      if (max_conn - j > 0 &&
          srv_migrate_conns_to_remove(&srv->safe_conns[i],
...
static int cfg_parse_idle_pool_shared(char **args, int section_type,
...
static int cfg_parse_pool_fd_ratio(char **args, int section_type,
                                   struct proxy *curpx, struct proxy *defpx,
                                   const char *file, int line, char **err) {
  int arg = -1;
  ...
  if (arg < 0 || arg > 100) {
    memprintf(err, "'%s' expects an integer argument between 0 and 100.",
              args[0]);
    return -1;
  }
