 ...
 */
static enum act_return tcp_action_req_set_src(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
    ...
    if (smp) {
      ...
      if (smp->data.type == SMP_T_IPV4) {
        ...
        ((struct sockaddr_in *)cli_conn->src)->sin_addr.s_addr =
            smp->data.u.ipv4.s_addr;
        ((struct sockaddr_in *)cli_conn->src)->sin_port = port;
      } else if (smp->data.type == SMP_T_IPV6) {
        ((struct sockaddr_in6 *)cli_conn->src)->sin6_family = AF_INET6;
        memcpy(&((struct sockaddr_in6 *)cli_conn->src)->sin6_addr,
               &smp->data.u.ipv6, sizeof(struct in6_addr));
 ...
 * previous family was neither AF_INET nor AF_INET6, the port is set to zero.
 */
static enum act_return tcp_action_req_set_dst(struct act_rule *rule,
                                              struct proxy *px,
                                              struct session *sess,
                                              struct stream *s, int flags) {
  struct connection *cli_conn;
  ...
  if ((cli_conn = objt_conn(sess->origin)) && conn_get_dst(cli_conn)) {
    ...
    if (smp) {
      ...
      if (smp->data.type == SMP_T_IPV4) {
        ...
        memcpy(&((struct sockaddr_in6 *)cli_conn->dst)->sin6_addr,
               &smp->data.u.ipv6, sizeof(struct in6_addr));
...
tcp_action_req_set_src_port(struct act_rule *rule, struct proxy *px,
 ...
 */
static enum act_return
tcp_action_req_set_dst_port(struct act_rule *rule, struct proxy *px,
                            struct session *sess, struct stream *s, int flags) {
  struct connection *cli_conn;
  ...
  if ((cli_conn = objt_conn(sess->origin)) && conn_get_dst(cli_conn)) {
    ...
    if (smp) {
      if (cli_conn->dst->ss_family == AF_INET6) {
        ((struct sockaddr_in6 *)cli_conn->dst)->sin6_port =
            htons(smp->data.u.sint);
      } else {
        ...
        ((struct sockaddr_in *)cli_conn->dst)->sin_port =
            htons(smp->data.u.sint);
      }
    }
  }
...
/* Executes the "silent-drop" action. May be called from
 * {tcp,http}{request,response} */
static enum act_return tcp_exec_action_silent_drop(struct act_rule *rule,
                                                   struct proxy *px,
                                                   struct session *sess,
                                                   struct stream *strm,
                                                   int flags) {
...
#ifdef TCP_REPAIR
  if (setsockopt(conn->handle.fd, SOL_TCP, TCP_REPAIR, &one, sizeof(one)) ==
      0) {
...
static enum act_parse_ret tcp_parse_set_src_dst(const char **args,
...
/* Parse a "silent-drop" action. It takes no argument. It returns ACT_RET_PRS_OK
 * on success, ACT_RET_PRS_ERR on error.
 */
static enum act_parse_ret tcp_parse_silent_drop(const char **args,
                                                int *orig_arg, struct proxy *px,
                                                struct act_rule *rule,
                                                char **err) {
  rule->action = ACT_CUSTOM;
...
static struct action_kw_list tcp_req_conn_actions = {
    ILH,
...
static struct action_kw_list tcp_req_sess_actions = {
    ILH,
    {{"set-src", tcp_parse_set_src_dst},
     {"set-src-port", tcp_parse_set_src_dst},
     {"set-dst", tcp_parse_set_src_dst},
...
static struct action_kw_list tcp_res_cont_actions = {
    ILH, {{"silent-drop", tcp_parse_silent_drop}, {/* END */}}};
