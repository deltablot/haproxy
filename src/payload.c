 ...
 */
static int smp_fetch_wait_end(const struct arg *args, struct sample *smp,
...
static int smp_fetch_len(const struct arg *args, struct sample *smp,
                         const char *kw, void *private) {
  if (smp->strm) {
    struct channel *chn = ((smp->opt & SMP_OPT_DIR) == SMP_OPT_DIR_RES)
                              ? &smp->strm->res
                              : &smp->strm->req;
    ...
    smp->data.u.sint =
        ((check->cs && IS_HTX_CS(check->cs)) ? (htxbuf(&check->bi))->data
                                             : b_data(&check->bi));
  } else
    return 0;
 ...
 */
static int smp_fetch_req_ssl_st_ext(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  ...
  if (ext_len > 32 ||
      ext_len > (hs_len - 35)) /* check for correct session_id len */
    goto not_ssl_hello;
  ...
  if (hs_len < 4 || /* minimum one cipher */
      (ext_len = (data[0] << 8) + data[1]) <
          2 || /* minimum 2 bytes for a cipher */
      ext_len > hs_len)
    goto not_ssl_hello;
 ...
 */
static int smp_fetch_req_ssl_ec_ext(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  int hs_len, ext_len, bleft;
  struct channel *chn;
  unsigned char *data;
...
static int smp_fetch_ssl_hello_type(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  int hs_len;
  int hs_type, bleft;
  struct channel *chn;
 ...
 */
static int smp_fetch_req_ssl_ver(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  ...
  if ((*data >= 0x14 && *data <= 0x17) || (*data == 0xFF)) {
    ...
    version =
        (data[1] << 16) + data[2];      /* record layer version: major, minor */
    ...
    version =
        (data[4] << 16) + data[5]; /* client hello version: major, minor */
  } else {
 ...
 */
static int smp_fetch_ssl_hello_sni(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  int hs_len, ext_len, bleft;
  struct channel *chn;
  unsigned char *data;
 ...
 */
static int smp_fetch_ssl_hello_alpn(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  int hs_len, ext_len, bleft;
  struct channel *chn;
  unsigned char *data;
  ...
  while (hs_len >= 4) {
    ...
    if (ext_type == 16) { /* ALPN */
      if (ext_len <
          3) /* one list length [uint16] + at least one name length [uint8] */
        goto not_ssl_hello;
      ...
      smp->data.type = SMP_T_STR;
      smp->data.u.str.area =
          (char *)data + name_offset + 1; /* +1 to skip name_len */
      smp->data.u.str.data = name_len;
      smp->flags = SMP_F_VOLATILE | SMP_F_CONST;
 ...
 * <clen> is empty (cname is then ignored). It returns the data into sample
 * <smp> of type SMP_T_CSTR. Note: this decoder only works with non-wrapping
 * data.
 */
int fetch_rdp_cookie_name(struct stream *s, struct sample *smp,
                          const char *cname, int clen) {
  int bleft;
  const unsigned char *data;
 ...
 */
int smp_fetch_rdp_cookie(const struct arg *args, struct sample *smp,
                         const char *kw, void *private) {
  if (!smp->strm)
    return 0;
...
static int smp_fetch_rdp_cookie_cnt(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  int ret;
...
static int smp_fetch_payload_lv(const struct arg *arg_p, struct sample *smp,
                                const char *kw, void *private) {
  ...
  /* buf offset could be absolute or relative to len offset + len size if
   * prefixed by + or - */
...
static int smp_fetch_payload(const struct arg *arg_p, struct sample *smp,
                             const char *kw, void *private) {
  ...
  smp->data.type = SMP_T_BIN;
  smp->flags = SMP_F_VOLATILE | SMP_F_CONST;
  chunk_initlen(&smp->data.u.str, head + buf_offset, 0,
                buf_size ? buf_size : (data - buf_offset));
...
/* This function is used to validate the arguments passed to a "payload_lv"
 * fetch keyword. This keyword allows two positive integers and an optional
 * signed one, with the second one being strictly positive and the third one
 * being greater than the opposite of the two others if negative. It is assumed
 * that the types are already the correct ones. Returns 0 on error, non-zero if
 * OK. If <err_msg> is not NULL, it will be filled with a pointer to an error
 * message in case of error, that the caller is responsible for freeing. The
 * initial location must either be freeable or NULL.
 *
 * Note that offset2 is stored with SINT type, but its not directly usable as
 * is. The value is contained in the 63 MSB and the LSB is used as a flag for
 * marking the "relative" property of the value.
 */
...
static int smp_fetch_distcc_param(const struct arg *arg_p, struct sample *smp,
                                  const char *kw, void *private) {
  ...
  while (1) {
    if (ofs + 12 > ci_data(chn)) {
      ...
      if (ofs + 12 <=
          channel_recv_limit(chn) + b_orig(&chn->buf) - ci_head(chn))
        goto too_short;
...
static int smp_fetch_distcc_body(const struct arg *arg_p, struct sample *smp,
                                 const char *kw, void *private) {
  unsigned int match_tok = arg_p[0].data.sint;
  unsigned int match_occ = arg_p[1].data.sint;
  unsigned int token;
      ...
      if (!match_occ || match_occ == occ) {
        ...
        if (ofs + body > ci_head(chn) - b_orig(&chn->buf) + ci_data(chn)) {
          ...
          if (ofs + body >
            ...
            smp->flags |= SMP_F_MAY_CHANGE;
            body = channel_recv_limit(chn) + b_orig(&chn->buf) - ci_head(chn) -
                   ofs;
          }
        }
 ...
 * "distcc_body" sample fetch keyword. They take a mandatory token name of
 * exactly 4 characters, followed by an optional occurrence number starting
 * at 1. It is assumed that the types are already the correct ones. Returns 0 on
 * error, non- zero if OK. If <err_msg> is not NULL, it will be filled with a
...
static struct sample_fetch_kw_list smp_kws = {
    ILH,
    {
        {"distcc_body", smp_fetch_distcc_body, ARG2(1, STR, SINT), val_distcc,
         SMP_T_BIN, SMP_USE_L6REQ | SMP_USE_L6RES},
        {"distcc_param", smp_fetch_distcc_param, ARG2(1, STR, SINT), val_distcc,
         SMP_T_SINT, SMP_USE_L6REQ | SMP_USE_L6RES},
        {"payload", smp_fetch_payload, ARG2(2, SINT, SINT), NULL, SMP_T_BIN,
         SMP_USE_L6REQ | SMP_USE_L6RES},
        {"payload_lv", smp_fetch_payload_lv, ARG3(2, SINT, SINT, STR),
         val_payload_lv, SMP_T_BIN, SMP_USE_L6REQ | SMP_USE_L6RES},
        {"rdp_cookie", smp_fetch_rdp_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_L6REQ},
        {"rdp_cookie_cnt", smp_fetch_rdp_cookie_cnt, ARG1(0, STR), NULL,
         SMP_T_SINT, SMP_USE_L6REQ},
        {"rep_ssl_hello_type", smp_fetch_ssl_hello_type, 0, NULL, SMP_T_SINT,
         SMP_USE_L6RES},
        {"req_len", smp_fetch_len, 0, NULL, SMP_T_SINT, SMP_USE_L6REQ},
        {"req_ssl_hello_type", smp_fetch_ssl_hello_type, 0, NULL, SMP_T_SINT,
         SMP_USE_L6REQ},
        {"req_ssl_sni", smp_fetch_ssl_hello_sni, 0, NULL, SMP_T_STR,
         SMP_USE_L6REQ},
        {"req_ssl_ver", smp_fetch_req_ssl_ver, 0, NULL, SMP_T_SINT,
         SMP_USE_L6REQ},

        {"req.len", smp_fetch_len, 0, NULL, SMP_T_SINT, SMP_USE_L6REQ},
        {"req.payload", smp_fetch_payload, ARG2(2, SINT, SINT), NULL, SMP_T_BIN,
         SMP_USE_L6REQ},
        {"req.payload_lv", smp_fetch_payload_lv, ARG3(2, SINT, SINT, STR),
         val_payload_lv, SMP_T_BIN, SMP_USE_L6REQ},
        {"req.rdp_cookie", smp_fetch_rdp_cookie, ARG1(0, STR), NULL, SMP_T_STR,
         SMP_USE_L6REQ},
        {"req.rdp_cookie_cnt", smp_fetch_rdp_cookie_cnt, ARG1(0, STR), NULL,
         SMP_T_SINT, SMP_USE_L6REQ},
        {"req.ssl_ec_ext", smp_fetch_req_ssl_ec_ext, 0, NULL, SMP_T_BOOL,
         SMP_USE_L6REQ},
        {"req.ssl_st_ext", smp_fetch_req_ssl_st_ext, 0, NULL, SMP_T_SINT,
         SMP_USE_L6REQ},
        {"req.ssl_hello_type", smp_fetch_ssl_hello_type, 0, NULL, SMP_T_SINT,
         SMP_USE_L6REQ},
        {"req.ssl_sni", smp_fetch_ssl_hello_sni, 0, NULL, SMP_T_STR,
         SMP_USE_L6REQ},
        {"req.ssl_alpn", smp_fetch_ssl_hello_alpn, 0, NULL, SMP_T_STR,
         SMP_USE_L6REQ},
        {"req.ssl_ver", smp_fetch_req_ssl_ver, 0, NULL, SMP_T_SINT,
         SMP_USE_L6REQ},
        {"res.len", smp_fetch_len, 0, NULL, SMP_T_SINT, SMP_USE_L6RES},
        {"res.payload", smp_fetch_payload, ARG2(2, SINT, SINT), NULL, SMP_T_BIN,
         SMP_USE_L6RES},
        {"res.payload_lv", smp_fetch_payload_lv, ARG3(2, SINT, SINT, STR),
         val_payload_lv, SMP_T_BIN, SMP_USE_L6RES},
        {"res.ssl_hello_type", smp_fetch_ssl_hello_type, 0, NULL, SMP_T_SINT,
         SMP_USE_L6RES},
