...
const int http_err_codes[HTTP_ERR_SIZE] = {
    ...
    [HTTP_ERR_400] = 400, [HTTP_ERR_401] = 401, [HTTP_ERR_403] = 403,
    [HTTP_ERR_404] = 404, [HTTP_ERR_405] = 405, [HTTP_ERR_407] = 407,
    [HTTP_ERR_408] = 408, [HTTP_ERR_410] = 410, [HTTP_ERR_413] = 413,
    [HTTP_ERR_421] = 421, [HTTP_ERR_425] = 425, [HTTP_ERR_429] = 429,
    [HTTP_ERR_500] = 500, [HTTP_ERR_502] = 502, [HTTP_ERR_503] = 503,
    [HTTP_ERR_504] = 504,
};
...
const char *http_err_msgs[HTTP_ERR_SIZE] = {
    ...
    [HTTP_ERR_405] = "HTTP/1.1 405 Method Not Allowed\r\n"
                     ...
                     "was made of a resource using a request method not "
    ...
    [HTTP_ERR_413] = "HTTP/1.1 413 Payload Too Large\r\n"
                     ...
                     "entity exceeds the maximum allowed.\n</body></html>\n",
...
const struct ist http_known_methods[HTTP_METH_OTHER] = {
    [HTTP_METH_OPTIONS] = IST("OPTIONS"), [HTTP_METH_GET] = IST("GET"),
    [HTTP_METH_HEAD] = IST("HEAD"),       [HTTP_METH_POST] = IST("POST"),
    [HTTP_METH_PUT] = IST("PUT"),         [HTTP_METH_DELETE] = IST("DELETE"),
    [HTTP_METH_TRACE] = IST("TRACE"),     [HTTP_METH_CONNECT] = IST("CONNECT"),
};
...
enum http_meth_t find_http_meth(const char *str, const int len) {
  ...
  else if (isteq(m, ist("HEAD")))
    return HTTP_METH_HEAD;
  else if (isteq(m, ist("POST")))
    return HTTP_METH_POST;
  else if (isteq(m, ist("CONNECT")))
    return HTTP_METH_CONNECT;
  else if (isteq(m, ist("PUT")))
    return HTTP_METH_PUT;
  else if (isteq(m, ist("OPTIONS")))
    return HTTP_METH_OPTIONS;
  else if (isteq(m, ist("DELETE")))
    return HTTP_METH_DELETE;
  else if (isteq(m, ist("TRACE")))
    return HTTP_METH_TRACE;
  else
...
struct ist http_get_authority(const struct ist uri, int no_userinfo) {
  ...
  if (isalpha((unsigned char)*ptr)) {
     ...
     */
    ptr++;
    while (ptr < end && (isalnum((unsigned char)*ptr) || *ptr == '+' ||
                         *ptr == '-' || *ptr == '.'))
      ptr++;
 ...
 */
int http_header_match2(const char *hdr, const char *end, const char *name,
                       int len) {
  const char *val;
...
char *http_extract_cookie_value(char *hdr, const char *hdr_end,
   ...
   */
  for (att_beg = hdr; att_beg + cookie_name_l + 1 < hdr_end;
       att_beg = next + 1) {
 ...
 */
int http_find_next_url_param(const char **chunks, const char *url_param_name,
                             size_t url_param_name_l, const char **vstart,
                             const char **vend, char delim) {
  ...
  if (url_param_name_l) {
    ...
    arg_start = http_find_url_param_pos(chunks, url_param_name,
                                        url_param_name_l, delim);
    /* Check for wrapping. */
    if (arg_start >= qs_end)
      qs_end = chunks[3];
  }
  ...
  if (!url_param_name_l) {
    while (1) {
      ...
      if (!value_start) {
        /* Check for wrapping. */
        if (arg_start >= chunks[0] && arg_start < chunks[1] && chunks[2]) {
          arg_start = chunks[2];
          qs_end = chunks[3];
          continue;
        }
  ...
  while (1) {
    ...
    if (value_end < qs_end)
      break;
    /* process buffer wrapping. */
    if (value_end >= chunks[0] && value_end < chunks[1] && chunks[2]) {
      value_end = chunks[2];
      qs_end = chunks[3];
      continue;
    }
 ...
 */
int http_parse_header(const struct ist hdr, struct ist *name,
                      struct ist *value) {
  char *p = hdr.ptr;
  char *end = p + hdr.len;
 ...
 */
int http_parse_stline(const struct ist line, struct ist *p1, struct ist *p2,
                      struct ist *p3) {
  char *p = line.ptr;
  char *end = p + line.len;
 ...
 */
int http_parse_status_val(const struct ist value, struct ist *status,
                          struct ist *reason) {
  char *p = value.ptr;
