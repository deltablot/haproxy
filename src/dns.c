 ...
 */
static __inline int dns_hostname_cmp(const char *name1, const char *name2,
                                     int len) {
  int i;
...
struct dns_srvrq *new_dns_srvrq(struct server *srv, char *fqdn) {
  ...
  hostname_dn_len =
      dns_str_to_dn_label(fqdn, fqdn_len + 1, trash.area, trash.size);
...
static void dns_update_resolvers_timeout(struct dns_resolvers *resolvers) {
  ...
  if (!LIST_ISEMPTY(&resolvers->resolutions.curr)) {
    res =
        LIST_NEXT(&resolvers->resolutions.curr, struct dns_resolution *, list);
    next = MIN(next, tick_add(res->last_query, resolvers->timeout.retry));
  }

  list_for_each_entry(res, &resolvers->resolutions.wait, list) next =
      MIN(next, tick_add(res->last_resolution, dns_resolution_timeout(res)));
...
static int dns_connect_namesaver(struct dns_nameserver *ns) {
  ...
  if (connect(fd, (struct sockaddr *)&ns->addr, get_addr_len(&ns->addr)) ==
      -1) {
    send_log(
        NULL, LOG_WARNING,
 ...
 * too short.
 */
static int dns_build_query(int query_id, int query_type,
                           unsigned int accepted_payload_size,
                           char *hostname_dn, int hostname_dn_len, char *buf,
                           int bufsize) {
  ...
  if (sizeof(dns_hdr) + sizeof(qinfo) + sizeof(edns) + hostname_dn_len >=
      bufsize)
    return -1;
  ...
  dns_hdr.flags =
      htons(0x0100); /* qr=0, opcode=0, aa=0, tc=0, rd=1, ra=0, z=0, rcode=0 */
...
static int dns_send_query(struct dns_resolution *resolution) {
  ...
  len =
      dns_build_query(resolution->query_id, resolution->query_type,
                      resolvers->accepted_payload_size, resolution->hostname_dn,
                      resolution->hostname_dn_len, trash.area, trash.size);
 ...
 */
static int dns_run_resolution(struct dns_resolution *resolution) {
  struct dns_resolvers *resolvers = resolution->resolvers;
  int query_id, i;
...
void dns_trigger_resolution(struct dns_requester *req) {
  ...
  if (resolvers->t &&
      (res->status != RSLV_STATUS_VALID || !tick_isset(res->last_resolution) ||
...
int dns_read_name(unsigned char *buffer, unsigned char *bufend,
  ...
  while (1) {
    ...
    if ((*reader & 0xc0) == 0xc0) {
      ...
      n = dns_read_name(buffer, bufend,
                        buffer + (*reader & 0x3f) * 256 + reader[1], dest,
                        dest_len - nb_bytes, offset, depth);
      if (n == 0)
        goto err;
...
static void dns_check_dns_response(struct dns_resolution *res) {
  ...
  list_for_each_entry_safe(item, itemback, &res->response.answer_list, list) {
    ...
    if (ar_item &&
        (ar_item->last_seen + resolvers->hold.obsolete / 1000) < now.tv_sec) {
      ...
      list_for_each_entry_safe(req, reqback, &res->requesters, list) {
        ...
        for (srv = srvrq->proxy->srv; srv != NULL; srv = srv->next) {
          ...
          if (srv->srvrq == srvrq && srv->svc_port == item->port &&
              ...
              !dns_hostname_cmp(srv->hostname_dn, item->target,
                                item->data_len)) {
    ...
    list_for_each_entry_safe(req, reqback, &res->requesters, list) {
      ...
      if (srv) {
        ...
        if ((item->ar_item != NULL) &&
            (item->ar_item->type == DNS_RTYPE_A ||
             item->ar_item->type == DNS_RTYPE_AAAA)) {

          switch (item->ar_item->type) {
          case DNS_RTYPE_A:
            update_server_addr(
                srv,
                &(((struct sockaddr_in *)&item->ar_item->address)->sin_addr),
                AF_INET, "DNS additional record");
            break;
          case DNS_RTYPE_AAAA:
            update_server_addr(
                srv,
                &(((struct sockaddr_in6 *)&item->ar_item->address)->sin6_addr),
                AF_INET6, "DNS additional record");
            break;
          }
        ...
        if (!srv->hostname_dn) {
          ...
          if (dns_dn_label_to_str(item->target, item->data_len + 1, hostname,
                                  DNS_MAX_NAME_SIZE) == -1) {
            HA_SPIN_UNLOCK(SERVER_LOCK, &srv->lock);
            continue;
          }
        ...
        /* now we have an IP address associated to this server, we can update
         * its status */
...
static int dns_validate_dns_response(unsigned char *resp, unsigned char *bufend,
  ...
  for (dns_query_record_id = 0; dns_query_record_id < dns_p->header.qdcount;
       dns_query_record_id++) {
    ...
    offset = 0;
    len = dns_read_name(resp, bufend, reader, dns_query->name,
                        DNS_MAX_NAME_SIZE, &offset, 0);
  ...
  for (i = 0; i < dns_p->header.ancount; i++) {
    ...
    if (dns_query->type != DNS_RTYPE_SRV &&
        dns_hostname_cmp(previous_dname, tmpname, len) != 0) {
      if (i == 0) {
    ...
    case DNS_RTYPE_CNAME:
      /* Check if this is the last record and update the caller about the
       * status: no IP could be found and last record was a CNAME. Could be
       * triggered by a wrong query type
       *
    ...
    list_for_each_entry(tmp_record, &dns_p->answer_list, list) {
      ...
      case DNS_RTYPE_A:
        if (!memcmp(
                &((struct sockaddr_in *)&dns_answer_record->address)->sin_addr,
      ...
      case DNS_RTYPE_AAAA:
        if (!memcmp(&((struct sockaddr_in6 *)&dns_answer_record->address)
                         ->sin6_addr,
      ...
      case DNS_RTYPE_SRV:
        if (dns_answer_record->data_len == tmp_record->data_len &&
            !dns_hostname_cmp(dns_answer_record->target, tmp_record->target,
                              dns_answer_record->data_len) &&
  ...
  for (i = 0; i < dns_p->header.arcount; i++) {
    ...
    if (found == 1) {
      ...
      // looking for the SRV record in the response list linked to this
      // additional record
      list_for_each_entry(tmp_record, &dns_p->answer_list, list) {
        if (tmp_record->type == DNS_RTYPE_SRV &&
            !dns_hostname_cmp(tmp_record->target, dns_answer_record->name,
                              tmp_record->data_len)) {
...
int dns_get_ip_from_response(struct dns_response_packet *dns_p,
                             ...
                             short currentip_sin_family, void **newip,
                             short *newip_sin_family, void *owner) {
  ...
  *newip_sin_family = AF_UNSPEC;
   ...
   *  4 - preferred network.
   *  2 - if the ip in the record is not affected to any other server in the
   * same backend (duplication) 1 - current ip. The result with the biggest
   * score is returned.
   */
  ...
  list_for_each_entry(record, &dns_p->answer_list, list) {
    ...
    for (j = 0; j < dns_opts->pref_net_nb; j++) {
      ...
      if ((ip_type == AF_INET &&
           in_net_ipv4(ip, &dns_opts->pref_net[j].mask.in4,
                       &dns_opts->pref_net[j].addr.in4)) ||
          (ip_type == AF_INET6 &&
           in_net_ipv6(ip, &dns_opts->pref_net[j].mask.in6,
    ...
    if (ip_type == currentip_sin_family &&
        ((currentip_sin_family == AF_INET && !memcmp(ip, currentip, 4)) ||
         (currentip_sin_family == AF_INET6 && !memcmp(ip, currentip, 16)))) {
      score++;
      currentip_sel = 1;
    } else
      currentip_sel = 0;
 ...
 */
static struct dns_resolution *
dns_pick_resolution(struct dns_resolvers *resolvers, char **hostname_dn,
                    int hostname_dn_len, int query_type) {
  struct dns_resolution *res;
...
static void dns_free_resolution(struct dns_resolution *resolution) {
  ...
  list_for_each_entry_safe(item, itemback, &resolution->response.answer_list,
                           list) {
    LIST_DEL(&item->list);
    if (item->ar_item) {
      pool_free(dns_answer_item_pool, item->ar_item);
...
int dns_link_resolution(void *requester, int requester_type,
  ...
  case OBJ_TYPE_SERVER:
    ...
    query_type =
        ((srv->dns_opts.family_prio == AF_INET) ? DNS_RTYPE_A : DNS_RTYPE_AAAA);
    break;
  ...
  case OBJ_TYPE_STREAM:
    ...
    query_type =
        ((stream->dns_ctx.parent->arg.dns.dns_opts->family_prio == AF_INET)
             ? DNS_RTYPE_A
             : DNS_RTYPE_AAAA);
    break;
  default:
  ...
  if ((res = dns_pick_resolution(resolvers, hostname_dn, hostname_dn_len,
                                 query_type)) == NULL)
    goto err;
...
static void dns_resolve_recv(struct dgram_conn *dgram) {
  ...
  while (fd_recv_ready(fd)) {
    ...
    if ((buflen = recv(fd, (char *)buf, resolvers->accepted_payload_size + 1,
                       0)) < 0) {
    ...
    max_answer_records = (resolvers->accepted_payload_size - DNS_HEADER_SIZE) /
                         DNS_MIN_RECORD_SIZE;
    ...
    if (dns_resp != DNS_RESP_VALID &&
        res->nb_responses < resolvers->nb_nameservers)
      continue;
    ...
    if (query && dns_hostname_cmp(query->name, res->hostname_dn,
                                  res->hostname_dn_len) != 0) {
...
static void dns_resolve_send(struct dgram_conn *dgram) {
  ...
  list_for_each_entry(res, &resolvers->resolutions.curr, list) {
    ...
    len = dns_build_query(res->query_id, res->query_type,
                          resolvers->accepted_payload_size, res->hostname_dn,
                          res->hostname_dn_len, trash.area, trash.size);
    if (len == -1)
      goto snd_error;
 ...
 */
static struct task *dns_process_resolvers(struct task *t, void *context,
...
static int dns_finalize_config(void) {
  ...
  list_for_each_entry(resolvers, &dns_resolvers, list) {
    ...
    list_for_each_entry(ns, &resolvers->nameservers, list) {
      ...
      if ((fd = socket(ns->addr.ss_family, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
        ha_alert("config : resolvers '%s': can't create socket for nameserver "
                 "'%s'.\n",
                 resolvers->id, ns->id);
      ...
      if (connect(fd, (struct sockaddr *)&ns->addr, get_addr_len(&ns->addr)) ==
          -1) {
        ha_alert("config : resolvers '%s': can't connect socket for nameserver "
                 "'%s'.\n",
                 resolvers->id, ns->id);
        close(fd);
        err_code |= (ERR_ALERT | ERR_ABORT);
      ...
      if (ns->extra_counters) {
        ns->counters =
            EXTRA_COUNTERS_GET(ns->extra_counters, &dns_stats_module);
    ...
    for (srv = px->srv; srv; srv = srv->next) {
      ...
      if (srv->srvrq && !srv->srvrq->resolvers) {
        ...
        if (dns_link_resolution(srv->srvrq, OBJ_TYPE_SRVRQ, 0) == -1) {
          ha_alert("config : %s '%s' : unable to set DNS resolution for server "
                   "'%s'.\n",
                   proxy_type_str(px), px->id, srv->id);
          err_code |= (ERR_ALERT | ERR_ABORT);
          continue;
      ...
      if (dns_link_resolution(srv, OBJ_TYPE_SERVER, 0) == -1) {
        ha_alert(
            "config : %s '%s', unable to set DNS resolution for server '%s'.\n",
            proxy_type_str(px), px->id, srv->id);
        err_code |= (ERR_ALERT | ERR_ABORT);
        continue;
      }
...
static int stats_dump_dns_to_buffer(struct stream_interface *si,
  ...
  list_for_each_entry(mod, stat_modules, list) {
    struct counters_node *counters =
        EXTRA_COUNTERS_GET(ns->extra_counters, mod);
 ...
 */
int stats_dump_dns(struct stream_interface *si, struct field *stats,
                   size_t stats_count, struct list *stat_modules) {
  struct appctx *appctx = __objt_appctx(si->end);
  struct channel *rep = si_ic(si);
  ...
  list_for_each_entry_from(resolver, &dns_resolvers, list) {
    ...
    list_for_each_entry_from(ns, &resolver->nameservers, list) {
      ...
      if (!stats_dump_dns_to_buffer(si, ns, stats, stats_count, stat_modules)) {
        return 0;
      }
    }
...
int dns_allocate_counters(struct list *stat_modules) {
  ...
  list_for_each_entry(resolvers, &dns_resolvers, list) {
    list_for_each_entry(ns, &resolvers->nameservers, list) {
      EXTRA_COUNTERS_REGISTER(&ns->extra_counters, COUNTERS_DNS, alloc_failed);

      list_for_each_entry(mod, stat_modules, list) {
        EXTRA_COUNTERS_ADD(mod, ns->extra_counters, mod->counters,
                           mod->counters_size);
      }
      ...
      list_for_each_entry(mod, stat_modules, list) {
        memcpy(ns->extra_counters->data +
                   mod->counters_off[ns->extra_counters->type],
               mod->counters, mod->counters_size);

        /* Store the ns counters pointer */
        if (!strcmp(mod->name, "dns")) {
          ns->counters = (struct dns_counters *)ns->extra_counters->data +
                         mod->counters_off[COUNTERS_DNS];
          ns->counters->id = ns->id;
        }
      }
    }
...
static int cli_parse_stat_resolvers(char **args, char *payload,
 ...
 */
static int action_prepare_for_resolution(struct stream *stream,
                                         const char *hostname) {
  char *hostname_dn;
  ...
  hostname_dn_len =
      dns_str_to_dn_label(hostname, hostname_len + 1, hostname_dn, tmp->size);
  if (hostname_dn_len == -1)
    goto err;
...
err:
  free(stream->dns_ctx.hostname_dn);
  stream->dns_ctx.hostname_dn = NULL;
...
use_cache:
  if (s->dns_ctx.dns_requester &&
    ...
    if (resolution->step == RSLV_STEP_NONE) {
      ...
      if (resolution->status == RSLV_STATUS_VALID) {
        ...
        dns_get_ip_from_response(&resolution->response, rule->arg.dns.dns_opts,
                                 NULL, 0, &ip, &ip_sin_family, NULL);
        ...
        if (ip) {
          ...
          vars_set_by_name(rule->arg.dns.varname, strlen(rule->arg.dns.varname),
                           &smp);
        }
      }
    }
  ...
  /* Check if there is a fresh enough response in the cache of our associated
   * resolution */
 ...
 * will be stored (mandatory)
 *   - <resolversSectionName> is the name of the resolvers section to use to
 * perform the resolution (mandatory)
 *   - <resolvePrefer> can be either 'ipv4' or 'ipv6' and is the IP family we
 * would like to resolve first (optional), defaults to ipv6
...
enum act_parse_ret dns_parse_do_resolve(const char **args, int *orig_arg,
  ...
  /* orig_arg points to the first argument, but we need to analyse the command
   * itself first */
  cur_arg = *orig_arg - 1;
  ...
  beg = beg + 1; /* beg should points to the first character after opening
                    parenthesis '(' */
  end = strchr(beg, ',');
  if (end == NULL)
    goto do_resolve_parse_error;
...
do_resolve_parse_error:
  free(rule->arg.dns.varname);
  rule->arg.dns.varname = NULL;
  free(rule->arg.dns.resolvers_id);
  rule->arg.dns.resolvers_id = NULL;
  memprintf(err,
            "Can't parse '%s'. Expects "
            "'do-resolve(<varname>,<resolvers>[,<options>]) <expr>'. Available "
...
static struct action_kw_list http_req_kws = {
    {}, {{"do-resolve", dns_parse_do_resolve, 1}, {/* END */}}};

INITCALL1(STG_REGISTER, http_req_keywords_register, &http_req_kws);

static struct action_kw_list tcp_req_cont_actions = {
    ILH, {{"do-resolve", dns_parse_do_resolve, 1}, {/* END */}}};
 ...
 * filled.
 */
int check_action_do_resolve(struct act_rule *rule, struct proxy *px,
                            char **err) {
  struct dns_resolvers *resolvers = NULL;

  if (rule->arg.dns.resolvers_id == NULL) {
    memprintf(err, "Proxy '%s': %s", px->id,
              "do-resolve action without resolvers");
    return 0;
  }

  resolvers = find_resolvers_by_id(rule->arg.dns.resolvers_id);
  if (resolvers == NULL) {
    memprintf(err, "Can't find resolvers section '%s' for do-resolve action",
              rule->arg.dns.resolvers_id);
    return 0;
  }
