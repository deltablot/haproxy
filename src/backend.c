...
static unsigned int gen_hash(const struct proxy *px, const char *key,
                             unsigned long len) {
  unsigned int hash;
...
void recount_servers(struct proxy *px) {
  ...
  for (srv = px->srv; srv != NULL; srv = srv->next) {
    ...
    if (srv->flags & SRV_F_BACKUP) {
      if (!px->srv_bck && !(px->options & PR_O_USE_ALL_BK))
        px->lbprm.fbck = srv;
      px->srv_bck++;
 ...
 */
static struct server *get_server_ph(struct proxy *px, const char *uri,
...
static struct server *get_server_rnd(struct stream *s,
   ...
   */
  if (curr && (curr->nbpend ||
               (curr->maxconn && curr->served >= srv_dynamic_maxconn(curr))))
    curr = NULL;
 ...
 * server, which should still be present in target_srv(&s->target) before the
 * call. The function tries to keep the original connection slot if it
 * reconnects to the same server, otherwise it releases it and tries to offer
 * it.
 *
...
int assign_server(struct stream *s) {
  ...
  /* We will now try to find the good server and store it into
   * <objt_server(s->target)>. Note that <objt_server(s->target)> may be NULL in
   * case of dispatch or proxy mode, as well as if no server is available (check
   * error code).
   */
  ...
  if ((s->be->lbprm.algo & BE_LB_KIND) != BE_LB_KIND_HI &&
    ...
    list_for_each_entry(srv_list, &s->sess->srv_list, srv_list) {
      ...
      if (tmpsrv && tmpsrv->proxy == s->be &&
          ((s->sess->flags & SESS_FL_PREFER_LAST) ||
           (!s->be->max_ka_queue || server_has_room(tmpsrv) ||
  ...
  if (s->be->lbprm.algo & BE_LB_KIND) {
     ...
     */
    if (s->be->nbpend &&
        (((s->be->lbprm.algo & (BE_LB_KIND | BE_LB_NEED | BE_LB_PARM)) ==
          BE_LB_ALGO_FAS) || // first
         ((s->be->lbprm.algo & (BE_LB_KIND | BE_LB_NEED | BE_LB_PARM)) ==
          BE_LB_ALGO_RR) || // roundrobin
         ((s->be->lbprm.algo & (BE_LB_KIND | BE_LB_NEED | BE_LB_PARM)) ==
          BE_LB_ALGO_SRR))) { // static-rr
      err = SRV_STATUS_FULL;
      goto out;
    }
    ...
    case BE_LB_LKUP_MAP:
      ...
      case BE_LB_HASH_SRC:
        ...
        if (conn && conn_get_src(conn) && conn->src->ss_family == AF_INET) {
          srv = get_server_sh(
              s->be, (void *)&((struct sockaddr_in *)conn->src)->sin_addr, 4,
              prev_srv);
        } else if (conn && conn_get_src(conn) &&
                   conn->src->ss_family == AF_INET6) {
          srv = get_server_sh(
              s->be, (void *)&((struct sockaddr_in6 *)conn->src)->sin6_addr, 16,
              prev_srv);
        } else {
          /* unknown IP family */
          err = SRV_STATUS_INTERNAL;
...
int assign_server_address(struct stream *s) {
  ...
  if ((s->flags & SF_DIRECT) || (s->be->lbprm.algo & BE_LB_KIND)) {
    ...
    *s->target_addr = __objt_server(s->target)->addr;
    ...
    if (!is_addr(s->target_addr) && cli_conn) {
      ...
      if (!conn_get_dst(cli_conn)) {
        ...
        ((struct sockaddr_in *)s->target_addr)->sin_addr =
            ((struct sockaddr_in *)cli_conn->dst)->sin_addr;
      } else if (cli_conn->dst->ss_family == AF_INET6) {
        ((struct sockaddr_in6 *)s->target_addr)->sin6_addr =
    ...
    *s->target_addr = s->be->dispatch_addr;
     ...
     */
    if (conn_get_dst(cli_conn) && (cli_conn->dst->ss_family == AF_INET ||
                                   cli_conn->dst->ss_family == AF_INET6))
 ...
 *
 *   SRV_STATUS_OK       if everything is OK.
 *   SRV_STATUS_NOSRV    if no server is available. objt_server(s->target) =
 * NULL. SRV_STATUS_QUEUED   if the connection has been queued. SRV_STATUS_FULL
 * if the server(s) is/are saturated and the connection could not be queued at
 * the server's, which may be NULL if we queue on the backend.
 *   SRV_STATUS_INTERNAL for other unrecoverable errors.
 *
 */
 ...
 */
static struct connection *conn_backend_get(struct stream *s, struct server *srv,
                                           int is_safe) {
  ...
  if (conn) {
    _HA_ATOMIC_STORE(&srv->next_takeover,
                     (i + 1 == global.nbthread) ? 0 : i + 1);

    srv_use_conn(srv, conn);

    _HA_ATOMIC_SUB(&srv->curr_idle_conns, 1);
    _HA_ATOMIC_SUB(conn->flags & CO_FL_SAFE_LIST ? &srv->curr_safe_nb
                                                 : &srv->curr_idle_nb,
                   1);
 ...
 * emitted. The server-facing stream interface is expected to hold a
 * pre-allocated connection in s->si[1].conn.
 */
int connect_server(struct stream *s) {
  ...
  if (srv && !reuse) {
    ...
    if (srv->available_conns && !LIST_ISEMPTY(&srv->available_conns[tid]) &&
      ...
      srv_conn =
          LIST_ELEM(srv->available_conns[tid].n, struct connection *, list);
      reuse = 1;
    } else if (!srv_conn && srv->curr_idle_conns > 0) {
      if (srv->idle_conns && srv->safe_conns &&
          ((s->be->options & PR_O_REUSE_MASK) != PR_O_REUSE_NEVR && s->txn &&
  ...
  if (ha_used_fds > global.tune.pool_high_count && srv && srv->idle_conns) {
    ...
    tokill_conn = MT_LIST_POP(&srv->idle_conns[tid], struct connection *, list);
    if (tokill_conn)
    ...
    /* If not, iterate over other thread's idling pool, and try to grab one */
      ...
      for (i = tid; (i = ((i + 1 == global.nbthread) ? 0 : i + 1)) != tid;) {
        ...
        HA_SPIN_LOCK(OTHER_LOCK, &idle_conns[tid].takeover_lock);
        tokill_conn =
            MT_LIST_POP(&srv->idle_conns[i], struct connection *, list);
        if (!tokill_conn)
          tokill_conn =
              MT_LIST_POP(&srv->safe_conns[i], struct connection *, list);
        if (tokill_conn) {
          /* We got one, put it into the concerned thread's to kill list, and
           * wake it's kill task */
  ...
  *srv_conn->dst = *s->target_addr;
  ...
  if (!conn_xprt_ready(srv_conn) && !srv_conn->mux) {
    ...
    if (srv)
      conn_prepare(srv_conn, protocol_by_family(srv_conn->dst->ss_family),
                   srv->xprt);
  ...
  if (srv && srv->ssl_ctx.sni) {
    ...
    smp =
        sample_fetch_as_type(s->be, s->sess, s, SMP_OPT_DIR_REQ | SMP_OPT_FINAL,
...
static int back_may_abort_req(struct channel *req, struct stream *s) {
  return ((req->flags & (CF_READ_ERROR)) ||
          ((req->flags &
            (CF_SHUTW_NOW | CF_SHUTW)) && /* empty and client aborted */
 ...
 * SI_ST_CON and SI_ST_EST. Flags must have previously been updated for timeouts
...
void back_try_conn_req(struct stream *s) {
  ...
  if (si->state == SI_ST_ASS) {
    ...
    if (back_may_abort_req(req, s)) {
      ...
      DBG_TRACE_STATE("connection aborted",
    ...
    if (conn_err == SF_ERR_NONE) {
      ...
      if (srv)
        srv_set_sess_last(srv);
      DBG_TRACE_STATE("connection attempt", STRM_EV_STRM_PROC | STRM_EV_SI_ST,
                      s);
      goto end;
    }
    ...
    if (conn_err == SF_ERR_INTERNAL) {
      ...
      if (s->srv_error)
        s->srv_error(s, si);
      DBG_TRACE_STATE("internal error during connection",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
      goto end;
    }
    ...
    DBG_TRACE_STATE("connection error, retry",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
    ...
    if (!pendconn_dequeue(s)) {
      ...
      DBG_TRACE_STATE("dequeue connection request",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST, s);
      goto end;
    }
    ...
    if (si->flags & SI_FL_EXP) {
      ...
      if (s->srv_error)
        s->srv_error(s, si);
      DBG_TRACE_STATE("connection request still queued",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST, s);
      goto end;
    }
    ...
    if (back_may_abort_req(req, s)) {
      ...
      DBG_TRACE_STATE("abort queued connection request",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
      goto abort_connection;
    }
    ...
    DBG_TRACE_STATE("retry connection now", STRM_EV_STRM_PROC | STRM_EV_SI_ST,
                    s);
  }
...
void back_handle_st_req(struct stream *s) {
  ...
  if (unlikely(obj_type(s->target) == OBJ_TYPE_APPLET)) {
    ...
    if (!appctx) {
      ...
      if (s->srv_error)
        s->srv_error(s, si);
      DBG_TRACE_STATE("failed to register applet",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
      goto end;
    }
  ...
  if (srv_redispatch_connect(s) != 0) {
    ...
    if (si->state == SI_ST_QUE) {
      DBG_TRACE_STATE("connection request queued",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST, s);
      goto end;
    }
    ...
    if (s->srv_error)
      s->srv_error(s, si);
    DBG_TRACE_STATE("connection request failed",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
    goto end;
  }
  ...
  si->state = SI_ST_ASS;
  be_set_sess_last(s->be);
  DBG_TRACE_STATE("connection request assigned to a server",
                  STRM_EV_STRM_PROC | STRM_EV_SI_ST, s);
...
void back_handle_st_con(struct stream *s) {
  ...
  if ((rep->flags & CF_SHUTW) ||
    ...
    DBG_TRACE_STATE("client abort during connection attempt",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
    goto end;
  }
  ...
  if (si->flags & (SI_FL_EXP | SI_FL_ERR)) {
    ...
    si->state = SI_ST_CER;
    DBG_TRACE_STATE("connection failed, retry",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
  }
...
void back_handle_st_cer(struct stream *s) {
  ...
  if (objt_server(s->target)) {
    ...
    if ((si->flags & SI_FL_ERR) && conn &&
       ...
       */
      si->conn_retries = 0;
      DBG_TRACE_DEVEL("Bad SSL cert, disable connection retries",
                      STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
    }
  }
  ...
  if (si->conn_retries < 0 || !(s->be->retry_type & PR_RE_CONN_FAILED)) {
    ...
    DBG_TRACE_STATE("connection failed",
                    STRM_EV_STRM_PROC | STRM_EV_SI_ST | STRM_EV_STRM_ERR, s);
    goto end;
  }
  ...
  if (si->flags & SI_FL_ERR) {
    ...
    si->flags &= ~SI_FL_ERR;
    DBG_TRACE_STATE("retry a new connection", STRM_EV_STRM_PROC | STRM_EV_SI_ST,
                    s);
  }
...
void back_handle_st_rdy(struct stream *s) {
   ...
   * now take over.
   */
  DBG_TRACE_STATE("connection established", STRM_EV_STRM_PROC | STRM_EV_SI_ST,
                  s);
  si->err_type = SI_ET_NONE;
  si->state = SI_ST_EST;
...
int tcp_persist_rdp_cookie(struct stream *s, struct channel *req, int an_bit) {
  ...
  ret = fetch_rdp_cookie_name(s, &smp, s->be->rdp_cookie_name,
                              s->be->rdp_cookie_len);
  if (ret == 0 || (smp.flags & SMP_F_MAY_CHANGE) || smp.data.u.str.data == 0)
    goto no_cookie;

  /* Considering an rdp cookie detected using acl, str ended with <cr><lf> and
   * should return. The cookie format is <ip> "." <port> where "ip" is the
   * integer corresponding to the server's IP address in network order, and
   * "port" is the integer corresponding to the server's port in network order.
   * Comments please Emeric.
   */
  ...
  s->target = NULL;
  while (srv) {
    if (srv->addr.ss_family == AF_INET && port == srv->svc_port &&
...
int be_downtime(struct proxy *px) {
  if (px->lbprm.tot_weight &&
      px->last_change < now.tv_sec) // ignore negative time
    return px->down_time;
  ...
  if (!strcmp(args[0], "roundrobin")) {
    ...
    if (*(args[0] + 6) == '(' && *(args[0] + 7) != ')') { /* number of draws */
      ...
      if (*end != ')') {
        if (!*end)
          ...
          memprintf(err, "random : unexpected character '%c' after argument.",
                    *end);
        return -1;
      }
    ...
    while (*args[arg]) {
      if (!strcmp(args[arg], "len")) {
        if (!*args[arg + 1] || (atoi(args[arg + 1]) <= 0)) {
          memprintf(err, "%s : '%s' expects a positive integer (got '%s').",
                    args[0], args[arg], args[arg + 1]);
          return -1;
        }
        ...
        memprintf(err,
                  "%s only accepts parameters 'len', 'depth', 'path-only', and "
                  "'whole' (got '%s').",
                  args[0], args[arg]);
        return -1;
      }
    ...
    if (*args[2]) {
      if (strcmp(args[2], "check_post")) {
        memprintf(err, "%s only accepts 'check_post' modifier (got '%s').",
                  args[0], args[2]);
        return -1;
      }
    }
    ...
    if (*args[1]) {
      if (strcmp(args[1], "use_domain_only")) {
        memprintf(err, "%s only accepts 'use_domain_only' modifier (got '%s').",
                  args[0], args[1]);
        return -1;
      }
      curproxy->lbprm.arg_opt1 = 1;
    ...
    memprintf(
        err, "only supports 'roundrobin', 'static-rr', 'leastconn', 'source', "
             "'uri', 'url_param', 'hdr(name)' and 'rdp-cookie(name)' options.");
    return -1;
  }
  return 0;
}
 ...
 * undefined behaviour.
 */
static int smp_fetch_nbsrv(const struct arg *args, struct sample *smp,
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_is_up(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  ...
  if (!(srv->cur_admin & SRV_ADMF_MAINT) &&
      (!(srv->check.state & CHK_ST_CONFIGURED) ||
       (srv->cur_state != SRV_ST_STOPPED)))
    smp->data.u.sint = 1;
  else
    smp->data.u.sint = 0;
  return 1;
 ...
 * undefined behaviour.
 */
static int smp_fetch_connslots(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
...
static int smp_fetch_be_id(const struct arg *args, struct sample *smp,
                           const char *kw, void *private) {
...
static int smp_fetch_be_name(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
  struct proxy *px = NULL;
...
static int smp_fetch_srv_id(const struct arg *args, struct sample *smp,
                            const char *kw, void *private) {
...
static int smp_fetch_srv_name(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
  struct server *srv = NULL;
 ...
 * backend. Accepts exactly 1 argument. Argument is a backend, other types will
 * lead to undefined behaviour.
 */
static int smp_fetch_be_sess_rate(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 * undefined behaviour.
 */
static int smp_fetch_be_conn(const struct arg *args, struct sample *smp,
                             const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 * undefined behaviour.
 */
static int smp_fetch_be_conn_free(const struct arg *args, struct sample *smp,
                                  const char *kw, void *private) {
  struct server *iterator;
  struct proxy *px;
 ...
 * undefined behaviour.
 */
static int smp_fetch_queue_size(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
...
/* set temp integer to the total number of queued connections on the backend
 * divided by the number of running servers and rounded up. If there is no
 * running server, we return twice the total, just as if we had half a running
 * server. This is more or less correct anyway, since we expect the last server
 * to come back soon. Accepts exactly 1 argument. Argument is a backend, other
 * types will lead to undefined behaviour.
 */
static int smp_fetch_avg_queue_size(const struct arg *args, struct sample *smp,
                                    const char *kw, void *private) {
  int nbsrv;
  struct proxy *px;
...
/* set temp integer to the number of concurrent connections on the server in the
 * backend. Accepts exactly 1 argument. Argument is a server, other types will
 * lead to undefined behaviour.
 */
static int smp_fetch_srv_conn(const struct arg *args, struct sample *smp,
                              const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
...
/* set temp integer to the number of available connections on the server in the
 * backend. Accepts exactly 1 argument. Argument is a server, other types will
 * lead to undefined behaviour.
 */
static int smp_fetch_srv_conn_free(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  unsigned int maxconn;
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_queue(const struct arg *args, struct sample *smp,
                               const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_sess_rate(const struct arg *args, struct sample *smp,
                                   const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_weight(const struct arg *args, struct sample *smp,
                                const char *kw, void *private) {
  struct server *srv = args->data.srv;
  ...
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
  smp->data.u.sint = (srv->cur_eweight * px->lbprm.wmult + px->lbprm.wdiv - 1) /
                     px->lbprm.wdiv;
  return 1;
}
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_iweight(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
 ...
 * undefined behaviour.
 */
static int smp_fetch_srv_uweight(const struct arg *args, struct sample *smp,
                                 const char *kw, void *private) {
  smp->flags = SMP_F_VOL_TEST;
  smp->data.type = SMP_T_SINT;
...
static int smp_fetch_be_server_timeout(const struct arg *args,
...
static int smp_fetch_be_tunnel_timeout(const struct arg *args,
                                       struct sample *smp, const char *km,
                                       void *private) {
  struct proxy *px = NULL;
...
static int sample_conv_nbsrv(const struct arg *args, struct sample *smp,
                             void *private) {
...
static int sample_conv_srv_queue(const struct arg *args, struct sample *smp,
                                 void *private) {
  struct proxy *px;
  struct server *srv;
  char *bksep;
