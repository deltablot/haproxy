...
const char *log_facilities[NB_LOG_FACILITIES] = {
    ...
    "ntp",    "audit",  "alert",  "cron2",  "local0", "local1",
    "local2", "local3", "local4", "local5", "local6", "local7"};

const char *log_levels[NB_LOG_LEVELS] = {"emerg",   "alert",  "crit", "err",
                                         "warning", "notice", "info", "debug"};

const char sess_term_cond[16] =
    "-LcCsSPRIDKUIIII"; /* normal, Local, CliTo, CliErr, SrvTo, SrvErr, PxErr,
                           Resource, Internal, Down, Killed, Up, -- */
const char sess_fin_state[8] = "-RCHDLQT"; /* cliRequest, srvConnect, srvHeader,
                                              Data, Last, Queue, Tarpit */
...
static const struct logformat_type logformat_keywords[] = {
    ...
    {"B", LOG_FMT_BYTES, PR_MODE_TCP, LW_BYTES,
     NULL}, /* bytes from server to client */
    ...
    {"Ta", LOG_FMT_Ta, PR_MODE_HTTP, LW_BYTES,
     NULL}, /* Time active (tr to end) */
    ...
    {"Tl", LOG_FMT_DATELOCAL, PR_MODE_TCP, LW_INIT,
     NULL},                                           /* date local timezone */
    {"Tq", LOG_FMT_TQ, PR_MODE_HTTP, LW_BYTES, NULL}, /* Tq=Th+Ti+TR */
    {"Tr", LOG_FMT_Tr, PR_MODE_HTTP, LW_BYTES, NULL}, /* Tr */
    {"TR", LOG_FMT_TR, PR_MODE_HTTP, LW_BYTES,
     NULL}, /* Time to receive a valid request */
    {"Td", LOG_FMT_TD, PR_MODE_TCP, LW_BYTES,
     NULL}, /* Td = Tt - (Tq + Tw + Tc + Tr) */
    ...
    {"U", LOG_FMT_BYTES_UP, PR_MODE_TCP, LW_BYTES,
     NULL}, /* bytes from client to server */
    ...
    {"bi", LOG_FMT_BACKENDIP, PR_MODE_TCP, LW_BCKIP,
     prepare_addrsource}, /* backend source ip */
    {"bp", LOG_FMT_BACKENDPORT, PR_MODE_TCP, LW_BCKIP,
     prepare_addrsource}, /* backend source port */
    {"bq", LOG_FMT_BCKQUEUE, PR_MODE_TCP, LW_BYTES, NULL}, /* backend_queue */
    {"ci", LOG_FMT_CLIENTIP, PR_MODE_TCP, LW_CLIP | LW_XPRT,
     NULL}, /* client ip */
    {"cp", LOG_FMT_CLIENTPORT, PR_MODE_TCP, LW_CLIP | LW_XPRT,
     NULL},                                              /* client port */
    {"f", LOG_FMT_FRONTEND, PR_MODE_TCP, LW_INIT, NULL}, /* frontend */
    {"fc", LOG_FMT_FECONN, PR_MODE_TCP, LW_BYTES, NULL}, /* feconn */
    {"fi", LOG_FMT_FRONTENDIP, PR_MODE_TCP, LW_FRTIP | LW_XPRT,
     NULL}, /* frontend ip */
    {"fp", LOG_FMT_FRONTENDPORT, PR_MODE_TCP, LW_FRTIP | LW_XPRT,
     NULL}, /* frontend port */
    {"ft", LOG_FMT_FRONTEND_XPRT, PR_MODE_TCP, LW_INIT,
     NULL}, /* frontend with transport mode */
    {"hr", LOG_FMT_HDRREQUEST, PR_MODE_TCP, LW_REQHDR,
     NULL}, /* header request */
    {"hrl", LOG_FMT_HDRREQUESTLIST, PR_MODE_TCP, LW_REQHDR,
     NULL}, /* header request list */
    {"hs", LOG_FMT_HDRRESPONS, PR_MODE_TCP, LW_RSPHDR,
     NULL}, /* header response */
    {"hsl", LOG_FMT_HDRRESPONSLIST, PR_MODE_TCP, LW_RSPHDR,
     NULL}, /* header response list */
    {"HM", LOG_FMT_HTTP_METHOD, PR_MODE_HTTP, LW_REQ, NULL}, /* HTTP method */
    {"HP", LOG_FMT_HTTP_PATH, PR_MODE_HTTP, LW_REQ,
     NULL}, /* HTTP relative or absolute path */
    {"HPO", LOG_FMT_HTTP_PATH_ONLY, PR_MODE_HTTP, LW_REQ,
     NULL}, /* HTTP path only (without host nor query string) */
    ...
    {"ms", LOG_FMT_MS, PR_MODE_TCP, LW_INIT,
     NULL}, /* accept date millisecond */
    {"pid", LOG_FMT_PID, PR_MODE_TCP, LW_INIT, NULL},     /* log pid */
    {"r", LOG_FMT_REQ, PR_MODE_HTTP, LW_REQ, NULL},       /* request */
    {"rc", LOG_FMT_RETRIES, PR_MODE_TCP, LW_BYTES, NULL}, /* retries */
    {"rt", LOG_FMT_COUNTER, PR_MODE_TCP, LW_REQ,
     NULL}, /* request counter (HTTP or TCP session) */
    {"s", LOG_FMT_SERVER, PR_MODE_TCP, LW_SVID, NULL},    /* server */
    {"sc", LOG_FMT_SRVCONN, PR_MODE_TCP, LW_BYTES, NULL}, /* srv_conn */
    {"si", LOG_FMT_SERVERIP, PR_MODE_TCP, LW_SVIP,
     NULL}, /* server destination ip */
    {"sp", LOG_FMT_SERVERPORT, PR_MODE_TCP, LW_SVIP,
     NULL}, /* server destination port */
    {"sq", LOG_FMT_SRVQUEUE, PR_MODE_TCP, LW_BYTES, NULL}, /* srv_queue */
    {"sslc", LOG_FMT_SSL_CIPHER, PR_MODE_TCP, LW_XPRT,
     NULL}, /* client-side SSL ciphers */
    {"sslv", LOG_FMT_SSL_VERSION, PR_MODE_TCP, LW_XPRT,
     NULL}, /* client-side SSL protocol version */
    {"t", LOG_FMT_DATE, PR_MODE_TCP, LW_INIT, NULL}, /* date */
    {"tr", LOG_FMT_tr, PR_MODE_HTTP, LW_INIT,
     NULL}, /* date of start of request */
    {"trg", LOG_FMT_trg, PR_MODE_HTTP, LW_INIT,
     NULL}, /* date of start of request, GMT */
    {"trl", LOG_FMT_trl, PR_MODE_HTTP, LW_INIT,
     NULL}, /* date of start of request, local */
    {"ts", LOG_FMT_TERMSTATE, PR_MODE_TCP, LW_BYTES,
     NULL}, /* termination state */
    {"tsc", LOG_FMT_TERMSTATE_CK, PR_MODE_TCP, LW_INIT,
     NULL}, /* termination state */

    /* The following tags are deprecated and will be removed soon */
    {"Bi", LOG_FMT_BACKENDIP, PR_MODE_TCP, LW_BCKIP, prepare_addrsource,
     "bi"}, /* backend source ip */
    {"Bp", LOG_FMT_BACKENDPORT, PR_MODE_TCP, LW_BCKIP, prepare_addrsource,
     "bp"}, /* backend source port */
    {"Ci", LOG_FMT_CLIENTIP, PR_MODE_TCP, LW_CLIP | LW_XPRT, NULL,
     "ci"}, /* client ip */
    {"Cp", LOG_FMT_CLIENTPORT, PR_MODE_TCP, LW_CLIP | LW_XPRT, NULL,
     "cp"}, /* client port */
    {"Fi", LOG_FMT_FRONTENDIP, PR_MODE_TCP, LW_FRTIP | LW_XPRT, NULL,
     "fi"}, /* frontend ip */
    {"Fp", LOG_FMT_FRONTENDPORT, PR_MODE_TCP, LW_FRTIP | LW_XPRT, NULL,
     "fp"}, /* frontend port */
    {"Si", LOG_FMT_SERVERIP, PR_MODE_TCP, LW_SVIP, NULL,
     "si"}, /* server destination ip */
    {"Sp", LOG_FMT_SERVERPORT, PR_MODE_TCP, LW_SVIP, NULL,
     "sp"}, /* server destination port */
    {"cc", LOG_FMT_CCLIENT, PR_MODE_HTTP, LW_REQHDR, NULL,
     "CC"}, /* client cookie */
    {"cs", LOG_FMT_CSERVER, PR_MODE_HTTP, LW_RSPHDR, NULL,
     "CS"}, /* server cookie */
    {"st", LOG_FMT_STATUS, PR_MODE_HTTP, LW_RESP, NULL, "ST"}, /* status code */
    {0, 0, 0, 0, NULL}};

char default_http_log_format[] =
    "%ci:%cp [%tr] %ft %b/%s %TR/%Tw/%Tc/%Tr/%Ta %ST %B %CC %CS %tsc "
    "%ac/%fc/%bc/%sc/%rc %sq/%bq %hr %hs %{+Q}r"; // default format
char clf_http_log_format[] =
    "%{+Q}o %{-Q}ci - - [%trg] %r %ST %B \"\" \"\" %cp %ms %ft %b %s %TR %Tw "
    "%Tc %Tr %Ta %tsc %ac %fc %bc %sc %rc %sq %bq %CC %CS %hrl %hsl";
char default_tcp_log_format[] =
...
int parse_logformat_var_args(char *args, struct logformat_node *node,
  ...
  if (args == NULL) {
    memprintf(
        err,
        "internal error: parse_logformat_var_args() expects non null 'args'");
    return 0;
  }
 ...
 */
int parse_logformat_var(char *arg, int arg_len, char *var, int var_len,
                        struct proxy *curproxy, struct list *list_format,
  ...
  for (j = 0; logformat_keywords[j].name; j++) { // search a log type
    if (strlen(logformat_keywords[j].name) == var_len &&
      ...
      if (logformat_keywords[j].mode != PR_MODE_HTTP ||
        ...
        if (logformat_keywords[j].replace_by)
          ha_warning("parsing [%s:%d] : deprecated variable '%s' in '%s', "
                     "please replace it with '%s'.\n",
                     curproxy->conf.args.file, curproxy->conf.args.line,
                     logformat_keywords[j].name, fmt_directive(curproxy),
                     logformat_keywords[j].replace_by);
        return 1;
      } else {
  ...
  var[var_len] = 0;
  memprintf(err,
            "no such format variable '%s'. If you wanted to emit the '%%' "
            "character verbatim, you need to use '%%%%'",
            var);
  var[var_len] = j;
 ...
 *
 */
int add_to_logformat_list(char *start, char *end, int type,
                          struct list *list_format, char **err) {
  char *str;
 ...
 * success. At the moment, sample converters are not yet supported but fetch
 * arguments should work. The curpx->conf.args.ctx must be set by the caller. If
 * an end pointer is passed in <endptr>, it will be updated with the pointer to
 * the first character not part of the sample expression.
 *
 * In error case, the function returns 0, otherwise it returns 1.
 */
int add_sample_to_logformat_list(char *text, char *arg, int arg_len,
                                 struct proxy *curpx, struct list *list_format,
  ...
  expr =
      sample_parse_expr(cmd, &cmd_arg, curpx->conf.args.file,
                        curpx->conf.args.line, err, &curpx->conf.args, endptr);
  if (!expr) {
  ...
  if (!(expr->fetch->val & cap)) {
    memprintf(err,
              "sample fetch <%s> may not be reliably used here because it "
              "needs '%s' which is not available here",
 ...
 * Variable name are preceded by % and composed by characters [a-zA-Z0-9]* :
 * %varname You can set arguments using { } : %{many arguments}varname. The
...
int parse_logformat_string(const char *fmt, struct proxy *curproxy,
                           struct list *list_format, int options, int cap,
                           char **err) {
  ...
  for (cformat = LF_INIT; cformat != LF_END; str++) {
    ...
    /* The principle of the two-step state machine below is to first detect a
     * change, and second have all common paths processed at one place. The
     * common paths are the ones encountered in text areas (LF_INIT, LF_TEXT,
     * LF_SEPARATOR) and at the end (LF_END). We use the common LF_INIT state to
     * dispatch to the different final states.
     */
    switch (pformat) {
    case LF_STARTVAR: // text immediately following a '%'
      ...
      if (*str == '{') { // optional argument
        ...
        cformat =
            LF_TEXT; // convert this character to a literal (useful for '%')
        ...
        memprintf(
            err,
            "unexpected variable name near '%c' at position %d line : '%s'. "
            "Maybe you want to write a single '%%', use the syntax '%%%%'",
    ...
    case LF_EDARG: // text immediately following '%{arg}'
      ...
      memprintf(err,
                "parse argument modifier without variable name near '%%{%s}'",
                arg);
      goto fail;
    ...
    case LF_STEXPR: // text immediately following '%['
       ...
       * angle bracket.
       */
      if (!add_sample_to_logformat_list(var, arg, arg_len, curproxy,
                                        list_format, options, cap, err, &str))
        goto fail;
    ...
    if (cformat != pformat || pformat == LF_SEPARATOR) {
      switch (pformat) {
      case LF_VAR:
        if (!parse_logformat_var(arg, arg_len, var, var_len, curproxy,
                                 list_format, &options, err))
          goto fail;
        break;
      case LF_TEXT:
      case LF_SEPARATOR:
      ...
      sp = str; /* new start of text at every state switch and at every
                   separator */
    }
  }
...
/*
 * Parse the first range of indexes from a string made of a list of comma
 * separated ranges of indexes. Note that an index may be considered as a
 * particular range with a high limit to the low limit.
 */
int get_logsrv_smp_range(unsigned int *low, unsigned int *high, char **arg,
                         char **err) {
  char *end, *p;
...
int parse_logsrv(char **args, struct list *logsrvs, int do_del, char **err) {
  ...
  if (strcmp(args[cur_arg], "sample") == 0) {
    ...
    *smp_sz_str++ = '\0';
    ...
    while (p != end) {
      ...
      if (smp_rgs &&
          smp_log_ranges_overlap(smp_rgs, smp_rgs_sz, low, high, err))
        goto error;
    ...
    if (!new_smp_sz || beg != end) {
      memprintf(err, "wrong sample size '%s' for sample range '%s'", smp_sz_str,
                args[cur_arg + 1]);
      goto error;
    }
    ...
    if (new_smp_sz < smp_sz) {
      memprintf(err,
                "sample size %zu should be greater or equal to "
  ...
  sk = str2sa_range(args[1], NULL, &port1, &port2, &fd, NULL, err, NULL, NULL,
                    PA_O_RESOLVE | PA_O_PORT_OK | PA_O_RAW_FD | PA_O_DGRAM);
  if (!sk)
    goto error;
...
static void print_message(const char *label, const char *fmt, va_list argp) {
  ...
  memprintf(&head, "[%s] %03d/%02d%02d%02d (%d) : ", label, tm.tm_yday,
...
void ha_alert(const char *fmt, ...) {
  ...
  if (!(global.mode & MODE_QUIET) ||
      (global.mode & (MODE_VERBOSE | MODE_STARTING))) {
 ...
 * <escape>.
 */
static char *lf_encode_string(char *start, char *stop, const char escape,
                              const long *map, const char *string,
 ...
 * <escape>.
 */
static char *lf_encode_chunk(char *start, char *stop, const char escape,
                             const long *map, const struct buffer *chunk,
                             struct logformat_node *node) {
  char *str, *end;
 ...
 */
char *lf_text_len(char *dst, const char *src, size_t len, size_t size,
  ...
  if (src && len) {
    ...
    size -= len;
    dst += len;
  } else if ((node->options & (LOG_OPT_QUOTE | LOG_OPT_MANDATORY)) ==
             LOG_OPT_MANDATORY) {
    if (size < 2)
      return NULL;
    *(dst++) = '-';
...
static inline char *lf_text(char *dst, const char *src, size_t size,
                            const struct logformat_node *node) {
 ...
 */
char *lf_ip(char *dst, const struct sockaddr *sockaddr, size_t size,
            const struct logformat_node *node) {
  char *ret = dst;
  int iret;
  ...
  if (node->options & LOG_OPT_HEXA) {
    ...
    case AF_INET:
      addr =
          (unsigned char *)&((struct sockaddr_in *)sockaddr)->sin_addr.s_addr;
      iret = snprintf(dst, size, "%02X%02X%02X%02X", addr[0], addr[1], addr[2],
                      addr[3]);
      break;
    case AF_INET6:
      addr = (unsigned char *)&((struct sockaddr_in6 *)sockaddr)
                 ->sin6_addr.s6_addr;
      iret = snprintf(
          dst, size,
 ...
 * +X option write in hexadecimal notation, most significant byte on the left
 */
char *lf_port(char *dst, const struct sockaddr *sockaddr, size_t size,
              const struct logformat_node *node) {
  char *ret = dst;
  int iret;
  ...
  if (node->options & LOG_OPT_HEXA) {
    const unsigned char *port =
        (const unsigned char *)&((struct sockaddr_in *)sockaddr)->sin_port;
...
void send_log(struct proxy *p, int level, const char *format, ...) {
  ...
  __send_log((p ? &p->logsrvs : NULL), (p ? &p->log_tag : NULL), level, logline,
             data_len, default_rfc5424_sd_log_format, 2);
}
/*
  ...
  *nbelem = 0;
  ...
  if (format == LOG_FORMAT_UNSPEC) {
    ...
    if (metadata) {
      ...
      if (metadata[LOG_META_HOST].len) {
         ...
         * in documentation.
         */
        if ((metadata[LOG_META_TIME].len == 1 &&
             metadata[LOG_META_TIME].ptr[0] == '-') ||
  ...
  case LOG_FORMAT_SHORT:
    ...
    hdr_ctx.ist_vector[*nbelem].ptr =
        &hdr_ctx.priority_buffer[3]; /* last digit of the log level */
    do {
      *hdr_ctx.ist_vector[*nbelem].ptr = '0' + fac_level % 10;
      fac_level /= 10;
      hdr_ctx.ist_vector[*nbelem].ptr--;
    } while (fac_level &&
             hdr_ctx.ist_vector[*nbelem].ptr > &hdr_ctx.priority_buffer[0]);
    *hdr_ctx.ist_vector[*nbelem].ptr = '<';
    hdr_ctx.ist_vector[(*nbelem)++].len =
        &hdr_ctx.priority_buffer[5] - hdr_ctx.ist_vector[0].ptr;
    break;
  case LOG_FORMAT_ISO:
  case LOG_FORMAT_RAW:
    break;
  ...
  case LOG_FORMAT_RFC3164:
    ...
    if (metadata && metadata[LOG_META_TIME].len == LOG_LEGACYTIME_LEN) {
      ...
      if (month <= 11) {
        ...
        len =
            snprintf(hdr_ctx.timestamp_buffer, sizeof(hdr_ctx.timestamp_buffer),
                     "%s %c%c ", monthname[month],
                     timestamp[8] != '0' ? timestamp[8] : ' ', timestamp[9]);
        /* we reused the timestamp_buffer, signal that it does not
         * contain local time anymore
         */
        hdr_ctx.cur_legacy_time = 0;
        if (len == 7) {
          hdr_ctx.ist_vector[(*nbelem)++] =
              ist2(&hdr_ctx.timestamp_buffer[0], len);
    ...
    if (unlikely(time != hdr_ctx.cur_legacy_time)) {
      ...
      len = snprintf(hdr_ctx.timestamp_buffer, sizeof(hdr_ctx.timestamp_buffer),
                     "%s %2d %02d:%02d:%02d ", monthname[logtime.tm_mon],
    ...
    if (likely(hdr_ctx.cur_legacy_time))
      hdr_ctx.ist_vector[(*nbelem)++] =
          ist2(&hdr_ctx.timestamp_buffer[0], LOG_LEGACYTIME_LEN + 1);
    else
      hdr_ctx.ist_vector[(*nbelem)++] =
          ist2("Jan  1 00:00:00 ", LOG_LEGACYTIME_LEN + 1);
    break;
  case LOG_FORMAT_RFC5424:
    /* adds rfc5425 version prefix */
    hdr_ctx.ist_vector[(*nbelem)++] = ist2("1 ", 2);
    if (metadata && metadata[LOG_META_TIME].len == 1 &&
        metadata[LOG_META_TIME].ptr[0] == '-') {
  ...
  case LOG_FORMAT_ISO:
    ...
    if (metadata && metadata[LOG_META_TIME].len >= LOG_ISOTIME_MINLEN) {
      ...
      if (month < 12) {
        ...
        len =
            snprintf(hdr_ctx.timestamp_buffer, sizeof(hdr_ctx.timestamp_buffer),
                     "%4d-%02d-%c%cT", logtime.tm_year + 1900, month + 1,
                     timestamp[4] != ' ' ? timestamp[4] : '0', timestamp[5]);
    ...
    hdr_ctx.ist_vector[(*nbelem)++] =
        ist2(timeofday_as_iso_us(1), LOG_ISOTIME_MAXLEN + 1);
    break;
  case LOG_FORMAT_PRIO:
  case LOG_FORMAT_SHORT:
  case LOG_FORMAT_RAW:
 ...
 */
static inline void __do_send_log(struct logsrv *logsrv, int nblogger, int level,
                                 int facility, struct ist *metadata,
                                 char *message, size_t size) {
  static THREAD_LOCAL struct iovec iovec[NB_LOG_HDR_MAX_ELEMENTS + 1 + 1] =
      {}; /* header elements + message + LF */
  static THREAD_LOCAL struct msghdr msghdr = {//.msg_iov = iovec,
                                              .msg_iovlen =
                                                  NB_LOG_HDR_MAX_ELEMENTS + 2};
  ...
  if (plogfd && unlikely(*plogfd < 0)) {
    ...
    if ((*plogfd = socket(
             logsrv->addr.ss_family, SOCK_DGRAM,
  ...
  msg_header =
      build_log_header(logsrv->format, level, facility, metadata, &nbelem);
send:
  if (logsrv->type == LOG_TARGET_BUFFER) {
    struct ist msg;
    ...
    sent = fd_write_frag_line(*plogfd, logsrv->maxlen, msg_header, nbelem, &msg,
                              1, 1);
  } else {
    int i = 0;
  ...
  list_for_each_entry(logsrv, logsrvs, list) {
    ...
    if (in_range)
      __do_send_log(logsrv, ++nblogger, MAX(level, logsrv->minlvl),
                    (facility == -1) ? logsrv->facility : facility, metadata,
                    message, size);
  }
}
...
void __send_log(struct list *logsrvs, struct buffer *tagb, int level,
  ...
  while (metadata[LOG_META_STDATA].len &&
         metadata[LOG_META_STDATA].ptr[metadata[LOG_META_STDATA].len - 1] ==
             ' ')
...
const char sess_cookie[8] =
    "NIDVEOU7"; /* No cookie, Invalid cookie, cookie for a Down server, Valid
                   cookie, Expired cookie, Old cookie, Unused, unknown */
const char sess_set_cookie[8] =
    "NPDIRU67"; /* No set-cookie, Set-cookie found and left unchanged (passive),
                   Set-cookie Deleted, Set-Cookie Inserted, Set-cookie
                   Rewritten, Set-cookie Updated, unknown, unknown */
 ...
 */
int sess_build_logline(struct session *sess, struct stream *s, char *dst,
                       size_t maxsize, struct list *list_format) {
  ...
  if (likely(s)) {
    ...
    if ((fe->mode == PR_MODE_HTTP) && fe_conn && fe_conn->mux &&
        fe_conn->mux->ctl) {
      enum mux_exit_status es =
          fe_conn->mux->ctl(fe_conn, MUX_EXIT_STATUS, NULL);
  ...
  list_for_each_entry(tmp, list_format, list) {
    ...
    case LOG_FMT_EXPR: // sample expression, may be request or response
      ...
      if (tmp->options & LOG_OPT_REQ_CAP)
        key = sample_fetch_as_type(be, sess, s, SMP_OPT_DIR_REQ | SMP_OPT_FINAL,
                                   tmp->expr, SMP_T_STR);
      if (!key && (tmp->options & LOG_OPT_RES_CAP))
        key = sample_fetch_as_type(be, sess, s, SMP_OPT_DIR_RES | SMP_OPT_FINAL,
                                   tmp->expr, SMP_T_STR);
      if (tmp->options & LOG_OPT_HTTP)
        ret = lf_encode_chunk(tmplog, dst + maxsize, '%', http_encode_map,
                              key ? &key->data.u.str : &empty, tmp);
      else
        ret = lf_text_len(tmplog, key ? key->data.u.str.area : NULL,
    ...
    case LOG_FMT_CLIENTIP: // %ci
      ...
      if (conn && conn_get_src(conn))
        ret = lf_ip(tmplog, (struct sockaddr *)conn->src,
                    dst + maxsize - tmplog, tmp);
      else
    ...
    case LOG_FMT_FRONTENDIP: // %fi
      ...
      if (conn && conn_get_dst(conn)) {
        ret = lf_ip(tmplog, (struct sockaddr *)conn->dst,
                    dst + maxsize - tmplog, tmp);
      } else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_FRONTENDPORT: // %fp
      ...
      if (conn && conn_get_dst(conn)) {
        if (conn->dst->ss_family == AF_UNIX)
          ...
          ret = lf_port(tmplog, (struct sockaddr *)conn->dst,
                        dst + maxsize - tmplog, tmp);
      } else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_BACKENDIP: // %bi
      if (be_conn && conn_get_src(be_conn))
        ret = lf_ip(tmplog, (const struct sockaddr *)be_conn->src,
                    dst + maxsize - tmplog, tmp);
      else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_BACKENDPORT: // %bp
      if (be_conn && conn_get_src(be_conn))
        ret = lf_port(tmplog, (struct sockaddr *)be_conn->src,
                      dst + maxsize - tmplog, tmp);
      else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_SERVERIP: // %si
      if (be_conn && conn_get_dst(be_conn))
        ret = lf_ip(tmplog, (struct sockaddr *)be_conn->dst,
                    dst + maxsize - tmplog, tmp);
      else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_SERVERPORT: // %sp
      if (be_conn && conn_get_dst(be_conn))
        ret = lf_port(tmplog, (struct sockaddr *)be_conn->dst,
                      dst + maxsize - tmplog, tmp);
      else
        ret = lf_text_len(tmplog, NULL, 0, dst + maxsize - tmplog, tmp);
    ...
    case LOG_FMT_DATE: // %t = accept date
      ...
      ret =
          date2str_log(tmplog, &tm, &logs->accept_date, dst + maxsize - tmplog);
      if (ret == NULL)
        goto out;
      tmplog = ret;
      last_isspace = 0;
    ...
    case LOG_FMT_tr: // %tr = start of request date
      ...
      tv_ms_add(&tv, &logs->accept_date,
                logs->t_idle >= 0 ? logs->t_idle + logs->t_handshake : 0);
    ...
    case LOG_FMT_DATELOCAL: // %Tl = accept date, local
      get_localtime(logs->accept_date.tv_sec, &tm);
      ret = localdate2str_log(tmplog, logs->accept_date.tv_sec, &tm,
                              dst + maxsize - tmplog);
      if (ret == NULL)
        goto out;
      tmplog = ret;
    ...
    case LOG_FMT_TS: // %Ts
      if (tmp->options & LOG_OPT_HEXA) {
        iret = snprintf(tmplog, dst + maxsize - tmplog, "%04X",
                        (unsigned int)logs->accept_date.tv_sec);
    ...
    case LOG_FMT_MS: // %ms
      if (tmp->options & LOG_OPT_HEXA) {
        iret = snprintf(tmplog, dst + maxsize - tmplog, "%02X",
                        (unsigned int)logs->accept_date.tv_usec / 1000);
        if (iret < 0 || iret > dst + maxsize - tmplog)
          goto out;
        last_isspace = 0;
        tmplog += iret;
        ...
        if ((dst + maxsize - tmplog) < 4)
          goto out;
        ret =
            utoa_pad((unsigned int)logs->accept_date.tv_usec / 1000, tmplog, 4);
        if (ret == NULL)
          goto out;
        tmplog = ret;
        last_isspace = 0;
    ...
    case LOG_FMT_TR: // %TR = HTTP request time
      ret = ltoa_o(
          (t_request >= 0) ? t_request - logs->t_idle - logs->t_handshake : -1,
    ...
    case LOG_FMT_TC: // %Tc
      ret =
          ltoa_o((logs->t_connect >= 0) ? logs->t_connect - logs->t_queue : -1,
                 tmplog, dst + maxsize - tmplog);
      if (ret == NULL)
        goto out;
      tmplog = ret;
    ...
    case LOG_FMT_TD: // %Td
      if (be->mode == PR_MODE_HTTP)
        ...
        ret = ltoa_o((logs->t_connect >= 0) ? logs->t_close - logs->t_connect
                                            : -1,
                     tmplog, dst + maxsize - tmplog);
      if (ret == NULL)
        goto out;
    ...
    case LOG_FMT_TERMSTATE_CK: // %tsc, same as TS with cookie state (for mode
                               // HTTP)
      LOGCHAR(sess_term_cond[(s_flags & SF_ERR_MASK) >> SF_ERR_SHIFT]);
      LOGCHAR(sess_fin_state[(s_flags & SF_FINST_MASK) >> SF_FINST_SHIFT]);
      LOGCHAR((txn && (be->ck_opts & PR_CK_ANY))
                  ? sess_cookie[(txn->flags & TX_CK_MASK) >> TX_CK_SHIFT]
                  : '-');
      LOGCHAR((txn && (be->ck_opts & PR_CK_ANY))
                  ? sess_set_cookie[(txn->flags & TX_SCK_MASK) >> TX_SCK_SHIFT]
                  : '-');
      last_isspace = 0;
      break;
      ...
      if (fe->nb_req_cap && s && s->req_cap) {
        ...
        for (hdr = 0; hdr < fe->nb_req_cap; hdr++) {
          ...
          if (s->req_cap[hdr] != NULL) {
            ret = lf_encode_string(tmplog, dst + maxsize, '#', hdr_encode_map,
                                   s->req_cap[hdr], tmp);
            if (ret == NULL || *ret != '\0')
              goto out;
            tmplog = ret;
    ...
    case LOG_FMT_REQ: // %r
      ...
      ret = lf_encode_string(tmplog, dst + maxsize, '#', url_encode_map, uri,
                             tmp);
      if (ret == NULL || *ret != '\0')
        goto out;
      tmplog = ret;
    ...
    case LOG_FMT_HTTP_PATH: // %HP
      ...
      ret = lf_encode_chunk(tmplog, dst + maxsize, '#', url_encode_map, &chunk,
                            tmp);
      if (ret == NULL || *ret != '\0')
        goto out;
    ...
    case LOG_FMT_UNIQUEID: // %ID
      ret = NULL;
      if (s)
        ret = lf_text_len(tmplog, s->unique_id.ptr, s->unique_id.len,
                          maxsize - (tmplog - dst), tmp);
      else
...
void strm_log(struct stream *s) {
  ...
  err = (s->flags & SF_REDISP) || ((s->flags & SF_ERR_MASK) > SF_ERR_LOCAL) ||
  ...
  if (size > 0) {
    ...
    __send_log(&sess->fe->logsrvs, &sess->fe->log_tag, level, logline, size + 1,
               logline_rfc5424, sd_size);
    s->logs.logwait = 0;
  }
}
...
void sess_log(struct session *sess) {
  ...
  if (!LIST_ISEMPTY(&sess->fe->logformat_sd)) {
    sd_size =
        sess_build_logline(sess, NULL, logline_rfc5424, global.max_syslog_len,
                           &sess->fe->logformat_sd);
  }

  size = sess_build_logline(sess, NULL, logline, global.max_syslog_len,
                            &sess->fe->logformat);
  if (size > 0) {
    _HA_ATOMIC_ADD(&sess->fe->log_count, 1);
    __send_log(&sess->fe->logsrvs, &sess->fe->log_tag, level, logline, size + 1,
...
void app_log(struct list *logsrvs, struct buffer *tag, int level,
             const char *format, ...) {
  va_list argp;
  int data_len;
  ...
  __send_log(logsrvs, tag, level, logline, data_len,
 ...
 *
 * <metadata> must point on a preallocated array of
 * LOG_META_FIELDS*sizeof(struct ist) struct ist len will be set to 0 if field
 * is not found <level> and <facility> will be set to -1 if not found.
 */
void parse_log_message(char *buf, size_t buflen, int *level, int *facility,
  ...
  *message = buf;
  ...
  if ((*size > 2) && (p[0] == '1') && (p[1] == ' ')) {
    ...
    if (metadata[LOG_META_HOST].len == 1 &&
        metadata[LOG_META_HOST].ptr[0] == '-')
    ...
    if (metadata[LOG_META_MSGID].len == 1 &&
        metadata[LOG_META_MSGID].ptr[0] == '-')
    ...
    if (metadata[LOG_META_STDATA].len == 1 &&
        metadata[LOG_META_STDATA].ptr[0] == '-')
...
void syslog_fd_handler(int fd) {
  ...
  if (fdtab[fd].ev & FD_POLL_IN) {
    ...
    do {
      ...
      ret = recvfrom(fd, buf->area, buf->size, 0, (struct sockaddr *)&saddr,
                     &saddrlen);
      if (ret < 0) {
        if (errno == EINTR)
          continue;
      ...
      process_send_log(&l->bind_conf->frontend->logsrvs, level, facility,
...
static void syslog_io_handler(struct appctx *appctx) {
  ...
  while (co_data(si_oc(si))) {
    ...
    process_send_log(&frontend->logsrvs, level, facility, metadata, message,
                     size);
  }
...
int cfg_parse_log_forward(const char *file, int linenum, char **args, int kwm) {
  ...
  if (strcmp(args[0], "log-forward") == 0) {
    ...
    if (px) {
      ha_alert("Parsing [%s:%d]: log-forward section '%s' has the same name as "
               "another log-forward section declared at %s:%d.\n",
    ...
    if (px) {
      ha_alert("Parsing [%s:%d]: log forward section '%s' has the same name as "
               "%s '%s' declared at %s:%d.\n",
               file, linenum, args[1], proxy_type_str(px), px->id,
    ...
    if (warnifnotcap(cfg_log_forward, PR_CAP_FE, file, linenum, args[0],
    ...
    bind_conf = bind_conf_alloc(cfg_log_forward, file, linenum, NULL,
                                xprt_get(XPRT_RAW));
    if (!bind_conf) {
    ...
    if (!str2listener(args[1], cfg_log_forward, bind_conf, file, linenum,
                      &errmsg)) {
      if (errmsg && *errmsg) {
        indent_msg(&errmsg, 2);
    ...
    if (!str2receiver(args[1], cfg_log_forward, bind_conf, file, linenum,
                      &errmsg)) {
      if (errmsg && *errmsg) {
        indent_msg(&errmsg, 2);
        ha_alert("parsing [%s:%d] : '%s %s' : %s\n", file, linenum, args[0],
    ...
    list_for_each_entry(l, &bind_conf->listeners, by_bind) {
      /* the fact that the sockets are of type dgram is guaranteed by
       * str2receiver() */
      l->maxaccept = global.tune.maxaccept ? global.tune.maxaccept : 64;
    ...
    if (!parse_logsrv(args, &cfg_log_forward->logsrvs, (kwm == KWM_NO),
                      &errmsg)) {
    ...
    if (strcmp(args[1], "client") != 0) {
      ha_alert(
          "parsing [%s:%d] : unknown keyword '%s %s' in log-forward section.\n",
    ...
    if (res == PARSE_TIME_OVER) {
      memprintf(&errmsg,
                "timer overflow in argument '%s' to 'timeout client' (maximum "
                "value is 2147483647 ms or ~24.8 days)",
                args[2]);
    } else if (res == PARSE_TIME_UNDER) {
      memprintf(&errmsg,
                "timer underflow in argument '%s' to 'timeout client' (minimum "
                "non-null value is 1 ms)",
                args[2]);
    } else if (res) {
...
/* parse the "show startup-logs" command, returns 1 if a message is returned,
 * otherwise zero */
static int cli_parse_show_startup_logs(char **args, char *payload,
