...
void ha_spin_init(HA_SPINLOCK_T *l) { HA_SPIN_INIT(l); }

/* these calls are used as callbacks at init time when debugging is on */
void ha_rwlock_init(HA_RWLOCK_T *l) { HA_RWLOCK_INIT(l); }
...
static int thread_cpus_enabled() {
  ...
  cpuset_t cpuset;
  if (cpuset_getaffinity(CPU_LEVEL_CPUSET, CPU_WHICH_PID, -1, sizeof(cpuset),
                         &cpuset) == 0)
...
__attribute__((constructor)) static void __thread_init(void) {
  char *ptr = NULL;
  ...
  memprintf(&ptr,
            "Built with multi-threading support (MAX_THREADS=%d, default=%d).",
...
int parse_nbthread(const char *arg, char **err) {
  ...
  if (nbthread != 1) {
    memprintf(
        err,
        "specified with a value other than 1 while HAProxy is not compiled "
        "with threads support. Please check build options for USE_THREAD");
    return 0;
  }
#else
  if (nbthread < 1 || nbthread > MAX_THREADS) {
    memprintf(err, "value must be between 1 and %d (was %ld)", MAX_THREADS,
              nbthread);
    return 0;
  }
