...
static int uxst_bind_listener(struct listener *listener, char *errmsg,
                              int errlen);
...
/*
 * This function initiates a UNIX connection establishment to the target
 * assigned to connection <conn> using (si->{target,dst}). The source address is
 * ignored and will be selected by the system. conn->target may point either to
 * a valid server or to a backend, depending on conn->target. Only
 * OBJ_TYPE_PROXY and OBJ_TYPE_SERVER are supported. The <data> parameter is a
 * boolean indicating whether there are data waiting for being sent or not, in
 * order to adjust data write polling and on some platforms. The <delack>
 * argument is ignored.
 *
...
static int uxst_connect_server(struct connection *conn, int flags) {
  ...
  if (connect(fd, (struct sockaddr *)conn->dst, get_addr_len(conn->dst)) ==
      -1) {
    if (errno == EINPROGRESS || errno == EALREADY) {
      ...
      if (errno == EAGAIN || errno == EADDRNOTAVAIL) {
        msg = "can't connect to destination unix socket, check backlog size on "
              "the server";
      ...
      conn->flags |= CO_FL_ERROR;
      return SF_ERR_SRVTO;
    } else { // (errno == ECONNREFUSED || errno == ENETUNREACH || errno ==
             // EACCES || errno == EPERM)
      close(fd);
      conn->err_code = CO_ER_SOCK_ERR;
      conn->flags |= CO_FL_ERROR;
