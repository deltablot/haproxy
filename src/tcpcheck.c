...
DECLARE_POOL(pool_head_tcpcheck_rule, "tcpcheck_rule",
             sizeof(struct tcpcheck_rule));
...
int tcpcheck_get_step_id(struct check *check, struct tcpcheck_rule *rule) {
  ...
  if (rule->index == 0 && rule->action == TCPCHK_ACT_CONNECT &&
 ...
 * NULL if none was found.
 */
static struct tcpcheck_rule *
get_last_tcpcheck_rule(struct tcpcheck_rules *rules) {
  struct tcpcheck_rule *r;
 ...
 */
static struct tcpcheck_rule *
get_next_tcpcheck_rule(struct tcpcheck_rules *rules,
                       struct tcpcheck_rule *start) {
  struct tcpcheck_rule *r;
...
static void tcpcheck_expect_onerror_message(struct buffer *msg,
                                            struct check *check,
   ...
   *     3. the expect rule is part of a protocol check (http, redis, mysql...),
   * do nothing
  ...
  if (istlen(info)) {
    ...
    msg->data += sess_build_logline(check->sess, NULL, b_tail(msg), b_room(msg),
                                    &rule->expect.onerror_fmt);
    goto comment;
  }

  if (check->type == PR_O2_TCPCHK_CHK &&
      (check->tcpcheck_rules->flags & TCPCHK_RULES_PROTO_CHK) !=
          TCPCHK_RULES_TCP_CHK)
    goto comment;

  chunk_strcat(msg, (match ? "TCPCHK matched unwanted content"
                           : "TCPCHK did not match content"));
  switch (rule->expect.type) {
  case TCPCHK_EXPECT_HTTP_STATUS:
    chunk_appendf(msg, "(status codes) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_STRING:
  case TCPCHK_EXPECT_HTTP_BODY:
    chunk_appendf(msg, " '%.*s' at step %d",
                  (unsigned int)istlen(rule->expect.data),
                  istptr(rule->expect.data), tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_BINARY:
    chunk_appendf(msg, " (binary) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_STRING_REGEX:
  case TCPCHK_EXPECT_HTTP_STATUS_REGEX:
  case TCPCHK_EXPECT_HTTP_BODY_REGEX:
    chunk_appendf(msg, " (regex) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_BINARY_REGEX:
    chunk_appendf(msg, " (binary regex) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_STRING_LF:
  case TCPCHK_EXPECT_HTTP_BODY_LF:
    chunk_appendf(msg, " (log-format string) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_BINARY_LF:
    chunk_appendf(msg, " (log-format binary) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_CUSTOM:
    chunk_appendf(msg, " (custom function) at step %d",
                  tcpcheck_get_step_id(check, rule));
    break;
  case TCPCHK_EXPECT_HTTP_HEADER:
    chunk_appendf(msg, " (header pattern) at step %d",
                  tcpcheck_get_step_id(check, rule));
  case TCPCHK_EXPECT_UNDEF:
    /* Should never happen. */
    return;
 ...
 */
static void tcpcheck_expect_onsuccess_message(struct buffer *msg,
                                              struct check *check,
                                              struct tcpcheck_rule *rule,
                                              struct ist info) {
  struct sample *smp;
 ...
 */
static enum tcpcheck_eval_ret
tcpcheck_mysql_expect_packet(struct check *check, struct tcpcheck_rule *rule,
  ...
  if (*b_peek(&check->bi, offset + 4) == '\xff') {
    ...
    desc =
        ist2(b_peek(&check->bi, offset + 7), b_data(&check->bi) - offset - 7);
    goto error;
  }
   ...
   */
  status =
      ((rule->expect.ok_status != HCHK_STATUS_UNKNOWN) ? rule->expect.ok_status
                                                       : HCHK_STATUS_L7OKD);
...
enum tcpcheck_eval_ret tcpcheck_ldap_expect_bindrsp(struct check *check,
   ...
   */
  /* size of LDAPMessage */
  msglen = (*(b_head(&check->bi) + 1) & 0x80)
               ? (*(b_head(&check->bi) + 1) & 0x7f)
               : 0;
  ...
  msglen += (*(b_head(&check->bi) + msglen + 6) & 0x80)
                ? (*(b_head(&check->bi) + msglen + 6) & 0x7f)
                : 0;
...
tcpcheck_spop_expect_agenthello(struct check *check, struct tcpcheck_rule *rule,
                                int last_read) {
  enum tcpcheck_eval_ret ret = TCPCHK_EVAL_CONTINUE;
  enum healthcheck_status status;
  struct buffer *msg = NULL;
  ...
  if (spoe_handle_healthcheck_response(b_peek(&check->bi, 4), framesz,
                                       b_orig(&trash), HCHK_DESC_LEN) == -1) {
    status = HCHK_STATUS_L7RSP;
...
enum tcpcheck_eval_ret tcpcheck_agent_expect_reply(struct check *check,
  ...
  if (!*p) {
    ...
    set_server_check_status(check, check->status,
                            "Ignoring incomplete line from agent");
    goto out;
  }
  ...
  *p = 0;
  ...
  while (*cmd) {
    ...
    if (strcasecmp(cmd, "up") == 0) {
      check->server->check.health =
          check->server->check.rise + check->server->check.fall - 1;
  ...
  if (hs) {
    ...
    t = get_trash_chunk();
    chunk_printf(t, "via agent : %s%s%s%s", hs, *msg ? " (" : "", msg,
                 *msg ? ")" : "");
...
enum tcpcheck_eval_ret tcpcheck_eval_connect(struct check *check,
  ...
  *conn->dst = (is_addr(&connect->addr)
  ...
  xprt = ((connect->options & TCPCHK_OPT_SSL)
              ? xprt_get(XPRT_SSL)
              : ((connect->options & TCPCHK_OPT_DEFAULT_CONNECT)
                     ? check->xprt
                     : xprt_get(XPRT_RAW)));
  ...
  if ((connect->options & TCPCHK_OPT_SOCKS4) && s &&
      (s->flags & SRV_F_SOCKS4_PROXY)) {
    conn->send_proxy_ofs = 1;
    conn->flags |= CO_FL_SOCKS4;
  } else if ((connect->options & TCPCHK_OPT_DEFAULT_CONNECT) && s &&
             s->check.via_socks4 && (s->flags & SRV_F_SOCKS4_PROXY)) {
    conn->send_proxy_ofs = 1;
    conn->flags |= CO_FL_SOCKS4;
  }
  ...
  if (connect->options & TCPCHK_OPT_SEND_PROXY) {
    ...
    conn->flags |= CO_FL_SEND_PROXY;
             ...
             s->check.send_proxy && !(check->state & CHK_ST_AGENT)) {
    conn->send_proxy_ofs = 1;
    conn->flags |= CO_FL_SEND_PROXY;
  }
  ...
  if (conn_ctrl_ready(conn) &&
      (conn->flags & (CO_FL_SEND_PROXY | CO_FL_SOCKS4))) {
   ...
   */
  if (!s ||
      ((connect->options & TCPCHK_OPT_DEFAULT_CONNECT) && check->mux_proto) ||
    ...
    if (connect->mux_proto)
      ...
      int mode = ((check->tcpcheck_rules->flags & TCPCHK_RULES_PROTO_CHK) ==
                          TCPCHK_RULES_HTTP_CHK
                      ? PROTO_MODE_HTTP
                      : PROTO_MODE_TCP);
    ...
    if (mux_ops &&
        conn_install_mux(conn, mux_ops, cs, proxy, check->sess) < 0) {
      status = SF_ERR_INTERNAL;
      goto fail_check;
    }
  }
...
fail_check:
   ...
   * Additionally, in the case of SF_ERR_RESOURCE, an emergency log will be
   * emitted. Note that we try to prevent the network stack from sending the ACK
   * during the connect() when a pure TCP check is used (without PROXY
   * protocol).
   */
  switch (status) {
  case SF_ERR_NONE:
 ...
 * TCPCHK_EVAL_STOP if an error occurred.
 */
enum tcpcheck_eval_ret tcpcheck_eval_send(struct check *check,
                                          struct tcpcheck_rule *rule) {
  enum tcpcheck_eval_ret ret = TCPCHK_EVAL_CONTINUE;
  ...
  case TCPCHK_SEND_BINARY:
    if (istlen(send->data) >= b_size(&check->bo)) {
      chunk_printf(&trash,
                   "tcp-check send : string too large (%u) for buffer size "
                   "(%u) at step %d",
                   (unsigned int)istlen(send->data),
                   (unsigned int)b_size(&check->bo),
                   tcpcheck_get_step_id(check, rule));
    ...
    break;
  case TCPCHK_SEND_STRING_LF:
    check->bo.data = sess_build_logline(check->sess, NULL, b_orig(&check->bo),
                                        b_size(&check->bo), &rule->send.fmt);
    if (!b_data(&check->bo))
      goto out;
    break;
  case TCPCHK_SEND_BINARY_LF: {
    ...
    if (!tmp)
      goto error_lf;
    tmp->data = sess_build_logline(check->sess, NULL, b_orig(tmp), b_size(tmp),
                                   &rule->send.fmt);
    if (!b_data(tmp))
      goto out;
    tmp->area[tmp->data] = '\0';
  ...
  case TCPCHK_SEND_HTTP: {
    ...
    if (send->http.flags & TCPCHK_SND_HTTP_FL_URI_FMT) {
      tmp->data = sess_build_logline(check->sess, NULL, b_orig(tmp),
                                     b_size(tmp), &send->http.uri_fmt);
    ...
    if ((istlen(vsn) == 6 && *(vsn.ptr + 5) == '2') ||
        (istlen(vsn) == 8 && (*(vsn.ptr + 5) > '1' || (*(vsn.ptr + 5) == '1' &&
                                                       *(vsn.ptr + 7) >= '1'))))
    ...
    if (check->proxy->options2 & PR_O2_CHK_SNDST) {
      ...
      if (!htx_add_header(htx, ist("X-Haproxy-Server-State"),
                          ist2(b_orig(tmp), b_data(tmp))))
        goto error_htx;
    }
...
do_send:
  if (conn->mux->snd_buf(
          cs, &check->bo,
          (IS_HTX_CONN(conn) ? (htxbuf(&check->bo))->data : b_data(&check->bo)),
          0) <= 0) {
  ...
  if ((IS_HTX_CONN(conn) && !htx_is_empty(htxbuf(&check->bo))) ||
      b_data(&check->bo)) {
...
error_htx:
  ...
  chunk_printf(&trash,
               "tcp-check send : failed to build HTTP request at step %d",
               tcpcheck_get_step_id(check, rule));
  set_server_check_status(check, HCHK_STATUS_L7RSP, trash.area);
  ret = TCPCHK_EVAL_STOP;
  goto out;
...
error_lf:
  chunk_printf(&trash,
               "tcp-check send : failed to build log-format string at step %d",
               tcpcheck_get_step_id(check, rule));
  set_server_check_status(check, HCHK_STATUS_L7RSP, trash.area);
  ret = TCPCHK_EVAL_STOP;
  goto out;
...
enum tcpcheck_eval_ret tcpcheck_eval_recv(struct check *check,
  ...
  while ((cs->flags & CS_FL_RCV_MORE) ||
         (!(conn->flags & CO_FL_ERROR) &&
          !(cs->flags & (CS_FL_ERROR | CS_FL_EOS)))) {
    max = (IS_HTX_CS(cs) ? htx_free_space(htxbuf(&check->bi))
                         : b_room(&check->bi));
    read = conn->mux->rcv_buf(cs, &check->bi, max, 0);
    cur_read += read;
    if (!read || (cs->flags & CS_FL_WANT_ROOM) || (--read_poll <= 0) ||
...
end_recv:
  is_empty =
      (IS_HTX_CS(cs) ? htx_is_empty(htxbuf(&check->bi)) : !b_data(&check->bi));
  ...
  if (!cur_read) {
    ...
    if (is_empty) {
      ...
      status = ((rule->expect.err_status != HCHK_STATUS_UNKNOWN)
                    ? rule->expect.err_status
                    : HCHK_STATUS_L7RSP);
...
/* Evaluates an HTTP TCPCHK_ACT_EXPECT rule. If <last_read> is set , no more
 * data are expected. Returns TCPCHK_EVAL_WAIT to wait for more data,
 * TCPCHK_EVAL_CONTINUE to evaluate the next rule or TCPCHK_EVAL_STOP if an
 * error occurred.
 */
enum tcpcheck_eval_ret tcpcheck_eval_expect_http(struct check *check,
  ...
  last_read |=
      (!htx_free_data_space(htx) || (htx_get_tail_type(htx) == HTX_BLK_EOM));
  ...
  if (check->server && (check->server->proxy->options & PR_O_DISABLE404) &&
      (check->server->next_state != SRV_ST_STOPPED) && (check->code == 404)) {
  ...
  case TCPCHK_EXPECT_HTTP_STATUS_REGEX:
    match =
        regex_exec2(expect->regex, HTX_SL_RES_CPTR(sl), HTX_SL_RES_CLEN(sl));
  ...
  case TCPCHK_EXPECT_HTTP_HEADER: {
    ...
    int full = (expect->flags & (TCPCHK_EXPT_FL_HTTP_HVAL_NONE |
                                 TCPCHK_EXPT_FL_HTTP_HVAL_FULL));
    ...
    if (expect->flags & TCPCHK_EXPT_FL_HTTP_HNAME_FMT) {
      ...
      nbuf->data = sess_build_logline(check->sess, NULL, b_orig(nbuf),
                                      b_size(nbuf), &expect->hdr.name_fmt);
      if (!b_data(nbuf)) {
        status = HCHK_STATUS_L7RSP;
    ...
    if (expect->flags & TCPCHK_EXPT_FL_HTTP_HVAL_FMT) {
      ...
      vbuf->data = sess_build_logline(check->sess, NULL, b_orig(vbuf),
                                      b_size(vbuf), &expect->hdr.value_fmt);
      if (!b_data(vbuf)) {
        status = HCHK_STATUS_L7RSP;
        desc = ist("log-format string evaluated to an empty string");
        goto error;
    ...
    while (1) {
      ...
      case TCPCHK_EXPT_FL_HTTP_HVAL_END:
        if (istlen(value) < istlen(vpat))
          break;
        value =
            ist2(istptr(value) + istlen(value) - istlen(vpat), istlen(vpat));
        if (isteq(value, vpat)) {
          match = 1;
          goto end_of_match;
        }
  ...
  case TCPCHK_EXPECT_HTTP_BODY_LF:
    ...
    if (!last_read &&
        ((expect->type == TCPCHK_EXPECT_HTTP_BODY &&
          b_data(&trash) < istlen(expect->data)) ||
         ((expect->type == TCPCHK_EXPECT_HTTP_BODY_LF &&
           b_data(&trash) < b_data(tmp))) ||
    ...
    if (expect->type == TCPCHK_EXPECT_HTTP_BODY)
      match = my_memmem(b_orig(&trash), b_data(&trash), istptr(expect->data),
                        istlen(expect->data)) != NULL;
    else if (expect->type == TCPCHK_EXPECT_HTTP_BODY_LF)
      match = my_memmem(b_orig(&trash), b_data(&trash), b_orig(tmp),
                        b_data(tmp)) != NULL;
    else
    ...
    status = ((status != HCHK_STATUS_UNKNOWN) ? status : HCHK_STATUS_L7RSP);
    if (LIST_ISEMPTY(&expect->onerror_fmt))
      desc = (inverse ? ist("HTTP check matched unwanted content")
 ...
 */
enum tcpcheck_eval_ret tcpcheck_eval_expect(struct check *check,
                                            struct tcpcheck_rule *rule,
                                            int last_read) {
  enum tcpcheck_eval_ret ret = TCPCHK_EVAL_CONTINUE;
  struct tcpcheck_expect *expect = &rule->expect;
   ...
   */
  if (!last_read) {
    if ((expect->type == TCPCHK_EXPECT_STRING ||
  ...
  /* Make GCC happy ; initialize match to a failure state. */
  match = inverse;
  status = ((expect->err_status != HCHK_STATUS_UNKNOWN) ? expect->err_status
                                                        : HCHK_STATUS_L7RSP);

  switch (expect->type) {
  case TCPCHK_EXPECT_STRING:
  case TCPCHK_EXPECT_BINARY:
    match = my_memmem(b_head(&check->bi), b_data(&check->bi),
                      istptr(expect->data), istlen(expect->data)) != NULL;
    break;
  case TCPCHK_EXPECT_STRING_REGEX:
    match = regex_exec2(expect->regex, b_head(&check->bi),
  ...
  case TCPCHK_EXPECT_BINARY_REGEX:
    ...
    match = regex_exec2(expect->regex, b_head(&trash),
                        MIN(b_data(&trash), b_size(&trash) - 1));
    break;
  ...
  case TCPCHK_EXPECT_BINARY_LF:
    ...
    if (expect->type == TCPCHK_EXPECT_BINARY_LF) {
      ...
      if (parse_binary(b_orig(tmp), &tmp->area, &len, NULL) == 0) {
        status = HCHK_STATUS_L7RSP;
        desc = ist("Failed to parse hexastring resulting of eval of a "
                   "log-format string");
        goto error;
      }
      tmp->data = len;
    ...
    match = my_memmem(b_head(&check->bi), b_data(&check->bi), b_orig(tmp),
                      b_data(tmp)) != NULL;
    break;
...
error:
  /* From this point on, we matched something we did not want, this is an error
   * state. */
  ret = TCPCHK_EVAL_STOP;
  msg = alloc_trash_chunk();
  if (msg)
 ...
 * waits.
 */
enum tcpcheck_eval_ret tcpcheck_eval_action_kw(struct check *check,
                                               struct tcpcheck_rule *rule) {
  enum tcpcheck_eval_ret ret = TCPCHK_EVAL_CONTINUE;
...
int tcpcheck_main(struct check *check) {
    ...
    case TCPCHK_ACT_CONNECT:
      ...
      if (cs && check->current_step != rule) {
        ...
        if (check->wait_list.events)
          cs->conn->mux->unsubscribe(cs, check->wait_list.events,
                                     &check->wait_list);
      ...
      conn = cs_conn(cs);
      last_read = 0;
      must_read = ((cs && IS_HTX_CS(cs)) ? htx_is_empty(htxbuf(&check->bi))
                                         : !b_data(&check->bi));
      break;
    case TCPCHK_ACT_SEND:
      check->current_step = rule;
    ...
    case TCPCHK_ACT_EXPECT:
      ...
      if (must_read) {
        ...
        if (eval_ret == TCPCHK_EVAL_STOP)
          ...
          goto out;
        last_read = ((conn->flags & CO_FL_ERROR) ||
                     (cs->flags & (CS_FL_ERROR | CS_FL_EOS)));
        must_read = 0;
      }

      eval_ret = ((check->tcpcheck_rules->flags & TCPCHK_RULES_PROTO_CHK) ==
                          TCPCHK_RULES_HTTP_CHK
  ...
  if (check->current_step) {
    ...
    if (rule->action == TCPCHK_ACT_EXPECT) {
      ...
      status = ((rule->expect.ok_status != HCHK_STATUS_UNKNOWN)
                    ? rule->expect.ok_status
                    : HCHK_STATUS_L7OKD);
      ...
      const char *msg =
          ((rule->connect.options & TCPCHK_OPT_IMPLICIT) ? NULL
                                                         : "(tcp-check)");
...
parse_tcpcheck_action(char **args, int cur_arg, struct proxy *px,
  ...
  cur_arg++;
  if (kw->parse((const char **)args, &cur_arg, px, actrule, errmsg) ==
      ACT_RET_PRS_ERR) {
 ...
 * returned on error.
 */
struct tcpcheck_rule *parse_tcpcheck_connect(char **args, int cur_arg,
                                             struct proxy *px,
                                             struct list *rules,
  ...
  list_for_each_entry(chk, rules, list) {
    ...
    memprintf(
        errmsg,
        "first step MUST also be a 'connect', "
  ...
  while (*(args[cur_arg])) {
    if (strcmp(args[cur_arg], "default") == 0)
      ...
      if (!*(args[cur_arg + 1])) {
        memprintf(errmsg, "'%s' expects <ipv4|ipv6> as argument.",
                  args[cur_arg]);
        goto error;
      }
      ...
      if (!*(args[cur_arg + 1])) {
        memprintf(
            errmsg,
            "'%s' expects a port number or a sample expression as argument.",
            args[cur_arg]);
        goto error;
      }
      cur_arg++;
      ...
      if (p != end) {
        ...
        if (!port_expr) {
          memprintf(errmsg, "error detected while parsing port expression : %s",
                    *errmsg);
          goto error;
        }
        if (!(port_expr->fetch->val & SMP_VAL_BE_CHK_RUL)) {
          memprintf(errmsg,
                    "error detected while parsing port expression : "
        ...
        memprintf(errmsg,
                  "expects a valid TCP port (from range 1 to 65535) or a "
                  "sample expression, got %s.",
                  args[cur_arg]);
        goto error;
      }
    } else if (strcmp(args[cur_arg], "proto") == 0) {
      if (!*(args[cur_arg + 1])) {
        memprintf(errmsg, "'%s' expects a MUX protocol as argument.",
                  args[cur_arg]);
        goto error;
      }
      mux_proto =
          get_mux_proto(ist2(args[cur_arg + 1], strlen(args[cur_arg + 1])));
      if (!mux_proto) {
        memprintf(errmsg, "'%s' : unknown MUX protocol '%s'.", args[cur_arg],
                  args[cur_arg + 1]);
        goto error;
      }

      if (strcmp(args[0], "tcp-check") == 0 &&
          mux_proto->mode != PROTO_MODE_TCP) {
        memprintf(errmsg, "'%s' : invalid MUX protocol '%s' for tcp-check",
                  args[cur_arg], args[cur_arg + 1]);
        goto error;
      } else if (strcmp(args[0], "http-check") == 0 &&
                 mux_proto->mode != PROTO_MODE_HTTP) {
        memprintf(errmsg, "'%s' : invalid MUX protocol '%s' for http-check",
                  args[cur_arg], args[cur_arg + 1]);
        goto error;
      }
      ...
      cur_arg++;
#else
      memprintf(errmsg, "'%s' : library does not support TLS ALPN extension.",
                args[cur_arg]);
      goto error;
#endif
    }
      ...
      memprintf(
          errmsg,
          "expects 'comment', 'port', 'addr', 'send-proxy'"
#ifdef USE_OPENSSL
          ", 'ssl', 'sni', 'alpn'"
#endif /* USE_OPENSSL */
...
struct tcpcheck_rule *parse_tcpcheck_send(char **args, int cur_arg,
                                          struct proxy *px, struct list *rules,
                                          const char *file, int line,
                                          char **errmsg) {
  struct tcpcheck_rule *chk = NULL;
  ...
  if (!*(args[cur_arg + 1])) {
    memprintf(errmsg, "'%s' expects a %s as argument",
              (type == TCPCHK_SEND_BINARY ? "binary string" : "string"),
              args[cur_arg]);
    goto error;
  }
  ...
  case TCPCHK_SEND_BINARY_LF:
    ...
    px->conf.args.ctx = ARGC_SRV;
    if (!parse_logformat_string(data, px, &chk->send.fmt, 0, SMP_VAL_BE_CHK_RUL,
                                errmsg)) {
...
struct tcpcheck_rule *parse_tcpcheck_send_http(char **args, int cur_arg,
                                               struct proxy *px,
                                               struct list *rules,
                                               const char *file, int line,
                                               char **errmsg) {
  ...
  while (*(args[cur_arg])) {
    if (strcmp(args[cur_arg], "meth") == 0) {
      ...
      if (!*args[cur_arg + 1] || !*args[cur_arg + 2]) {
        memprintf(errmsg, "'%s' expects <name> and <value> as arguments",
                  args[cur_arg]);
        goto error;
      }
      ...
      if (strcasecmp(args[cur_arg + 1], "host") == 0) {
        if (host_hdr >= 0) {
          memprintf(errmsg,
                    "'%s' header already defined (previous value is '%s')",
    ...
    skip_hdr:
      ...
      memprintf(errmsg,
                "expects 'comment', 'meth', 'uri', 'uri-lf', 'ver', 'hdr', "
                "'body' or 'body-lf'"
  ...
  if (uri) {
    if (chk->send.http.flags & TCPCHK_SND_HTTP_FL_URI_FMT) {
      ...
      px->conf.args.ctx = ARGC_SRV;
      if (!parse_logformat_string(uri, px, &chk->send.http.uri_fmt, 0,
                                  SMP_VAL_BE_CHK_RUL, errmsg)) {
        memprintf(errmsg, "'%s' invalid log-format string (%s).\n", uri,
                  *errmsg);
        goto error;
      }
  ...
  for (i = 0; istlen(hdrs[i].n); i++) {
    ...
    ist0(hdrs[i].v);
    if (!parse_logformat_string(istptr(hdrs[i].v), px, &hdr->value, 0,
                                SMP_VAL_BE_CHK_RUL, errmsg))
      goto error;
  ...
  if (body) {
    if (chk->send.http.flags & TCPCHK_SND_HTTP_FL_BODY_FMT) {
      ...
      px->conf.args.ctx = ARGC_SRV;
      if (!parse_logformat_string(body, px, &chk->send.http.body_fmt, 0,
                                  SMP_VAL_BE_CHK_RUL, errmsg)) {
        memprintf(errmsg, "'%s' invalid log-format string (%s).\n", body,
                  *errmsg);
        goto error;
...
struct tcpcheck_rule *parse_tcpcheck_comment(char **args, int cur_arg,
                                             struct proxy *px,
                                             struct list *rules,
                                             const char *file, int line,
                                             char **errmsg) {
 ...
 */
struct tcpcheck_rule *
parse_tcpcheck_expect(char **args, int cur_arg, struct proxy *px,
                      struct list *rules, unsigned int proto, const char *file,
                      int line, char **errmsg) {
  ...
  rescan:
    if (strcmp(args[cur_arg], "min-recv") == 0) {
      ...
      if (min_recv < -1 || min_recv > INT_MAX) {
        memprintf(errmsg, "'%s' expects -1 or an integer from 0 to INT_MAX",
                  args[cur_arg - 1]);
        goto error;
      }
      ...
      if (proto != TCPCHK_RULES_HTTP_CHK)
        type = ((*(args[cur_arg]) == 's') ? TCPCHK_EXPECT_STRING
                                          : TCPCHK_EXPECT_STRING_REGEX);
      else
        type = ((*(args[cur_arg]) == 's') ? TCPCHK_EXPECT_HTTP_BODY
                                          : TCPCHK_EXPECT_HTTP_BODY_REGEX);

      if (!*(args[cur_arg + 1])) {
        memprintf(errmsg, "'%s' expects a <pattern> as argument",
                  args[cur_arg]);
        goto error;
      }
      cur_arg++;
      pattern = args[cur_arg];
    } else if (strcmp(args[cur_arg], "binary") == 0 ||
               strcmp(args[cur_arg], "rbinary") == 0) {
      if (proto == TCPCHK_RULES_HTTP_CHK)
        goto bad_http_kw;
      if (type != TCPCHK_EXPECT_UNDEF) {
        memprintf(errmsg, "only on pattern expected");
      ...
      type = ((*(args[cur_arg]) == 'b') ? TCPCHK_EXPECT_BINARY
                                        : TCPCHK_EXPECT_BINARY_REGEX);
      ...
      cur_arg++;
      pattern = args[cur_arg];
    } else if (strcmp(args[cur_arg], "string-lf") == 0 ||
               strcmp(args[cur_arg], "binary-lf") == 0) {
      if (type != TCPCHK_EXPECT_UNDEF) {
        memprintf(errmsg, "only on pattern expected");
        goto error;
      }
      if (proto != TCPCHK_RULES_HTTP_CHK)
        type = ((*(args[cur_arg]) == 's') ? TCPCHK_EXPECT_STRING_LF
                                          : TCPCHK_EXPECT_BINARY_LF);
      else {
        if (*(args[cur_arg]) != 's')
          goto bad_http_kw;
      ...
      cur_arg++;
      pattern = args[cur_arg];
    } else if (strcmp(args[cur_arg], "status") == 0 ||
               strcmp(args[cur_arg], "rstatus") == 0) {
      if (proto != TCPCHK_RULES_HTTP_CHK)
        goto bad_tcp_kw;
      if (type != TCPCHK_EXPECT_UNDEF) {
        memprintf(errmsg, "only on pattern expected");
      ...
      type = ((*(args[cur_arg]) == 's') ? TCPCHK_EXPECT_HTTP_STATUS
      ...
      if (!*(args[cur_arg + 1]) || !*(args[cur_arg + 2]) ||
          (strcmp(args[cur_arg + 1], "name") != 0 &&
           strcmp(args[cur_arg + 1], "name-lf") != 0)) {
        memprintf(errmsg,
                  "'%s' expects at the name keyword as first argument followed "
                  "by a pattern",
                  args[orig_arg]);
        goto error;
      }
      ...
      if (strcmp(args[cur_arg], "-m") == 0) {
        if (!*(args[cur_arg + 1])) {
          memprintf(errmsg,
                    "'%s' : '%s' expects at a matching pattern ('str', 'beg', "
                    "'end', 'sub' or 'reg')",
        ...
        if (strcmp(args[cur_arg + 1], "str") == 0)
          ...
          if (flags & TCPCHK_EXPT_FL_HTTP_HNAME_FMT) {
            memprintf(errmsg,
                      "'%s': log-format string is not supported with a regex "
                      "matching method",
                      args[orig_arg]);
            goto error;
          }
          ...
          memprintf(errmsg,
                    "'%s' : '%s' only supports 'str', 'beg', 'end', 'sub' or "
                    "'reg' (got '%s')",
      ...
      if (!*(args[cur_arg + 1]) ||
          (strcmp(args[cur_arg + 1], "value") != 0 &&
           strcmp(args[cur_arg + 1], "value-lf") != 0)) {
      ...
      if (!*(args[cur_arg + 2])) {
        memprintf(errmsg, "'%s' expect a pattern with the value keyword",
                  args[orig_arg]);
        goto error;
      }
      vpat = args[cur_arg + 2];
      ...
      if (strcasecmp(args[cur_arg + 1], "L7OK") == 0)
        ...
        memprintf(errmsg,
                  "'%s' only supports 'L4OK', 'L6OK', 'L7OK' or 'L7OKC' status "
                  "(got '%s').",
                  args[cur_arg], args[cur_arg + 1]);
        goto error;
      }
      ...
      if (strcasecmp(args[cur_arg + 1], "L7RSP") == 0)
        ...
        memprintf(errmsg,
                  "'%s' only supports 'L4CON', 'L6RSP', 'L7RSP' or 'L7STS' "
                  "status (got '%s').",
                  args[cur_arg], args[cur_arg + 1]);
        goto error;
      }
      ...
      if (!*(args[cur_arg + 1])) {
        memprintf(errmsg, "'%s' expects an expression as argument",
                  args[cur_arg]);
        goto error;
      }
      ...
      if (!status_expr) {
        memprintf(errmsg,
                  "error detected while parsing status-code expression : %s",
                  *errmsg);
        goto error;
      }
      if (!(status_expr->fetch->val & SMP_VAL_BE_CHK_RUL)) {
        memprintf(errmsg,
                  "error detected while parsing status-code expression : "
                  " fetch method '%s' extracts information from '%s', "
                  "none of which is available here.\n",
      ...
      if (strcasecmp(args[cur_arg + 1], "L7TOUT") == 0)
        ...
        memprintf(errmsg,
                  "'%s' only supports 'L4TOUT', 'L6TOUT' or 'L7TOUT' status "
                  "(got '%s').",
                  args[cur_arg], args[cur_arg + 1]);
        goto error;
      }
      ...
      bad_http_kw:
        memprintf(errmsg,
                  "'only supports min-recv, [!]string', '[!]rstring', "
                  "'[!]string-lf', '[!]status', "
                  "'[!]rstatus', [!]hdr, [!]fhdr or comment but got '%s' as "
                  "argument.",
                  args[cur_arg]);
      } else {
      bad_tcp_kw:
        memprintf(
            errmsg,
            "'only supports min-recv, '[!]binary', '[!]string', '[!]rstring', "
            "'[!]string-lf'"
  ...
  if (on_success_msg) {
    px->conf.args.ctx = ARGC_SRV;
    if (!parse_logformat_string(on_success_msg, px, &chk->expect.onsuccess_fmt,
                                0, SMP_VAL_BE_CHK_RUL, errmsg)) {
      memprintf(errmsg, "'%s' invalid log-format string (%s).\n",
  ...
  if (on_error_msg) {
    px->conf.args.ctx = ARGC_SRV;
    if (!parse_logformat_string(on_error_msg, px, &chk->expect.onerror_fmt, 0,
                                SMP_VAL_BE_CHK_RUL, errmsg)) {
      memprintf(errmsg, "'%s' invalid log-format string (%s).\n", on_error_msg,
                *errmsg);
      goto error;
  ...
  case TCPCHK_EXPECT_HTTP_STATUS: {
    ...
    while (1) {
      ...
      chk->expect.codes.codes =
          my_realloc2(chk->expect.codes.codes,
      ...
      if (*p == '\0')
        break;
      if (*p != ',') {
        memprintf(errmsg, "invalid character '%c' in the list of status codes",
                  *p);
        goto error;
      }
      p++;
  ...
  case TCPCHK_EXPECT_HTTP_BODY_LF:
    ...
    px->conf.args.ctx = ARGC_SRV;
    if (!parse_logformat_string(pattern, px, &chk->expect.fmt, 0,
                                SMP_VAL_BE_CHK_RUL, errmsg)) {
      memprintf(errmsg, "'%s' invalid log-format string (%s).\n", pattern,
                *errmsg);
      goto error;
  ...
  case TCPCHK_EXPECT_HTTP_HEADER:
    ...
    if (chk->expect.flags & TCPCHK_EXPT_FL_HTTP_HNAME_REG) {
      ...
      if (!parse_logformat_string(npat, px, &chk->expect.hdr.name_fmt, 0,
                                  SMP_VAL_BE_CHK_RUL, errmsg)) {
        memprintf(errmsg, "'%s' invalid log-format string (%s).\n", npat,
                  *errmsg);
        goto error;
    ...
    if (!vpat) {
      ...
      if (!parse_logformat_string(vpat, px, &chk->expect.hdr.value_fmt, 0,
                                  SMP_VAL_BE_CHK_RUL, errmsg)) {
        memprintf(errmsg, "'%s' invalid log-format string (%s).\n", npat,
                  *errmsg);
        goto error;
  ...
  list_for_each_entry_rev(prev_check, rules, list) {
    ...
    if (prev_check->action != TCPCHK_ACT_COMMENT &&
        prev_check->action != TCPCHK_ACT_ACTION_KW)
      break;
  }
  return chk;
 ...
 */
void tcpcheck_overwrite_send_http_rule(struct tcpcheck_rule *old,
                                       struct tcpcheck_rule *new) {
  struct logformat_node *lf, *lfb;
  struct tcpcheck_http_hdr *hdr, *bhdr;
  ...
  if (!(new->send.http.flags &TCPCHK_SND_HTTP_FL_URI_FMT) &&
      isttest(new->send.http.uri)) {
  ...
  } else if ((new->send.http.flags &TCPCHK_SND_HTTP_FL_URI_FMT) &&
             !LIST_ISEMPTY(&new->send.http.uri_fmt)) {
    if (!(old->send.http.flags & TCPCHK_SND_HTTP_FL_URI_FMT))
      istfree(&old->send.http.uri);
    else
      free_tcpcheck_fmt(&old->send.http.uri_fmt);
  ...
  if (!(new->send.http.flags &TCPCHK_SND_HTTP_FL_BODY_FMT) &&
  ...
  } else if ((new->send.http.flags &TCPCHK_SND_HTTP_FL_BODY_FMT) &&
             !LIST_ISEMPTY(&new->send.http.body_fmt)) {
    if (!(old->send.http.flags & TCPCHK_SND_HTTP_FL_BODY_FMT))
      istfree(&old->send.http.body);
    else
      free_tcpcheck_fmt(&old->send.http.body_fmt);
 ...
 * message.
 */
int tcpcheck_add_http_rule(struct tcpcheck_rule *chk,
                           struct tcpcheck_rules *rules, char **errmsg) {
  struct tcpcheck_rule *r;
  ...
  if (chk->action == TCPCHK_ACT_SEND &&
      (chk->send.http.flags & TCPCHK_SND_HTTP_FROM_OPT)) {
    /* Tries to add an implicit http-check send rule from an "option httpchk"
     * line. First, the first rule is retrieved, skipping the first CONNECT, if
     * any, and following tests are performed :
     *
     *  1- If there is no such rule or if it is not a send rule, the implicit
     * send rule is pushed in front of the ruleset
     *
     *  2- If it is another implicit send rule, it is replaced with the new one.
     *
     *  3- Otherwise, it means it is an explicit send rule. In this case we
     * merge both, overwriting the old send rule (the explicit one) with info of
     * the new send rule (the implicit one).
     */
    ...
    /* Tries to add an explicit http-check rule. First of all we check the
     * typefo the last inserted rule to be sure it is valid. Then for send rule,
     * we try to merge it with an existing implicit send rule, if any. At the
     * end, if there is no error, the rule is appended to the list.
     */
    ...
    if (!r || (r->action == TCPCHK_ACT_SEND &&
      ...
      /* no error */;
    else if (r->action != TCPCHK_ACT_CONNECT &&
             chk->action == TCPCHK_ACT_SEND) {
      memprintf(errmsg,
                "unable to add http-check send rule at step %d (missing "
                "connect rule).",
                chk->index + 1);
      return 0;
    } else if (r->action != TCPCHK_ACT_SEND && r->action != TCPCHK_ACT_EXPECT &&
               chk->action == TCPCHK_ACT_EXPECT) {
      memprintf(errmsg,
                "unable to add http-check expect rule at step %d (missing send "
                "rule).",
                chk->index + 1);
      return 0;
    } else if (r->action != TCPCHK_ACT_EXPECT &&
               chk->action == TCPCHK_ACT_CONNECT) {
      memprintf(errmsg,
                "unable to add http-check connect rule at step %d (missing "
                "expect rule).",
                chk->index + 1);
      return 0;
    }
...
static int check_proxy_tcpcheck(struct proxy *px) {
  ...
  if (!(px->cap & PR_CAP_BE) ||
      (px->options2 & PR_O2_CHK_ANY) != PR_O2_TCPCHK_CHK) {
  ...
  if (!px->tcpcheck_rules.list) {
    ha_alert(
        "config : proxy '%s' : tcp-check configured but no ruleset defined.\n",
        px->id);
  ...
  /* HTTP ruleset only :  */
  if ((px->tcpcheck_rules.flags & TCPCHK_RULES_PROTO_CHK) ==
      TCPCHK_RULES_HTTP_CHK) {
    struct tcpcheck_rule *next;

    /* move remaining implicit send rule from "option httpchk" line to the right
     * place. If such rule exists, it must be the first one. In this case, the
     * rule is moved after the first connect rule, if any. Otherwise, nothing is
     * done.
     */
    ...
    /* add implicit expect rule if the last one is a send. It is inherited from
     * previous versions where the http expect rule was optional. Now it is
     * possible to chained send/expect rules but the last expect may still be
     * implicit.
     */
    chk = get_last_tcpcheck_rule(&px->tcpcheck_rules);
    if (chk && chk->action == TCPCHK_ACT_SEND) {
      next = parse_tcpcheck_expect(
          (char *[]){"http-check", "expect", "status", "200-399", ""}, 1, px,
          px->tcpcheck_rules.list, TCPCHK_RULES_HTTP_CHK, px->conf.file,
          px->conf.line, &errmsg);
      if (!next) {
        ha_alert("config : proxy '%s': unable to add implicit http-check "
                 "expect rule "
                 "(%s).\n",
                 px->id, errmsg);
        free(errmsg);
  ...
  if (!chk || chk->action != TCPCHK_ACT_CONNECT) {
    ...
    if (!chk) {
      ha_alert(
          "config : proxy '%s': unable to add implicit tcp-check connect rule "
          "(out of memory).\n",
          px->id);
      ret |= ERR_ALERT | ERR_FATAL;
      goto out;
...
int add_tcpcheck_send_strs(struct tcpcheck_rules *rules,
...
static int proxy_parse_tcpcheck(char **args, int section, struct proxy *curpx,
  ...
  chunk_printf(&trash, "*tcp-check-%s_%s-%d",
               ((curpx == defpx) ? "defaults" : curpx->id), curpx->conf.file,
               curpx->conf.line);
  ...
  if (strcmp(args[cur_arg], "connect") == 0)
    chk = parse_tcpcheck_connect(args, cur_arg, curpx, &rs->rules, file, line,
                                 errmsg);
  else if (strcmp(args[cur_arg], "send") == 0 ||
           strcmp(args[cur_arg], "send-binary") == 0 ||
           strcmp(args[cur_arg], "send-lf") == 0 ||
           strcmp(args[cur_arg], "send-binary-lf") == 0)
    chk = parse_tcpcheck_send(args, cur_arg, curpx, &rs->rules, file, line,
                              errmsg);
  else if (strcmp(args[cur_arg], "expect") == 0)
    chk = parse_tcpcheck_expect(args, cur_arg, curpx, &rs->rules, 0, file, line,
                                errmsg);
  else if (strcmp(args[cur_arg], "comment") == 0)
    chk = parse_tcpcheck_comment(args, cur_arg, curpx, &rs->rules, file, line,
                                 errmsg);
  else {
    ...
    if (!kw) {
      ...
      memprintf(errmsg,
                "'%s' only supports 'comment', 'connect', 'send', "
                "'send-binary', 'expect'"
                "%s%s. but got '%s'",
                args[0], (*trash.area ? ", " : ""), trash.area, args[1]);
      goto error;
    }
    chk = parse_tcpcheck_action(args, cur_arg, curpx, &rs->rules, kw, file,
                                line, errmsg);
  }
...
static int proxy_parse_httpcheck(char **args, int section, struct proxy *curpx,
  ...
  if (strcmp(args[cur_arg], "connect") == 0)
    ...
    chk = parse_tcpcheck_send_http(args, cur_arg, curpx, &rs->rules, file, line,
                                   errmsg);
  else if (strcmp(args[cur_arg], "expect") == 0)
    chk = parse_tcpcheck_expect(args, cur_arg, curpx, &rs->rules,
                                TCPCHK_RULES_HTTP_CHK, file, line, errmsg);
  else if (strcmp(args[cur_arg], "comment") == 0)
    chk = parse_tcpcheck_comment(args, cur_arg, curpx, &rs->rules, file, line,
                                 errmsg);
  else {
    ...
    if (!kw) {
      action_kw_tcp_check_build_list(&trash);
      memprintf(errmsg,
                "'%s' only supports 'disable-on-404', 'send-state', 'comment', "
                "'connect',"
...
int proxy_parse_redis_check_opt(char **args, int cur_arg, struct proxy *curpx,
                                struct proxy *defpx, const char *file,
                                int line) {
  ...
  chk = parse_tcpcheck_send((char *[]){"tcp-check", "send", redis_req, ""}, 1,
                            curpx, &rs->rules, file, line, &errmsg);
  if (!chk) {
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "string", redis_res, "error-status",
                 "L7STS", "on-error", "%[res.payload(0,0),cut_crlf]",
...
int proxy_parse_ssl_hello_chk_opt(char **args, int cur_arg, struct proxy *curpx,
                                  struct proxy *defpx, const char *file,
                                  int line) {
  ...
  static char sslv3_client_hello[] = {
      ...
      "0002"
      "0003"
      "0004" /* 39 most common ciphers :  */
      "0005"
      "0006"
      "0007"
      "0008" /* 0x01...0x1B, 0x2F...0x3A  */
      "0009"
      "000A"
      "000B"
      "000C" /* This covers RSA/DH,       */
      "000D"
      "000E"
      "000F"
      "0010" /* various bit lengths,      */
      "0011"
      "0012"
      "0013"
      "0014" /* SHA1/MD5, DES/3DES/AES... */
      "0015"
      "0016"
      "0017"
      "0018"
  ...
  chk = parse_tcpcheck_send(
      (char *[]){"tcp-check", "send-binary-lf", sslv3_client_hello, ""}, 1,
      curpx, &rs->rules, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rbinary", "^1[56]", "min-recv", "5",
                 "ok-status", "L6OK", "error-status", "L6RSP", "tout-status",
                 "L6TOUT", ""},
...
int proxy_parse_smtpchk_opt(char **args, int cur_arg, struct proxy *curpx,
                            struct proxy *defpx, const char *file, int line) {
  ...
  if (*args[cur_arg] && *args[cur_arg + 1] &&
      (strcmp(args[cur_arg], "EHLO") == 0 ||
       strcmp(args[cur_arg], "HELO") == 0)) {
    /* <EHLO|HELO> + space (1) + <host> + null byte (1) */
    cmd = calloc(strlen(args[cur_arg]) + 1 + strlen(args[cur_arg + 1]) + 1,
                 sizeof(*cmd));
    if (cmd)
      sprintf(cmd, "%s %s", args[cur_arg], args[cur_arg + 1]);
  } else {
    /* this just hits the default for now, but you could potentially expand it
       to allow for other stuff though, it's unlikely you'd want to send
       anything other than an EHLO or HELO */
  ...
  chk = parse_tcpcheck_connect(
      (char *[]){"tcp-check", "connect", "default", "linger", ""}, 1, curpx,
      &rs->rules, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  }
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rstring", "^[0-9]{3}[ \r]", "min-recv",
                 "4", "error-status", "L7RSP", "on-error",
                 "%[res.payload(0,0),cut_crlf]", ""},
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rstring", "^2[0-9]{2}[ \r]",
                 "min-recv", "4", "error-status", "L7STS", "on-error",
                 "%[res.payload(4,0),ltrim(' '),cut_crlf]", "status-code",
                 "res.payload(0,3)", ""},
      1, curpx, &rs->rules, TCPCHK_RULES_SMTP_CHK, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
  ...
  chk = parse_tcpcheck_send((char *[]){"tcp-check", "send-lf", smtp_req, ""}, 1,
                            curpx, &rs->rules, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rstring", "^2[0-9]{2}[- \r]",
                 "min-recv", "4", "error-status", "L7STS", "on-error",
                 "%[res.payload(4,0),ltrim(' '),cut_crlf]", "on-success",
                 "%[res.payload(4,0),ltrim(' '),cut_crlf]", "status-code",
                 "res.payload(0,3)", ""},
      1, curpx, &rs->rules, TCPCHK_RULES_SMTP_CHK, file, line, &errmsg);
  if (!chk) {
...
int proxy_parse_pgsql_check_opt(char **args, int cur_arg, struct proxy *curpx,
                                struct proxy *defpx, const char *file,
                                int line) {
  ...
  if (!*args[cur_arg] || !*args[cur_arg + 1]) {
    ha_alert(
        "parsing [%s:%d] : '%s %s' expects 'user <username>' as argument.\n",
  ...
  chk = parse_tcpcheck_send(
      (char *[]){"tcp-check", "send-binary-lf", pgsql_req, ""}, 1, curpx,
      &rs->rules, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "!rstring", "^E", "min-recv", "5",
                 "error-status", "L7RSP", "on-error", "%[res.payload(6,0)]",
                 ""},
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rbinary",
                 "^52000000(08|0A|0C)000000(00|02|03|04|05|06)", "min-recv",
                 "9", "error-status", "L7STS", "on-success",
                 "PostgreSQL server is ok", "on-error",
                 "PostgreSQL unknown error", ""},
      1, curpx, &rs->rules, TCPCHK_RULES_PGSQL_CHK, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
...
int proxy_parse_mysql_check_opt(char **args, int cur_arg, struct proxy *curpx,
                                struct proxy *defpx, const char *file,
                                int line) {
  /* This is an example of a MySQL >=4.0 client Authentication packet kindly
   * provided by Cyril Bonte. const char mysql40_client_auth_pkt[] = {
  ...
  static char mysql40_req[] = {
      "%[var(check.header),hex]" /* 3 bytes for the packet length and 1 byte for
                                    the sequence ID */
  ...
  /* This is an example of a MySQL >=4.1  client Authentication packet provided
   *by Nenad Merdanovic. const char mysql41_client_auth_pkt[] = {
   * 	"\x0e\x00\x00\"		// packet length
   * 	"\x01"			// packet number
  ...
  if (*args[cur_arg]) {
    ...
    if (strcmp(args[cur_arg], "user") != 0) {
      ha_alert("parsing [%s:%d] : '%s %s' only supports optional values: "
               "'user' (got '%s').\n",
    ...
    if (!*args[cur_arg + 2] || strcmp(args[cur_arg + 2], "post-41") == 0) {
      ...
      ha_alert("parsing [%s:%d] : keyword '%s' only supports 'post-41' and "
               "'pre-41' (got '%s').\n",
  ...
  if (mysql_req) {
    chk = parse_tcpcheck_send(
        (char *[]){"tcp-check", "send-binary-lf", mysql_req, ""}, 1, curpx,
        &rs->rules, file, line, &errmsg);
    if (!chk) {
      ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
      goto error;
  ...
  chk = parse_tcpcheck_expect((char *[]){"tcp-check", "expect", "custom", ""},
                              1, curpx, &rs->rules, TCPCHK_RULES_MYSQL_CHK,
                              file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
...
int proxy_parse_ldap_check_opt(char **args, int cur_arg, struct proxy *curpx,
                               struct proxy *defpx, const char *file,
                               int line) {
  ...
  chk =
      parse_tcpcheck_send((char *[]){"tcp-check", "send-binary", ldap_req, ""},
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "rbinary", "^30", "min-recv", "14",
  ...
  chk = parse_tcpcheck_expect((char *[]){"tcp-check", "expect", "custom", ""},
                              1, curpx, &rs->rules, TCPCHK_RULES_LDAP_CHK, file,
                              line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
...
int proxy_parse_spop_check_opt(char **args, int cur_arg, struct proxy *curpx,
                               struct proxy *defpx, const char *file,
                               int line) {
  struct tcpcheck_ruleset *rs = NULL;
  struct tcpcheck_rules *rules = &curpx->tcpcheck_rules;
  ...
  chk = parse_tcpcheck_send(
      (char *[]){"tcp-check", "send-binary", b_head(&trash), ""}, 1, curpx,
      &rs->rules, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  ...
  chk = parse_tcpcheck_expect(
      (char *[]){"tcp-check", "expect", "custom", "min-recv", "4", ""}, 1,
      curpx, &rs->rules, TCPCHK_RULES_SPOP_CHK, file, line, &errmsg);
  if (!chk) {
    ha_alert("parsing [%s:%d] : %s\n", file, line, errmsg);
    goto error;
  }
...
static struct tcpcheck_rule *proxy_parse_httpchk_req(char **args, int cur_arg,
                                                     struct proxy *px,
                                                     char **errmsg) {
  struct tcpcheck_rule *chk = NULL;
  struct tcpcheck_http_hdr *hdr = NULL;
  ...
  if (hdrs || body) {
    memprintf(
        errmsg,
        "hiding headers or body at the end of the version string is deprecated."
  ...
  if (hdrs) {
    ...
    ret = h1_headers_to_hdr_list(b_orig(&trash), b_tail(&trash), tmp_hdrs,
                                 sizeof(tmp_hdrs) / sizeof(tmp_hdrs[0]), &h1m,
                                 NULL);
    if (ret <= 0) {
    ...
    for (i = 0; istlen(tmp_hdrs[i].n); i++) {
      ...
      ist0(tmp_hdrs[i].v);
      if (!parse_logformat_string(istptr(tmp_hdrs[i].v), px, &hdr->value, 0,
                                  SMP_VAL_BE_CHK_RUL, errmsg))
        goto error;
      LIST_ADDQ(&chk->send.http.hdrs, &hdr->list);
    }
...
int proxy_parse_httpchk_opt(char **args, int cur_arg, struct proxy *curpx,
                            struct proxy *defpx, const char *file, int line) {
  struct tcpcheck_ruleset *rs = NULL;
  struct tcpcheck_rules *rules = &curpx->tcpcheck_rules;
  struct tcpcheck_rule *chk;
...
int proxy_parse_tcp_check_opt(char **args, int cur_arg, struct proxy *curpx,
                              struct proxy *defpx, const char *file, int line) {
  struct tcpcheck_ruleset *rs = NULL;
  struct tcpcheck_rules *rules = &curpx->tcpcheck_rules;
  int err_code = 0;
  ...
  if ((rules->flags & TCPCHK_RULES_PROTO_CHK) == TCPCHK_RULES_TCP_CHK) {
    ...
    chunk_printf(&trash, "*tcp-check-defaults_%s-%d", defpx->conf.file,
                 defpx->conf.line);
    rs = find_tcpcheck_ruleset(b_orig(&trash));
    if (rs)
      goto ruleset_found;
