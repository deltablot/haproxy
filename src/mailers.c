...
DECLARE_STATIC_POOL(pool_head_email_alert, "email_alert",
                    sizeof(struct email_alert));
...
static struct task *process_email_alert(struct task *t, void *context,
...
static int enqueue_one_email_alert(struct proxy *p, struct server *s,
...

  {
    ...
    if (strftime(datestr, sizeof(datestr), "%a, %d %b %Y %T %z (%Z)", &tm) ==
        0) {
      goto error;
    }
...
static void enqueue_email_alert(struct proxy *p, struct server *s,
                                const char *msg) {
  int i;
  struct mailer *mailer;
