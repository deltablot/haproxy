...
static const char *get_addr(struct in6_addr *addr) {
  static char out[50];
  snprintf(out, 50, "%08x:%08x:%08x:%08x", addr->s6_addr32[0],
           addr->s6_addr32[1], addr->s6_addr32[2], addr->s6_addr32[3]);
  return out;
}
...
static inline int a_le_b(struct in6_addr *a, struct in6_addr *b) {
  ...
  if (a->s6_addr32[0] > b->s6_addr32[0])
    return 0;
  if (a->s6_addr32[1] < b->s6_addr32[1])
    return 1;
  if (a->s6_addr32[1] > b->s6_addr32[1])
    return 0;
  if (a->s6_addr32[2] < b->s6_addr32[2])
    return 1;
  if (a->s6_addr32[2] > b->s6_addr32[2])
    return 0;
  if (a->s6_addr32[3] < b->s6_addr32[3])
    return 1;
  if (a->s6_addr32[3] > b->s6_addr32[3])
    return 0;
  return 1;
...
/* a + b -> r */
static inline struct in6_addr *a_plus_b(struct in6_addr *a, struct in6_addr *b,
                                        struct in6_addr *r) {
...
/* a - b -> r */
static inline struct in6_addr *a_minus_b(struct in6_addr *a, struct in6_addr *b,
                                         struct in6_addr *r) {
...
/* a & b -> r */
static inline struct in6_addr *a_and_b(struct in6_addr *a, struct in6_addr *b,
                                       struct in6_addr *r) {
 ...
 */
static void convert_range(struct in6_addr *low, struct in6_addr *high,
                          const char *eol, const char *pfx) {
  int bit;
  struct in6_addr r0;
  struct in6_addr r1;
  ...
  while (bit >= 0 && is_set(a_plus_b(a_minus_b(high, low, &r0), &one, &r0))) {
    ...
    /* shrink mask */
    if (is_set(a_and_b(a_plus_b(a_minus_b(high, low, &r0), &one, &r0),
                       one_ls_b(bit, &r1), &r1))) {
...
static void usage(const char *argv0) {
  fprintf(stderr,
          ...
          "\n"
          "This program reads lines starting by two IP addresses and outputs "
          "them with\n"
          "the two IP addresses replaced by a netmask covering the range "
          "between these\n"
          "IPs (inclusive). When multiple ranges are needed, as many lines are "
          "emitted.\n"
          "The IP addresses may be delimited by spaces, tabs or commas. Quotes "
          "are\n"
          "stripped, and lines beginning with a sharp character ('#') are "
          "ignored. The\n"
          "IP addresses may be either in the dotted format or represented as a "
          "32-bit\n"
          "integer value in network byte order.\n"
          "\n"
          "For each optional <addr> specified, only the network it belongs to "
          "is returned,\n"
  ...
  while (fgets(line, sizeof(line), stdin) != NULL) {
    ...
    *(he++) = 0;
    ...
    if (inet_pton(AF_INET6, lb, &sa) <= 0) {
      fprintf(stderr,
              "Failed to parse source address <%s> at line %d, skipping line\n",
              lb, lnum);
      continue;
    }
    ...
    if (inet_pton(AF_INET6, hb, &da) <= 0) {
      fprintf(stderr,
              "Failed to parse destination address <%s> at line %d, skipping "
              "line\n",
              hb, lnum);
      continue;
    }
    ...
    if (argc > 1) {
      for (l = 1; l < argc; l++) {
        if (inet_pton(AF_INET6, argv[l], &da) <= 0)
          continue;
        in6_bswap(&ta);
        if ((a_le_b(&sa, &ta) && a_le_b(&ta, &da)) ||
            (a_le_b(&da, &ta) && a_le_b(&ta, &sa)))
