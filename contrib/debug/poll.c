...
void do_acc(int fd) {
  ...
  if (sfd < 0)
    sfd = ret;
  if (verbose)
    printf("cmd #%d stp #%d: %s(%d): ret=%d%s\n", cmd, cmdstep, __FUNCTION__,
           fd, ret, get_errno(ret));
}
...
void do_pol(int fd) {
  struct pollfd fds = {
      .fd = fd, .events = POLLIN | POLLOUT | POLLRDHUP, .revents = 0};
  int ret;

  ret = poll(&fds, 1, 0);
  if (verbose) {
    printf("cmd #%d stp #%d: %s(%d): ret=%d%s ev=%#x ", cmd, cmdstep,
           __FUNCTION__, fd, ret, get_errno(ret), ret > 0 ? fds.revents : 0);
