...
#define PROMEX_FL_SCOPE_ALL                                                    \
  (PROMEX_FL_SCOPE_GLOBAL | PROMEX_FL_SCOPE_FRONT | PROMEX_FL_SCOPE_BACK |     \
...
/* Matrix used to dump frontend metrics. Each metric points to the next one to
 * be processed or 0 to stop the dump. */
...
const struct ist promex_inf_metric_desc[INF_TOTAL_FIELDS] = {
    ...
    [INF_POOL_ALLOC_MB] =
        IST("Total amount of memory allocated in pools (in bytes)."),
    [INF_POOL_USED_MB] =
        IST("Total amount of memory used in pools (in bytes)."),
    ...
    [INF_HARD_MAXCONN] =
        IST("Initial Maximum number of concurrent connections."),
    ...
    [INF_MAX_SSL_CONNS] =
        IST("Configured maximum number of concurrent SSL connections."),
    ...
    [INF_CONN_RATE_LIMIT] =
        IST("Configured maximum number of connections per second."),
    [INF_MAX_CONN_RATE] =
        IST("Maximum observed number of connections per second."),
    [INF_SESS_RATE] =
        IST("Current number of sessions per second over last elapsed second."),
    [INF_SESS_RATE_LIMIT] =
        IST("Configured maximum number of sessions per second."),
    [INF_MAX_SESS_RATE] =
        IST("Maximum observed number of sessions per second."),
    [INF_SSL_RATE] = IST(
        "Current number of SSL sessions per second over last elapsed second."),
    [INF_SSL_RATE_LIMIT] =
        IST("Configured maximum number of SSL sessions per second."),
    [INF_MAX_SSL_RATE] =
        IST("Maximum observed number of SSL sessions per second."),
    [INF_SSL_FRONTEND_KEY_RATE] = IST("Current frontend SSL Key computation "
                                      "per second over last elapsed second."),
    [INF_SSL_FRONTEND_MAX_KEY_RATE] =
        IST("Maximum observed frontend SSL Key computation per second."),
    [INF_SSL_FRONTEND_SESSION_REUSE_PCT] =
        IST("SSL session reuse ratio (percent)."),
    [INF_SSL_BACKEND_KEY_RATE] = IST("Current backend SSL Key computation per "
                                     "second over last elapsed second."),
    [INF_SSL_BACKEND_MAX_KEY_RATE] =
        IST("Maximum observed backend SSL Key computation per second."),
    ...
    [INF_COMPRESS_BPS_IN] = IST("Number of bytes per second over last elapsed "
                                "second, before http compression."),
    [INF_COMPRESS_BPS_OUT] = IST("Number of bytes per second over last elapsed "
                                 "second, after http compression."),
    [INF_COMPRESS_BPS_RATE_LIM] =
        IST("Configured maximum input compression rate in bytes."),
    [INF_ZLIB_MEM_USAGE] = IST("Current memory used for zlib in bytes."),
    [INF_MAX_ZLIB_MEM_USAGE] =
        IST("Configured maximum amount of memory for zlib in bytes."),
    ...
    [INF_UNSTOPPABLE_JOBS] = IST("Current number of active jobs that can't be "
                                 "stopped during a soft stop."),
    ...
    [INF_TOTAL_SPLICED_BYTES_OUT] =
        IST("Total number of bytes emitted through a kernel pipe."),
    [INF_BYTES_OUT_RATE] =
        IST("Number of bytes emitted over the last elapsed second."),
    [INF_DEBUG_COMMANDS_ISSUED] = IST("Number of debug commands issued on this "
                                      "process (anything > 0 is unsafe)."),
};

/* Description of all stats fields */
const struct ist promex_st_metric_desc[ST_F_TOTAL_FIELDS] = {
    [ST_F_PXNAME] = IST("The proxy name."),
    [ST_F_SVNAME] = IST("The service name (FRONTEND for frontend, BACKEND for "
                        "backend, any name for server/listener)."),
    ...
    [ST_F_STATUS] =
        IST("Current status of the service (frontend: 0=STOP, 1=UP - backend: "
            "0=DOWN, 1=UP - server: 0=DOWN, 1=UP, 2=MAINT, 3=DRAIN, 4=NOLB)."),
    [ST_F_WEIGHT] = IST("Service weight."),
    [ST_F_ACT] = IST("Current number of active servers."),
    [ST_F_BCK] = IST("Current number of backup servers."),
    [ST_F_CHKFAIL] = IST("Total number of failed check (Only counts checks "
                         "failed when the server is up)."),
    [ST_F_CHKDOWN] = IST("Total number of UP->DOWN transitions."),
    [ST_F_LASTCHG] =
        IST("Number of seconds since the last UP<->DOWN transition."),
    [ST_F_DOWNTIME] = IST("Total downtime (in seconds) for the service."),
    [ST_F_QLIMIT] =
        IST("Configured maxqueue for the server (0 meaning no limit)."),
    ...
    [ST_F_THROTTLE] =
        IST("Current throttle percentage for the server, when slowstart is "
            "active, or no value if not in slowstart."),
    [ST_F_LBTOT] = IST("Total number of times a service was selected, either "
                       "for new sessions, or when redispatching."),
    [ST_F_TRACKED] = IST("Id of proxy/server if tracking is enabled."),
    [ST_F_TYPE] = IST(
        "Service type (0=frontend, 1=backend, 2=server, 3=socket/listener)."),
    [ST_F_RATE] =
        IST("Current number of sessions per second over last elapsed second."),
    ...
    [ST_F_CHECK_STATUS] =
        IST("Status of last health check (HCHK_STATUS_* values)."),
    [ST_F_CHECK_CODE] =
        IST("layer5-7 code, if available of the last health check."),
    [ST_F_CHECK_DURATION] =
        IST("Total duration of the latest server health check, in seconds."),
    ...
    [ST_F_REQ_RATE] = IST(
        "Current number of HTTP requests per second over last elapsed second."),
    [ST_F_REQ_RATE_MAX] =
        IST("Maximum observed number of HTTP requests per second."),
    [ST_F_REQ_TOT] = IST("Total number of HTTP requests received."),
    [ST_F_CLI_ABRT] =
        IST("Total number of data transfers aborted by the client."),
    [ST_F_SRV_ABRT] =
        IST("Total number of data transfers aborted by the server."),
    [ST_F_COMP_IN] =
        IST("Total number of HTTP response bytes fed to the compressor."),
    [ST_F_COMP_OUT] =
        IST("Total number of HTTP response bytes emitted by the compressor."),
    [ST_F_COMP_BYP] = IST("Total number of bytes that bypassed the HTTP "
                          "compressor (CPU/BW limit)."),
    [ST_F_COMP_RSP] =
        IST("Total number of HTTP responses that were compressed."),
    [ST_F_LASTSESS] =
        IST("Number of seconds since last session assigned to server/backend."),
    ...
    [ST_F_CTIME] =
        IST("Avg. connect time for last 1024 successful connections."),
    [ST_F_RTIME] =
        IST("Avg. response time for last 1024 successful connections."),
    [ST_F_TTIME] = IST("Avg. total time for last 1024 successful connections."),
    [ST_F_AGENT_STATUS] = IST("Status of last agent check."),
    [ST_F_AGENT_CODE] =
        IST("Numeric code reported by agent if any (unused for now)."),
    [ST_F_AGENT_DURATION] = IST("Time in ms taken to finish last agent check."),
    [ST_F_CHECK_DESC] =
        IST("Short human-readable description of the last health status."),
    [ST_F_AGENT_DESC] =
        IST("Short human-readable description of the last agent status."),
    [ST_F_CHECK_RISE] =
        IST("Server's \"rise\" parameter used by health checks"),
    [ST_F_CHECK_FALL] =
        IST("Server's \"fall\" parameter used by health checks"),
    [ST_F_CHECK_HEALTH] =
        IST("Server's health check value between 0 and rise+fall-1"),
    [ST_F_AGENT_RISE] = IST("Agent's \"rise\" parameter, normally 1."),
    [ST_F_AGENT_FALL] = IST("Agent's \"fall\" parameter, normally 1."),
    [ST_F_AGENT_HEALTH] =
        IST("Agent's health parameter, between 0 and rise+fall-1"),
    [ST_F_ADDR] =
        IST("address:port or \"unix\". IPv6 has brackets around the address."),
    ...
    [ST_F_CONN_RATE] = IST("Current number of connections per second over the "
                           "last elapsed second."),
    [ST_F_CONN_RATE_MAX] =
        IST("Maximum observed number of connections per second."),
    ...
    [ST_F_SRV_ICUR] =
        IST("Current number of idle connections available for reuse"),
    ...
    [ST_F_CT_MAX] =
        IST("Maximum observed time spent waiting for a connection to complete"),
    [ST_F_RT_MAX] =
        IST("Maximum observed time spent waiting for a server response"),
    [ST_F_TT_MAX] = IST("Maximum observed total request+response time "
                        "(request+queue+connect+response+processing)"),
 ...
 */
static int promex_metric_to_str(struct buffer *out, struct field *f,
                                size_t max) {
  int ret = 0;
  ...
  case FF_EMPTY:
    ...
    break;
  case FF_S32:
    ret = chunk_appendf(out, "%d\n", f->u.s32);
    break;
  case FF_U32:
    ret = chunk_appendf(out, "%u\n", f->u.u32);
    break;
  case FF_S64:
    ret = chunk_appendf(out, "%lld\n", (long long)f->u.s64);
    break;
  case FF_U64:
    ret = chunk_appendf(out, "%llu\n", (unsigned long long)f->u.u64);
    break;
  case FF_FLT:
    ret = chunk_appendf(out, "%f\n", f->u.flt);
    break;
  case FF_STR:
    ret = chunk_strcat(out, "Nan\n");
    break;
 ...
 * <name>
 */
static void promex_metric_name(struct appctx *appctx, struct ist *name,
                               const struct ist prefix) {
...
static int promex_dump_metric_header(struct appctx *appctx, struct htx *htx,
                                     const struct ist name, struct ist *out,
                                     size_t max) {
  ...
  if (istcat(out, ist("# HELP "), max) == -1 || istcat(out, name, max) == -1 ||
      istcat(out, ist(" "), max) == -1 ||
      istcat(out, desc[appctx->st2], max) == -1 ||
      istcat(out, ist("\n# TYPE "), max) == -1 ||
      istcat(out, name, max) == -1 || istcat(out, ist(" "), max) == -1 ||
...
/* Dump global metrics (prefixed by "haproxy_process_"). It returns 1 on
...
/* Dump frontends metrics (prefixed by "haproxy_frontend_"). It returns 1 on
 * success, 0 if <htx> is full and -1 in case of any error. */
...
/* Dump backends metrics (prefixed by "haproxy_backend_"). It returns 1 on
 * success, 0 if <htx> is full and -1 in case of any error. */
static int promex_dump_back_metrics(struct appctx *appctx, struct htx *htx) {
  ...
  while (appctx->st2 && appctx->st2 < ST_F_TOTAL_FIELDS) {
    while (appctx->ctx.stats.obj1) {
      ...
      case ST_F_STATUS:
        metric =
            mkf_u32(FO_STATUS, (px->lbprm.tot_weight > 0 || !px->srv) ? 1 : 0);
        break;
      case ST_F_SCUR:
      ...
      case ST_F_BOUT:
        ...
        break;
      case ST_F_QTIME:
        secs = (double)swrate_avg(px->be_counters.q_time, TIME_STATS_SAMPLES) /
               1000.0;
        metric = mkf_flt(FN_AVG, secs);
        break;
      case ST_F_CTIME:
        secs = (double)swrate_avg(px->be_counters.c_time, TIME_STATS_SAMPLES) /
               1000.0;
        metric = mkf_flt(FN_AVG, secs);
        break;
      case ST_F_RTIME:
        secs = (double)swrate_avg(px->be_counters.d_time, TIME_STATS_SAMPLES) /
               1000.0;
        metric = mkf_flt(FN_AVG, secs);
        break;
      case ST_F_TTIME:
        secs = (double)swrate_avg(px->be_counters.t_time, TIME_STATS_SAMPLES) /
               1000.0;
        metric = mkf_flt(FN_AVG, secs);
        break;
      case ST_F_QT_MAX:
      ...
      case ST_F_SRV_ABRT:
        ...
        break;
      case ST_F_WEIGHT:
        weight = (px->lbprm.tot_weight * px->lbprm.wmult + px->lbprm.wdiv - 1) /
                 px->lbprm.wdiv;
...
/* Dump servers metrics (prefixed by "haproxy_server_"). It returns 1 on
 * success, 0 if <htx> is full and -1 in case of any error. */
  ...
  while (appctx->st2 && appctx->st2 < ST_F_TOTAL_FIELDS) {
    while (appctx->ctx.stats.obj1) {
      ...
      while (appctx->ctx.stats.obj2) {
        ...
        if ((appctx->ctx.stats.flags & PROMEX_FL_NO_MAINT_SRV) &&
        ...
        case ST_F_BOUT:
          ...
          break;
        case ST_F_QTIME:
          secs = (double)swrate_avg(sv->counters.q_time, TIME_STATS_SAMPLES) /
                 1000.0;
          metric = mkf_flt(FN_AVG, secs);
          break;
        case ST_F_CTIME:
          secs = (double)swrate_avg(sv->counters.c_time, TIME_STATS_SAMPLES) /
                 1000.0;
          metric = mkf_flt(FN_AVG, secs);
          break;
        case ST_F_RTIME:
          secs = (double)swrate_avg(sv->counters.d_time, TIME_STATS_SAMPLES) /
                 1000.0;
          metric = mkf_flt(FN_AVG, secs);
          break;
        case ST_F_TTIME:
          secs = (double)swrate_avg(sv->counters.t_time, TIME_STATS_SAMPLES) /
                 1000.0;
          metric = mkf_flt(FN_AVG, secs);
          break;
        case ST_F_QT_MAX:
        ...
        case ST_F_SRV_ABRT:
          ...
          break;
        case ST_F_WEIGHT:
          weight = (sv->cur_eweight * px->lbprm.wmult + px->lbprm.wdiv - 1) /
                   px->lbprm.wdiv;
          metric = mkf_u32(FN_AVG, weight);
          break;
        case ST_F_CHECK_STATUS:
          if ((sv->check.state & (CHK_ST_ENABLED | CHK_ST_PAUSED)) !=
              CHK_ST_ENABLED)
            goto next_sv;
        ...
        case ST_F_CHECK_CODE:
          if ((sv->check.state & (CHK_ST_ENABLED | CHK_ST_PAUSED)) !=
              CHK_ST_ENABLED)
            goto next_sv;
          metric = mkf_u32(FN_OUTPUT, (sv->check.status < HCHK_STATUS_L57DATA)
                                          ? 0
                                          : sv->check.code);
          break;
        ...
        case ST_F_SRV_ILIM:
          metric = mkf_u32(FO_CONFIG | FN_LIMIT,
                           (sv->max_idle_conns == -1) ? 0 : sv->max_idle_conns);
          break;
    ...
    next_px:
      ...
      appctx->ctx.stats.obj2 =
          (appctx->ctx.stats.obj1
               ? ((struct proxy *)appctx->ctx.stats.obj1)->srv
               : NULL);
    }
  next_metric:
    appctx->ctx.stats.flags |= PROMEX_FL_METRIC_HDR;
    appctx->ctx.stats.obj1 = proxies_list;
    appctx->ctx.stats.obj2 =
        (appctx->ctx.stats.obj1 ? ((struct proxy *)appctx->ctx.stats.obj1)->srv
                                : NULL);
...
static int promex_dump_metrics(struct appctx *appctx,
                               struct stream_interface *si, struct htx *htx) {
  int ret;
 ...
 * full. */
static int promex_send_headers(struct appctx *appctx,
                               struct stream_interface *si, struct htx *htx) {
  struct channel *chn = si_ic(appctx->owner);
  struct htx_sl *sl;
  unsigned int flags;
  ...
  if (!htx_add_header(htx, ist("Cache-Control"), ist("no-cache")) ||
      ...
      !htx_add_header(htx, ist("Content-Type"),
                      ist("text/plain; version=0.0.4")) ||
 ...
 * the applet.
 */
static int promex_appctx_init(struct appctx *appctx, struct proxy *px,
                              struct stream *strm) {
...
static void promex_appctx_handle_io(struct appctx *appctx) {
  ...
  case PROMEX_ST_HEAD:
    if (!promex_send_headers(appctx, si, res_htx))
      goto out;
    appctx->st0 =
        ((s->txn->meth == HTTP_METH_HEAD) ? PROMEX_ST_DONE : PROMEX_ST_DUMP);
    /* fall through */
...
service_parse_prometheus_exporter(const char **args, int *cur_arg,
...
static struct action_kw_list service_actions = {
    ILH,
    {{"prometheus-exporter", service_parse_prometheus_exporter}, {/* END */}}};
