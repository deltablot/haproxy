...
static int do_read(int sock, void *buf, int read_len) {
  fd_set readfds;
...
static int do_write(int sock, void *buf, int write_len) {
  fd_set writefds;
...
static int read_frame(int sock, struct worker *w) {
  uint32_t netint;
...
static int write_frame(int sock, struct worker *w) {
  uint32_t netint;
 ...
 * number of written bytes. */
static int encode_spoe_varint(uint64_t i, char *buf) {
  int idx;
 ...
 * bytes is returned. */
static int decode_spoe_varint(char *buf, char *end, uint64_t *i) {
 ...
 * written bytes. */
static int encode_spoe_string(const char *str, size_t len, char *dst) {
  int idx = 0;
 ...
 * NULL. */
static int decode_spoe_string(char *buf, char *end, char **str, uint64_t *len) {
  int r, idx = 0;
...
static int skip_spoe_data(char *frame, char *end) {
  uint64_t sz = 0;
  int r, idx = 0;
...
static int decode_spoe_data(char *frame, char *end, struct spoe_data *data) {
  uint64_t sz = 0;
  int type, r, idx = 0;
...
static int check_proto_version(struct worker *w, int idx) {
  char *str;
  uint64_t sz;
 ...
 * read bytes otherwise. */
static int check_max_frame_size(struct worker *w, int idx) {
  uint64_t sz;
  int type, i;
 ...
 * read bytes otherwise. */
static int check_healthcheck(struct worker *w, int idx) {
  int type;
 ...
 * bytes. */
static int handle_hahello(struct worker *w) {
 ...
 * occurred, 0 if the frame must be skipped, otherwise the number of read
 * bytes. */
static int handle_hadiscon(struct worker *w) {
  char *end = w->buf + w->len;
  int i, idx = 0;
...
static inline int set_var_name(struct worker *w, const char *name, int name_len,
                               unsigned char scope) {
  w->ack[w->ack_len++] = SPOE_ACT_T_SET_VAR; /* Action type */
  w->ack[w->ack_len++] = 3;                  /* Number of args */
  w->ack[w->ack_len++] = scope;              /* Arg 1: the scope */
  w->ack_len += encode_spoe_string(
      name, name_len, w->ack + w->ack_len); /* Arg 2: variable name */
  return 1;
}

int set_var_null(struct worker *w, const char *name, int name_len,
                 unsigned char scope) {
...
int set_var_bool(struct worker *w, const char *name, int name_len,
...
static inline int set_var_int(struct worker *w, const char *name, int name_len,
                              unsigned char scope, int type, uint64_t value) {
  if (!set_var_name(w, name, name_len, scope))
    return 0;
  w->ack[w->ack_len++] = SPOE_DATA_T_UINT32;
  w->ack_len += encode_spoe_varint(
      value, w->ack + w->ack_len); /* Arg 3: variable value */
  return 1;
}

int set_var_uint32(struct worker *w, const char *name, int name_len,
                   unsigned char scope, uint32_t value) {
  return set_var_int(w, name, name_len, scope, SPOE_DATA_T_UINT32, value);
}

int set_var_int32(struct worker *w, const char *name, int name_len,
                  unsigned char scope, int32_t value) {
  return set_var_int(w, name, name_len, scope, SPOE_DATA_T_INT32, value);
}

int set_var_uint64(struct worker *w, const char *name, int name_len,
                   unsigned char scope, uint64_t value) {
  return set_var_int(w, name, name_len, scope, SPOE_DATA_T_INT32, value);
}

int set_var_int64(struct worker *w, const char *name, int name_len,
                  unsigned char scope, int64_t value) {
  return set_var_int(w, name, name_len, scope, SPOE_DATA_T_INT32, value);
}

int set_var_ipv4(struct worker *w, const char *name, int name_len,
                 unsigned char scope, struct in_addr *ipv4) {
  if (!set_var_name(w, name, name_len, scope))
    return 0;
...
int set_var_ipv6(struct worker *w, const char *name, int name_len,
                 unsigned char scope, struct in6_addr *ipv6) {
  if (!set_var_name(w, name, name_len, scope))
    return 0;
...
static inline int set_var_buf(struct worker *w, const char *name, int name_len,
                              unsigned char scope, int type, const char *str,
                              int str_len) {
  if (!set_var_name(w, name, name_len, scope))
    return 0;
...
int set_var_string(struct worker *w, const char *name, int name_len,
                   unsigned char scope, const char *str, int strlen) {
  return set_var_buf(w, name, name_len, scope, SPOE_DATA_T_STR, str, strlen);
}

int set_var_bin(struct worker *w, const char *name, int name_len,
                unsigned char scope, const char *str, int strlen) {
 ...
 * occurred, 0 if the frame must be skipped, otherwise the number of read
 * bytes. */
static int handle_hanotify(struct worker *w) {
  char *end = w->buf + w->len;
  ...
  DEBUG("Notify frame received: stream-id=%u - frame-id=%u", w->stream_id,
        w->frame_id);
  ...
  while (idx < w->len) {
    ...
    for (index = 0; index < nbargs; index++) {
      ...
      /* Read the key name */
      if ((i = decode_spoe_string(w->buf + idx, end, &args[index].name.str,
                                  &length)) == -1) {
        w->status_code = SPOE_FRM_ERR_INVALID;
        goto error;
      }
...
static int prepare_agenthello(struct worker *w) {
  int idx = 0;
  unsigned int flags = 0;
  ...
  w->buf[idx++] = SPOE_DATA_T_STR;
  idx += encode_spoe_string(SPOA_CAPABILITIES, SLEN(SPOA_CAPABILITIES),
                            w->buf + idx);
 ...
 * occurred, the number of written bytes otherwise. */
static int prepare_agentdicon(struct worker *w) {
  const char *reason;
  int rlen, idx = 0;
  unsigned int flags = 0;
...
static int hello_handshake(int sock, struct worker *w) {
...
static int notify_ack_roundtip(int sock, struct worker *w) {
  if (read_frame(sock, w) < 0) {
  ...
  DEBUG("Ack frame sent: stream-id=%u - frame-id=%u", w->stream_id,
        w->frame_id);
  return 0;
error_or_quit:
  return -1;
...
static void *spoa_worker(void *data) {
  struct worker w;
...
static void usage(char *prog) {
  fprintf(stderr,
          "Usage: %s [-h] [-d] [-p <port>] [-n <num-workers>] -f <file>\n",
          prog);
  fprintf(stderr, "    -h                  Print this message\n");
  fprintf(stderr, "    -d                  Enable the debug mode\n");
  fprintf(stderr, "    -p <port>           Specify the port to listen on "
                  "(default: 12345)\n");
  fprintf(
      stderr,
      "    -n <num-workers>    Specify the number of workers (default: 5)\n");
  fprintf(stderr, "    -f <file>           Specify the file whoch contains the "
                  "processing code.\n");
  fprintf(
      stderr,
