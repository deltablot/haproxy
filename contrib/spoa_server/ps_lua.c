...
static int ps_lua_exec_message(struct worker *w, void *ref, int nargs,
                               struct spoe_kv *args);
...
static int ps_lua_register_message(lua_State *L) {
  ...
  if (!lua_isfunction(L, 2)) {
    const char *msg =
        lua_pushfstring(L, "function expected, got %s", luaL_typename(L, 2));
...
__attribute__((constructor)) static void __ps_lua_init(void) {
