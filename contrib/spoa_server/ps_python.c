...
static int ps_python_exec_message(struct worker *w, void *ref, int nargs,
                                  struct spoe_kv *args);
...
static int ps_python_check_overflow(Py_ssize_t len) {
  ...
  if (len >= (Py_ssize_t)INT_MAX) {
    PyErr_Format(spoa_error, "%zd is over 2GB. Please split in smaller pieces.",
                 len);
    return -1;
  } else {
...
static PyObject *PyInit_spoa_module(void) { return module_spoa; }
#endif /* IS_PYTHON_3K */
...
static int ps_python_exec_message(struct worker *w, void *ref, int nargs,
  ...
  for (i = 0; i < nargs; i++) {
    ...
    case SPOE_DATA_T_STR:
      value = PY_STRING_FROM_STRING_AND_SIZE(args[i].value.u.buffer.str,
                                             args[i].value.u.buffer.len);
      break;
    case SPOE_DATA_T_BIN:
      value = PY_BYTES_FROM_STRING_AND_SIZE(args[i].value.u.buffer.str,
                                            args[i].value.u.buffer.len);
      break;
    default:
      Py_INCREF(Py_None);
...
__attribute__((constructor)) static void __ps_python_init(void) {
