...
#define FLT_OT_EVENT_DEF(a, b, c, d, e, f)                                     \
  {AN_##b##_##a, SMP_OPT_DIR_##b, SMP_VAL_FE_##c, SMP_VAL_BE_##d, e, f},
const struct flt_ot_event_data flt_ot_event_data[FLT_OT_EVENT_MAX] = {
 ...
 */
static int flt_ot_scope_run_span(struct stream *s, struct filter *f,
                                 struct channel *chn, uint dir,
                                 struct flt_ot_scope_span *span,
                                 struct flt_ot_scope_data *data,
                                 const struct flt_ot_conf_span *conf_span,
                                 const struct timespec *ts, char **err) {
  struct flt_ot_conf *conf = FLT_OT_CONF(f);
  int retval = FLT_OT_RET_OK;

  FLT_OT_FUNC("%p, %p, %p, %u, %p, %p, %p, %p, %p:%p", s, f, chn, dir, span,
              data, conf_span, ts, FLT_OT_DPTR_ARGS(err));
  ...
  if (span->span == NULL) {
    span->span =
        ot_span_init(conf->tracer->tracer, span->id, ts, NULL, span->ref_type,
                     FLT_OT_DEREF(span->ref_ctx, idx, -1), span->ref_span,
                     data->tags, data->num_tags, err);
    if (span->span == NULL)
  ...
  if ((span->span != NULL) && (conf_span->ctx_id != NULL)) {
    ...
    span_ctx =
        ot_inject_http_headers(conf->tracer->tracer, span->span, &writer, err);
    if (span_ctx != NULL) {
      int i = 0;

      if (conf_span->ctx_flags &
          (FLT_OT_CTX_USE_VARS | FLT_OT_CTX_USE_HEADERS)) {
        for (text_map = &(writer.text_map); i < text_map->count; i++) {
          if (!(conf_span->ctx_flags & FLT_OT_CTX_USE_VARS))
            /* Do nothing. */;
          else if (flt_ot_var_register(FLT_OT_VARS_SCOPE, conf_span->ctx_id,
                                       text_map->key[i], err) == -1)
            retval = FLT_OT_RET_ERROR;
          else if (flt_ot_var_set(s, FLT_OT_VARS_SCOPE, conf_span->ctx_id,
                                  text_map->key[i], text_map->value[i], dir,
                                  err) == -1)
            retval = FLT_OT_RET_ERROR;

          if (!(conf_span->ctx_flags & FLT_OT_CTX_USE_HEADERS))
            /* Do nothing. */;
          else if (flt_ot_http_header_set(chn, conf_span->ctx_id,
                                          text_map->key[i], text_map->value[i],
                                          err) == -1)
            retval = FLT_OT_RET_ERROR;
        }
      }
      ...
      otc_text_map_destroy(&text_map,
                           OTC_TEXT_MAP_FREE_KEY | OTC_TEXT_MAP_FREE_VALUE);
    }
  }
 ...
 *   any other value otherwise.
 */
int flt_ot_scope_run(struct stream *s, struct filter *f, struct channel *chn,
                     struct flt_ot_conf_scope *conf_scope,
                     const struct timespec *ts, uint dir, char **err) {
  struct flt_ot_conf *conf = FLT_OT_CONF(f);
  ...
  FLT_OT_FUNC("%p, %p, %p, %p, %p, %u, %p:%p", s, f, chn, conf_scope, ts, dir,
  ...
  if (conf_scope->cond != NULL) {
    ...
    res =
        acl_exec_cond(conf_scope->cond, s->be, s->sess, s, dir | SMP_OPT_FINAL);
    rc = acl_pass(res);
  ...
  list_for_each_entry(conf_ctx, &(conf_scope->contexts), list) {
    ...
    if (conf_ctx->flags & FLT_OT_CTX_USE_HEADERS)
      text_map =
          flt_ot_http_headers_get(chn, conf_ctx->id, conf_ctx->id_len, err);
    else
      text_map = flt_ot_vars_get(s, FLT_OT_VARS_SCOPE, conf_ctx->id, dir, err);

    if (text_map != NULL) {
      if (flt_ot_scope_context_init(f->ctx, conf->tracer->tracer, conf_ctx->id,
                                    conf_ctx->id_len, text_map, dir,
                                    err) == NULL)
        retval = FLT_OT_RET_ERROR;
  ...
  list_for_each_entry(conf_span, &(conf_scope->spans), list) {
    ...
    span = flt_ot_scope_span_init(f->ctx, conf_span->id, conf_span->id_len,
                                  conf_span->ref_type, conf_span->ref_id,
                                  conf_span->ref_id_len, dir, err);
    if (span == NULL)
      retval = FLT_OT_RET_ERROR;
    ...
    list_for_each_entry(sample, &(conf_span->tags), list) {
      ...
      if (flt_ot_sample_add(s, dir, sample, &data, FLT_OT_EVENT_SAMPLE_TAG,
                            err) == FLT_OT_RET_ERROR)
        retval = FLT_OT_RET_ERROR;
    }
    ...
    list_for_each_entry(sample, &(conf_span->logs), list) {
      ...
      if (flt_ot_sample_add(s, dir, sample, &data, FLT_OT_EVENT_SAMPLE_LOG,
                            err) == FLT_OT_RET_ERROR)
        retval = FLT_OT_RET_ERROR;
    }
    ...
    list_for_each_entry(sample, &(conf_span->baggages), list) {
      ...
      if (flt_ot_sample_add(s, dir, sample, &data, FLT_OT_EVENT_SAMPLE_BAGGAGE,
                            err) == FLT_OT_RET_ERROR)
        retval = FLT_OT_RET_ERROR;
    }

    if (retval != FLT_OT_RET_ERROR)
      if (flt_ot_scope_run_span(s, f, chn, dir, span, &data, conf_span, ts,
                                err) == FLT_OT_RET_ERROR)
        retval = FLT_OT_RET_ERROR;
  ...
  list_for_each_entry(finish, &(conf_scope->finish),
                      list) if (flt_ot_scope_finish_mark(f->ctx, finish->str,
                                                         finish->str_len) == -1)
      retval = FLT_OT_RET_ERROR;
 ...
 *   any other value otherwise.
 */
int flt_ot_event_run(struct stream *s, struct filter *f, struct channel *chn,
                     int event, char **err) {
  struct flt_ot_conf *conf = FLT_OT_CONF(f);
  struct flt_ot_conf_scope *conf_scope;
  struct timespec ts;
...
#ifdef DEBUG_OT
  _HA_ATOMIC_ADD(conf->cnt.event[event].htx +
                     (htx_is_empty(htxbuf(&(chn->buf))) ? 1 : 0),
                 1);
#endif
  ...
  list_for_each_entry(conf_scope, &(conf->scopes), list) {
    if (conf_scope->event != event)
      ...
      FLT_OT_DBG(3, "scope '%s' %d not used", conf_scope->id,
                 conf_scope->event);
    else if (flt_ot_scope_run(s, f, chn, conf_scope, &ts,
                              flt_ot_event_data[event].smp_opt_dir,
                              err) == FLT_OT_RET_ERROR)
      retval = FLT_OT_RET_ERROR;
  }
  ...
  FLT_OT_DBG(3, "event = %d, chn = %p, s->req = %p, s->res = %p", event, chn,
             &(s->req), &(s->res));
