...
REGISTER_POOL(&pool_head_ot_scope_span, "ot_scope_span",
              sizeof(struct flt_ot_scope_span));
#endif
#ifdef USE_POOL_OT_SCOPE_CONTEXT
REGISTER_POOL(&pool_head_ot_scope_context, "ot_scope_context",
              sizeof(struct flt_ot_scope_context));
#endif
#ifdef USE_POOL_OT_RUNTIME_CONTEXT
REGISTER_POOL(&pool_head_ot_runtime_context, "ot_runtime_context",
              sizeof(struct flt_ot_runtime_context));
#endif
...
void flt_ot_pools_info(void) {
  ...
  FLT_OT_DBG(2, "sizeof_pool(buffer) = %u",
             FLT_OT_DEREF(pool_head_buffer, size, 0));
#endif
#ifdef USE_TRASH_CHUNK
  FLT_OT_DBG(2, "sizeof_pool(trash) = %u",
             FLT_OT_DEREF(pool_head_trash, size, 0));
#endif

#ifdef USE_POOL_OT_SCOPE_SPAN
  FLT_OT_DBG(2, "sizeof_pool(ot_scope_span) = %u",
             pool_head_ot_scope_span->size);
#endif
#ifdef USE_POOL_OT_SCOPE_CONTEXT
  FLT_OT_DBG(2, "sizeof_pool(ot_scope_context) = %u",
             pool_head_ot_scope_context->size);
#endif
#ifdef USE_POOL_OT_RUNTIME_CONTEXT
  FLT_OT_DBG(2, "sizeof_pool(ot_runtime_context) = %u",
             pool_head_ot_runtime_context->size);
#endif
}
...
flt_ot_runtime_context_init(struct stream *s, struct filter *f, char **err) {
  ...
  retptr =
      flt_ot_pool_alloc(pool_head_ot_runtime_context, sizeof(*retptr), 1, err);
  if (retptr == NULL)
  ...
  (void)snprintf(
      retptr->uuid.s, sizeof(retptr->uuid.s),
      "%08x-%04hx-%04hx-%04hx-%012" PRIx64, retptr->uuid.time_low,
      retptr->uuid.time_mid,
      (retptr->uuid.time_hi_and_version & UINT16_C(0xfff)) | UINT16_C(0x4000),
      retptr->uuid.clock_seq | UINT16_C(0x8000), (uint64_t)retptr->uuid.node);

  if (flt_ot_var_register(FTL_OT_VAR_UUID, err) != -1)
    (void)flt_ot_var_set(s, FTL_OT_VAR_UUID, retptr->uuid.s, SMP_OPT_DIR_REQ,
                         err);
...
flt_ot_scope_span_init(struct flt_ot_runtime_context *rt_ctx, const char *id,
                       size_t id_len, otc_span_reference_type_t ref_type,
                       const char *ref_id, size_t ref_id_len, uint dir,
                       char **err) {
  ...
  FLT_OT_FUNC("%p, \"%s\", %zu, %d, \"%s\", %zu, %u, %p:%p", rt_ctx, id, id_len,
              ref_type, ref_id, ref_id_len, dir, FLT_OT_DPTR_ARGS(err));

  if ((rt_ctx == NULL) || (id == NULL))
    FLT_OT_RETURN(retptr);

  list_for_each_entry(span, &(rt_ctx->spans),
                      list) if ((span->id_len == id_len) &&
                                (memcmp(span->id, id, id_len) == 0)) {
  ...
  if (ref_id != NULL) {
    list_for_each_entry(span, &(rt_ctx->spans),
                        list) if ((span->id_len == ref_id_len) &&
                                  (memcmp(span->id, ref_id, ref_id_len) == 0)) {
      ref_span = span->span;
    ...
    if (ref_span != NULL) {
      ...
      list_for_each_entry(
          ctx, &(rt_ctx->contexts),
          list) if ((ctx->id_len == ref_id_len) &&
                    (memcmp(ctx->id, ref_id, ref_id_len) == 0)) {
        ref_ctx = ctx->context;
...
flt_ot_scope_context_init(struct flt_ot_runtime_context *rt_ctx,
                          struct otc_tracer *tracer, const char *id,
                          size_t id_len, const struct otc_text_map *text_map,
                          uint dir, char **err) {
  struct otc_http_headers_reader reader;
  struct otc_span_context *span_ctx;
  struct flt_ot_scope_context *retptr = NULL;

  FLT_OT_FUNC("%p, %p, \"%s\", %zu, %p, %u, %p:%p", rt_ctx, tracer, id, id_len,
              text_map, dir, FLT_OT_DPTR_ARGS(err));

  if ((rt_ctx == NULL) || (tracer == NULL) || (id == NULL) ||
      (text_map == NULL))
    FLT_OT_RETURN(retptr);

  list_for_each_entry(retptr, &(rt_ctx->contexts),
                      list) if ((retptr->id_len == id_len) &&
                                (memcmp(retptr->id, id, id_len) == 0)) {
  ...
  retptr =
      flt_ot_pool_alloc(pool_head_ot_scope_context, sizeof(*retptr), 1, err);
  if (retptr == NULL)
    FLT_OT_RETURN(retptr);
 ...
 *   -
 */
int flt_ot_scope_finish_mark(const struct flt_ot_runtime_context *rt_ctx,
  ...
  if (FLT_OT_STR_CMP(FLT_OT_SCOPE_SPAN_FINISH_ALL, id, id_len)) {
    ...
    list_for_each_entry(span, &(rt_ctx->spans),
                        list) if (span->smp_opt_dir == SMP_OPT_DIR_REQ) {
      span->flag_finish = 1;
      span_cnt++;
    }

    list_for_each_entry(ctx, &(rt_ctx->contexts),
                        list) if (ctx->smp_opt_dir == SMP_OPT_DIR_REQ) {
      ctx->flag_finish = 1;
      span_cnt++;
    }

    FLT_OT_DBG(2, "marked REQuest channel %d span(s), %d context(s)", span_cnt,
               ctx_cnt);
  } else if (FLT_OT_STR_CMP(FLT_OT_SCOPE_SPAN_FINISH_RES, id, id_len)) {
    list_for_each_entry(span, &(rt_ctx->spans),
                        list) if (span->smp_opt_dir == SMP_OPT_DIR_RES) {
      span->flag_finish = 1;
      span_cnt++;
    }

    list_for_each_entry(ctx, &(rt_ctx->contexts),
                        list) if (ctx->smp_opt_dir == SMP_OPT_DIR_RES) {
      ctx->flag_finish = 1;
      ctx_cnt++;
    }

    FLT_OT_DBG(2, "marked RESponse channel %d span(s), %d context(s)", span_cnt,
               ctx_cnt);
  } else {
    list_for_each_entry(span, &(rt_ctx->spans),
                        list) if ((span->id_len == id_len) &&
    ...
    list_for_each_entry(ctx, &(rt_ctx->contexts),
                        list) if ((ctx->id_len == id_len) &&
                                  (memcmp(ctx->id, id, id_len) == 0)) {
      ctx->flag_finish = 1;
      ctx_cnt++;
 ...
 *   This function does not return a value.
 */
void flt_ot_scope_finish_marked(const struct flt_ot_runtime_context *rt_ctx,
                                const struct timespec *ts_finish) {
  struct flt_ot_scope_span *span;
  struct flt_ot_scope_context *ctx;

  FLT_OT_FUNC("%p, %p", rt_ctx, ts_finish);

  list_for_each_entry(span, &(rt_ctx->spans), list) if (span->flag_finish) {
  ...
  list_for_each_entry(ctx, &(rt_ctx->contexts), list) if (ctx->flag_finish) {
 ...
 *   This function does not return a value.
 */
void flt_ot_scope_free_unused(struct flt_ot_runtime_context *rt_ctx,
                              struct channel *chn) {
  ...
  if (!LIST_ISEMPTY(&(rt_ctx->spans))) {
    ...
    list_for_each_entry_safe(span, span_back, &(rt_ctx->spans),
                             list) if (span->span == NULL)
        flt_ot_scope_span_free(&span);
  }
  ...
  if (!LIST_ISEMPTY(&(rt_ctx->contexts))) {
    ...
    list_for_each_entry_safe(ctx, ctx_back, &(rt_ctx->contexts),
                             list) if (ctx->context == NULL) {
      /*
      ...
      (void)flt_ot_vars_unset(rt_ctx->stream, FLT_OT_VARS_SCOPE, ctx->id,
                              ctx->smp_opt_dir, NULL);
