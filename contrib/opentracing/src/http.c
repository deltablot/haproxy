 ...
 *   -
 */
struct otc_text_map *flt_ot_http_headers_get(struct channel *chn,
                                             const char *prefix, size_t len,
                                             char **err) {
  const struct htx *htx;
  size_t prefix_len =
      (!FLT_OT_STR_ISVALID(prefix) || (len == 0)) ? 0 : (len + 1);
  int32_t pos;
  struct otc_text_map *retptr = NULL;

  FLT_OT_FUNC("%p, \"%s\", %zu, %p:%p", chn, prefix, len,
  ...
  for (pos = htx_get_first(htx); pos != -1; pos = htx_get_next(htx, pos)) {
    ...
    if (type == HTX_BLK_HDR) {
      ...
      if ((prefix_len == 0) ||
          ((n.len >= prefix_len) && (strncasecmp(n.ptr, prefix, len) == 0))) {
        if (retptr == NULL) {
         ...
         */
        if (otc_text_map_add(
                retptr, n.ptr + prefix_len, n.len - prefix_len, v.ptr, v.len,
                OTC_TEXT_MAP_DUP_KEY | OTC_TEXT_MAP_DUP_VALUE) == -1) {
 ...
 *   -
 */
int flt_ot_http_header_set(struct channel *chn, const char *prefix,
                           const char *name, const char *value, char **err) {
  ...
  FLT_OT_FUNC("%p, \"%s\", \"%s\", \"%s\", %p:%p", chn, prefix, name, value,
              FLT_OT_DPTR_ARGS(err));

  if ((chn == NULL) ||
      (!FLT_OT_STR_ISVALID(prefix) && !FLT_OT_STR_ISVALID(name)))
  ...
  while (http_find_header(htx, ist(""), &ctx, 1) == 1) {
    ...
    if (isteqi(n, ist_name))
      if (http_remove_header(htx, &ctx) == 1)
        FLT_OT_DBG(3, "HTTP header '%.*s: %.*s' removed", (int)n.len, n.ptr,
                   (int)v.len, v.ptr);
  }
 ...
 *   -
 */
int flt_ot_http_headers_remove(struct channel *chn, const char *prefix,
                               char **err) {
  int retval;
