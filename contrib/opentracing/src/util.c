...
void flt_ot_args_dump(char **args) {
  ...
  (void)fprintf(stderr, FLT_OT_DBG_FMT("%.*sargs[%d]: { '%s' "),
                dbg_indent_level, FLT_OT_DBG_INDENT, n, args[0]);
...
void flt_ot_filters_dump(void) {
  ...
  for (px = proxies_list; px != NULL; px = px->next) {
    ...
    list_for_each_entry(fconf, &(px->filter_configs),
                        list) if (fconf->id == ot_flt_id) {
...
const char *flt_ot_list_debug(const struct list *head) {
  ...
  if ((head == NULL) || LIST_ISEMPTY(head)) {
    (void)strncpy(retptr, (head == NULL) ? "{ null list }" : "{ empty list }",
                  sizeof(retbuf[0]));
    ...
    (void)snprintf(retptr, sizeof(retbuf[0]), "{ %p %p %zu }", head->p, head->n,
                   count);
  }
 ...
 *   -
 */
ssize_t flt_ot_chunk_add(struct buffer *chk, const void *src, size_t n,
                         char **err) {
...
void flt_ot_args_to_str(char **args, int idx, char **str) {
  ...
  for (i = idx; FLT_OT_ARG_ISVALID(i); i++)
    (void)memprintf(str, "%s%s%s", (*str == NULL) ? "" : *str,
                    (i == idx) ? "" : " ", args[i]);
}
 ...
 *   -
 */
double flt_ot_strtod(const char *nptr, double limit_min, double limit_max,
                     char **err) {
  char *endptr = NULL;
  double retval;
  ...
  if ((errno != 0) || FLT_OT_STR_ISVALID(endptr))
    ...
    FLT_OT_ERR("'%s' : value out of range [%.2f, %.2f]", nptr, limit_min,
               limit_max);
 ...
 *   -
 */
int64_t flt_ot_strtoll(const char *nptr, int64_t limit_min, int64_t limit_max,
                       char **err) {
  char *endptr = NULL;
  int64_t retval;
  ...
  if ((errno != 0) || FLT_OT_STR_ISVALID(endptr))
    ...
    FLT_OT_ERR("'%s' : value out of range [%" PRId64 ", %" PRId64 "]", nptr,
               limit_min, limit_max);
 ...
 *   -
 */
int flt_ot_sample_to_str(const struct sample_data *data, char *value,
  ...
  *value = '\0';
  ...
  if (data->type == SMP_T_ANY) {
    ...
    else if (inet_ntop(AF_INET, &(data->u.ipv4), value, INET_ADDRSTRLEN) ==
             NULL)
    ...
    if (INET6_ADDRSTRLEN > size)
      FLT_OT_ERR("sample data size too large");
    else if (inet_ntop(AF_INET6, &(data->u.ipv6), value, INET6_ADDRSTRLEN) ==
             NULL)
 ...
 *   -
 */
int flt_ot_sample_to_value(const char *key, const struct sample_data *data,
                           struct otc_value *value, char **err) {
  int retval = -1;
  ...
  if (data->type == SMP_T_BOOL) {
    ...
    if (value->value.string_value == NULL)
      ...
      retval = flt_ot_sample_to_str(data, (char *)value->value.string_value,
                                    global.tune.bufsize, err);
  }
 ...
 */
int flt_ot_sample_add(struct stream *s, uint dir,
                      struct flt_ot_conf_sample *sample,
                      struct flt_ot_scope_data *data, int type, char **err) {
  ...
  FLT_OT_FUNC("%p, %u, %p, %p, %d, %p:%p", s, dir, data, sample, type,
  ...
  list_for_each_entry(expr, &(sample->exprs), list) {
     ...
     */
    if (sample_process(s->be, s->sess, s, dir | SMP_OPT_FINAL, expr->expr,
                       &smp) != NULL) {
    ...
    if ((sample->num_exprs == 1) && (type == FLT_OT_EVENT_SAMPLE_TAG)) {
      ...
      if (buffer.area == NULL) {
        chunk_init(&buffer, FLT_OT_CALLOC(1, global.tune.bufsize),
                   global.tune.bufsize);
        if (buffer.area == NULL) {
          FLT_OT_ERR("out of memory");
      ...
      rc = flt_ot_sample_to_str(&(smp.data), buffer.area + buffer.data,
                                buffer.size - buffer.data, err);
      if (rc == -1) {
        retval = FLT_OT_RET_ERROR;
      } else {
        buffer.data += rc;
  ...
  if (retval == FLT_OT_RET_ERROR) {
    ...
    if (data->baggage == NULL) {
      ...
      retval = FLT_OT_RET_ERROR;
    } else if (otc_text_map_add(data->baggage, sample->key, 0,
                                value.value.string_value, 0, 0) == -1) {
      FLT_OT_ERR("out of memory");

      retval = FLT_OT_RET_ERROR;
    } else
      FLT_OT_DBG(3, "baggage[%zu]: '%s' -> '%s'", data->baggage->count - 1,
                 data->baggage->key[data->baggage->count - 1],
                 data->baggage->value[data->baggage->count - 1]);
  }
