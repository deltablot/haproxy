...
static void flt_ot_vars_scope_dump(struct vars *vars, const char *scope) {
  ...
  list_for_each_entry(var, &(vars->head), l)
      FLT_OT_DBG(2, "'%s.%s' -> '%.*s'", scope, var->name,
                 (int)var->data.u.str.data, var->data.u.str.area);
 ...
 *   -
 */
static inline struct vars *flt_ot_get_vars(struct stream *s,
                                           const char *scope) {
 ...
 *   -
 */
static int flt_ot_normalize_name(char *var_name, size_t size, int *len,
                                 const char *name, char **err) {
  int retval = 0;

  FLT_OT_FUNC("%p, %zu, %p, \"%s\", %p:%p", var_name, size, len, name,
 ...
 *   -
 */
static int flt_ot_var_name(const char *scope, const char *prefix,
                           const char *name, char *var_name, size_t size,
                           char **err) {
  int retval = 0;

  FLT_OT_FUNC("\"%s\", \"%s\", \"%s\", %p, %zu, %p:%p", scope, prefix, name,
              var_name, size, FLT_OT_DPTR_ARGS(err));
  ...
  if (retval == -1)
    FLT_OT_ERR("failed to construct variable name '%s.%s.%s'", scope, prefix,
               name);
 ...
 *   -
 */
int flt_ot_var_register(const char *scope, const char *prefix, const char *name,
                        char **err) {
  struct arg arg;
  char var_name[BUFSIZ];
  int retval;

  FLT_OT_FUNC("\"%s\", \"%s\", \"%s\", %p:%p", scope, prefix, name,
              FLT_OT_DPTR_ARGS(err));

  retval =
      flt_ot_var_name(scope, prefix, name, var_name, sizeof(var_name), err);
  if (retval == -1)
    FLT_OT_RETURN(retval);
 ...
 *   -
 */
int flt_ot_var_set(struct stream *s, const char *scope, const char *prefix,
                   const char *name, const char *value, uint opt, char **err) {
  struct sample smp;
  char var_name[BUFSIZ];
  int retval;

  FLT_OT_FUNC("%p, \"%s\", \"%s\", \"%s\", \"%s\", %u, %p:%p", s, scope, prefix,
 ...
 *   -
 */
int flt_ot_var_unset(struct stream *s, const char *scope, const char *prefix,
                     const char *name, uint opt, char **err) {
  struct sample smp;
  char var_name[BUFSIZ];
  int retval;

  FLT_OT_FUNC("%p, \"%s\", \"%s\", \"%s\", %u, %p:%p", s, scope, prefix, name,
 ...
 *   -
 */
int flt_ot_vars_unset(struct stream *s, const char *scope, const char *prefix,
                      uint opt, char **err) {
  struct sample smp;
  struct vars *vars;
  ...
  FLT_OT_FUNC("%p, \"%s\", \"%s\", %u, %p:%p", s, scope, prefix, opt,
              FLT_OT_DPTR_ARGS(err));
  ...
  var_prefix_len =
      flt_ot_var_name(NULL, prefix, NULL, var_prefix, sizeof(var_prefix), err);
  if (var_prefix_len == -1)
    FLT_OT_RETURN(retval);
  ...
  list_for_each_entry_safe(var, var_back, &(vars->head), l) {
    FLT_OT_DBG(3, "variable cmp '%s' '%s' %d", var_prefix, var->name,
               var_prefix_len);

    if (strncmp(var_prefix, var->name, var_prefix_len) == 0) {
      var_name_len =
          snprintf(var_name, sizeof(var_name), "%s.%s", scope, var->name);
      ...
      FLT_OT_DBG(2, "- '%s' -> '%.*s'", var_name, (int)var->data.u.str.data,
                 var->data.u.str.area);
 ...
 *   -
 */
int flt_ot_var_get(struct stream *s, const char *scope, const char *prefix,
                   const char *name, char **value, uint opt, char **err) {
  struct sample smp;
  char var_name[BUFSIZ], var_value[BUFSIZ];
  int retval;

  FLT_OT_FUNC("%p, \"%s\", \"%s\", \"%s\", %p:%p, %u, %p:%p", s, scope, prefix,
              name, FLT_OT_DPTR_ARGS(value), opt, FLT_OT_DPTR_ARGS(err));
  ...
  if (vars_get_by_name(var_name, retval, &smp)) {
    retval =
        flt_ot_sample_to_str(&(smp.data), var_value, sizeof(var_value), err);
    if (retval != -1)
      FLT_OT_DBG(3, "data type %d: '%s' = '%s'", smp.data.type, var_name,
                 var_value);
  } else {
...
struct otc_text_map *flt_ot_vars_get(struct stream *s, const char *scope,
  ...
  list_for_each_entry(var, &(vars->head), l) {
    ...
    if (strncmp(var_name, var->name, rc) == 0) {
      ...
      if (retptr == NULL) {
        break;
      } else if (otc_text_map_add(retptr, ot_var_name, i, var->data.u.str.area,
                                  var->data.u.str.data,
                                  OTC_TEXT_MAP_DUP_KEY |
                                      OTC_TEXT_MAP_DUP_VALUE) == -1) {
