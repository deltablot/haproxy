...
REGISTER_POOL(&pool_head_ot_span_context, "ot_span_context",
              MAX(sizeof(struct otc_span), sizeof(struct otc_span_context)));
#endif
...
void ot_text_map_show(const struct otc_text_map *text_map) {
  ...
  if ((text_map->key != NULL) && (text_map->value != NULL) &&
      (text_map->count > 0)) {
    size_t i;
 ...
 *   -
 */
static void *ot_mem_malloc(FLT_OT_DBG_ARGS(const char *func, int line, )
                               size_t size) {
 ...
 *   This function does not return a value.
 */
static void ot_mem_free(FLT_OT_DBG_ARGS(const char *func,
                                        int line, ) void *ptr) {
 ...
 *   -
 */
int ot_init(struct otc_tracer **tracer, const char *config, const char *plugin,
            char **err) {
  char cwd[PATH_MAX], path[PATH_MAX], errbuf[BUFSIZ] = "";
  int rc, retval = -1;

  FLT_OT_FUNC("%p:%p \"%s\", \"%s\", %p:%p", FLT_OT_DPTR_ARGS(tracer), config,
              plugin, FLT_OT_DPTR_ARGS(err));

  flt_ot_pools_info();
#ifdef USE_POOL_OT_SPAN_CONTEXT
  FLT_OT_DBG(2, "sizeof_pool(ot_span_context) = %u",
             pool_head_ot_span_context->size);
#endif
  ...
  *tracer = otc_tracer_init(path, config, NULL, errbuf, sizeof(errbuf));
  if (*tracer == NULL) {
    FLT_OT_ERR("%s", (*errbuf == '\0') ? "failed to initialize tracing library"
                                       : errbuf);
  } else {
 ...
 */
struct otc_span *
ot_span_init(struct otc_tracer *tracer, const char *operation_name,
             const struct timespec *ts_steady, const struct timespec *ts_system,
             int ref_type, int ref_ctx_idx, const struct otc_span *ref_span,
             const struct otc_tag *tags, int num_tags, char **err) {
  ...
  FLT_OT_FUNC("%p, \"%s\", %p, %p, %d, %d, %p, %p, %d, %p:%p", tracer,
              operation_name, ts_steady, ts_system, ref_type, ref_ctx_idx,
              ref_span, tags, num_tags, FLT_OT_DPTR_ARGS(err));
  ...
  if (ts_steady != NULL)
    (void)memcpy(&(options.start_time_steady.value), ts_steady,
                 sizeof(options.start_time_steady.value));

  if (ts_system != NULL)
    (void)memcpy(&(options.start_time_system.value), ts_system,
                 sizeof(options.start_time_system.value));

  if (FLT_OT_IN_RANGE(ref_type, otc_span_reference_child_of,
                      otc_span_reference_follows_from)) {
 ...
 */
struct otc_span *
ot_span_init_va(struct otc_tracer *tracer, const char *operation_name,
                const struct timespec *ts_steady,
                const struct timespec *ts_system, int ref_type, int ref_ctx_idx,
                const struct otc_span *ref_span, char **err,
                const char *tag_key, const char *tag_value, ...) {
  struct otc_tag tags[FLT_OT_MAXTAGS];
  int num_tags = 0;
  struct otc_span *retptr;

  FLT_OT_FUNC("%p, \"%s\", %p, %p, %d, %d, %p, %p:%p, \"%s\", \"%s\", ...",
              tracer, operation_name, ts_steady, ts_system, ref_type,
              ref_ctx_idx, ref_span, FLT_OT_DPTR_ARGS(err), tag_key, tag_value);

  if (tag_key != NULL) {
    va_list ap;

    va_start(ap, tag_value);
    for (num_tags = 0; (num_tags < FLT_OT_TABLESIZE(tags)) &&
  ...
  retptr = ot_span_init(tracer, operation_name, ts_steady, ts_system, ref_type,
                        ref_ctx_idx, ref_span, tags, num_tags, err);
 ...
 *   -
 */
int ot_span_tag(struct otc_span *span, const struct otc_tag *tags,
                int num_tags) {
  int retval = -1;
...
int ot_span_tag_va(struct otc_span *span, const char *key, int type, ...) {
  ...
  va_start(ap, type);
  for (retval = 0;
       (key != NULL) && FLT_OT_IN_RANGE(type, otc_value_bool, otc_value_null);
       retval++) {
    ot_value.type = type;
    if (type == otc_value_bool)
      ot_value.value.bool_value = va_arg(ap, typeof(ot_value.value.bool_value));
    else if (type == otc_value_double)
      ot_value.value.double_value =
          va_arg(ap, typeof(ot_value.value.double_value));
    else if (type == otc_value_int64)
      ot_value.value.int64_value =
          va_arg(ap, typeof(ot_value.value.int64_value));
    else if (type == otc_value_uint64)
      ot_value.value.uint64_value =
          va_arg(ap, typeof(ot_value.value.uint64_value));
    else if (type == otc_value_string)
      ot_value.value.string_value =
          va_arg(ap, typeof(ot_value.value.string_value));
 ...
 *   -
 */
int ot_span_log(struct otc_span *span, const struct otc_log_field *log_fields,
                int num_fields) {
  int retval = -1;
...
int ot_span_log_va(struct otc_span *span, const char *key, const char *value,
  ...
  va_start(ap, value);
  for (retval = 0; (retval < FLT_OT_TABLESIZE(log_field)) && (key != NULL);
       retval++) {
 ...
 *   -
 */
int ot_span_set_baggage(struct otc_span *span,
                        const struct otc_text_map *baggage) {
  size_t i;
  int retval = -1;
  ...
  for (retval = i = 0; i < baggage->count; i++) {
    FLT_OT_DBG(3, "set baggage: \"%s\" -> \"%s\"", baggage->key[i],
               baggage->value[i]);
 ...
 *   -
 */
int ot_span_set_baggage_va(struct otc_span *span, const char *key,
                           const char *value, ...) {
  va_list ap;
  int retval = -1;
 ...
 *   -
 */
struct otc_text_map *ot_span_baggage_va(const struct otc_span *span,
                                        const char *key, ...) {
  va_list ap;
  ...
  for (i = 0; (i < n) && (key != NULL); i++) {
    ...
    if (value != NULL) {
      (void)otc_text_map_add(retptr, key, 0, value, 0,
                             OTC_TEXT_MAP_DUP_KEY | OTC_TEXT_MAP_DUP_VALUE);

      FLT_OT_DBG(3, "get baggage[%d]: \"%s\" -> \"%s\"", i, retptr->key[i],
                 retptr->value[i]);
    } else {
...
ot_inject_text_map(struct otc_tracer *tracer, const struct otc_span *span,
                   struct otc_text_map_writer *carrier) {
 ...
 */
struct otc_span_context *
ot_inject_http_headers(struct otc_tracer *tracer, const struct otc_span *span,
                       struct otc_http_headers_writer *carrier, char **err) {
  struct otc_span_context *retptr = NULL;
  int rc;

  FLT_OT_FUNC("%p, %p, %p, %p:%p", tracer, span, carrier,
 ...
 */
struct otc_span_context *
ot_inject_binary(struct otc_tracer *tracer, const struct otc_span *span,
                 struct otc_custom_carrier_writer *carrier) {
  struct otc_span_context *retptr = NULL;
  int rc;
  ...
  if (rc != otc_propagation_error_code_success) {
    ...
    FLT_OT_DBG(
        3,
        "trace context: %016" PRIx64 "%016" PRIx64 ":%016" PRIx64 ":%016" PRIx64
        ":%02hhx <%s> <%s>",
        ctx->trace_id[0], ctx->trace_id[1], ctx->span_id, ctx->parent_span_id,
        ctx->flags,
        flt_ot_str_hex(ctx->baggage, carrier->binary_data.size - sizeof(*ctx)),
        flt_ot_str_ctrl(ctx->baggage,
                        carrier->binary_data.size - sizeof(*ctx)));
    FLT_OT_DBG_SPAN_CONTEXT(retptr);
#endif
  }
 ...
 */
struct otc_span_context *
ot_extract_text_map(struct otc_tracer *tracer,
                    struct otc_text_map_reader *carrier,
                    const struct otc_text_map *text_map) {
  struct otc_span_context *retptr = NULL;
  int rc;
 ...
 */
struct otc_span_context *
ot_extract_http_headers(struct otc_tracer *tracer,
                        struct otc_http_headers_reader *carrier,
                        const struct otc_text_map *text_map, char **err) {
  struct otc_span_context *retptr = NULL;
  int rc;

  FLT_OT_FUNC("%p, %p, %p, %p:%p", tracer, carrier, text_map,
              FLT_OT_DPTR_ARGS(err));
 ...
 */
struct otc_span_context *
ot_extract_binary(struct otc_tracer *tracer,
                  struct otc_custom_carrier_reader *carrier,
                  const struct otc_binary_data *binary_data) {
  struct otc_span_context *retptr = NULL;
  int rc;
  ...
  if ((FLT_OT_DEREF(binary_data, data, NULL) != NULL) &&
      (binary_data->size > 0)) {
    (void)memset(carrier, 0, sizeof(*carrier));
    (void)memcpy(&(carrier->binary_data), binary_data,
                 sizeof(carrier->binary_data));
...
void ot_span_finish(struct otc_span **span, const struct timespec *ts_finish,
                    const struct timespec *log_ts, const char *log_key,
  ...
  FLT_OT_FUNC("%p:%p, %p, %p, \"%s\", \"%s\", ...", FLT_OT_DPTR_ARGS(span),
              ts_finish, log_ts, log_key, log_value);
  ...
  if (ts_finish != NULL)
    (void)memcpy(&(options.finish_time.value), ts_finish,
                 sizeof(options.finish_time.value));
  ...
  if (log_key != NULL) {
    ...
    if (log_ts != NULL)
      (void)memcpy(&(log_records.timestamp.value), log_ts,
                   sizeof(log_records.timestamp.value));
