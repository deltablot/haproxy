 ...
 *
 * Based on "A Random IP reputation service acting as a Stream Processing
 * Offload Agent" Copyright 2016 HAProxy Technologies, Christopher Faulet
 * <cfaulet@haproxy.com>
 *
...
static int check_proto_version(struct spoe_frame *frame, char **buf,
                               char *end) {
  char *str, *p = *buf;
  uint64_t sz;
  int ret;
 ...
 * read bytes otherwise. */
static int check_max_frame_size(struct spoe_frame *frame, char **buf,
                                char *end) {
  char *p = *buf;
  uint64_t sz;
  int type, ret;
...
static int check_capabilities(struct spoe_frame *frame, char **buf, char *end) {
  ...
  DEBUG(frame->worker, "<%lu> HAProxy capabilities : %.*s", client->id, (int)sz,
        str);
  ...
  while (sz) {
    ...
    if (sz >= 10 && !strncmp(str, "pipelining", 10)) {
      ...
      sz -= 5;
      if (!sz || isspace(*str) || *str == ',') {
        DEBUG(frame->worker, "<%lu> HAProxy supports asynchronous frame",
              client->id);
        client->async = true;
      }
 ...
 * of read bytes otherwise. */
static int check_discon_message(struct spoe_frame *frame, char **buf,
                                char *end) {
  char *str, *p = *buf;
  uint64_t sz;
  int ret;
...
static void signal_cb(evutil_socket_t sig, short events, void *user_data) {
  ...
  for (i = 0; i < num_workers; i++) {
    ...
    DEBUG(&null_worker, "Event loop stopped for worker %02d", workers[i].id);
  }
}
...
static void process_frame_cb(evutil_socket_t fd, short events, void *arg) {
  ...
  while (p < end) {
    ...
    frame->offset =
        (p - frame->buf); /* Save index to handle errors and skip args */
    if (!memcmp(str, "check-request", sz)) {
      ...
      frame->defender_status =
          defender_process_request(frame->worker, &request);
    } else {
    skip_message:
...
static void read_frame_cb(evutil_socket_t fd, short events, void *arg) {
  ...
  n = read(client->fd, frame->buf + frame->offset, frame->len - frame->offset);
  if (n <= 0) {
    if (n < 0) {
...
static void write_frame_cb(evutil_socket_t fd, short events, void *arg) {
  ...
  n = write(client->fd, frame->buf + frame->offset, frame->len - frame->offset);
  if (n <= 0) {
    if (n < 0) {
  ...
  DEBUG(client->worker, "<%lu> Frame of %u bytes send", client->id, frame->len);

  switch (client->state) {
  case SPOA_ST_CONNECTING:
    if (frame->hcheck == true) {
      DEBUG(client->worker, "<%lu> Close client after healthcheck", client->id);
      goto close;
    }
...
static void *worker_function(void *data) {
...
static void usage(char *prog) {
  fprintf(
      ...
      "    -c <capability>      Enable the support of the specified "
      "capability\n"
      "    -t <time>            Set a delay to process a message (default: 0)\n"
      "                           The value is specified in milliseconds by "
      "default,\n"
      "                           but can be in any other unit if the number "
      "is suffixed\n"
      "                           by a unit (us, ms, s)\n"
      "\n"
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < num_workers; ++i) {
    ...
    if ((w->base = event_base_new()) == NULL) {
      LOG(&null_worker, "Failed to initialize libevent for worker %02d : %m",
          w->id);
      goto error;
    }

    w->monitor_event =
        event_new(w->base, fd, EV_PERSIST, worker_monitor_cb, (void *)w);
    ...
    if (pthread_create(&w->thread, NULL, worker_function, (void *)w)) {
      LOG(&null_worker, "Failed to start thread for worker %02d : %m", w->id);
    }
    DEBUG(&null_worker, "Worker %02d initialized", w->id);
  }

  accept_event =
      event_new(base, fd, EV_READ | EV_PERSIST, accept_cb, (void *)base);
  ...
  DEBUG(&null_worker,
        "Server is ready"
        " [fragmentation=%s - pipelining=%s - async=%s - debug=%s - "
        "max-frame-size=%u]",
        (fragmentation ? "true" : "false"), (pipelining ? "true" : "false"),
        (async ? "true" : "false"), (debug ? "true" : "false"), max_frame_size);
