...
static const char *defender_read_config(const char *file) {
  ...
  defender_module_config =
      defender_module.create_dir_config(defender_pool, "/");
  ...
  err = read_module_config(server, defender_module_config, defender_module.cmds,
...
static const char *defender_set_logger(const char *file) {
  ...
  if (apr_file_open(&server->error_log, logname,
    ...
    return apr_pstrcat(defender_pool, "Cannot open log file, ", logname, NULL);
  }
...
static apr_status_t defender_input_filter(ap_filter_t *f,
  ...
  bb = (apr_bucket_brigade *)apr_table_get(f->r->notes,
                                           DEFENDER_BRIGADE_REQUEST);
...
int defender_init(const char *config_file, const char *log_file) {
  ...
  rv = apr_sockaddr_info_get(&server->addrs->host_addr, "127.0.0.1", APR_UNSPEC,
                             0, 0, defender_pool);
  if (rv != APR_SUCCESS) {
  ...
  ap_register_input_filter(DEFENDER_INPUT_FILTER, defender_input_filter, NULL,
                           AP_FTYPE_RESOURCE);
...
int defender_process_request(struct worker *worker,
  ...
  if (request->id.data.u.str.area && request->id.data.u.str.data > 0) {
    ...
    apr_table_setn(r->subprocess_env, "UNIQUE_ID", DEFENDER_DEFAULT_UNIQUE_ID);
  }
  ...
  r->unparsed_uri =
      defender_printf(r->pool, "%.*s%s%.*s", path->data, path->area,
                      query->data > 0 ? "?" : "", query->data, query->area);
  if (!r->unparsed_uri)
    goto out;
  ...
  r->protocol =
      defender_printf(r->pool, "%s%.*s", version->data > 0 ? "HTTP/" : "",
                      version->data, version->area);
  if (!r->protocol)
    goto out;

  r->the_request = defender_printf(r->pool, "%.*s %s%s%s", method->data,
                                   method->area, r->unparsed_uri,
                                   version->data > 0 ? " " : "", r->protocol);
  if (!r->the_request)
    goto out;
