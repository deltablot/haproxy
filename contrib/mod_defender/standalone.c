...
static char x2c(const char *what) {
  ...
  xstr[4] = '\0';
  digit =
      apr_xlate_conv_byte(ap_hdrs_from_ascii, 0xFF & strtol(xstr, NULL, 16));
      ...
      *x = *y;
      ...
      if (!apr_isxdigit(*(y + 1)) || !apr_isxdigit(*(y + 2))) {
        ...
        *x = '%';
        ...
        char decoded;
        decoded = x2c(y + 1);
        if ((decoded == '\0') || (forbid && ap_strchr_c(forbid, decoded))) {
          badpath = 1;
          *x = decoded;
          y += 2;
...
static void log_error_core(const char *file, int line, int module_index,
                           ...
                           apr_pool_t *pool, const char *fmt, va_list args) {
...
ap_log_error_(const char *file, int line, int module_index, int level,
  ...
  va_start(args, fmt);
  log_error_core(file, line, module_index, level, status, s, NULL, NULL, NULL,
                 fmt, args);
  va_end(args);
}
...
AP_DECLARE(void)
ap_log_cerror_(const char *file, int line, int module_index, int level,
               apr_status_t status, const conn_rec *c, const char *fmt, ...) {
  va_list args;
...
AP_DECLARE(apr_file_t *) ap_piped_log_write_fd(piped_log *pl) { return NULL; }

static cmd_parms default_parms = {NULL, 0,    0,    NULL, -1,   NULL, NULL,
                                  NULL, NULL, NULL, NULL, NULL, NULL, NULL};

AP_DECLARE(char *) ap_server_root_relative(apr_pool_t *p, const char *file) {
  char *newpath = NULL;
  apr_status_t rv;
  rv = apr_filepath_merge(&newpath, ap_server_root, file, APR_FILEPATH_TRUENAME,
                          p);
  if (newpath && (rv == APR_SUCCESS || APR_STATUS_IS_EPATHWILD(rv) ||
                  APR_STATUS_IS_ENOENT(rv) || APR_STATUS_IS_ENOTDIR(rv))) {
    return newpath;
  } else {
    return NULL;
  }
...
ap_get_brigade(ap_filter_t *next, apr_bucket_brigade *bb, ap_input_mode_t mode,
...
static void trie_node_link(apr_pool_t *p, filter_trie_node *parent,
  ...
  if (parent->nchildren == parent->size) {
    ...
    parent->size *= 2;
    new = (filter_trie_child_ptr *)apr_palloc(
        p, parent->size * sizeof(filter_trie_child_ptr));
...
static filter_trie_node *trie_node_alloc(apr_pool_t *p,
  ...
  new_node->children = (filter_trie_child_ptr *)apr_palloc(
      p, new_node->size * sizeof(filter_trie_child_ptr));
  return new_node;
}
...
ap_register_input_filter(const char *name, ap_in_filter_func filter_func,
                         ap_init_filter_func filter_init,
                         ap_filter_type ftype) {
  ap_filter_func f;
  ...
  if (frec->ftype < AP_FTYPE_PROTOCOL) {
    if (r) {
      ...
      ap_log_cerror(
          ...
          APLOGNO(00080) "a content filter was added without a request: %s",
          frec->name);
      return NULL;
    }
    ...
    if (r) {
      ...
      ap_log_cerror(
          APLOG_MARK, APLOG_ERR, 0, c,
          APLOGNO(00081) "a protocol filter was added without a request: %s",
          frec->name);
      return NULL;
    }
  } else {
...
static ap_filter_t *
add_any_filter(const char *name, void *ctx, request_rec *r, conn_rec *c,
               const filter_trie_node *reg_filter_set, ap_filter_t **r_filters,
  ...
  if (reg_filter_set) {
    ...
    if (node && node->frec) {
      return add_any_filter_handle(node->frec, ctx, r, c, r_filters, p_filters,
                                   c_filters);
    }
  }

  ap_log_cerror(APLOG_MARK, APLOG_ERR, 0, r ? r->connection : c,
                APLOGNO(00082) "an unknown filter was not added: %s", name);
  return NULL;
}

AP_DECLARE(ap_filter_t *)
ap_add_input_filter(const char *name, void *ctx, request_rec *r, conn_rec *c) {
...
#ifdef DEBUG
  ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Done with config file %s",
               cfp->name);
#endif
...
static apr_status_t cfg_close(void *param) { return apr_file_close(param); }
...
static int cfg_trim_line(char *buf) {
  ...
  ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL,
               APLOGNO(00555) "Read config: '%s'", buf);
#endif
  return end - start + 1;
}
...
static apr_status_t pcfg_openfile(ap_configfile_t **ret_cfg, apr_pool_t *p,
                                  const char *name) {
  ...
  if (name == NULL) {
    ap_log_error(
        ...
        APLOGNO(
            00552) "Internal error: pcfg_openfile() called with NULL filename");
    return APR_EBADF;
  }

  status =
      apr_file_open(&file, name, APR_READ | APR_BUFFERED, APR_OS_DEFAULT, p);
#ifdef DEBUG
  ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, NULL,
               APLOGNO(00553) "Opening config file %s (%s)", name,
               (status != APR_SUCCESS) ? apr_strerror(status, buf, sizeof(buf))
                                       : "successful");
#endif
  ...
  if (finfo.filetype != APR_REG && strcmp(name, "/dev/null") != 0) {
    ap_log_error(
        APLOG_MARK, APLOG_ERR, 0, NULL,
        APLOGNO(00554) "Access to file %s denied by server: not a regular file",
        name);
    apr_file_close(file);
    return APR_EBADF;
  }
...
static const char *invoke_cmd(const command_rec *cmd, cmd_parms *parms,
  ...
  case NO_ARGS:
    if (*args != 0)
      return apr_pstrcat(parms->pool, cmd->name, " takes no arguments", NULL);
...
static char *make_full_path(apr_pool_t *a, const char *src1, const char *src2) {
...
static const char *
process_resource_config_nofnmatch(const char *fname, apr_array_header_t *ari,
                                  apr_pool_t *p, apr_pool_t *ptemp,
                                  unsigned depth, int optional) {
  const char *error;
  apr_status_t rv;
  ...
  if (is_directory(ptemp, fname)) {
    ...
    if (rv != APR_SUCCESS) {
      return apr_psprintf(p, "Could not open config directory %s: %pm", path,
                          &rv);
    }
    ...
    if (candidates->nelts != 0) {
      qsort((void *)candidates->elts, candidates->nelts, sizeof(fnames),
            fname_alphasort);
      ...
      for (current = 0; current < candidates->nelts; ++current) {
        ...
        error = process_resource_config_nofnmatch(fnew->fname, ari, p, ptemp,
                                                  depth, optional);
        if (error) {
          return error;
        }
...
static const char *process_resource_config_fnmatch(
    const char *path, const char *fname, apr_array_header_t *ari, apr_pool_t *p,
    apr_pool_t *ptemp, unsigned depth, int optional) {
  const char *rest;
  apr_status_t rv;
  apr_dir_t *dirp;
  apr_finfo_t dirent;
  ...
  if (!apr_fnmatch_test(fname)) {
    ...
    if (!rest) {
      return process_resource_config_nofnmatch(path, ari, p, ptemp, 0,
                                               optional);
    } else {
      return process_resource_config_fnmatch(path, rest, ari, p, ptemp, 0,
                                             optional);
    }
  }
  ...
  while (apr_dir_read(&dirent, APR_FINFO_DIRENT | APR_FINFO_TYPE, dirp) ==
    ...
    /* strip out '.' and '..' */
    if (strcmp(dirent.name, ".") && strcmp(dirent.name, "..") &&
        (apr_fnmatch(fname, dirent.name, APR_FNM_PERIOD) == APR_SUCCESS)) {
  ...
  if (candidates->nelts != 0) {
    ...
    for (current = 0; current < candidates->nelts; ++current) {
      fnew = &((fnames *)candidates->elts)[current];
      if (!rest) {
        error = process_resource_config_nofnmatch(fnew->fname, ari, p, ptemp, 0,
                                                  optional);
      } else {
        error = process_resource_config_fnmatch(fnew->fname, rest, ari, p,
                                                ptemp, 0, optional);
      }
      if (error) {
        return error;
    ...
    if (!optional) {
      return apr_psprintf(p,
                          "No matches for the wildcard '%s' in '%s', failing "
...
static const char *process_fnmatch_configs(const char *fname,
  ...
  if (!apr_fnmatch_test(fname)) {
    ...
    status =
        apr_filepath_root(&rootpath, &filepath, APR_FILEPATH_TRUENAME, ptemp);

    /* we allow APR_SUCCESS and APR_EINCOMPLETE */
    if (APR_ERELATIVE == status) {
      return apr_pstrcat(p, "Include must have an absolute path, ", fname,
                         NULL);
    ...
    /* walk the filepath */
    return process_resource_config_fnmatch(rootpath, filepath, ari, p, ptemp, 0,
                                           optional);
  }
}

const char *read_module_config(server_rec *s, void *mconfig,
                               const command_rec *cmds, apr_pool_t *p,
                               apr_pool_t *ptemp, const char *filename) {
  ...
  while (ari->nelts || arr->nelts) {
    ...
    while (!(cfg_getline(line, MAX_STRING_LEN, parms->config_file))) {
      ...
      if (!strcasecmp(cmd_name, "IncludeOptional") ||
        ...
        if (*w == '\0' || *args != 0) {
          errmsg =
              apr_pstrcat(parms->pool, cmd_name, " takes one argument", NULL);
          goto out;
        }
      ...
      if (!(cmd = find_command(cmd_name, cmds))) {
        errmsg =
            apr_pstrcat(parms->pool, "Invalid command '", cmd_name, "'", NULL);
        goto out;
      }
...
int lookup_builtin_method(const char *method, apr_size_t len) {
  ...
  case 4:
    ...
    case 'P':
      return (method[1] == 'O' && method[2] == 'S' && method[3] == 'T'
                  ...
                  : UNKNOWN_METHOD);
    case 'M':
      return (method[1] == 'O' && method[2] == 'V' && method[3] == 'E'
                  ? M_MOVE
                  : UNKNOWN_METHOD);
    case 'L':
      return (method[1] == 'O' && method[2] == 'C' && method[3] == 'K'
                  ? M_LOCK
                  : UNKNOWN_METHOD);
    case 'C':
      return (method[1] == 'O' && method[2] == 'P' && method[3] == 'Y'
                  ? M_COPY
                  : UNKNOWN_METHOD);
    default:
      return UNKNOWN_METHOD;
  ...
  case 10:
    switch (method[0]) {
    case 'U':
      return (memcmp(method, "UNCHECKOUT", 10) == 0 ? M_UNCHECKOUT
                                                    : UNKNOWN_METHOD);
    case 'M':
      return (memcmp(method, "MKACTIVITY", 10) == 0 ? M_MKACTIVITY
                                                    : UNKNOWN_METHOD);
    default:
      return UNKNOWN_METHOD;
    }
  ...
  case 11:
    return (memcmp(method, "MKWORKSPACE", 11) == 0 ? M_MKWORKSPACE
                                                   : UNKNOWN_METHOD);

  case 15:
    return (memcmp(method, "VERSION-CONTROL", 15) == 0 ? M_VERSION_CONTROL
                                                       : UNKNOWN_METHOD);

  case 16:
    return (memcmp(method, "BASELINE-CONTROL", 16) == 0 ? M_BASELINE_CONTROL
                                                        : UNKNOWN_METHOD);
