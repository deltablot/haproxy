...
int main(int argc, char **argv) {
  ...
  for (line = 1; fgets(hex, sizeof(hex), stdin); line++) {
    ...
    for (idx = 0; idx < outlen - 1; idx++) {
      ...
      //       list[idx].n.ptr ? istpad(trash.str, list[idx].n).ptr :
      //       h2_phdr_to_str(list[idx].n.len));

      // printf("\e[1;35m%s\e[0m\n", istpad(trash.str, list[idx].v).ptr);

      printf("      %s: ", list[idx].n.ptr ? istpad(trash.area, list[idx].n).ptr
                                           : h2_phdr_to_str(list[idx].n.len));

      printf("%s [n=(%p,%d) v=(%p,%d)]\n", istpad(trash.area, list[idx].v).ptr,
             list[idx].n.ptr, (int)list[idx].n.len, list[idx].v.ptr,
             (int)list[idx].v.len);
    }
    puts(">>>");
#ifdef DEBUG_HPACK
