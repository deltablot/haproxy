...
static char *chunk_strdup(struct request_rec *req, const char *str,
                          size_t len) {
  char *out;
...
int modsecurity_load(const char *file) {
  ...
  if (modsec_config == NULL) {
    LOG(&null_worker,
        "ModSecurity default configuration initialisation failed.\n");
    return -1;
  }
...
int modsecurity_process(struct worker *worker,
  ...
  if (req->hostname != NULL) {
    req->parsed_uri.hostname =
        chunk_strdup(req, req->hostname, strlen(req->hostname));
  } else {
  ...
  req->the_request =
      printf_dup(req, "%.*s %.*s%s%.*s %.*s", meth_len, meth, path_len, path,
                 qs_len > 0 ? "?" : "", qs_len, qs, vers_len, vers);
  if (!req->the_request) {
    errno = ENOMEM;
    goto fail;
  }
  ...
  if (req->hostname && req->parsed_uri.scheme && req->parsed_uri.path) {
    i = snprintf(NULL, 0, "%s://%s%s", req->parsed_uri.scheme, req->hostname,
                 req->parsed_uri.path);
    ...
    i = snprintf(req->uri, i + 1, "%s://%s%s", req->parsed_uri.scheme,
                 req->hostname, req->parsed_uri.path);
  }
  ...
  apr_table_setn(req->subprocess_env, "UNIQUE_ID",
                 chunk_strdup(req, uniqueid, uniqueid_len));
  ...
  data_bucket =
      apr_bucket_alloc(sizeof(*data_bucket), req->connection->bucket_alloc);
  if (!data_bucket) {
    errno = ENOMEM;
    goto fail;
  }
  ...
  link_bucket =
      apr_bucket_alloc(sizeof(*link_bucket), req->connection->bucket_alloc);
  if (!link_bucket) {
    errno = ENOMEM;
    goto fail;
  }
...
fail:
  ...
  if (fail) {
    ...
    switch (errno) {
    case ERANGE:
      LOG(worker, "Invalid range");
    case ENOMEM:
      LOG(worker, "Out of memory error");
    case EINVAL:
      LOG(worker, "Invalid value");
    default:
      LOG(worker, "Unknown error");
    }
  }
