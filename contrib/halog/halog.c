...
#define FILT_COUNT_URL_ANY                                                     \
  (FILT_COUNT_URL_ONLY | FILT_COUNT_URL_COUNT | FILT_COUNT_URL_ERR |           \
   FILT_COUNT_URL_TTOT | FILT_COUNT_URL_TAVG | FILT_COUNT_URL_TTOTO |          \
   FILT_COUNT_URL_TAVGO | FILT_COUNT_URL_BAVG | FILT_COUNT_URL_BTOT)
...
void filter_count_url(const char *accept_field, const char *time_field,
                      struct timer **tptr);
void filter_count_ip(const char *source_field, const char *accept_field,
                     const char *time_field, struct timer **tptr);
void filter_count_srv_status(const char *accept_field, const char *time_field,
                             struct timer **tptr);
void filter_count_cook_codes(const char *accept_field, const char *time_field,
                             struct timer **tptr);
void filter_count_term_codes(const char *accept_field, const char *time_field,
                             struct timer **tptr);
void filter_count_status(const char *accept_field, const char *time_field,
                         struct timer **tptr);
void filter_graphs(const char *accept_field, const char *time_field,
                   struct timer **tptr);
void filter_output_line(const char *accept_field, const char *time_field,
                        struct timer **tptr);
void filter_accept_holes(const char *accept_field, const char *time_field,
                         struct timer **tptr);
...
void usage(FILE *output, const char *msg) {
  fprintf(
      ...
      "       [-s <skip>] [-e|-E] [-H] [-rt|-RT <time>] [-ad <delay>] [-ac "
      "<count>]\n"
      "       [-v] [-Q|-QS] [-tcn|-TCN <termcode>] [ -hs|-HS [min][:[max]] ] [ "
      "-time [min][:[max]] ] < log\n"
      "\n",
      msg ? msg : "");
}
...
void help() {
  ...
  printf(
      ...
      " -H                      only match lines containing HTTP logs (ignore "
      "TCP)\n"
      " -E                      only match lines without any error (no 5xx "
      "status)\n"
      " -e                      only match lines with errors (status 5xx or "
      "negative)\n"
      " -rt|-RT <time>          only match response times larger|smaller than "
      "<time>\n"
      " -Q|-QS                  only match queued requests (any queue|server "
      "queue)\n"
      " -tcn|-TCN <code>        only match requests with/without termination "
      "code <code>\n"
      " -hs|-HS <[min][:][max]> only match requests with HTTP status codes "
      "within/not\n"
      "                         within min..max. Any of them may be omitted. "
      "Exact\n"
      "                         code is checked for if no ':' is specified.\n"
      " -time <[min][:max]>     only match requests recorded between "
      "timestamps.\n"
      ...
      " -s <skip_n_fields>      skip n fields from the beginning of a line "
      "(default %d)\n"
      "                         you can also use -n to start from earlier then "
      "field %d\n"
      "\n"
...
int convert_date_to_timestamp(const char *field) {
  ...
  if (likely(timeinfo)) {
    if (timeinfo->tm_min == m && timeinfo->tm_hour == h &&
        timeinfo->tm_mday == d && timeinfo->tm_mon == mo - 1 &&
...
int main(int argc, char **argv) {
  ...
  void (*line_filter)(const char *accept_field, const char *time_field,
  ...
  while (argc > 0) {
    ...
    if (strcmp(argv[0], "-ad") == 0) {
      ...
      if (argc < 2)
        die("missing option for -s");
      argc--;
      argv++;
      skip_fields = atol(*argv);
    } else if (strcmp(argv[0], "-m") == 0) {
      if (argc < 2)
        die("missing option for -m");
      argc--;
      argv++;
      lines_max = atol(*argv);
      ...
      if (argc < 2)
        die("missing option for -hs/-HS ([min]:[max])");
      ...
      if (argc < 2)
        die("missing option for -time ([min]:[max])");
  ...
  if (!line_filter && /* FILT_COUNT_ONLY ( see above), and no input filter (see
                         below) */
      !(filter &
        (FILT_HTTP_ONLY | FILT_TIME_RESP | FILT_ERRORS_ONLY | FILT_HTTP_STATUS |
         FILT_QUEUE_ONLY | FILT_QUEUE_SRV_ONLY | FILT_TERM_CODE_NAME)) &&
      !(filter2 & (FILT2_TIMESTAMP))) {
  ...
  while ((line = fgets2(stdin)) != NULL) {
    ...
    if (filter & FILT_TIME_RESP) {
      ...
      field_stop(time_field + 1);
      /* we have field TIME_FIELD in [time_field]..[e-1], let's check only the
       * response time */
    ...
    if (filter & (FILT_ERRORS_ONLY | FILT_HTTP_STATUS)) {
      ...
      if (filter & FILT_ERRORS_ONLY)
        test &= (val < 0 || (val >= 500 && val <= 599)) ^
                !!(filter & FILT_INVERT_ERRORS);

      if (filter & FILT_HTTP_STATUS)
        test &= (val >= filt_http_status_low && val <= filt_http_status_high) ^
                !!(filter & FILT_INVERT_HTTP_STATUS);
    }
    ...
    if (filter & FILT_TERM_CODE_NAME) {
      ...
      test &= (b[0] == filter_term_code_name[0] &&
               b[1] == filter_term_code_name[1]) ^
              !!(filter & FILT_INVERT_TERM_CODE_NAME);
    }
...
skip_filters:
  ...
  if (filter & (FILT_ACC_COUNT | FILT_ACC_DELAY)) {
     ...
     *    <percent> <total> <max_req_time> <max_conn_time> <max_resp_time>
     * <max_data_time> We don't count errs.
     */
    ...
    printf("#srv_name 1xx 2xx 3xx 4xx 5xx other tot_req req_ok pct_ok avg_ct "
           "avg_rt\n");
    ...
    while (srv_node) {
      ...
      printf("%s %d %d %d %d %d %d %d %d %.1f %d %d\n", srv_node->key,
             srv->st_cnt[1], srv->st_cnt[2], srv->st_cnt[3], srv->st_cnt[4],
             srv->st_cnt[5], srv->st_cnt[0], tot_rq, srv->nb_ok,
             (double)srv->nb_ok * 100.0 / (tot_rq ? tot_rq : 1),
             (int)(srv->cum_ct / (srv->nb_ct ? srv->nb_ct : 1)),
             (int)(srv->cum_rt / (srv->nb_rt ? srv->nb_rt : 1)));
    ...
    if (!(filter & FILT_COUNT_URL_ONLY)) {
      ...
      for (node = eb_first(&timers[0]); node; node = next) {
        ...
        if (filter & (FILT_COUNT_URL_COUNT | FILT_COUNT_IP_COUNT))
          ...
          ustat->node.val.key =
              (ustat->nb_req - ustat->nb_err)
                  ? ustat->total_time_ok / (ustat->nb_req - ustat->nb_err)
                  : 0;
    ...
    while (node) {
      ...
      printf("%d %d %Ld %Ld %Ld %Ld %Ld %Ld %s\n", ustat->nb_req, ustat->nb_err,
             ...
             (ustat->nb_req - ustat->nb_err)
                 ...
                 : 0,
             ustat->nb_req ? ustat->total_bytes_sent / ustat->nb_req : 0,
             ustat->total_bytes_sent, ustat->url);
...
void filter_count_url(const char *accept_field, const char *time_field,
  ...
  if (!time_field) {
    time_field = field_start(accept_field, TIME_FIELD - ACCEPT_FIELD +
                                               1); // avg 115 ns per line
    if (unlikely(!*time_field)) {
      truncated_line(linenum, line);
      return;
    }
   ...
   * the next field because it's a capture. Let's fall back to the "method"
   * itself if there's nothing else.
   */
...
void filter_graphs(const char *accept_field, const char *time_field,
  ...
  if (filter & FILT_GRAPH_TIMERS) {
    if (err) {
      ...
      t2 = insert_timer(&timers[1], tptr, array[0]);
      t2->count++; // req
      t2 = insert_timer(&timers[2], tptr, array[2]);
      t2->count++; // conn
      t2 = insert_timer(&timers[3], tptr, array[3]);
      t2->count++; // resp

      v = array[4] - array[0] - array[1] - array[2] - array[3]; // data time
      if (v < 0 && !(filter & FILT_QUIET))
        fprintf(stderr, "ERR: %s (%d %d %d %d %d => %d)\n", line, array[0],
                array[1], array[2], array[3], array[4], v);
      t2 = insert_timer(&timers[4], tptr, v);
      t2->count++;
      lines_out++;
    }
  } else { /* percentile */
    if (err) {
      ...
      t2 = insert_value(&timers[1], tptr, array[0]);
      t2->count++; // req
      t2 = insert_value(&timers[2], tptr, array[2]);
      t2->count++; // conn
      t2 = insert_value(&timers[3], tptr, array[3]);
      t2->count++; // resp
      ...
      if (v < 0 && !(filter & FILT_QUIET))
        fprintf(stderr, "ERR: %s (%d %d %d %d %d => %d)\n", line, array[0],
                array[1], array[2], array[3], array[4], v);
      t2 = insert_value(&timers[4], tptr, v);
      t2->count++;
      lines_out++;
    }
  }
