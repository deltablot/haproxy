...
/* return non-zero if the argument contains at least one zero byte. See
 * principle above. */
...
static char *find_lf(char *next, char *end) {
  ...
  if (sizeof(long) == 4) { /* 32-bit system */
    ...
    if (next <= end) {
      ...
      if ((((unsigned long)next) & 4) &&
          !has_zero32(*(unsigned int *)next ^ 0x0A0A0A0AU))
        next += 4;
    }
...
const char *fgets2(FILE *stream) {
  static char
      buffer[FGETS2_BUFSIZE + 68]; /* Note: +32 is enough on 32-bit systems */
