...
// sed -n '/^static inline/s:^\([^ ]\+\) \([^ ]\+\) \(.*[* ]\)\([^*
// ]\+(\)\(.*\):\3f_\4\5 { return \4); }\nstatic int test_\4)\n{\n\treturn
// 0;\n}\n:p' include/common/ist.h sed -n '/^static inline/s:^\([^ ]\+\) \([^
// ]\+\) \(.*[* ]\)\([^* ]\+(\)\(.*\):\tif (test_\4)) printf("\4)\\n");:p'
// include/common/ist.h sed -n '/^static inline/s:^\([^ ]\+\) \([^ ]\+\) \(.*[*
// ]\)\([^* ]\+(\)\(.*\):\tprintf("%4d \4)\\n", test_\4));:p'
// include/common/ist.h

struct ist f_ist(const void *str) {
  return ist(str);
}
static int test_ist() {
  if (ist("foo").ptr == NULL)
...
struct ist f_ist2(const void *ptr, size_t len) {
  return ist2(ptr, len);
}
int test_ist2() {
...
struct ist f_istnext(struct ist ist) {
  return istnext(ist);
}
int test_istnext() {
...
struct ist f_istpad(void *buf, const struct ist ist) {
  return istpad(buf, ist);
}
int test_istpad() {
  char buf[5] = "xxxxx";
...
struct ist f_isttrim(struct ist ist, size_t size) {
  return isttrim(ist, size);
}
int test_isttrim() {
...
struct ist f_istzero(struct ist ist, size_t size) {
  return istzero(ist, size);
}
int test_istzero() {
  char buf[5] = "xxxxx";
...
int f_istdiff(const struct ist ist1, const struct ist ist2) {
  return istdiff(ist1, ist2);
}
int test_istdiff() {
...
int f_istmatch(const struct ist ist1, const struct ist ist2) {
  return istmatch(ist1, ist2);
}
int test_istmatch() {
...
int f_istnmatch(struct ist ist1, struct ist ist2, size_t count) {
  return istnmatch(ist1, ist2, count);
}
int test_istnmatch() {
...
int f_isteq(const struct ist ist1, const struct ist ist2) {
  return isteq(ist1, ist2);
}
int test_isteq() {
  if (isteq(ist(""), ist("")) == 0)
    return __LINE__;
...
int f_istneq(struct ist ist1, struct ist ist2, size_t count) {
  return istneq(ist1, ist2, count);
}
int test_istneq() {
...
ssize_t f_istcpy(struct ist *dst, const struct ist src, size_t count) {
  return istcpy(dst, src, count);
}
int test_istcpy() {
  char buf[100] = "foobar";
...
ssize_t f_istscpy(struct ist *dst, const struct ist src, size_t count) {
...
ssize_t f_istcat(struct ist *dst, const struct ist src, size_t count) {
  return istcat(dst, src, count);
}
int test_istcat() {
  char buf[11] = "foobar";
  struct ist dst = ist(buf);
...
ssize_t f_istscat(struct ist *dst, const struct ist src, size_t count) {
  return istscat(dst, src, count);
}
int test_istscat() {
  char buf[11] = "foobar";
  struct ist dst = ist(buf);
...
struct ist f_istfind(struct ist ist, char chr) {
  return istfind(ist, chr);
}
int test_istfind() {
  struct ist foobar = ist("foobar");
...
struct ist f_istskip(struct ist ist, char chr) {
  return istskip(ist, chr);
}
int test_istskip() {
  struct ist foobar = ist("foobar");
  struct ist r = ist("r");
...
struct ist f_istist(struct ist ist, const struct ist pat) {
  return istist(ist, pat);
}
int test_istist() {
  struct ist foobar = ist("foobar");
