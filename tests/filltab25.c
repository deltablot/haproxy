...
static inline void dequeue_srv(struct srv *s) { eb32_delete(&s->node); }

/* queues a server into the correct tree depending on ->next */
void put_srv(struct srv *s) {
  if (s->w <= 0 ||
      s->next >=
          2 * sw || /* delay everything which does not fit into the window */
      s->next >= sw + nsw) { /* and everything which does not fit into the
                                theorical new window */
    /* put into next tree */
    s->next -=
        sw; // readjust next in case we could finally take this back to current.
    queue_by_weight_0(next_tree, s);
  } else {
    // The overflow problem is caused by the scale we want to apply to user
    // weight to turn it into effective weight. Since this is only used to
    // provide a smooth slowstart on very low weights (1), it is a pure waste.
    // Thus, we just have to apply a small scaling factor and warn the user that
    // slowstart is not very smooth on low weights. The max key is about
    // ((scale*maxw)*(scale*maxw)*nbsrv)/ratio  (where the ratio is the
    // arbitrary divide we perform in the examples above). Assuming that
    // ratio==scale, this translates to maxkey=scale*maxw^2*nbsrv, so
    //    max_nbsrv=2^32/255^2/scale ~= 66051/scale
    // Using a scale of 16 is enough to support 4000 servers without overflow,
    // providing 6% steps during slowstart.

    s->node.key = 256 * s->next + (16 * 255 + s->rem - s->w) / 16;

    /* check for overflows */
    if ((int)s->node.key < 0)
      printf(" OV: srv=%p w=%d rem=%d next=%d key=%d", s, s->w, s->rem, s->next,
             s->node.key);
...
static inline void get_srv_init(struct srv *s) { s->next = s->rem = 0; }

/* prepares a server when extracting it from the next tree */
static inline void get_srv_next(struct srv *s) { s->next += sw; }

/* prepares a server when extracting it from the next tree */
static inline void get_srv_down(struct srv *s) { s->next = p; }
...
main(int argc, char **argv) {
  ...
  while (1) {
    ...
    printf("s=%02d v=%04d w=%03d n=%03d r=%03d ", s->num, s->node.key, s->w,
           s->next, s->rem);
    ...
    if (s->next >= sw * 2) {
      ...
      // s->node.key = sw * s->next + s->rem;  /// seems best (check with
      // filltab15) !
  ...
  next_iteration:
    ...
    if (/*conns == 30*/ /**/ random() % 100 == 0 /**/) {
      ...
      if (s->tree == init_tree) {
        ...
        get_srv(s);
        printf(" -- chgwght3(%d): %d->%d, n=%d, sw=%d, nsw=%d --", s->num, s->w,
               w, s->next, sw, nsw);
