...
uint32_t hash_id(uint32_t a) { return a; }
...
void dump_hash_results(char *name, int counts[NSERV][NSERV]) {
  ...
  for (nsrv = 0; nsrv < NSERV; nsrv++) {
    ...
    for (srv = 0; srv <= nsrv; srv++) {
      err = 100.0 * (counts[nsrv][srv] - (double)counts[0][0] / (nsrv + 1)) /
            (double)counts[0][0];
      // printf("%6d ", counts[nsrv][srv]);
      printf("% 3.1f%%%c ", err,
             counts[nsrv][srv]
                 ? ' '
                 : '*'); /* display '*' when a server is never selected */
      err = fabs(err);
      total_err += err;
      if (err > max_err)
        max_err = err;
...
int main() {
  ...
  for (nr = 0; nr < 0x10; nr++) {
     ...
     */
    count_hash_results(hash_bj6(address & mask) + (address & mask),
                       counts_bj6x); // 1.07s / 100M
    count_hash_results(hash_bj7(address & mask) + (address & mask),
                       counts_bj7x); // 1.20s / 100M
  }
